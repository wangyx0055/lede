...
VOID dump_rxinfo(RTMP_ADAPTER *pAd, RXINFO_STRUC *pRxInfo)
{
	...
	if (pAd->chipCap.hif_type == HIF_RLT)
		dump_rlt_rxinfo(pAd, pRxInfo);
#endif /* RLT_MAC */

#ifdef RTMP_MAC
	if (pAd->chipCap.hif_type == HIF_RTMP) {
		dump_rtmp_rxinfo(pAd, pRxInfo);
	}
#endif /* RTMP_MAC */
#else
...
VOID dump_txinfo(RTMP_ADAPTER *pAd, TXINFO_STRUC *pTxInfo)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TxInfo Fields:\n"));
...
VOID dump_tmac_info(RTMP_ADAPTER *pAd, UCHAR *tmac_info)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TxWI Fields:\n"));
#ifdef RLT_MAC
	if (pAd->chipCap.hif_type == HIF_RLT)
...
VOID dump_rmac_info(RTMP_ADAPTER *pAd, UCHAR *rmac_info)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("RxWI Fields:\n"));
#ifdef RLT_MAC
	if (pAd->chipCap.hif_type == HIF_RLT)
...
#ifdef RLT_MAC
	if (pAd->chipCap.hif_type == HIF_RLT) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): TBD for this function!\n", __FUNCTION__));
	}
#endif /* RLT_MAC */
...
INT rtmp_mac_fifo_stat_update(RTMP_ADAPTER *pAd)
{
	...
	/* ignore NoACK and MGMT frame use 0xFF as WCID */
		...
		if ((pEntry->HTPhyMode.field.MCS == 0) && (pid == 15))
		{
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():SucMCS(%d) != TxMCS(%d)\n", 
			...
			{			
				...
				IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
				{
					if(IS_ENTRY_TDLS(pEntry))
					{
						...
						{
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TDLS: TxFail >= 15 LinkTearDown !!!\n"));
				...
				if(pEntry->PsMode == PWR_ACTIVE)
				{
					...
					{ 
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Entry %02x:%02x:%02x:%02x:%02x:%02x Blocked!! (Fail Cnt = %d)\n",
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("(%d): reTry %d , (TxMCS = %d, SuccessRate = %d)\n", 
...
/*==================================================================*/
	...
	if ((OneSecTransmitCount+Retry + Fail) > 0)
		TxErrorRatio = (( Retry + Fail) *100) / (OneSecTransmitCount+Retry + Fail);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("TX ERROR Rate = %ld %%, Retry = %ld, Fail = %ld, Total = %ld  \n",TxErrorRatio, Retry, Fail, (OneSecTransmitCount+Retry + Fail)));
	...
	IN HTTRANSMIT_SETTING *pTransmit)
{
	...
	*/
	...
	if (pMac)
	{
		...
		mpdu_density = TxBaDensityDown(pAd, mpdu_density);
	...
	if ((RTMP_TEST_MORE_FLAG(pAd, fASIC_CAP_MCS_LUT)) && 
		(WCID < 128) && 
		(pMac && pMac->bAutoTxRateSwitch == TRUE))
	{
		lut_en = TRUE;
	}
#endif	
	...
	{
#ifdef RLT_MAC
		if (pMac->TxSndgType == SNDG_TYPE_NDP  || pMac->TxSndgType == SNDG_TYPE_SOUNDING || pTxWI->TXWI_N.eTxBF)
#else
...
#endif
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : NDP Sounding and NDPSndRate = %d\n", __FUNCTION__, ndp_rate));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : BSSID[5] = %x, txwi_n->TxEAPId = %d\n", __FUNCTION__, pAd->CommonCfg.Bssid[5], TxEAPId_Cal));
		}
	}
...
VOID write_tmac_info_Data(RTMP_ADAPTER *pAd, UCHAR *buf, TX_BLK *pTxBlk)
{
	...
	*/
...
#ifdef BT_COEXISTENCE_SUPPORT
		basize = TxBaSizeDown(pAd, basize);
#endif /* BT_COEXISTENCE_SUPPORT */
	}
	...
	if (pTxBlk->TxSndgPkt == SNDG_TYPE_SOUNDING)
	{
		sounding = 1;
		eTxBf = 0;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ETxBF in %s(): sending normal sounding, eTxBF=%d\n",
	...
	else if (pTxBlk->TxSndgPkt == SNDG_TYPE_NDP)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ETxBF in %s(): NDP sounding, eTxBF=%d, ndp_rate = %d\n",
					__FUNCTION__, pTransmit->field.eTxBF, ndp_rate));
	}
	else
	{
#ifdef MFB_SUPPORT
	...
	if (pTxBlk->TxSndgPkt == SNDG_TYPE_NDP  || pTxBlk->TxSndgPkt == SNDG_TYPE_SOUNDING || eTxBf)
		stbc = 0;
#endif /* TXBF_SUPPORT */
...
/*	if (ACMR_IS_ENABLED(pAd))*/
	{
		...
		if (PhyModeMin != ACMR_PHY_NONE)
		{
			...
			if (ACMP_PacketPhyModeMCSCheck(
							...
							mcs) != ACM_RTN_OK)
			{
				phy_mode = RTMP_GET_PACKET_ACM_MIN_PHY_MODE(pTxBlk->pPacket);
				mcs = RTMP_GET_PACKET_ACM_MIN_PHY_MCS(pTxBlk->pPacket);

#ifdef DOT11_N_SUPPORT
				sgi = 0; /* force to use regular GI */
#endif /* DOT11_N_SUPPORT */
			}
		}
	}
...
#if 0
	/*
		To prevent complained by klocwork,
		I think it is safe to remove pMacEntry checking here.
		Snowpin @20131120
	*/
	if(ampdu && pMacEntry)
#else
...
#endif
	{
...
#ifdef BT_COEXISTENCE_SUPPORT
		{
			...
			basize = TxBaSizeDown(pAd, pAd->BATable.BAOriEntry[RABAOriIdx].BAWinSize);
			mpdu_density = TxBaDensityDown(pAd, pMacEntry->MpduDensity);
		}
#endif /* BT_COEXISTENCE_SUPPORT */
	}
#endif /* DOT11_N_SUPPORT */
...
#else
		if ((pMacEntry->mrqCnt > 0) && (pMacEntry->toTxMrq == 1))
#endif
		{
			eTxBf = ~(pTransmit->field.eTxBF);
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE,("ETxBF in AP_AMPDU_Frame_Tx(): invert eTxBF\n"));
		}
		else
#endif	/* MFB_SUPPORT */
			eTxBf = pTransmit->field.eTxBF;
...
INT rtmp_mac_sys_reset(RTMP_ADAPTER *pAd, BOOLEAN bHardReset)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():MACVersion[Ver:Rev=0x%08x]\n",
...
#ifdef RELEASE_EXCLUDE
		/* 
			For RT6352, MAC CR controlled BBP_RESET is not work, just use 
			BBP CR directly reset BBP and shall not touch MAC CR for BBP RESET
		*/
#endif /* RELEASE_EXCLUDE */
		if (IS_RT6352(pAd)) {
			UCHAR BbpReg;
	...
	if (pAd->chipCap.hif_type == HIF_RTMP) {
#ifdef RTMP_MAC_USB
	// TODO: why USB use 0x3 and not adaptive change by bHardReset or not??
		mac_val = 0x03;
#endif /* RTMP_MAC_USB */
...
#ifdef RELEASE_EXCLUDE
	/* 
		For rt2860E (((pAd->MACVersion & 0xffff) != 0x0101)) and after, 
		init TXOP_CTRL_CFG with 0x583f. This is for extension channel overlapping IOT.
	*/
#endif /* RELEASE_EXCLUDE */
