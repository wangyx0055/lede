...
/*
    ...
    ==========================================================================
 */
VOID APMlmeDynamicTxRateSwitching(MP_ADAPTER *pAd)
{
    PUCHAR pTable;
    ...
    if (RTMP_SA_WORK_ON(pAd))
    {
            ...
            if (RTMP_SA_WORK_ON(pAd))
            {
                ...
                if ((pEntry == pTrainEntry->pMacEntry) && (pTrainEntry->trainStage != SA_INVALID_STAGE))
                {
                    ...
                    else
                    {
                        ...
                        */
                        if (pAd->SoftAP.ApCfg.ApQuickResponeForRateUpTimerRunning == FALSE)
        ...
        */
        ...
        if (pPort->CommonCfg.DebugFlags & DBF_SHOW_RA_LOG)
...
{
    PMP_ADAPTER         pAd = (PMP_ADAPTER)FunctionContext;
    PUCHAR                  pTable;
    UCHAR                   TableSize = 0;
    UCHAR                   CurrRateIdx;
    ...
    for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++)
    {
...
#endif
        {
            ...
            */
            ...
            if (IS_RT3290(pAd) &&
                ((AccuTxTotalCnt > 150) || (pAd->HwCfg.AntennaDiversityState == 1)) &&
        ...
        if (TxTotalCnt)
        {
            ...
            */
            ...
            if (0) //IS_RT3290(pAd) &&  ((AccuTxTotalCnt > 150) || (pAd->HwCfg.AntennaDiversityInfo.AntennaDiversityState == 1)) && (pPort->BBPCurrentBW == BW_40))
            {
                WLAN_FUN_CTRL_STRUC WlanFunCtrl = {.word = 0};
    ...
    IN PVOID SystemSpecific3) 
{
    ...
    McsDown(pAd, CurrRateIdx, pCurrTxRate, &UpRateIdx, &DownRateIdx);
...
#endif
        {
            ...
            pAd->Counter.MTKCounters.OneSecTxRetryOkCount += TxRetransmit;
            pAd->Counter.MTKCounters.OneSecTxNoRetryOkCount += TxSuccess;
            pAd->Counter.MTKCounters.OneSecTxFailCount += TxFailCount;
        ...
        else
        {
            ...
            if (pEntry->wcid >= 1 && pEntry->wcid <= 8)
            {
                ...
                if (HwTxCnt)
                    ...
                    DBGPRINT(RT_DEBUG_INFO, ("%s():TxErrRatio(wcid:%d, MCS:%d, Hw:0x%x-0x%x, Sw:0x%x-%x)\n", 
                        __FUNCTION__, pEntry->wcid, pEntry->HTPhyMode.field.MCS, 
                        HwTxCnt, HwErrRatio, TxTotalCnt, TxErrorRatio));
