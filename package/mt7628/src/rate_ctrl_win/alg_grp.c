...
*/
VOID MlmeSetMcsGroup(MP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
    ...
    {
        if ((pPort->CommonCfg.TxStream == 2) && (pEntry->SupportVHTMCS[10] == 0x1))
            pEntry->mcsGroup = 2;
        else
            pEntry->mcsGroup = 1;
    }
    ...
    if ((pEntry->HTCapability.MCSSet[2] == 0xff) && (pPort->CommonCfg.TxStream == 3))
        pEntry->mcsGroup = 3;
     else
    ...
    IN RTMP_RA_GRP_TB *pCurrTxRate)
{
    ...
    while (1)
    {
        ...
        DBGPRINT(RT_DEBUG_TRACE, ("MlmeSelectUpRate : grp_cnt %d upMcs1 %d upMcs2 %d\n", grp_cnt, pCurrTxRate->upMcs1, pCurrTxRate->upMcs2));

        DBGPRINT(RT_DEBUG_TRACE, ("MlmeSelectUpRate : pEntry->MaxPhyCfg.Default.BW = %d, pEntry->MaxPhyCfg.Default.ShortGI = %d\n", pEntry->MaxPhyCfg.Default.BW, pEntry->MaxPhyCfg.Default.ShortGI));

        if(pEntry->pTable == NULL)
        {
            DbgPrint("RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR\n");
            break;
        }
        ...
        if (PTX_RA_GRP_ENTRY(pEntry->pTable, UpRateIdx)->CurrMCS == 32)
        {
            ...
            if ((pEntry->HTCapability.MCSSet[4] & 0x1)
                ...
                && (pPort->CommonCfg.DebugFlags & DBF_ENABLE_HT_DUP)
#endif /* DBG_CTRL_SUPPORT */
            )
                mcs32Supported = 1;

#ifdef DBG_CTRL_SUPPORT
            if ((pPort->CommonCfg.DebugFlags & DBF_DISABLE_20MHZ_MCS0)==0)
                mcs0Fallback = 1;
#endif /* DBG_CTRL_SUPPORT */

            if (pEntry->MaxPhyCfg.Default.BW != BW_40 ||
        ...
        if (PTX_RA_GRP_ENTRY(pEntry->pTable, UpRateIdx)->ShortGI &&
            !pEntry->MaxPhyCfg.Default.ShortGI)
        {
    ...
    IN UCHAR CurrRateIdx)
{
    ...
    while (1)
    {
...
#ifdef DBG_CTRL_SUPPORT
                || (pPort->CommonCfg.DebugFlags & DBF_ENABLE_CCK_5G)
#endif /* DBG_CTRL_SUPPORT */
                 )
#ifdef DBG_CTRL_SUPPORT
                && ((pPort->CommonCfg.DebugFlags & DBF_DISABLE_CCK)==0)
#endif /* DBG_CTRL_SUPPORT */
            )
                break;
        }
        else if (pDownRate->CurrMCS == MCS_32)
        {
            ...
            if ((pEntry->MaxPhyCfg.Default.BW == BW_40 && pPort->BBPCurrentBW == BW_40)
#ifdef DOT11_VHT_AC
                || (pEntry->MaxPhyCfg.Default.BW == BW_80 && pPort->BBPCurrentBW == BW_80)
#endif /* DOT11_VHT_AC */
            )
                valid_mcs32 = TRUE;
            ...
            if (valid_mcs32
#ifdef DBG_CTRL_SUPPORT
                && ((pPort->CommonCfg.DebugFlags & DBF_DISABLE_20MHZ_MCS0)==0)
#endif /* DBG_CTRL_SUPPORT */
            )
                break;
    ...
    OUT CHAR mcs[])
{
    ...
    for (idx = 0; idx < RATE_TABLE_SIZE(pTable); idx++)
    {
        ...
        if ((pCurrTxRate->CurrMCS >= 8 && pPort->CommonCfg.TxStream < 2) ||
            (pCurrTxRate->CurrMCS >= 16 && pPort->CommonCfg.TxStream < 3))
            continue;
    ...
    if (pPort->CommonCfg.DebugFlags & DBF_INIT_MCS_DIS1)
        mcs[23] = mcs[15] = mcs[7] = mcs[22] = mcs[14] = mcs[6] = 0;
#endif /* DBG_CTRL_SUPPORT */

#if defined(RT2883) || defined(RT3883)
    /*  Debug Option: Limit PHY Rate if enabled */
    if (pPort->CommonCfg.PhyRateLimit != 0)
    {
        USHORT  phyRateLimit20;
    
        phyRateLimit20 = pEntry->HTPhyMode.field.BW==BW_20 ? pPort->CommonCfg.PhyRateLimit : pPort->CommonCfg.PhyRateLimit * 13 / 27;
...
*/
VOID MlmeNewTxRate(MP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
    RTMP_RA_LEGACY_TB *pNextTxRate;
    UCHAR *pTable;
    RTMP_TX_RATE tx_rate;

    DBGPRINT(RT_DEBUG_TRACE, ("MlmeNewTxRate 1 : -->"));
    ...
    DBGPRINT(RT_DEBUG_TRACE, ("MlmeNewTxRate 2 : -->pEntry->CurrTxRateIndex = %d\n", pEntry->CurrTxRateIndex));
    ...
    {
        DBGPRINT(RT_DEBUG_TRACE, ("%s :ItemNo=%d, TrainUp=%d, TrainDown = %d\n", __FUNCTION__, pNextTxRate->ItemNo, pNextTxRate->TrainUp, pNextTxRate->TrainDown));
        DBGPRINT(RT_DEBUG_TRACE, ("%s :Mode=%d, CurrMCS=%d, \n", __FUNCTION__, pNextTxRate->Mode, pNextTxRate->CurrMCS));
        DBGPRINT(RT_DEBUG_TRACE, ("%s :BW=%d, STBC=%d, ShortGI=%d\n", __FUNCTION__, pNextTxRate->BW, pNextTxRate->STBC, pNextTxRate->ShortGI));
        MlmeSetTxRate(pAd, pEntry, (PRTMP_TX_RATE_SWITCH)pNextTxRate, pTable);
    }
    ...
    pAd->LastTxRate = pEntry->TxPhyCfg.DoubleWord;

#ifdef STREAM_MODE_SUPPORT
    /*  Enable/disable stream mode based on MCS */
    if (pPort->CommonCfg.StreamMode!=0 &&
        pEntry->StreamModeMACReg!=0)
    {
        ...
        mcsDisable = (pEntry->HTPhyMode.field.MCS < 16) &&
                (pPort->CommonCfg.StreamModeMCS & (1<<pEntry->HTPhyMode.field.MCS))==0 &&
...
UCHAR get_rate_idx_by_rate(MP_ADAPTER *pAd, UCHAR *rate_tb,  USHORT rate)
{
    ...
    IN CHAR     RssiOffset)
{
    ...
    if (pPort->CommonCfg.DebugFlags & DBF_INIT_MCS_MARGIN)
        RssiOffset += 6;
#endif /* DBG_CTRL_SUPPORT */
                    ...
                    || pTable == RateTableVht2S_MCS7)
    {
        ...
        {
            DBGPRINT_RAW(RT_DEBUG_INFO, ("%s: GRP: 2*2, RssiOffset=%d\n", __FUNCTION__, RssiOffset));
            ...
            DBGPRINT_RAW(RT_DEBUG_INFO, ("%s: GRP: 1*1, RssiOffset=%d\n", __FUNCTION__, RssiOffset));
    ...
    IN ULONG TxErrorRatio)
{
    ...
    else if ((pCurrTxRate->Mode==MODE_HTMIX) || (pCurrTxRate->Mode==MODE_HTGREENFIELD))
    {
        if (pEntry->MaxPhyCfg.Default.BW==BW_40 
#ifdef DBG_CTRL_SUPPORT
            || (pPort->CommonCfg.DebugFlags & DBF_FORCE_40MHZ)
#endif /* DBG_CTRL_SUPPORT */
        )
            estTP *= 2;
    }
...
PMP_PORT pPort = pAd->PortList[FXXK_PORT_0];
    ...
    */
    ...
    DBGPRINT(RT_DEBUG_WARN, ("--Hybrid: low/high/new TP=%ld/%ld/%ld old/new=%ld/%ld--",
            pPort->CommonCfg.TrainUpLowThrd*pEntry->LastTxOkCount/100,
            pPort->CommonCfg.TrainUpHighThrd*pEntry->LastTxOkCount/100,
            NewTxOkCount,
            oldTP/100, newTP/100));
#endif

#if 0
    if (100*NewTxOkCount < pPort->CommonCfg.TrainUpLowThrd*pEntry->LastTxOkCount)
        return TRUE;

    if (100*NewTxOkCount > pPort->CommonCfg.TrainUpHighThrd*pEntry->LastTxOkCount)
        return FALSE;
    ...
    IN ULONG            TxErrorRatio)
{
...
#ifdef DBG_CTRL_SUPPORT
            && ((pPort->CommonCfg.DebugFlags & DBF_NO_BF_AWARE_RA)==0)
#endif /* DBG_CTRL_SUPPORT */
        )
        {
            invertTxBf = TRUE;
...
#ifdef DBG_CTRL_SUPPORT
        if ((pPort->CommonCfg.DebugFlags & DBF_NO_BF_AWARE_RA)==0)
#endif /* DBG_CTRL_SUPPORT */
        {
    ...
    if ((pEntry->LastSecTxRateChangeAction != RATE_NO_CHANGE)
#ifdef DBG_CTRL_SUPPORT
        || (pPort->CommonCfg.DebugFlags & DBF_FORCE_QUICK_DRS)
#endif /* DBG_CTRL_SUPPORT */
    )
    {
        if (pEntry->LastSecTxRateChangeAction!=RATE_NO_CHANGE)
    ...
    else
    {
        ...
        IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
        {
            if (!pAd->SoftAP.ApCfg.ApQuickResponeForRateUpTimerRunning)
            {
                RTMPSetTimer(&pAd->SoftAP.ApCfg.ApQuickResponeForRateUpTimer, pAd->ra_fast_interval);
                pAd->SoftAP.ApCfg.ApQuickResponeForRateUpTimerRunning = TRUE;
            }
        }
#endif /*  CONFIG_AP_SUPPORT */
#ifdef CONFIG_STA_SUPPORT
        ...
        if (pAd->OpMode == OPMODE_STA)
        {
            ...
            {
                RTMPSetTimer(pAd, &pAd->StaCfg.StaQuickResponeForRateUpTimer, (ULONG)pAd->ra_fast_interval);
...
//  UCHAR       index;
    ...
    )
    {
        ...
        if (pAd->OpMode == OPMODE_STA)
        {
            if (!pAd->StaCfg.StaQuickResponeForRateUpTimerRunning)
            {
                RTMPSetTimer(pAd, &pAd->StaCfg.y, (ULONG)pAd->ra_fast_interval);
                pAd->StaCfg.StaQuickResponeForRateUpTimerRunning = TRUE;
            }
        }
#endif /*  CONFIG_STA_SUPPORT */
    ...
    IN UINT idx) 
{
    ...
    UINT32 TxTotalCnt;
    PMP_PORT pPort = pAd->MacTabPoolArray[idx].pPort;

    FUNC_ENTER;

#if (OS_WIN==1)
    pEntry = &pAd->MacTabPoolArray[idx];//MlmeSyncMacTabMatchedWlanIdx(pPort, (USHORT)idx); /* point to information of the individual station */
    ...
    if ((pPort->CommonCfg.DebugFlags & DBF_SHOW_RA_LOG) || (pPort->CommonCfg.DebugFlags & DBF_DBQ_RA_LOG))
    ...
    if ( (TxTotalCnt <= 15) && (pEntry->LastSecTxRateChangeAction == RATE_UP))
    {
        ...
        DBGPRINT(RT_DEBUG_TRACE, ("   QuickDRS: TxTotalCnt <= 15, back to original rate \n"));
...
#ifdef DBG_CTRL_SUPPORT
        && (pPort->CommonCfg.DebugFlags & DBF_FORCE_QUICK_DRS)==0
#endif /* DBG_CTRL_SUPPORT */
    )
...
*/
    ...
    if (pEntry->LastSecTxRateChangeAction == RATE_UP)
    {
        ...
        */
        if (pPort->CommonCfg.TrainUpRule == 0)
        {
            useOldRate = (pEntry->LastTxOkCount + 2) >= OneSecTxNoRetryOKRationCount;
        }
        else if (pPort->CommonCfg.TrainUpRule==2 && Rssi<=pPort->CommonCfg.TrainUpRuleRSSI)
        {
            useOldRate = MlmeRAHybridRule(pAd, pEntry, pCurrTxRate, OneSecTxNoRetryOKRationCount, TxErrorRatio);
        }
        else if (pPort->CommonCfg.TrainUpRule==3 && Rssi<=pPort->CommonCfg.TrainUpRuleRSSI)
        {
    ...
    else if (pEntry->LastSecTxRateChangeAction == RATE_DOWN)
    {
        if ((TxErrorRatio >= 50) || (TxErrorRatio >= TrainDown)) /* there will be train down again */
        {
            ...
            DBGPRINT(RT_DEBUG_TRACE,("   QuickDRS: (Down) direct train down (TxErrorRatio >= TrainDown)\n"));
        }
        else if ((pEntry->LastTxOkCount + 2) >= OneSecTxNoRetryOKRationCount)
        {
            MlmeRestoreLastRate(pEntry);
            DBGPRINT(RT_DEBUG_TRACE,("   QuickDRS: (Down) bad tx ok count (L:%ld, C:%ld)\n", pEntry->LastTxOkCount, OneSecTxNoRetryOKRationCount));
        }
        else
        {
            MlmeSetMcsGroup(pAd, pEntry);
            DBGPRINT(RT_DEBUG_TRACE,("   QuickDRS: (Down) keep rate-down (L:%ld, C:%ld)\n", pEntry->LastTxOkCount, OneSecTxNoRetryOKRationCount));
        }
    }
...
VOID DynamicTxRateSwitchingAdaptMT(MP_ADAPTER *pAd, UINT i)
{
    PUCHAR pTable;
    UCHAR UpRateIdx =0, DownRateIdx = 0, CurrRateIdx = 0, TrainUp, TrainDown;
    UINT32 Rate1TxCnt, Rate1SuccessCnt, Rate1FailCount;
    UINT32 TxTotalCnt;
    MAC_TABLE_ENTRY *pEntry;
    RTMP_RA_GRP_TB *pCurrTxRate;
    ...
    DBGPRINT(RT_DEBUG_TRACE, ("TxTotalCnt = %d\n", TxTotalCnt));
    DBGPRINT(RT_DEBUG_TRACE, ("Rate1 Tx Cnt = %d\n", TxInfo.Rate1TxCnt));
    DBGPRINT(RT_DEBUG_TRACE, ("Rate2 Tx Cnt = %d\n", TxInfo.Rate2TxCnt));
    DBGPRINT(RT_DEBUG_TRACE, ("Rate3 Tx Cnt = %d\n", TxInfo.Rate3TxCnt));
    DBGPRINT(RT_DEBUG_TRACE, ("Rate4 Tx Cnt = %d\n", TxInfo.Rate4TxCnt));
    DBGPRINT(RT_DEBUG_TRACE, ("Rate5 Tx Cnt = %d\n", TxInfo.Rate5TxCnt));
    DBGPRINT(RT_DEBUG_TRACE, ("Rate1 fail = %d\n", TxInfo.Rate1FailCnt));
    ...
    DBGPRINT(RT_DEBUG_TRACE, ("Rate1ErrorRatio = %d\n", Rate1ErrorRatio));

    HwAggRateIndex = TxInfo.RateIndex;

    DBGPRINT(RT_DEBUG_TRACE, ("HwAggRateIndex = %d\n", HwAggRateIndex));
    ...
    DBGPRINT(RT_DEBUG_TRACE, ("Average PER %d, Cur %x, Up %x, Dn %x\n", Rate1ErrorRatio
    ...
    if (TxTotalCnt <= 15)
    {
        ...
        if (pEntry->lowTrafficCount >= pPort->CommonCfg.lowTrafficThrd)
        {
            UCHAR TxRateIdx;
            CHAR mcs[24];
            CHAR RssiOffset = 0;
    ...
    IN UINT idx) 
{
    ...
    pEntry = &pPort->MacTab.Content[idx];
    ...
    else
    {
        ...
        if ((pAd->chipCap.hif_type == HIF_RTMP) || (pAd->chipCap.hif_type == HIF_RLT)) {
            if ((pEntry->wcid >= 1) && (pEntry->wcid <= 8))
            {
                ...
                DBGPRINT(RT_DEBUG_INFO,("%s()=>Wcid:%d, MCS:%d, TxErrRation(Hw:0x%lx-0x%lx, Sw:0x%lx-%lx)\n", 
    ...
    if (TxCnt <= 15)
    {
        /*  Go back to the original rate */
        MlmeRestoreLastRate(pEntry);
        DBGPRINT(RT_DEBUG_INFO, ("   QuickDRS: TxTotalCnt <= 15, back to original rate \n"));
    ...
    */
    ...
    else if (pEntry->LastSecTxRateChangeAction == RATE_DOWN)
    {
        ...
        {
            MlmeSetMcsGroup(pAd, pEntry);
            DBGPRINT(RT_DEBUG_INFO,("   QuickDRS: (Down) direct train down (TxErrorRatio >= TrainDown)\n"));
        }
        else if ((pEntry->LastTxOkCount + 2) >= OneSecTxNoRetryOKRationCount)
        {
            MlmeRestoreLastRate(pEntry);
            DBGPRINT(RT_DEBUG_INFO,("   QuickDRS: (Down) bad tx ok count (L:%ld, C:%ld)\n", pEntry->LastTxOkCount, OneSecTxNoRetryOKRationCount));
        }
        else
        {
            MlmeSetMcsGroup(pAd, pEntry);
            DBGPRINT(RT_DEBUG_INFO,("   QuickDRS: (Down) keep rate-down (L:%ld, C:%ld)\n", pEntry->LastTxOkCount, OneSecTxNoRetryOKRationCount));
        }
    }
...
/*
    ...
    ==========================================================================
 */
VOID APMlmeDynamicTxRateSwitchingAdapt(MP_ADAPTER *pAd, UINT i)
{
    PUCHAR pTable;
    ...
    pEntry = &pPort->MacTab.Content[i]; /* point to information of the individual station */
    pTable = pEntry->pTable;
        ...
        if ((pAd->chipCap.hif_type == HIF_RTMP) || (pAd->chipCap.hif_type == HIF_RLT)) {
            if (pEntry->wcid >= 1 && pEntry->wcid <= 8)
            {
                ...
                if (HwTxCnt)
                    ...
                    HwErrRatio = 0;
                DBGPRINT(RT_DEBUG_INFO,("%s()=>Wcid:%d, MCS:%d, TxErrRatio(Hw:0x%lx-0x%lx, Sw:0x%lx-%lx)\n", 
                        __FUNCTION__, pEntry->wcid, pEntry->HTPhyMode.field.MCS, 
                        HwTxCnt, HwErrRatio, TxTotalCnt, TxErrorRatio));
    ...
    DBGPRINT(RT_DEBUG_INFO, ("Average PER %d, Cur %x, Up %x, Dn %x\n", TxErrorRatio,
                                CurrRateIdx, UpRateIdx, DownRateIdx));
    ...
    */
    ...
    if ((pPort->CommonCfg.FixedRate != -1) && (pPort->CommonCfg.FixedRate < RATE_TABLE_SIZE(pTable)) )
    {
        pEntry->CurrTxRateIndex = pPort->CommonCfg.FixedRate;
        MlmeNewTxRate(pAd, pEntry);
    ...
    if (TxTotalCnt <= 15)
    {
        ...
        if (pPort->CommonCfg.DebugFlags & DBF_NO_BF_AWARE_RA)
            eTxBFProbing(pAd, pEntry);
#else
        if (pAd->chipCap.FlgHwTxBfCap)                                                                                          
             eTxBFProbing(pAd, pEntry); 
    ...
    IN CHAR  Rssi)
{
...
#ifdef RELEASE_EXCLUDE
    DBGPRINT(RT_DEBUG_INFO,("   QuickDRS: TxSuccess=%ld, TxRetransmit=%ld, TxFail=%ld, RSSI=%d\n",
    ...
    */
    if (TxTotalCnt <= 12)
    {
        /* Go back to the original rate */
        MlmeRestoreLastRate(pEntry);
        DBGPRINT(RT_DEBUG_INFO,("   QuickDRS: TxTotalCnt <= 12, back to original rate \n"));
    ...
    if (pEntry->LastSecTxRateChangeAction == RATE_UP)
    {
        ...
        */
        ...
        if (useOldRate)
        {
            ...
            MlmeRestoreLastRate(pEntry);
            DBGPRINT(RT_DEBUG_INFO,("   QuickDRS: (Up) bad tx ok count (L:%ld, C:%ld)\n", pEntry->LastTxOkCount, OneSecTxNoRetryOKRationCount));
        }
        else
        {
            RTMP_RA_GRP_TB *pLastTxRate = PTX_RA_GRP_ENTRY(pTable, pEntry->lastRateIdx);
            ...
            if (pEntry->mcsGroup == 0)
                MlmeSetMcsGroup(pAd, pEntry);
            DBGPRINT(RT_DEBUG_INFO,("   QuickDRS: (Up) keep rate-up (L:%ld, C:%ld)\n", pEntry->LastTxOkCount, OneSecTxNoRetryOKRationCount));
        }
    }
    else if (pEntry->LastSecTxRateChangeAction == RATE_DOWN)
    {
    ...
    IN ULONG TxFailCount)
{
    ...
    if ((pPort->MacTab.Size == 1) || (IS_ENTRY_DLS(pEntry)))
    {
...
*/
INT Set_RateTable_Proc(MP_ADAPTER *pAd, RTMP_STRING *arg)
{
    ...
    {
        pEntry = &pPort->MacTab.Content[i];
        if (pEntry->ValidAsCLI == TRUE && pEntry->Sst==SST_ASSOC)
            break;
    }
    ...
    /* Get rate index */
    itemNo = (int)simple_strtol(arg, &arg, 10);
    ...
    IN  RTMP_STRING *arg)
{
    ...
    for (i=0; i<MAX_LEN_OF_MAC_TABLE; i++){
        pPort->MacTab.Content[i].perThrdAdj = (BOOLEAN)simple_strtol(arg, 0, 10);
    }
    return TRUE;    
}
...
INT Set_LowTrafficThrd_Proc(
    ...
    IN  RTMP_STRING *arg)
{
    pPort->CommonCfg.lowTrafficThrd = (USHORT)simple_strtol(arg, 0, 10);
...
INT Set_TrainUpRule_Proc(
    ...
    IN  RTMP_STRING *arg)
{
    pPort->CommonCfg.TrainUpRule = (BOOLEAN)simple_strtol(arg, 0, 10);
...
INT Set_TrainUpRuleRSSI_Proc(
    ...
    IN  RTMP_STRING *arg)
{
    pPort->CommonCfg.TrainUpRuleRSSI = (SHORT)simple_strtol(arg, 0, 10);
...
INT Set_TrainUpLowThrd_Proc(
    ...
    IN  RTMP_STRING *arg)
{
    pPort->CommonCfg.TrainUpLowThrd = (USHORT)simple_strtol(arg, 0, 10);
...
INT Set_TrainUpHighThrd_Proc(
    ...
    IN  RTMP_STRING *arg)
{
    pPort->CommonCfg.TrainUpHighThrd = (USHORT)simple_strtol(arg, 0, 10);
