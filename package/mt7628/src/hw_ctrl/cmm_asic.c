...
VOID AsicNotSupportFunc(RTMP_ADAPTER *pAd, const RTMP_STRING *caller)
{
	...
	DBGPRINT(RT_DEBUG_OFF, ("%s(): NotSupportedFunc for this arch(%s)!\n",
				caller, str));
}
	...
	IN UCHAR channel) 
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s: SlotTime =%d us, SIFS Time = %d us, RIFS Time = %d, EIFS Time = %d\n",
...
VOID AsicWOWSendNullFrame(RTMP_ADAPTER *pAd, UCHAR TxRate, BOOLEAN bQosNull)
{
	...
	if (pAd->chipCap.hif_type == HIF_RTMP ||pAd->chipCap.hif_type == HIF_RLT)
	{
		RtAsicWOWSendNullFrame(pAd, TxRate, bQosNull);
		return;
	}
#endif
