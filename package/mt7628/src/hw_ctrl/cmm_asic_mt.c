	...
	IN BOOLEAN bNonGFExist)
{
	...
	if ((SetMask & ALLN_SETPROTECT) != 0)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s():OperationMode = %d. \n",
...
#ifdef COMPOS_WIN
{
    PMP_PORT pPort = pAd->PortList[FXXK_PORT_0];
                    		pPort->CommonCfg.IOTestParm.bRTSLongProtOn = TRUE;
}
#else
...
#ifdef COMPOS_WIN
// TODO: windows use this, need to merge with MtAsicSetBssid()
VOID MtAsicSetMultiBssidEx(
    IN PMP_ADAPTER pAd,
    IN PUCHAR pBssid,
    IN UCHAR    BssidIndex)
{
    ULONG val = 0, accessCR0 = 0, accessCR1 = 0;

    switch(BssidIndex)
    {
        case 0:
            accessCR0 = RMAC_CB0R0;
            accessCR1 = RMAC_CB0R1;
            break;
        case 1:
            accessCR0 = RMAC_CB1R0;
            accessCR1 = RMAC_CB1R1;
            break;
        case 2:
            accessCR0 = RMAC_CB2R0;
            accessCR1 = RMAC_CB2R1;
            break;
        default:
            accessCR0 = 0;
            accessCR1 = 0;
    ...
    if (accessCR0 == 0 && accessCR1 == 0)
        return;

    DBGPRINT(RT_DEBUG_TRACE, ("%s(): Set BSSID=%02x:%02x:%02x:%02x:%02x:%02x\n",
                __FUNCTION__, pBssid[0], pBssid[1], pBssid[2], pBssid[3], pBssid[4], pBssid[5]));
    ...
    val = (UINT32)((pBssid[0]) |
                  ...
                  (UINT32)(pBssid[2] << 16) |
                  (UINT32)(pBssid[3] << 24));
    HW_IO_WRITE32(pAd, accessCR0, val);

    val = (UINT32)(pBssid[4]) | (UINT32)(pBssid[5] << 8) | (1 <<16);
    HW_IO_WRITE32(pAd, accessCR1, val);
    //HW_IO_WRITE32(pAd, RMAC_CB0R1, val);

    // If we enable BSSID0, we should not enable MBSS0, or the BSSID index will wrong
    //HW_IO_READ32(pAd, RMAC_ACBEN, &val);
    //val |= 0x1;
    //HW_IO_WRITE32(pAd, RMAC_ACBEN, val);
    ...
    IN UCHAR    BssidIndex)
{
    ...
    {
        case 0:
            accessCR0 = RMAC_OMA0R0;
            accessCR1 = RMAC_OMA0R1;
            break;
        case 1:
            accessCR0 = RMAC_OMA1R0;
            accessCR1 = RMAC_OMA1R1;
            break;
        case 2:
            accessCR0 = RMAC_OMA2R0;
            accessCR1 = RMAC_OMA2R1;
            break;
        default:
            accessCR0 = 0;
            accessCR1 = 0;
	...
	*/
	...
	else if (NumOfMacs <= 4)
	{
		if (pAd->CurrentAddress[5] % 4 != 0)
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("The 4-BSSID mode is enabled, the BSSID byte5 MUST be the multiple of 4\n"));
	...
	else if (NumOfMacs <= 8)
	{
		if (pAd->CurrentAddress[5] % 8 != 0)
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("The 8-BSSID mode is enabled, the BSSID byte5 MUST be the multiple of 8\n"));
...
#ifdef RELEASE_EXCLUDE
    /*
    *       in MT7628, repeater entry extend from 16 --> 24,
    *       the first 16 entries share the same arranging way with MT7603.
    *       the extra 8 entries, concatenate to the tail of the total 32 entries.
    *
    *       0---------------16--------------32--------------40--------------47
    *       |       16 cli  |       16 root |       8 cli   |       8 root  |
    *       -----------------------------------------------------------------
    */
...
INT MtAsicSetRxFilter(RTMP_ADAPTER *pAd)
{
    ...
    {
        HW_IO_READ32(pAd, RMAC_RFCR, &Value);
        ...
        HW_IO_WRITE32(pAd, RMAC_RFCR, Value);
    }
...
{
	UINT32 Value;
#if 0 /* unused for clearing build warnings */
	UINT32 rx_filter_flag = 0;
	UINT32 Value;

	/* enable RX of MAC block*/
	// TODO: shiang-usw, pAd->OpMode!
	if ((pAd->OpMode == OPMODE_AP)
#ifdef P2P_SUPPORT
		|| P2P_GO_ON(pAd)
#endif /* P2P_SUPPORT */
...
#ifdef MT7636
	/*WHQA_00017127*/
	if (MT_REV_GTE(pAd, MT76x6, MT76x6E1))
	{
		Value |= DROP_DUPLICATE;
	}
...
UINT32 MtAsicGetWmmParam(RTMP_ADAPTER *pAd, UINT32 ac, UINT32 type)
{
 ...
 INT MtAsicSetWmmParam(RTMP_ADAPTER *pAd, UINT ac, UINT type, UINT val)
 {
	 ...
	 if (addr && mask) {
		 ...
		 if (pAd->CommonCfg.bTXRX_RXV_ON)
			 MtAsicSetMacTxRx(pAd, ASIC_MAC_TXRX_RXV, FALSE);
		 else
			 MtAsicSetMacTxRx(pAd, ASIC_MAC_TXRX, FALSE);

		 MAC_IO_READ32(pAd, addr, &cr_val);
		 cr_val &= (~mask);
		 cr_val |= (val << shift);
		 MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(%d): ac=%d, type=%d, val=%x, addr=%x, cr_val=%x\n",
					 __FUNCTION__, __LINE__, ac, type, val, addr, cr_val));

		 MAC_IO_WRITE32(pAd, addr, cr_val);

		 /*enable txrx first*/
		 if (pAd->CommonCfg.bTXRX_RXV_ON)
			 MtAsicSetMacTxRx(pAd, ASIC_MAC_TXRX_RXV, TRUE);
		 else
 ...
 static inline VOID wmm_param_set(RTMP_ADAPTER *pAd, UINT32 addr, UINT32 mask, UINT32 shift,UINT32 val)
 {
	 UINT32 cr_val;
	 if (addr && mask)
	 {
		 ...
		 cr_val &= (~mask);
		 cr_val |= (val << shift);
		 MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(%d): val=%x, addr=%x, cr_val=%x\n",
					 __FUNCTION__, __LINE__, val, addr, cr_val));
 ...
 {
	 UINT32 addr = 0, mask = 0, shift,val,ac;
	 ...
	 for(ac=0;ac <=WMM_PARAM_AC_3;ac++)
	 {

		 addr = wmm_txop_mask[ac].Address;
		 mask = wmm_txop_mask[ac].Mask;
		 shift = wmm_txop_mask[ac].Shift;
		 val = pEdcaParm->Txop[ac];

		 wmm_param_set(pAd,addr,mask,shift,val);

		 addr = wmm_aifsn_mask[ac].Address;
		 mask = wmm_aifsn_mask[ac].Mask;
		 shift = wmm_aifsn_mask[ac].Shift;
		 val = pEdcaParm->Aifsn[ac];
 ...
 */
#ifdef COMPOS_WIN
// TODO: shiang-usw, remove this and sync back to unify one!
VOID MtAsicSetEdcaParm(MP_ADAPTER *pAd, MP_PORT *pPort, PEDCA_PARM pEdcaParm, BOOLEAN bBaseChannel)
{
    //int i;
    ...
    {
        // Base Channel :
        // 1. Channel on which the TDLS peer STA is associated with an AP.
        // 2. Channel on which any STA is associated/joined with an AP/Adhoc network.
        // In the base channel, we use the flag of fOP_STATUS_WMM_INUSED to check if enable WMM.
        if (bBaseChannel)
        {
            OPSTATUS_SET_FLAG(pPort, fOP_STATUS_WMM_INUSED);
        }
        ...
        if (pAd->OpMode == OPMODE_STA)
        {
            if (INFRA_ON(pAd))
            {
                PMAC_TABLE_ENTRY pEntry = MlmeSyncMacTabMatchedRoleType(pPort, ROLE_WLANIDX_BSSID);
 ...
 */
 ...
 VOID MtAsicSetSlotTime(RTMP_ADAPTER *pAd, UINT32 SlotTime, UINT32 SifsTime)
 {
	 ...
	 MAC_IO_READ32(pAd, TMAC_ICR, &Value);
	 Value &= ~ICR_SLOT_TIME_MASK;
	 Value |= ICR_SLOT_TIME(SlotTime);

	 Value &= ~ICR_SIFS_TIME_MASK;

	 Value |= ICR_SIFS_TIME(SifsTime);

	 Value &= ~ICR_RIFS_TIME_MASK;
	 Value |= ICR_RIFS_TIME(RIFS_TIME);

	 Value &= ~ICR_EIFS_TIME_MASK;
	 Value |= ICR_EIFS_TIME(EIFS_TIME);

	 /* Before Update Slot/SIFS/RIFS, wee need disable MAC TX */
	 MtAsicSetMacTxRx(pAd, ASIC_MAC_TX, FALSE);
	 MAC_IO_WRITE32(pAd, TMAC_ICR, Value);
	 MtAsicSetMacTxRx(pAd, ASIC_MAC_TX, TRUE);
 }
#else
VOID MtAsicSetSlotTime(RTMP_ADAPTER *pAd, UINT32 SlotTime, UINT32 SifsTime)
{
...
#ifdef COMPOS_WIN
VOID MtAsicTxCntUpdate(MP_ADAPTER *pAd, UCHAR wcid, MT_TX_COUNTER *pTxInfo)
{
    ...
    if ( pTxInfo->TxFailCount == 0 )
        pAd->Counter.MTKCounters.OneSecTxNoRetryOkCount += pTxInfo->TxCount;
    else
        pAd->Counter.MTKCounters.OneSecTxRetryOkCount += pTxInfo->TxCount;
    pAd->Counter.MTKCounters.OneSecTxFailCount += pTxInfo->TxFailCount;
...
VOID MtAsicRssiUpdate(MP_ADAPTER *pAd)
{
    union WTBL_2_DW13 wtbl_2_d13;
    MAC_TABLE_ENTRY *pEntry;
    //UINT16 i;
    //INT32 Rssi0 = 0, Rssi1 = 0, Rssi2 = 0;
    CHAR Rssi0 = 0, Rssi1 = 0, Rssi2 = 0;
    ...
    IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
    {
        if ( pAd->MacTab.Size == 0 )
        {
            ...
            pAd->SoftAP.ApCfg.RssiSample.AvgRssi[0] = Rssi0;
            pAd->SoftAP.ApCfg.RssiSample.AvgRssi[1] = Rssi1;
            pAd->SoftAP.ApCfg.RssiSample.AvgRssi[2] = Rssi2;

            pAd->SoftAP.ApCfg.RssiSample.LastRssi[0] = Rssi0;
            pAd->SoftAP.ApCfg.RssiSample.LastRssi[1] = Rssi1;
            pAd->SoftAP.ApCfg.RssiSample.LastRssi[2] = Rssi2;
        }
        else
        {
#ifdef COMPOS_WIN
            ...
            pAd->SoftAP.ApCfg.RssiSample.AvgRssi[0] = pAd->SoftAP.ApCfg.RssiSample.LastRssi[0] = TotalRssi0 / pPort->MacTab.Size;
            pAd->SoftAP.ApCfg.RssiSample.AvgRssi[1] = pAd->SoftAP.ApCfg.RssiSample.LastRssi[1] = TotalRssi1 / pPort->MacTab.Size;
            pAd->SoftAP.ApCfg.RssiSample.AvgRssi[2] = pAd->SoftAP.ApCfg.RssiSample.LastRssi[2] = TotalRssi2 / pPort->MacTab.Size;
        }
...
//  IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
    {
#ifdef COMPOS_WIN
        pEntry = MlmeSyncMacTabMatchedRoleType(pPort, ROLE_WLANIDX_BSSID);
        if(pEntry == NULL)
        {
            DBGPRINT(RT_DEBUG_TRACE,("%s(), Line:%d, Mac Entry is NULL\n", __FUNCTION__, __LINE__));
            return;
...
#endif
        Wtbl2RcpiGet(pAd, (UINT8)pEntry->wcid, &wtbl_2_d13);
...
VOID MtAsicMcsLutUpdate(MP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
    ...
    if (pEntry->TxPhyCfg.Default.ShortGI) {
    ...
    nss = get_nss_by_mcs((UINT8)pEntry->TxPhyCfg.Default.Mode, (UINT8)pEntry->TxPhyCfg.Default.MCS, (UINT8)pEntry->TxPhyCfg.Default.STBC);

#if 0
    if ( (pEntry->HTPhyMode.field.STBC == 0) || (pEntry->MmpsMode != MMPS_STATIC) ) {
    ...
    rate[0] = tx_rate_to_tmi_rate((UINT8)pEntry->TxPhyCfg.Default.Mode,
                                            (UINT8)pEntry->TxPhyCfg.Default.MCS,
                                            nss,
                                            (UINT8)pEntry->TxPhyCfg.Default.STBC,
                                            preamble);
    rate[0] &= 0xfff;
        ...
        for ( ucIndex = 1; ucIndex < 8 ; ucIndex++ )
        {
            if (ADAPT_RATE_TABLE(pEntry->pTable)) {
                ...
                if (pEntry->TxPhyCfg.Default.ShortGI)
                {
                    ...
                    if ( pCurrTxRate->CurrMCS == pEntry->TxPhyCfg.Default.MCS )
                    {
                ...
                stbc = (UCHAR)pEntry->TxPhyCfg.Default.STBC;
            } else {
            ...
            nss = get_nss_by_mcs(mode, mcs, (UINT8)pEntry->TxPhyCfg.Default.STBC);
...
#else
    Wtbl2RateTableUpdate(pAd, (UINT8)pEntry->wcid, wtbl_2_d9.word, rate);
#endif


    DBGPRINT(RT_DEBUG_TRACE, ("%s():WCID=%d, HTPhyMode=0x%x\n",
                __FUNCTION__, pEntry->wcid, pEntry->TxPhyCfg.DoubleWord));

    DBGPRINT(RT_DEBUG_TRACE, ("\tCurTxRateIdx=%d, Mode/BW/MCS/STBC/SGI=%d/%d/%d/%d/%d\n\n",
        pEntry->CurrTxRateIndex,
        pEntry->TxPhyCfg.Default.Mode,
        pEntry->TxPhyCfg.Default.BW,
        pEntry->TxPhyCfg.Default.MCS,
        pEntry->TxPhyCfg.Default.STBC,
        pEntry->TxPhyCfg.Default.ShortGI));
}
...
VOID MtAsicMcsLutUpdate(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
	...
	*/
	...
	// TODO: shiang-MT7603, dirty job, need to remove it after FW is ready or change as semaphore!
	do {
		MAC_IO_READ32(pAd, WTBL_OFF_WIUCR, &reg_val);
		if ((reg_val & 0x10000)  == 0)
			break;
		wait_cnt++;
		RtmpusecDelay(50);
	...
	reg_val = wtbl_2_d9.word;
	MAC_IO_WRITE32(pAd, WTBL_ON_RIUCR0, reg_val);
	reg_val = (rate[0] | (rate[1] << 12) | (rate[2] << 24));
	MAC_IO_WRITE32(pAd, WTBL_ON_RIUCR1, reg_val);
	reg_val = ((rate[2] >> 8) | (rate[3] << 4) | (rate[4] << 16) | (rate[5] << 28));
	MAC_IO_WRITE32(pAd, WTBL_ON_RIUCR2, reg_val);
	reg_val = ((rate[5] >> 4) | (rate[6] << 8) | (rate[7] << 20));
	MAC_IO_WRITE32(pAd, WTBL_ON_RIUCR3, reg_val);

	// TODO: shiang-MT7603, shall we also clear TxCnt/RxCnt/AdmCnt here??
	reg_val = (pEntry->wcid | (1 << 13));
	MAC_IO_WRITE32(pAd, WTBL_OFF_WIUCR, reg_val);
#else
...
VOID MtAsicUpdateBASession(RTMP_ADAPTER *pAd, UCHAR wcid, UCHAR tid, UINT16 sn, UCHAR basize, BOOLEAN isAdd, INT ses_type)
{
	...
	if (ses_type == BA_SESSION_RECP)
	{
		...
		{
#ifdef COMPOS_WIN
            mac_entry = MlmeSyncMacTabMatchedWlanIdx(pPort, wcid);
            if(mac_entry == NULL)
            {
...
VOID MtAsicUpdateRxWCIDTable(RTMP_ADAPTER *pAd, USHORT WCID, UCHAR *pAddr)
{
...
#ifdef MT7636
	union WTBL_2_DW13 *dw13 = (union WTBL_2_DW13 *)&tb_entry.wtbl_2.wtbl_2_d13.word;
#endif
	struct rtmp_mac_ctrl *wtbl_ctrl = &pAd->mac_ctrl;
	MAC_TABLE_ENTRY *mac_entry;
	UINT32 Value, Index;
	...
	{
#ifdef COMPOS_WIN
		WCID = (WCID < wtbl_ctrl->wtbl_entry_cnt[0] ? WCID : WLANIDX_MBCAST_PORT0);
		pPort = pAd->MacTabPoolArray[WCID].pPort;
#else
...
#ifdef COMPOS_WIN
    if (pAd->MacTabPoolArray[WCID].WlanIdxRole == ROLE_WLANIDX_MBCAST)
#else
    if (WCID == MCAST_WCID)
#endif
    {
...
#ifdef COMPOS_WIN
    else if (pAd->MacTabPoolArray[WCID].WlanIdxRole == ROLE_WLANIDX_APCLI_MCAST)
#else
...
#ifdef COMPOS_WIN
        mac_entry = &pAd->MacTabPoolArray[WCID];
        if(mac_entry == NULL)
        {
            DBGPRINT(RT_DEBUG_TRACE,("%s(), Line:%d, Mac Entry is NULL  Wcid = %d\n", __FUNCTION__, __LINE__, WCID));
            return;
        }
        //dw0->field.muar_idx = 0x1;
        dw0->field.muar_idx = mac_entry->pPort->OwnMACAddressIdx;
#else
        mac_entry = &pAd->MacTab.Content[WCID];
...
/*7636 E1 workaroud, must after RX counter clear*/
	...
	if (WCID != MCAST_WCID && WCID != APCLI_MCAST_WCID)
	{
		if (IS_HT_STA(mac_entry)){

			HW_IO_READ32(pAd, tb_entry.wtbl_addr[1] + (13 * 4), &dw13->word);
			dw13->field.mm = mac_entry->MpduDensity;
			dw13->field.af = mac_entry->MaxRAmpduFactor;
			HW_IO_WRITE32(pAd, tb_entry.wtbl_addr[1] + (13 * 4), dw13->word);
      		}
	}
#endif
    ...
    switch(CipherAlg)
    {
        ...
        default:
            if (CmdKey.ucAddRemove)
                break;
            MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(%d): Not support Cipher[%d] for HIF_MT yet!\n",
...
#ifdef COMPOS_WIN
        if (pPort && (pPort->SoftAP.ApCfg.bSoftAPReady == TRUE))
        {
...
INT32 MtAsicDMASchedulerInit(RTMP_ADAPTER *pAd, INT mode)
{
...
#endif /* RTMP_MAC_PCI */
#ifdef RTMP_MAC_USB
	HW_IO_READ32(pAd, 0xa0000120, &mac_val);
	page_cnt = (mac_val & 0x0fff0000) >> 16;
#endif /* RTMP_MAC_USB */
	...
	if ((dma_mode == DMA_SCH_LMAC) || (dma_mode == DMA_SCH_HYBRID))
	{
		...
		/* FFA1 max threshold */
            HIF_IO_WRITE32(pAd, MT_RSV_MAX_THD, page_cnt);
		...
		if (MTK_REV_GTE(pAd, MT7603, MT7603E1) && MTK_REV_LT(pAd, MT7603, MT7603E2)) {
			...
			/* Group 0 + 1+ 2 = PSE P0  */
			HIF_IO_WRITE32(pAd, MT_GROUP_THD_0, (page_cnt - bcn_restore_val - mcu_restore_val));
			HIF_IO_WRITE32(pAd, MT_GROUP_THD_1, bcn_restore_val);
			HIF_IO_WRITE32(pAd, MT_BMAP_0, 0x0080ff5f);
			HIF_IO_WRITE32(pAd, MT_GROUP_THD_2, mcu_restore_val);
			HIF_IO_WRITE32(pAd, MT_BMAP_1, 0x00000020);
		}
		...
		if (dma_mode == DMA_SCH_HYBRID)
		{
			...
			*/
			...
			// Disable TxD padding
			HIF_IO_WRITE32(pAd, DMA_DCR1, &mac_val);
			mac_val &= (~(0x7<<8));
			HIF_IO_WRITE32(pAd, DMA_DCR1, mac_val);
		}
	}
...
#ifdef RELEASE_EXCLUDE
    /*
    *   Per ChihLung's comment,
    *   All chip with DMA_SCH shall apply these settings as below,
    */
#endif /* RELEASE_EXCLUDE */
	//if (MTK_REV_GTE(pAd, MT7603,MT7603E1)) {
		mac_val = 0xfffff;
...
VOID MtAsicInitMac(RTMP_ADAPTER *pAd)
{
	...
	*/
    ...
    /* For Rx > 21 AMPDU */
    mac_val = 0x0e00ffff;
    HW_IO_WRITE32(pAd, RMAC_MAXMINLEN, mac_val);
#endif /* COMPOS_WIN */
...
#endif
#ifdef RTMP_MAC_USB
		HIF_IO_READ32(pAd, MT_TOP_THEMAL_ADC, &mac_restore_val);
#endif
		if (type == 0) {
