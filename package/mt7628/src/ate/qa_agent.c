	...
	INT32 Status)
{
	...
	else
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("HQA command(0x%04x)[Magic number(0x%08x)] is done\n",
										OS_NTOHS(HqaCmdFrame->Id), OS_NTOHL(HqaCmdFrame->MagicNo)));
...
#if 0
static VOID NdisMoveMemory_exl(PRTMP_ADAPTER pAd, UCHAR *dst, UCHAR *src, ULONG len)
{
	UINT32 i, Value = 0;
