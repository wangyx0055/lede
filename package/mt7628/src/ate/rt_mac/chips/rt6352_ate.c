	...
	IN PRTMP_ADAPTER pAd)
{
	...
	do
	{
		...
		do
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Wait RF calibration done BBP_R0 value = 0x%02x\n", BBPValue));
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Retry = %d, CalRF57_PassBand = 0x%02x\n", ReTry, CalRF57_PassBand));
		...
		while (TRUE)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("loopcnt = %d, CalRF57_StopBand = 0x%x,\
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s CaliBW20RfR24 = 0x%x, CaliBW40RfR24 = 0x%x\n", 
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	if (pAd->bExtPA)
	{
		/* This function is for internal PA. */
		return;
	}
...
/*	pChip->curr_temperature = (INT32)BBPR49; */ 
	...
	pa_mode_used = (pa_mode == 0x00) ? pa_mode_00 :
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("base power                    %8d\n", base_power[0]));
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("bbp_6db_power                 %8d\n", bbp_6db_power));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("pChip->ant_pwr_offset[chain]  %8d\n", pChip->ant_pwr_offset[chain]));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("pkt_type delta                %8d\n", pkt_type_delta));
        ...
        IN PRTMP_ADAPTER pAd) 
{
	...
	/* give it up (i.e., cannot get readings) */
	if ((BBPValue & 0x10) != 0)
		return;
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Current Temperature from BBP_R49=0x%x\n", __FUNCTION__, pAd->CurrTemperature));
...
/*		TxAgcStep          = pAd->TxAgcStepG; */
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("** delta_pwr =%d **\n", delta_pwr));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - delta_pwr = %d, TssiCalibratedOffset = %d, TssiMpOffset = %d\n",
...
static VOID RT635xATEAsicAdjustTxPower(
	IN	PRTMP_ADAPTER	pAd)
{
	...
	{
		RT635xATETssiCompensation(pAd);
		return;
	}
#endif /* RTMP_INTERNAL_TX_ALC */

#ifdef RTMP_TEMPERATURE_COMPENSATION
	if (pAd->bAutoTxAgcG == TRUE)
	{
		RT635xATETemperatureCompensation(pAd);
		return;
	}
#endif /* RTMP_TEMPERATURE_COMPENSATION */
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			...
			"pATEInfo->TxPower0 = %d\n pATEInfo->TxPower1 = %d\n", 
			__FUNCTION__, Channel, pATEInfo->TxWI.TXWI_O.BW, pATEInfo->RFFreqOffset,
	...
	for (index = 0; index < NUM_OF_6352_CHNL; index++)
	{
		if (Channel == FreqItems6352[index].Channel)
		{
			...
			{
				if(pATEInfo->TxWI.TXWI_O.BW == RT6352_RFDCCal_BW[reg_index].BW)
				{
...

			{
				...
				if ((pAd->CommonCfg.Chip_VerID > 1) && (pAd->CommonCfg.Chip_E_Number >= 2))
				{
					...
					/* ADC clock selection */
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ADC clock selection for E3 !!!\n"));
#if 0
...
/*		filter_coefficient_ctrl(pAd, Channel); */
	...
	{
		if(pATEInfo->TxWI.TXWI_O.BW == RT6352_BBP_GLRT_BW[reg_index].BW)
		{
...
#if 0	
	ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R158, BBP_R140);
	ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R159, 0x4C);
#endif
...
/*	UCHAR Channel = pATEInfo->Channel; */
	...
	if (index == 0)
	{
		...
		if ((pATEInfo->TxWI.TXWI_O.BW == BW_40) && (pATEInfo->bAutoTxAlc == TRUE))
		{
...
/*	ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R83, 0x7A); */
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, 
				...
				__FUNCTION__, pATEInfo->Channel, pATEInfo->TxWI.TXWI_O.BW, 
				LNAGain, R66));
#endif /* RELEASE_EXCLUDE */
	...
	INOUT	PCHAR				pTotalDeltaPower) 
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: <Before BBP R1> TotalDeltaPower = %d dBm\n", __FUNCTION__, *pTotalDeltaPower));
	...
	if (*pTotalDeltaPower <= -12)
	{
		*pTotalDeltaPower += 12;
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Drop the transmit power by 12 dBm (BBP R1)\n", __FUNCTION__));
	}
	else if ((*pTotalDeltaPower <= -6) && (*pTotalDeltaPower > -12))
	{
		*pTotalDeltaPower += 6;
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Drop the transmit power by 6 dBm (BBP R1)\n", __FUNCTION__));
	}
	else
	{
...
==========================================================================
*/
INT	RT635x_Set_ATE_TX_BW_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
	UCHAR value = 0;
	UCHAR BBPCurrentBW;
			...
			case 1:	/* 1R */					
				switch (pATEInfo->RxAntennaSel)
				{
					case ANT_0:	
						...
						RTMP_IO_WRITE32(pAd, RTMP_RF_BYPASS0, Value);
						Value = 0x00000200;
...
==========================================================================
*/
INT	RT635x_Set_ATE_TX_FREQ_OFFSET_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
struct _ATE_CHIP_STRUCT RALINK6352 =
{
	...
	.AdjustTxPower = RT635xATEAsicAdjustTxPower,
