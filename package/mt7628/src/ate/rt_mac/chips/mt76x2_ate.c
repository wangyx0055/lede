...
*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s::BBP 0x2320=0x%08x\n", __FUNCTION__, value));
	RTMP_BBP_IO_READ32(ad, AGC1_R9, &value);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s::BBP 0x2324=0x%08x\n", __FUNCTION__, value));
	RTMP_BBP_IO_READ32(ad, AGC1_R4, &value);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s::BBP 0x2310=0x%08x\n", __FUNCTION__, value));
	RTMP_BBP_IO_READ32(ad, AGC1_R5, &value);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s::BBP 0x2314=0x%08x\n", __FUNCTION__, value));
	
	/* Fix dynamic agc gain in ATE Rx test not been initialized problem, and will cause AGC1_R8 register having very small value */
	mt76x2_get_agc_gain(ad);
	...
	IN RTMP_ADAPTER *ad)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : tx0_target_pwr=%d, tx0_pwr_ch_delta=%d, tx_pwr_bw_delta=%d, tx0_pwr=%d\n"
		, __FUNCTION__, tx0_target_pwr, tx0_pwr_ch_delta, tx_pwr_bw_delta, tx0_pwr));

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : tx1_target_pwr=%d, tx1_pwr_ch_delta=%d, tx_pwr_bw_delta=%d, tx1_pwr=%d\n"
    ...
    IN PRTMP_ADAPTER ad)
{
	PATE_INFO   pATEInfo = &(ad->ate);
	UINT32 BbpValue = 0;
	UINT32 Value = 0 , TxPinCfg = 0;

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : Mode = %d\n", __FUNCTION__, pATEInfo->Mode));
		...
		case 2:
			switch (pATEInfo->TxAntennaSel)
			{
				...
				default: /* all DACs */
					/* bypass MAC control DAC selection */
					RTMP_BBP_IO_READ32(ad, IBI_R9, &BbpValue);
					BbpValue |= 0xFFFFF7FF; /* 0x2124[11]=1 */
					RTMP_BBP_IO_WRITE32(ad, IBI_R9, BbpValue);
    ...
    IN CHAR rx)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : tx=%d, rx=%d\n", __FUNCTION__, tx, rx));
...
INT	mt76x2_set_ate_tx_bw_proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Set_ATE_TX_BW_Proc!! Error!! Unknow bw.\n"));
		return TRUE;
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Set_ATE_TX_BW_Proc!! Warning!! CCK only supports 20MHZ!!\nBandwidth switch to 20\n"));
		ad->ate.TxWI.TXWI_N.BW = BW_20;
	}
	...
	if ( (ad->ate.TxWI.TXWI_N.PHYMODE == MODE_VHT) && (ad->ate.TxWI.TXWI_N.BW == BW_20) )
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Set_ATE_TX_BW_Proc!! Warning!! VHT MCS9 not support Bandwidth switch to 20\n"));
			ad->ate.TxWI.TXWI_N.BW = BW_40;
		}
	}
...
//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): bw=%d, Set AGC1_R0=0x%x, agc_r0=0x%x\n", __FUNCTION__, bw, agc, agc_r0));
//		RTMP_BBP_IO_READ32(pAd, AGC1_R0, &agc);
//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): bw=%d, After write, Get AGC1_R0=0x%x,\n", __FUNCTION__, bw, agc));
	}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_TX_BW_Proc Success\n"));
...
void mt76x2_ate_temp_tx_alc(RTMP_ADAPTER *ad)
{
	...
	if (pChipCap->temp_tx_alc_enable) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s::temp_diff=%d (0x%x), dB_diff=%d (0x%x)\n", 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s::Tx0 power compensation = 0x%x\n", 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s::Tx1 power compensation = 0x%x\n", 
		...
		IN UINT32 *slope_offset)
{
	...
	else if(CurrentTemperature <= 29)
	{
		TSSIOffsetDelta0 = 0;
		TSSIOffsetDelta1 = 0;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("TSSIOffsetDelta = 0, not need adjust\n"));
		return;
	}
...
INT	mt76x2_set_ate_tx_freq_offset_proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	UINT32 freq_offset = 0;
	UINT32 value = 0;
	UINT32 misc_ctrl = 0;
	UINT8 count = 0;
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_TX_FREQOFFSET_Proc (RFFreqOffset = %d)\n", ad->ate.RFFreqOffset));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_TX_FREQOFFSET_Proc Success\n"));
	...
	IN PRTMP_ADAPTER pAd, UCHAR ate_mode)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: channel=%d ate_mode=0x%x\n", __FUNCTION__, pAd->ate.Channel, ate_mode));
	...
	switch ( ate_mode )
	{
		...
		case ATE_TXFRAME:
			/* LC Calibration */
			if (channel > 14)
				CHIP_CALIBRATION(pAd, LC_CALIBRATION_7662, 0x00);
			RtmpOsMsDelay(100);

			/* TX LOFT */
			if (channel > 14) {
				CHIP_CALIBRATION(pAd, TX_LOFT_CALIBRATION_7662, 0x1);
			} else {
				CHIP_CALIBRATION(pAd, TX_LOFT_CALIBRATION_7662, 0x0);
			}
			RtmpOsMsDelay(200);
			...
			/* DPD Calibration */
			CHIP_CALIBRATION(pAd, DPD_CALIBRATION_7662, channel);
			RtmpOsMsDelay(100);
			...
			for (i=R_CALIBRATION_7662;i<=RXIQC_FD_CALIBRATION_7662;i++)
			{
				switch(i)
				{
					case RXDCOC_CALIBRATION_7662:
						record_calibration_info(pAd, RXDCOC_CALIBRATION_7662);
						break;
					case RC_CALIBRATION_7662:
						record_calibration_info(pAd, RC_CALIBRATION_7662);
						break;
					case LC_CALIBRATION_7662:
						record_calibration_info(pAd, LC_CALIBRATION_7662);
						break;
					case TX_LOFT_CALIBRATION_7662:
						record_calibration_info(pAd, TX_LOFT_CALIBRATION_7662);
						break;
					case TXIQ_CALIBRATION_7662:
						record_calibration_info(pAd, TXIQ_CALIBRATION_7662);
						break;
					case TSSI_CALIBRATION_7662:
						record_calibration_info(pAd, TSSI_CALIBRATION_7662);
						break;
					case DPD_CALIBRATION_7662:
						record_calibration_info(pAd, DPD_CALIBRATION_7662);
						break;		
					case RXIQC_FI_CALIBRATION_7662:
						record_calibration_info(pAd, RXIQC_FI_CALIBRATION_7662);
						break;		
					case RXIQC_FD_CALIBRATION_7662:
						record_calibration_info(pAd, RXIQC_FD_CALIBRATION_7662);
						break;			
					default:
						break;
				}
