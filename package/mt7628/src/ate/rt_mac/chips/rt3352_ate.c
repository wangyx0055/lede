    ...
    IN PRTMP_ADAPTER pAd)
{
	...
	*/
	...
	{ 
		/* Xtal=40M */
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RT3352:SwitchChannel#%d(RFIC=%d, Pwr0=%d, Pwr1=%d, %dT), N=0x%02X, K=0x%02X, R=0x%02X\n",
		Channel, 
		pAd->RfIcType, 
		TxPwer,
		TxPwer2,
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RT3352Xtal20M:SwitchChannel#%d(RFIC=%d, Pwr0=%d, Pwr1=%d, %dT), N=0x%02X, K=0x%02X, R=0x%02X\n",
		Channel, 
		pAd->RfIcType, 
		TxPwer,
		TxPwer2,
	...
	IN PRTMP_ADAPTER pAd)
{
    ...
    if (pAd->TxPowerCtrl.bInternalTxALC == TRUE)
    {
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RF_R47 = 0x%02x\n", RFValue));
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Pwr1 = 0x%02x\n", (pAd->TxPower[pAd->ate.Channel-1].Power2)));			  
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("AntennaDeltaPwr = %d\n", AntennaDeltaPwr));			  
		...
		ATE_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R1, &BbpR1);
			...
			ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R1, BbpR1);
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TPC: %s: Drop the transmit power by 12 dBm (BBP R1)\n", __FUNCTION__));
			}
			...
			ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R1, BbpR1);
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TPC: %s: Drop the transmit power by 6 dBm (BBP R1)\n", __FUNCTION__));
			}
			else
			{
...
#endif /* RTMP_INTERNAL_TX_ALC */
	{
		/* TX power compensation for temperature variation based on TSSI boundaries. */
		/* Do it per second. */
		if (1 /* pAd->ate.OneSecPeriodicRound % 4 == 0 */)
	{
		if (pAd->ate.Channel <= 14)
		{
			/* bg channel */
