    ...
    IN PRTMP_ADAPTER pAd)
{
	...
	for (index = 0; index < NUM_OF_3572_CHNL; index++)
	{
		if (Channel == FreqItems3572[index].Channel)
		{
			...
			if (/*!bScan &&*/(pATEInfo->TxWI.BW == BW_40))
			{
				...
				if (pATEInfo->TxWI.PHYMODE == MODE_HTMIX)
...
#ifdef HUAWEI_ATE
			/* This is supposed just for rt3062 now. */
			if ((Channel == 1) && (pATEInfo->TxWI.TXWI_O.PHYMODE == MODE_CCK))
			{
				RFValue = 0x68;
	...
	*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RT35xx:SwitchChannel#%d(RFIC=%d, Pwr0=%d, Pwr1=%d, %dT), N=0x%02X, K=0x%02X, R=0x%02X\n",
		Channel, 
		pAd->RfIcType, 
		TxPwer,
		TxPwer2,
...
INT	RT35xx_Set_ATE_TX_BW_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (pATEInfo->TxWI.BW == BW_20)
	{
		...
		if (pATEInfo->Channel > 14)
		{
			value = 0x22 + (pAd->hw_cfg.lan_gain * 5) / 3;
		}
		else
		{
			value = 0x1C + 2 * pAd->hw_cfg.lan_gain;
		}				
		...
		/* set BW = 20 MHz */
		/* R24, R31, one is for tx, the other is for rx */
		if (pATEInfo->TxWI.TXWI_O.PHYMODE == MODE_CCK)/* CCK */
		{
			ATE_RF_IO_WRITE8_BY_REG_ID(pAd, RF_R24, 0x1f);
		}
		else
