	...
	IN PRTMP_ADAPTER		pAd)
{
	...
	if (IS_MT7601(pAd))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s::Currently not support singledriver.\n", __FUNCTION__));
		return 0;
 	}
#endif /* MT7601 */
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: desiredTSSI = %d, Latest Tx setting: MODE = %d, MCS = %d, STBC = %d\n", 
		__FUNCTION__, 
		desiredTSSI, 
		phy_mode, 
		MCS, 
	...
	if (pAd->chipCap.hif_type == HIF_RTMP)
	{
		...
		*/
		...
		if ((phy_mode == MODE_CCK) && (pATEInfo->Channel == 14) && (bw == BW_20))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("SetJapanFilter!!!\n"));
		}
		else
		{
			BbpData &= 0xdf;    /* turn off */
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ClearJapanFilter!!!\n"));
		}
	...
	IN PRTMP_ADAPTER pAd) 
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!! ATEAPStop !!!\n"));
...
VOID RTMPStationStop(
    IN  PRTMP_ADAPTER   pAd)
{
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> RTMPStationStop\n"));
...
VOID RTMPStationStart(
    IN  PRTMP_ADAPTER   pAd)
{
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> RTMPStationStart\n"));
	...
	IN PUCHAR pBssid) 
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> ATEAsicSetBssid %x:%x:%x:%x:%x:%x\n",
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ATE : ===> %s\n", __FUNCTION__));
...
#else
				pDestTxD = (TXD_STRUC *)pAd->TxRing[QID_AC_BE].Cell[ring_index].AllocVa;
...
/*	pAd->ContinBulkIn = FALSE; */
	...
	*/
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("We are still in ATE mode, "));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("so we keep ATE periodic timer running.\n"));
	}
	...
	if (IS_RT3050(pAd))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("EEPROM: RF FreqOffset=%d \n", pATEInfo->RFFreqOffset));
	}
	/*-----------------------------------------------*/
#endif /* RT305x */
#endif /* HUAWEI_ATE */
	...
	if (IS_RT6352(pAd))
	{
#if defined(RT6352_EP_SUPPORT) || defined(RT6352_EL_SUPPORT)
		{
			...
			if ((SysRegValue & 0x100000) == 0x0)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("Change as GPIO Mode(0x%x)\n", SysRegValue));
			}
		}
...
#if defined(RT6352_EP_SUPPORT) || defined(RT6352_EL_SUPPORT)
		{
			...
			if ((SysRegValue & 0x100000) == 0x100000)
			{
				SysRegValue &= (~0x100000);
				RTMP_SYS_IO_WRITE32(0xb0000060, SysRegValue);
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("Change as Normal Mode(0x%x)\n", SysRegValue));
			}
	}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ATE : ===> %s(Count=%u)\n", __FUNCTION__, pATEInfo->TxCount));
	...
	if (pATEInfo->bQATxStart == FALSE)  
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IPGstr=%s\n", IPGStr));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("bulk out count = %d\n", atomic_read(&pAd->BulkOutRemained)));
	ASSERT((atomic_read(&pAd->BulkOutRemained) >= 0));

	if (atomic_read(&pAd->BulkOutRemained) == 0)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Send packet continuously\n"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Send packets depend on counter\n"));
		...
		if ((IS_RT3350(pAd)) || (IS_RT3352(pAd))) 
		{
				...
				&& (pATEInfo->TxWI.TXWI_O.BW == BW_20)	&& (pATEInfo->TxAntennaSel == 1))                  
			{
				if (pATEInfo->Channel == 7)
				{
					...
					RtmpOsMsDelay(500);
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Channel %d, Calibrated Tx.Power0= 0x%04x\n", CurrentChannel, ChannelPower));

					/* read BBP R49[4:0] and write to EEPROM 0x6E */
					ATE_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R49, &BbpData);
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BBP R49 = 0x%02x\n", BbpData)); 
					BbpData &= 0x1f;
					TssiRefPerChannel[CurrentChannel-1] = BbpData;
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TSSI = 0x%02x\n", TssiRefPerChannel[CurrentChannel-1]));  
				}
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Channel %d, Calibrated Tx.Power0= 0x%04x\n", CurrentChannel, ChannelPower));
				}
			}
		}
	}
	...
	if ((pATEInfo->bTSSICalbrEnableG == TRUE) || (pATEInfo->bTSSICalbrEnableA == TRUE))
	{
		if (IS_RT5592(pAd))
		{
				...
				&& (pATEInfo->TxWI.TXWI_O.BW == BW_20))                  
			{
				if (pATEInfo->bTSSICalbrEnableG == TRUE)
				{
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Read  BBP_R49\n"));
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Channel %d\n", pATEInfo->Channel));

						ATE_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R49, &BbpData);

						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BBP R49=0x%x\n", BbpData));			
...
INT	Set_ATE_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_Proc Success\n"));

#ifdef CONFIG_AP_SUPPORT
#ifdef GEMTEK_ATE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, (KERN_EMERG "Gemtek:Success\n"));
#endif /* GEMTEK_ATE */
#endif /* CONFIG_AP_SUPPORT */
		return TRUE;
	}
...
INT	Set_ATE_DA_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
...
#ifdef CONFIG_AP_SUPPORT		
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_DA_Proc (DA = %02x:%02x:%02x:%02x:%02x:%02x)\n", 
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_DA_Proc Success\n"));
...
INT	Set_ATE_SA_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
...
#ifdef CONFIG_AP_SUPPORT		
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_SA_Proc (SA = %02x:%02x:%02x:%02x:%02x:%02x)\n", 
		pATEInfo->Addr3[0], pATEInfo->Addr3[1], pATEInfo->Addr3[2], pATEInfo->Addr3[3],
		pATEInfo->Addr3[4], pATEInfo->Addr3[5]));
#ifdef GEMTEK_ATE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, (KERN_EMERG "Gemtek:Success\n"));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_SA_Proc Success\n"));
...
INT	Set_ATE_BSSID_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
...
#ifdef CONFIG_AP_SUPPORT		
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_BSSID_Proc (BSSID = %02x:%02x:%02x:%02x:%02x:%02x)\n",	
		pATEInfo->Addr2[0], pATEInfo->Addr2[1], pATEInfo->Addr2[2], pATEInfo->Addr2[3],
		pATEInfo->Addr2[4], pATEInfo->Addr2[5]));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_BSSID_Proc Success\n"));
...
INT	Set_ATE_MAC_Filter_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_MAC_Filter_Proc (FILETER = %02x:%02x:%02x:%02x:%02x:%02x)\n",	
		pATEInfo->Filter[0], pATEInfo->Filter[1], pATEInfo->Filter[2],
		pATEInfo->Filter[3], pATEInfo->Filter[4], pATEInfo->Filter[5]));

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_MAC_Filter_Proc Success\n"));
...
INT Set_ATE_MAC_Filter_Enable_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (MACFilterEnabled == 0)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ATE MAC Filter diabled !\n"));
	}
	else
	{
		pATEInfo->bMACFilterEnable = TRUE;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ATE MAC Filter enabled !\n"));
	}

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_MAC_Filter_Enable_Proc Success\n"));
...
INT	Set_ATE_CHANNEL_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_CHANNEL_Proc (ATE Channel = %d)\n", pATEInfo->Channel));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_CHANNEL_Proc Success\n"));
...
INT	Set_ATE_INIT_CHAN_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_INIT_CHAN_Proc Success\n"));
...

{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_TX_POWER%d_Proc Success\n", index));
...
INT	Set_ATE_TX_Antenna_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_TX_Antenna_Proc (Antenna = %d)\n", pATEInfo->TxAntennaSel));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_TX_Antenna_Proc Success\n"));
...
INT	Set_ATE_RX_Antenna_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_RX_Antenna_Proc (Antenna = %d)\n", pATEInfo->RxAntennaSel));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_RX_Antenna_Proc Success\n"));
	...
	IN	PRTMP_ADAPTER 		pAd)
{
	...
	{	
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Offset =0x13D8, TxPwr = 0x%08X, ", (UINT)ExtraPwrOverTxPwrCfg8));
	}
	
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Offset = 0x13D4, TxPwr = 0x%08X, Offset = 0x13DC, TxPwr = 0x%08X\n", 
...
#endif /* RTMP_RF_RW_SUPPORT */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_PA_Bias_Proc (PABias = %d)\n", pATEInfo->PABias));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_PA_Bias_Proc Success\n"));
...
INT	Set_ATE_TX_FREQ_OFFSET_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (ret == TRUE)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_TX_FREQ_OFFSET_Proc (RFFreqOffset = %d)\n", pATEInfo->RFFreqOffset));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_TX_FREQ_OFFSET_Proc Success\n"));
...
f (pAd->chipCap.hif_type == HIF_RTMP)
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_TX_BW_Proc (BBPCurrentBW = %d)\n", bw));
	}	
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_TX_BW_Proc %s\n", status == TRUE ? "success" : "failed"));
...
INT	Set_ATE_TX_LENGTH_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_TX_LENGTH_Proc (TxLength = %d)\n", pATEInfo->TxLength));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_TX_LENGTH_Proc Success\n"));
...
INT	Set_ATE_TX_COUNT_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_TX_COUNT_Proc (TxCount = %d)\n", pATEInfo->TxCount));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_TX_COUNT_Proc Success\n"));
...
INT	Set_ATE_TX_MCS_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_TX_MCS_Proc (MCS = %d)\n", MCS));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_TX_MCS_Proc Success\n"));
...
INT	Set_ATE_TX_STBC_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_TX_STBC_Proc (GI = %d)\n", stbc));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_TX_STBC_Proc Success\n"));
...
INT	Set_ATE_TX_MODE_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (phy_mode > MODE_VHT)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("0: CCK, 1: OFDM, 2: HT_MIX, 3: HT_GREEN_FIELD, 4: VHT.\n"));
		return FALSE;
	}
	...
	if (phy_mode == MODE_CCK)
	{
		...
		ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R4, BbpData);
#endif /* RTMP_BBP */
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Set_ATE_TX_MODE_Proc::CCK Only support 20MHZ. Switch to 20MHZ.\n"));
	}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_TX_MODE_Proc (TxMode = %d)\n", phy_mode));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_TX_MODE_Proc Success\n"));
...
INT	Set_ATE_TX_GI_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_TX_GI_Proc (GI = %d)\n", sgi));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_TX_GI_Proc Success\n"));
...
INT	Set_ATE_RX_FER_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_RX_FER_Proc (bRxFER = %d)\n", pATEInfo->bRxFER));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_RX_FER_Proc Success\n"));
...
INT Set_ATE_Read_RF_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (IS_RT30xx(pAd) || IS_RT3572(pAd))
	{
		...
		for (index = 0; index < 32; index++)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("R%d=%d\n",index,RFValue));
		}
#endif /* RTMP_RF */
	}
	else
...
#endif /* RTMP_RF_RW_SUPPORT */
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("R1 = %x\n", pAd->LatchRfRegs.R1));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("R2 = %x\n", pAd->LatchRfRegs.R2));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("R3 = %x\n", pAd->LatchRfRegs.R3));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("R4 = %x\n", pAd->LatchRfRegs.R4));
	}
	return TRUE;
}
...
INT Set_ATE_LED_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_LED_Proc :: %d\n", pATEInfo->LedIndicatorStrength));
...
INT	Set_ATE_AUTO_ALC_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (value > 0)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ATEAUTOALC = TRUE , auto alc enabled!\n"));
	}
	else
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ATEAUTOALC = FALSE , auto alc disabled!\n"));
	}	
...
INT	Set_ATE_TEMP_SENSOR_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (value > 0)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ATETEMPSENSOR = TRUE , temperature sensor enabled!\n"));
	}
	else
	{
		pATEInfo->bLowTemperature = FALSE;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ATETEMPSENSOR = FALSE , temperature sensor disabled!\n"));
	}	
...
INT	Set_ATE_VCO_CAL_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (value > 0)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ATEVCOCAL = TRUE , auto vcoc enabled!\n"));
	}
	else
	{
		pAd->ate.bAutoVcoCal = FALSE;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ATEVCOCAL = FALSE , auto vcoc disabled!\n"));
	}	
...
#ifndef MT76x2
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("Divider Cal Done:\n"
...
INT Set_ATE_ForceBBP_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_ForceBBP_Proc:(forceBBPReg value=%d)\n", pATEInfo->forceBBPReg));
...
INT	Set_ATE_IPG_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (value <= 0)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Set_ATE_IPG_Proc::IPG is disabled(IPG == 0).\n"));
		return TRUE;
	}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_IPG_Proc (IPG = %u)\n", pATEInfo->IPG));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_IPG_Proc Success\n"));
...
INT	Set_ATE_Payload_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_Payload_Proc Success\n"));
...
INT	Set_ATE_Fixed_Payload_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_ATE_Fixed_Payload_Proc (Fixed Payload  = %d)\n", pATEInfo->bFixedPayload));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink: Set_ATE_Fixed_Payload_Proc Success\n"));
...
INT	Set_ATE_IQ_Compensation_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (value > 0)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("bIQCompEnable = TRUE , I/Q compensation enabled!\n"));
	}
	else
	{
		pATEInfo->bIQCompEnable = FALSE;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("bIQCompEnable = FALSE , I/Q compensation disabled!\n"));
	}	
...
INT	Set_ATE_TTR_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (value > 0)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ATETTR = TRUE , test time reduction enabled!\n"));
	}
	else
	{
		pAd->ate.bTestTimeReduction = FALSE;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ATETTR = FALSE , test time reduction disabled!\n"));
	}	
...
INT	Set_ATE_Show_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATE Mode=%s\n", Mode_String));
#ifdef RT3350
	if (IS_RT3350(pAd))
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("PABias=%u\n", pATEInfo->PABias));
#endif /* RT3350 */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TxPower0=%d\n", pATEInfo->TxPower0));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TxPower1=%d\n", pATEInfo->TxPower1));
#ifdef DOT11N_SS3_SUPPORT
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TxPower2=%d\n", pATEInfo->TxPower2));
#endif /* DOT11N_SS3_SUPPORT */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TxAntennaSel=%d\n", pATEInfo->TxAntennaSel));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("RxAntennaSel=%d\n", pATEInfo->RxAntennaSel));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("BBPCurrentBW=%u\n", bw));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("GI=%u\n", sgi));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("MCS=%u\n", mcs));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TxMode=%s\n", TxMode_String));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Addr1=%02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Addr3=%02x:%02x:%02x:%02x:%02x:%02x\n",
		pATEInfo->Addr3[0], pATEInfo->Addr3[1], pATEInfo->Addr3[2], pATEInfo->Addr3[3], pATEInfo->Addr3[4], pATEInfo->Addr3[5]));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Channel=%u\n", pATEInfo->Channel));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TxLength=%u\n", pATEInfo->TxLength));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TxCount=%u\n", pATEInfo->TxCount));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("RFFreqOffset=%u\n", pATEInfo->RFFreqOffset));
#ifdef RT35xx
	if (IS_RT3572(pAd))
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("LedIndicatorStregth=%d\n", pATEInfo->LedIndicatorStrength));
#endif /* RT35xx */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("bAutoTxAlc=%d\n", pATEInfo->bAutoTxAlc));
#ifdef VCORECAL_SUPPORT 
	if (IS_RT3593(pAd) || IS_MT76x0(pAd))
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("bAutoVcoCal=%d\n", pATEInfo->bAutoVcoCal));
#endif /* VCORECAL_SUPPORT */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("bTestTimeReduction=%d\n", pATEInfo->bTestTimeReduction));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("IPG=%u\n", pATEInfo->IPG));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Payload=0x%02x\n", pATEInfo->Payload));
#ifdef TXBF_SUPPORT
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("bTxBF=%d\n", pATEInfo->bTxBF));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("txSoundingMode=%d\n", pATEInfo->txSoundingMode));
#endif /* TXBF_SUPPORT */
#ifdef IQ_CAL_SUPPORT
	if (IS_RT5592(pAd))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("bIQCompEnable=%d\n", pATEInfo->bIQCompEnable));
	}
#endif /* IQ_CAL_SUPPORT */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Set_ATE_Show_Proc Success\n"));
#ifdef CONFIG_AP_SUPPORT
#ifdef GEMTEK_ATE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, (KERN_EMERG "Gemtek:Success\n"));
#endif /* GEMTEK_ATE */
...
INT	Set_ATE_Help_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
...
#else
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATE=APSTOP, APSTART, TXCONT, TXCARR, TXCARS, TXFRAME, RXFRAME\n"));
#endif /* CONFIG_RT2880_ATE_CMD_NEW */
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATECHANNEL, range:0~14(unless A band !)\n"));
...
#ifdef DOT11N_SS3_SUPPORT
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATETXPOW2, set power level of antenna 3.\n"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATETXANT, set TX antenna. 0:all, 1:antenna one, 2:antenna two, 3:antenna three.\n"));
#else
...
#ifdef RT3350
	if (IS_RT3350(pAd))
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATEPABIAS, set power amplifier bias for EVM, range 0~15\n"));
#endif /* RT3350 */
#ifdef RTMP_RF_RW_SUPPORT
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATETXFREQOFFSET, set frequency offset, range 0~95\n"));
#else
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATETXFREQOFFSET, set frequency offset, range 0~63\n"));
#endif /* RTMP_RF_RW_SUPPORT */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATETXBW, set BandWidth, 0:20MHz, 1:40MHz , 2:80MHz.\n"));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATEWRF1, set RF1 register.\n"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATEWRF2, set RF2 register.\n"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATEWRF3, set RF3 register.\n"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATEWRF4, set RF4 register.\n"));
#endif /* !RTMP_RF_RW_SUPPORT */
	...
	if (IS_RT3572(pAd))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATELED, test signal LED, range 0~31.\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATEHWLED, control LED by hardware.\n"));
	}
#endif /* RT35xx */
#endif /* LED_CONTROL_SUPPORT */
...
#ifdef VCORECAL_SUPPORT 
	if (IS_RT3593(pAd) || (IS_MT76x0(pAd)))
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATEVCOCAL, enable ATE Tx VCO calibration.\n"));
#endif /* VCORECAL_SUPPORT */
...
#ifdef TXBF_SUPPORT
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATETXBF, enable ATE Tx beam forming.\n"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATETXSOUNDING, Sounding mode 0:none, 1:Data sounding, 2:2 stream NDP, 3:3 stream NDP.\n"));
#endif /* TXBF_SUPPORT */ 
#ifdef RTMP_INTERNAL_TX_ALC
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATETSSICBA, start internal TSSI calibration.\n"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATETSSICBAEX, start extended internal TSSI calibration.\n"));
#endif /* RTMP_INTERNAL_TX_ALC */
#ifdef RTMP_TEMPERATURE_COMPENSATION
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATETSSICBAEX, start advanced temperature TSSI calibration.\n"));
#endif /* RTMP_TEMPERATURE_COMPENSATION */
#ifdef RT33xx	
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATETXEVMSHOW, show the information of EVM calibration.\n"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATETXEVMCAL, write EVM calibration value to corresponding RF register.\n"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATETXEVMFILL, fill EVM calibration result in corresponding EEPROM.\n"));
#endif /* RT33xx */
#ifdef IQ_CAL_SUPPORT
	if (IS_RT5592(pAd))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ATEIQCOMP, enable I/Q gain/phase compensation.\n"));
	}
#endif /* IQ_CAL_SUPPORT */
...
/*	PATE_INFO pATEInfo = &(pAd->ate); */
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("curr_temperature = 0x%x\n", curr_temperature));
								...
								&pAd->CurrTemperature) == TRUE)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("curr_reading = 0x%x\n", curr_reading));
...
INT RT335x_Set_ATE_TSSI_CALIBRATION_ENABLE_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
	...
	if (strcmp(arg, "0") == 0)
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TSSI calibration disabled!\n"));
	}
	else if (strcmp(arg, "1") == 0)
			{
		bTSSICalbrEnableG = TRUE;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TSSI calibration enabled!\n"));
			}
	else
	{
		return FALSE;
...
{    
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TSSI_ref = 0x%02x\n", TssiRefPerChannel[CurrentChannel-1]));  
	RT28xx_EEPROM_READ16(pAd, EEPROM_TSSI_OVER_OFDM_54, EEPData);
	EEPData &= 0xff00;
	EEPData |= BbpData;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Write  E2P 0x6e: 0x%04x\n", EEPData));  
	...
	for (CurrentChannel = 1; CurrentChannel <= 14; CurrentChannel++)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Channel %d, TSSI= 0x%x, TssiDelta=0x%x\n", 
...
INT RT5592_ATEReadExternalTSSI(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (pATEInfo->bTSSICalbrEnableG == TRUE)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Sum of TSSI readings (2.4G) = 0x%x\n", TempBbp));
	...
	if (pATEInfo->bTSSICalbrEnableA == TRUE)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Sum of TSSI readings (5G) = 0x%x\n", TempBbp));
...
INT RT5592_Set_ATE_TSSI_CALIBRATION_ENABLE_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (strcmp(arg, "0") == 0)
	{
		...
		bTSSICalbrEnableG = FALSE;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("temperature calibration disabled!\n"));
	}
	else if (strcmp(arg, "1") == 0)
	{
		bTSSICalbrEnableA = TRUE;
		bTSSICalbrEnableG = FALSE;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("A band temperature calibration enabled!\n"));
	}
	else if (strcmp(arg, "2") == 0)
	{
		bTSSICalbrEnableA = FALSE;
		bTSSICalbrEnableG = TRUE;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("G band temperature calibration enabled!\n"));
	}
	else if (strcmp(arg, "3") == 0)
	{
		bTSSICalbrEnableA = TRUE;
		bTSSICalbrEnableG = TRUE;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Dual band temperature calibration enabled!\n"));
	}
	else
	{
		return FALSE;
	...
	do
 	{
		...
		if (Value == 0xffff)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Invalid value of TxEvmCal (%x)\n", Value));
			break;
		}
...
INT Set_ATE_TX_EVM_CALIBRATION_Show_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("RfReg=%d, Range=%d, BitMap=%x, Shift=%d\n", RfReg, Range, BitMap, Shift));
...
INT Set_ATE_TX_EVM_CALIBRATION_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (Value > Range)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Invalid Range value(%ld), Range=%d\n", Value, Range));
		return TRUE;
	}
...
INT Set_ATE_TX_EVM_CALIBRATION_Fill_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	switch (RfReg)
	{
		...
		case RF_R29:
			...
			break;
		default:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Invalid RfReg(%d)", RfReg));
			return TRUE;
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Invalid Range value(%d), Range=%d",TargetValue, Range));
		return TRUE;
	}
...
INT Set_ATE_Read_Temperature_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Set_MT7601ATE_Read_Temperature_Proc\n"));

	MT7601_Read_Temperature(pAd, &Temperature);

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Temperature = %d (0x%X)\n", Temperature, Temperature));
...
INT Set_ATE_Read_TSSI_DC_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Set_ATE_Read_TSSI_DC_Proc\n"));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("TSSI DC = %d (0x%X)\n", BbpReg, BbpReg));
	RtmpOsMsDelay(1);
...
INT Set_ATE_DO_CALIBRATION_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (arg)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():cal_id = %d, param = %d\n", __FUNCTION__, cal_id, param));
		if (rv == 2)
		{
...
INT Set_ATE_Load_CR_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (arg)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():mode = %d, temp_level = %d, channel = %d\n", __FUNCTION__, mode, temp_level, channel));
		if (rv == 3)
		{
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	if (IS_RT8592(pAd))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): RALINK85592 hook !\n", __FUNCTION__));
	...
	if (IS_RT6570(pAd))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): RALINK6570 hook !\n", __FUNCTION__));
		return NDIS_STATUS_SUCCESS;
	}
#endif /* RTMP_MAC_USB */
	...
	if (IS_MT7610(pAd))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): RALINK6590 hook !\n", __FUNCTION__));
		return NDIS_STATUS_SUCCESS;
	}
#endif /* RTMP_MAC_PCI */
#endif /* MT76x0 */
	...
	if (IS_MT76x2(pAd))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): MT76x2 hook !\n", __FUNCTION__));
		return NDIS_STATUS_SUCCESS;
	}
#endif /* MT76x2 */
	...
	if (IS_RT6352(pAd))
	{
		...
		*/
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): RALINK6352 hook !\n", __FUNCTION__));
		return NDIS_STATUS_SUCCESS;
	}
#endif /* IS_RT6352 */
	...
	if (IS_RT2883(pAd))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): RALINK2883 hook !\n", __FUNCTION__));
		return NDIS_STATUS_SUCCESS;
	}
#endif /* RT2883 */
	...
	if (IS_RT3883(pAd))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): RALINK3883 hook !\n", __FUNCTION__));
		return NDIS_STATUS_SUCCESS;
	}
#endif /* RT3883 */
	...
	if (IS_RT3050(pAd))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): RALINK3050 hook !\n", __FUNCTION__));
		return NDIS_STATUS_SUCCESS;
	}
#endif /* RT3050 */
	...
	if (IS_RT3052(pAd))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): RALINK3052 hook !\n", __FUNCTION__));
		return NDIS_STATUS_SUCCESS;
	}
#endif /* RT3052 */
	...
	if (IS_RT3350(pAd))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): RALINK3350 hook !\n", __FUNCTION__));
		return NDIS_STATUS_SUCCESS;
	}
#endif /* RT3350 */
	...
	if (IS_RT3352(pAd))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): RALINK3352 hook !\n", __FUNCTION__));
		return NDIS_STATUS_SUCCESS;
	}
#endif /* RT3352 */
	...
	if (IS_RT5350(pAd))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): RALINK5350 hook !\n", __FUNCTION__));
		return NDIS_STATUS_SUCCESS;
	}
#endif /* RT5350 */
#endif /* RT305x */
	...
	if (IS_RT5592(pAd))
	{
		...
		if (IS_PCIE_INF(pAd))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Hook RALINK5592 !!!\n"));
		}
#endif /* RTMP_MAC_PCI */
#ifdef RTMP_MAC_USB
		if (IS_USB_INF(pAd))
	...
	if (IS_RT2860(pAd))
	{
		...
		if (IS_PCI_ONLY_INF(pAd))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): RALINK2860 hook !\n", __FUNCTION__));
			return NDIS_STATUS_SUCCESS;
		}
#endif /* RT2860 */
		...
		if (IS_RBUS_INF(pAd))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): RALINK2880 hook !\n", __FUNCTION__));
			return NDIS_STATUS_SUCCESS;
		}
#endif /* RT2880 */
	}
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	switch (Bbp22Value)
	{
...
#endif /* RTMP_MAC_PCI */
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("START TXFRAME\n"));
		...
		case BBP22_TXCONT_OR_CARRSUPP:
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("BBP22_TXCONT_OR_CARRSUPP\n"));
#ifdef RT65xx
				if (IS_RT65XX(pAd))
				{
					UINT32 bbp_val;
				...
				switch (Bbp24Value)
				{
					case BBP24_TXCONT:
						{
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("START TXCONT\n"));
							pATEInfo->bQATxStart = TRUE;
					...
					case BBP24_CARRSUPP:
						{
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("START TXCARRSUPP\n"));
							pATEInfo->bQATxStart = TRUE;
		...
		case BBP22_TXCARR:
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("START TXCARR\n"));
				pATEInfo->bQATxStart = TRUE;
...
INT Set_ADCDump_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (pAd->chipCap.hif_type == HIF_RTMP)
	{
		...
		if (src && *src)
		{
			...
			if (IS_FILE_OPEN_ERR(srcf)) 
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("--> Error opening %s\n", src));
				return FALSE;
			}
			else 
			{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("--> Error src  or srcf is null\n"));
			return FALSE;
		}
		...
		if (IS_RT6352(pAd))
		{
			...
			if ((pATEInfo->bAutoTxAlc == FALSE) && (pATEInfo->bQAEnabled == FALSE))
			{
				if (pATEInfo->bLowTemperature == TRUE)
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s: Current Temperature from BBP_R49=0x%x\n", __FUNCTION__, pAd->CurrTemperature));
				}
			}
			else
#endif /* RTMP_TEMPERATURE_CALIBRATION */
