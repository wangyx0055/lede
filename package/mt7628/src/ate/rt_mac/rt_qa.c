...
#ifdef RELEASE_EXCLUDE
	/* RT3883, RT3352 and RT5350 don't need BbpSoftReset() by TX_METHOD_0 */
	/* RT3883, RT3352 and RT5350 will use TX_METHOD_1 */
#endif /* RELEASE_EXCLUDE */
	if (pATEInfo->TxMethod == TX_METHOD_0)
	{
...
static VOID RTMP_IO_READ_BULK(PRTMP_ADAPTER pAd, UCHAR *dst, UINT32 offset, UINT32 len)
{
	...
	for (index = 0 ; index < (len >> 2); index++)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("mac r 0x%04X=0x%08X\n", offset, Value));
...
VOID CalNoiseLevel(PRTMP_ADAPTER pAd, UCHAR channel, INT32 RSSI[3][10])
{
	...
	/***********************************************************************/	
	{
		...
		if (IS_RT3352(pAd))
		{
			...
			{
				data = (pAd->hw_cfg.lan_gain << 1) + 0x1C;
			}
			else
			{
				data = (pAd->hw_cfg.lan_gain << 1) + 0x24;
			}
	...
	IN  INT Status)
{
	...
	{
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("RACFG command(0x%04x)[magic number(0x%08x)] is done\n", 
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_START\n"));

	pATEInfo->bQAEnabled = TRUE;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("pATEInfo->bQAEnabled = %s\n", (pATEInfo->bQAEnabled)? "TRUE":"FALSE"));
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_STOP\n"));
	...
	*/
...
#endif /* LINUX */
#ifdef VXWORKS
		ret = kill(pATEInfo->AtePid, SIGTERM);
		if (ret)
		{
			ATEDBGPRINT(RT_DEBUG_ERROR, ("%s: unable to signal thread\n", 
			pAd->net_dev->devObject.name));
		}
#endif /* VXWORKS */
	}
	...
	if (pAd->CSRBaseAddress == (PUCHAR)RTMP_MAC_CSR_ADDR)
	{
		/* Prepare feedback as soon as we can to avoid QA timeout. */
		ResponseToGUI(pRaCfg, wrq, sizeof(pRaCfg->status), NDIS_STATUS_SUCCESS);
	}
...
#endif
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RACFG_CMD_AP_START is done !\n"));
	}
	return NDIS_STATUS_SUCCESS;
}
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	if ((offset & 0xFFFF0000) == 0x10000000)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("mac r 0x%08X=0x%08X\n", offset | 0xa0000000, value));
	}
	else
		...
		*/
		...
		if (IS_RT6352(pAd))
		{
			if (offset == RF_CSR_CFG)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("rf%u r R%u=0x%02X\n", bank, regID, rfValue));
			}
			else
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("mac r 0x%04X=0x%08X\n", offset, value));
		...
		if (IS_MT76x0(pAd))
		{
			...
			{
				rfcsr = (RF_CSR_CFG_STRUC)value;
				regID = (UCHAR)(rfcsr.bank_65xx.RF_CSR_REG_ID);
				bank = (UCHAR)(rfcsr.bank_65xx.RF_CSR_REG_BANK);
				rfValue = (UCHAR)(rfcsr.bank_65xx.RF_CSR_DATA);
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("iwpriv %s0 set rf=%u-%u-%02X\n"
					, INF_MAIN_DEV_NAME, bank, regID, rfValue));
			}
			else
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("iwpriv %s0 mac %04X=%08X\n"
		...
		if (IS_RT8592(pAd))
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("mac r 0x%04X=0x%08X\n", offset, value));
		}
		else
#endif /* RT8592 */
		{
	...
	*/
	...
	if (IS_RT6352(pAd))
	{
		if (offset == RF_CSR_CFG)
		{
			...
			bank = (UCHAR)((rfcsr.bank_6352.TESTCSR_RFACC_REGNUM & 0x000003FF) >> 6);
			rfValue = (UCHAR)(rfcsr.bank_6352.RF_CSR_DATA);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("rf%u w R%u=0x%02X\n", bank, regID, rfValue));
		}
		else
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("mac w 0x%04X=0x%08X\n", offset, value));
		}
	}
	else
#endif /* RT6352 */
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("bbp r R%u=0x%02X\n", offset, value));
	pRaCfg->data[0] = value;
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("bbp w R%u=0x%02X\n", offset, value));
	...
	IN struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_QUERY_BF_RSP\n"));
	...
	IN struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_QUERY_BF_TAG %d\n", profileNum));
	...
	IN struct ate_racfghdr *pRaCfg)
{
	...
	*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_QUERY_BF_PROFILE\n"));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("BF Read elasped = %ld usec\n", tval2.tv_usec - tval1.tv_usec));
#endif /* LINUX */
	...
	IN struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_WRITE_EBF_TAG %d\n", profileNum));
	...
	IN struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_WRITE_BF_PROFILE-%d %d\n", startCarrier, numCarriers));
...
#ifdef LINUX
	do_gettimeofday(&tval2);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("BF Write elasped = %ld usec\n", tval2.tv_usec - tval1.tv_usec));
#endif /* LINUX */
...
// TODO: shiang-usw, this function need to revise for new arch.
...
#ifdef LINUX
	do_gettimeofday(&tval1);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("Cal Cap c=%x, t1=%ld t2=%ld\n", capCtrl,
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("DO_RACFG_CMD_ATE_MT76x2_Calibration\n"));				
	...
	if ( length >= 8 )
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("cal_id = %x\n", cal_id));
	}

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Calibration ID = %d\n", cal_id));				

	if ( length >= 12 )
	{
		memcpy(&param0, pRaCfg->data + 6, 4);
		param0 = OS_NTOHL(param0);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("param0 = %x\n", param0));
	}
...
/*		p = (USHORT *)(&pRaCfg->data[TXWISize + TxInfoSize + 2]); */
		...
		{			
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Sending sounding frame\n"));
				pATEInfo->sounding = 1;
...
static  INT DO_RACFG_CMD_TX_STOP(
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_TX_STOP\n"));
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_RX_START\n"));
...
static  INT DO_RACFG_CMD_RX_STOP(
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_RX_STOP\n"));
...
static  INT DO_RACFG_CMD_ATE_START_TX_CARRIER(
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_START_TX_CARRIER\n"));
...
static  INT DO_RACFG_CMD_ATE_START_TX_CONT(
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_START_TX_CONT\n"));
...
static  INT DO_RACFG_CMD_ATE_START_TX_FRAME(
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_START_TX_FRAME\n"));
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("DO_RACFG_CMD_ATE_START_TX_FRAME_V2\n"));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("DO_RACFG_CMD_ATE_START_TX_FRAME_V2 (TxCount = %lu)\n", pATEInfo->TxCount));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("DO_RACFG_CMD_ATE_START_TX_FRAME_V2 (TxLength = %d)\n", pATEInfo->TxLength));
...
{
	SHORT    value = 0;
	RTMP_STRING str[LEN_OF_ARG];

	NdisZeroMemory(str, LEN_OF_ARG);
	
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_SET_BW\n"));				
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_SET_TX_POWER0\n"));				
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_SET_TX_POWER1\n"));				
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_SET_TX_POWER2\n"));				
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_SET_FREQ_OFFSET\n"));				
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("DO_RACFG_CMD_ATE_SET_TSSI_ON_OFF\n"));				
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_GET_STATISTICS\n"));
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_RESET_COUNTER\n"));				
	...
	IN  struct ate_racfghdr *pRaCfg)	
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_SEL_TX_ANTENNA\n"));				
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_SEL_RX_ANTENNA\n"));				
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_SET_PREAMBLE\n"));				
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_SET_CHANNEL\n"));				
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_SET_ADDR1\n"));
#ifdef RELEASE_EXCLUDE
	/*
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_SET_ADDR2\n"));
#ifdef RELEASE_EXCLUDE
	/*
		Addr is an array of UCHAR,
		so no need to perform endian swap.
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_SET_ADDR3\n"));
#ifdef RELEASE_EXCLUDE
	/*
		Addr is an array of UCHAR,
		so no need to perform endian swap.
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_SET_RATE\n"));				
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	value = OS_NTOHS(value);
	value ++;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("DO_RACFG_CMD_ATE_SET_VHT_NSS: %d\n", value));
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_SET_TX_FRAME_LEN\n"));				
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_SET_TX_FRAME_COUNT\n"));				
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_ETH_EXT_SETTING\n"));
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_RUN_CPUBUSY\n"));
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_CALIBRATION\n"));				
	...
	if (IS_MT76x0(pAd))
	{
		switch (cal_id)
		{
			case 0: /* do all calibration */
					MT76x0ATE_Calibration(pAd, value /* Channel */, TRUE, TRUE);
				RtmpusecDelay(1000);
				break;
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_TSSI_COMPENSATION\n"));				
	memcpy((PUCHAR)&value, pRaCfg->data-2, 4);
	value = OS_NTOHL(value);
	...
	switch (value)
	{
		case 0:
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: disable TSSI compensation.\n", __FUNCTION__));
			break;
		case 1:
#ifdef RTMP_INTERNAL_TX_ALC
			/* enable TSSI compensation */
			Set_ATE_AUTO_ALC_Proc(pAd, "1");
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s: enable TSSI compensation.\n", __FUNCTION__));
#else
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RACFG_CMD_ATE_TEMP_COMPENSATION\n"));				
	memcpy((PUCHAR)&value, pRaCfg->data-2, 4);
	value = OS_NTOHL(value);
	...
	switch (value)
	{
		case 0:
			...
			Set_ATE_AUTO_ALC_Proc(pAd, "0");
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: disable temperature compensation.\n", __FUNCTION__));
			break;
		case 1:
#ifdef RTMP_TEMPERATURE_COMPENSATION
			/* enable temperature compensation */
			Set_ATE_AUTO_ALC_Proc(pAd, "1");
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s: enable temperature compensation.\n", __FUNCTION__));
#else
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	for (pos = offset; pos < (offset+len); pos++)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("rf r R%u=0x%02X\n"
			, pos, pRaCfg->data[pos - offset]));
	}
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("\n\n"));
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	for (pos = offset; pos < (offset+len); pos++)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("rf w R%u=0x%02X\n", pos, *value));
	}
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("\n\n"));
...
//		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("rf bank%u r R%u=0x%02X\n"
//			, bank, pos, pRaCfg->data[pos - offset]));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("rf bank%u r R%u=0x%02X\n"
			, bank, pos, pRaCfg->data[pos - offset]));
	}
//	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("\n\n"));
...
//		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("rf bank%u w R%u=0x%02X\n", bank, pos, *value));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("iwpriv %s0 set rf=%u-%u-%02X\n"
...
//		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("rf bank%u r R%u=0x%02X\n"
//			, bank, pos, pRaCfg->data[pos - offset]));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("rf bank%u r 0x%04X=0x%04X\n"
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	*/
	if (OS_NTOHS(pRaCfg->length) != 0)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("TxInfoLen = %d\n", pATEInfo->TxInfoLen));
...
/*		RTMPDescriptorEndianChange((PUCHAR) &pATEInfo->TxInfo, TYPE_TXINFO); */
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("TxWILen = %d\n", pATEInfo->TxWILen));
		...
		pATEInfo->TxCount = OS_NTOHL(pATEInfo->TxCount);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("TxCount = %u\n", pATEInfo->TxCount));

		/* always use QID_AC_BE */
		pATEInfo->QID = 0;
		p = (USHORT *)(&pRaCfg->data[4+2+TXWISize+4+2]);
		pATEInfo->HLen = OS_NTOHS(*p);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("HLen = %d\n", pATEInfo->HLen));
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PLen = %d\n", pATEInfo->PLen));
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("pattern = 0x%02x\n", pATEInfo->Pattern[0]));
...
#ifdef MT7601
			if (IS_MT7601(pAd))
				TxWIMPDUByteCnt = pATEInfo->TxWI.TXWI_X.MPDUtotalByteCnt;
			else
#endif /* MT7601 */
...
#endif /* RLT_MAC */
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Err!!Unknown HIF type(%d)!\n", pAd->chipCap.hif_type));
			TxWIMPDUByteCnt = pATEInfo->HLen;
		}
		pATEInfo->DLen = TxWIMPDUByteCnt - pATEInfo->HLen;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("data length = %d\n", pATEInfo->DLen));
	}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("START TXFRAME V2\n"));
...
static  INT DO_RACFG_CMD_ATE_TXBF_DUT_INIT(
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("DO_RACFG_CMD_ATE_TXBF_DUT_INIT\n"));
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("DO_RACFG_CMD_ATE_TXBF_LNA_CAL\n"));

	band = OS_NTOHS(pRaCfg->status);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : band=0x%x(0x%x)\n", 
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("DO_RACFG_CMD_ATE_TXBF_DIV_CAL\n"));
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("DO_RACFG_CMD_ATE_TXBF_PHASE_CAL\n"));
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("DO_RACFG_CMD_ATE_TXBF_GOLDEN_INIT\n"));
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("DO_RACFG_CMD_ATE_TXBF_VERIFY\n"));
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("DO_RACFG_CMD_ATE_TXBF_VERIFY_NOCOMP\n"));
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("DO_RACFG_CMD_ATE_GET_HW_COUNTER\n"));
...
#if defined(MT76x2) || defined(MT7603) || defined(MT7628) || defined(MT7636)
static RACFG_CMD_HANDLER MT76x2_CMD_SET1[] =
{
	...
	NULL,	/* 0x100D */
	DO_RACFG_CMD_ATE_SET_TX_POWER0,	/* 0x010E */
	DO_RACFG_CMD_ATE_SET_TX_POWER1,	/* 0x010F */
...
#endif /* defined(MT76x2) || defined(MT7603) || defined(MT7628) || defined(MT7636) */
	...
	if (pHeader != NULL)
	{
		if (pHeader->FC.Type == FC_TYPE_DATA)
		{
			if (pRxInfo->U2M)
			{
				pATEInfo->U2M++;
#if defined(RT2883) || defined(RT3883) || defined(RT3593)
				if (pRxWI->RXWI_O.MPDUtotalByteCnt >= 32)
				{
					lastRxRate = (pRxWI->RXWI_O.mcs) + (pRxWI->RXWI_O.bw << 7)
						+ (pRxWI->RXWI_O.sgi << 8) + (pRxWI->RXWI_O.stbc << 9)
						+ (pRxWI->RXWI_O.phy_mode << 14)
						+ (pRxWI->RXWI_O.MPDUtotalByteCnt << 16);
				}
#endif /* defined(RT2883) || defined(RT3883) || defined(RT3593) */
			}
			else
...
#endif /* TXBF_SUPPORT */
		pATEInfo->BF_SNR[0] = BF_SNR_OFFSET + pRxWI->RXWI_O.BF_SNR0;
		pATEInfo->BF_SNR[1] = BF_SNR_OFFSET + pRxWI->RXWI_O.BF_SNR1;
#ifdef DOT11N_SS3_SUPPORT
		pATEInfo->BF_SNR[2] = BF_SNR_OFFSET + pRxWI->RXWI_O.BF_SNR2;
#endif /* DOT11N_SS3_SUPPORT */
	}
#endif /* defined(RT2883) || defined(RT3883) || defined(RT3593) */
...
{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Set_TxStop_Proc\n"));
...
{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Set_RxStop_Proc\n"));
...
INT Set_BBPRead_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%x\n", value));
