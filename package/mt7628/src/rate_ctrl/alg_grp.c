		...
		if (grp_cnt == 3)
		{
			switch (pEntry->mcsGroup)
			{
				...
				case 1:
					...
					break;
				default:
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("3ss:wrong mcsGroup value\n"));
					break;
			}
		}
		else if (grp_cnt == 2)
		{
			switch (pEntry->mcsGroup)
			{
				...
				case 1:
					...
					break;
				default:
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("2ss:wrong mcsGroup value %d\n", pEntry->mcsGroup));
					break;
			}
		}
		else if (grp_cnt == 1)
		{
			switch (pEntry->mcsGroup)
			{
				...
				case 0:
					...
					break;
				default:
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("1ss:wrong mcsGroup value %d\n", pEntry->mcsGroup));
					break;
			}
		} else {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("wrong mcsGroup cnt %d\n", grp_cnt));
		}
	...
	IN CHAR		RssiOffset)
{
					...
					|| pTable == RateTableVht2S_MCS7)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, (DBG_LVL_INFO | DBG_FUNC_RA), ("%s: GRP: 2*2, RssiOffset=%d\n", __FUNCTION__, RssiOffset));
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, (DBG_LVL_INFO | DBG_FUNC_RA), ("%s: GRP: 1*1, RssiOffset=%d\n", __FUNCTION__, RssiOffset));
	...
	IN ULONG			TxErrorRatio)
{
	...
	*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("--Hybrid: low/high/new TP=%ld/%ld/%ld old/new=%ld/%ld--",
    ...
    IN UINT idx)
{
	...
	if ((pEntry->LastSecTxRateChangeAction == RATE_UP) &&
        (((TxTotalCnt <= 15) && (pCurrTxRate->Mode >= MODE_HTMIX)) || ((TxTotalCnt <= 7) && (pCurrTxRate->Mode < MODE_HTMIX))))
	{
	...
	IN CHAR  Rssi)
{
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, (DBG_LVL_INFO | DBG_FUNC_RA),("   QuickDRS: TxSuccess=%ld, TxRetransmit=%ld, TxFail=%ld, RSSI=%d\n",
	...
	*/
	if (TxTotalCnt <= 12)
	{
		/* Go back to the original rate */
		MlmeRestoreLastRate(pEntry);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, (DBG_LVL_INFO | DBG_FUNC_RA),("   QuickDRS: TxTotalCnt <= 12, back to original rate \n"));
	...
	IN ULONG TxFailCount)
{
	...
	else
	{
		...
		if (pEntry->wcid >= 1 && pEntry->wcid <= 8)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE ,("%s():TxErrRatio(Wcid:%d, MCS:%d, Hw:0x%x-0x%x, Sw:0x%x-%x)\n",
					__FUNCTION__, pEntry->wcid, pEntry->HTPhyMode.field.MCS,
					HwTxCnt, HwErrRatio, TxTotalCnt, TxErrorRatio));
...
#ifdef RELEASE_EXCLUDE
	/*
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, (DBG_LVL_INFO | DBG_FUNC_RA), ("DRS: TxSuccess=%lu, TxRetransmit=%lu, TxFailCount=%lu, TxErrorRatio=%lu\n",
	...
	*/
	...
	if (pEntry->fLastChangeAccordingMfb == TRUE)
	{
		pEntry->fLastChangeAccordingMfb = FALSE;
		pEntry->LastSecTxRateChangeAction = RATE_NO_CHANGE;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("DRS: MCS is according to MFB, and ignore tuning this sec \n"));
	...
	*/
	...
	if (pEntry->fLastSecAccordingRSSI == TRUE)
	{
		pEntry->fLastSecAccordingRSSI = FALSE;
		pEntry->LastSecTxRateChangeAction = RATE_NO_CHANGE;
		/* MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("DRS: MCS is according to RSSI, and ignore tuning this sec \n")); */
...
INT Set_RateTable_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	{
	    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Set_RateTable_Proc: Empty MAC Table\n"));
		return FALSE;
	}
	...
	if (*arg != ':') {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Set_RateTable_Proc::%d\n", itemNo));
	}
	else {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Set_RateTable_Proc::%d:%d:%d\n", itemNo, rtIndex, value));
	}

    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%d, 0x%02x, %d, %d, %d, %d, %d, %d, %d, %d\n",
