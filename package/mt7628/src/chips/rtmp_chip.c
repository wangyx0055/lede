...
VOID HWAntennaDiversityEnable(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("HwAnDiv --> Enable!\n"));
}
...
INT AsicGetMacVersion(RTMP_ADAPTER *pAd)
{
	...
	if (WaitForAsicReady(pAd) != TRUE) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() failed!\n", __FUNCTION__));
		return FALSE;
	} 
	...
	if (pAd->MACVersion == 0xffffffff)
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("MACVersion[Ver:Rev]=0x%08x : 0x%08x\n",
