	...
	*/
	...
	if (Channel <= 14)
	{
		for (BBPIndex = 0; BBPIndex <= 7; BBPIndex++)
		{
			...
			if (E2PValue == 0xFFFF)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s() : EEPROM 0x%X is not calibrated !\n", __FUNCTION__, 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() : 5 GHz band not supported !\n", __FUNCTION__));
}
	...
	IN EEPROM_ANTENNA_STRUC	*pAntenna)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("E2PROM error, hard code as 0x%04x\n", pAntenna->word));	
}
