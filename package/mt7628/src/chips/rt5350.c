	...
	IN PRTMP_ADAPTER		pAd)
{
	...
	pChipCap->EEPROM_DEFAULT_BIN = RT5350_EeBuffer;
	...
	IN	PRTMP_ADAPTER pAd)
{
	...
	for (i = 0; i < RT5350_NUM_RF_REG_PARMS; i++)
	{
	  ...
	  if(value!=0xFFFF) { //EEPROM is empty
		if(value & 1<<14) { //TX0: 0=internal PA, 1=external PA
		...
		if(value & 1<<15) { //TX1: 0=internal PA, 1=external PA
		    RT30xxReadRFRegister(pAd, RF_R34, (PUCHAR)&RfReg);
	...
	IN PRTMP_ADAPTER			pAd)
{
	...
	if (ATE_ON(pAd))
	{
		...
		if (IS_RT5350(pAd))
		{
			...
			ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R66, (0x26 + lan_gain));
		}
		else
...
#endif // CONFIG_ATE //
	{
		bbp_set_agc(pAd, (0x26 + lan_gain), RX_CHAIN_ALL);
	}
...
VOID RT5350_ChipBBPAdjust(RTMP_ADAPTER *pAd)
{
	UINT8 rf_bw, ext_ch;
	UCHAR R66 = 0;
...
#if defined(RT5350)
	if (IS_RT5350(pAd) && (pAd->CommonCfg.Channel <= 14))
	{
		CHAR lan_gain = pAd->hw_cfg.lan_gain;
	...
	IN PCHAR 					pDeltaPowerByBbpR1)
{
	...
	if (pAd->Mlme.OneSecPeriodicRound % 4 == 0)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("DesiredTSSI = %d, CurrentTSSI = %d (Range: %d ~ %d, BBP_R49=0x%X)\n",  
		...
		if (bKeepRF == FALSE)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RF_R49 = 0x%X ", RFValue));
		}

		/* Tx power adjustment over MAC */
		TotalDeltaPower = pAd->TxPowerCtrl.MAC_PowerDelta;

		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Current index of TuningTable = %d {RF_TX_ALC = 0x%X, MAC_PowerDelta = %d}\n",
...
INT32 TSSIDelta2PowDelta(UINT32 TSSI_x_10000, UINT32 TSSI_ref)
{
	...
	if (ratio_x <= 1903) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TSSIRatio2Delta: ratio_x(%d) is ourt range(7 ~ -8)\n", ratio_x)); 
		power_delta = -8;
	}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TSSIRatio2Delta: TSSI_x_10000=%u, TSSI_ref=%u ratio_x=%u power_delta=%d\n", TSSI_x_10000, TSSI_ref, ratio_x, power_delta)); 
...
UINT32 TSSIRatioDot85(INT32 delta_power)
{
	...
	switch(delta_power)
	{
		...
		case 12:
			...
			break;
		default:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Invalid delta_power %d\n", delta_power));
			break;
	...
	IN PRTMP_ADAPTER			pAd)
{
    ...
    USHORT Value = 0;
    CHAR BWPowerDelta = 0;
    UCHAR init_channel = 0;

    if (pAd->TxPowerCtrl.bInternalTxALC == FALSE)
    {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("pAd->TxPowerCtrl.bInternalTxALC == FALSE !\n"));
		return;
    }
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: 20/40M BW Delta Power is disabled since EEPROM is not calibrated.\n", __FUNCTION__));
	}
	else
	{
		if ((Value & 0xC0) == 0xC0)
	...
	*/
	if (((E2PValue >> 8) & 0x04) == 0x04)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s : slope of TSSI is 0.7\n", __FUNCTION__));
	}
	else
	{
		pChipOps->TSSIRatio = TSSIRatioDot85;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s : slope of TSSI is 0.85\n", __FUNCTION__));
	}
	...
	for(i=1;i<15;i++)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Channel %d TSSI delta=%d\n", i, TSSI_Set[i].delta));
	}

	/* choose a mid-frequency channel for desired TSSI table initialization. */
	init_channel = 6;
	TSSI_x.delta = TSSI_Set[init_channel].delta;

	RT28xx_EEPROM_READ16(pAd, (EEPROM_OFDM_MCS6_MCS7 - 1), TxPowerOFDM54);
	TxPowerOFDM54 = (0x000F & (TxPowerOFDM54 >> 8));

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TSSIBase(0x6E) = %X, TxPowerOFDM54 = %X\n", TSSIBase, TxPowerOFDM54));

    /* The desired TSSI over CCK */
    RT28xx_EEPROM_READ16(pAd, EEPROM_CCK_MCS0_MCS1, TxPower);
    TxPower = (TxPower & 0x000F);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("EEPROM_CCK_MCS0_MCS1(0xDE) = 0x%X\n", TxPower));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("EEPROM_CCK_MCS2_MCS3(0xDF) = 0x%X\n", TxPower));
    ...
    for (i = 0; i < 4; i++)/* CCK: MCS 0 ~ MCS 3 */
    {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RT5350_desiredTSSIOverCCK[%d] = %d\n", i, RT5350_desiredTSSIOverCCK[i]));
    }

    /* The desired TSSI over OFDM */
    RT28xx_EEPROM_READ16(pAd, EEPROM_OFDM_MCS0_MCS1, TxPower);
    TxPower = (TxPower & 0x000F);
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("EEPROM_OFDM_MCS0_MCS1(0xE0) = 0x%X\n", TxPower));
    ...
    TxPower = ((TxPower >> 8) & 0x000F);
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("EEPROM_OFDM_MCS2_MCS3(0xE1) = 0x%X\n", TxPower));
    ...
    TxPower = (TxPower & 0x000F);
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("EEPROM_OFDM_MCS4_MCS5(0xE2) = 0x%X\n", TxPower));
    ...
    TxPower = ((TxPower >> 8) & 0x000F);
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("EEPROM_OFDM_MCS6_MCS7(0xE3) = 0x%X\n", TxPower));
    ...
    for (i = 0; i < 8; i++)/* OFDM: MCS 0 ~ MCS 7 */
    {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RT5350_desiredTSSIOverOFDM[%d] = %d\n", i, RT5350_desiredTSSIOverOFDM[i]));
    }

    /* The desired TSSI over HT */
    RT28xx_EEPROM_READ16(pAd, EEPROM_HT_MCS0_MCS1, TxPower);
    TxPower = (TxPower & 0x000F);
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("EEPROM_HT_MCS0_MCS1(0xE4) = 0x%X\n", TxPower));
    ...
    TxPower = ((TxPower >> 8) & 0x000F);
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("EEPROM_HT_MCS2_MCS3(0xE5) = 0x%X\n", TxPower));
    ...
    TxPower = (TxPower & 0x000F);
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("EEPROM_HT_MCS4_MCS5(0xE6) = 0x%X\n", TxPower));
    ...
    TxPower = ((TxPower >> 8) & 0x000F);
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("EEPROM_HT_MCS6_MCS7(0xE7) = 0x%X\n", TxPower));
    ...
    for (i = 0; i < 8; i++)/* HT: MCS 0 ~ MCS 7 */
    {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("desiredTSSIOverHT[%d] = %d\n", i, RT5350_desiredTSSIOverHT[i]));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("desiredTSSIOverHT40[%d] = %d\n", i, RT5350_desiredTSSIOverHT40[i]));
    }
	...
	OUT PUCHAR				pBbpR49)
{
	...
	for (i=0;i<100;i++)
	{
		...
		{ 
			...
			if (TssiInfo0 > 0x7C)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TSSI: BBP_R49=%X is native value\n", TssiInfo0));
				*pBbpR49 = TssiInfo0 = 0;
			}
			else
			{
			...
			switch (TssiInfo1 & 0x03)
			{
				case 0: /* CCK */
				    ...
				    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("CCK: desiredTSSI = %d, MCS = %d\n", desiredTSSI, MCS));
					break;
				...
				case 1: /* OFDM */
				    ...
				    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("OFDM: desiredTSSI = %d, MCS = %d\n", desiredTSSI, MCS));
					break;

				case 2: /* HT */
				    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("mixed mode or green-field mode\n"));
				    ...
				    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("HT: desiredTSSI = %d, MCS = %d\n", desiredTSSI, MCS));
					break;
			}
			break;
	    }
