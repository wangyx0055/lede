	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: bMinusBw40ABand = %d, bMinusBw80ABand = %d, bMinusBw40GBand = %d\n", 
		__FUNCTION__, 
		bMinusBw40ABand, 
		bMinusBw80ABand, 
		bMinusBw40GBand));

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: TxPwrBw40ABand = %d, TxPwrBw80ABand = %d, TxPwrBw40GBand = %d\n", 
		__FUNCTION__, 
		TxPwrBw40ABand, 
		TxPwrBw80ABand, 
		TxPwrBw40GBand));
	...
	for (i = 0; i < 5; i++)
	{
		...
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s(MAC:0x%x): data = 0x%X,  DataBw40ABand = 0x%X,  DataBw80ABand = 0x%X, DataBw40GBand = 0x%x\n", 
	...
	IN BOOLEAN bResetWLAN)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("==>%s(): OnOff:%d, pAd->WlanFunCtrl:0x%x, Reg-WlanFunCtrl=0x%x\n",
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("WlanFunCtrl.word = 0x%x\n", WlanFunCtrl.word));
	...
	IN UCHAR Channel)
{
	if (IS_RT8592(pAd))
	{
		...
		//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: IQControl = 0x%04x\n", __FUNCTION__,pAd->IQControl));
...
VOID ReadIQCompensationConfiguraiton(RTMP_ADAPTER *pAd)
{
	...
	if (IS_RT8592(pAd))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IQControl = 0x%04x\n",pAd->IQControl));
...
VOID dump_pwr_info(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tEEPROM Setting\n"));
	for (idx = 0; idx < (sizeof(ee_addlist) / sizeof(USHORT)); idx++)
	{
		RT28xx_EEPROM_READ16(pAd, ee_addlist[idx], ee_val);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\tEEPROM(%x)=0x%04x\n", ee_addlist[idx], ee_val));
	}

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tMAC Setting\n"));
	for (idx = 0; idx < (sizeof(mac_reglist) / sizeof(UINT32)); idx++)
	{
		RTMP_IO_READ32(pAd, mac_reglist[idx], &mac_val);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\tMAC(%x)=0x%08x(%s)\n", mac_reglist[idx], mac_val, pwr_rate_str[idx]));
	}
	
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tRF Setting\n"));
	RT30xxReadRFRegister(pAd, RF_R49, &rf_val);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\tRF_R49(TX0_ALC)=0x%x(0x%x)\n", rf_val & 0x3f, rf_val));
	RT30xxReadRFRegister(pAd, RF_R50, &rf_val);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\tRF_R50(TX1_ALC)=0x%x(0x%x)\n", rf_val & 0x3f, rf_val));
	
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tBBP Setting\n"));
...
INT rx_temp_compensation(RTMP_ADAPTER *pAd)
{
	...
	if (rx_temp_dbg)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): do Rx Temp comp, temper_val=%d\n", __FUNCTION__, temper_val));
	...
	if (rx_temp_dbg)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("\tDepends on temperature value, rx_temp_comp_tb[idx:%d, compensate:%d]\n",
					idx, comp_db));
	...
	if (rx_temp_dbg)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("\tSet AGC1_R8=0x%x\n", bbp_val));
	...
	if (rx_temp_dbg)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("\tSet AGC1_R9=0x%x\n", bbp_val));
...
// TODO: ----End
		...
		if (round != 0) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s():round=%d, pd_cfg=%d\n",
...
static INT rf_tr_agc_config(RTMP_ADAPTER *pAd, INT rf_bw)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): BW Calibration Value=>20MHz:%02x, 40MHz:%02x,80MHz:%02x\n",
				__FUNCTION__, pAd->bw_cal[0], pAd->bw_cal[1], pAd->bw_cal[2]));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("\tApply BW Calibration Value for BW%d =>RF_R32:%02x, RF_R31:%02x!\n",
...
static CHAR lp_tx_filter_bw_cal(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("\t\tCheck CAL_R1(Cnt:%d, Value:0x%x)\n",
			cnt, bbp_val));
	...
	if (cal_val >= 0x40)
		cal_val -= 128;
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("\t\tCAL_R32:0x%x => cal_val=0x%02x\n",
...
INT bw_filter_cal(RTMP_ADAPTER *pAd)
{
	...
	do{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():Start BW Cal for %dMHz\n", 
...
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("\tcal_r32_init=%d\n", cal_r32_init));
#endif /* RELEASE_EXCLUDE */
...
do_cal:
...
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("\t\tR32(Init=%d,Cal=%d,Diff=%d,FilterTarget=%d,TxAgcFc=%d)\n",
		...
		{
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("\t\tBW%dM FilterCal cannot find valid val(loop=%d), set TxAgcFc=0\n",
							(loop == 0 ? 20 : 40), loop));
#endif /* RELEASE_EXCLUDE */
			tx_agc_fc = 0;
		}
		else if ((cal_diff <= filter_target) && (tx_agc_fc < 0x1f))
		{
			tx_agc_fc++;
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("\t\ttx_agc_fc++, redo cal!\n"));
#endif /* RELEASE_EXCLUDE */
			goto do_cal;
		}
		else
		{
			/* do nothing */
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("\t\tget tx_agc_fc, cal done!\n"));
#endif /* RELEASE_EXCLUDE */
		}

#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("\t=>BW %dMHz FilterCal Value=0x%x!\n",
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BW Cal Result =>\n\t20MHz: %02x\n\t40MHz: %02x\n\t80MHz: %02x\n", 
...
{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("-->%s(): Do R Calibration!\n", __FUNCTION__));
...
// TODO: shiang-8592, clear RF_CSR_REG busy bit first and try to work-around it in RT6856 platform
	{
		...
		if (reg_val.word != 0)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Before Init RF, RF_CSR_CFG.word = 0x%x, RF_CSR_CFG.Kick=%d\n",
										reg_val.word, reg_val.non_bank.RF_CSR_KICK));
			RTMP_IO_WRITE32(pAd, RF_CSR_CFG, 0);
			RTMP_IO_READ32(pAd, RF_CSR_CFG, &reg_val.word);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("After write RF_CSR_CFG, read value back=0x%x\n",
										reg_val.word));
		}
	}
// TODO: ---End
	...
	for(IdReg=0; IdReg<rf_init_tb_sz; IdReg++)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("RF_R%02d=%02x\n",
...
//+++Add by shiang for debug
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\trf_bw=%d, ext_ch=%d, PrimCh=%d, HT-CentCh=%d, VHT-CentCh=%d\n",
...
#ifdef DOT11_N_SUPPORT
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("\t%s, ChannelWidth=%d, Channel=%d, ExtChanOffset=%d(%d) \n",
									ext_str[ext_ch],
	...
	BOOLEAN	 bScan)
{
	...
	*/
	...
	if (pAd->chipCap.bTempCompTxALC) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): TempCompTxALC, set RF_R27 Init TssiGain=0x%x\n",
	...
	*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): Set TxAlc for RF_R49/50=0x%x: 0x%x\n",
...
// TODO: End---
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): When Set Channel, AGC1_R8/R9=0x%02x:0x%02x\n",
...
check_tx_again_1:
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (">> 0x10E0 = 0x%08x\n", MacReg));

	RTMP_IO_READ32(pAd, 0x10E4, &MacReg);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (">> 0x10E4 = 0x%08x\n", MacReg));

	RTMP_IO_READ32(pAd, 0x10E8, &MacReg);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (">> 0x10E8 = 0x%08x\n", MacReg));

	RTMP_IO_READ32(pAd, 0x10EC, &MacReg);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (">> 0x10EC = 0x%08x\n", MacReg));

	RTMP_IO_READ32(pAd, 0x10F0, &MacReg);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (">> 0x10F0 = 0x%08x\n", MacReg));

	RTMP_IO_READ32(pAd, 0x10F4, &MacReg);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (">> 0x10F4 = 0x%08x\n", MacReg));

	RTMP_IO_READ32(pAd, MAC_STATUS_CFG, &MacReg);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (">> 0x1200 = 0x%08x\n", MacReg));
...
check_tx_again_2:
	...
	*/
	...
	RTMP_IO_READ32(pAd, 0x0a30, &MacReg);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("0x0a30 = 0x%08x (before disable MAC TX/RX)\n", MacReg));

	RTMP_IO_READ32(pAd, 0x0a34, &MacReg);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("0x0a34 = 0x%08x (before disable MAC TX/RX)\n", MacReg));

	RTMP_IO_READ32(pAd, 0x0430, &MacReg);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("0x0430 = 0x%08x (before disable MAC TX/RX)\n", MacReg));
	...
	RTMP_IO_READ32(pAd, MAC_SYS_CTRL, &MacReg);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("0x1004 = 0x%08x (after disable MAC TX/RX)\n", MacReg));
	
	RTMP_IO_READ32(pAd, 0x0a30, &MacReg);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("0x0a30 = 0x%08x (after disable MAC TX/RX)\n", MacReg));

	RTMP_IO_READ32(pAd, 0x0a34, &MacReg);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("0x0a34 = 0x%08x (after disable MAC TX/RX)\n", MacReg));

	RTMP_IO_READ32(pAd, 0x0430, &MacReg);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("0x0430 = 0x%08x (after disable MAC TX/RX)\n", MacReg));
