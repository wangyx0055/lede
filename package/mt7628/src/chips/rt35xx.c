...
========================================================================
*/
VOID RT35xx_Init(RTMP_ADAPTER *pAd)
{
...
#ifdef RTMP_FLASH_SUPPORT
        pChipCap->EEPROM_DEFAULT_BIN = RT3572_EeBuffer;
#endif /* RTMP_FLASH_SUPPORT */
	...
	pChipCap->FreqCalInitMode = FREQ_CAL_INIT_MODE1;
	pChipCap->FreqCalMode = FREQ_CAL_MODE0;
	pChipCap->RxWIFrqOffset = RXWI_FRQ_OFFSET_FIELD0;
#endif /* CONFIG_STA_SUPPORT */
	...
	if (IS_RT3572(pAd) /*&& (pAd->RfIcType == RFIC_3052)*/)
	{
		for (index = 0; index < NUM_OF_3572_CHNL; index++)
		{
			if (Channel == FreqItems3572[index].Channel)
			{
...
#else
				RT30xxReadRFRegister(pAd, RF_R15, (PUCHAR)&RFValue);
				if  (Channel <= 14)
					RFValue = (RFValue & 0x8F) | 0x50;
				else
					RFValue = (RFValue & 0x8F) | 0x50;
				RT30xxWriteRFRegister(pAd, RF_R15, (UCHAR)RFValue);
#endif
				/* tx_mx1_ic*/
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RT35xx: SwitchChannel#%d(RF=%d, Pwr0=%d, Pwr1=%d, %dT), N=0x%02X, K=0x%02X, R=0x%02X\n",
					Channel, 
					pAd->RfIcType, 
					TxPwer,
					TxPwer2,
	...
	IN UCHAR				BandWidth)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("RT35xx_SetAGCInitValue - Ch=%d, BandWidth=%d, LNA_GAIN=0x%x, set R66 as 0x%x \n", pAd->LatchRfRegs.Channel, BandWidth, pAd->hw_cfg.lan_gain, R66));
	...
	IN UCHAR 			OrigR66Value)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RT35xx_ChipAGCAdjust - Mismatch MACVersion = 0x%x \n", pAd->MACVersion));
		return R66;
	}
...
VOID RT35xx_ChipBBPAdjust(RTMP_ADAPTER *pAd)
{
	UCHAR bbp_val;
	UINT8 rf_bw, ext_ch;
	...
	if (IS_RT3593(pAd))
	{
		...
		bbp_set_txdac(pAd, pAd->Antenna.field.TxPath - 1);
	}
#endif /* RT3593 */
	...
	if (IS_RT3572(pAd)) {
		bbp_val = (pAd->CommonCfg.Channel > 14) ? 0x00 : 0x0A;
...
#ifdef RELEASE_EXCLUDE
/*
	35xx will use this function; therefore, we could not remove the IS_RT3572
*/
#endif /* RELEASE_EXCLUDE */
#ifdef RT35xx
	if(IS_RT3572(pAd))
