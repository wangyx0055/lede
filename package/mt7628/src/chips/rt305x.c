...
UCHAR RT3052_EeBuffer[EEPROM_SIZE] = {
	...
	/* 	
		RT3350 shares the same BBP init table with RT3050/3052.
		Because we can't recognize between RT3350 and 3050R/S/T/T1/T2 without RfIC type.
		The EEPROM routine is initialized after BBP initializtion. 
		Suggestion : Set RT3350 specific register in RTMP_EEPROM_ASIC_INIT.

		The init value of BBP_R105 for RT3350 shall be 0x04.
		But it will be modified for IOT issue(BCM & Atheros at near distance).
		So. we don't set default for BBP_R105.
	*/
...
#if 0
/* Remove this table for RT3350. */
#ifdef RT3350
REG_PAIR   RT3350_BBPRegTable[] = {
	{BBP_R31,		0x08},		//gary recommend for ACE
	{BBP_R78,		0x0E},
	{BBP_R80,		0x08}, // requested by Gary for high power
	{BBP_R103,		0xC0}, 
...
REG_PAIR   RT305x_RFRegTable[] = {
	...
	{RF_R07,		0x60}, //0x50 --> 0x60 for moisture env
	{RF_R08,		0x39}, //
	{RF_R09,		0x0F}, //0x04},
...
VOID NICInitRT305xRFRegisters(IN PRTMP_ADAPTER pAd)
{
		...
		(pAd->RfIcType == RFIC_3021) || (pAd->RfIcType == RFIC_3022))
	{
		...
		if (!IS_RT3350(pAd))
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Not RT3350!\n"));
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Is RT3350!\n"));
...
#ifdef RELEASE_EXCLUDE
	/*
		This is request from Gary about RF_R27 and RF_R28.
		It is for RT3050/RT3052/RT3350.
		(date : 2011.01.19)
	*/
#endif /* RELEASE_EXCLUDE */
	...
	IN RSSI_SAMPLE				*pRssi)
{
	...
	/*
		high-power issue:
		request by Brian, If our 2070/3070/305x-B AP/Station is too close to receive the stations/APs signal,
	...
	*/
		...
		/*
			...
			(Only 2R have High Power issue)
		*/
#endif /* RELEASE_EXCLUDE */
		if ((pAd->Antenna.field.RxPath == 2) && (IS_RT3052B(pAd)))
		{
			if ((pRssi->AvgRssi[0] != 0) && (pRssi->AvgRssi[0] > (pAd->BbpRssiToDbmDelta - 42) ))
			{
	...
	IN PRTMP_ADAPTER		pAd)
{
	...
	if (!IS_RT3350(pAd))
	{
		pChipCap->pBBPRegTable = RT305x_BBPRegTable;
		pChipCap->bbpRegTbSize = RT305x_NUM_BBP_REG_PARMS;
	}
#ifdef RT3350
	else
	{
		pChipCap->pBBPRegTable = RT3350_BBPRegTable;
		RT305x_NUM_BBP_REG_PARMS = (sizeof(RT3350_BBPRegTable) / sizeof(REG_PAIR));
		pChipCap->bbpRegTbSize = RT305x_NUM_BBP_REG_PARMS;
	}
#endif // RT3350 //
#endif
	...
	pChipCap->EEPROM_DEFAULT_BIN = RT3050_EeBuffer;
	if (IS_RT3052(pAd))
		pChipCap->EEPROM_DEFAULT_BIN = RT3052_EeBuffer;
	...
	IN PRTMP_ADAPTER		pAd)
{
	...
	for (IdReg = 0; IdReg < RT305x_NUM_BBP_REG_PARMS; IdReg++)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BBP_R%d=%d\n", RT305x_BBPRegTable[IdReg].Register, 
									RT305x_BBPRegTable[IdReg].Value));
	}

#ifdef RELEASE_EXCLUDE
	/*
		Fix bad throughput issue with BCM & Atheros at near distance.
		For 1x1 designs(3350), R105=0x31. But for 2x2 (3352, etc), then we should set R105=0x35, to enable MLD for 2SS Rx
	*/
#endif /* RELEASE_EXCLUDE */
	...
	IN PRTMP_ADAPTER 		pAd)
{
	...
	if (pLatestTxHTSetting->field.MODE == MODE_CCK)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: incorrect MCS: MCS = %d\n", __FUNCTION__, MCS));
			MCS = 0;
		}
	...
	IN PCHAR 					pDeltaPowerByBbpR1)
{
	...
	if ((pAd->TxPowerCtrl.bInternalTxALC == TRUE) && (IS_RT3350(pAd)))
	{
		if ((pAd->Mlme.OneSecPeriodicRound % 4 == 0) && (*pDeltaPowerByBbpR1 == 0))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: desiredTSSI = %d, currentTSSI = %d, idxTxPowerTable = %d, TuningTableIndex = %d, {RF_TX_ALC = %d, MAC_PowerDelta = %d}\n", 
				__FUNCTION__, 
				desiredTssi, 
				currentTssi, 
				pAd->TxPowerCtrl.idxTxPowerTable, 
