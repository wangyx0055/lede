...
#if 0
#define RTMP_OS_FREE_TIMER(__pAd)											\
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: free timer resources!\n", __FUNCTION__));\
	RTMP_OS_Free_Rscs(&((__pAd)->RscTimerMemList))
#define RTMP_OS_FREE_TASK(__pAd)											\
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: free task resources!\n", __FUNCTION__));	\
	RTMP_OS_Free_Rscs(&((__pAd)->RscTaskMemList))
#define RTMP_OS_FREE_LOCK(__pAd)											\
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: free lock resources!\n", __FUNCTION__));	\
	RTMP_OS_Free_Rscs(&((__pAd)->RscLockMemList))
#define RTMP_OS_FREE_TASKLET(__pAd)											\
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: free tasklet resources!\n", __FUNCTION__));\
	RTMP_OS_Free_Rscs(&((__pAd)->RscTaskletMemList))
#define RTMP_OS_FREE_SEM(__pAd)												\
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: free semaphore resources!\n", __FUNCTION__));\
	RTMP_OS_Free_Rscs(&((__pAd)->RscSemMemList))
#define RTMP_OS_FREE_ATOMIC(__pAd)												\
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: free atomic resources!\n", __FUNCTION__));\
...
#define RTMP_IRQ_UNLOCK(__lock, __irqflags)			\
{													\
	spin_unlock_irqrestore((spinlock_t *)(__lock), __irqflags);			\
}
#else
#define RTMP_IRQ_LOCK(__lock, __irqflags)		\
{												\
	if(__irqflags != 0)                                             \
		RtmpOsSpinLockIrqSave(__lock, &__irqflags);		\
    else                                                            \
		RtmpOsSpinLockBh(__lock);					\
}
...
{												\
	if(__irqflags != 0)                                                     \
        RtmpOsSpinUnlockIrqRestore(__lock, &__irqflags);      \
    else                                                                    \
		RtmpOsSpinUnLockBh(__lock);					\
}
