...
#define TRACE_USB_BULK_IN_INFO trace_usb_bulk_in_info
TRACE_EVENT(usb_bulk_in_info,
	TP_PROTO(RTMP_ADAPTER *pAd, CHAR *function, PRX_CONTEXT pRxContext, int ret
	),

	TP_ARGS(pAd, function, pRxContext, ret
	),

	TP_STRUCT__entry(
		__string(func, function)
		__field(s32, ret)
		__field(u32, bulk_in_addr)
		__field(u32, bulk_in_offset)
	),
	...
	TP_fast_assign(
		...
		__entry->ret = ret;
		__entry->bulk_in_addr = pChipCap->DataBulkInAddr;
		__entry->bulk_in_offset = pRxContext->BulkInOffset;
	),
	
	TP_printk("function = %s\n\
			   ret = %d\n\
			   bulk in addr = %d\n\
			   bulk in offset = 0x%x\n\
			  ",
				__get_str(func),
				__entry->ret,
				__entry->bulk_in_addr,
				__entry->bulk_in_offset
	)
);


#define TRACE_USB_BULK_IN_RX_COMPLETE trace_usb_bulk_in_rx_complete
TRACE_EVENT(usb_bulk_in_rx_complete,
	TP_PROTO(CHAR *function, UINT32 status, UINT32 transfer_len, UINT32 actual_len
	),

	TP_ARGS(function, status, transfer_len, actual_len
	),
	...
	TP_fast_assign(
		...
		__entry->actual_len = actual_len;
	),

	TP_printk("function = %s\n\
			   USB bulk in completie(Status = %d)\n\
			   USB transfer length = 0x%x\n\
			   USB actual length = 0x%x\n",
			__get_str(func),
			__entry->status,
...
#define TRACE_USB_BULK_IN_RECEIVE trace_usb_bulk_in_receive
TRACE_EVENT(usb_bulk_in_receive,
	TP_PROTO(RTMP_ADAPTER *pAd, CHAR *function, UINT8 stage, PRX_CONTEXT pRxContext
	),

	TP_ARGS(pAd, function, stage, pRxContext
	),
	...
	TP_STRUCT__entry(
		...
		__field(u8, stage)
		__field(u32, bulk_in_offset)
		__field(u32, read_position)
		__field(u32, transfer_buffer_len)
	),
	...
	TP_fast_assign(
		...
		__entry->stage = stage;
		__entry->bulk_in_offset = pRxContext->BulkInOffset;
		__entry->read_position = pRxContext->ReadPosition;
		__entry->transfer_buffer_len = pAd->TransferBufferLength;
	),

	TP_printk("function = %s\n\
			   USB bulk in receive stage = %d\n\
			   bulk in offset = 0x%x\n\
			   read position= 0x%x\n\
			   transfer buffer len = 0x%x",
				__get_str(func),
				__entry->stage,
				__entry->bulk_in_offset,
				__entry->read_position,
				__entry->transfer_buffer_len
	)
);


#define TRACE_USB_BULK_QLEN trace_usb_bulk_qlen
TRACE_EVENT(usb_bulk_qlen,
	TP_PROTO(RTMP_ADAPTER *pAd, CHAR *function, UINT32 qindex, MT_DL_LIST *list
	),

	TP_ARGS(pAd, function, qindex, list
	),
	...
	TP_fast_assign(
		...
		__entry->qlen = RTUSBQueueLen(pAd, list);
	),
	...
	TP_printk("function = %s\n\
			   ...
			   qlen = 0x%x",
				__get_str(func),
				__print_symbolic(__entry->qindex,
								{0, "RxFreeQ"},	
								{1, "RxBulkInQ"},
								{2, "RxProcessingQ"}),
				__entry->qlen
