...
struct wifi_dev{
	...
	/* 7636 psm */	USHORT Psm;		/* power management mode   (PWR_ACTIVE|PWR_SAVE), Please use this value to replace  pAd->StaCfg.Psm in the future*/
	BOOLEAN bBeaconLost;
	UINT8	ucDtimPeriod;
...
#define SDIO_EVENT_CMD              BIT(0)
#define SDIO_EVENT_INT              BIT(1)
#define SDIO_EVENT_TXREQ            BIT(2)
#define SDIO_EVENT_TX_DATA_REQ      BIT(2)
#define SDIO_EVENT_CMD_BIT          (0)
#define SDIO_EVENT_INT_BIT          (1)
#define SDIO_EVENT_TXREQ_BIT        (2)
#define SDIO_EVENT_TX_DATA_REQ_BIT  (3)
#define SDIO_EVENT_TX_BCN_BIT       (4)

#define SDIO_RX_RFB_NUM 40
...
typedef enum _ENUM_TRAFFIC_CLASS_INDEX_T {
    ...
    TC4_INDEX,       /* HIF TX: BMC packets */
    TC5_INDEX,       /* HIF TX: AC4 (MGMT) packets */
    TC6_INDEX,       /* HIF TX: CPU packets */
    TC7_INDEX,       /* HIF TX: BCN packets */
	/* 2nd HW interface not used currently */
...
#if 0 /* soumik - unused in 7636 */
    UINT16                  au2FreeBufferCount[TC_NUM];
    UINT16                  au2MaxNumOfBuffer[TC_NUM];
#endif
#ifdef MT_SDIO_ADAPTIVE_TC_RESOURCE_CTRL
    	UINT16 au2GuaranteedTcResource[TC_NUM]; /* The minimum amount of resource when extremely busy */
    	//UINT16 au2MinReservedTcResource[TC_NUM];
	UINT16 au2CurrentTcResource[TC_NUM];
	UINT16 au2AvgDemand[TC_NUM];
	//UINT16 au2acVariation[TC_NUM];
	UINT16 u2TimeToUpdateQueLen;
	UINT16 u2TimeToAdjustTcResource;
	BOOLEAN fgTcResourcePostAnnealing;
	BOOLEAN fQuotaAdjustFast;
...
typedef struct _RX_CTRL_T {
    UINT32                 u4Reserved;
//#ifdef CFG_SDIO_RX_AGG
#if 1
    PUINT8                 pucRxCoalescingBufPtr;
#endif
	QUEUE_HEADER           	rFreeSwRfbList;
	QUEUE_HEADER           	rReceivedRfbList;
	QUEUE_HEADER			rIndicatedRfbList;
	NDIS_SPIN_LOCK rFreeSwRfbLock;
	NDIS_SPIN_LOCK rReceivedRfbLock;

} RX_CTRL_T, *P_RX_CTRL_T;

typedef  struct _SW_RFB_T {
	QUEUE_ENTRY  rQueEntry;
...
#ifdef RTMP_SDIO_SUPPORT
	UINT32 BlockSize;
    	RTMP_OS_TASK rSDIOTask;
    	//RTMP_OS_ATOMIC rTaskEvent;
    	volatile unsigned long u4TaskEvent;

	//NDIS_SPIN_LOCK SdioWorkTaskLock;
	//MT_DL_LIST SdioWorkTaskList;
	//NDIS_SPIN_LOCK IntStatusLock;
	NDIS_SPIN_LOCK TcCountLock;
	UINT32 IntStatus;
	PNDIS_PACKET SDIORxPacket;
	BOOLEAN fgSDIORxPacKetInUsed;
	UINT32                 u4CoalescingBufCachedSize;
	PUINT8                 pucCoalescingBufCached;
#if (CFG_SDIO_INTR_ENHANCE == 1)
    P_ENHANCE_MODE_DATA_STRUCT_T    penhanced_status;
#endif
    	//struct cmd_msg *prPendingTxDoneMsg;
	//NDIS_SPIN_LOCK SDIOTxPacketListLock[NUM_OF_TX_RING];
	//MT_DL_LIST SDIOTxPacketList[NUM_OF_TX_RING];
	TX_CTRL_T rTxCtrl;
	RX_CTRL_T rRxCtrl;
#endif
...
/*********************************************************/
	...
	MT_DL_LIST RxProcessingQ;
	NDIS_SPIN_LOCK RxBulkInQLock;
	MT_DL_LIST RxBulkInQ;
...
/*=======Common=========== */
	...
	*/
	...
	TRACE_TABLE_IPARP iparp_trace_tx;
	TRACE_TABLE_IPARP iparp_trace_rx;
...
{
	UINT32 length;
	UCHAR  data[RX_80211_PKT_SIZE];
}RX_PKT;

typedef struct _RX_PKT_QUEUE
{
	INT32 head;  // read place 
	INT32 tail; // write place 
	RX_PKT buf[RX_QUEUE_SIZE];
}RX_PKT_QUEUE ,*PRX_PKT_QUEUE;
...
VOID _RTMPDeQueuePacket(
	...
	IN INT wcid,
	IN INT Max_Tx_Packets);
VOID MTAdaptiveResourceAllocation(
	IN RTMP_ADAPTER *pAd,
	IN INT wcid,
	IN UCHAR queue);

VOID MTAdaptiveResourceCheckFastAdjustment(
			IN RTMP_ADAPTER *pAd,
			IN INT wcid,
			IN UCHAR queue);
VOID MTAdjustTcQuotas(
	IN RTMP_ADAPTER *pAd);
#endif
