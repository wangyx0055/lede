	...
	IN PVOID SystemSpecific3)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("ASSOC - enqueue MT2_ASSOC_TIMEOUT \n"));
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("ASSOC - enqueue MT2_REASSOC_TIMEOUT \n"));
#endif /* RELEASE_EXCLUDE */
	/* Do nothing if the driver is starting halt state. */
	/* This might happen when timer already been fired before cancel timer with mlmehalt */
	if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_HALT_IN_PROGRESS | fRTMP_ADAPTER_NIC_NOT_EXIST))
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("ASSOC - enqueue MT2_DISASSOC_TIMEOUT \n"));
#endif /* RELEASE_EXCLUDE */
	/* Do nothing if the driver is starting halt state. */
	/* This might happen when timer already been fired before cancel timer with mlmehalt */
	if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_HALT_IN_PROGRESS | fRTMP_ADAPTER_NIC_NOT_EXIST))
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	else if (MlmeAssocReqSanity(pAd, Elem->Msg, Elem->MsgLen, ApAddr, &CapabilityInfo, &Timeout, &ListenIntv))
	{
		...
		/*
		   ...
		   We don't need to do below action here.
		 */
...
/* #endif */
	...
	else if (P2P_INF_ON(pAd))
	{
		...
		if (pAd->P2pCfg.P2pManagedParm.TotalNumOfP2pAttribute > 0)
		{
			...
			FrameLen += tmp;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ASSOCREQ - Insert P2P IE in case for managed AP to recognize me\n"));
		}
	}
#endif /* P2P_SUPPORT */
	...
	else if (MlmeAssocReqSanity(pAd, Elem->Msg, Elem->MsgLen, ApAddr, &CapabilityInfo, &Timeout, &ListenIntv))
	{
		...
		if (pAd->MlmeAux.APEdcaParm.bValid) {
			if (pAd->StaCfg.wdev.UapsdInfo.bAPSDCapable
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("uapsd> MaxSPLength = %d!\n", QosInfo.MaxSPLength));
				WmeIe[8] |= *(PUCHAR) & QosInfo;
			}
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	USHORT Status;
        ...
        //coex hook disconnect 
       MLMEHook(pAd, WLAN_DISCONNECT,QueryHashID(pAd, pAd->CommonCfg.Bssid, FALSE));
			       ...
			       &CkipFlag, ie_list))
	{
		...
		if (MAC_ADDR_EQUAL(Addr2, pAd->MlmeAux.Bssid)) {
			...
			if (pAd->StaCfg.Dot11RCommInfo.bFtSupport && pAd->MlmeAux.FtIeInfo.Len) {
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():ASSOC - FTIE\n", __FUNCTION__));
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
...
//+++Add by shiang for debug
				// TODO: shiang-usw, we need more solid way to do this!
				{
					struct wifi_dev *wdev = &pAd->StaCfg.wdev;
...
//---Add by shiang for debug
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ASSOC - %s() sanity check fail\n", __FUNCTION__));
	}
...
VOID PeerDisassocAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ASSOC - PeerDisassocAction()\n"));
...
#ifdef WMM_ACM_SUPPORT
			ACMP_StationDelete(pAd,
					   ACMR_STA_ENTRY_GET(pAd,
							      pAd->CommonCfg.Bssid));
#endif /* WMM_ACM_SUPPORT */
...
{
	USHORT Status;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ASSOC - AssocTimeoutAction\n"));
	pAd->Mlme.AssocMachine.CurrState = ASSOC_IDLE;
...
{
	USHORT Status;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ASSOC - ReassocTimeoutAction\n"));
	pAd->Mlme.AssocMachine.CurrState = ASSOC_IDLE;
	Status = MLME_REJ_TIMEOUT;
...
{
	USHORT Status;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ASSOC - DisassocTimeoutAction\n"));
	pAd->Mlme.AssocMachine.CurrState = ASSOC_IDLE;
	Status = MLME_SUCCESS;
