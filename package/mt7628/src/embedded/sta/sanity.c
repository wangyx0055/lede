	...
	OUT UCHAR *pSsidLen)
{
	...
	if (Info->SsidLen > MAX_LEN_OF_SSID) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): fail - wrong SSID length\n",
									__FUNCTION__));
		return FALSE;
	}
	...
	while ((Length + 2 + pEid->Len) <= MsgLen)
	{
		switch (pEid->Eid)
		{
			...
			case IE_VENDOR_SPECIFIC:
				...
				/* handle WME PARAMTER ELEMENT */
				WscCheckWpsIeFromWpsAP(pAd, pEid, NULL, NULL);
			...
			case IE_FT_FTIE:
				...
				if (pAd->StaCfg.Dot11RCommInfo.bFtSupport) {
					if (pEid->Len >= sizeof (FT_FTIE)) {
						...
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():pEid->Len(%d) < sizeof(FT_FTIE)\n", __FUNCTION__, pEid->Len));
				}
				break;
			case IE_RSN:
...
#if 0
		/*if (*IdxPtr) */
		/*    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("TIM bitmap = 0x%02x\n", *IdxPtr)); */
#endif
