	...
	IN PVOID SystemSpecific3)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("SYNC - Scan Timeout \n"));
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	*/
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("MlmeScanReqAction SDIO Ctl->Stage != FW_RUN_TIME \n"));
		return;
	}
#endif
#ifdef PCIE_PS_SUPPORT
...
#define WAC_AP		0x04
		...
		if (pAd->StaCfg.WfdCfg.bSuppInsertWfdIe)
		{
			...
			MlmeFreeMemory(pAd, pOutBuffer);
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("SYNC - send ProbeReq @ channel=%d, Len=%ld\n", pAd->MlmeAux.Channel, FrameLen));
#endif /* RELEASE_EXCLUDE */
		}
	} while (FALSE);
	...
	IN MLME_QUEUE_ELEM *Elem)
{
						...
						ie_list, &LenVIE, pVIE, FALSE) == TRUE)
	{
			...
			&& ((ie_list->Channel > 0) && (ie_list->Channel <= 14)))
		{
			...
			{
#ifdef RELEASE_EXCLUDE
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s():Check Channel(%d) with ChList[Idx=%d, Ch=%d, bEffected=%d]",
								...
								TRUE))
	{
		...
		*/
		if (MAC_ADDR_EQUAL(pAd->MlmeAux.Bssid, &ie_list->Bssid[0]))
		{
			...
			{
				int p2pIdx;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Recv desired Beacon, update SSID[%d][%s]\n",
			...
			*/
				...
				(WMODE_CAP_N(pAd->CommonCfg.PhyMode) && bAllowNrate))
			{
					...
					(ie_list->vht_cap_len))
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): CentralChannel=>%d\n",
			...
			if (pAd->StaCfg.BssType == BSS_INFRA)
			{
				...
				if (BwFallBack == 1)
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("P2P STA connection to 40MHz GO, but Infra extra and P2P Group extra is different!!!\n"));
...
MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("-->%s():INFRA_ON=%d, jiffies=0x%x, OS_HZ=%d\n", __FUNCTION__, INFRA_ON(pAd), now_time, OS_HZ));
}
//---Add by shiang for debug
#endif
...
#ifdef RELEASE_EXCLUDE
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("SYNC - New AP added to SsidBssTab[%ld], RSSI=%d, MAC=%02x:%02x:%02x:%02x:%02x:%02x\n",
...
//                        DBGPRINT(RT_DEBUG_OFF, ("%s PASSIVE SCANNING: %s [%d]\n", __FUNCTION__, bcn_ie_list->Ssid, Bssidx));
		...
		*/
		if ((! is_my_bssid) && ADHOC_ON(pAd))
		{
			...
			{
#ifdef RELEASE_EXCLUDE
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("SYNC - Not matched wep status %d %d or auth mode %d %d\n",
								wdev->WepStatus, pAd->ScanTab.BssEntry[Bssidx].WepStatus,
								wdev->AuthMode, pAd->ScanTab.BssEntry[Bssidx].AuthMode));
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("bssid=%s\n", pAd->ScanTab.BssEntry[Bssidx].Bssid));
#endif /* RELEASE_EXCLUDE */
				goto LabelOK;
			}
#if 0
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" !!! 1. merge Adhoc !!!\n"));
				bMerge = TRUE;
			}
			else if (Elem->TimeStamp.u.HighPart == TimeStamp.u.HighPart)
			{
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" !!! 2. merge Adhoc (delta = %d)!!!\n", TimeStamp.u.LowPart - Elem->TimeStamp.u.LowPart));
					bMerge = TRUE;
				}
			}
			...
			if (bMerge)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("SYNC - merge to the IBSS with bigger TSF (BSSID=%02x:%02x:%02x:%02x:%02x:%02x)\n",
								PRINT_MAC(Bssid)));
...
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("SYNC - PeerBeacon from %02x:%02x:%02x:%02x:%02x:%02x - Dtim=%d/%d, Rssi=%02x\n",
			PRINT_MAC(bcn_ie_list->Bssid), bcn_ie_list->DtimCount, bcn_ie_list->DtimPeriod, RealRssi));
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("SYNC - my beacon from %02x:%02x:%02x:%02x:%02x:%02x - Dtim=%d/%d, Rssi=%d, Rssi2=%d, Rssi3=%d\n",
				...
				&& (pAd->StaCfg.IWscInfo.bBlockConnection == FALSE)
			...
			)
                    {
						...
						if (pEntry->bPeerHigherMAC == FALSE)
						{
							...
							*/
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ADHOC - EnqueueStartForPSKTimer.\n"));
				...
				if (pEntry && IS_ENTRY_CLIENT(pEntry))
				{
					...
					if (pEntry->bUpdateInfoFromPeerBeacon == FALSE)
					{
						if (StaUpdateMacTableEntry(pAd,
												...
												bcn_ie_list->CapabilityInfo) == FALSE)
						{
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ADHOC 2 - Add Entry failed.\n"));
							return;
						}
...
/*7636 psm*/
#if defined(MT7636)	
			/* MT7603/MT7636 STA Legacy PS and WMM PS is offload to FW */
#else
			if ((INFRA_ON(pAd) && (RtmpPktPmBitCheck(pAd) == TRUE)) || (pAd->CommonCfg.bAPSDForcePowerSave))
			{
				...
				*/
				...
				{
#ifdef RELEASE_EXCLUDE
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("SYNC - AP backlog unicast-to-me, stay AWAKE, send PSPOLL\n"));
#endif /* RELEASE_EXCLUDE */
#ifdef PCIE_PS_SUPPORT
					if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_ADVANCE_POWER_SAVE_PCIE_DEVICE))
					{
...
#endif /* PCIE_PS_SUPPORT */
#ifdef RELEASE_EXCLUDE
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("SYNC - AP backlog broadcast/multicast, stay AWAKE\n"));
#endif /* RELEASE_EXCLUDE */
				}
...
#ifdef RELEASE_EXCLUDE
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("SYNC - outgoing frame in TxRing/MgmtRing, stay AWAKE\n"));
#endif /* RELEASE_EXCLUDE */
#ifdef PCIE_PS_SUPPORT
					if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_ADVANCE_POWER_SAVE_PCIE_DEVICE))
					{
...
#ifdef RELEASE_EXCLUDE
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("SYNC - PHY sleeps for %d TBTT, Dtim=%d/%d\n", TbttNumToNextWakeUp, bcn_ie_list->DtimCount, bcn_ie_list->DtimPeriod));
#endif /* RELEASE_EXCLUDE */
...
NdisGetSystemUpTime(&now_time);
MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("<--%s(), jiffies=0x%x\n", __FUNCTION__, now_time));
}
//---Add by shiang for debug
#endif
...
#endif
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("SYNC - Send PROBE_RSP to %02x:%02x:%02x:%02x:%02x:%02x...\n",
	...
	*/
	...
	)
	{
		...
		if(NStatus == NDIS_STATUS_SUCCESS)
		{
			...
			MlmeFreeMemory(pAd, pOutBuffer);
			OS_WAIT(20);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ScanTimeoutAction():Send PWS NullData frame to notify the associated AP!\n"));
		}
		else
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): Cannot get memory to send NullFrame out!\n", __FUNCTION__));
#endif
	}
#endif /* RTMP_MAC_USB */
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("SYNC - send PsPoll ...\n"));
#endif /* RELEASE_EXCLUDE */
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BuildEffectedChannelList:CtrlCh=%d,CentCh=%d,AuxCtrlCh=%d,AuxExtCh=%d\n",
								pAd->CommonCfg.Channel, pAd->CommonCfg.CentralChannel,
