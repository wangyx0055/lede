	...
	IN	BOOLEAN	bEnable)
{
	...
	if (pAd->CommonCfg.bWmmCapable)
	{
		...
		if (pAd->StaCfg.wdev.UapsdInfo.bAPSDCapable)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("tdls uapsd> UAPSD %d %d %d %d %d!\n",
	...
	IN	UCHAR						*pPeerMac)
{
	...
	if (pMacEntry == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("tdls_cmd> ERROR! No such peer in %s!\n",
				__FUNCTION__));
		return;
	}
