	...
	IN UCHAR TargetChannelBW)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Can't find TDLS entry on Mac TABLE !!!!\n"));
	}
	...
	IN UCHAR DtimCount)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s - AP Prohibite TDLS Channel Switch !!!\n", __FUNCTION__));
		return;
	}
	...
	if (bCanDoChSwitch)
	{
		if (DtimCount == 0)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("101. %ld !!!\n", (jiffies * 1000) / OS_HZ));
			...
			if (pTdlsEntry->Valid && pTdlsEntry->bDoingPeriodChannelSwitch)
			{
...
#ifdef RTMP_MAC_USB
				RTMPCancelTimer(&pTdlsControl->TdlsPeriodGoBackBaseChTimer, &TimerCancelled);
				RTMPSetTimer(&pTdlsControl->TdlsPeriodGoBackBaseChTimer, (pAd->CommonCfg.BeaconPeriod  - 25));
#endif /* RTMP_MAC_USB */
				...
				if (NStatus != NDIS_STATUS_SUCCESS) 
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s() - Build Channel Switch Request Fail !!!\n", __FUNCTION__));
				else
    ...
    IN MLME_QUEUE_ELEM *Elem)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s - TDLS only support infra mode !!!\n", __FUNCTION__));
	}
    ...
    IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (INFRA_ON(pAd))
	{
		...
		if (NStatus != NDIS_STATUS_SUCCESS)	
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Build Channel Switch Response Fail !!!\n", __FUNCTION__));
		}
		else
		{
			RtmpusecDelay(300);
    ...
    IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (pAd->StaActive.ExtCapInfo.TDLSChSwitchProhibited == TRUE)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s - AP Prohibite TDLS Channel Switch !!!\n", __FUNCTION__));
		return;
	}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s():  from %02x:%02x:%02x:%02x:%02x:%02x !!!\n",
						__FUNCTION__, PRINT_MAC(PeerAddr)));

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("300. %ld !!!\n", (jiffies * 1000) / OS_HZ));
...

	{
		...
		for (MTxCycle = 0; MTxCycle < 50; MTxCycle++)
		{
			...
			if (TxCount > 0)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("MTxCycle = %d, %ld !!!\n", MTxCycle, (jiffies * 1000) / OS_HZ));
				break;
			}
			else
				RtmpusecDelay(1000);
		...
		if (MTxCycle >= 50)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - TDLS Transmit Channel Switch Response Fail !!!\n", __FUNCTION__));
		}
	}
	...
	if (NStatus == NDIS_STATUS_SUCCESS)
	{
		if  (TargetChannel != pAd->CommonCfg.Channel)
		{
			BOOLEAN TimerCancelled;
#ifdef RTMP_MAC_USB
			LONG Now, GapJiffies, BPtoJiffies, GapTimes;
#endif /* RTMP_MAC_USB */
			...
			if (RTDebugLevel < DBG_LVL_WARN)
				RtmpusecDelay(300);
			else
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("1041. %ld !!!\n", (jiffies * 1000) / OS_HZ));
			...
			GapJiffies = (pAd->StaCfg.LastBeaconRxTime + BPtoJiffies - Now);
			GapTimes = (GapJiffies * 1000) / OS_HZ;

			if (GapTimes > 20)
				RTMPSetTimer(&pTdlsControl->TdlsPeriodGoBackBaseChTimer, GapTimes - 20);
			else
				RTMPSetTimer(&pTdlsControl->TdlsPeriodGoBackBaseChTimer, 1);
    ...
    IN MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s() - from %02x:%02x:%02x:%02x:%02x:%02x !!!\n",
					__FUNCTION__, PRINT_MAC(PeerAddr)));
	...
	if (!PeerTdlsChannelSwitchRspSanity(pAd,
								...
								&PeerChSwitchTimeOut))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - Sanity Check Fail !!!\n", __FUNCTION__));
		return;
	}
	...
	if (LinkId == -1 || LinkId == MAX_NUM_OF_TDLS_ENTRY)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - can not find the MAC on TDLS Table !!!\n", __FUNCTION__));
		return;
	}
	...
	if (StatusCode == MLME_REQUEST_DECLINED)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s() - received a failed StatusCode = %d !!!\n",
						__FUNCTION__, StatusCode ));
		return;
	}
	...
	if (StatusCode == MLME_SUCCESS)
	{
		if (pTDLS->ChannelSwitchCurrentState == TDLS_CHANNEL_SWITCH_NONE)
		{
			if (pTdlsControl->bChannelSwitchInitiator)
			{
				...
				RTMPCancelTimer(&pTdlsControl->TdlsPeriodGoBackBaseChTimer, &TimerCancelled);
#endif /* RTMP_MAC_USB */
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s() - i am Initiator !!!\n", __FUNCTION__));
			}
			else
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s() - i am responder!!!\n",  __FUNCTION__));
			}
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("106. %ld !!!\n", (jiffies * 1000) / OS_HZ));
		...
		else
		{
			if (pTdlsControl->TdlsDesireChannel != pAd->CommonCfg.Channel)
			{
				if (pTdlsControl->bChannelSwitchInitiator)
				{
					...
					if (RTDebugLevel < DBG_LVL_WARN)
						...
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("104. %ld !!!\n", (jiffies * 1000) / OS_HZ));
					TDLS_DisablePktChannel(pAd, FIFO_EDCA);
			...
			else
			{
				...
				if (pTdlsControl->bChannelSwitchInitiator)
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s() - i am channel switch Initiator !!!\n", __FUNCTION__));
				}
				else
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s() - i am channel switch responder!!!\n",  __FUNCTION__));
				}
...
#endif
#if 0
		NStatus = TDLS_SendOutActionFrame(pAd,
										&pAd->MacTab.Content[pTDLS->MacTabMatchWCID],
										Header802_3,
										LENGTH_802_3,
										pOutBuffer,
				...
				(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_RESET_IN_PROGRESS)))
			{
				for(Index = 0; Index < 5; Index ++)
					if(pAd->TxSwQueue[Index].Number > 0)
						RTMPDeQueuePacket(pAd, FALSE, Index, WCID_ALL, MAX_TX_PROCESS);
			}
		}
	} while (FALSE);
	...
	IN PVOID SystemSpecific3) 
{
	...
	if ((pAd != NULL) && INFRA_ON(pAd))
	{
		...
		if (pAd->StaCfg.TdlsInfo.TdlsAsicOperateChannel != pAd->CommonCfg.Channel)
		{
			...
			if (pTdlsControl->bChannelSwitchInitiator)
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("200. Initiator %ld !!!\n", (jiffies * 1000) / OS_HZ));
			else
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("201. Responder %ld !!!\n", (jiffies * 1000) / OS_HZ));
		...
		{
			TDLS_EnablePktChannel(pAd, FIFO_HCCA | FIFO_EDCA);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("I am lready on Base Channel :(%d) !!!\n", pAd->CommonCfg.Channel));
		}
	}
	...
	if ((pAd != NULL) && INFRA_ON(pAd))
	{
		...
		for (idx = 0; idx < MAX_NUM_OF_TDLS_ENTRY; idx++)
		{
			...
			if (pTdlsEntry->Valid && pTdlsEntry->bDoingPeriodChannelSwitch)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("102. %d,%d,%ld !!!\n",
	...
	IN PVOID SystemSpecific3) 
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Channel switch procedure for (%02x:%02x:%02x:%02x:%02x:%02x)\n",
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("105. %ld !!!\n", (jiffies * 1000) / OS_HZ));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("24. %ld @!!!\n", (jiffies * 1000) / OS_HZ));
	...
	IN PVOID SystemSpecific3) 
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Channel switch timeout\n"));
