	...
	IN	PRTMP_ADAPTER		pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ap_ftkd> Initialize FT KDP Module...\n"));

	/* allocate control block */
	FT_MEM_ALLOC(pAd, &pAd->ApCfg.FtTab.pFT_KDP_Ctrl_BK, sizeof(FT_KDP_CTRL_BLOCK));
	if (pAd->ApCfg.FtTab.pFT_KDP_Ctrl_BK == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ap_ftkd> Allocate control block fail!\n"));
		return;
	}
	...
	IN	PRTMP_ADAPTER		pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ap_ftkd> Release FT KDP Module...\n"));
	...
	IN	VOID				*pCB)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ap_ftkd> Queued Events are too much!\n"));
		return;
	}
#endif /* FT_KDP_FUNC_SOCK_COMM */
	...
	if (pFtKdp == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ap_ftkd> Allocate signal content fail!\n"));
		return;
	}
	pFtKdp->Sig = EventId;
	...
	switch(EventId)
	{
		...
		case FT_KDP_SIG_FT_REASSOCIATION:
		{
			...
			*/
			...
			{
#if 0
				/* no IP so we can not send MOVE-REQ to the old AP */
				FT_MEM_FREE(pAd, pFtKdp);

				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
						("ap_ftkd> Can not find the IP of old AP!\n"));
				return;
#endif
			}
		}
		...
		case FT_KDP_SIG_KEY_REQ:
		{
									...
									NULL, &PeerIP) == FALSE)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
						("ap_ftkd> Can not find the IP of R0KH!\n"));
				return;
#endif
		...
		case FT_KDP_SIG_ACTION:
		{
			...
			if (pCB == NULL)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ap_ftkd> pCB == NULL!\n"));
				return;
			}
	...
	IN	FT_KDP_EVT_KEY_ELM	*pEvtKeyReq)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ap_ftkd> Key Req format incorrect!\n"));
	}
#endif /* FT_KDP_EMPTY */
	...
	IN	INT32				InfoLen)
{
		...
		(pEvtKeyRsp->OUI[2] == FT_KDP_ELM_PRI_OUI_2))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MacAddr"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ap_ftkd> Key Rsp format incorrect!\n"));
	}
#endif /* FT_KDP_EMPTY */
}
	...
	IN	INT32				InfoLen)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ap_ftkd> All BSS are open mode!\n"));
	...
	IN		UINT32				*pDataLen)
{
	...
	if (pOutputBuf == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ap_ftkd> Allocate encryption buffer fail!\n"));
		return;
	}
	...
	IN	UINT32				*pDataLen)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ap_ftkd>Decryption length is too small!\n"));
		return;
	}
	...
	if (pOutputBuf == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ap_ftkd> Allocate decryption buffer fail!\n"));
		return;
	}
	...
	IN	UINT32				IP)
{
	...
	if (FlgIsSameRecordFound == FALSE)
	{
		...
		if (pInfo == NULL)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ap_ftkd> Allocate R0KH INFO fail!\n"));
			return FALSE;
		}
