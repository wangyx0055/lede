...
MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s():shiang! PeerProbeReqSanity failed!\n", __FUNCTION__));
	...
	if ((NdisEqualMemory(NINTENDO_SSID_NAME, ProbeReqParam.Ssid, NINTENDO_SSID_NAME_LN)) && (0 == pAd->bNintendoCapable)) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("iwpriv ra0 nintendocapable 0 (Not Enable)\n"));
	}
...
*/
...
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%02x:%02x:%02x:%02x:%02x:%02x is a legacy BSS (rate# =%d, ERP=%d), set Use_Protection bit\n",
		...
		{
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("(RSSI:%d) %02x:%02x:%02x:%02x:%02x:%02x is a legacy BSS (rate# =%d, ERP=%d), set Use_Protection bit\n",
...
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Channel = %d, RealRssi = %d\n", ie_list->Channel, RealRssi));
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("(RSSI:%d) %02x:%02x:%02x:%02x:%02x:%02x is a [legacy] BSS\n",
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s - SUPP_RATES., Len=%d. Rates[0]=%x\n", __FUNCTION__, *RatesLen, **ppRates));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Rates[1]=%x %x %x %x %x %x %x\n",
