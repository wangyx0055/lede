	...
	IN UCHAR Channel)
{
	...
	if(pBssInfoTab == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("pAd->pBssInfoTab equal NULL.\n"));
	...
	)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s(): Select Channel %d\n", __FUNCTION__, ch));
	return ch;
	...
	)
{
	...
	for (BssTab_idx = 0; BssTab_idx < pBssInfoTab->BssNr; BssTab_idx++)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" ch%d bssid=%02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	if (candidate_ch >= 0)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Rule 1 CCA value : Min Dirtiness (Include extension channel) ==> Select Channel %d \n", ch));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Min Dirty = %u\n", min_dirty));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ExChannel = %d , %d\n", candidate_ExChannel[0], candidate_ExChannel[1]));
	...
	{
		ch = pAd->ChannelList[candidate_ch].Channel;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Rule 2 CCA value : Min False CCA value ==> Select Channel %d, min falsecca = %d \n", ch, min_falsecca));
		return	ch;
	}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Rule 3 CCA value : Randomly Select ==> Select Channel %d\n", ch));
	return ch;
}
	...
	)
{
   ...
   pAd->ApCfg.AutoChannel_Channel = 0;
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Rule 1 APCnt : dirtiness == 0 (no one used and no interference) ==> Select Channel %d\n", pAd->ChannelList[channel_index].Channel));
	...
	for (dirty = 30; dirty <= 32; dirty++)
	{
		...
		if (candidate_num)
		{
			...
			if (final_channel != 0)
			{				
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Rule 2 APCnt : minimum APCnt with  minimum interference(dirtiness: 30~32) ==> Select Channel %d\n", final_channel));
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" Dirtiness = %d ,  Min ApCnt = %d\n", dirty, min_ApCnt));
				return final_channel;
			}
		}
	}
#if 0
	/* choose min dirtiness */
	for (channel_index=0 ; channel_index < pAd->ChannelListNum ; channel_index++)
	{
		UINT min_dirty=0xFFFFFFFF;
		final_channel = 0;
		if (pChannelInfo->dirtyness[channel_index] == TRUE)
			continue;
		if (RadarChannelCheck(pAd, pAd->ChannelList[channel_index].Channel)
			&&(pAd->ApCfg.bAvoidDfsChannel == TRUE)
			&&(pChannelInfo->IsABand == TRUE))
			continue;
		/* QLOAD ALARM */
		if (QBSS_LoadIsBusyTimeAccepted(pAd,pChannelInfo->chanbusytime[channel_index]) == FALSE)
			continue;

		if (min_dirty > pChannelInfo->dirtyness[channel_index])
		{
			final_channel = pAd->ChannelList[channel_index].Channel;
			min_dirty = pChannelInfo->dirtyness[channel_index];
		}
	}
	if (final_channel != 0)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Rule 3 APCnt ==> Select Channel %d\n",final_channel));
		return final_channel;
	}
#endif
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Rule 3 APCnt : Randomly Select  ==> Select Channel %d\n",final_channel));
	return final_channel;
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (pAd->pBssInfoTab)
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s Fail to alloc memory for pAd->pBssInfoTab", __FUNCTION__));
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (pAd->pChannelInfo)
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s Fail to alloc memory for pAd->pChannelInfo", __FUNCTION__));
