		...
		switch (pEid->Eid)
		{
			...
			case IE_VHT_CAP:
				if (pEid->Len == sizeof(VHT_CAP_IE)) {
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s():wrong IE_VHT_CAP\n", __FUNCTION__));
				}
				break;
			case IE_VHT_OP:
				if (pEid->Len == sizeof(VHT_OP_IE)) {
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s():wrong IE_VHT_OP\n", __FUNCTION__));
				}
				break;
#endif /* DOT11_VHT_AC */
#endif /* DOT11_N_SUPPORT */
	...
	for(idx = 0; idx < MAX_APCLI_NUM; idx++)
	{
		...
		if (apcli_entry && (&apcli_entry->wdev == wdev))
		{
			...
			if (pAd->ApCfg.bMACRepeaterEn == TRUE)
			{
				...
				{
#if 0
					if ((!MAC_ADDR_EQUAL(apcli_entry->wdev.if_addr, (pSrcBufVA + MAC_ADDR_LEN))) &&
						(!MAC_ADDR_EQUAL(pAd->ApCfg.BridgeAddress, (pSrcBufVA + MAC_ADDR_LEN))) &&
						(pAd->ApCfg.RepeaterCliSize < MAX_EXT_MAC_ADDR_SIZE)
					)
#else
...
#if 0//def MT_MAC , it should check the apclin's port secured, then insert the Entry
						else if (pAd->chipCap.hif_type == HIF_MT) {
							RTMPInsertRepeaterEntry(pAd, idx, (pSrcBufVA + MAC_ADDR_LEN));
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, (" Receive trigger packet !!!\n"));
							return FALSE;
						}
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("IF-apcli%d : B/M-cast frame pRxBlk->wcid=%d, keyidx=%d \n",
...
#ifdef RELEASE_EXCLUDE
	/* debug print */
	for (entryIdx=0; entryIdx<pSsidBssTab->BssNr;entryIdx++)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ssid=%s, Channel=%d, Rssi=%d, AuthMode=%s, AuthModeAux=%s, EncrypType=%s\n",
					pSsidBssTab->BssEntry[entryIdx].Ssid,
					pSsidBssTab->BssEntry[entryIdx].Channel,
					pSsidBssTab->BssEntry[entryIdx].Rssi,
					GetAuthMode(pSsidBssTab->BssEntry[entryIdx].AuthMode),
					GetAuthMode(pSsidBssTab->BssEntry[entryIdx].AuthModeAux),
					GetEncryptType(pSsidBssTab->BssEntry[entryIdx].WepStatus)));
