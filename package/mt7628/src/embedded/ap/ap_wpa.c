...
BOOLEAN RTMPCheckAKM(PUCHAR sta_akm, PUCHAR ap_rsn_ie, INT iswpa2)
{
    ...
    while (Count > 0)
    {
		/*rtmp_hexdump(DBG_LVL_TRACE,"MBSS WPA_IE AKM ",pTmp,4); */
	...
	IN UCHAR			rsnie_len)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("[PMF]%s : Invalid PMF Capability !!!\n", __FUNCTION__));
...
VOID WpaSend(RTMP_ADAPTER *pAdapter, UCHAR *pPacket, ULONG Len)
{
    ...
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("WpaSend ==> Len=%ld\n",Len));
    ...
    {	
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("WpaSend - No such MAC - %02x:%02x:%02x:%02x:%02x:%02x\n", PRINT_MAC(Addr)));
#endif /* RELEASE_EXCLUDE */
		return;
    }
...
#if 0 /*Benson add for big-endian 20081016 --> */
...
#ifdef RELEASE_EXCLUDE
		/* Change endian before calculate MIC, fix bug for big-endian platform */
#endif /* RELEASE_EXCLUDE */
