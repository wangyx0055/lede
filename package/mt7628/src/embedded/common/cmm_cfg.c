...
INT RT_CfgSetMbssWirelessMode(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("AP doesn't support 2.4G band! (RfIcType=%d)\n", pAd->RfIcType));
		return FALSE;
	}
#endif /* MT76x0 */
...
static unsigned long SiteSurvey_Time = 0;

INT Set_SiteSurvey_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	NDIS_802_11_SSID Ssid;
	unsigned long SiteSurvey_Timeout = 3000;
	//check if the interface is down
	if (!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE))
	{
	...
	SiteSurvey_Timeout = ((3000 * OS_HZ) / 1000);//time out 3 second later
	if((SiteSurvey_Time != 0)&&(time_before(jiffies, SiteSurvey_Time))){
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!!!!!!!!!!!!!!site_survey not time out, jiffies = %ul, SiteSurvey_Time = %ul\n", jiffies, SiteSurvey_Time));
		//SiteSurvey_Time = jiffies + SiteSurvey_Timeout; 
		return TRUE;
	}
	else{ //time out		
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!!!!!!!!!!!!!!site_survey time out, jiffies = %ul, SiteSurvey_Time = %ul\n", jiffies, SiteSurvey_Time));
		SiteSurvey_Time = jiffies + SiteSurvey_Timeout;
	}	
...
#endif /* P2P_SUPPORT */
	{
		...
		if( pAd->ApCfg.ApCliTab[1].TrialCh == 0 )
		{
#endif
	...
	switch (UsedAnt)
	{
		...
		case ANT_DIVERSITY_ENABLE:
					...
					pAd->CommonCfg.bHWRxAntDiversity) /* HW_ANT_DIV (PPAD) */
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("<== %s(Auto Switch Mode), (%d,%d)\n",
			...
			else if (pAd->CommonCfg.bSWRxAntDiversity)/* SW_ANT_DIV */
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("<== %s(Auto Switch Mode) EEPROM not enable diversity(%d/%d)\n",
		...
		case ANT_SW_DIVERSITY_ENABLE:
			if (pAd->CommonCfg.bSWRxAntDiversity)
			{
				pAd->RxAnt.EvaluateStableCnt = 0;
				pAd->CommonCfg.RxAntDiversityCfg = UsedAnt;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("<== %s(Diversity Mode --> SW), (%d,%d)\n",
						__FUNCTION__, pAd->RxAnt.Pair1PrimaryRxAnt, pAd->RxAnt.Pair1SecondaryRxAnt));
			}
			else
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("<== %s(Diversity Mode --> SW) EEPROM not enable diversity(%d/%d)\n",
						__FUNCTION__, pAd->CommonCfg.bSWRxAntDiversity, pAd->CommonCfg.bHWRxAntDiversity));
		...
		case ANT_HW_DIVERSITY_ENABLE:
					...
					&& (pAd->CommonCfg.bHWRxAntDiversity)) /* HW_ANT_DIV (PPAD) */
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("<== %s(Diversity Mode --> HW), (%d,%d)\n",
					__FUNCTION__, pAd->RxAnt.Pair1PrimaryRxAnt, pAd->RxAnt.Pair1SecondaryRxAnt));
			}
			else
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("<== %s(Diversity Mode --> HW), EEPROM not enable diversity(%d/%d)\n",
					__FUNCTION__, pAd->CommonCfg.bSWRxAntDiversity, pAd->CommonCfg.bHWRxAntDiversity));
...
INT SetPSERegister_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (idx == 1)
	{
			...
			DBGPRINT(RT_DEBUG_OFF, ("0x800c006c [3]=0x%x\n",Value));
	...
	{ 
		...
		if (IS_ENTRY_CLIENT(pEntry) && (pEntry->Sst == SST_ASSOC))
			 ...
			 NdisZeroMemory(&tb_entry, sizeof(tb_entry));
                         ...
                         RTMP_IO_READ32(pAd, tb_entry.wtbl_addr[0]+12, &dw3->word);
                         DBGPRINT(RT_DEBUG_OFF, ("wcid %d on PSM RES ==> %d\n", wcid, dw3->field.psm));
		}
		else
			goto err;	
