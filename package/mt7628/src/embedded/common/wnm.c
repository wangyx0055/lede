						...
						IN struct _BSS_STRUCT *pMbss)
{
	...
	if (ProtoType == ETH_P_ARP)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("The Packet is GratuitousARP\n"));
			return TRUE;
		}
	...
	if (ProtoType == ETH_P_IPV6)
	{
		...
		if (RTMPEqualMemory(Pos, AllNodeLinkLocalMulticastAddr, 16))
		{
			...
			if (*Pos == 0x88)
			{
				...
				if ((*Pos & 0x40) == 0x00)
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("The Packet is UnsolicitedNeighborAdver\n"));
					Pos += 4;
					return TRUE;
				}
			}
						   		...
						   		IN PUCHAR pData)
{
	...
	if (ProtoType == ETH_P_ARP)
	{
		...
		if (ARPOperation == 0x0001)
		{
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IPv4ProxyARPCandidate\n"));
				return TRUE;
			}
		}
	}
	...
	if (ProtoType == ETH_P_IPV6)
	{
		...
		if (IS_UNSPECIFIED_IPV6_ADDR(*pIPv6Addr))
		{
			...
			if (RTMPEqualMemory(Pos, SolicitedMulticastAddr, 13))
			{
				...
				if (*Pos == 0x87)
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("THe Packet is for Ipv6DuplicateAddrDetect\n"));
	...
	if (ProtoType == ETH_P_IPV6)
	{
		...
		//{
			...
			if (RTMPEqualMemory(Pos, SolicitedMulticastAddr, 13))
			{
				...
				if (*Pos == 0x87)
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("The Packet is IPv6ProxyARPCandidate\n")); 
					return TRUE;
				}
			}
		//}
    ...
    if (ProtoType == ETH_P_IPV6)
    {
        ...
        /* Check if DHCPv6 solicitation */
		{
			...
			if ((*type == 1) || (*type == 4))
			{
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("The Packet is DHCPv6 Solicitation,msg type=%d\n", *type));
					return TRUE;
				}
			}
        }
								 ...
								 IN PUCHAR pData)
{
	...
	if (ProtoType == ETH_P_IPV6)
	{
		...
		if (*Pos == 0x85)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("The Packet is IPv6 Router Solicitation\n"));
			return TRUE;
		}
	}
								  ...
								  IN PUCHAR pOffset)
{
	...
	if (ProtoType == ETH_P_IPV6)
	{
		...
		Pos += 40;
			...
			do
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IsIPv6RouterAdvertisement: nextheader=0x%x, %d, %d\n", NextHeader, PayloadLen, IsExtenHeader));
				switch (NextHeader)
				{
					case IP_PROTO_HOPOPTS:
					case IP_PROTO_ROUTING:
		...
		if (*Pos == 0x86)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("The Packet is IPv6 Router Advertisement\n"));
					 ...
					 IN PUCHAR pData)
{
	...
	if (ProtoType == 0x890d)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("THe Packet is TDLS\n"));
		return TRUE;
	}
							...
							PUCHAR pTargetIPAddr)
{
	...
	DlListForEach(ProxyARPEntry, &pWNMCtrl->IPv6ProxyARPList, PROXY_ARP_IPV6_ENTRY, List)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s, the Mac address and IPv6 Address exactly same as the one in List already!\n", __FUNCTION__));
			RTMP_SEM_EVENT_UP(&pWNMCtrl->ProxyARPIPv6ListLock);
			return FALSE;
		}
		...
		if ((MAC_ADDR_EQUAL(ProxyARPEntry->TargetMACAddr, pTargetMACAddr) == FALSE) &&
			IPV6_ADDR_EQUAL(ProxyARPEntry->TargetIPAddr, pTargetIPAddr))
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s, different Mac address use IPv6 address which already in List!\n", __func__));
			RTMP_SEM_EVENT_UP(&pWNMCtrl->ProxyARPIPv6ListLock);
			IsDAD = TRUE;
	...
	DlListForEach(ProxyARPEntry, &pWNMCtrl->IPv4ProxyARPList, PROXY_ARP_IPV4_ENTRY, List)
	{
		if (IPV4_ADDR_EQUAL(ProxyARPEntry->TargetIPAddr, TargetIPAddr))
		{
			...
			(IPV4_ADDR_EQUAL(SourceIPAddr, ZERO_IP_ADDR) == TRUE)
				...
				//it's a DAD arp.
				IsDAD = TRUE;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s, Found DAD!!!!\n", __FUNCTION__));
				printk("found dad...\n");
			}
			...
			PUCHAR pSrcBuf)
{
	...
	if (pMbss->WNMCtrl.ProxyARPEnable)
	{
		...
		if (IsIPv6RouterAdvertisement(pAd, pSrcBuf - 2, &Offset))
		{
			...
			INT32 PayloadLen; 
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("This packet is router advertisement\n"));
							...
							IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (!pWNMCtrl)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s Can not find Peer Control\n", __FUNCTION__));
		return;
	}
	...
	if (IsFound) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s Find peer address in BTMPeerList already\n", __FUNCTION__));
		return;
	}
						  ...
						  IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (!IsFound) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Not found peer entry in list\n"));
		return;
	}
...
#endif /* CONFIG_STA_SUPPORT */
		default:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Invalid action field = %d\n", Action));
			break;
	}
}
