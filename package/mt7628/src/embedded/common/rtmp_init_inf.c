...
int rt28xx_init(VOID *pAdSrc, RTMP_STRING *pDefaultMac, RTMP_STRING *pHostName)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MAC[Ver:Rev/ID=0x%08x : 0x%08x]\n",
...
/*if not init ATECtrl->Mode = ATE_STOP ,Rxr receive will pending, and Fw_init will fail*/
	...
	*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: pAd->FreqCalibrationCtrl.bEnableFrequencyCalibration = %d\n",
...
VOID RTMPDrvClose(VOID *pAdSrc, VOID *net_dev)
{
	...
	UINT32 count = 0;
	struct MCU_CTRL *prCtl = &pAd->MCUCtrl;
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("In InitHWCoexistence ...\n"));
	...
	*/
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Coexistence Initialize Hardware\n"));
		...
		*/
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Hardware Coexistence Initialized\n"));
	}
}
#endif /* HW_COEXISTENCE_SUPPORT */
