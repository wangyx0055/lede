...
static VOID dump_txblk(TX_BLK *pTxBlk)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Dump TX_BLK Structure:\n"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("\tTxFrameType=%d!\n", pTxBlk->TxFrameType));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("\tTotalFrameLen=%d\n", pTxBlk->TotalFrameLen));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("\tTotalFrameNum=%d!\n", pTxBlk->TxPacketList.Number));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("\tTotalFragNum=%d!\n", pTxBlk->TotalFragNum));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("\tpPacketList=\n"));
	...
	{	int j;
		...
		if (pPacket)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("\t\t[%d]:ptr=0x%x, Len=%d!\n", i, (UINT32)(GET_OS_PKT_DATAPTR(pPacket)), GET_OS_PKT_LEN(pPacket)));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("\t\t"));
			for (j =0 ; j < GET_OS_PKT_LEN(pPacket); j++)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%02x ", (pBuf[j] & 0xff)));
				if (j == 16)
					break;
			}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("\twdev_idx=%d!\n", pTxBlk->wdev_idx));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("----EndOfDump\n"));
...
NDIS_STATUS MlmeHardTransmitMgmtRing(RTMP_ADAPTER *pAd, UCHAR QueIdx, PNDIS_PACKET pPacket)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
	{
		...
		if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_DOZE))
		{
			...
			/*7636 psm, in legacy psp, all frames sent from STA is PM=1, since 7636 PM is offload, we need to make
			sure 7636 H/W is in PM=1 to handle this case*/
#if defined(MT7636)
	...
	*/
	...
	else
	{
		...
		if ((pAd->P2pCfg.bLowRateQoSNULL == TRUE) &&
			...
			(pHeader_802_11->FC.SubType == SUBTYPE_QOS_NULL)))
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s:: Using Low Rate to send QOS NULL!!\n", __FUNCTION__));
...
#if 0 //defined(MT7603) || defined(MT7628) || defined(MT7636)
// TODO: shiang-7603
...
//---Add by shiang for debug
	...
	hex_dump("before MtSdioSubmitMgmtToSDIOWorker",pBuf,uLen);

    	MtSdioSubmitMgmtToSDIOWorker(pAd, mac_info.q_idx, pPacket, pSrcBufVA, SrcBufLen);
#else
...
#ifdef RELEASE_EXCLUDE
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("@@ %s: Dongle Packet)\n", __FUNCTION__));
#endif /* RELEASE_EXCLUDE */
				}
...
//YF MCC
...
#ifdef RELEASE_EXCLUDE
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("RTMP_FillTxBlkInfo: ApClient - Rxwcid(%d), wdev_idx(%d))\n",pTxBlk->Wcid, pTxBlk->wdev_idx));
#endif /* RELEASE_EXCLUDE */
				}
				else
#endif /* APCLI_SUPPORT */
	...
	if (RTMP_GET_PKT_CSO(pPacket) || RTMP_GET_PKT_TSO(pPacket))
	{
		...
		if (RTMP_GET_PKT_TSO(pPacket)) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): Transmit a TSO/CSO frame:\n", __FUNCTION__));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("\tcso/tso=%d:%d, is_tcp=%d, ips=%d, tups=%d, mss=%d!\n",
...
INT rtmp_enq_req(RTMP_ADAPTER *pAd, PNDIS_PACKET pkt, UCHAR qidx, STA_TR_ENTRY *tr_entry, BOOLEAN FlgIsLocked,QUEUE_HEADER *pPktQueue)
{
	...
	TRACE_IPARP_TX_CHK_PNT(pAd, RTMP_GET_PACKET_TRACEIPARP(pkt), P10, 0);
...
#ifdef RELEASE_EXCLUDE
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_LOUD, ("TxRing(%d) full, PktFragCnt=%d\n",
								QueIdx, RTMP_GET_PACKET_FRAGMENTS(pPacket)));
#ifdef RTMP_MAC_PCI
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO | DBG_FUNC_TXQ, ("Deq:NoFreeTxD[%d](CIDX=%d,TxSwFreeIdx=%d)\n",
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("RTMPDeQueuePacket (Tx:%d)--> \n", max_cnt));
#endif /* RELEASE_EXCLUDE */
...
#ifdef TRACELOG_IPARP
		if (pTxBlk->pPacket)
			TRACE_IPARP_TX_CHK_PNT(pAd, RTMP_GET_PACKET_TRACEIPARP(pTxBlk->pPacket), P12, 0);
#endif /* TRACELOG_IPARP */
...
#ifdef RTMP_SDIO_SUPPORT
	MtSdioSubmitDataToSDIOWorker(pAd);
#else
...
#ifdef TRACELOG_IPARP
	TRACE_IPARP_SET_TX_STARTING_PNT(pAd, pMacEntry->Aid, pPacket, (pSrcBuf+12));
	TRACE_IPARP_TX_CHK_PNT(pAd, RTMP_GET_PACKET_TRACEIPARP(pPacket), P0, 0);
#endif /* TRACELOG_IPARP */
...
#ifdef CONFIG_TSO_SUPPORT
	if (pTxBlk->naf_type)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():Error, try to encrypt a pkt marked to use NAF(%d)!\n",
...
INT TxOPUpdatingAlgo(RTMP_ADAPTER *pAd)
{
	...
	{
#ifdef RELEASE_EXCLUDE
	/* It means make sure TXOP 0x80 only for one apclient to RootAp and no sta connected in our Extender,
         * For Example,
         * case 1. [ROOTAP] -- 2.4G --[[APCLIENT] Extender [AP]] -- 5G -- [STA]   :: TXOP OK
         * case 2. [ROOTAP] -- 2.4G --[[APCLIENT] Extender [AP]] -- 2.4G -- [STA] :: TXOP NO
         * case 3. [ROOTAP] -- 2.4G --[[APCLIENT] Extender [AP]] -- 5G -- [STA]x2 :: TXOP OK
         * case 4. [ROOTAP] -- 2.4G --[[APCLIENT] Extender [AP]] -- ETH -- [PC]   :: TXOP OK
         * case 5. [ROOTAP] -- 2.4G --[[APCLIENT] Extender [AP]] -- ETH -- [PC]x2 :: TXOP OK
         * case 6. [ROOTAP] -- 5G --[[APCLIENT] Extender [AP]] -- 2.4G -- [STA]   :: TXOP OK
         * case 7. [ROOTAP] -- 5G --[[APCLIENT] Extender [AP]] -- 5G -- [STA]     :: TXOP NO
         */
#endif /* RELEASE_EXCLUDE */
		...
		{
#ifdef RELEASE_EXCLUDE
        /* It means make sure TXOP 0x80 only for one wireless connection between the RootAp and our Extender,
         * For Example,
         * case 1. [ROOTAP] -- 2.4G --[[APCLIENT] Extender [AP]] -- 5G -- [STA]  
         * Ans: RepTable Size = 1, MacTableSize = 2 (1 for ApClientEntry, 1 for ReptEntry) 
         * => TXOP OK
         *
         * case 2. [ROOTAP] -- 2.4G --[[APCLIENT] Extender [AP]] -- 2.4G -- [STA]
         * Ans: RepTable Size = 1, MacTableSize = 3 (1 for ApClientEntry, 1 for ReptEntry, 1 for ClientEntry) 
         * => TXOP NO
	 *
         * case 3. [ROOTAP] -- 2.4G --[[APCLIENT] Extender [AP]] -- ETH -- [PC]   
         * Ans: RepTable Size = 1, MacTableSize = 2 (1 for ApClientEntry, 1 for ReptEntry) 
         * => TXOP OK
	 *
         * case 4. [ROOTAP] -- 2.4G --[[APCLIENT] Extender [AP]] -- ETH -- [PC]x2 
	 * Ans: RepTable Size = 2, MacTableSize = 3 (1 for ApClientEntry, 2 for ReptEntry) 
         * => TXOP NO
         *
         */
#endif /* RELEASE_EXCLUDE */
