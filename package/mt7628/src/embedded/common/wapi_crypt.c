...
void print_mem(void *p, int len)
{
	...
	{
		/*printf("0x%02x, ", pp[i]); */
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%02x ", pp[i]));
		if((i + 1) %16 == 0)
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("\n\t"));
	}
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("\n"));
		...
		OUT	UCHAR 			*mic_hdr)
{
	...
	/*MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("mhdr_len(%d)\n", mhdr_len)); */
...
{	
	...
	if (bSanityIV)
	{
		...
		{
#if 0
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ERROR : the received IV is invalid!!!\n"));	
	...
	{			
#if 0
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("WPI MIC Different!!!\n"));	
