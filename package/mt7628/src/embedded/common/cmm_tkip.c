	...
	IN	UINT			Len)
{
	...
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("RTMPTkipCompareMICValueWithLLC(): TKIP MIC Error !\n"));  /*MIC error.*/
	
#ifdef RELEASE_EXCLUDE
		{
			INT		i;
		
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO, ("Orig MIC value ="));  /*MIC error.*/
			for (i = 0; i < 8; i++)
			{
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO, ("%02x:", OldMic[i]));  /*MIC error.*/
			}
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO, ("\n"));  /*MIC error.*/
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO, ("Calculated MIC value ="));  /*MIC error.*/
			for (i = 0; i < 8; i++)
			{
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO, ("%02x:", pAd->PrivateInfo.Rx.MIC[i]));  /*MIC error.*/
			}
...
/*	ARCFOURCONTEXT 	ArcFourContext;*/
	...
	if (pWpaKey->KeyLen == 0)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : the key is empty)\n", __FUNCTION__));
		return FALSE;
	}

	/* allocate memory */
	os_alloc_mem(NULL, (UCHAR **)&pArcFourContext, sizeof(ARCFOURCONTEXT));
...
/*	else if (to_ds == 1 && from_ds == 1)*/
	...
	pnh = cpu2le32(pnh);	
	RTMPTkipMixKey(pWpaKey->Key, TA, pnl, pnh, RC4Key, p1k);

	ARCFOUR_INIT(pArcFourContext, RC4Key, 16); 
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RTMPSoftDecryptTKIP, WEP Data ICV Error !\n"));	 /*ICV error.*/
		goto LabelErr;
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RTMPSoftDecryptTKIP, WEP Data MIC Error !\n"));	 /*MIC error.*/
	...
	RTMPFrameEndianChange(pAd, (PUCHAR)pData, DIR_READ, FALSE);
#endif
	/*MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, "RTMPSoftDecryptTKIP Decript done!!\n");*/
	if (pArcFourContext != NULL)
	...
	IN	UINT			Len)
{
	...
	if(!NdisEqualMemory(pAd->PrivateInfo.Rx.MIC, OldMic, 8))
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("RTMPTkipCompareMICValue(): TKIP MIC Error !\n"));  /*MIC error.*/
	...
	IN 		UINT16			*DataByteCnt)
{
	...
	*/
    ...
    if(crc32 != cpu2le32(trailfcs))
    {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("! WEP Data CRC Error !\n"));	 /*CRC error.*/
		return FALSE;
	}
