...
VOID RTMPResetTxRxRingMemory(RTMP_ADAPTER *pAd)
{
	...
	for (index=0;index< NUM_OF_RX_RING;index++)
	{
		...
		NdisZeroMemory(pAd->RxRing[index].Cell, RxRingSize * sizeof(RTMP_DMACB));
	}
...
VOID PSEResetAndRecovery(RTMP_ADAPTER *pAd)
{
	...
	UINT32 Ret;
	RTMP_SEM_EVENT_WAIT(&pAd->IndirectUpdateLock, Ret);
	if(Ret != 0)
...
#endif
		{                      		
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<==RT28xxPciStaAsicForceWakeup::Release the MCU Lock(3090)\n"));
...
#endif
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("<--PsPollWakeExec:: calls AsicForceWakeup(pAd, DOT11POWERSAVE) in advance \n"));
	...
	IN PVOID SystemSpecific3) 
{
	...
	if (IS_RT3290(pAd) && (pAd->StaCfg.bRadio == TRUE))
	{
		...
		if (pAd->StaCfg.bDelayedPhyPowerIndication == TRUE)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s: Delayed PHY power state indication.\n", __FUNCTION__));
		}
#endif
...
#endif
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RT28xxPciAsicRadioOff::3090 return to skip the following TbttNumToNextWakeUp setting for 279x\n"));
		pAd->bPCIclkOff = TRUE;
		RTMP_CLEAR_PSFLAG(pAd, fRTMP_PS_DISABLE_TX);
