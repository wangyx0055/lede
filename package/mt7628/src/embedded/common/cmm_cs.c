	...
	IN PRTMP_ADAPTER pAd)
{
	...
	/*
	*	(1)
	*	in NORMAL state, if count == 0, then enter SILENCE state;
	*	in SILENCE state, if RadarToneCount in one second < CARRIER_GONE_TRESHOLD, 
	*	then return to NORMAL state.
	*
	*	(2)
	*	mcu command: 3xxx Carrier Detection Read(0x66)
	*			arg0: not used
	*			arg1: not used
	*		firmware will update Recheck and RadarToneCount after
	*		this command is sent. then driver can read the two values.
	*/
	...
	/* Debug */
	if (pAd->CommonCfg.CarrierDetect.Debug == DBG_LVL_TRACE)
	{
		...
		case CD_NORMAL:
			if (CarrDetectParam.ReCheck == 0)
			{
				*pCD_State = CD_SILENCE;				
				if (pAd->CommonCfg.CarrierDetect.Debug != DBG_LVL_TRACE)
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Carrier Detected\n"));
...
VOID RTMPHandleRadarInterrupt(PRTMP_ADAPTER  pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RTMPHandleRadarInterrupt()\n"));
	...
	if(pAd->chipCap.carrier_func == TONE_RADAR_V2)
	{
		...
		if (bbp & 0x1)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CS bit not cleared!!!\n"));
			CarrierDetectionResetStatus(pAd);
		}
...
INT Set_CarrierReCheck_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set Recheck = %u\n", pAd->CommonCfg.CarrierDetect.recheck1));
#ifdef CARRIER_DETECTION_FIRMWARE_SUPPORT
...
INT Set_CarrierGoneThreshold_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set CarrierGoneThreshold = %u\n", pAd->CommonCfg.CarrierDetect.CarrierGoneThreshold));
	return TRUE;
}
...
{		
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("CarrierDetectionStart\n"));
...
#ifdef RELEASE_EXCLUDE
		/* 
		start/stop firmware operation for USB carrier detection,
		arg0: 0/1 Disable/Enable
		*/
#endif /* RELEASE_EXCLUDE */
		{
...
VOID ToneRadarProgram_v1(PRTMP_ADAPTER pAd, ULONG threshold)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ToneRadarProgram v1\n"));
...
VOID ToneRadarProgram_v2(PRTMP_ADAPTER pAd, ULONG threshold)
{
	...
	/* programe delta delay & division bit*/
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ToneRadarProgram v2\n"));
...
VOID ToneRadarProgram_v3(PRTMP_ADAPTER pAd, ULONG threshold)
{
	...
	*/
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ToneRadarProgram v3\n"));
	CarrierDetectionEnable(pAd, 0);	
