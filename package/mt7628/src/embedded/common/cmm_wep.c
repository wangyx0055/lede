	...
	OUT	ARC4_CTX_STRUC  *pARC4_CTX)
{
	...
	if (seed == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: seed Allocate memory fail!!!\n", __FUNCTION__));
		return;
	}
	...
	IN 		ULONG			DataByteCnt)
{
	...
	if (ARC4_CTX == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: ARC4_CTX Allocate memory fail!!!\n", __FUNCTION__));
		return FALSE;
	}
	...
	INOUT 	UINT16			*DataByteCnt)
{
	...
	{
		os_free_mem(NULL, ARC4_CTX);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : The key is not available !\n", __FUNCTION__));
		return FALSE;
	}
