    ...
    IN MLME_QUEUE_ELEM  *Elem)
{
    ...
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WpaEAPOLStartAction ===> \n"));
    ...
    else
    {
        ...
        */
        if (pEntry &&
			...
			pEntry->bWscCapable)
        {
            MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WPS enabled AP: Ignore EAPOL-Start frames received from clients.\n"));
            return;
        }
	...
	tr_entry = &pAd->MacTab.tr_entry[pEntry->wcid];
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" PortSecured(%d), WpaState(%d), AuthMode(%d), PMKID_CacheIdx(%d) \n",
    ...
    IN MLME_QUEUE_ELEM  *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("WpaEAPOLKeyAction ===>\n"));
	...
	*((USHORT *)&peerKeyInfo) = cpu2le16(*((USHORT *)&peerKeyInfo));
	...
	do
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Receive EAPoL-Key frame from STA %02X-%02X-%02X-%02X-%02X-%02X\n", PRINT_MAC(pEntry->Addr)));

		if (eapol_len > Elem->MsgLen - LENGTH_802_11 - LENGTH_802_1_H)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("The length of EAPoL packet is invalid \n"));
			break;
		}
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Key descripter does not match with WPA rule\n"));
			break;
		}
		...
		if (IS_FT_RSN_STA(pEntry))
		{
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Key descripter version(%d) not match(FT) \n", peerKeyInfo.KeyDescVer));
				break;
			}
		}
		else
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Key descripter version not match(TKIP) \n"));
			break;
		}
#ifdef DOT11W_PMF_SUPPORT
		else if (CLIENT_STATUS_TEST_FLAG(pEntry, fCLIENT_STATUS_USE_SHA256) && (peerKeyInfo.KeyDescVer != KEY_DESC_EXT))
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Key descripter version not match(AES-128)\n"));
		}
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Key descripter version not match(AES) pEntry->WepStatus=%d, peerKeyInfo.KeyDescVer=%d\n", pEntry->WepStatus, peerKeyInfo.KeyDescVer));
			break;
		}
			...
			else
			{
				...
				*/
					...
					(peerKeyInfo.Error == 1))
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Received an Michael MIC Failure Report, active countermeasure \n"));
				...
				else if ((peerKeyInfo.Request == 1) && (peerKeyInfo.Error == 0))
				{
					...
					if (peerKeyInfo.KeyType == GROUPKEY)
					{
						...
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("REQUEST=1, ERROR=0, update group key\n"));
					...
					{
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("REQUEST=1, ERROR= 0, update pairwise key\n"));
    ...
    IN BOOLEAN bClearFrame)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("%s - len=%d, AID=%d, DA=%02x:%02x:%02x:%02x:%02x:%02x\n",
						__FUNCTION__, HdrLen+DataLen, pEntry->Aid,
						PRINT_MAC(pHeader802_3)));
	...
	if (wdev && wdev->tx_pkt_handle)
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():Invalid wdev(%p) or tx_pkt_handle(%p)!\n",
    ...
    IN 	MAC_TABLE_ENTRY  	*pEntry)
{
	...
	*((USHORT *)&EapolKeyInfo) = cpu2le16(*((USHORT *)&EapolKeyInfo));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("The message type is invalid(%d)! \n", MsgType));
		goto LabelErr;
	}
	...
	if (bReplayDiff)
	{
		...
		{
           	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Replay Counter Different in pairwise msg %d of 4-way handshake!\n", MsgType));
		}
		else
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Replay Counter Different in group msg %d of 2-way handshake!\n", (MsgType - EAPOL_PAIR_MSG_4)));
		}
	...
	if (MsgType != EAPOL_PAIR_MSG_1)
	{
        ...
        if (!NdisEqualMemory(rcvd_mic, mic, LEN_KEY_DESC_MIC))
        {
			...
			if (MsgType < EAPOL_GROUP_MSG_1)
			{
            	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("MIC Different in pairwise msg %d of 4-way handshake!\n", MsgType));
			}
			else
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("MIC Different in group msg %d of 2-way handshake!\n", (MsgType - EAPOL_PAIR_MSG_4)));
			}
	...
	if (CONV_ARRARY_TO_UINT16(pMsg->KeyDesc.KeyDataLen) > 0)
	{
		...
		{
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("The Key Data Length should be zero !!!\n"));
#endif /* RELEASE_EXCLUDE */
    ...
    IN ULONG			TimeInterval)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> WPAStart4WayHS\n"));

	if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_RESET_IN_PROGRESS |
							fRTMP_ADAPTER_HALT_IN_PROGRESS))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("[ERROR]WPAStart4WayHS : The interface is closed...\n"));
		return;
	}
...
#endif /* P2P_SUPPORT */
	{
	        ...
	        {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("[ERROR]WPAStart4WayHS : The entry doesn't exist.\n"));
			return;
	        }
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("[ERROR]WPAStart4WayHS : cannot get binding wdev(%p).\n", pEntry->wdev));
			return;
        	}
	...
	if (pBssid == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("[ERROR]WPAStart4WayHS : No corresponding Authenticator.\n"));
		return;
    }
    ...
    {
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("[ERROR]WPAStart4WayHS : Not expect calling\n"));
        return;
    }
        ...
        pMbss->WscControl.EapMsgRunning)
    {
        ...
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("This is a WSC-Enrollee. Not expect calling WPAStart4WayHS here \n"));
        return;
    }
#endif /* WSC_AP_SUPPORT */
	...
	if ((pEntry->AuthMode == Ndis802_11AuthModeWPA2) && (pEntry->PMKID_CacheIdx != ENTRY_NOT_FOUND))
	{
		...
		&& (CLIENT_STATUS_TEST_FLAG(pEntry, fCLIENT_STATUS_PMF_CAPABLE)))
        {
                ...
                {
                        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[PMF]%s: send Msg1 of 4-way using SHA256\n", __FUNCTION__));
                        RT_HMAC_SHA256(pMbss->PMK, PMK_LEN, PMK_key, 20, digest, LEN_PMKID);
                }
                else {
                        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[PMF]%s: send Msg1 of 4-way using SHA1\n", __FUNCTION__));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<=== WPAStart4WayHS: send Msg1 of 4-way \n"));
    ...
    IN MLME_QUEUE_ELEM  *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> PeerPairMsg1Action \n"));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<=== PeerPairMsg1Action: send Msg2 of 4-way \n"));
}
    ...
    IN MLME_QUEUE_ELEM  *Elem)
{
    ...
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> PeerPairMsg2Action \n"));
    ...
    do
    {
			...
			pAd->ApCfg.MBSSID[apidx].HotSpotCtrl.DGAFDisable)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s:Random unique GTK for each mobile device when dgaf disable\n", __FUNCTION__));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<=== PeerPairMsg2Action: send Msg3 of 4-way \n"));
}
    ...
    IN MLME_QUEUE_ELEM  *Elem)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
	{
		...
		#if 0
		/*
			Fix Apple TSN security (RSN (AES or TKIP unicast cipher) with WEP104 group cipher) when p2p0 is up;
			Due to driver will use group pkt s/w decryption when p2p0 is up, and RTMPSwCipherKeySelection() will reference the pAd->SharedKey[BSS0][0],
			But at this point, pAd->SharedKey[BSS0][0] will be overwirte by pEntry->PairwiseKey
		*/
		...
		pEntry->AuthMode == Ndis802_11AuthModeWPA2)
	{
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Apcli auto connected:PeerPairMsg3Action() \n"));
...
#endif /* CONFIG_STA_SUPPORT */
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PeerPairMsg3Action: AuthMode(%s) PairwiseCipher(%s) GroupCipher(%s) \n",
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<=== PeerPairMsg3Action: send Msg4 of 4-way \n"));
}
    ...
    IN MLME_QUEUE_ELEM  *Elem)
{
    ...
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> PeerPairMsg4Action\n"));
	...
	tr_entry = &pAd->MacTab.tr_entry[pEntry->wcid];
			...
			pEntry->AuthMode == Ndis802_11AuthModeWPA2PSK)
		{
			...
			if (IS_FT_RSN_STA(pEntry))
			{
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("The entry in R1KH table doesn't exist\n"));
				}
			}
			else
#endif /* DOT11R_FT_SUPPORT */
			...
			if (pEntry->AuthMode == Ndis802_11AuthModeWPA2)
        	{
				...
				RTMPAddPMKIDCache(pAd, pEntry->func_tb_idx, pEntry->Addr, digest, pAd->ApCfg.MBSSID[pEntry->func_tb_idx].PMK);
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Calc PMKID=%02x:%02x:%02x:%02x:%02x:%02x\n", digest[0],digest[1],digest[2],digest[3],digest[4],digest[5]));
        	}
#endif /* DOT1X_SUPPORT */
#endif /* CONFIG_AP_SUPPORT */
		...
		if (pEntry->IsBTMReqValid == TRUE)
		{
	        ...
	        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("AP SETKEYS DONE - WPA2, AuthMode(%d)=%s, WepStatus(%d)=%s, GroupWepStatus(%d)=%s\n\n",
    ...
    IN MAC_TABLE_ENTRY  *pEntry)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> WPAStart2WayGroupHS\n"));
	...
	if (pBssid == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: pBssid == NULL!\n", __FUNCTION__));
		return;
	}
    ...
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<=== WPAStart2WayGroupHS : send out Group Message 1 \n"));
    ...
    IN MLME_QUEUE_ELEM  *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> PeerGroupMsg1Action \n"));
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		if ((pAd->ApCfg.ApCliAutoConnectRunning == TRUE)
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Apcli auto connected:PeerGroupMsg1Action() \n"));
				pAd->ApCfg.ApCliAutoConnectRunning = FALSE;
			}
#endif /* APCLI_AUTO_CONNECT_SUPPORT */
#endif /* APLCI_SUPPORT */
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PeerGroupMsg1Action: AuthMode(%s) PairwiseCipher(%s) GroupCipher(%s) \n",
									GetAuthMode(pEntry->AuthMode),
									GetEncryptType(pEntry->WepStatus),
									GetEncryptType(group_cipher)));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<=== PeerGroupMsg1Action: send group message 2\n"));
}
...
#ifdef DOT1X_SUPPORT
			case EAPOL_START_1X:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Enqueue EAPoL-Start-1X for sta(%02x:%02x:%02x:%02x:%02x:%02x) \n", PRINT_MAC(pEntry->Addr)));
    ...
    IN UINT             MsgLen)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> PeerGroupMsg2Action \n"));
    ...
    do
    {
		...
		else
		{
        	...
        	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("AP SETKEYS DONE - WPA1, AuthMode(%d)=%s, WepStatus(%d)=%s, GroupWepStatus(%d)=%s\n\n",
										pEntry->AuthMode, GetAuthMode(pEntry->AuthMode),
										pEntry->WepStatus, GetEncryptType(pEntry->WepStatus),
										group_cipher, GetEncryptType(group_cipher)));
		}
	...
	OUT	INT		*MsgType)
{
	switch (EAPType)
	{
		...
		default:
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("WpaMsgTypeSubst : return FALSE; \n"));
#endif /* RELEASE_EXCLUDE */
			return FALSE;
	}
	return TRUE;
...
void inc_iv_byte(UCHAR *iv, UINT len, UINT cnt)
{
	...
	if (pos >= len)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("!!! inc_iv_byte overflow !!!\n"));
#else
	int 	pos = len - 1;
	int 	carry = 0;
	UCHAR	pre_iv;
	...
	if (pos < 0)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!! inc_iv_byte overflow !!!\n"));
#endif
}
	...
	IN	INT		len)
{
    ...
    if (input == NULL)
    {
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!!PRF: no memory!!!\n"));
        return;
    }
	...
	IN	USHORT	len)
{
	...
	if (input == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!!KDF: no memory!!!\n"));
		return;
	}
            ...
            case Ndis802_11AuthModeWPA1WPA2:
                ...
                IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
                {
                        if (pAd->ApCfg.MBSSID[apidx].PmfCfg.MFPR) {
                                ...
                                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("[PMF]%s: Insert 1X-SHA256 to AKM of RSNIE\n", __FUNCTION__));
            ...
            case Ndis802_11AuthModeWPA1PSKWPA2PSK:
                ...
                IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
                {
                        if (pAd->ApCfg.MBSSID[apidx].PmfCfg.MFPR) {
                                ...
                                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("[PMF]%s: Insert PSK-SHA256 to AKM of RSNIE\n", __FUNCTION__));
                        } else if ((pAd->ApCfg.MBSSID[apidx].PmfCfg.MFPC) && (pAd->ApCfg.MBSSID[apidx].PmfCfg.PMFSHA256)) {
                ...
                IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
                {
                        ...
                        BssIdx = BssTableSearchWithSSID(&pAd->MlmeAux.SsidBssTab, pAd->MlmeAux.Bssid, pAd->MlmeAux.Ssid, pAd->MlmeAux.SsidLen, pAd->CommonCfg.Channel);
                        if (BssIdx != BSS_NOT_FOUND)
                        {
                                pInBss = &pAd->MlmeAux.SsidBssTab.BssEntry[BssIdx];
                                if (CLIENT_STATUS_TEST_FLAG(pInBss, fCLIENT_STATUS_USE_SHA256))
...
VOID RTMPMakeRSNIE(RTMP_ADAPTER *pAd, UINT AuthMode, UINT WepStatus, UCHAR apidx)
{
	...
	do
	{
		...
		if (apidx >= MIN_NET_DEVICE_FOR_P2P_GO)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("==> RTMPMakeRSNIE(p2p%d)\n", MAIN_MBSSID));
			...
			if (WepStatus == Ndis802_11TKIPAESMix)
			{
				...
				FlexibleCipher = pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.WpaMixPairCipher;
			}
			else
				pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.GroupKeyWepStatus = WepStatus;
		...
		if (apidx >= MIN_NET_DEVICE_FOR_MESH)
		{
			...
			if ((WepStatus != Ndis802_11TKIPEnable) &&
				(WepStatus != Ndis802_11AESEnable))
				return;

			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("==> RTMPMakeRSNIE(mesh)\n"));
		...
		if (apidx >= MIN_NET_DEVICE_FOR_APCLI)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("==> RTMPMakeRSNIE(ApCli)\n"));
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RTMPMakeRSNIE: invalid apcliIfidx(%d)\n", apcliIfidx));
				return;
			}
		}
#endif /* APCLI_SUPPORT */
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("==> RTMPMakeRSNIE(AP-ra%d)\n", apidx));
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("==> RTMPMakeRSNIE(STA)\n"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: rsnielen_cur_p == NULL!\n", __FUNCTION__));
		return;
	}
...
#endif /* CONFIG_AP_SUPPORT */
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Receive EAP-Packet frame, TYPE = 0, Length = %ld\n", Body_len));
			break;
		case EAPOLStart:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Receive EAPOL-Start frame, TYPE = 1 \n"));
			if (pEntry->EnqueueEapolStartTimerRunning != EAPOL_START_DISABLE)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Cancel the EnqueueEapolStartTimerRunning \n"));
				RTMPCancelTimer(&pEntry->EnqueueStartForPSKTimer, &Cancelled);
				pEntry->EnqueueEapolStartTimerRunning = EAPOL_START_DISABLE;
			}
			break;
		case EAPOLLogoff:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Receive EAPOLLogoff frame, TYPE = 2 \n"));
			break;
		case EAPOLKey:
			Body_len = (*(pData+2)<<8) | (*(pData+3));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Receive EAPOL-Key frame, TYPE = 3, Length = %ld\n", Body_len));
			break;
		case EAPOLASFAlert:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Receive EAPOLASFAlert frame, TYPE = 4 \n"));
			break;
		default:
			return FALSE;
	}
	...
	*Offset = 0;
	...
	while (len > sizeof(RSNIE2))
	{
		...
		if ((pEid->Eid == IE_WPA) && (NdisEqualMemory(pEid->Octet, WPA_OUI, 4)))
		{
			...
			{
#ifdef RELEASE_EXCLUDE
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s ==> WPA/WPAPSK RSN IE matched, Length(%d) \n",
...
#ifdef RELEASE_EXCLUDE
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s ==> WPA2/WPA2PSK RSN IE matched, Length(%d) \n",
											__FUNCTION__, (pEid->Len + 2)));
#endif /* RELEASE_EXCLUDE */
					result = TRUE;
			}
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("RTMPCheckRSNIE ==> skip_offset(%d) \n", *Offset));
#endif /* RELEASE_EXCLUDE */
	...
	IN  MAC_TABLE_ENTRY *pEntry)
{
    ...
    {
#ifdef RELEASE_EXCLUDE
		/* In TGn WiFi Test plan #5.2.14, Test-Bed AP(Atheros) send incorrect RSN_IE in MSG3 cause STA can't connect. */
#endif /* RELEASE_EXCLUDE */
#if 0
		...
		if (!RTMPCheckRSNIE(pAd, pKeyData, KeyDataLen, pEntry, &skip_offset))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RSN_IE Different in msg %d of 4-way handshake!\n", MsgType));
...
#endif /* 0 */
		{
			if (bWPA2 && MsgType == EAPOL_PAIR_MSG_3)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RTMPParseEapolKeyData ==> WPA2/WPA2PSK RSN IE matched in Msg 3, Length(%d) \n", skip_offset));
			}
			else
				return TRUE;
		}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RTMPParseEapolKeyData ==> KeyDataLength %d without RSN_IE \n", KeyDataLength));
				...
				case WPA_KDE_TYPE:
					{
						...
						if (NdisEqualMemory(pKDE->OUI, OUI_WPA2, 3))
    					{
							if (pKDE->DataType == KDE_GTK)
							{
								...
								if (GTKLEN < LEN_WEP64)
								{
									MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ERROR: GTK Key length is too short (%d) \n", GTKLEN));
        							return FALSE;
								}
								NdisMoveMemory(GTK, pKdeGtk->GTK, GTKLEN);
								MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("GTK in KDE format ,DefaultKeyID=%d, KeyLen=%d \n", DefaultIdx, GTKLEN));
    						}
#ifdef DOT11W_PMF_SUPPORT
	...
	else if (!bWPA2 && MsgType == EAPOL_GROUP_MSG_1)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("GTK without KDE, DefaultKeyID=%d, KeyLen=%d \n", DefaultIdx, GTKLEN));
	}

	/* Sanity check - shared key index must be 0 ~ 3*/
	if (DefaultIdx > 3)
    {
     	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ERROR: GTK Key index(%d) is invalid in %s %s \n", DefaultIdx, ((bWPA2) ? "WPA2" : "WPA"), GetEapolMsgType(MsgType)));
        return FALSE;
    }
    ...
    OUT PEAPOL_PACKET       pMsg)
{
	...
	*(USHORT *)(&pMsg->KeyDesc.KeyInfo) = cpu2le16(*(USHORT *)(&pMsg->KeyDesc.KeyInfo));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("===> ConstructEapolMsg for %s %s\n", ((bWPA2) ? "WPA2" : "WPA"), GetEapolMsgType(MsgType)));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("	     Body length = %d \n", CONV_ARRARY_TO_UINT16(pMsg->Body_Len)));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("	     Key length  = %d \n", CONV_ARRARY_TO_UINT16(pMsg->KeyDesc.KeyLength)));
	...
	OUT PEAPOL_PACKET   pMsg)
{
	...
	if ((MsgType == EAPOL_PAIR_MSG_3 && bWPA2Capable) || (MsgType == EAPOL_GROUP_MSG_1))
	{
		...
		if ((bWPA2Capable)
                        && (CLIENT_STATUS_TEST_FLAG(pEntry, fCLIENT_STATUS_PMF_CAPABLE)))
		{
 			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("[PMF]:%s - Insert IGTK\n", __FUNCTION__));
	...
	OUT PEAPOL_PACKET pMsg)
{
    ...
    if (OutBuffer == NULL)
    {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!!CalculateMIC: no memory!!!\n"));
		return;
    }
...
BOOLEAN rtmp_chk_tkip_mic(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry, RX_BLK *pRxBlk)
{
	...
	if (RTMPTkipCompareMICValue(pAd,
								...
								DataSize) == FALSE)
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("Rx MIC Value error 2\n"));

#ifdef CONFIG_STA_SUPPORT
		if (RX_BLK_TEST_FLAG(pRxBlk, fRX_AP) || RX_BLK_TEST_FLAG(pRxBlk, fRX_ADHOC)) {
#ifdef WPA_SUPPLICANT_SUPPORT
#ifdef APCLI_SUPPORT
			if (IS_ENTRY_APCLI(pEntry) &&
				pAd->ApCfg.ApCliTab[pEntry->func_tb_idx].wpa_supplicant_info.WpaSupplicantUP)
			{
	...
	IN MAC_TABLE_ENTRY *pEntry)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : Invalid key index(%d) !!!\n",
								  __FUNCTION__, keyIdx));
		return NULL;
	}
	...
	INOUT 	UINT16			*DataByteCnt)
{
	switch (pKey->CipherAlg)
    {
		...
		case CIPHER_TKIP:
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ERROR : SW decrypt TKIP data fails.\n"));
				/* give up this frame*/
				return NDIS_STATUS_FAILURE;
			}
			break;
		...
		case CIPHER_AES:
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ERROR : SW decrypt AES data fails.\n"));
				/* give up this frame*/
				return NDIS_STATUS_FAILURE;
        	}
			break;
		...
		case CIPHER_SMS4:
			{
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ERROR : SW decrypt SMS4 data fails(%d).\n", ret));
					/* give up this frame*/
					return NDIS_STATUS_FAILURE;
				}
#ifdef RT_BIG_ENDIAN
		...
		OUT	UINT8	*count)
{
	...
	*count = 0;
	...
	if (pEid->Eid == IE_WPA)
	{
		...
		if (len < sizeof(RSNIE))
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : The length is too short for WPA\n", __FUNCTION__));
			goto out;
		}
		else
		{
			...
			if (len < offset)
		{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : The expected lenght(%d) exceed the remaining length(%d) for WPA-RSN \n",
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : The count of pairwise cipher is %d\n",
												__FUNCTION__, u_cnt));
						*count = u_cnt;
	...
	else if (pEid->Eid == IE_RSN)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : The length is too short for WPA2\n", __FUNCTION__));
			goto out;
		}
		else
		{
			...
			if (len < offset)
		{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : The expected lenght(%d) exceed the remaining length(%d) for WPA2-RSN \n",
											__FUNCTION__, offset, len));
				goto out;
		}
			else
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : Unknown IE (%d)\n", __FUNCTION__, pEid->Eid));
		goto out;
	}
	...
	pAkm = (PRSNIE_AUTH)pBuf;
		...
		if (len < offset)
	{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : The expected lenght(%d) exceed the remaining length(%d) for AKM \n",
										__FUNCTION__, offset, len));
			goto out;
	}
		else
	...
	if (type == AKM_SUITE)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : The count of AKM is %d\n",
											__FUNCTION__, a_cnt));
				*count = a_cnt;
	...
	if (pEid->Eid == IE_WPA)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : The remaining shall be ignored in WPA mode\n",
									__FUNCTION__));
		goto out;
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : The peer RSNIE doesn't include RSN-Cap\n", __FUNCTION__));
		goto out;
	}
	else
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : Extract RSN Capabilities\n", __FUNCTION__));
			*count = 1;
			return pBuf;
		}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : The peer RSNIE doesn't include PMKID list Count\n", __FUNCTION__));
		goto out;
	}
	else
	{
		...
		if (p_count > 0)
		{
			...
			if (len < offset)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : The expected lenght(%d) exceed the remaining length(%d) in PMKID-field \n",
											__FUNCTION__, offset, len));
				goto out;
			}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[PMF]%s : The peer RSNIE doesn't include Group_mgmt_cipher_suite\n", __FUNCTION__));
		goto out;
	}
	else
	{
	...
	IN UINT8  pmkid_len)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : no PMKID-List included(%d).\n", __FUNCTION__, pmkid_len));
	}
	...
	BOOLEAN				bAE)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : fails (wcid-%d)\n",
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> SW_ENC ON(wcid=%d) \n", pEntry->wcid));
	...
	UINT8				GtkLen)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : The BSS-index(%d) is out of range for MBSSID link. \n",
									__FUNCTION__, BssIdx));
		return;
	}
    ...
    if (Wcid == APCLI_MCAST_WCID(BssIdx))
    {
        ...
        u4MaxSharedKeySize = sizeof(pApCliEntry->SharedKey)/sizeof(pApCliEntry->SharedKey[0]);
	if(KeyIdx >= u4MaxSharedKeySize)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): Over buffer size!\n", __FUNCTION__));
   		return;
	}
	...
	NdisZeroMemory(pSharedKey, sizeof(*pSharedKey));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : fails (IF/ra%d) \n",
										__FUNCTION__, BssIdx));
		return;
	}
		...
		GroupCipher == Ndis802_11GroupWEP104Enabled)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : (IF/ra%d) WEP key invlaid(%d) \n",
	...
	else
	{
		...
		if (GtkLen < LEN_TK)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : (IF/ra%d) WPA key invlaid(%d) \n",
										__FUNCTION__, BssIdx, GtkLen));
			return;
		}
	...
	UINT8				KeyTabFlag)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : Unsupport cipher Alg (%d) for Wcid-%d \n",
    ...
    IN  BOOLEAN          bUnicast)
{
	...
	RtmpOSWrielessEventSend(pAd->net_dev, RT_WLAN_EVENT_CUSTOM, -1, NULL, (PUCHAR)custom, strlen(custom));
