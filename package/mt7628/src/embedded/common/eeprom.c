...
INT rtmp_read_txpwr_from_eeprom(RTMP_ADAPTER *pAd)
{
	...
	if (IS_RT6352(pAd))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("E2PROM: D0 target power=0x%x \n", pAd->E2p_D0_Value));
		...
		else
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, (" pAd->TemperatureRef25C = 0x%x\n", pAd->TemperatureRef25C));
		}
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("E2PROM: 40 MW Power Delta= %d \n", pAd->BW_Power_Delta));
...
INT NICReadEEPROMParameters(RTMP_ADAPTER *pAd, RTMP_STRING *mac_addr)
{
	...
	if (IS_MT76x0(pAd))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("0x24 = 0x%04x, 0x0104 = 0x%08x\n", value, reg_val));
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ext_pa_current_setting = %d\n", pAd->chipCap.ext_pa_current_setting));
		...
		if (IS_MT7610E(pAd) && (pAd->chipCap.ext_pa_current_setting == 0))
		{
			...
			*/
			...
			RTMP_IO_READ32(pAd, 0x11C, &MacReg);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): 0x11C = 0x%x\n", __FUNCTION__, MacReg));
		}
#endif /* RTMP_MAC_PCI */
    ...
    if (IS_MT7628(pAd)) {
        ...
        {
            DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("%s(): Antenna field error, RxPath = %d, TxPath = %d\n",
	...
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s(): AfterAdjust, RxPath = %d, TxPath = %d\n",
					__FUNCTION__, Antenna.field.RxPath, Antenna.field.TxPath));
	...
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s(): RxPath = %d, TxPath = %d, RfIcType = %d\n",
					__FUNCTION__, Antenna.field.RxPath, Antenna.field.TxPath,
	...
	if (WMODE_CAP_5G(pAd->CommonCfg.PhyMode))
	{
		...
		{
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR,
	...
	if (IS_MT76x0(pAd))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: EEPROM_MT76x0_TEMPERATURE_OFFSET = 0x%x\n", __FUNCTION__, value));
		if ((value & 0xFF) == 0xFF)
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: TemperatureOffset = 0x%x\n", __FUNCTION__, pAd->chipCap.temp_offset));
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: a_band_mid_ch = %d, a_band_high_ch = %d\n",
...
UCHAR RtmpEepromGetDefault(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s::e2p_dafault=%d\n", __FUNCTION__, e2p_dafault));
	return e2p_dafault;
}
...
efuse:
	...
	if(pAd->bUseEfuse)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("NVM is EFUSE\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Efuse Size=0x%x [Range:%x-%x] \n",
	...
	else
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("NVM is EEPROM\n"));
	}
#endif /* RTMP_EFUSE_SUPPORT */
	...
	switch(iface_type)
	{
		...
		case RTMP_DEV_INF_PCIE:
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("--> E2PROM_CSR=0x%x, EEPROMAddressNum=%d\n",
		...
		case RTMP_DEV_INF_USB:
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("pChipOps->eeread = RTUSBReadEEPROM16\n"));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("pChipOps->eewrite = RTUSBWriteEEPROM16\n"));
			break;
#endif /* RTMP_USB_SUPPORT */
		default:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RtmpChipOpsEepromHook() failed!\n"));
			break;
	}
...
INT RtmpChipOpsEepromHook(RTMP_ADAPTER *pAd, INT infType,INT forceMode)
{
	...
	{
		// TODO: shiang-MT7615
		DBGPRINT(RT_DEBUG_OFF, ("%s(): MT7615 Not Ready yet!\n", __FUNCTION__));
		return -1;
	}
#endif /* MT7615 */
