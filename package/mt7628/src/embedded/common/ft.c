	...
	IN UCHAR RsnIeLen)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("AUTH_RSP - Send FT-AUTH response (%d)...\n", StatusCode));
	...
	OUT PFT_INFO pFtInfoBuf)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<--- %s done. StatusCode(%d)\n", __FUNCTION__, result));
	...
	OUT PFT_INFO pFtInfoBuf)
{
	...
	do 
	{
		...
		if (pFtInfo->RSNIE_Len != 0)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Fast BSS transition in a RSN \n", __FUNCTION__));
	...
	OUT PFT_INFO	pFtInfoBuf)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: it's FT client \n", __FUNCTION__));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: it isn't FT client \n", __FUNCTION__));
	}
	...
	IN PUINT8 pStaMac)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: FT_R1khEntryTab full.\n", __FUNCTION__));
		return -1;
	}
	...
	if(os_alloc_mem(pAd, (PUCHAR *)&pEntry, sizeof(FT_R1HK_ENTRY)) ==
						NDIS_STATUS_FAILURE)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: Unable to alloc memory.\n", __FUNCTION__));
		return -1;
	}
	...
	switch(pFtAction->Action)
	{
		...
		case FT_ACTION_BT_CONFIRM:
			...
			if (IsTerminate)
			{
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: invalid STA.\n", __FUNCTION__));
					goto out;
				}
		...
		default:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: unknow action type (%d).\n", \
	...
	IN INT32 PktLen)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: allocate auth buffer fail!\n",
					__FUNCTION__));
		return;
	} /* End of if */
	...
	OUT	PFT_FTIE_INFO		pFtInfo)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : key idx(%d) \n", __FUNCTION__, key_idx));
	...
	OUT FT_KDP_EVT_KEY_ELM	*pEvtKeyReq)
{
	...
	if (CacheIdx == -1)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : The PMKR0 doesn't exist for %02x:%02x:%02x:%02x:%02x:%02x\n", 
	...
	if (pR1khEntry == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : No initial association information 2 for %02x:%02x:%02x:%02x:%02x:%02x\n", 
							__FUNCTION__, PRINT_MAC(pEvtKeyReq->MacAddr)));
		return FALSE;
	}
		...
		switch(subEidPtr->SubElementId)
		{
			case FT_R0KH_ID:
				if ((subEidPtr->Len > 0) && (subEidPtr->Len <=FT_ROKH_ID_LEN))
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s Invalid R0KHID Length (%d)\n",
			...
			case FT_R1KH_ID:
				if (subEidPtr->Len == FT_R1KH_ID_LEN)
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s Invalid R1KHID Length (%d)\n",
						__FUNCTION__, subEidPtr->Len));
				break;
	...
	OUT		PUCHAR		pR0KH_Id)
{
	...
	*R0KH_IdLen = 0;
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("FT_TEMP- FtIeParse (  FtIeLen = %d )\n", FtIeLen));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("FT_TEMP- FtIeParse ( Len that doesn't include subelement = %d )\n", RemainLen));
	...
	while (RemainLen > 0)
	{
		...
		if (subEidPtr->SubElementId == FT_R1KH_ID)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : R1KHID length(%d)\n", __FUNCTION__, subEidPtr->Len));
		}
		else if (subEidPtr->SubElementId == FT_GTK)
		{
			*GTKLen = subEidPtr->Len;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("FT_TEMP- FtIeParse ( *GTKLen = %d )\n", *GTKLen));
			...
			{
				*GTKLen = 0;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("FT- FtIeParse ( Invalid  GTKLen  = %d)\n", *GTKLen));
			}
		...
		else if (subEidPtr->SubElementId == FT_R0KH_ID)
		{
			*R0KH_IdLen = subEidPtr->Len;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("FT_TEMP- FtIeParse ( *R0KH_IdLen = %d )\n", *R0KH_IdLen));
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("FT- FtIeParse ( Invalid  R0KH_IdLen = %d )\n",*R0KH_IdLen));
				*R0KH_IdLen = 0;
			}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s done\n", __FUNCTION__));
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> FT_CheckForRoaming\n"));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<== FT_CheckForRoaming (BssNr=%d)\n", pRoamTab->BssNr));
	if (pRoamTab->BssNr > 0)
	{
	...
	OUT PUINT8		mic)
{
    ...
    if (OutBuffer == NULL)
    {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!!FT_CalculateMIC: no memory!!!\n"));
		return;
    }
	...
	RTMP_STRING *pBuffer)
{
	...
	if(RTMPGetKeyParameter("FtSupport", tmpbuf, 32, pBuffer, TRUE))
	{
	    ...
	    for (Loop = 0, macptr = rstrtok(tmpbuf,";"); macptr; macptr = rstrtok(NULL,";"), Loop++)
	    {
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("I/F(ra%d) Dot11rFtEnable=%d\n",
	...
	if(RTMPGetKeyParameter("FtRic", tmpbuf, 32, pBuffer, TRUE))
	{
	    ...
	    for (Loop = 0, macptr = rstrtok(tmpbuf,";"); macptr; macptr = rstrtok(NULL,";"), Loop++)
	    {
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("I/F(ra%d) Dot11rFtRic=%d\n",
	...
	if(RTMPGetKeyParameter("FtOtd", tmpbuf, 32, pBuffer, TRUE))
	{
	    ...
	    for (Loop = 0, macptr = rstrtok(tmpbuf,";"); macptr; macptr = rstrtok(NULL,";"), Loop++)
	    {
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("I/F(ra%d) Dot11rFtOtd=%d\n",
		...
		*/
		...
		if(RTMPGetKeyParameter(tok_str, tmpbuf, FT_MDID_LEN + 1, pBuffer, FALSE))
		{
			if (strlen(tmpbuf) == FT_MDID_LEN)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::FtMdid(%d)=%c%c\n", __FUNCTION__, Loop,
											pAd->ApCfg.MBSSID[Loop].FtCfg.FtMdId[0],
											pAd->ApCfg.MBSSID[Loop].FtCfg.FtMdId[1]));
			}
			else
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Invalid MdId=%s\n", __FUNCTION__,tmpbuf));
			}
		}
		...
		*/
		...
		if(RTMPGetKeyParameter(tok_str, tmpbuf, FT_ROKH_ID_LEN + 1, pBuffer, FALSE))
		{
			if (strlen(tmpbuf) <= FT_ROKH_ID_LEN)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::FtR0khId(%d)=%s\n", __FUNCTION__, Loop,
											pAd->ApCfg.MBSSID[Loop].FtCfg.FtR0khId));
			}
			else
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Invalid R0khId(%d)=%s Len=%d\n",
...
INT Set_FT_Enable(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (apidx > pAd->ApCfg.BssidNum)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: Invalid interface number (%d).\n",
			__FUNCTION__, apidx));
		return TRUE;
	}
...
INT Set_FT_R0khid(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (strlen(arg) > FT_ROKH_ID_LEN)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: The length shall be in range from 1 to 48 octects.\n",
			__FUNCTION__));
		return TRUE;
	}		
...
INT	Show_FTConfig_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("MDID=%c%c\n", pFtCfg->FtMdId[0], pFtCfg->FtMdId[1]));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("R0KHID=%s, Len=%d\n", pFtCfg->FtR0khId, pFtCfg->FtR0khIdLen));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("FT Enable=%d\n", pFtCfg->FtCapFlag.Dot11rFtEnable));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("FT RIC=%d\n", pFtCfg->FtCapFlag.RsrReqCap));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("FT OTD=%d\n", pFtCfg->FtCapFlag.FtOverDs));
