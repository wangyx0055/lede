...
static NTSTATUS ResetBulkOutHdlr(IN PRTMP_ADAPTER pAd, IN PCmdQElmt CMDQelmt)
{
	...
	while ((atomic_read(&pAd->PendingRx) > 0) && (!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_NIC_NOT_EXIST)))
	{
		...
		{
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("BulkIn IRP Pending!!cancel it!\n"));
	...
	if ((atomic_read(&pAd->PendingRx) == 0) && (!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_HALT_IN_PROGRESS)))
	{
		...
		for (i = 0; i < (RX_RING_SIZE); i++)
		{
			...
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("RTUSBBulkReceive\n"));
	}
#endif

	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("CmdThread : CMDTHREAD_RESET_BULK_OUT<===\n"));
#endif
...
static NTSTATUS ResetBulkInHdlr(IN PRTMP_ADAPTER pAd, IN PCmdQElmt CMDQelmt)
{
...
#endif /* CONFIG_ATE */
	{
		...
		{
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("BulkIn IRP Pending!!!\n"));
			RTUSBCancelPendingBulkInIRP(pAd);
			RtmpusecDelay(100000);
			pAd->PendingRx = 0;
		}
												...
												fRTMP_ADAPTER_HALT_IN_PROGRESS | fRTMP_ADAPTER_NIC_NOT_EXIST)))))
	{
		...
		for (i = 0; i < pAd->CommonCfg.NumOfBulkInIRP; i++)
		{
			...
			{	/* success*/
				...
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("CMDTHREAD_RESET_BULK_IN: Submit Rx URB Done, status=%d!\n", RTMP_USB_URB_STATUS_GET(pUrb)));
	...
	else
	{
		...
		{
			RTMP_SET_FLAG(pAd, fRTMP_ADAPTER_NIC_NOT_EXIST);
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("CMDTHREAD_RESET_BULK_IN: Read Register Failed!Card must be removed!!\n\n"));
		}
		else
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("CMDTHREAD_RESET_BULK_IN: Cannot do bulk in because flags(0x%lx) on !\n", pAd->Flags));
	}
