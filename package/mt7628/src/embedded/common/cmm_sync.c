...
VOID BuildChannelList(RTMP_ADAPTER *pAd)
{
	...
	for (i=0; i<pAd->ChannelListNum; i++)
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE,("\tChannel # %d: Pwr0/1 = %d/%d, Flags = %x\n ",
									 pAd->ChannelList[i].Channel,
...
UCHAR NextChannel(RTMP_ADAPTER *pAd, UCHAR channel)
{
	...
	if (pAd->ScanCtrl.ScanType == SCAN_P2P_SEARCH)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Error !! P2P Discovery state machine has change to Listen state during scanning !\n"));
			return next_channel;
		}
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("SYNC -  next_channel equals to CurrentChannel= %d\n", next_channel));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("SYNC -  ListenChannel List : %d  %d  %d\n",
...
#endif
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("SYNC - P2P Scan return channel = %d. Listen Channel = %d.\n",
	...
	IN UCHAR channel)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
	{
		...
		/*7636 psm*/
		if (pAd->StaCfg.wdev.Psm == PWR_SAVE)
		{
	...
	if (pAd->OpMode == OPMODE_STA)
	{
			...
			/* && (pAd->StaCfg.WscControl.bWscTrigger == TRUE)) */
		{
			...
			if (os_alloc_mem(pAd, (UCHAR **)&pWscBuf, 512) == NDIS_STATUS_SUCCESS)
			{
				...
				/* MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Created WPS IE for Probe Request; Len = %d\n", WscIeLen)); */
	...
	/* MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Really Sending out Probe Request\n")); */
