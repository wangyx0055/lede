...
/* 
	Winbond-arm platform sometimes will fail to swap 32/16 bytes; therefore 
	the following is used to fix this bug
*/
#endif /* RELEASE_EXCLUDE */
  UINT32 WINBOND_GET32(unsigned char *source)
{
    unsigned char swaper[4];
    UINT32 ret32;
    ...
    memcpy((void *)&ret32,swaper,4);
    return ret32;

}
UINT16 WINBON_GET16(unsigned char *source)
{
    unsigned char swaper[2];
    UINT32 ret16;

    swaper[0] = *(source+1);
    swaper[1] = *(source+0);

    memcpy((void *)&ret16,swaper,2);
    return ret16;
    ...
    unsigned char *cipherText,  int *ctx_len)
{
    ...
    if(0 != RT_EVP_EncryptInit(&ctx, RT_EVP_aes_128_cbc(), key, iv))
    {
    ...
    *ctx_len = 0;
    while(data_len)
    {
        ...
        if(0 == RT_EVP_EncryptUpdate(&ctx, outBuf, &outLen, bufPtr, currentLength))
        {
    ...
    if(0 == RT_EVP_EncryptFinal(&ctx, outBuf, &outLen))
    {
    ...
    unsigned char *plainText,   int *ptx_len)
{
    ...
    if(0 != RT_EVP_DecryptInit(&ctx, RT_EVP_aes_128_cbc(), key, iv))
    {
    ...
    *ptx_len=0;
    while(data_len)
    {
        ...
        if(0 == RT_EVP_DecryptUpdate(&ctx, outBuf, &outLen, bufPtr, currentLength))
        {
    ...
    if(0 == RT_EVP_DecryptFinal(&ctx, outBuf, &outLen))
    {
#ifdef RELEASE_EXCLUDE
