	...
	OUT PUCHAR				Ssid)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("MBSSID[%d] do not support Easy Config, skip the checking EC OUI\n",apidx));
		return;
	}
    ...
    IN	UCHAR			ApIdx)
{
	...
	if (DataLen > 255)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s: DataLen > 255\n", __FUNCTION__));
		return FALSE;
	}
	...
	if (pBuf == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s: pBuf alloc fail !!\n", __FUNCTION__));
		return FALSE;
	}
	...
	OUT	PUCHAR				pMIC)
{
	...
	if(pTemp == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s: pTemp alloc fail !!\n", __FUNCTION__));
		return;
	}
	...
	if(pDigest == NULL)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s: pDigest alloc fail !!\n", __FUNCTION__));
		return;
	}
	...
	OUT PUCHAR			pDataLen)
{
	...
	if (pIE_Data == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s pIE_Data alloc fail !!\n", __FUNCTION__));
		return;
	}
	...
	IN 	UCHAR			OutDataLen)
{
	...
	while((eid_data + eid_len) <= ((UCHAR*)pInData + InDataLen))
    {
		if (eid == AttrID)
		{
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s: OutData is not enough to record data, eid_len = %d\n", 
	...
	IN  PEASY_CONFIG_INFO	pEasyInfo)
{
	switch(pWAC_Req->FrameType)
	{
		...
		default:
			pWAC_Req->RV = FALSE;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s: Unknow FrameType = 0x%x\n", 
	...
	IN  RTMP_IOCTL_INPUT_STRUCT	*wrq)
{
	...
	if (pWAC_Req == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s: pWAC_Req alloc fail !!\n", __FUNCTION__));
		return -ENOMEM;
	}
		...
		if (Status == NDIS_STATUS_SUCCESS)
		{
			...
			if (pWAC_Req->bReqType)
			{
				...
				switch (pWAC_Req->Command)
				{
					...
					default:
						pWAC_Req->RV = FALSE;
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s: Unknow command = 0x%x\n", 
	...
	if (pAd->ApCfg.MBSSID[ApIdx].EasyConfigInfo.bEnableWAC)
	{
		...
		if (pIE_Data)
		{
			...
			else
			{
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s: EapMsgRunning = %d\n", __FUNCTION__, 
...
#endif /* WSC_AP_SUPPORT */
				{
					if (PeerRssi < -25)
					{
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s: PeerRssi = %d\n", __FUNCTION__, PeerRssi));
						pIE_Data[11] = 3;
    ...
    {    	
        switch(eid)
        {
	        case IE_VENDOR_SPECIFIC:
				...
				{
					oui_type = eid_data[3];
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Samsung OUI Type = 0x%02X\n", __FUNCTION__, oui_type));
					if (oui_type == SAMSUNG_OUI_TYPE)
					{
						...
						{
							/* SAMSUNG DMC */
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: WAC_DEVICE_INFO_ID Parsing...\n", __FUNCTION__));
							...
							{
								MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: WAC Device Information Parsing Error \n", __FUNCTION__));					
							}
										  ...
										  8))
							{
								...
								*/
								...
								MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: WscPinCode = %08u\n", __FUNCTION__, pWpsCtrl->WscPinCode));
#endif /* WSC_INCLUDED */
							}
	...
	IN  MAC_TABLE_ENTRY *pEntry)
{
	if (pEntry == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: pEntry is NULL!!\n", __FUNCTION__));
		return;
	}
	...
	INOUT ULONG			*pFrameLen)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Found Samsung WAC OUI - bPeerWAC = %d, ParseResult = 0x%x\n", __FUNCTION__, bPeerWAC, ParseResult));
		os_alloc_mem(NULL, &pWAC_IE, 128);
		if (pWAC_IE)
		{
			ULONG WACTmpLen = 0;
	...
	for (i = 0; i < pAd->ScanTab.BssNr; i++) 
	{
		...
		if ((pInBss->WpsAP & 0x04) && (Count < 8))
		{
			...
			else
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s: pInBss->Rssi = %d\n", __FUNCTION__, pInBss->Rssi));
			}
		}
	}
