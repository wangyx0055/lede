...
static VOID RFConfigRecover(RTMP_ADAPTER *pAd, RF_REG_PAIR RF_record[][13])
{
	...
	for (chain_index = 0; chain_index < 2; chain_index++)
	{
		for (record_index = 0; record_index < 13; record_index++)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("bank: %d, rf_register: %d, value: %x\n",
...
UINT32 Do_FFT_Accumulation(IN RTMP_ADAPTER *pAd, UCHAR tone_idx, UCHAR read_neg)
{
	...
	power_int = power_tmp;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("I = %d,  Q = %d, power = %x\n", fftout_i, fftout_q, power_int));
	if(read_neg){
...
UINT32 Read_FFT_Accumulation(IN RTMP_ADAPTER *pAd, UCHAR tone_idx)
{
	...
	power_tmp = power_tmp + (fftout_q * fftout_q);
	power_int = power_tmp;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("I = %d,  Q = %d, power = %x\n", fftout_i, fftout_q, power_int));
	...
	OUT UCHAR dc_result[][RF_ALC_NUM][2])	
{
	...
	for (bit_index = 5; bit_index >= 0; bit_index--)
	{
		for (iorq = 0; iorq <= 1; iorq++)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("alc=%u, IorQ=%u, idx_final=%2x\n", alc_idx, iorq, index_final[iorq])); 
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("pwr0=%x, pwr1=%x, pwr_final=%x, idx_0=%x, idx_1=%x, inverted_bit=%x !\n"
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("IorQ=%u, idx_final[%u]:%x, pwr_final:%8x\n"
	...
	for (iorq = 0; iorq <= 1; iorq++){
		...
		for (index0 = -31; index0 < 31; index0 = index0 + 1)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("alc=%u, IorQ=%u, idx_0=%x, pwr0=%x, idx_final=%2x  pwr_final=%x, !\n",
	...
	OUT UCHAR *phase_error_store)	
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IQCalibration Start!\n"));
#if 1 
	for (bit_index = 5; bit_index >= 1; bit_index--)
	{
		for (gain_or_phase = 0; gain_or_phase < 2; gain_or_phase++)
		{
			...
			if ((gain_or_phase == 1) || (bit_index < 4))
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("pwr0=%x, pwr1=%x, pwer_final=%x, index0=%x, index1=%x, iq_err=%x, gain_or_phase=%d, inverted_bit=%x !\n",
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("IQCalibration pwr_final=%8x (%2x, %2x) !\n"
	...
	for(gain_err_fine = gain_st; gain_err_fine <= gain_end; gain_err_fine = gain_err_fine + 1)
		for(phase_err_fine = phase_st; phase_err_fine <= phase_end; phase_err_fine = phase_err_fine + 1){
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Fine IQCalibration pwr1=%8x pwr_final=%8x (%2x, %2x) !\n",
	...
	for (gain_or_phase = 0; gain_or_phase < 2; gain_or_phase++){
		...
		for(	iq_err =  ((gain_or_phase == 0) ? -7 : -31);
			...
			if(gain_or_phase == 0)
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("chain_idx %d, iq_err %x, pwr0 %x, gain_err %x, pwr_final %x\n",
							 chain_idx, iq_err & 0xf, pwr0, gain_err, pwr_final));
			else
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("chain_idx %d, iq_err %x, pwr0 %x, phase_err %x, pwr_final %x\n",
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IQCalibration Done! CH = %u, (gain=%2x, phase=%2x)\n"
...
static VOID RF_SELF_TXDC_CAL(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("RF Tx_0 self calibration start\n"));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("RF Tx self calibration end\n"));
...
//    UCHAR VGA_gain[]     = {0x3b, 0x3b, 0x3b, 0x3b, 0x3b, 0x3bi;
	...
	*/
			...
			/* step 12: manually set RF VGA gain */
 			if(rf_alc_idx == 0) {				
				...
				for(;VGA_gain[chain_idx] > 0;VGA_gain[chain_idx] = VGA_gain[chain_idx] -2){ 
					...
					pwr1 = Do_FFT_Accumulation(pAd, 0xa,0);
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("LOFT AGC %d %d\n",pwr0,pwr1)); 
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Used VGA %d %x\n",VGA_gain[chain_idx], RFVGA_gain_table[VGA_gain[chain_idx]])); 
	...
	for (rf_alc_idx = 0; rf_alc_idx < 3; rf_alc_idx++)
	{
//		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("dc_result[0][%u][0]=%x, dc_result[0][%u][1]=%x\n"
//			, rf_alc_idx, LOFT_DC_Search_Result[0][rf_alc_idx][0], rf_alc_idx
//			, LOFT_DC_Search_Result[0][rf_alc_idx][1])); 
//		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("dc_result[1][%u][0]=%x, dc_result[1][%u][1]=%x\n"
...
//			table_dc_q1 = LOFT_DC_Search_Result[1][(idx == 0) ? 0 : (idx+2)][1] - LOFT_DC_Search_Result[1][0][1];
			...
			RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R159, BBPValue);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" ALC %2x,", BBPValue));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("LOFT Calibration Done!\n"));
	...
	*/
...
#endif /* RT6352_EP_SUPPORT */
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IQ AGC %d %d\n",pwr0,pwr1));
#ifdef RT6352_EP_SUPPORT
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IQ AGC IDX 10 %d %d\n",pwr0_idx10,pwr1_idx10));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("TX IQ Calibration Done!\n"));
