		...
		while (AGCtimeOutCount < 20) // SW AGC update to make sure I peak value can prevent peak value from satuation or too low
		{
			...
			switch (i)
			{
				case 0: 
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Loop0\n"));
					//printk("Path0 0x34=%x\n", rfValue[0]);
					//printk("Path1 0x34=%x\n", rfValue[1]);
					break;
				case 1: 
					...
					//mt76x2_rf_read(pAd, RF_Path0, 0x34, &rfValue[0]); 	
					//mt76x2_rf_read(pAd, RF_Path1, 0x34, &rfValue[1]);
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Loop1\n"));
					//printk("Path0 0x34=%x\n", rfValue[0]);
					//printk("Path1 0x34=%x\n", rfValue[1]);
					break;
				default:
			...
			while (phaseCaliStatus & 0x80)
			{
				if (timeOutCount == 10) 
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("phaseCaliStatus = %x\n", phaseCaliStatus));
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("LNA HW calibration can't finish process\n"));
					break;
				}
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("SW AGC = %d\n", VGAGainIdx[i]));
			AGCtimeOutCount++;	
#else
			AGCtimeOutCount = 20;
			SwAgc1stflg = FALSE;
	...
	UCHAR r25Setting)
{
	...
	*/
	...
	/*MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("==>CalCap: i=%dus\n", i*20)); */
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("DoCalibrationCapture c=%x, t1=%ld t2=%ld\n", capCtrl,
	...
	OUT UCHAR *divPhase)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s : Divider calibration duration1 = %d ms\n", 
	...
	NdisGetSystemUpTime(&stTimeChk1);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s : Divider calibration duration2 = %d ms\n", 
		     __FUNCTION__, (stTimeChk1 - stTimeChk0)*1000/OS_HZ));
	...
	OUT UCHAR *divPhase)
{
	...
	//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("==> ITxBFDividerCalibration cf=%d cm=%d\n", calFunction, calMethod));
	...
	
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s(cnt=%d,time=0x%lx):stop DTx,dmaCfg=%d!\n",
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s(cnt=%d,time=0x%lx):stop DRx, dmaCfg=%d!\n",
			...
			(dt_time > 50) || (dr_time > 50) || (mr_time> 50) || (mt_time > 50))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s():After Disable DMA/MAC Tx/Rx, dmaBusy=%d,macBusy=%d!\n",
                                __FUNCTION__, (dmaCfg & 0xa), (macStatus & 0x3)));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s():DMA=>Tx(time:cycle)=(0x%lx:%d), Rx(time:cycle)=(0x%lx:%d)!\n",
					__FUNCTION__, dt_time, DTxCycle, dr_time, DRxCycle));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s():MAC=>Tx(time:cycle)=(0x%lx:%d), Rx(time:cycle)=(0x%lx:%d)!\n", 
					__FUNCTION__, mt_time, MTxCycle, mr_time, MRxCycle));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s():PBF=>Original:0x%x, now:0x%x!\n", 
	...
	case 1:
		...
		/* Display parameters */
		if (displayParams) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Method #1\nPhase0 = [%d %d] deg\n"
						 "Phase1 = [%d %d %d] deg\n"
						 "Peak  = [%d, %d, %d] [%d, %d, %d]\n"
						 "Delta = [%d, %d] deg\n",
	...
	case 2:
		...
		/* Display parameters */
		if (displayParams) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Method #2\nPhase = [%d, %d] deg\n"
						 "Peak  = [%d, %d, %d]\n"
						 "Delta = [%d, %d] deg\n",
						DEG(phase[0][0]), DEG(phase[0][2]),
	...
	default:
		...
		/* Display parameters */
		if (displayParams) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Method #3\n"
		...
		*/
		...
		if (channel == 1) {
			...
			*/
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Invalid channel: %d\nMust calibrate channel 1, 14, 36, 120 or 165", channel) );
			result = FALSE;
			goto exitDivCal;
		}
...
exitDivCal:
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s t1=%ld t2=%ld t3=%ld t4=%ld t5=%ld\n", __FUNCTION__,
			tval1.tv_usec - tval0.tv_usec, tval2.tv_usec - tval0.tv_usec,
	...
	OUT UCHAR *divPhase)
{
	...
	
	{
		...
		for (DRxCycle = 0; DRxCycle < ITXBF_MAX_WAIT_CYCLE; DRxCycle++)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s: Inband state = 0x%x \n", 
		...
		if (DRxCycle == ITXBF_MAX_WAIT_CYCLE)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s(cnt=%d,time=0x%lx):BBp idle, dmaCfg=%d!\n",
					__FUNCTION__, DRxCycle, dr_time, dmaCfg));
		}
	}	

	NdisGetSystemUpTime(&stTimeChk1);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s: Divider calibration duration3 = %d ms\n", 
		     __FUNCTION__, (stTimeChk1 - stTimeChk0)*1000/OS_HZ));
	...
	IN int calMethod)
{
	...
	if (displayParams) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("LNA\n"));
		for (i=0; i<capCount; i++) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,
					("  %c Phase = [%d __ %d] deg, "
					 "  Peak = [%d %d %d]\n",
	...
	IN int calMethod)
{
	...
	if (displayParams) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("Diff LNA Method #%d\n", calMethod));
	...
	IN int calMethod)
{
	...
	if (displayParams) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("LNA Method #%d\n", calMethod));
	...
	IN BOOLEAN gBand)
{
	...
	switch (calFunction) {
	case 0:
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("M-L/H-L/H-M:  [%d %d %d] deg = [0x%02X  0x%02X]\n"
	...
	IN	BOOLEAN implicitProfile)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%d: ", profileNum));

	Read_TagField(pAd, row, profileNum);
	for (byteIndex=EXP_MAX_BYTES; --byteIndex >= 0; )
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%02X ", row[byteIndex]));
	...
	if (implicitProfile) {
		...
		case 2:
		case 3:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s %dx%s %c", bwTable[row[7] & 0x7],
				(row[0] & 0x3)+1, modeTable[mode], (row[7] & 0x80)? 'S': 'L') );
			break;
		default:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Invalid"));
		}
	}
	else {
		...
		case 9:
		case 10:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%2dM %s", (row[7] & 0x10)? 40: 20, tagTable[row[7] & 0xF]) );
			break;
		default:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Invalid"));
		}
