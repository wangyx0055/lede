...
INT TYPE_FUNC FT_KDP_CMD_R0KH_InfoShow(RTMP_ADAPTER *pAd, RTMP_STRING *pArgv)
{
	...
	while(pInfo != NULL)
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("\n%03d. R0KHID = 0x", IdInfo));
		...
		for(IdArray=0; IdArray<sizeof(pInfo->R0KHID); IdArray++)
		{
			...
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, (" %02x", pInfo->R0KHID[IdArray]));
		} /* End of for */
	...
	IN	CHAR				*pArgv)
{
	...
	if (pFtKdp == NULL)
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("ft_kdp> No any event!\n"));
		return;
	} /* End of if */
	...
	while(pFtKdp != NULL)
	{
		switch(pFtKdp->Sig)
		{
			case FT_KDP_SIG_KEY_TIMEOUT:
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("KEY TIMEOUT\n"));
				break;

			case FT_KDP_SIG_KEY_REQ:
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("KEY REQUEST\n"));
				break;

			case FT_KDP_SIG_FT_ASSOCIATION:
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("STATION FT ASSOCIATION\n"));
				break;

			case FT_KDP_SIG_TERMINATE:
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("TERMINATE\n"));
				break;

			default:
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("UNKNOWN\n"));
				break;
		} /* End of switch */
...
static VOID TYPE_FUNC FT_RIC_CMD_SimRscReqHandle(
	...
	for(IdRsc=0; IdRsc<gFT_RIC_TspecIndex; IdRsc++)
	{
		...
		if (pElmDataInfo->StatusCode == 0)
		{
			...
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("Medium Time(%d)",
	...
	IN	CHAR				*pArgv)
{
	...
	for (HashIdx = 0; HashIdx < FT_R1KH_ENTRY_HASH_TABLE_SIZE; HashIdx++)
	{
		...
		while (pEntry != NULL)
		{
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("StaMac=%02x:%02x:%02x:%02x:%02x:%02x, ",
			...
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("PairwisChipher="));
			for (i=0; i<4; i++)
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("%02x:", pEntry->PairwisChipher[i]));
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("\nAkmSuite="));
			for (i=0; i<4; i++)
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("%02x:", pEntry->AkmSuite[i]));
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("\nPmkR0Name="));
			for (i=0; i<16; i++)
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("%02x:", pEntry->PmkR0Name[i]));
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("\nPmkR1Key="));
			for (i=0; i<32; i++)
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("%02x:", pEntry->PmkR1Key[i]));
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("\nPmkR1Name="));
			for (i=0; i<16; i++)
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("%02x:", pEntry->PmkR1Name[i]));
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("\n"));
			pEntry = pEntry->pNext;
		}
	}
	...
	IN	CHAR				*pArgv)
{
	...
	/* Make 802.11 header. */
	ActHeaderInit(pAd, &Hdr, pAd->ApCfg.MBSSID[FtEntry->func_tb_idx].wdev.bssid,
		FtEntry->Addr, pAd->ApCfg.MBSSID[FtEntry->func_tb_idx].wdev.bssid);
...
static VOID TYPE_FUNC FT_RIC_CMD_StatusDisplay(FT_RIC_STATUS *pRspStatus)
{
	...
	if (pRspStatus->FlgHasBaResource == TRUE)
	{
		...
		{
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("\tBA Resource is accepted.\n"));
		}
	else
		{
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("\tBA Resource is not accepted.\n"));
		} /* End of if */
	} /* End of if */

	if (pRspStatus->TspecNumberOfRequested > 0)
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("\tNumber of requested TSPEC: %d\n",
					pRspStatus->TspecNumberOfRequested));
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("\tNumber of accepted TSPEC: %d\n",
					pRspStatus->TspecNumberOfAccepted));

		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("\tTotal requested Time: %d us\n",
					pRspStatus->TspecMediumTimeTotalCur));
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("\tTotal accepted Time: %d us\n",
					pRspStatus->TspecMediumTimeTotalNew));
	} /* End of if */

	if (pRspStatus->UnknownRDIE > 0)
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_OFF, ("\tNumber of unknown RDIE: %d\n",
...
INT TYPE_FUNC FT_Ioctl(RTMP_ADAPTER *pAd, RTMP_STRING *pArgvIn)
{
	...
	switch(Command)
	{
		...
		case FT_KDP_CMD_DAEMON_KILL:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft_iocl> kill daemon!\n"));
			FT_KDP_EventInform(pAd, BSS0, FT_KDP_SIG_TERMINATE, NULL, 0, 0, NULL);
			break;

#ifndef FT_KDP_FUNC_SOCK_COMM
		case FT_KDP_CMD_EVENT_LIST:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft_iocl> event list!\n"));
			FT_KDP_CMD_EventList(pAd, Argc, pArgv);
			break;
#endif /* FT_KDP_FUNC_SOCK_COMM */

		case FT_KDP_CMD_DEBUG_FLAG_CTRL:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft_iocl> IAPP daemon debug flag control!\n"));
		...
		case FT_KDP_CMD_SM_EVT_FT_ASSOC:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft_iocl> (sm) ft assoc!\n"));
			FT_KDP_CMD_SimEvtFtAssoc(pAd, Argc, pArgv);
			break;

		case FT_KDP_CMD_SM_EVT_FT_REASSOC:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft_iocl> (sm) ft reassoc!\n"));
			FT_KDP_CMD_SimEvtFtReAssoc(pAd, Argc, pArgv);
			break;

		case FT_KDP_CMD_SM_EVT_KEY_REQ:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft_iocl> (sm) key req!\n"));
			FT_KDP_CMD_SimKeyReq(pAd, Argc, pArgv);
			break;

#ifdef FT_KDP_FUNC_R0KH_IP_RECORD
		case FT_KDP_CMD_SM_R0KH_INFO_CREATE:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft_iocl> (sm) R0KH INFO create!\n"));
			FT_KDP_CMD_SimR0KH_InfoCreate(pAd, Argc, pArgv);
			break;
#endif /* FT_KDP_FUNC_R0KH_IP_RECORD */

		case FT_RIC_CMD_SM_REQ_HANDLE:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft_iocl> (sm) handle a resource request!\n"));
			FT_RIC_CMD_SimRscReqHandle(pAd, Argc, pArgv);
			break;

		case FT_RIC_CMD_SM_REQ_HDL_TSPEC:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft_iocl> (sm) make a resource request TSPEC!\n"));
			FT_RIC_CMD_SimRscReqHdlTspec(pAd, Argc, pArgv);
			break;

		case FT_RIC_CMD_SM_RRB_SEND:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft_iocl> (sm) send a RRB frame!\n"));
			FT_RRB_CMD_SimSend(pAd, Argc, pArgv);
			break;

		case FT_11K_CMD_INFO_REQ:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft_iocl> (sm) send a info request frame!\n"));
			FT_11K_CMD_SimInfoReq(pAd, Argc, pArgv);
			break;

		case FT_KDP_KEY_SHOW:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft_iocl> (sm) show key information!\n"));
		...
		case FT_KDP_CMD_SM_R0KH_INFO_SHOW:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft_iocl> (sm) R0KH INFO show!\n"));
			FT_KDP_CMD_R0KH_InfoShow(pAd, (RTMP_STRING *)pArgv);
			break;
#endif /* FT_KDP_FUNC_R0KH_IP_RECORD */

		case FT_R1KH_INFO_SHOW:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft_iocl> (sm) R1KH INFO show!\n"));
			FT_R1KH_InfoShow(pAd, Argc, pArgv);
			break;

		case FT_REQ_ACT:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft_iocl> FT_REQ_ACT!\n"));
			FT_OverDs_SimReq(pAd, Argc, pArgv);
			break;

		case FT_CONFIRM_ACT:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft_iocl> FT_CONFIRM_ACT!\n"));
		...
		case FT_RIC_CMD_SM_REQ_START:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft_iocl> (sm) Resource Request Start!\n"));
			FT_RIC_CMD_SimRscReqStart(pAd, Argc, pArgv);
			break;

		case FT_RIC_CMD_SM_REQ:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft_iocl> (sm) Resource Request!\n"));
			FT_RIC_CMD_SimRscReq(pAd, Argc, pArgv);
			break;

		case FT_RIC_CMD_SM_REQ_END:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft_iocl> (sm) Resource Request End!\n"));
			FT_RIC_CMD_SimRscReqEnd(pAd, Argc, pArgv);
			break;

		case FT_RIC_CMD_SM_REQ_RSP_LIST:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft_iocl> (sm) Resource Req/Rsp List!\n"));
