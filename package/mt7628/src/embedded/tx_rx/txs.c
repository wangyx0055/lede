...
{	
	...
	{
		DBGPRINT(RT_DEBUG_OFF, ("got TXS, MakeIbssBeacon\n"));
		MakeIbssBeacon(pAd);
	}
					...
					TXS_HANDLER TxSHandler, BOOLEAN DumpTxSReport)
{
	...
	DlListForEach(SearchTxSType, &TxSCtl->TxSType[Pid % TOTAL_PID_HASH_NUMS], TXS_TYPE, List) 
	{
		...
		{
			DBGPRINT(RT_DEBUG_OFF, ("%s: already registered TxSType (PID = %d, Format = %d\n",
						...
						BOOLEAN TxS2Mcu, BOOLEAN TxS2Host)
{
	...
	DBGPRINT(RT_DEBUG_OFF, ("%s: can not find TxSType(PID = %d, Format = %d)\n", 
								__FUNCTION__, Pid, Format));
	return -1;
}
...
{ 
	...
	{
		DBGPRINT(RT_DEBUG_OFF, ("\tType=TimeStamp/FrontTime Mode(TXSFM=%d, \
		...
		DBGPRINT(RT_DEBUG_OFF, ("\t\tTimeStamp=0x%x, FrontTime=0x%x, TxPwr=0x%x\n",\
                                    txs_d1->timestamp, txs_d2->field_ft.front_time, txs_d2->field_ft.tx_pwr_dBm));
		DBGPRINT(RT_DEBUG_OFF, ("\t\tTxDelay=0x%x, RxVSeqNum=0x%x, Wlan Idx=0x%x\n",\
                                    txs_d3->transmission_delay, txs_d3->rxv_sn, txs_d3->wlan_idx));
		DBGPRINT(RT_DEBUG_OFF, ("\t\tSN=0x%x, TxBW=0x%x, AMPDU=%d, PID=0x%x, MPDU TxCnt=%d, MCS Idx=%d\n",
                                    txs_d4->sn, txs_d4->tbw, txs_d4->am, txs_d4->pid, txs_d4->mpdu_tx_cnt, txs_d4->last_tx_rate_idx));

	}
	else if (Format == TXS_FORMAT1)
	{
		DBGPRINT(RT_DEBUG_OFF, ("\tType=Noisy/RCPI Mode\n"));
	}
	else
	{
		DBGPRINT(RT_DEBUG_ERROR, ("%s: Unknown TxSFormat(%d)\n", __FUNCTION__, Format)); 
	}
}
