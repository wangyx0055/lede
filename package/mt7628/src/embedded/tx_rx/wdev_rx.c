	...
	IN UCHAR op_mode)
{
	...
	TRACE_IPARP_RX_CHK_PNT(pAd, RTMP_GET_PACKET_TRACEIPARP(pPacket), P11, 0);
...
#define REORDERING_PACKET_TIMEOUT		((100 * OS_HZ)/1000)	/* system ticks -- 100 ms*/
		...
		if (Wcid < MAX_LEN_OF_MAC_TABLE)
		{
			...
			if (Idx != 0)
			{
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Indicate_Legacy_Packet():flush reordering_timeout_mpdus! pRxBlk->Flags=%d, pRxBlk->TID=%d, pRxInfo->AMPDU=%d!\n",
...
#if 0
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("pCliHwAddr = %02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	IN UCHAR wdev_idx)
{
		...
		if ((sup_info != NULL) && (sup_info->WpaSupplicantUP))
		{
			...
			*/
				...
				(eapcode == EAP_CODE_SUCCESS))
			{
				...
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("Receive EAP-SUCCESS Packet\n"));
#ifdef CONFIG_AP_SUPPORT
				if (IS_ENTRY_APCLI(pEntry)) {
	...
	if (to_mlme)
	{
		...
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE,
...
#if 0//def MAC_REPEATER_SUPPORT
				if (pAd->chipCap.hif_type == HIF_MT)
				{
					/* for check duplicate BAR frame */
					if ((last_wcid == pRxBlk->wcid) && (last_ssn == bar->BAStartingSeq.StartSeq))
					{
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): BAR packet duplicated Wcid %d, Seq %d\n",
                                 __FUNCTION__, pRxBlk->wcid, bar->BAStartingSeq.StartSeq));
						break;
					}

					last_wcid = pRxBlk->wcid;
					last_ssn = bar->BAStartingSeq.StartSeq;				
				}
#endif
#endif /* MT_MAC */
			...
			if (pAd->CommonCfg.vht_bw_signal && pRxBlk->wcid <= MAX_LEN_OF_MAC_TABLE)
			{
				...
				if ((rts->Addr1[0] & 0x1) == 0x1) {
					...
					if (srv_field->dyn_bw == 1) {
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%02x:%02x:%02x:%02x:%02x:%02x, WCID:%d, DYN,BW=%d\n",
...
//---Add by shiang for debug
		...
		default:
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("ignore CNTL (subtype=%d)\n",
						pHeader->FC.SubType));
...
//+++Add by shiang for work-around, should remove it once we correctly configure the BSSID!
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
	{
		...
		if ((pRxBlk->DataSize < 14) || (pRxBlk->MPDUtotalByteCnt > MAX_AGGREGATION_SIZE))
		{
			...
			*/
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("rx pkt len err(%d, %d)\n",
		...
		if (pHdr)
		{
			...
			{
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("%s: Line(%d) (ToDs Packet not allow in STA Mode)\n", __FUNCTION__, __LINE__));
				return NDIS_STATUS_FAILURE;
			}
#endif /* CLIENT_WDS */
		}
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
	{
		...
		{
#ifdef RELEASE_EXCLUDE
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO,("CIPHER error = %d\n",pRxInfo->CipherErr));
#endif /* RELEASE_EXCLUDE */

			if (pRxInfo->CipherErr == 2)
				{DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE,("RxErr: ICV ok but MICErr"));}
			else if (pRxInfo->CipherErr == 1)
				{DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE,("RxErr: ICV Err"));}
			else if (pRxInfo->CipherErr == 3)
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE,("RxErr: Key not valid"));
			...
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE,("%s(): %d (len=%d, Mcast=%d, MyBss=%d, Wcid=%d, KeyId=%d)\n",
...
INT ate_rx_done_handle(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
	...
	if (pAd->chipCap.FlgHwTxBfCap)
	{
		...
		if (pHeader->FC.Type == FC_TYPE_MGMT)
		{
			...
			{
				if (pHeader->FC.Order)
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL,  DBG_LVL_WARN, ("fcsubtype=%x\ndata[0]=%x\ndata[1]=%x\n",
	...
	if (bDirectForward)
	{
		...
		else if (RouteId >= 0)
		{
			...
			if ((LinkId >= 0) && VALID_MESH_LINK_ID(LinkId))
			{
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): tx_pkt_handle not assigned!\n",
								__FUNCTION__));
...
#endif /* CONFIG_STA_SUPPORT */
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Atheros Problem. Turn on RTS/CTS!!!\n"));
#endif /* RELEASE_EXCLUDE */
	}
#endif /* WFA_VHT_PF */
...
#ifdef TRACELOG_IPARP
		hdr_protype = NdisEqualMemory(SNAP_802_1H, pRxBlk->pData, 6) ? (pRxBlk->pData + 6) : pRxBlk->pData;
		TRACE_IPARP_SET_RX_STARTING_PNT(pAd, pRxBlk->wcid, pRxBlk->pRxPacket, hdr_protype);
		TRACE_IPARP_RX_CHK_PNT(pAd, RTMP_GET_PACKET_TRACEIPARP(pRxBlk->pRxPacket), P0, 0);
		TRACE_IPARP_RECORD_RX_MCS_SETTING(pAd, RTMP_GET_PACKET_TRACEIPARP(pRxBlk->pRxPacket), &pRxBlk->rx_rate);
#endif /* TRACELOG_IPARP */
...
if (!(pRxInfo->Mcast || pRxInfo->Bcast)){
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("-->%s(%d): Dump RxData FramePayload=%d\n", __FUNCTION__, __LINE__, pRxBlk->DataSize));
	dump_rxinfo(pAd, pRxInfo);
...
if (!(pRxInfo->Mcast || pRxInfo->Bcast)) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Dump the RxBlk info, MC/BC:%d,%d\n", pRxInfo->Mcast, pRxInfo->Bcast));
		dump_rxblk(pAd, pRxBlk);
...
#ifdef RELEASE_EXCLUDE
   	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Rcv packet to IF(ra%d)\n", wdev->wdev_idx));
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO, ("!!! report BCAST DATA to LLC (len=%d) !!!\n", pRxBlk->DataSize));
#endif /* RELEASE_EXCLUDE */
		return;
	}
#endif /* CONFIG_STA_SUPPORT */
...
extern volatile PRX_PKT_QUEUE p_queue ;
extern int enqueue(PRX_PKT_QUEUE p_queue,UCHAR * data, USHORT datasize);
...
//+++Add by shiang for debug
		...
		if (1 || (pHeader->FC.Type == FC_TYPE_DATA)) {
			...
			if (pAd->chipCap.hif_type == HIF_RLT) {
				...
				RTMPWIEndianChange(pAd , (UCHAR *)pRxWI, TYPE_RXWI);
#endif /* RT_BIG_ENDIAN */
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Dump the RxWI:\n"));
...
#ifdef CONFIG_ELIAN_SUPPORT
		//24 ==M80211_HEADER_LEN_MIN
		if(pHeader->FC.Type ==FC_TYPE_DATA && pRxBlk->DataSize <= RX_80211_PKT_SIZE)
		{
			enqueue(p_queue,pRxBlk->pData,pRxBlk->DataSize);
			sccb_rx(pRxBlk->pData,pRxBlk->DataSize);
		}	
#endif
