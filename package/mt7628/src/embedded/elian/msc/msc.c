...
void msc_stop(void *priv)
{
	...
	{	
		MSC_PRINT(MSC_DEBUG_OFF,("chl_switch thread not finished, waiting 50ms!\n"));
		msc_msleep(50);
		if(count++ >= 40)
		{
			timeout=1;
	...
	if(timeout)
	{
		MSC_PRINT(MSC_DEBUG_ERROR,("chl_switch thread not finished, waiting 50ms!\n"));
		return;
	}
...
//	int i=0 ;
	...
	MSC_PRINT(MSC_DEBUG_OFF,("[MSC] msc_cmd_handler %s \n", cmd));
	if(osal_strncmp(p, "start", 5) == 0) {
		if(len>6) { //+1 for "enter"
			...
			para.chls = (char *)&chl;
			MSC_PRINT(MSC_DEBUG_OFF,("[MSC] msc_cmd_handler %s %d\n", p, chl));
			para.chl_num = 1;
		} else {
			para.chls = NULL;
			para.chl_num = 0;
...
#if 0		
		MSC_PRINT(MSC_DEBUG_OFF,("[MSC] AM=%d, ssid=%s, pwd=%s, user=%s, cust_data_len=%d, cust_data=%s,\n",
	...
	}else if (osal_strcmp(p, "monitor_on") == 0)
	{
		if(msc_ctx.state == MSC_STATE_STARTED)
			return 0;
		
		msc_ctx.m_info.chl_info.chan_id = 1;
		msc_ctx.channel = msc_listen_chls;
		msc_ctx.chl_num = sizeof(msc_listen_chls);
		msc_ctx.m_info.filter = 0x00000000;
		msc_ctx.m_info.priv = NULL;
		...
		sccb_enter_monitor_mode(&msc_ctx.m_info, priv);
		msc_ctx.state = MSC_STATE_STARTED;
	}else if (osal_strcmp(p, "monitor_off") == 0)
	{
		if (msc_ctx.state == MSC_STATE_STOPED)
			return 0;
		sccb_leave_monitor_mode(priv);
