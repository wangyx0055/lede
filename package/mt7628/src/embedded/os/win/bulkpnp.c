...
--*/
{
    ...
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("BulkUsb_IoIncrement::%d\n", result));
...
--*/
{
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("CanDeviceSuspend\n"));
...
--*/
{
    ...
    if(urb) {
        ...
        if(deviceDescriptor) {
            ...
            //MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Failed to allocate memory for deviceDescriptor\n"));
        ...
        //MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Failed to allocate memory for urb\n"));
...
--*/
{
    ...
    if(!irp) {
        ...
        //MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("IoBuildDeviceIoControlRequest failed\n"));
   ...
   // BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("CallUSBD::"));
...
--*/
{
    ...
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("BulkUsb_IoDecrement::%d\n", result));
...
--*/
{
   ...
   // RTMP_ADAPTER *deviceExtension;
    ...
    if(urb) {
        ...
        if(configurationDescriptor) {
            ...
            if(!NT_SUCCESS(ntStatus)) {
                ...
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("UsbBuildGetDescriptorRequest failed\n"));
                goto ConfigureDevice_Exit;
            }
        }
        else {

            MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Failed to allocate mem for config Descriptor\n"));
        ...
        if(configurationDescriptor) {
            ...
            if(!NT_SUCCESS(ntStatus)) {
                ...
                BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 1,("Failed to read configuration descriptor\n"));
                goto ConfigureDevice_Exit;
            }
        }
        else {

            MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Failed to alloc mem for config Descriptor\n"));
            ntStatus = STATUS_INSUFFICIENT_RESOURCES;
            goto ConfigureDevice_Exit;
        }
    }
    ...
    if(configurationDescriptor) {
        ...
        //
        pAd->UsbConfigurationDescriptor = configurationDescriptor;
        ...
        ntStatus = SelectInterfaces(pAd, configurationDescriptor);
    }
...
//    PUCHAR                      pInf;
   ...
   // RTMP_ADAPTER *deviceExtension;
    ...
    if(!tmp) {
        ...
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Failed to allocate mem for interfaceList\n"));
        return STATUS_INSUFFICIENT_RESOURCES;
    }
    ...
    if(urb) {
        ...
        if(NT_SUCCESS(ntStatus)) {
            ...
            //
            pAd->UsbInterface = (PUSBD_INTERFACE_INFORMATION)ExAllocatePool(NonPagedPool,
                                                           Interface->Length);

            if(pAd->UsbInterface) {
                
                RtlCopyMemory(pAd->UsbInterface,
                              Interface,
                              Interface->Length);
            }
            else {
                ...
                ntStatus = STATUS_INSUFFICIENT_RESOURCES;
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("memory alloc for UsbInterface failed\n"));
            }
            ...
            BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("---------\n"));
            BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("NumberOfPipes 0x%x\n", 
                                 Interface->NumberOfPipes));
            BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("Length 0x%x\n", 
                                 Interface->Length));
            BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("Alt Setting 0x%x\n", 
                                 Interface->AlternateSetting));
            BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("Interface Number 0x%x\n", 
                                 Interface->InterfaceNumber));
            BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("Class, subclass, protocol 0x%x 0x%x 0x%x\n",
                                 Interface->Class,
                                 Interface->SubClass,
                                 Interface->Protocol));
            //
            ...
            if(Interface->NumberOfPipes) {
                ...
                if(pAd->PipeContext) {
                    ...
                    for(i=0; i<Interface->NumberOfPipes; i++) {
                        ...
                        pAd->PipeContext[i].PipeOpen = FALSE;

			//-->werner++
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("i = %d\n", i));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("InterfaceInfo->Pipes[%d].PipeType = %d\n",i, Interface->Pipes[i].PipeType));
			if ((Interface->Pipes[i].PipeType == UsbdPipeTypeBulk) && (USB_ENDPOINT_DIRECTION_IN(Interface->Pipes[i].EndpointAddress)))
			{
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("BULK IN EndpointAddress = 0x%02x\n", Interface->Pipes[i].EndpointAddress));
									
					Interface->Pipes[i].MaximumTransferSize = USB_BULK_MAX_TRANSFER_SIZE;				

					pAd->UsbBulkInPipeInfo = (PUSBD_PIPE_INFORMATION)&Interface->Pipes[i];
					pAd->UsbBulkInPipeHandle[RxPipeCnt] = Interface->Pipes[i].PipeHandle;
					pAd->UsbBulkInMaxPacketSize = Interface->Pipes[i].MaximumPacketSize;

					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("BULK IN MaximumPacketSize = %d\n", pAd->UsbBulkInMaxPacketSize));

					RxPipeCnt++;
				}
				else
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s Warnning: Too much bulk pipe!\n",__FUNCTION__));
					continue;
				}
			}
			else if ((Interface->Pipes[i].PipeType == UsbdPipeTypeBulk) && (USB_ENDPOINT_DIRECTION_OUT(Interface->Pipes[i].EndpointAddress)))
			{
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("BULK OUT EndpointAddress = 0x%02x\n", Interface->Pipes[i].EndpointAddress));
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("BULK OUT EndpointAddress = 0x%02x\n", Interface->Pipes[i].EndpointAddress));
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("i = %d, txPipeCnt = %d\n", i, TxPipeCnt));

					Interface->Pipes[i].MaximumTransferSize = USB_BULK_MAX_TRANSFER_SIZE;


					pAd->UsbBulkOutPipeInfo = (PUSBD_PIPE_INFORMATION)&Interface->Pipes[i];
					pAd->UsbBulkOutPipeHandle[TxPipeCnt] = Interface->Pipes[i].PipeHandle;
					pAd->UsbBulkOutMaxPacketSize = Interface->Pipes[i].MaximumPacketSize;

					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("BULK OUT MaximumPacketSize = %d\n", pAd->UsbBulkOutMaxPacketSize));

					TxPipeCnt++;
				}
				else
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Warnning: Too much bulk pipe!\n"));
					continue;
				}
			}
			//<--werner++						
            ...
            for(i=0; i<Interface->NumberOfPipes; i++) {
                ...
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PipeType 0x%x\n", 
                                     Interface->Pipes[i].PipeType));
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("EndpointAddress 0x%x\n", 
                                     Interface->Pipes[i].EndpointAddress));
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MaxPacketSize 0x%x\n", 
                                    Interface->Pipes[i].MaximumPacketSize));
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Interval 0x%x\n", 
                                     Interface->Pipes[i].Interval));
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Handle 0x%p\n", 
                                     Interface->Pipes[i].PipeHandle));
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MaximumTransferSize 0x%x\n", 
                                    Interface->Pipes[i].MaximumTransferSize));
            }
            ...
            MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Failed to select an interface\n"));
        }
    }
    else {
        
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("USBD_CreateConfigurationRequestEx failed\n"));
        ntStatus = STATUS_INSUFFICIENT_RESOURCES;
    }
...
--*/
{
    ...
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("HandleQueryStopDevice - begins\n"));
    ...
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("HandleQueryStopDevice::"));
    ...
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("HandleQueryStopDevice - ends\n"));
...
--*/
{
    ...
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("HandleCancelStopDevice - begins\n"));
    ...
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("HandleCancelStopDevice - ends\n"));
...
--*/
{
    ...
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("ProcessQueuedRequests - begins\n"));
    ...
    while(1) {
        ...
        if(nextIrp->Cancel) {
            ...
            if(FailRequests == DeviceExtension->QueueState) {
                ...
                //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("ProcessQueuedRequests::"));
    ...
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("ProcessQueuedRequests - ends\n"));
...
--*/
{
    ...
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("HandleQueryRemoveDevice - begins\n"));
    ...
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("HandleQueryRemoveDevice::"));
    BulkUsb_IoDecrement(deviceExtension);
    ...
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("HandleQueryRemoveDevice - ends\n"));
...
--*/
{
    ...
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("HandleCancelRemoveDevice - begins\n"));
    ...
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("HandleCancelRemoveDevice - ends\n"));
...
--*/
{
    ...
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("HandleSurpriseRemoval - begins\n"));
    ...
    if(!NT_SUCCESS(ntStatus)) {
        ...
        //MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("IoSetDeviceInterfaceState::disable:failed\n"));
    }
   ...
   // BulkUsb_WmiDeRegistration(deviceExtension);
    ...
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("HandleSurpriseRemoval - ends\n"));
...
//    PUSBD_PIPE_INFORMATION      pipeInformation;
    ...
    //
    //deviceExtension = (RTMP_ADAPTER *) DeviceObject->DeviceExtension;
    pipeContext = pAd->PipeContext;
    interfaceInfo = pAd->UsbInterface;
    
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("BulkUsb_AbortPipes - begins\n"));
    ...
    for(i=0; i<interfaceInfo->NumberOfPipes; i++) {
        ...
        if(pipeContext[i].PipeOpen) {
            ...
            //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("Aborting open pipe %d\n", i));
            ...
            if(urb) {
                ...
                //MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Failed to alloc memory for urb\n"));
    ...
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("BulkUsb_AbortPipes - ends\n"));
...
//    PIRP                   irp;
    ...
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("IdleRequestWorkerRoutine - begins\n"));
    ...
    if(CanDeviceSuspend(deviceExtension)) {
        ...
        //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("Device is idle\n"));

        ntStatus = SubmitIdleRequestIrp(deviceExtension);

        if(!NT_SUCCESS(ntStatus)) {

            //MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("SubmitIdleRequestIrp failed\n"));
        }
    }
    else {

        //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("Device is not idle\n"));
    }
    ...
    //BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("IdleRequestsWorkerRoutine - ends\n"));
}
...
--*/
{
    ...
    BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("HandleQueryCapabilities - begins\n"));
    ...
    if(pdc->Version < 1 || pdc->Size < sizeof(DEVICE_CAPABILITIES)) {
        ...
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("HandleQueryCapabilities::request failed\n"));
    ...
    if(NT_SUCCESS(ntStatus)) {
		...
		BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("PowerDownLevel = %d\n", deviceExtension->PowerDownLevel));
    ...
    BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("HandleQueryCapabilities - ends, PowerDownLevel = %d\n", deviceExtension->PowerDownLevel));
...
//    NTSTATUS           ntStatus;
    ...
    BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 3, ("BulkUsb_DispatchClean::"));
    BulkUsb_IoIncrement(deviceExtension);
