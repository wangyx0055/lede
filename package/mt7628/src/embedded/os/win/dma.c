...
VOID InitDMA(RTMP_ADAPTER *pDevExt)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("initDMA :  ===>	ac 0 base= %x\n", value));
...
//	RTMP_Real_IO_WRITE32(pDevExt, RA_HCCA_DSCP_BASE, value);
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("initDMA :  <===\n"));
	//value=0x500004D5;
...
//	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: ProbeDmaData  ===>\n");
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	{		
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Descriptor require more than one page memory\n"));
			Status = STATUS_UNSUCCESSFUL;
			break;
		}
		
		pAd->RingMain.AllocSize = DescriptorRequiredTotBytes;

		pAd->RingMain.AllocVa = 
			pAd->pDmaAdapter->DmaOperations->AllocateCommonBuffer( 
				pAd->pDmaAdapter, 
				pAd->RingMain.AllocSize, 
				&pAd->RingMain.AllocPa, 
				TRUE
			);
		
		if (!pAd->RingMain.AllocVa)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Failed to allocate a big buffer\n"));
			Status = STATUS_UNSUCCESSFUL;
			break;
		}
		
		// Zero init this memory block
		RtlZeroMemory(pAd->RingMain.AllocVa, pAd->RingMain.AllocSize);
		
		// Save PA & VA for further operation
		RingBasePaHigh = pAd->RingMain.AllocPa.HighPart;
		RingBasePaLow  = pAd->RingMain.AllocPa.LowPart;
		RingBaseVa	= pAd->RingMain.AllocVa;
		...
		for (index = 0; index < AC0_RING_SIZE+AC0_TEMP_SIZE; index++)
		{
			...
			pAd->Ac0RingMain[index].AllocSize = RING_TX_DESCRIPTOR_SIZE;
			pAd->Ac0RingMain[index].AllocVa = RingBaseVa;
			pAd->Ac0RingMain[index].AllocPa.HighPart = RingBasePaHigh;
			pAd->Ac0RingMain[index].AllocPa.LowPart = RingBasePaLow;
			...
			pAd->Ac0RingMain[index].DmaBuf.AllocSize = AC0_BUFFER_SIZE;
			

			pAd->Ac0RingMain[index].DmaBuf.AllocVa = 
				pAd->pDmaAdapter->DmaOperations->AllocateCommonBuffer( 
					pAd->pDmaAdapter, 
					pAd->Ac0RingMain[index].DmaBuf.AllocSize, 
					&pAd->Ac0RingMain[index].DmaBuf.AllocPa, 
					TRUE
				);

			if (!pAd->Ac0RingMain[index].DmaBuf.AllocVa)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Failed to allocate a big buffer\n"));
				Status = STATUS_UNSUCCESSFUL;
				break;
			...
			TxDescriptor = (PTXD_STRUC)(pAd->Ac0RingMain[index].AllocVa);				
			...
			TxDescriptor->SDP0= pAd->Ac0RingMain[index].DmaBuf.AllocPa.LowPart;
		
			//--------------------------------------------------------------------

			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac0RingMain VA = 0x%x, PA = 0x%x, ",
				pAd->Ac0RingMain[index].AllocVa, pAd->Ac0RingMain[index].AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac0RingMain[index].AllocSize));
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac0 Buf VA = 0x%x, PA = 0x%x, ",
				pAd->Ac0RingMain[index].DmaBuf.AllocVa, pAd->Ac0RingMain[index].DmaBuf.AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac0RingMain[index].DmaBuf.AllocSize));
		}
		...
		{
			// Init Tx Ring Size, Va, Pa variables
			pAd->Ac1RingMain[index].AllocSize = RING_TX_DESCRIPTOR_SIZE;
			pAd->Ac1RingMain[index].AllocVa = RingBaseVa;
			pAd->Ac1RingMain[index].AllocPa.HighPart = RingBasePaHigh;
			pAd->Ac1RingMain[index].AllocPa.LowPart = RingBasePaLow;
			...
			// Setup Tx Buffer size & allocate share memory
			pAd->Ac1RingMain[index].DmaBuf.AllocSize = AC1_BUFFER_SIZE;
			

			pAd->Ac1RingMain[index].DmaBuf.AllocVa = 
				pAd->pDmaAdapter->DmaOperations->AllocateCommonBuffer( 
					pAd->pDmaAdapter, 
					pAd->Ac1RingMain[index].DmaBuf.AllocSize, 
					&pAd->Ac1RingMain[index].DmaBuf.AllocPa, 
					TRUE
				);

			if (!pAd->Ac1RingMain[index].DmaBuf.AllocVa)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Failed to allocate a big buffer\n"));
				Status = STATUS_UNSUCCESSFUL;
				break;
			...
			// Fill buffer address in each TX descriptor
			TxDescriptor = (PTXD_STRUC)(pAd->Ac1RingMain[index].AllocVa);	
			...
			TxDescriptor->SDP0= pAd->Ac1RingMain[index].DmaBuf.AllocPa.LowPart;
		
			//--------------------------------------------------------------------


			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac1RingMain VA = 0x%x, PA = 0x%x, ",
				pAd->Ac1RingMain[index].AllocVa, pAd->Ac1RingMain[index].AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac1RingMain[index].AllocSize));
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac1 Buf VA = 0x%x, PA = 0x%x, ",
				pAd->Ac1RingMain[index].DmaBuf.AllocVa, pAd->Ac1RingMain[index].DmaBuf.AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac1RingMain[index].DmaBuf.AllocSize));
		}
		...
		for (index = 0; index < AC2_RING_SIZE+AC2_TEMP_SIZE; index++)
		{
			...
			pAd->Ac2RingMain[index].AllocSize = RING_TX_DESCRIPTOR_SIZE;
			pAd->Ac2RingMain[index].AllocVa = RingBaseVa;
			pAd->Ac2RingMain[index].AllocPa.HighPart = RingBasePaHigh;
			pAd->Ac2RingMain[index].AllocPa.LowPart = RingBasePaLow;
			...
			pAd->Ac2RingMain[index].DmaBuf.AllocSize = AC2_BUFFER_SIZE;
			
			pAd->Ac2RingMain[index].DmaBuf.AllocVa = 
				pAd->pDmaAdapter->DmaOperations->AllocateCommonBuffer( 
					pAd->pDmaAdapter, 
					pAd->Ac2RingMain[index].DmaBuf.AllocSize, 
					&pAd->Ac2RingMain[index].DmaBuf.AllocPa, 
					TRUE
				);

			if (!pAd->Ac2RingMain[index].DmaBuf.AllocVa)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Failed to allocate a big buffer\n"));
				Status = STATUS_UNSUCCESSFUL;
				break;
			...
			// Fill buffer address in each TX descriptor
			TxDescriptor = (PTXD_STRUC)(pAd->Ac2RingMain[index].AllocVa);			
			...
			TxDescriptor->SDP0= pAd->Ac2RingMain[index].DmaBuf.AllocPa.LowPart;
		
			//--------------------------------------------------------------------
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac2RingMain VA = 0x%x, PA = 0x%x, ",
				pAd->Ac2RingMain[index].AllocVa, pAd->Ac2RingMain[index].AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac2RingMain[index].AllocSize));
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac2 Buf VA = 0x%x, PA = 0x%x, ",
				pAd->Ac2RingMain[index].DmaBuf.AllocVa, pAd->Ac2RingMain[index].DmaBuf.AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac2RingMain[index].DmaBuf.AllocSize));
		}
		...
		{
			// Init Priority Ring Size, Va, Pa variables
			pAd->Ac3RingMain[index].AllocSize = RING_TX_DESCRIPTOR_SIZE;
			pAd->Ac3RingMain[index].AllocVa = RingBaseVa;
			pAd->Ac3RingMain[index].AllocPa.HighPart = RingBasePaHigh;
			pAd->Ac3RingMain[index].AllocPa.LowPart = RingBasePaLow;
			...
			// Setup Priority Buffer size & allocate share memory
			pAd->Ac3RingMain[index].DmaBuf.AllocSize = AC3_BUFFER_SIZE;
			
			pAd->Ac3RingMain[index].DmaBuf.AllocVa = 
				pAd->pDmaAdapter->DmaOperations->AllocateCommonBuffer( 
					pAd->pDmaAdapter, 
					pAd->Ac3RingMain[index].DmaBuf.AllocSize, 
					&pAd->Ac3RingMain[index].DmaBuf.AllocPa, 
					TRUE
				);

			if (!pAd->Ac3RingMain[index].DmaBuf.AllocVa)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Failed to allocate a big buffer\n"));
				Status = STATUS_UNSUCCESSFUL;
				break;
			...
			// Fill buffer address in each TX descriptor
			TxDescriptor = (PTXD_STRUC)(pAd->Ac3RingMain[index].AllocVa);		
			...
			TxDescriptor->SDP0= pAd->Ac3RingMain[index].DmaBuf.AllocPa.LowPart;
		
			//--------------------------------------------------------------------
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac3RingMain VA = 0x%x, PA = 0x%x, ",
				pAd->Ac3RingMain[index].AllocVa, pAd->Ac3RingMain[index].AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac3RingMain[index].AllocSize));
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac3 Buf VA = 0x%x, PA = 0x%x, ",
				pAd->Ac3RingMain[index].DmaBuf.AllocVa, pAd->Ac3RingMain[index].DmaBuf.AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac3RingMain[index].DmaBuf.AllocSize));
		}
		...
		{
			// Init Priority Ring Size, Va, Pa variables
			pAd->Ac4RingMain[index].AllocSize = RING_TX_DESCRIPTOR_SIZE;
			pAd->Ac4RingMain[index].AllocVa = RingBaseVa;
			pAd->Ac4RingMain[index].AllocPa.HighPart = RingBasePaHigh;
			pAd->Ac4RingMain[index].AllocPa.LowPart = RingBasePaLow;
			...
			// Setup Priority Buffer size & allocate share memory
			pAd->Ac4RingMain[index].DmaBuf.AllocSize = AC4_BUFFER_SIZE;
			
			pAd->Ac4RingMain[index].DmaBuf.AllocVa = 
				pAd->pDmaAdapter->DmaOperations->AllocateCommonBuffer( 
					pAd->pDmaAdapter, 
					pAd->Ac4RingMain[index].DmaBuf.AllocSize, 
					&pAd->Ac4RingMain[index].DmaBuf.AllocPa, 
					TRUE
				);

			if (!pAd->Ac4RingMain[index].DmaBuf.AllocVa)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Failed to allocate a big buffer\n"));
				Status = STATUS_UNSUCCESSFUL;
				break;
			...
			// Fill buffer address in each TX descriptor
			TxDescriptor = (PTXD_STRUC)(pAd->Ac4RingMain[index].AllocVa);		
			...
			TxDescriptor->SDP0= pAd->Ac4RingMain[index].DmaBuf.AllocPa.LowPart;
		
			//--------------------------------------------------------------------
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac4RingMain VA = 0x%x, PA = 0x%x, ",
				pAd->Ac4RingMain[index].AllocVa, pAd->Ac4RingMain[index].AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac4RingMain[index].AllocSize));
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac4 Buf VA = 0x%x, PA = 0x%x, ",
				pAd->Ac4RingMain[index].DmaBuf.AllocVa, pAd->Ac4RingMain[index].DmaBuf.AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac4RingMain[index].DmaBuf.AllocSize));
		}
		...
		{
			// Init Priority Ring Size, Va, Pa variables
			pAd->Ac5RingMain[index].AllocSize = RING_TX_DESCRIPTOR_SIZE;
			pAd->Ac5RingMain[index].AllocVa = RingBaseVa;
			pAd->Ac5RingMain[index].AllocPa.HighPart = RingBasePaHigh;
			pAd->Ac5RingMain[index].AllocPa.LowPart = RingBasePaLow;
			...
			// Setup Priority Buffer size & allocate share memory
			pAd->Ac5RingMain[index].DmaBuf.AllocSize = AC5_BUFFER_SIZE;
			
			pAd->Ac5RingMain[index].DmaBuf.AllocVa = 
				pAd->pDmaAdapter->DmaOperations->AllocateCommonBuffer( 
					pAd->pDmaAdapter, 
					pAd->Ac5RingMain[index].DmaBuf.AllocSize, 
					&pAd->Ac5RingMain[index].DmaBuf.AllocPa, 
					TRUE
				);

			if (!pAd->Ac5RingMain[index].DmaBuf.AllocVa)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Failed to allocate a big buffer\n"));
				Status = STATUS_UNSUCCESSFUL;
				break;
			...
			// Fill buffer address in each TX descriptor
			TxDescriptor = (PTXD_STRUC)(pAd->Ac5RingMain[index].AllocVa);		
			...
			TxDescriptor->SDP0= pAd->Ac5RingMain[index].DmaBuf.AllocPa.LowPart;
		
			//--------------------------------------------------------------------
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac5RingMain VA = 0x%x, PA = 0x%x, ",
				pAd->Ac5RingMain[index].AllocVa, pAd->Ac5RingMain[index].AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac5RingMain[index].AllocSize));
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac5 Buf VA = 0x%x, PA = 0x%x, ",
				pAd->Ac5RingMain[index].DmaBuf.AllocVa, pAd->Ac5RingMain[index].DmaBuf.AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac5RingMain[index].DmaBuf.AllocSize));
		}
		...
		{
			// Init Priority Ring Size, Va, Pa variables
			pAd->Ac6RingMain[index].AllocSize = RING_TX_DESCRIPTOR_SIZE;
			pAd->Ac6RingMain[index].AllocVa = RingBaseVa;
			pAd->Ac6RingMain[index].AllocPa.HighPart = RingBasePaHigh;
			pAd->Ac6RingMain[index].AllocPa.LowPart = RingBasePaLow;
			...
			// Setup Priority Buffer size & allocate share memory
			pAd->Ac6RingMain[index].DmaBuf.AllocSize = AC6_BUFFER_SIZE;
			
			pAd->Ac6RingMain[index].DmaBuf.AllocVa = 
				pAd->pDmaAdapter->DmaOperations->AllocateCommonBuffer( 
					pAd->pDmaAdapter, 
					pAd->Ac6RingMain[index].DmaBuf.AllocSize, 
					&pAd->Ac6RingMain[index].DmaBuf.AllocPa, 
					TRUE
				);

			if (!pAd->Ac6RingMain[index].DmaBuf.AllocVa)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Failed to allocate a big buffer\n"));
				Status = STATUS_UNSUCCESSFUL;
				break;
			...
			// Fill buffer address in each TX descriptor
			TxDescriptor = (PTXD_STRUC)(pAd->Ac6RingMain[index].AllocVa);		
			...
			TxDescriptor->SDP0= pAd->Ac6RingMain[index].DmaBuf.AllocPa.LowPart;
		
			//--------------------------------------------------------------------
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac6RingMain VA = 0x%x, PA = 0x%x, ",
				pAd->Ac6RingMain[index].AllocVa, pAd->Ac6RingMain[index].AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac6RingMain[index].AllocSize));
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac6 Buf VA = 0x%x, PA = 0x%x, ",
				pAd->Ac6RingMain[index].DmaBuf.AllocVa, pAd->Ac6RingMain[index].DmaBuf.AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac6RingMain[index].DmaBuf.AllocSize));
		}
		...
		{
			// Init Priority Ring Size, Va, Pa variables
			pAd->Ac7RingMain[index].AllocSize = RING_TX_DESCRIPTOR_SIZE;
			pAd->Ac7RingMain[index].AllocVa = RingBaseVa;
			pAd->Ac7RingMain[index].AllocPa.HighPart = RingBasePaHigh;
			pAd->Ac7RingMain[index].AllocPa.LowPart = RingBasePaLow;
			...
			// Setup Priority Buffer size & allocate share memory
			pAd->Ac7RingMain[index].DmaBuf.AllocSize = AC7_BUFFER_SIZE;
			
			pAd->Ac7RingMain[index].DmaBuf.AllocVa = 
				pAd->pDmaAdapter->DmaOperations->AllocateCommonBuffer( 
					pAd->pDmaAdapter, 
					pAd->Ac7RingMain[index].DmaBuf.AllocSize, 
					&pAd->Ac7RingMain[index].DmaBuf.AllocPa, 
					TRUE
				);

			if (!pAd->Ac7RingMain[index].DmaBuf.AllocVa)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Failed to allocate a big buffer\n"));
				Status = STATUS_UNSUCCESSFUL;
				break;
			...
			// Fill buffer address in each TX descriptor
			TxDescriptor = (PTXD_STRUC)(pAd->Ac7RingMain[index].AllocVa);		
			...
			TxDescriptor->SDP0= pAd->Ac7RingMain[index].DmaBuf.AllocPa.LowPart;
		
			//--------------------------------------------------------------------
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac7RingMain VA = 0x%x, PA = 0x%x, ",
				pAd->Ac7RingMain[index].AllocVa, pAd->Ac7RingMain[index].AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac7RingMain[index].AllocSize));
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac7 Buf VA = 0x%x, PA = 0x%x, ",
				pAd->Ac7RingMain[index].DmaBuf.AllocVa, pAd->Ac7RingMain[index].DmaBuf.AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac7RingMain[index].DmaBuf.AllocSize));
		}
		...
		{
			// Init Priority Ring Size, Va, Pa variables
			pAd->Ac8RingMain[index].AllocSize = RING_TX_DESCRIPTOR_SIZE;
			pAd->Ac8RingMain[index].AllocVa = RingBaseVa;
			pAd->Ac8RingMain[index].AllocPa.HighPart = RingBasePaHigh;
			pAd->Ac8RingMain[index].AllocPa.LowPart = RingBasePaLow;
			...
			// Setup Priority Buffer size & allocate share memory
			pAd->Ac8RingMain[index].DmaBuf.AllocSize = AC8_BUFFER_SIZE;
			
			pAd->Ac8RingMain[index].DmaBuf.AllocVa = 
				pAd->pDmaAdapter->DmaOperations->AllocateCommonBuffer( 
					pAd->pDmaAdapter, 
					pAd->Ac8RingMain[index].DmaBuf.AllocSize, 
					&pAd->Ac8RingMain[index].DmaBuf.AllocPa, 
					TRUE
				);

			if (!pAd->Ac8RingMain[index].DmaBuf.AllocVa)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Failed to allocate a big buffer\n"));
				Status = STATUS_UNSUCCESSFUL;
				break;
			...
			// Fill buffer address in each TX descriptor
			TxDescriptor = (PTXD_STRUC)(pAd->Ac8RingMain[index].AllocVa);		
			...
			TxDescriptor->SDP0= pAd->Ac8RingMain[index].DmaBuf.AllocPa.LowPart;
		
			//--------------------------------------------------------------------
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac8RingMain VA = 0x%x, PA = 0x%x, ",
				pAd->Ac8RingMain[index].AllocVa, pAd->Ac8RingMain[index].AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac8RingMain[index].AllocSize));
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac8 Buf VA = 0x%x, PA = 0x%x, ",
				pAd->Ac8RingMain[index].DmaBuf.AllocVa, pAd->Ac8RingMain[index].DmaBuf.AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac8RingMain[index].DmaBuf.AllocSize));
		}
		...
		{
			// Init Priority Ring Size, Va, Pa variables
			pAd->Ac9RingMain[index].AllocSize = RING_TX_DESCRIPTOR_SIZE;
			pAd->Ac9RingMain[index].AllocVa = RingBaseVa;
			pAd->Ac9RingMain[index].AllocPa.HighPart = RingBasePaHigh;
			pAd->Ac9RingMain[index].AllocPa.LowPart = RingBasePaLow;
			...
			// Setup Priority Buffer size & allocate share memory
			pAd->Ac9RingMain[index].DmaBuf.AllocSize = AC9_BUFFER_SIZE;
			
			pAd->Ac9RingMain[index].DmaBuf.AllocVa = 
				pAd->pDmaAdapter->DmaOperations->AllocateCommonBuffer( 
					pAd->pDmaAdapter, 
					pAd->Ac9RingMain[index].DmaBuf.AllocSize, 
					&pAd->Ac9RingMain[index].DmaBuf.AllocPa, 
					TRUE
				);

			if (!pAd->Ac9RingMain[index].DmaBuf.AllocVa)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Failed to allocate a big buffer\n"));
				Status = STATUS_UNSUCCESSFUL;
				break;
			...
			// Fill buffer address in each TX descriptor
			TxDescriptor = (PTXD_STRUC)(pAd->Ac9RingMain[index].AllocVa);		
			...
			TxDescriptor->SDP0= pAd->Ac9RingMain[index].DmaBuf.AllocPa.LowPart;
		
			//--------------------------------------------------------------------
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac9RingMain VA = 0x%x, PA = 0x%x, ",
				pAd->Ac9RingMain[index].AllocVa, pAd->Ac9RingMain[index].AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac9RingMain[index].AllocSize));
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac9 Buf VA = 0x%x, PA = 0x%x, ",
				pAd->Ac9RingMain[index].DmaBuf.AllocVa, pAd->Ac9RingMain[index].DmaBuf.AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac9RingMain[index].DmaBuf.AllocSize));
		}
		...
		{
			// Init Priority Ring Size, Va, Pa variables
			pAd->Ac10RingMain[index].AllocSize = RING_TX_DESCRIPTOR_SIZE;
			pAd->Ac10RingMain[index].AllocVa = RingBaseVa;
			pAd->Ac10RingMain[index].AllocPa.HighPart = RingBasePaHigh;
			pAd->Ac10RingMain[index].AllocPa.LowPart = RingBasePaLow;
			...
			// Setup Priority Buffer size & allocate share memory
			pAd->Ac10RingMain[index].DmaBuf.AllocSize = AC10_BUFFER_SIZE;
			
			pAd->Ac10RingMain[index].DmaBuf.AllocVa = 
				pAd->pDmaAdapter->DmaOperations->AllocateCommonBuffer( 
					pAd->pDmaAdapter, 
					pAd->Ac10RingMain[index].DmaBuf.AllocSize, 
					&pAd->Ac10RingMain[index].DmaBuf.AllocPa, 
					TRUE
				);

			if (!pAd->Ac10RingMain[index].DmaBuf.AllocVa)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Failed to allocate a big buffer\n"));
				Status = STATUS_UNSUCCESSFUL;
				break;
			...
			// Fill buffer address in each TX descriptor
			TxDescriptor = (PTXD_STRUC)(pAd->Ac10RingMain[index].AllocVa);		
			...
			TxDescriptor->SDP0= pAd->Ac10RingMain[index].DmaBuf.AllocPa.LowPart;
		
			//--------------------------------------------------------------------
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac10RingMain VA = 0x%x, PA = 0x%x, ",
				pAd->Ac10RingMain[index].AllocVa, pAd->Ac10RingMain[index].AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac10RingMain[index].AllocSize));
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac10 Buf VA = 0x%x, PA = 0x%x, ",
				pAd->Ac10RingMain[index].DmaBuf.AllocVa, pAd->Ac10RingMain[index].DmaBuf.AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac10RingMain[index].DmaBuf.AllocSize));
		}
		...
		{
			// Init Priority Ring Size, Va, Pa variables
			pAd->Ac11RingMain[index].AllocSize = RING_TX_DESCRIPTOR_SIZE;
			pAd->Ac11RingMain[index].AllocVa = RingBaseVa;
			pAd->Ac11RingMain[index].AllocPa.HighPart = RingBasePaHigh;
			pAd->Ac11RingMain[index].AllocPa.LowPart = RingBasePaLow;
			...
			// Setup Priority Buffer size & allocate share memory
			pAd->Ac11RingMain[index].DmaBuf.AllocSize = AC11_BUFFER_SIZE;
			
			pAd->Ac11RingMain[index].DmaBuf.AllocVa = 
				pAd->pDmaAdapter->DmaOperations->AllocateCommonBuffer( 
					pAd->pDmaAdapter, 
					pAd->Ac11RingMain[index].DmaBuf.AllocSize, 
					&pAd->Ac11RingMain[index].DmaBuf.AllocPa, 
					TRUE
				);

			if (!pAd->Ac11RingMain[index].DmaBuf.AllocVa)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Failed to allocate a big buffer\n"));
				Status = STATUS_UNSUCCESSFUL;
				break;
			...
			// Fill buffer address in each TX descriptor
			TxDescriptor = (PTXD_STRUC)(pAd->Ac11RingMain[index].AllocVa);		
			...
			TxDescriptor->SDP0= pAd->Ac11RingMain[index].DmaBuf.AllocPa.LowPart;
		
			//--------------------------------------------------------------------
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac11RingMain VA = 0x%x, PA = 0x%x, ",
				pAd->Ac11RingMain[index].AllocVa, pAd->Ac11RingMain[index].AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac11RingMain[index].AllocSize));
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Ac11 Buf VA = 0x%x, PA = 0x%x, ",
				pAd->Ac11RingMain[index].DmaBuf.AllocVa, pAd->Ac11RingMain[index].DmaBuf.AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Ac11RingMain[index].DmaBuf.AllocSize));
		}
		...
		{
			// Init Priority Ring Size, Va, Pa variables
			pAd->MgmtRingMain[index].AllocSize = RING_TX_DESCRIPTOR_SIZE;
			pAd->MgmtRingMain[index].AllocVa = RingBaseVa;
			pAd->MgmtRingMain[index].AllocPa.HighPart = RingBasePaHigh;
			pAd->MgmtRingMain[index].AllocPa.LowPart = RingBasePaLow;
			...
			// Setup Priority Buffer size & allocate share memory
			pAd->MgmtRingMain[index].DmaBuf.AllocSize = MGMT_BUFFER_SIZE;
			
			pAd->MgmtRingMain[index].DmaBuf.AllocVa = 
				pAd->pDmaAdapter->DmaOperations->AllocateCommonBuffer( 
					pAd->pDmaAdapter, 
					pAd->MgmtRingMain[index].DmaBuf.AllocSize, 
					&pAd->MgmtRingMain[index].DmaBuf.AllocPa, 
					TRUE
				);

			if (!pAd->MgmtRingMain[index].DmaBuf.AllocVa)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Failed to allocate a big buffer\n"));
				Status = STATUS_UNSUCCESSFUL;
				break;
			...
			// Fill buffer address in each TX descriptor
			TxDescriptor = (PTXD_STRUC)(pAd->MgmtRingMain[index].AllocVa);				
			...
			TxDescriptor->SDP0= pAd->MgmtRingMain[index].DmaBuf.AllocPa.LowPart;
		
			//--------------------------------------------------------------------
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: MgmtRingMain VA = 0x%x, PA = 0x%x, ", pAd->MgmtRingMain[index].AllocVa, pAd->MgmtRingMain[index].AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->MgmtRingMain[index].AllocSize));
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Mgmt Buf VA = 0x%x, PA = 0x%x, ", pAd->MgmtRingMain[index].DmaBuf.AllocVa, pAd->MgmtRingMain[index].DmaBuf.AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->MgmtRingMain[index].DmaBuf.AllocSize));
		}
		...
		{
			// Init Priority Ring Size, Va, Pa variables
			pAd->HccaRingMain[index].AllocSize = RING_TX_DESCRIPTOR_SIZE;
			pAd->HccaRingMain[index].AllocVa = RingBaseVa;
			pAd->HccaRingMain[index].AllocPa.HighPart = RingBasePaHigh;
			pAd->HccaRingMain[index].AllocPa.LowPart = RingBasePaLow;
			...
			// Setup Priority Buffer size & allocate share memory
			pAd->HccaRingMain[index].DmaBuf.AllocSize = HCCA_BUFFER_SIZE;
			
			pAd->HccaRingMain[index].DmaBuf.AllocVa = 
				pAd->pDmaAdapter->DmaOperations->AllocateCommonBuffer( 
					pAd->pDmaAdapter, 
					pAd->HccaRingMain[index].DmaBuf.AllocSize, 
					&pAd->HccaRingMain[index].DmaBuf.AllocPa, 
					TRUE
				);

			if (!pAd->HccaRingMain[index].DmaBuf.AllocVa)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Failed to allocate a big buffer\n"));
				Status = STATUS_UNSUCCESSFUL;
				break;
			...
			// Fill buffer address in each TX descriptor
			TxDescriptor = (PTXD_STRUC)(pAd->HccaRingMain[index].AllocVa);	
			...
			TxDescriptor->SDP0= pAd->HccaRingMain[index].DmaBuf.AllocPa.LowPart;
		
			//--------------------------------------------------------------------
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: HccaRingMain VA = 0x%x, PA = 0x%x, ", pAd->HccaRingMain[index].AllocVa, pAd->HccaRingMain[index].AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->HccaRingMain[index].AllocSize));
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Hcca Buf VA = 0x%x, PA = 0x%x, ", pAd->HccaRingMain[index].DmaBuf.AllocVa, pAd->HccaRingMain[index].DmaBuf.AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->HccaRingMain[index].DmaBuf.AllocSize));
		}
		...
		for (index = 0; index < RX0_RING_SIZE; index++)
		{
			...
			pAd->Rx0RingMain[index].AllocSize = RING_RX_DESCRIPTOR_SIZE;
			pAd->Rx0RingMain[index].AllocVa = RingBaseVa;
			pAd->Rx0RingMain[index].AllocPa.HighPart = RingBasePaHigh;
			pAd->Rx0RingMain[index].AllocPa.LowPart = RingBasePaLow;
			...
			pAd->Rx0RingMain[index].DmaBuf.AllocSize = RX_BUFFER_SIZE;
			

			pAd->Rx0RingMain[index].DmaBuf.AllocVa = 
				pAd->pDmaAdapter->DmaOperations->AllocateCommonBuffer( 
					pAd->pDmaAdapter, 
					pAd->Rx0RingMain[index].DmaBuf.AllocSize, 
					&pAd->Rx0RingMain[index].DmaBuf.AllocPa, 
					TRUE
				);

			if (!pAd->Rx0RingMain[index].DmaBuf.AllocVa)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Failed to allocate a big buffer\n"));
				Status = STATUS_UNSUCCESSFUL;
				break;
			...
			RxDescriptor					= (PRXD_STRUC)(pAd->Rx0RingMain[index].AllocVa);				
			RxDescriptor->SDL0	= RX_BUFFER_SIZE;
			RxDescriptor->SDP0	= pAd->Rx0RingMain[index].DmaBuf.AllocPa.LowPart;
			#ifdef RX_SCATTER
			 ...
			 *(ptr+2) = 0xFFFFFFFF;
			 *(ptr+3) = 0xFFFFFFFF;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RX DMA = 0 x%X\n",  *(ptr));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RX DMA = 1 x%X\n", *(ptr+1));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RX DMA = 2 x%X\n",*(ptr+2));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RX DMA = 3 x%X\n", *(ptr+3));*/			
			//++werner test<-
			
			
			RxDescriptor->DDONE 	= 0x0;

			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Rx0RingMain VA = 0x%x, PA = 0x%x, ", pAd->Rx0RingMain[index].AllocVa, pAd->Rx0RingMain[index].AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Rx0RingMain[index].AllocSize));
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Rx0 Buf VA = 0x%x, PA = 0x%x, ", pAd->Rx0RingMain[index].DmaBuf.AllocVa, pAd->Rx0RingMain[index].DmaBuf.AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Rx0RingMain[index].DmaBuf.AllocSize));
		}
		...
		{
			// Init Rx Ring Size, Va, Pa variables
			pAd->Rx1RingMain[index].AllocSize = RING_RX_DESCRIPTOR_SIZE;
			pAd->Rx1RingMain[index].AllocVa = RingBaseVa;
			pAd->Rx1RingMain[index].AllocPa.HighPart = RingBasePaHigh;
			pAd->Rx1RingMain[index].AllocPa.LowPart = RingBasePaLow;
			...
			// Setup Rx Buffer size & allocate share memory
			pAd->Rx1RingMain[index].DmaBuf.AllocSize = RX_BUFFER_SIZE;
			

			pAd->Rx1RingMain[index].DmaBuf.AllocVa = 
				pAd->pDmaAdapter->DmaOperations->AllocateCommonBuffer( 
					pAd->pDmaAdapter, 
					pAd->Rx1RingMain[index].DmaBuf.AllocSize, 
					&pAd->Rx1RingMain[index].DmaBuf.AllocPa, 
					TRUE
				);

			if (!pAd->Rx1RingMain[index].DmaBuf.AllocVa)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Failed to allocate a big buffer\n"));
				Status = STATUS_UNSUCCESSFUL;
				break;
			...
			// Fill buffer address in each RX descriptor
			RxDescriptor					= (PRXD_STRUC)(pAd->Rx1RingMain[index].AllocVa);				
			RxDescriptor->SDL0	= RX_BUFFER_SIZE;
			RxDescriptor->SDP0	= pAd->Rx1RingMain[index].DmaBuf.AllocPa.LowPart;
			#ifdef RX_SCATTER
			RxDescriptor->SDP1	= RxDescriptor->SDP0 + 40; // header length equ
			#endif
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Rx1RingMain VA = 0x%x, PA = 0x%x, ", pAd->Rx1RingMain[index].AllocVa, pAd->Rx1RingMain[index].AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Rx1RingMain[index].AllocSize));
			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: Rx1 Buf VA = 0x%x, PA = 0x%x, ", pAd->Rx1RingMain[index].DmaBuf.AllocVa, pAd->Rx1RingMain[index].DmaBuf.AllocPa));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" size = 0x%x\n", pAd->Rx1RingMain[index].DmaBuf.AllocSize));
		}
	...
	IN PDEVICE_OBJECT DeviceObject)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: FreeDMAMemory <=== \n"));
	return Status;
}
    ...
    )
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" idx = %d ,length =%d\n",idx,length));	
	
	
	if(data == RING_TYPE_AC0)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" TransferDMADataBuffer length = %x \n",length));		
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("AC0Ring"));
		...
		if(IsLoadFW)
		{
			if(length>=sizeof(FwTxD))
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s IsLoadFW = %d , length is not enough length=%d,  FwTxD lenght=%d\n",__FUNCTION__,IsLoadFW,length,sizeof(FwTxD)));		
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("AC1Ring"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("AC2Ring"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("AC3Ring"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("AC4Ring"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("AC5Ring"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("AC6Ring"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("AC7Ring"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("AC8Ring"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("AC9Ring"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("AC10Ring"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("AC11Ring"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("MgmtRing"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("HccaRing"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Rx0Ring"));
	...
	)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("TransferDMADescriptor load descriptor:  ===>	idx = %x ,length= %x\n", idx,length));	
	if(RING_TYPE_AC0 == data)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("load descriptor: RING_TYPE_AC0 ===>	idx=%d SDP0 %x\n",idx, TxDescriptorIn->SDP0));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("load descriptor:  ===>	data %x = 0x%x SDL0\n",data, TxDescriptorIn->SDL0));
		//--------------------------------------------------------------------
	}
	...
	{		
		...
		//--------------------------------------------------------------------
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("load descriptor:  RING_TYPE_AC1===>	data %x = 0x%x\n",data, TxDescriptorIn->SDL0));
	...
	{		
		...
		//TxDescriptor->Is802dot11 = TxDescriptorIn->Is802dot11;
		//--------------------------------------------------------------------
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("load descriptor: RING_TYPE_AC2 ===>	data %x = 0x%x\n",data, TxDescriptorIn->SDL0));
	...
	{				
		...
		//TxDescriptor->Is802dot11 = TxDescriptorIn->Is802dot11;
		//--------------------------------------------------------------------
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("load descriptor:  RING_TYPE_AC3===>	data %x = 0x%x\n",data, TxDescriptorIn->SDL0));
	...
	{				
		...
		//TxDescriptor->Is802dot11 = TxDescriptorIn->Is802dot11;
		//--------------------------------------------------------------------
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("load descriptor:  RING_TYPE_AC4===>	data %x = 0x%x\n",data, TxDescriptorIn->SDL0));
	...
	{				
		...
		//TxDescriptor->Is802dot11 = TxDescriptorIn->Is802dot11;
		//--------------------------------------------------------------------
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("load descriptor:  RING_TYPE_AC5===>	data %x = 0x%x\n",data, TxDescriptorIn->SDL0));
	...
	{				
		...
		//TxDescriptor->Is802dot11 = TxDescriptorIn->Is802dot11;
		//--------------------------------------------------------------------
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("load descriptor:  RING_TYPE_AC6===>	data %x = 0x%x\n",data, TxDescriptorIn->SDL0));
	...
	{				
		...
		//TxDescriptor->Is802dot11 = TxDescriptorIn->Is802dot11;
		//--------------------------------------------------------------------
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("load descriptor:  RING_TYPE_AC7===>	data %x = 0x%x\n",data, TxDescriptorIn->SDL0));
	...
	{				
		...
		//TxDescriptor->Is802dot11 = TxDescriptorIn->Is802dot11;
		//--------------------------------------------------------------------
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("load descriptor:  RING_TYPE_AC8===>	data %x = 0x%x\n",data, TxDescriptorIn->SDL0));
	...
	{				
		...
		//TxDescriptor->Is802dot11 = TxDescriptorIn->Is802dot11;
		//--------------------------------------------------------------------
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("load descriptor:  RING_TYPE_AC9===>	data %x = 0x%x\n",data, TxDescriptorIn->SDL0));
	...
	{				
		...
		//TxDescriptor->Is802dot11 = TxDescriptorIn->Is802dot11;
		//--------------------------------------------------------------------
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("load descriptor:  RING_TYPE_AC10===>	data %x = 0x%x\n",data, TxDescriptorIn->SDL0));
	...
	{				
		...
		//TxDescriptor->Is802dot11 = TxDescriptorIn->Is802dot11;
		//--------------------------------------------------------------------
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("load descriptor:  RING_TYPE_AC11===>	data %x = 0x%x\n",data, TxDescriptorIn->SDL0));
	...
	{	
		...
		//TxDescriptor->Is802dot11 = TxDescriptorIn->Is802dot11;
		//--------------------------------------------------------------------
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("load descriptor:  RING_TYPE_MGMT===>	data %x = 0x%x\n",data, TxDescriptorIn->SDL0));
	...
	{		
		...
		//TxDescriptor->Is802dot11 = TxDescriptorIn->Is802dot11;
		//--------------------------------------------------------------------
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("load descriptor:  RING_TYPE_HCCA===>	data %x = 0x%x\n",data, TxDescriptorIn->SDL0));
	...
	else if(RING_TYPE_RX0 == data)
	{
		...
		//TxDescriptor->Is802dot11 = TxDescriptorIn->Is802dot11;
		//--------------------------------------------------------------------	
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("load descriptor:  RING_TYPE_RX0===>	data %x = 0x%x\n",data, TxDescriptorIn->SDL0));
	}
	else if(RING_TYPE_RX1 == data)
	{
		...
		//TxDescriptor->Is802dot11 = TxDescriptorIn->Is802dot11;
		//--------------------------------------------------------------------	
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("load descriptor:  RING_TYPE_RX1===>	data %x = 0x%x\n",data, TxDescriptorIn->SDL0));
	}
	...
	)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: DumpDMA  ===>\n"));

	*ByteCount = 0;

	type	= *(PULONG)(pIBuffer+3);						// Ring type
	value	 = *(PULONG)(pIBuffer+4);						// Ring index
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: DumpDMA  ===>	Ring index = 0x%x\n", value));
	...
	if(type == 0)
	{
		...
		*ByteCount = AC0_BUFFER_SIZE + RING_TX_DESCRIPTOR_SIZE;
		...
		for(i=26; i<1504-1; i++)
		{
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("INC: data[%d+1]!=data[%d]+1---------------\n", i, i));			
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("INC: data[%d+1]=0x%x\n", i, ((UCHAR*)pDevExt->Ac0RingMain[value].DmaBuf.AllocVa)[i+1]));							
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("INC: data[%d]=0x%x\n", i, ((UCHAR*)pDevExt->Ac0RingMain[value].DmaBuf.AllocVa)[i]));											
			}
		}
	...
	)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: EraseDMA  ===>\n"));
...
void IncSequenceNumber(UCHAR* pDesBuffer, USHORT RingNum )
{
       ...
       //AsicMTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("SN: *pSequenceNumber = 0x%04x\n", *pSequenceNumber);		
...
VOID PayloadDataRandom(RTMP_ADAPTER *pDevExt, USHORT StartOffset, USHORT Length, UCHAR* pDesBuffer)
{
	...
	for(i=0; i<Length; i++)
	{
		*pucTemp = (UCHAR)(ucInitValue+i);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("SCT: 0x%x\n", *pucTemp));
		pucTemp++;
	}

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("SCT: Length = %d\n", Length));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("SCT: ====\n"));
}
