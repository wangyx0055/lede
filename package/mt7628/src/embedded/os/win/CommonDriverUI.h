...
typedef CHAR		INT8,  *PINT8,  **PPINT8;    /*  8-bit signed value & pointer */
typedef SHORT		INT16, *PINT16, **PPINT16;   /* 16-bit signed value & pointer */
typedef int			INT32, *PINT32, **PPINT32;   /* 32-bit signed value & pointer */
typedef LONGLONG	INT64, *PINT64, **PPINT64;   /* 64-bit signed value & pointer */

typedef UCHAR 		BOOLEAN;
/* Type definition for unsigned integers */
typedef UCHAR		UINT8,  *PUINT8,  **PPUINT8;  /*  8-bit unsigned value & pointer */
typedef USHORT		UINT16, *PUINT16, **PPUINT16; /* 16-bit unsigned value & pointer */
typedef unsigned int	UINT32, *PUINT32, **PPUINT32; /* 32-bit unsigned value & pointer */
typedef ULONGLONG	UINT64, *PUINT64, **PPUINT64; /* 64-bit unsigned value & pointer */
#if 0
/* Type definition for signed integers */
...
typedef	struct _FwRamEventTxD_STRUC 
{
	UINT32	u2RxByteCount:16;
	UINT32	u2PacketType: 16;
	UINT32	ucEID:8;
	...
	UINT32	aucReserved1:8;
	UINT32	ExtenEID:8;
	UINT32	aucReserved2:8;
	UINT32	aucReserved3:8;
	UINT32	aucReserved4:8;
}FwRamEventTxD_STRUC, *PFwRamEventTxD_STRUC;
...
typedef	struct _FW_EVENT_ACCESS_REG//_FwRequestResponseTxD_STRUC ,_FwStartRResponseTxD_STRUC
{	
	FwEventTxD_STRUC FwEventTxD;
	UINT32	u4Address;
	UINT32	u4Data;
}FW_EVENT_ACCESS_REG, *PFW_EVENT_ACCESS_REG;

#if 0
typedef	struct _FW_EVENT_NIC_CAPABILITY//_FwRequestResponseTxD_STRUC ,_FwStartRResponseTxD_STRUC
{	
	FwRamEventTxD_STRUC FwEvent;
	UINT16	ProductID;
	UINT16	FwVersion;
	UINT16	DriverVersion;
	UINT8	Hw5GBandDisabled;
	UINT8	EepromUsed;
	UINT8	MacAddr[6];
	UINT8	EndianOfMacAddrNumber;
	UINT8	Reserved;
}FW_EVENT_NIC_CAPABILITY, *PFW_EVENT_NIC_CAPABILITY;
#endif

typedef	struct _EXT_EVENT_MULTI_CR_ACCESS_T
{
	FwRamEventTxD_STRUC FwEvent;
	UINT32	reserved[5];
	UINT32	v1;
...
}EXT_EVENT_MULTI_CR_ACCESS_T, *PEXT_EVENT_MULTI_CR_ACCESS_T;
...
typedef	struct	_HQA_TX_FRAMES {
	UINT32		PacketCount;	//TxCount
	UINT16		PacketSize;		//TxLength
	UINT8		FC[2];			//Frame control
	UINT8		Duration[2];	//Druation
	UINT8		Addr1[6];	//DA
	UINT8		Addr2[6];	//SA
	UINT8		Addr3[6];	//BSSID	
	UINT8		Seq[2];		//Sequence
...
#define CMD_GET_RXV_RSSI							0x2D
//BF
#define CMD_READ_TXBF_PROFILETAG					0x2E
#define CMD_WRITE_TXBF_PROFILETAG				0x2F
#define CMD_READ_TXBF_PROFILEDATA				0x30
#define CMD_WRITE_TXBF_PROFILEDATA				0x31
#define CMD_VALID_TXBF_PROFILETAG					0x32
#define CMD_WRITEALL_TXBF_PROFILEDATA			0x33
#define CMD_READALL_TXBF_PROFILEDATA				0x34
#define CMD_TXBF_PROFILETAG_MCS					0x35
#define CMD_TXBF_PROFILETAG_MATRIX				0x36
#define CMD_TXBF_PROFILETAG_SNR					0x37
#define CMD_TXBF_PROFILETAG_TXSCALE				0x38
#define CMD_TXBF_PROFILETAG_MAC					0x39
#define CMD_TXBF_PROFILETAG_FLG					0x3A
#define CMD_TXBF_BYPASS_MAC						0x41
#define CMD_TXBF_POWER_NORMALIZE					0x42
#define CMD_TXBF_ENABLE							0x43
//~BF

//loop back
#define CMD_LOOPBACK_START						0x3B
#define CMD_LOOPBACK_STOP							0x3C
#define CMD_LOOPBACK_STATUS						0x3D
#define CMD_LOOPBACK_EXPECTRX						0x3E
#define CMD_LOOPBACK_RAWDATA						0x3F
#define CMD_LOOPBACK_EXPECTTX						0x40
//~loop back

#define CMD_GET_RXV_RSSI_AVG						0x44
#define CMD_SET_RXV_RSSI_LIMIT						0x45

#define CMD_IS_PFMU_CHANGE						0x46
#define CMD_IS_BFPROF_STORE						0x47

#define PKTCMD_EVENT_BUFFER 1024
/*
