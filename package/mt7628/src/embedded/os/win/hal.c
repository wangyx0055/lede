...
//:::TODO
	...
	//RTMP_IO_READ32USB(dx, 0x50029018,&U3DMAWLCFG);//UDMA_WLCFG_0
       /* MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("1 U3DMAWLCFG = %x\n", U3DMAWLCFG));
        U3DMAWLCFG.WL_RX_AGG_EN = 1;
        U3DMAWLCFG.WL_RX_AGG_LMT = 0x15;
        U3DMAWLCFG.WL_RX_AGG_TO = 0x80;
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("2 U3DMAWLCFG = %x\n", U3DMAWLCFG));
...
#ifdef RTMP_SDIO_SUPPORT
	RTMP_IO_WRITE32(pAd, 0x800C0040, dw[0]);
	RTMP_IO_WRITE32(pAd, 0x800C0044, dw[1]);
	RTMP_IO_WRITE32(pAd, 0x800C0048, dw[2]);
	RTMP_IO_WRITE32(pAd, 0x800C004C, dw[3]);
	RTMP_IO_WRITE32(pAd, 0x800C0050, dw[4]);
#endif
...
/**
 * 5.MtAsicDMASchedulerInit
 */
 /*
    DMA scheduer reservation page assignment
*/
int MtAsicDMASchedulerInit(RTMP_ADAPTER *pAd, int iMode)
{
	ULONG mac_val;
...
void StartContinuesTXTest(RTMP_ADAPTER *pAd,UCHAR Modulation, UCHAR BW, UCHAR pri_ch, UCHAR rate,UCHAR WFSelect)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s BW = %d error!!!\n",__FUNCTION__, BW));	
		return;
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s pri_ch = %d error!!!\n",__FUNCTION__, pri_ch));	
		return;
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s WFSelect = %d error!!!\n",__FUNCTION__, WFSelect));	
		return;
	}
	...
	if(MODULATION_SYSTEM_CCK == Modulation)
	{
		...
		switch(rate)
		{
			...
			case MCS_11:
				...
				break;
			default:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s CCK rate = %d error!!!\n",__FUNCTION__, rate));	
				return;
		}
	...
	else if(MODULATION_SYSTEM_OFDM == Modulation)
	{
		...
		switch(rate)
		{
			...
			case MCS_7:
				...
				break;
			default:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s OFDM rate = %d error!!!\n",__FUNCTION__, rate));		
				return;
		}
		RTMP_IO_WRITE32(pAd,0x1470C,CRValue);
	}
	else if(MODULATION_SYSTEM_HT20 == Modulation ||MODULATION_SYSTEM_HT40 == Modulation)
	{
		...
		{		
			...
			case MCS_32:
				...
				break;
			default:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s OFDM HT MCS = %d error!!!\n",__FUNCTION__, rate));	
				return;
		}
		RTMP_IO_WRITE32(pAd,0x1470C,CRValue);
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s Modulation = %d error!!!\n",__FUNCTION__, Modulation));
		return;
	}
...
{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s iTestType=%d-->\n",__FUNCTION__,iTestType));		
 ...
 BOOLEAN SetAIFS(RTMP_ADAPTER *pAd,ULONG iSetType,ULONG iSetValue)
{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s iSetType=%d iSetValue=0x%x<--\n",__FUNCTION__,iSetType,iSetValue));
...
void TxSetFramePower (RTMP_ADAPTER *pAd, ULONG eModulationSystem, ULONG eModulationType,UCHAR ucFramePowerDbm)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s--> eModulationSystem=%d,eModulationType=%d,ucFramePowerDbm=0x%x\n",
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("6013002C[14:8] = 0x%x\n",ucFramePowerDbm));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("temp  CRValue begin= 0x%x\n",CRValue));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("EfuseContent[0xA2]= 0x%x\n",pAd->EfuseContent[0xA2]));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("EfuseContent[0xA2]>>7= 0x%x\n",pAd->EfuseContent[0xA2]>>7));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("bit6= 0x%x\n",bit6));
	CRValue |= (0x0000007F & (ucFramePowerDbm + (pAd->EfuseContent[0xA2] >> 7) * (bit6*2 - 1)* (pAd->EfuseContent[0xA2] & 0x3F)));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("60130020 [6:0]  CRValue = 0x%x\n",CRValue));
	Lo0 = (ucFramePowerDbm + (pAd->EfuseContent[0xA2] >> 7) * (bit6*2 - 1)* (pAd->EfuseContent[0xA2] & 0x3F));



	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("60130020 [6:0] = 0x%x\n",(ucFramePowerDbm + (pAd->EfuseContent[0xA2] >> 7) * (bit6*2 - 1)* (pAd->EfuseContent[0xA2] & 0x3F))));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("60130020 [30:24] = 0x%x\n",((ucFramePowerDbm + (pAd->EfuseContent[0xA5] >> 7) * (bit6*2 - 1)* (pAd->EfuseContent[0xA5] & 0x3F)) << 24)));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("TMAC_FP0R0,CRValue 60130020 = 0x%x\n", CRValue));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("60130024 [30:24] = 0x%x\n",((ucFramePowerDbm + (pAd->EfuseContent[0xAA] >> 7) * (bit6*2 - 1)* (pAd->EfuseContent[0xAA] & 0x3F)) << 24)));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("TMAC_FP0R1,CRValue 60130024 = 0x%x\n", CRValue));	
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("6013002C [22:16] = 0x%x\n",((ucFramePowerDbm + (pAd->EfuseContent[0xA1] >> 7) * (bit6*2 - 1)* (pAd->EfuseContent[0xA1] & 0x3F)) << 16 )));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("TMAC_FP0R3,CRValue 6013002C = 0x%x\n", CRValue));
	RTMP_IO_WRITE32(pAd,TMAC_FP0R3,CRValue);
	//TMAC_FP0R4==================
	CRValue = 0;
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,( "HT40  PowDelta = 0x%x\n", PowDelta));
	//HT40 M6 A1
	//60130030 [22:16] 
	if((pAd->EfuseContent[0xA1] & 0x40) == 0x40)
	{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("60130030 [30:24] = 0x%x\n",((ucFramePowerDbm + (pAd->EfuseContent[0xAD] >> 7) * (bit6*2 - 1)* (pAd->EfuseContent[0xAD] & 0x3F)+ PowDelta) <<24)) );
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("TMAC_FP0R4,CRValue 60130030 = 0x%x\n", CRValue));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("TMAC_FP0R2, CRValue 60130028 = 0x%x\n", CRValue));
...
/*    do 
	{
        ...
        switch (eModulationSystem) 
		{
            case MODULATION_SYSTEM_CCK:    // CCK, OFDM 54.
            	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s MODULATION_SYSTEM_CCK \n",__FUNCTION__));
                u4PowerBaseReg = u4PowerBaseReg + 12;
                break;

            case MODULATION_SYSTEM_OFDM: // Other OFDM.
            	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s MODULATION_SYSTEM_OFDM \n",__FUNCTION__));
            ...
            case MODULATION_SYSTEM_HT20: // HT20
	            MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s MODULATION_SYSTEM_HT20 \n",__FUNCTION__));
                u4PowerBaseReg = u4PowerBaseReg + 4;
                break;

            case MODULATION_SYSTEM_HT40:  // HT 40
            	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s MODULATION_SYSTEM_HT40 \n",__FUNCTION__));
        ...
        switch (eModulationType) 
		{
            case MODULATION_TYPE_CCK_BPSK:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s MODULATION_TYPE_CCK_BPSK \n",__FUNCTION__));
		  ...
		  RTMP_IO_WRITE32(pAd,u4PowerBaseReg,Temp);
            ...
            case MODULATION_TYPE_DSSS_QPSK:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s MODULATION_TYPE_DSSS_QPSK \n",__FUNCTION__));
                if (eModulationSystem == MODULATION_SYSTEM_CCK) 
		  {
		       RTMP_IO_READ32(pAd, u4PowerBaseReg,&Temp);
		   ...
		   RTMP_IO_WRITE32(pAd,u4PowerBaseReg,Temp);
            ...
            case MODULATION_TYPE_16QAM:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s MODULATION_TYPE_16QAM \n",__FUNCTION__));
                if (eModulationSystem == MODULATION_SYSTEM_CCK) 
		  {
                    break;
                }
		   ...
		   RTMP_IO_WRITE32(pAd,u4PowerBaseReg,Temp);
            ...
            case MODULATION_TYPE_64QAM:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s MODULATION_TYPE_64QAM \n",__FUNCTION__));
                if (eModulationSystem == MODULATION_SYSTEM_CCK) 
		  {
                    break;
                }
                ...
                switch (eModulationSystem) 
		{
            ...
            case MODULATION_TYPE_54M_MCS7:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s MODULATION_TYPE_54M_MCS7 \n",__FUNCTION__));
                switch (eModulationSystem) 
		 {
                case MODULATION_SYSTEM_HT20:
		      RTMP_IO_READ32(pAd, u4PowerBaseReg2,&Temp);
            ...
            case MODULATION_TYPE_MCS32:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s MODULATION_TYPE_MCS32 \n",__FUNCTION__));
                switch (eModulationSystem) 
		 {
                case MODULATION_SYSTEM_HT40:
		      RTMP_IO_READ32(pAd, u4PowerBaseReg,&Temp);
...
void TxSetFrequencyOffset (RTMP_ADAPTER *pAd, ULONG iOffset)
{
	...
	if(pAd->IsTxSetFrequency == FALSE)
	{
		...
		{
			RTMP_IO_READ32(pAd,FREQ_OFFSET_MANUAL_ENABLE,&Value);
			Value = (Value & 0xFFFF80FF) | (0x7F << 8);
			RTMP_IO_WRITE32(pAd,FREQ_OFFSET_MANUAL_ENABLE,Value);
	...
	{
		RTMP_IO_READ32(pAd,FREQ_OFFSET_MANUAL_VALUE,&Value);	
		Value = (Value & 0xFFFF80FF) | (iOffset << 8);
		RTMP_IO_WRITE32(pAd,FREQ_OFFSET_MANUAL_VALUE,Value);
	}
...
void SetConfigMIBCounter(RTMP_ADAPTER *pAd, ULONG u4CntBitmap,ULONG u4PerBSSCntBitmap, BOOLEAN fgReadClear)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s u4CntBitmap=0x%x,u4PerBSSCntBitmap=0x%x,fgReadClear=%d-->\n",__FUNCTION__,u4CntBitmap,u4PerBSSCntBitmap,fgReadClear));
...
void GetStaticNumber(RTMP_ADAPTER *pAd, ULONG *pu4MdrdyCnt)
{
	...
	GetValue &= BITS(0,25);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s, -->\n",__FUNCTION__));
	//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s, g_u4MdrdyCnt=%d, GetValue=%d-->\n",__FUNCTION__,g_u4MdrdyCnt,GetValue));
	//g_u4MdrdyCnt += GetValue;
	//*pu4MdrdyCnt = g_u4MdrdyCnt;
	*pu4MdrdyCnt = GetValue;

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s, u4MdrdyCnt=%d-->\n",__FUNCTION__,GetValue));
...
void GetPHYMdrdy(RTMP_ADAPTER *pAd, ULONG *pu4MdrdyCntOFDM, ULONG *pu4MdrdyCntCCK)
{
	...
	*pu4MdrdyCntCCK = (GetValue&0x0000FFFF);
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s, MdrdyCntOFDM=%d ,MdrdyCntCCK=%d-->\n",__FUNCTION__,*pu4MdrdyCntOFDM,*pu4MdrdyCntCCK));
}
...
void GetMACFCSErrCnt(RTMP_ADAPTER *pAd,ULONG *pu4MACFCSErrCount)
{
	...
	*pu4MACFCSErrCount = FcsErrorCnt;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s, <-- return pu4MACFCSErrCount %d\n",__FUNCTION__,*pu4MACFCSErrCount));
}
...
void GetPHYMdrdyErrCnt(RTMP_ADAPTER *pAd,ULONG *pu4PhyMdrdyCountOFDM,ULONG *pu4PhyMdrdyCountCCK)
{
	...
	*pu4PhyMdrdyCountCCK  = TempCCK;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s, <-- return pu4PhyMdrdyCountOFDM %d,pu4PhyMdrdyCountCCK %d\n",__FUNCTION__,*pu4PhyMdrdyCountOFDM,*pu4PhyMdrdyCountCCK));
}
...
void GetPHYFCSErrCnt(RTMP_ADAPTER *pAd,ULONG *pu4PhyFCSErrCountOFDM,ULONG *pu4PhyFCSErrCountCCK)
{
	...
	*pu4PhyFCSErrCountCCK  = TempCCK;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s, <-- return pu4PhyFCSErrCountOFDM %d,pu4PhyFCSErrCountCCK %d\n",__FUNCTION__,*pu4PhyFCSErrCountOFDM,*pu4PhyFCSErrCountCCK));
}
...
void GetPhyPD(RTMP_ADAPTER *pAd,ULONG *pu4PhyPDOFDM,ULONG *pu4PhyPDCCK)
{
	...
	*pu4PhyPDOFDM = TempOFDM;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s, <-- return pu4PhyPDCCK %d,pu4PhyPDOFDM %d\n",__FUNCTION__,*pu4PhyPDCCK,*pu4PhyPDOFDM));
}
/**
...
void GetPhyCCK_SIG_SFD(RTMP_ADAPTER *pAd,ULONG *pu4SIG,ULONG *pu4SFD)
{
	...
	*pu4SIG = TempSig;
	*pu4SFD = TempSFD;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s, <-- return pu4SIG %d,pu4PhyPDOFDM %d\n",__FUNCTION__,*pu4SIG,*pu4SFD));
...
void GetPhyRSSI(RTMP_ADAPTER *pAd,ULONG *pWBRSSI0,ULONG *pIBRSSI0,ULONG *pWBRSSI1,ULONG *pIBRSSI1)
{
	...
	*pWBRSSI1 = (Temp&0x000000FF);
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s, <-- return pIBRSSI0 %d,pWBRSSI0 %d,pIBRSSI1 %d,pWBRSSI1 %d\n",__FUNCTION__,
...
void ResetCounter(RTMP_ADAPTER *pAd)
{
	ULONG value = 0;
	if(IS_MT76x6(pAd))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s, -->\n",__FUNCTION__));
		RTMP_IO_READ32(pAd,CR_PHYMUX_5, &value);//::TODO CMD_GET_MACFCSERRORCNT_COUNT
		value &= 0xFF0FFFFF;
		RTMP_IO_WRITE32(pAd,CR_PHYMUX_5, value);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s, -->\n",__FUNCTION__));		
		RTMP_IO_READ32(pAd,CR_PHYMUX_5, &value);//::TODO CMD_GET_MACFCSERRORCNT_COUNT
		value |= 0x00500000;
		RTMP_IO_WRITE32(pAd,CR_PHYMUX_5, value);
...
void GetRxvRSSI(RTMP_ADAPTER *pAd,ULONG *pRSSI0,ULONG *pRSSI1)
{	
	ULONG RSSI0 =0;
	ULONG RSSI1 =0;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s, -->\n",__FUNCTION__));
	RSSI0 = (pAd->OtherCounters.RCPI0) /2 -110;	
	RtlCopyMemory(pRSSI0, &RSSI0,sizeof(RSSI0));
	RSSI1 = (pAd->OtherCounters.RCPI1) /2 -110;
	RtlCopyMemory(pRSSI1, &RSSI1,sizeof(RSSI1));
	
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s, <-- return Rxv RSSI0 %d,Rxv RSSI1 %d\n",__FUNCTION__,
		*pRSSI0,*pRSSI1));
}
