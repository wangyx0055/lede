...
)
{
    ...
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): WCIR1:%x\n",__FUNCTION__,u4value));

    prAd->ChipID = GET_CHIP_ID(u4value);

    if(IS_MT76x6(prAd)){
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():chip is MT7636\n",__FUNCTION__));
	//return FALSE;
	}else{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():chip is not MT7636\n",__FUNCTION__));
	}
...
)
{
	...
	*/
	...
	if (rStatus == NDIS_STATUS_FAILURE) 
{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: unable to start MTSDIOCmdThread\n", RTMP_OS_NETDEV_GET_DEVNAME(prAd->net_dev)));
		return NDIS_STATUS_FAILURE;
	}
#endif
...
RtmpNetTaskInit(
    ...
    if (rStatus == NDIS_STATUS_FAILURE) {
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: create mtk sdio thread failed\n", __FUNCTION__));
