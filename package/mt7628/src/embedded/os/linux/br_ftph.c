	...
	IN		PNDIS_PACKET	pPacket)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("ft bg> BG_FTPH_PacketFromApHandle\n"));
	...
	if (pNetDev != NULL)
	{
		if (pNetDev->br_port != NULL)
		{
					...
					(pSrcFdbEntry->dst->dev != pNetDev)))
			{
				...
				if (packet_length(pRxPkt)-14 > pFdbEntry->dst->dev->mtu &&
#if LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,17)
					!skb_shinfo(pRxPkt)->tso_size)
#else
					!skb_is_gso(pRxPkt))
#endif

				(!(pFdbEntry->dst->dev->flags & IFF_PROMISC)))
#endif
...
LabelPassToUpperLayer:
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ft bg> Pass packet to bridge module.\n"));
	return 1;
