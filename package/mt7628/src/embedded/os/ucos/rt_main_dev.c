	...
	IN RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<--> NICInitTxRxRingAndBacklogQueue\n"));
...
/*	pAd->DeviceID = RT2860_PCI_DEVICE_ID; */
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():MacVersion[ Ver:Rev=0x%08lx]\n",
...
/*		NetJobAdd(APTxDataIntProcess, pAd, IntSource, 0); */
		...
		if (pAd->bStaFifoTest == TRUE)
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (">>>TxDataInt=0xfa<<< receive 0x%x\n", IntSource));
	...
	IN	PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ACK policy = %s\n", pAckStr[(UCHAR)Data & 0x02]));
