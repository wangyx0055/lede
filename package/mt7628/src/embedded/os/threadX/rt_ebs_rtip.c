	...
	IN	ULONG	Length)
{
	...
	if (!pNetBuf)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("can't allocate frag rx %ld size packet\n",Length));
	}
	...
	OUT	PVOID	*VirtualAddress)
{
	...
	if (pPkt == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("can't allocate tx %ld size packet\n",Length));
	...
	IN	UINT			DataLen)
{
	...
	{
		*ppPacket = NULL;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RTMPAllocateNdisPacket Fail\n\n"));
...
/*	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : pPacket = %p, len = %d\n", */
...
PNDIS_PACKET skb_clone(PNDIS_PACKET pSrcPkt, int flags)
{
	...
	if (pNewNetBuf == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): get Mblk failed!\n", __FUNCTION__));
		return NULL;
	}
...
PNDIS_PACKET DuplicatePacket(PNET_DEV pNetDev, PNDIS_PACKET pPacket)
{
	...
	/*MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): pRetPacket = 0x%x!\n", __FUNCTION__, (unsigned int)pRetPacket)); */
...
#if 0	
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("pRxBlk.pData=0x%x,Size=%d!\n", pRxBlk->pData, pRxBlk->DataSize));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("pNetPkt.data=0x%x(0x%x), Size=%d", pNetPkt->pDataPtr, pNetPkt->pFrameBuf, pNetPkt->pktLen));
...
#if 0
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("AfHdrCp: pNetPkt.Data=0x%x, len=%d\n", 
...
{				 
...

	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("0x1300 = %08x\n", reg));
	}
...
int rt28xx_packet_xmit(RTMP_RTIP_NET_BUF *pNetPkt)
{
	...
	{
		/*MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("bad packet size: %d\n", pkt->len)); */
...
int xn_bind_rt_wifi(int minor)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): DevTable Add\n", __FUNCTION__));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RT_WIFI: DevTable add failed\n"));
		return -1;
	}

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): RT_WIFI: DevTable add success!\n", __FUNCTION__));
}
	...
	OUT INT	*avilNum)
{
	...
	if(ifNameIdx < 32)
	{
		*avilNum = ifNameIdx;
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Assign the net device name as %s%d\n", pPrefixStr, *avilNum));
	...
	IN RTMP_OS_NETDEV_OP_HOOK *pDevOpHook)
{
	...
	*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Open ra0 interface\n"));
	...
	IN RTMP_STRING *pNamePrefix)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): request devNum failed!\n", __FUNCTION__));
		return NULL;
	}
