...
void NetTaskExec(IN ULONG entryInput)
{
	...
	while(1)
	{
		...
		/*MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("before NetTask(0x%x) Que=0x%x\n", NetTask, queueMsg)); */
		/* Execute netTask Job */
					   ...
					   IN ULONG TimeSlice, OUT PRTMP_NET_TASK_QUEUE pNetTaskQueue)
{
	...
	if (ret != TX_SUCCESS)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("tx_queue_create() : create %s fialed [%x]", NetTaskQueName, ret));
		goto ERROR_FAIL;
	}
...
void RTMPDestroyNetTaskQueue(IN PRTMP_NET_TASK_QUEUE pNetTaskQueue)
{
	...
	if (ret != TX_SUCCESS)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("tx_queue_delete(): delete %s failed[%d]\n", 
	...
	if (ret != TX_SUCCESS)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("tx_thread_delete(): delete %s service thread failed[%d]\n",
												pNetTaskQueue->NetTaskQueueName, ret));
		return NDIS_STATUS_FAILURE;
	}
...
int RTMPQueueNetTask(IN PRTMP_NET_TASK_QUEUE pNetTaskQueue, IN PRTMP_NET_TASK_STRUCT pNetTask)
{
	...
	if (ret != TX_SUCCESS)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RTMPQueueNetTask fail(%d), QueMsg = %d\n", ret, queueMsg));
		return NDIS_STATUS_FAILURE;
	}	
