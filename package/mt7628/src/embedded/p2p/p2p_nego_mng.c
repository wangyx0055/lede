...
#if 0
static VOID P2pProvisionReqAction(
	IN PRTMP_ADAPTER pAd,
	IN MLME_QUEUE_ELEM *Elem);
#endif
...
#if 0
static VOID P2pDeviceDiscReqAction(
	IN PRTMP_ADAPTER pAd,
	IN MLME_QUEUE_ELEM *Elem);
#endif
...
/*
	P2P_GO_FORM_IDLE : do nothing, wait SyncMachine send trigger event to notify the peer p2p device be found.
						Use Group Formation or Provision to negotiate.
	P2P_GO_FORM_NEGO : Process Gorup Negotiation Request / Response / Confirm.
	P2P_GO_FORM_PROV :	Process Provision Request / Response.
*/
...
#ifdef RELEASE_EXCLUDE
/*
	IDLE satte handle StartGroupForm(P2P device try to connect someone) and rece GoNegoReq action frame(peer P2P device send GoNegoReq frame to connect us).
*/
#endif /* RELEASE_EXCLUDE */
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (P2P_GO_ON(pAd))
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("P2pPeerGoNegoReqAction ==>P2PGetEntryCnt = %d\n", P2PEntryCnt));
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (p2p_handler)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:: CMD_ID = %ld\n", __FUNCTION__, Elem->Priv));
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (pAd->P2pCfg.Dpid != DEV_PASS_ID_NOSPEC)
	{
		if ((pAd->P2pCfg.Dpid == DEV_PASS_ID_USER) && ((pAd->P2pTable.Client[p2pIdx].ConfigMethod & WSC_CONFMET_DISPLAY) != 0))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Request : CONFIG_METHOD_DISPLAY	\n"));
		}
		else if ((pAd->P2pCfg.Dpid == DEV_PASS_ID_PBC) && ((pAd->P2pTable.Client[p2pIdx].ConfigMethod & WSC_CONFMET_PBC) != 0))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Request : CONFIG_METHOD_PUSHBUTTON  \n"));
		}
		else if ((pAd->P2pCfg.Dpid == DEV_PASS_ID_REG) && ((pAd->P2pTable.Client[p2pIdx].ConfigMethod&WSC_CONFMET_KEYPAD) != 0))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Request : CONFIG_METHOD_KEYPAD  \n"));
		}
		else
		{
			pAd->P2pTable.Client[p2pIdx].P2pClientState = P2PSTATE_SENT_PROVISION_REQ;
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if ((pP2PCtrl->ConfigMethod == WSC_CONFMET_DISPLAY) ||
		((pP2PCtrl->bP2pAutoAccept == TRUE) && (pP2PCtrl->ConfigMethod == WSC_CONFMET_PBC)))
