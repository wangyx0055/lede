...
extern INT Set_AP_WscSsid_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);

extern INT Set_AP_WscConfMode_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);

extern INT Set_AP_WscConfStatus_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);

extern INT Set_AP_WscMode_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);

extern INT Set_AP_WscGetConf_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);

extern INT Set_AP_WscPinCode_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);

extern INT Set_P2pCli_WscSsid_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);

extern INT Set_P2pCli_Ssid_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg);
	...
	IN PRTMP_ADAPTER pAd) 
{
	...
	pP2PCtrl->AssocList.maclist_count=0;
...
#ifdef HE_BD_SUPPORT    //default select PBC for HE_BD
	pP2PCtrl->DefaultConfigMethod = P2P_REG_CM_PBC;
#else		
	...
	IN PVOID SystemSpecific3) 
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s : Counter100ms[%ld] CounterAftScanButton[%ld] ClientConnCnt[%ld] MngAPEnReconnCnt[%ld] DisableRetryGrpFormCnt[%ld] NextScanRound[%ld]\n", 
	...
	if ((pP2PCtrl->bProvAutoRsp == FALSE) && (pP2PCtrl->P2pProvIndex != P2P_NOT_FOUND) )
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("P2P P2pPeriodicExec Scan Begin NextScanRound = %ld \n", pP2PCtrl->P2pCounter.NextScanRound));
			MlmeEnqueue(pAd, 
			...
			|| (pAd->P2pCfg.P2PConnectState == P2P_DO_GO_SCAN_OP_DONE))
		{
			...
			if (!P2P_GO_ON(pAd))
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("P2PConnectState[%s] : Do BssTableSearch\n", decodeP2PState(pP2PCtrl->P2PConnectState)));
			...
			if (BssIdx == BSS_NOT_FOUND) 
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CNTL - Nr= %d. Channel = %d. BSSID not found.  %02x:%02x:%02x:%02x:%02x:%02x.\n", pAd->ScanTab.BssNr, pAd->MlmeAux.Channel /*pAd->StaCfg.WscControl.WscAPChannel*/, PRINT_MAC(pP2PCtrl->Bssid)));
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CNTL - BSSID not found.	Goback to %s  \n", decodeP2PState(pP2PCtrl->P2PConnectState)));
				return;
			}
			...
			pP2PCtrl->PortCfg.SsidLen = pAd->ScanTab.BssEntry[BssIdx].SsidLen;
 			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("P2P P2pPeriodicExec P2P_DO_GO_SCAN_DONE. Find BssIdx = %ld\n", BssIdx));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Change P2PConnectState[%s -> %s]\n", decodeP2PState(pP2PCtrl->P2PConnectState), decodeP2PState(P2P_DO_WPS_ENROLLEE)));
...
#if 0
		if (pP2PCtrl->GONoASchedule.bNeedResumeNoA == TRUE)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: NeedResumeNoA is TRUE, Go Start NoA!\n", __FUNCTION__));
			pP2PCtrl->GONoASchedule.bNeedResumeNoA = FALSE;
			P2pGOStartNoA(pAd);
			P2pUpdateNoABeacon(pAd, MAIN_MBSSID, (PUCHAR) pAd->BeaconBufLen);
		}
#endif
}
...
#ifdef HE_SUPPORT
	P2pSendWirelessEvent(pAd, RT_P2P_STOP_CONNECT, NULL, pP2pCtrl->ConnectingMAC);
#endif /* HE_SUPPORT */
#endif /* RT_P2P_SPECIFIC_WIRELESS_EVENT */
	RTMPZeroMemory(pP2pCtrl->ConnectingMAC, MAC_ADDR_LEN);
}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("<---- P2P - P2pStopScan @channel = %d.\n", pAd->MlmeAux.Channel));
	...
	IN PRTMP_ADAPTER pAd) 
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("P2pGotoScan!  Set pP2PCtrl->NextScanRound = %ld Here!!!\n", pP2PCtrl->P2pCounter.NextScanRound));
}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("P2pGOStartNoA parameter.!!!!HighByte = %lx \n", pAd->P2pCfg.GONoASchedule.TsfHighByte));
	RTMP_IO_READ32(pAd, TBTT_TIMER, &TimeTillTbtt);
	TimeTillTbtt = TimeTillTbtt&0x1ffff;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("   .!!!!TimeTillTbtt =  %ld  \n", TimeTillTbtt));
...
#if 0
	pAd->P2pCfg.GONoASchedule.StartTime = (Value/102400)*102400 + 0x7d000 + 0x6400;
	pAd->P2pCfg.GONoASchedule.NextTargetTimePoint = ((Value/102400) + 1)*102400 + 0x7d000 + 0x6400 + pAd->P2pCfg.GONoASchedule.Interval -pAd->P2pCfg.GONoASchedule.Duration;
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("   .!!!!Expect Starttime= %ld. ThreToWrapAround = %ld. \n", temp, pAd->P2pCfg.GONoASchedule.ThreToWrapAround));
	temp = temp - Value;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("   .!!!!more = %ld  to start time \n", temp));
	...
	IN PMAC_TABLE_ENTRY	pMacClient)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("P2pStopNoA.!!!! \n"));
	
	RTMPCancelTimer(&pAd->P2pCfg.P2pPreAbsenTimer, &Cancelled);
	pAd->P2pCfg.bKeepSlient = FALSE;
	pAd->P2pCfg.bPreKeepSlient = FALSE;
#if 0
	if (pAd->TxSwNoAMgmtQueue.Number > 0)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("TxSwNoAMgmtQueue.Number = %d. Dequeue.!!!! \n", pAd->TxSwNoAMgmtQueue.Number));
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("P2P : !! P2pStartOpPS \n"));
	...
	IN PVOID SystemSpecific3) 
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s::  P2P Execute WSC has expire %dsecs, Stop it!\n", __FUNCTION__, (P2P_WSC_TIMER/1000)));
	...
	else if (P2P_GO_ON(pAd))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("P2pWscTimeOut - Total= %d. p2pEntry = %d.\n", pAd->MacTab.Size, p2pEntryCnt));
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s::  not P2P GO / CLI on !!\n", __FUNCTION__));
}
	...
	IN PVOID SystemSpecific3) 
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:: P2P Send to %02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	if (p2pindex < MAX_P2P_GROUP_SIZE)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ReTransmitCnt limit! stop connect this p2p device!\n"));
		...
		switch(pP2pEntry->P2pClientState)
		{
			case P2PSTATE_SENT_PROVISION_REQ:
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Re-Send PROVISION REQ. KEYPAD\n"));
					ConfigMthd = WSC_CONFMET_KEYPAD;
				}
				else if (pP2pCtrl->ConfigMethod == WSC_CONFMET_KEYPAD)
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Re-Send PROVISION REQ. DISPLAY\n"));
					ConfigMthd = WSC_CONFMET_DISPLAY;
				}
				else if (pP2pCtrl->ConfigMethod == WSC_CONFMET_PBC)
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Re-Send PROVISION REQ. PBC\n"));
					ConfigMthd = WSC_CONFMET_PBC;
				}
				else
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Re-Send PROVISION REQ. wrong Config Method(%d)\n", 
			...
			case P2PSTATE_SENT_GO_NEG_REQ:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Re-Send GO NEGO REQ.\n"));
			...
			case P2PSTATE_SENT_PROVISION_RSP:
				...
				break;
			default:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ReSendTimeout execute with unknown state - %s\n", decodeP2PClientState(pP2pEntry->P2pClientState)));
		}
	}
}
	...
	IN PUCHAR pData) 
{
	...
	if ((*pData == SUBID_P2P_NOA))
	{
		...
		else if (Value >= StartTime)
		{
			do
			{
				...
				if ((StartTime > Value) && ((StartTime-Value) > 0x80))
				{
					...
					if (P2pResetNoATimer(pAd, GPDiff))
					{
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("!!!!!Start NoA 2  GPDiff = %ld\n", GPDiff));
						pMacClient->P2pInfo.NoADesc[0].bValid = TRUE;
						pMacClient->P2pInfo.NoADesc[0].bInAwake = TRUE;
						pMacClient->P2pInfo.NoADesc[0].Token = index;
						return TRUE;
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Start time in before ..!!Check \n"));
		}
	}
	return FALSE;
	...
	IN	ULONG	DiffTimeInus)
{
	...
	if (GPDiff > 0)
	{
		AsicSetGPTimer(pAd, TRUE, GPDiff);
		return TRUE;
	}
	return FALSE;
}
	...
	IN ULONG		NextTimePoint) 
{
	...
	if (CurrentTimeStamp < pP2PCtrl->GONoASchedule.CurrentTargetTimePoint)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("P2pAdjustSwNoATimer HERE HERE!!!! \n"));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("OngoingAwakeTime = %ld. CurrentTimeStamp = %ld.!!!! \n", pP2PCtrl->GONoASchedule.OngoingAwakeTime, CurrentTimeStamp));
	...
	IN PRTMP_ADAPTER pAd) 
{
	...
	if (P2P_GO_ON(pAd))
	{
		if (pP2PCtrl->GONoASchedule.bValid == TRUE)
		{
			...
			{
				P2pStopNoA(pAd, NULL);
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("P2pGPTimeOutHandle.!!StopGP.	return.1 \n"));
				return;
			}
			...
			if (pP2PCtrl->GONoASchedule.ThreToWrapAround > pP2PCtrl->GONoASchedule.StartTime)
			{
				if (Value > pP2PCtrl->GONoASchedule.ThreToWrapAround)
				{
					...
					P2pStopNoA(pAd, NULL);
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("P2pGPTimeOutHandle.!!StopGP.	return.3. will resume.\n"));
					return;
				}
			}
			else
			{
				if ((Value > pP2PCtrl->GONoASchedule.ThreToWrapAround) && (Value < pP2PCtrl->GONoASchedule.StartTime))
				{
					pP2PCtrl->GONoASchedule.bNeedResumeNoA = TRUE;
					P2pStopNoA(pAd, NULL);
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("P2pGPTimeOutHandle.!!StopGP.	return.4. will resume. \n"));
					return;
				}
			}
			...
			if (Value <= pP2PCtrl->GONoASchedule.NextTargetTimePoint)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,(" NextTargetTimePoint = %ld. \n", pAd->P2pCfg.GONoASchedule.NextTargetTimePoint));
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("  Value = %ld.	GPDiff = %ld.\n", Value, GPDiff));
			}
			else
			{
				...
				{
					pP2PCtrl->GONoASchedule.bNeedResumeNoA = TRUE;
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" Prepare resume NoA. \n"));
				}
				P2pStopNoA(pAd, NULL);
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("3 NextTargetTimePoint = %ld. \n", pP2PCtrl->GONoASchedule.NextTargetTimePoint));
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" 3 Value = %ld= 0x%lx NextDiff = %ld.\n", Value, Value, NextDiff));
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("3 P2pGPTimeOutHandle.!!StopGP.  return.2 \n"));
				return;
			}

			if (pP2PCtrl->GONoASchedule.bInAwake == TRUE)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" ----------------------->>> NoA  Go to SLEEP \n"));		
...
#if 0/*UAPSD_SUPPORT*/
				{
					...
					for(i=2; i<MAX_LEN_OF_MAC_TABLE; i++)
					{
						if	(pAd->MacTab.Content[i].ValidAsCLI) 
							UAPSDServicePeriodClose(pAd, &pAd->MacTab.Content[i], TRUE);
					}
				}				
#endif
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" NoA	Wake UP ----------------------->>> \n"));
				pP2PCtrl->GONoASchedule.bInAwake = TRUE;
				pP2PCtrl->GONoASchedule.bWMMPSInAbsent = FALSE; /* Set to FALSE if changes state to Awake */
				pP2PCtrl->bKeepSlient = FALSE;	
					...
					&& (pP2PCtrl->GONoASchedule.Count > 100))
				{
					...
					{
						/*MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("SwBasedNoA : Dequeue here. %d\n", pAd->TxSwNoAMgmtQueue.Number));*/
	...
	else if (P2P_CLI_ON(pAd))
	{
		...
		if (pP2PCtrl->NoAIndex != pP2PCtrl->MyGOwcid)
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("P2pGPTimeOutHandle. !bug, please check driver %d. \n", pP2PCtrl->NoAIndex));
		...
		if (pEntry->P2pInfo.NoADesc[0].bValid == TRUE)
		{
			...
			if (pEntry->P2pInfo.NoADesc[0].Count == 0)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("P2pGPTimeOutHandle. Count down to zero!!StopGP.  return.1 \n"));
				return;
			}
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("2 Tsf	Timer  = %ld= 0x%lx	 NextTargetTimePoint = %ld.\n", MacValue,  MacValue,pEntry->P2pInfo.NoADesc[0].NextTargetTimePoint));
			...
			if (MacValue <= pEntry->P2pInfo.NoADesc[0].NextTargetTimePoint)
			{
				...
				P2pResetNoATimer(pAd, GPDiff);
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("3	Continue next NOA NextTargetTimePoint = %lx. \n", pEntry->P2pInfo.NoADesc[0].NextTargetTimePoint));
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("3	Value = %lx.  NextDiff = %lx.\n", MacValue, NextDiff));
			}
			else
			{
				P2pStopNoA(pAd, pEntry);
#ifdef RELEASE_EXCLUDE
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("4  P2pGPTimeOutHandle.!!StopGP.  Error Case. shoudn't happen! \n"));
#endif /* RELEASE_EXCLUDE */
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("4  NOA NextTargetTimePoint = %ld. \n", pEntry->P2pInfo.NoADesc[0].NextTargetTimePoint));
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("4  Value = %ld = 0x%lx.  NextDiff = %ld.\n", MacValue,  MacValue, NextDiff));
				return;
			}
			...
			if (pEntry->P2pInfo.NoADesc[0].bInAwake == TRUE)
			{
				...
				pP2PCtrl->bKeepSlient = TRUE;
				pP2PCtrl->bPreKeepSlient = TRUE;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Enter Absence now ======> %d\n", pP2PCtrl->bKeepSlient));
			}
			else
			{
...
#if 0/*UAPSD_SUPPORT*/
				{
					...
					if (pAd->CommonCfg.bAPSDCapable && pAd->CommonCfg.APEdcaParm.bAPSDCapable)
					{
						...
						{
							if (pAd->P2pCfg.LastMoreDataFromGO[i] == TRUE)
							{
								MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("P2pGPTimeOutHandle!WMM-PS!Wakeup! QueId#%d sends a Qos Null frame to GO due to MoreData \n", i)); 			
								RTMPSendNullFrame(pAd, pAd->PortList[0], pAd->CommonCfg.TxRate, TRUE);
								/* After sent a triggered frame, reset this record. */
								pAd->P2pCfg.LastMoreDataFromGO[i] = FALSE;
							}	
						}
					}
				}
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Enter Awake now ======= %d\n", pAd->P2pCfg.bKeepSlient));
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("--->P2pPauseBssSync  %s\n", decodeP2PState(pAd->P2pCfg.P2PConnectState)));
#if 0
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("--->P2pResumeBssSync  %s\n", decodeP2PState(pAd->P2pCfg.P2PConnectState)));
#if 0
	RTMP_IO_READ32(pAd, BCN_TIME_CFG, &csr.word);
	...
	IN UCHAR		Channel)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" P2pSetRule - Channel = %d. GoIntent = %x\n", Channel, PeerGOIntentAttri));
	...
	if (RealIntent > pP2PCtrl->GoIntentIdx)
	{
		...
		*/
		...
		{
			if ((pP2pEntry->ConfigMethod == WSC_CONFMET_KEYPAD) || (pP2pEntry->ConfigMethod == WSC_CONFMET_DISPLAY))
				pP2PCtrl->WscMode = WSC_PIN_MODE;
			else if (pP2pEntry->ConfigMethod == WSC_CONFMET_PBC)
				pP2PCtrl->WscMode = WSC_PBC_MODE;			
		}
#endif
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" P2P - I become Internal Enrollee!!    Enrollee. !! GOGOGO\n"));
	}
...
#endif
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" P2P - MyIntent = %d, PeerIntent = %d\n", pP2PCtrl->GoIntentIdx, RealIntent));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" P2P - I become Internal REGISTRA!!    REGISTRA. !! GOGOGO\n"));
	}
	else if (((PeerGOIntentAttri&1) == 1) && (RealIntent == pP2PCtrl->GoIntentIdx))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("P2pSetRule Peer Tie Breaker bit is On.  %x \n", RealIntent));
		pP2pEntry->Rule = P2P_IS_GO;
		pP2pEntry->GoIntent = RealIntent;
		/* Use peer addr as bssid */
		RTMPMoveMemory(pP2PCtrl->PortCfg.Bssid, PeerBssid, MAC_ADDR_LEN);
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("P2pSetRule Peer Tie Breaker bit is Off.  %x \n", RealIntent));
		pP2pEntry->Rule = P2P_IS_CLIENT;
		pP2pEntry->GoIntent = RealIntent;
		/* Use my addr as bssid */
		RTMPMoveMemory(pP2PCtrl->Bssid, pP2PCtrl->CurrentAddress, MAC_ADDR_LEN);
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("P2pSetRule pPort Bssid = %02x %02x %02x %02x %02x %02x  \n",  pP2PCtrl->PortCfg.Bssid[0], pP2PCtrl->PortCfg.Bssid[1],pP2PCtrl->PortCfg.Bssid[2],pP2PCtrl->PortCfg.Bssid[3],pP2PCtrl->PortCfg.Bssid[4],pP2PCtrl->PortCfg.Bssid[5]));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("GroupOpChannel =  %d  \n",pP2PCtrl->GroupOpChannel));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" P2pSetRule - pP2pEntry->Rule = %s. \n", decodeMyRule(pP2pEntry->Rule)));
	...
	IN PUCHAR	Addr)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("P2pProvision = %02x:%02x:%02x:%02x:%02x:%02x.  \n",  PRINT_MAC(Addr)));
	p2pindex = P2pGroupTabSearch(pAd, Addr);
	if (p2pindex < MAX_P2P_GROUP_SIZE)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Start P2pProvision = %02x:%02x:%02x:%02x:%02x:%02x.  \n",  PRINT_MAC(Addr)));
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("P2pConnect =  \n"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Addr = %02x:%02x:%02x:%02x:%02x:%02x.  \n",  PRINT_MAC(pAd->P2pCfg.ConnectingMAC)));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ListenChannel =  %d\n", Channel));
	...
	IN UINT32 ConnType)
{
	...
	if (p2pindex < MAX_P2P_GROUP_SIZE)
	{
...
#endif /*CONFIG_MULTI_CHANNEL*/
#ifdef HE_TV_SUPPORT
		/* Send Invitation and wait the provision request from peer, and need to be confirmed by user*/
		if(ConnType==P2PSTATE_INVITE_COMMAND)
			pAd->P2pCfg.bConfirmByUI = FALSE;
		else
#endif
			...
			Channel = pAd->P2pTable.Client[p2pindex].ListenChannel;
					...
					pAd->CommonCfg.CentralChannel == pAd->LatchRfRegs.Channel))
				{
					RTMPSendNullFrame(pAd, 
								  ...
								  PWR_SAVE);
					OS_WAIT(200);
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("P2pConnectPrepare():Send PWA NullData frame to notify the associated AP!\n"));
				}
#endif /*CONFIG_MULTI_CHANNEL*/
...
#if 1 //Patch Sigma 5.1.22 (Let P2pGroupMaintain() can send probe. req. soon!)
		pAd->P2pTable.Client[p2pindex].StateCount = 0;
#else
		pAd->P2pTable.Client[p2pindex].StateCount = 10;
#endif
		pAd->P2pTable.Client[p2pindex].bValid = TRUE;

		/* Set pEntry Client Status. */
#ifdef HE_TV_SUPPORT
		if ((pAd->P2pTable.Client[p2pindex].P2pClientState == P2PSTATE_DISCOVERY_GO) && (ConnType!=P2PSTATE_INVITE_COMMAND))
#else
		if ((pAd->P2pTable.Client[p2pindex].P2pClientState == P2PSTATE_DISCOVERY_GO))
#endif		
		{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("P2P Connect Command Type =  %s\n", decodeP2PClientState(ConnType)));
	...
	IN UCHAR		idx)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, (" P2pConnectAfterScan   %d. %s \n", pAd->P2pTable.Client[idx].P2pClientState, decodeP2PClientState(pAd->P2pTable.Client[idx].P2pClientState)));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, (" GroupOpChannel  = %d  \n", pP2PCtrl->GroupOpChannel));

	if (GrpIndex >= MAX_P2P_GROUP_SIZE)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("P2pConnectAfterScan Check - Reach Device Limit. return.  \n"));
		return FALSE;
	}
	...
	*/
	if (pAd->P2pTable.Client[GrpIndex].P2pClientState == P2PSTATE_DISCOVERY)
	{
		...
		if (P2P_CLI_ON(pAd) || (P2P_GO_ON(pAd)))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" P2pConnectAfterScan -  Use Invite %d.\n", GrpIndex));
			bAction = TRUE;
		}
		else if (IS_P2P_CONNECT_IDLE(pAd))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, (" P2pConnectAfterScan - Perst index %d.  \n", index));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, (" P2pConnectAfterScan - EnablePresistent %d.  \n", 
					pP2PCtrl->P2pControl.field.EnablePresistent));
			if ((index < MAX_P2P_TABLE_SIZE) && IS_PERSISTENT_ON(pAd))
			{
				/* Invite Case 3: */
				pAd->P2pTable.Client[GrpIndex].P2pClientState = P2PSTATE_INVITE_COMMAND;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, (" P2pConnectAfterScan -  Use reinvoke Invite %d.  \n", index));
				bAction = TRUE;
			}
			else
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" P2pConnectAfterScan -  Start From Group Forming   \n"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" case 1  = peer is go  \n"));
		P2pStopConnectThis(pAd);
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" case 2  = peer is client  \n"));
		P2pStopConnectThis(pAd);
		P2pConnectP2pClient(pAd,GrpIndex);
	}
	else
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" P2pConnectAfterScan - Peer state %s \n", 
	...
	IN UCHAR		index)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, (" P2pConnectAction   %d. %s \n", pAd->P2pTable.Client[index].P2pClientState, decodeP2PClientState(pAd->P2pTable.Client[index].P2pClientState)));

	if (bBeacon == TRUE)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, (" P2pConnectAction   from bBeacon \n"));
	...
	{		
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, (" P2pConnectAction  not from bBeacon \n"));
		if ((pAd->P2pTable.Client[index].P2pClientState == P2PSTATE_CONNECT_COMMAND)
			|| (pAd->P2pTable.Client[index].P2pClientState == P2PSTATE_INVITE_COMMAND))
		{
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("  case 1 \n"));
			...
			else if (pAd->P2pTable.Client[index].Rule == P2P_IS_GO)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("  case 2 \n"));
			...
			else if (IS_P2P_CONNECT_IDLE(pAd))
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("  case 3 \n"));
				/* since I am idle,  */
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("  case 4 ?????? \n"));
			if (bresult == TRUE)
			{
				/* do nothing */
			}
	...
	IN UCHAR		GrpIndex)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("P2pConnect to P2pClient====>.  \n"));
	...
	if (IS_P2P_CONNECT_IDLE(pAd))
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("P2P Client Not support Discoverability  %x \n", pAd->P2pTable.Client[GrpIndex].DevCapability));
		}
		else
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("P2P Client CAN Support Discoverability  %x \n", pAd->P2pTable.Client[GrpIndex].DevCapability));

		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("P2P Client DevCapability  %x. GoP2pIndex = %d. \n", pAd->P2pTable.Client[GrpIndex].DevCapability, GoP2pIndex));
		if ((GoP2pIndex < MAX_P2P_GROUP_SIZE) && (pAd->P2pTable.Client[GoP2pIndex].P2pClientState == P2PSTATE_DISCOVERY_GO))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("P2P  GrpIndex  %d . GoP2pIndex = %d\n", GrpIndex, GoP2pIndex));
			bGoToScan = TRUE;	
		}
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" P2pConnectIdle - peer device's state %s \n", decodeP2PClientState(pAd->P2pTable.Client[GrpIndex].P2pClientState)));
	}
	...
	IN UCHAR		idx)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("P2pConnectP2pGo.  %s\n", decodeP2PState(pAd->P2pCfg.P2PConnectState)));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("pAd->P2pCfg.Dpid = %d \n", pAd->P2pCfg.Dpid));
	/* Decide connect method when I am in COnnect_Idle state. This means I am a P2P Device. */
	if (P2P_GO_ON(pAd))
		return brc;
...
#if 0
		pAd->P2pCfg.P2PConnectState = P2P_ANY_IN_FORMATION_AS_CLIENT;
		pAd->P2pTable.Client[idx].P2pClientState = P2PSTATE_GO_WPS;
		pAd->P2pTable.Client[idx].Rule = P2P_IS_GO;
		RTMPMoveMemory(pAd->PortList[pAd->P2pCfg.PortNumber]->CommonCfg.Bssid, pAd->P2pTable.Client[idx].bssid, MAC_ADDR_LEN);
		RTMPMoveMemory(pAd->P2pCfg.Bssid, pAd->P2pTable.Client[idx].bssid, MAC_ADDR_LEN);
		RTMPMoveMemory(pAd->PortList[pAd->P2pCfg.PortNumber]->CommonCfg.Ssid, pAd->P2pTable.Client[idx].Ssid, 32);
		pAd->PortList[pAd->P2pCfg.PortNumber]->CommonCfg.SsidLen = pAd->P2pTable.Client[idx].SsidLen;
		pAd->P2pCfg.GroupOpChannel = pAd->P2pTable.Client[idx].OpChannel;
		P2pGoNegoDone(pAd, pAd->PortList[pAd->P2pCfg.PortNumber], &pAd->P2pTable.Client[idx]);
#endif /*0*/
		brc = TRUE;
	}
	else if (P2P_CLI_ON(pAd))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("invalid P2pConnectP2pGo command when %s \n", decodeP2PState(pAd->P2pCfg.P2PConnectState)));
	...
	IN UCHAR 		GoP2pTabIdx)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("P2pClientDiscovery  %d\n", GoP2pTabIdx));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("P2pClientDiscovery.  ClientP2PIndex= %d\n", ClientP2PIndex));
	...
	IN UCHAR 		P2pTabIdx)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("P2pInviteAsRule MyRule = %d. P2pTabIdx= %d \n",  MyRule, P2pTabIdx));
	pClient = &pAd->P2pTable.Client[P2pTabIdx];
	P2PMakeInviteReq(pAd, MyRule, 0, pClient->addr, pP2PCtrl->CurrentAddress, &FrameLen);

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("P2pInviteAsRule FrameLen = %ld.  \n", FrameLen));
	if (FrameLen > 0)
		return TRUE;
...
/*	UCHAR		index; */
	...
	else if (P2pTabIdx < MAX_P2P_GROUP_SIZE)
	{
		if ((pAd->P2pTable.Client[P2pTabIdx].DevCapability & DEVCAP_INVITE) == DEVCAP_INVITE)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Peer doesn't support Invite. DevCapability = %x \n", pAd->P2pTable.Client[P2pTabIdx].DevCapability));
	}
	else
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("invalid P2pInvite command. %d %d \n",  PersistentTabIdx, P2pTabIdx));
	...
	IN UCHAR		idx)
{
	...
	if (idx < MAX_P2P_GROUP_SIZE)
	{
		...
		(pAd->P2pTable.Client[idx].P2pClientState == P2PSTATE_SENT_GO_NEG_REQ))
		{
			...
			{
				pP2PCtrl->P2PConnectState = P2P_ANY_IN_FORMATION_AS_CLIENT;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s::  Start GroupForm .  \n", __FUNCTION__));
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, (" P2P - Make GO Negociation Req FrameLen  = %ld. \n", FrameLen));	
			}	
	...
	IN ULONG		DeviceNameLen)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("P2p : P2PDevice Not Match from %02x:%02x:%02x:%02x:%02x:%02x.. = %s \n", PRINT_MAC(Addr), decodeP2PState(pAd->P2pCfg.P2PConnectState)));
	return FALSE;
...
{ 
	...
	int i;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:: ==> P2P Connect State = %d.     %s.\n", 
            ...
            if (pP2PCtrl->Rule == P2P_IS_GO)
            {
                        if( pP2PCtrl->AssocList.maclist_count < 3)
                        {
                                    ...
                                    if ( ! is_found )
                                    {
                                                ...
                                                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("pP2PCtrl->AssocList.maclist_count = %d\n", pP2PCtrl->AssocList.maclist_count));
                                                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("pP2pEntry->InterfaceAddr = %02X:%02X:%02X:%02X:%02X:%02X\n", PRINT_MAC(pP2pEntry->InterfaceAddr)));
                                                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("pP2pEntry->addr = %02X:%02X:%02X:%02X:%02X:%02X\n", PRINT_MAC(pP2pEntry->addr)));
                                                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("pP2pEntry->bssid = %02X:%02X:%02X:%02X:%02X:%02X\n", PRINT_MAC(pP2pEntry->bssid)));
                                                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("pP2pEntry->DeviceName = %s\n", pP2pEntry->DeviceName));
	...
	if ((pP2PCtrl->Rule == P2P_IS_GO) && (P2P_GO_ON(pAd)))
	{
		...
		else
		{
			...
			else
			{
				if (INFRA_ON(pAd) || P2P_GO_ON(pAd))
				{
					if ((pAd->CommonCfg.RegTransmitSetting.field.BW == BW_40) && (pAd->LatchRfRegs.Channel != pAd->CommonCfg.CentralChannel))
						bChangeInitBW = TRUE;
				}
			}
		}
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:: Rule = %s. OpChannel = %d. CentralChannel = %d\n", 
					__FUNCTION__, decodeMyRule(pP2PCtrl->Rule), pAd->CommonCfg.Channel, pAd->CommonCfg.CentralChannel));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("      SSID[%d] = %s.    BSSID = %02x:%02x:%02x:%02x:%02x:%02x.\n", 
					pP2PCtrl->SSIDLen, pP2PCtrl->SSID, PRINT_MAC(pP2PCtrl->CurrentAddress)));
		pP2PCtrl->P2pCapability[1] |= GRPCAP_OWNER;
		
		/* P2P Nego GO up. */
		if (pAd->flg_p2p_OpStatusFlags == P2P_GO_UP)
		{
			BOOLEAN skipSetGo = FALSE;

			for (i=0; i<MAX_LEN_OF_MAC_TABLE; i++)
			{
				pEntry = &pAd->MacTab.Content[i];
				if (pEntry != NULL && (IS_P2P_CLI_ENTRY(pEntry) || (IS_P2P_GO_ENTRY(pEntry))) 
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("P2P_FIXED_MODE\n"));
			AsicBBPAdjust(pAd);
		}

		/* 
			1. Fix G3 IOT issue
			2. Shorten the P2P connection time
			3. Move here to fix MgmtRing error issue
		*/
		if (pWscControl->WscConfStatus == WSC_SCSTATE_UNCONFIGURED)
		{
			...
			pWscControl->WscKeyASCII = 0;
			WscGenRandomKey(pAd, pWscControl, pWscControl->WpaPsk, &pWscControl->WpaPskLen);
			RT_CfgSetWPAPSKKey(pAd, pWscControl->WpaPsk, pWscControl->WpaPskLen, (PUCHAR)pMbss->Ssid, pMbss->SsidLen, pMbss->PMK);
			NdisZeroMemory(pP2PCtrl->PhraseKey, 64);
			pP2PCtrl->PhraseKeyLen = pWscControl->WpaPskLen;
			NdisMoveMemory(pP2PCtrl->PhraseKey, pWscControl->WpaPsk, pP2PCtrl->PhraseKeyLen);
		}
		...
		if (pP2PCtrl->WscMode == WSC_PIN_MODE)
		{
			...
			{
				/* we are keypad, If ConfirmByUI is not true,
				   we shall ignore to trigger wps, until UI type PIN of peer done. */
				...
				*/
				...
				Set_AP_WscGetConf_Proc(pAd, "1");
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:: WSC_CONFMET_KEYPAD, bConfirmByUI == TRUE, do further WSC operation!!\n", __func__));
					/* Reset bConfirmByUI to its default value */
					pP2PCtrl->bConfirmByUI =  pP2PCtrl->bP2pAutoAccept;
				}	
				else
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:: WSC_CONFMET_KEYPAD, bConfirmByUI == FALSE, wait for Upper layer Trigger WSC operation!!\n", __func__));	
			}
		...
		{
			/* PBC here, If ConfirmByUI is not true,
			   we shall ignore to trigger wps, until UI say yes!! */
			...
			*/
			...
			OS_WAIT(700);
			Set_AP_WscGetConf_Proc(pAd, "1");
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:: WSC_PBC_MODE, bConfirmByUI == TRUE, do further WSC operation!!\n", __func__));
				/* Reset bConfirmByUI to its default value */
				pP2PCtrl->bConfirmByUI = pP2PCtrl->bP2pAutoAccept;
			}
			else
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:: WSC_PBC_MODE, bConfirmByUI == FALSE, wait for Upper layer Trigger WSC operation!!\n", __func__));	
		}
	...
	else if ((pP2PCtrl->Rule == P2P_IS_CLIENT) && (P2P_CLI_ON(pAd)))
	{
		...
		if (!INFRA_ON(pAd))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:: !INFRA_ON Rule = %s. OpChannel = %d. CentralChannel = %d\n", 
						__FUNCTION__, decodeMyRule(pP2PCtrl->Rule), pAd->CommonCfg.Channel, pAd->CommonCfg.CentralChannel));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, (" 	 SSID[%d] = %s.    BSSID = %02x:%02x:%02x:%02x:%02x:%02x.\n", 
		...
		{
			pAd->ApCfg.ApCliTab[0].Channel = pP2PCtrl->GroupOpChannel;
			/*N_SetCenCh(pAd); */
			AsicSwitchChannel(pAd, pAd->ApCfg.ApCliTab[0].Channel , FALSE);
			AsicLockChannel(pAd, pAd->ApCfg.ApCliTab[0].Channel );
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:: INFRA_ON Rule = %s. OpChannel = %d. CentralChannel = %d\n", 
						__FUNCTION__, decodeMyRule(pP2PCtrl->Rule), pAd->ApCfg.ApCliTab[0].Channel , pAd->ApCfg.ApCliTab[0].CentralChannel));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, (" 	 SSID[%d] = %s.    BSSID = %02x:%02x:%02x:%02x:%02x:%02x.\n", 
						pP2PCtrl->SSIDLen, pP2PCtrl->SSID, PRINT_MAC(pP2PCtrl->Bssid)));
		}
#else
		...
		else if (pP2PCtrl->WscMode == WSC_PBC_MODE)
		{
			...
			/*
				...
				once UI/wpa_supplicant ready, it will call P2pConnectPrepare to toggle ConfirmByUI
			*/
			if ((pAd->P2pCfg.bConfirmByUI == TRUE) || (pP2PCtrl->bSigmaEnabled == TRUE)) {
				OS_WAIT(700);
			Set_AP_WscGetConf_Proc(pAd, "1");
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:: WSC_PBC_MODE, bConfirmByUI == TRUE, do further WSC operation!!\n", __func__));
				/* Reset bConfirmByUI to its default value */
	...
	IN PRT_P2P_CLIENT_ENTRY pP2pEntry) 
{
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("<---- P2pGoNegoDone: ConnectState[%s] \n", decodeP2PState(pP2PCtrl->P2PConnectState)));
#endif /* RELEASE_EXCLUDE */
	...
	if (pP2PCtrl->P2PConnectState == P2P_ANY_IN_FORMATION_AS_GO)
	{
		if (INFRA_ON(pAd) &&
			(pAd->CommonCfg.Channel != pP2PCtrl->GroupOpChannel) &&
			((pP2PCtrl->CheckPeerChannelPolicy == CHECK_PEER_CHANNEL_EVEN_IF_CONNECTION) && (pP2pEntry->bGoUsePeerChannel == TRUE)))
		{
#ifdef RT_P2P_SPECIFIC_WIRELESS_EVENT
#ifdef HE_SUPPORT
			P2pSendWirelessEvent(pAd, RT_P2P_DEAUTH_AP, pP2pEntry, pAd->MlmeAux.Bssid);
#endif /* HE_SUPPORT */
#endif /* RT_P2P_SPECIFIC_WIRELESS_EVENT */
			LinkDown(pAd, FALSE);
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("pAd->P2pCfg.Dpid = %s \n", decodeDpid(pP2PCtrl->Dpid)));
		/* Set MyRule in P2P GroupFormat */
		...
		if (pP2PCtrl->Dpid != DEV_PASS_ID_NOSPEC)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("P2pGoNegoDone. NextScanRound= %ld\n", pP2PCtrl->P2pCounter.NextScanRound));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("2 WscMode = %lx     \n", pP2PCtrl->WscMode));
			/* Start WPS. */
			P2pSetWps(pAd, pP2pEntry);
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("P2pGoNegoDone-  invalid p2pstate = %s\n", decodeP2PState(pP2PCtrl->P2PConnectState)));
		
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("----> P2pGoNegoDone: P2PState= %s\n", decodeP2PState(pP2PCtrl->P2PConnectState)));
#endif /* RELEASE_EXCLUDE */
...
*/
	...
	if (pP2PCtrl->bWIDI)
	{
		...
		*/
		UCHAR * tPtr = pAd->P2pCfg.Bssid;
		if ( (!tPtr[0]) && (!tPtr[1]) && (!tPtr[2]) && (!tPtr[3]) && (!tPtr[4]) && (!tPtr[5]) )	
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Bssid is zero; Not sending this LINK DOWN\n"));
		}
		else
		{
			WidiUpdateStateToDaemon(pAd, MIN_NET_DEVICE_FOR_P2P_CLI, WIDI_MSG_TYPE_ASSOC_STATUS,
				pAd->P2pCfg.Bssid, NULL, 0, WIDI_DISASSOCIATED);		
		}
	}
#endif /* WIDI_SUPPORT */
...
*/
	...
	pP2PCtrl->bConfirmByUI = pP2PCtrl->bP2pAutoAccept;
	if (pP2PCtrl->CheckPeerChannelPolicy != CHECK_PEER_CHANNEL_DISABLE)
		pP2PCtrl->GroupChannel = 1;
	...
	if (IS_PERSISTENT_ON(pAd) && pP2PCtrl->bP2pCliReConnect)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("P2P CLIENT Re-Connect to %02x:%02x:%02x:%02x:%02x:%02x\n", 
					PRINT_MAC(pAd->P2pCfg.ConnectingMAC)));
	}
}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	/* Construct AP RSN_IE.  */
	RTMPMakeRSNIE(pAd, Ndis802_11AuthModeWPA2PSK, Ndis802_11AESEnable, 0 /*apidx*/);
	...
	/*AsicSetBssid(pAd, pP2PCtrl->CurrentAddress);  */
	{
		...
		if ((pAd->CurrentAddress[5] % 2 != 0)
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("The 2-BSSID mode is enabled, the BSSID byte5 MUST be the multiple of 2\n"));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!4 OpStatusFlags = %lx. CentralChannel = %d. Channel	= %d. !!! \n", pAd->CommonCfg.OpStatusFlags,pAd->CommonCfg.CentralChannel,pAd->CommonCfg.Channel));
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s <=== count = %d, Channels are %d, %d,%d separately   \n", 
	...
	IN PUCHAR	pChannelList)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" P2P - no common channel = %d...\n", *pChannelList));
		return FALSE;
	}
	...
	OUT 		UCHAR	*TempPid)
{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("P2pAckRequiredCheck --> This is a Confirm Frame. Pid = RequireACK !!!\n"));
		...
		if ((pFrame->Subtype == P2P_INVITE_RSP) && 
			...
			(pFrame->Category == CATEGORY_PUBLIC))
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("P2pAckRequiredCheck --> This is a Invite Rsp Frame. Pid = RequireACK !!!\n"));
			*TempPid = 0x7; /* PID_REQUIRE_ACK */
			return;
		}
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("P2pAckRequiredCheck --> This is a GO Discoverbility. Pid = RequireACK !!!\n"));
			*TempPid = 0x7; /* PID_REQUIRE_ACK */
			return;
		}
...
#endif /* RELEASE_EXCLUDE */
		if ((IS_P2P_REGISTRA(pAd)) && (pFrame->p80211Header.FC.Type == FC_TYPE_DATA))
		{
			...
			if (NdisEqualMemory(EAPHEAD, pDest, 8))
			{
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("P2pAckRequiredCheck --> This is a EAP FailFrame.	Pid = RequireACK!!!\n"));
					*TempPid = 0x7; /* PID_REQUIRE_ACK */
				}
			}
	...
	UINT32	value)
{
	...
	if ((pP2PCtrl->P2pCounter.bStartScan == TRUE) && (pP2PCtrl->P2pCounter.bListen == FALSE))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s:: Listen interval - %d\n", __FUNCTION__, pP2PCtrl->P2pCounter.ListenInterval));
#endif
	...
	UINT32	value)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: NextScanRound - %ld\n", __FUNCTION__, pP2PCtrl->P2pCounter.NextScanRound));
}
	...
	UINT32	value)
{
	...
	if (pP2PCtrl->P2pCounter.bStartScan == FALSE)
	{
		...
		if (pP2PCtrl->bWIDI)
			pP2PCtrl->P2pCounter.CounterAftrScanButton = 20;
		else
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:: CounterAftrScanButton = %lu.\n", __FUNCTION__, pP2PCtrl->P2pCounter.CounterAftrScanButton));
	}
}
	...
	PUSHORT PeerWscMethod)
{
	PRT_P2P_CONFIG pP2PCtrl = &pAd->P2pCfg;
	PRT_P2P_CLIENT_ENTRY pP2pEntry = &pAd->P2pTable.Client[index];
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:: Peer Dpid = %s.    My Dpid = %s.\n", __FUNCTION__, decodeDpid(pP2pEntry->Dpid), decodeDpid(pP2PCtrl->Dpid)));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:: Peer Config Method = %s.    My Config Method = %s.\n", __FUNCTION__, decodeConfigMethod(pP2pEntry->ConfigMethod), decodeConfigMethod(pP2PCtrl->ConfigMethod)));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("My P2P Wsc Configuration:\n"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WscMode = %d.    WscConfigMethod = %s.    WscDpid = %s.\n", pP2PCtrl->WscMode, decodeConfigMethod(pP2PCtrl->ConfigMethod), decodeDpid(pP2PCtrl->Dpid)));
	if (pP2PCtrl->ConfigMethod == WSC_CONFMET_DISPLAY)
	{
		/*
		...
		*/
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("    *                                               *\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("    *       PIN Code = %08u                     *\n", pAd->ApCfg.ApCliTab[0].WscControl.WscEnrolleePinCode));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("    *                                               *\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("    *************************************************\n"));
		{
#if 0 /* os_abl_move */
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("adding %d bytes\n", wrqu.data.length));
				    	wireless_send_event(pAd->p2p_dev, IWEVP2PSHOWPIN, &wrqu, NULL);
#else
/*
...
*/
	...
	else if (pP2PCtrl->ConfigMethod == WSC_CONFMET_KEYPAD)
	{
		/* Woody 2: Enter PIN Code */
		{
					...
					union iwreq_data wrqu;
					NdisZeroMemory(&wrqu, sizeof(wrqu));
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Key Pin Key Pin Key Pin Key Pin\n"));
				    	wireless_send_event(pAd->p2p_dev, IWEVP2PKEYPIN, &wrqu, NULL);
#else
/*
...
*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Peer P2P Wsc Configuration:\n"));
...
/* 
	...
	==========================================================================
*/
RTMP_STRING *ddecodeConfigMethod (USHORT ConfigMethos)
{
	RTMP_STRING *retval = "                                 ";
	ULONG		mm[4], i;
	mm[0] = 0;
...
{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("                         DeviceCapability = %x.\n", State));
	if (State & DEVCAP_SD)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("                             [0] bServiceDiscovery\n"));
	if (State & DEVCAP_CLIENT_DISCOVER)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("                             [1] bP2PClientDiscoverability\n"));
	if (State & DEVCAP_CLIENT_CONCURRENT)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("                             [2] bConcurrentOperation\n"));
	if (State & DEVCAP_INFRA_MANAGED)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("                             [3] bP2PInfrastructureManaged\n"));
	if (State & DEVCAP_DEVICE_LIMIT)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("                             [4] bP2PDeviceLimit\n"));
	if (State & DEVCAP_INVITE)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("                             [5] bP2PInvitationProcedure\n"));
}

VOID decodeGroupCap (UCHAR State)
{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("                         GroupCapability = %x.\n", State));
	if (State & GRPCAP_OWNER)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("                             [0] bP2PGroupOwner\n"));
	if (State & GRPCAP_PERSISTENT)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("                             [1] bPersistentP2PGroup\n"));
	if (State & GRPCAP_LIMIT)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("                             [2] bP2PGroupLimit\n"));
	if (State & GRPCAP_INTRA_BSS)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("                             [3] bIntra-BssDistribution\n"));
	if (State & GRPCAP_CROSS_CONNECT)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("                             [4] bCrossConnection\n"));
	if (State & GRPCAP_PERSISTENT_RECONNECT)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("                             [5] bPersistentReconnect\n"));
	if (State & GRPCAP_GROUP_FORMING)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("                             [6] bGroupFormation\n"));
}
	...
	IN UCHAR	macindex)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" ====================================================>P2PPrintMac i = %d,   \n", macindex));
	pEntry = &pAd->MacTab.Content[macindex];
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!! ,  HTMode = %x, WepStatus = %d, AuthMode = %d. \n", pEntry->HTPhyMode.word, pEntry->WepStatus, pEntry->AuthMode));

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ValidAsCLI = %d. ClientStatusFlags = %lx, \n", IS_ENTRY_CLIENT(pEntry), pEntry->ClientStatusFlags));
...
VOID P2PPrintP2PEntry(
	...
	IN UCHAR		p2pindex)
{
	RT_P2P_CLIENT_ENTRY *pClient = NULL;

	if (p2pindex >= MAX_P2P_GROUP_SIZE)
		return;
	
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:: p2p Entry idx = %d\n", __FUNCTION__, p2pindex));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("====================================================>\n"));
	pClient = &pAd->P2pTable.Client[p2pindex];
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("P2pClientState = %s  \n", decodeP2PClientState(pClient->P2pClientState)));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ConfigMethod = %x.    %s \n", pClient->ConfigMethod, decodeConfigMethod(pClient->ConfigMethod)));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Opchannel = %d.    Listenchannel = %d.\n", pClient->OpChannel, pClient->ListenChannel));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MyGOIndex = %d. rule = %s.\n", pClient->MyGOIndex, decodeMyRule(pClient->Rule)));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("addr = %02x:%02x:%02x:%02x:%02x:%02x\n", PRINT_MAC(pClient->addr)));
...
/*	BOOLEAN		bDot1xReload = FALSE; */
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> P2P_GoStartUp\n"));
...
#endif /* !DPA_S */
	pAd->ApCfg.MBSSID[0].wdev.UapsdInfo.bAPSDCapable = TRUE;
	...
	// We set rx filter after P2P_GO_ON() is configured!
	AsicSetRxFilter(pAd);
	...
	if (WMODE_CAP_N(pAd->CommonCfg.PhyMode))
	{
		if (pAd->CommonCfg.Channel > 14)
			pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.PhyMode = (WMODE_A | WMODE_AN);
		else
			pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.PhyMode = (WMODE_B | WMODE_G | WMODE_GN);
	}
	else
	{
		if (pAd->CommonCfg.Channel > 14)
			pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.PhyMode = WMODE_A;
		else
			pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.PhyMode = (WMODE_B | WMODE_G);
	}
...

	{
		...
		pMbss->wdev.AuthMode = Ndis802_11AuthModeWPA2PSK;
 		pMbss->wdev.WepStatus = Ndis802_11AESEnable;
		pMbss->WscSecurityMode = WPA2PSKAES;
		pMbss->wdev.GroupKeyWepStatus = pMbss->wdev.WepStatus;
		pMbss->CapabilityInfo =
			CAP_GENERATE(1, 0, (pMbss->wdev.WepStatus != Ndis802_11EncryptionDisabled), TxPreamble, pAd->CommonCfg.bUseShortSlotTime, SpectrumMgmt);
		...
		if (pMbss->wdev.WepStatus == Ndis802_11TKIPAESMix)
		{
			switch ((UCHAR)pMbss->wdev.AuthMode)
			{
				/* WPA mode */
				case Ndis802_11AuthModeWPA:
				case Ndis802_11AuthModeWPAPSK:
					pMbss->wdev.WpaMixPairCipher = WPA_TKIPAES_WPA2_NONE;
					break;	

				/* WPA2 mode */
				case Ndis802_11AuthModeWPA2:
				case Ndis802_11AuthModeWPA2PSK:
					pMbss->wdev.WpaMixPairCipher = WPA_NONE_WPA2_TKIPAES;
					break;
				...
				case Ndis802_11AuthModeWPA1PSKWPA2PSK:	
					...
					if (pMbss->wdev.WpaMixPairCipher == MIX_CIPHER_NOTUSE || 
						pMbss->wdev.WpaMixPairCipher == WPA_TKIPAES_WPA2_NONE || 
						pMbss->wdev.WpaMixPairCipher == WPA_NONE_WPA2_TKIPAES)
						pMbss->wdev.WpaMixPairCipher = WPA_TKIPAES_WPA2_TKIPAES;										
					break;				
			}
											
		}
		else
		pMbss->wdev.WpaMixPairCipher = MIX_CIPHER_NOTUSE;

		/* Generate the corresponding RSNIE */
		RTMPMakeRSNIE(pAd, pMbss->wdev.AuthMode, pMbss->wdev.WepStatus, MAIN_MBSSID + MIN_NET_DEVICE_FOR_P2P_GO);
		...
		if (pMbss->WscControl.WscV2Info.bEnableWpsV2)
		{
			...
			*/
			if ((pMbss->wdev.WepStatus == Ndis802_11WEPEnabled) || (pMbss->wdev.WepStatus == Ndis802_11TKIPEnable))
	...
	AsicSetTxStream(pAd, pAd->Antenna.field.TxPath, OPMODE_AP, TRUE);
	...
	pAd->MacTab.Content[MCAST_WCID].Addr[0] = 0x01;
	pAd->MacTab.Content[MCAST_WCID].HTPhyMode.field.MODE = MODE_OFDM;
	pAd->MacTab.Content[MCAST_WCID].HTPhyMode.field.MCS = 3;
	/*pAd->CommonCfg.CentralChannel = pAd->CommonCfg.Channel;*/
	...
	*/
...

	{
		...
		pMbss->wdev.PortSecured = WPA_802_1X_PORT_NOT_SECURED;

		if (IS_WPA_CAPABILITY(pMbss->wdev.AuthMode))
		{   
			pMbss->wdev.DefaultKeyId = 1;
		}
		...
		if (pMbss->wdev.WepStatus == Ndis802_11WEPEnabled)
		{
    			UCHAR	CipherAlg;
			UCHAR	idx_len;
			...
			for (idx=0; idx < SHARE_KEY_NUM; idx++)
			{
				...
				if (pAd->SharedKey[MAIN_MBSSID + 1][idx].KeyLen > 0)
				{
					...
					if (idx == pMbss->wdev.DefaultKeyId)
					{
		...
		else if ((pMbss->wdev.WepStatus == Ndis802_11TKIPEnable) ||
				 (pMbss->wdev.WepStatus == Ndis802_11TKIPEnable) ||
				 (pMbss->wdev.WepStatus == Ndis802_11TKIPAESMix))
		{
			/* Generate GMK and GNonce randomly per MBSS */
			GenRandom(pAd, pMbss->wdev.bssid, pMbss->GMK);
			GenRandom(pAd, pMbss->wdev.bssid, pMbss->GNonce);		
			...
			WPAInstallSharedKey(pAd, 
								pMbss->wdev.GroupKeyWepStatus, 
								MAIN_MBSSID + 1, 
								pMbss->wdev.DefaultKeyId, 
								Wcid,
								TRUE,
								pMbss->GTK,
								LEN_TKIP_GTK);
		...
		if ((pMbss->wdev.AuthMode == Ndis802_11AuthModeWPA) || (pMbss->wdev.AuthMode == Ndis802_11AuthModeWPA2)
        		|| (pMbss->wdev.AuthMode == Ndis802_11AuthModeWPA1WPA2) || (pMbss->wdev.IEEE8021X == TRUE))
		{
			;/*bDot1xReload = TRUE; */
    	}
#endif /* DOT1X_SUPPORT */

		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("### BSS(%d) AuthMode(%d)=%s, WepStatus(%d)=%s , AccessControlList.Policy=%ld\n", MAIN_MBSSID + 1, pMbss->wdev.AuthMode, GetAuthMode(pMbss->wdev.AuthMode), 
																  pMbss->wdev.WepStatus, GetEncryptType(pMbss->wdev.WepStatus), pMbss->AccessControlList.Policy));
...
*/
	...
	if (pAd->ApCfg.IdsEnable)
	{
		...
		if (pAd->CommonCfg.bWirelessEvent == FALSE)
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("!!! WARNING !!! The WirelessEvent parameter doesn't be enabled \n"));
	  ...
	  */
	{
		...
		for(num_idx=0; num_idx < pAd->CommonCfg.NumOfBulkInIRP; num_idx++)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RTUSBBulkReceive!\n" ));
		}
	}
#endif /* RTMP_MAC_USB */
	...
	IN PRTMP_ADAPTER pAd) 
{
	...
	INT			apidx;
	PRT_P2P_CONFIG  pP2PCtrl = &pAd->P2pCfg;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> P2P_GoStop\n"));
...
#ifdef WIDI_SUPPORT	
	if (pAd->StaCfg.bAutoReconnect == FALSE)
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	for (i = FirstWcid; i < MAX_LEN_OF_MAC_TABLE; i++) 
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%02x:%02x:%02x:%02x:%02x:%02x fail to complete ASSOC in %d sec\n",
...
/*		if (ACMP_IsAllACEnabled(pAd) == ACM_RTN_FAIL) */
		...
		/* detect the station alive status */
			...
			pAd->ApCfg.MBSSID[pEntry->func_tb_idx].StationKeepAliveTime = 3;
		if ((pAd->ApCfg.MBSSID[pEntry->func_tb_idx].StationKeepAliveTime > 0) &&
			(pEntry->NoDataIdleCount >= pAd->ApCfg.MBSSID[pEntry->func_tb_idx].StationKeepAliveTime))
		{
		...
		*/
		...
		{
			bDisconnectSta = TRUE;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("ageout %02x:%02x:%02x:%02x:%02x:%02x after %d-sec silence\n",
		...
		else if (pEntry->ContinueTxFailCnt >= pAd->ApCfg.EntryLifeCheck)
		{
			...
			*/
			...
			{
				bDisconnectSta = TRUE;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("STA-%02x:%02x:%02x:%02x:%02x:%02x had left\n",
					pEntry->Addr[0],pEntry->Addr[1],pEntry->Addr[2],pEntry->Addr[3],
		...
		if (bDisconnectSta)
		{
			...
			if (pEntry->Sst == SST_ASSOC)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("Send DEAUTH - Reason = %d frame  TO %02x:%02x:%02x:%02x:%02x:%02x. \n",
				...
				MgtMacHeaderInit(pAd, &DisassocHdr, SUBTYPE_DISASSOC, 0, pEntry->Addr, pAd->ApCfg.MBSSID[pEntry->func_tb_idx].Bssid);
	...
	IN PUCHAR pBssid) 
{
	...
	if (SubType == SUBTYPE_ACK)	/* sample, no use, it will conflict with ACTION frame sub type */
		pHdr80211->FC.Type = FC_TYPE_CNTL;
#endif
	...
	IN  PRTMP_ADAPTER  pAd)
{
...
#ifdef CONFIG_AP_SUPPORT
	{
		...
		RTMP_IRQ_LOCK(&pAd->irq_lock, IrqFlags);
#endif /* RTMP_MAC_PCI */
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("McastPsQueue.Number %ld...\n",pAd->MacTab.McastPsQueue.Number));
		if (pAd->MacTab.McastPsQueue.Number > 0)
			APCleanupPsQueue(pAd, &pAd->MacTab.McastPsQueue);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("2McastPsQueue.Number %ld...\n",pAd->MacTab.McastPsQueue.Number));
	...
	IN UCHAR		ifIndex)
{
	if(!OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED))
	{
		...
		RootApHtInfo = pAd->ApCfg.ApCliTab[0].MlmeAux.AddHtInfo.AddHtInfo;
			...
			pAd->CommonCfg.RegTransmitSetting.field.BW == BW_40)
		{
			N_SetCenCh(pAd, pAd->CommonCfg.Channel);
			SetCommonHT(pAd);
			AsicSwitchChannel(pAd, pAd->ApCfg.ApCliTab[0].MlmeAux.CentralChannel, FALSE);
			AsicLockChannel(pAd, pAd->ApCfg.ApCliTab[0].MlmeAux.CentralChannel);
	...
	IN	PRTMP_ADAPTER pAd)
{
	...
	pAd->ApCfg.MBSSID[apidx].wdev.GroupKeyWepStatus = Ndis802_11EncryptionDisabled;
	pAd->ApCfg.MBSSID[apidx].wdev.DefaultKeyId = 0;
	pAd->ApCfg.MBSSID[apidx].wdev.WpaMixPairCipher = MIX_CIPHER_NOTUSE;

#ifdef DOT1X_SUPPORT
	pAd->ApCfg.MBSSID[apidx].wdev.IEEE8021X = FALSE;
	...
	/* VLAN related */
	pAd->ApCfg.MBSSID[apidx].wdev.VLAN_VID = 0;

	/* Default MCS as AUTO */
	pAd->ApCfg.MBSSID[apidx].wdev.bAutoTxRateSwitch = TRUE;
	pAd->ApCfg.MBSSID[apidx].wdev.DesiredTransmitSetting.field.MCS = MCS_AUTO;
	...
	{
		pAd->ApCfg.ApCliTab[apcliIdx].wdev.AuthMode = Ndis802_11AuthModeOpen;
		pAd->ApCfg.ApCliTab[apcliIdx].wdev.WepStatus = Ndis802_11WEPDisabled;
	}
#endif /* APCLI_SUPPORT */
#endif /* P2P_SUPPORT */
	...
	pAd->ApCfg.RssiSample.AvgRssi[0] = -127;
	pAd->ApCfg.RssiSample.AvgRssi[1] = -127;
	pAd->ApCfg.RssiSample.AvgRssi[2] = -127;
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	pAd->CommonCfg.Channel = pAd->ApCfg.ApCliTab[0].MlmeAux.Channel;
#ifdef DOT11_N_SUPPORT
	/* Change to AP channel */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("CentralChannel = %d, Channel %d, BW = %d\n",
			pAd->CommonCfg.CentralChannel, pAd->CommonCfg.Channel,
			pAd->ApCfg.ApCliTab[0].MlmeAux.HtCapability.HtCapInfo.ChannelWidth));

	if ( (pAd->CommonCfg.CentralChannel > pAd->CommonCfg.Channel) &&
		(pAd->ApCfg.ApCliTab[0].MlmeAux.HtCapability.HtCapInfo.ChannelWidth == BW_40))
	{
		rf_channel = pAd->CommonCfg.CentralChannel;
		rf_bw = BW_40;
		ext_ch = EXTCHA_ABOVE;
	...
	IN PRTMP_ADAPTER	pAd)
{
...
#endif /* DOT11_N_SUPPORT */
			if ((pAd->ApCfg.ApCliTab[0].MlmeAux.SupRateLen == 4) &&
				(pAd->ApCfg.ApCliTab[0].MlmeAux.ExtRateLen == 0))
				/* B only AP */
				ProperMlmeRate = RATE_11;
			else
				ProperMlmeRate = RATE_24;
			
			if (pAd->ApCfg.ApCliTab[0].MlmeAux.Channel <= 14)
			MinimumRate = RATE_1;
			else
				MinimumRate = RATE_6;
			break;
	...
	for (i = 0; i < pAd->ApCfg.ApCliTab[0].MlmeAux.SupRateLen; i++)
	{
		for (j = 0; j < RateIdx; j++)
		{
			if ((pAd->ApCfg.ApCliTab[0].MlmeAux.SupRate[i] & 0x7f) == RateIdTo500Kbps[j])
			{
				if (j == ProperMlmeRate)
				{
					bMatch = TRUE;
	...
	if (bMatch == FALSE)
	{
		for (i = 0; i < pAd->ApCfg.ApCliTab[0].MlmeAux.ExtRateLen; i++)
		{
			for (j = 0; j < RateIdx; j++)
			{
				if ((pAd->ApCfg.ApCliTab[0].MlmeAux.ExtRate[i] & 0x7f) == RateIdTo500Kbps[j])
				{
					if (j == ProperMlmeRate)
					{
						bMatch = TRUE;
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RTMPUpdateMlmeRate ==>   MlmeTransmit = 0x%x  \n" , pAd->CommonCfg.MlmeTransmit.word));
}
	...
	IN  PRTMP_ADAPTER   pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("---> P2PAPInitialize\n"));
...
VOID P2PAPShutdown(
	IN PRTMP_ADAPTER pAd)
{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("---> P2PAPShutdown\n"));
	...
	IN	PRTMP_ADAPTER pAd)
{
	...
	{
		pAd->ApCfg.MBSSID[j].wdev.AuthMode = Ndis802_11AuthModeOpen;
		pAd->ApCfg.MBSSID[j].wdev.WepStatus = Ndis802_11EncryptionDisabled;
		pAd->ApCfg.MBSSID[j].wdev.GroupKeyWepStatus = Ndis802_11EncryptionDisabled;
		pAd->ApCfg.MBSSID[j].wdev.DefaultKeyId = 0;
		pAd->ApCfg.MBSSID[j].wdev.WpaMixPairCipher = MIX_CIPHER_NOTUSE;

#ifdef DOT1X_SUPPORT
		pAd->ApCfg.MBSSID[j].wdev.IEEE8021X = FALSE;
		...
		/* VLAN related */
		pAd->ApCfg.MBSSID[j].wdev.VLAN_VID = 0;

		/* Default MCS as AUTO */
		pAd->ApCfg.MBSSID[j].wdev.bAutoTxRateSwitch = TRUE;
		pAd->ApCfg.MBSSID[j].wdev.DesiredTransmitSetting.field.MCS = MCS_AUTO;
	...
	{
		pAd->ApCfg.ApCliTab[j].wdev.AuthMode = Ndis802_11AuthModeOpen;
		pAd->ApCfg.ApCliTab[j].wdev.WepStatus = Ndis802_11WEPDisabled;
		pAd->ApCfg.ApCliTab[j].wdev.bAutoTxRateSwitch = TRUE;
		pAd->ApCfg.ApCliTab[j].wdev.DesiredTransmitSetting.field.MCS = MCS_AUTO;
	}
...
//iversonnote
		...
		dev_name = get_dev_name_prefix(pAd, INT_P2P);
#endif
		new_dev_p = RtmpOSNetDevCreate(MC_RowID, &IoctlIF, INT_P2P, MAIN_MBSSID, sizeof(struct mt_dev_priv), INF_P2P_DEV_NAME);

		if (new_dev_p == NULL)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Allocate network device fail (MBSS)...\n"));
			return;
		}
		else
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Register P2P IF (%s)\n", RTMP_OS_NETDEV_GET_DEVNAME(new_dev_p)));
		}
...
//iversonnote
		...
		*/
		...
		COPY_MAC_ADDR(pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.bssid, pAd->P2PCurrentAddress);

		pApCliEntry->wdev.if_dev= new_dev_p;
		COPY_MAC_ADDR(pApCliEntry->wdev.if_addr, pAd->P2PCurrentAddress);
	...
	P2pGetRandomSSID(pAd, pAd->ApCfg.MBSSID[MAIN_MBSSID].Ssid, &(pAd->ApCfg.MBSSID[MAIN_MBSSID].SsidLen));
	pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.AuthMode = Ndis802_11AuthModeOpen;
	pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.WepStatus = Ndis802_11EncryptionDisabled;
	pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.DesiredTransmitSetting.field.MCS = pAd->StaCfg.wdev.DesiredTransmitSetting.field.MCS;

	if ((pAd->CommonCfg.bWmmCapable) || WMODE_CAP_N(pAd->CommonCfg.PhyMode))
		pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.bWmmCapable = TRUE;

	/*OPSTATUS_CLEAR_FLAG(pAd, fOP_STATUS_P2P_GO); */
	/*OPSTATUS_CLEAR_FLAG(pAd, fOP_STATUS_P2P_CLI); */

	pAd->flg_p2p_init = TRUE;
	pAd->ApCfg.ApCliTab[MAIN_MBSSID].wdev.AuthMode = Ndis802_11AuthModeOpen;
	pAd->ApCfg.ApCliTab[MAIN_MBSSID].wdev.WepStatus = Ndis802_11WEPDisabled;
	pAd->ApCfg.ApCliTab[MAIN_MBSSID].wdev.DesiredTransmitSetting.field.MCS = pAd->StaCfg.wdev.DesiredTransmitSetting.field.MCS;
	...
	IN	PNET_DEV					pDev)
{
	...
	COPY_MAC_ADDR(pMbss->wdev.bssid, pAd->CurrentAddress);
	...
	*/
	if (pAd->chipCap.MBSSIDMode == MBSSID_MODE1)
	pMbss->wdev.bssid[MAIN_MBSSID] += 2; 	
	else
	{
#ifdef P2P_ODD_MAC_ADJUST
		if ((pMbss->wdev.bssid[5] & 0x01) == 0x01)
			pMbss->wdev.bssid[5] -= 1;
		else
#endif /* P2P_ODD_MAC_ADJUST */
	pMbss->wdev.bssid[5] += FIRST_MBSSID;
	}
	...
	IN	RTMP_NET_PACKET_TRANSMIT	Func)
{
	...
	ASSERT(pAd);
		...
		if (P2P_CLI_ON(pAd))
		{
			...
			if (pApCli[MAIN_MBSSID].wdev.if_dev== pDev)
			{
				/* ya! find it */
...
#ifdef RELEASE_EXCLUDE
				if (!(*(GET_OS_PKT_DATAPTR(RTPKT_TO_OSPKT(pPktSrc))) & 0x01))
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s - unicast packet to (apcli%d)\n", 
...
//				RTMP_SET_PACKET_SOURCE(pPktSrc, PKTSRC_NDIS);
				...
				if (!(*(GET_OS_PKT_DATAPTR(RTPKT_TO_OSPKT(pPktSrc))) & 0x01))
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s - unicast packet to (p2p%d)\n", 
								__FUNCTION__, MAIN_MBSSID));
				}
#endif /* RELEASE_EXCLUDE */
