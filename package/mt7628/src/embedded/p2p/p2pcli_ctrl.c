	...
	IN MLME_QUEUE_ELEM *Elem)
{
 ...
 && (pAd->Multi_Channel_Enable == FALSE)
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s :: Channel offset = %d not match INFRA Channel offset %d .\n",
					...
					if (pAd->Multi_Channel_Enable == TRUE)
					{
								...
								&&(pAd->CommonCfg.Channel == pAd->LatchRfRegs.Channel) )
							{
								...
								MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ApCliCtrlJoinReqAction -- Send PSM Data frame\n"));
								OS_WAIT(200);
							}
		...
		ULONG bss_idx = 0;
			...
			if (pAd->Multi_Channel_Enable == TRUE)
			{
					...
					bss_idx = BssSsidTableSearch(&pAd->ScanTab, JoinReq.Bssid, (PCHAR)(JoinReq.Ssid), JoinReq.SsidLen, pAd->CommonCfg.Channel);
						if (bss_idx == BSS_NOT_FOUND)
						{
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("2 Can't find %s !\n",(PCHAR)pApCliEntry->CfgSsid));
							*pCurrState = APCLI_CTRL_PROBE;
							ApSiteSurvey(pAd, NULL, SCAN_WSC_ACTIVE, FALSE);
							return;
						}
						else
						{
								...
								&&(pAd->CommonCfg.Channel == pAd->LatchRfRegs.Channel) )
							{
								...
								MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ApCliCtrlJoinReqAction !bWscTrigger -- Send PSM Data frame\n"));
									OS_WAIT(200);
							}
...
/* Bug fix: Sometimes p2p connection will send probe request with incorrect channel. */
	...
	if (pApCliEntry->CfgSsidLen == 0)
	{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::  ApCliEntry SSID Len is 0 !!!.\n", __FUNCTION__));
			*pCurrState = APCLI_CTRL_PROBE;
			ApSiteSurvey(pAd, NULL, SCAN_WSC_ACTIVE, FALSE);
			return;
	}
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	*pCurrState = APCLI_CTRL_PROBE;
		...
		(pAd->ApCfg.ApCliTab[ifIndex].WscControl.bWscTrigger == TRUE))
    {
		...
		if (bss_idx == BSS_NOT_FOUND)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::  WSC exec. start SiteSurvey!\n", __FUNCTION__));
			*pCurrState = APCLI_CTRL_PROBE;
			ApSiteSurvey(pAd, NULL, SCAN_WSC_ACTIVE, FALSE);
			return;
		}
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("(%s) Probe respond success.\n", __FUNCTION__));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("(%s) Apcli-Interface Ssid=%s.\n", __FUNCTION__, pApCliEntry->Ssid));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("(%s) Apcli-Interface Bssid=%02x:%02x:%02x:%02x:%02x:%02x.\n", __FUNCTION__,
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("(%s) Sta Auth Rsp Failure.\n", __FUNCTION__));
