...
INT32 MCUSysInit(RTMP_ADAPTER *pAd)
{
	...
	// TODO: shiang-MT7615
	if (IS_MT7615(pAd)) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): MT7615 NotReadyYet!\n", __FUNCTION__));
	...
	if (pAd->chipCap.hif_type == HIF_MT) {
		...
		MtAsicDMASchedulerInit(pAd, DMA_SCH_LMAC);
...
VOID ChipOpsMCUHook(PRTMP_ADAPTER pAd, enum MCU_TYPE MCUType)
{
	...
	if (MCUType == ANDES) {
		...
		if (pAd->chipCap.hif_type == HIF_MT) {
			...
			if (pChipCap->need_load_fw) {
#ifdef MT7615
				pChipOps->pci_kick_out_cmd_msg = AndesMTPciKickOutCmdMsg2;
#else
