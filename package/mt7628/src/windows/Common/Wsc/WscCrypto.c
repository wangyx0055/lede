/* 
 * ---------------------------------------------------------------------------
 * 1. SHA2/HMAC-SHA2 codes are based on --- Copyright (c) 2002, Dr Brian 
 *  Gladman, Worcester, UK. All rights reserved.
 *  
 * 2. AES-CBC-128 code is based on OpenSSL.
 *
 * 3. DH-Key numbers routines are based on Natural numbers routines ---
...
/* SHA2 initialisation */
int_ret sha2_begin(unsigned long len, sha2_ctx ctx[1])
{
    switch(len)
    {
#if defined(SHA_256)
...
#if defined(SHA_256)
        case 32: 
            sha256_hash(data, len, CTX_256(ctx)); return;
#endif
    }
}
...
#if defined(SHA_256)
        case 32: 
            sha_end1(hval, CTX_256(ctx), SHA256_DIGEST_SIZE); return;
#endif
    }
}

int_ret sha2(unsigned char hval[], unsigned long size,
...
//WAPI used
//
void KD_hmac_sha256(const unsigned char key[], unsigned long key_len,
                        const unsigned char data[], unsigned long data_len,
                        unsigned char output[], unsigned long output_len)
{
    ...
    {
        hmac_sha(key, key_len, data, data_len, &output[i*SHA256_DIGEST_SIZE], SHA256_DIGEST_SIZE);
        data = &output[i*SHA256_DIGEST_SIZE];
        data_len = SHA256_DIGEST_SIZE;
    }
    if (output_len>0)
        hmac_sha(key, key_len, data, data_len, &output[i*SHA256_DIGEST_SIZE], output_len);      
}

// ************************************************************************
//  AES-128-CBC functionality
// ************************************************************************
...
int EVP_EncryptUpdate(EVP_CIPHER_CTX *ctx, unsigned char *outbuf, int *outlen, unsigned char *inbuf, int inlen)
{
    ...
    {
        AES_cbc_encrypt(inbuf + encryptSize, outbuf + encryptSize, AES_BLOCK_SIZE, &ctx->aesKey, ctx->iv, ctx->encrypt);
...
int EVP_EncryptFinal(EVP_CIPHER_CTX *ctx, unsigned char *outbuf, int *outlen)
{
    ...
    *outlen = AES_BLOCK_SIZE;
    AES_cbc_encrypt(ctx->buffer, outbuf, AES_BLOCK_SIZE, &ctx->aesKey, ctx->iv, ctx->encrypt);
...
int EVP_DecryptUpdate(EVP_CIPHER_CTX *ctx, unsigned char *outbuf, int *outlen, unsigned char *inbuf, int inlen)
{
    ...
    while (inlen >= AES_BLOCK_SIZE)
    {
        ...
        AES_cbc_encrypt(inbuf + decryptSize, outbuf + decryptSize, AES_BLOCK_SIZE, &ctx->aesKey, ctx->iv, ctx->encrypt);
    ...
    {
        ctx->bufferlen = inlen;
        AES_cbc_encrypt(inbuf + decryptSize, ctx->buffer, AES_BLOCK_SIZE, &ctx->aesKey, ctx->iv, ctx->encrypt);
    }
    *outlen = decryptSize;
    ...
    unsigned char *cipherText,  int *ctx_len)
{
    ...
    PlatformAllocateMemory(pAd, &outBuf, 1024);
    if (outBuf == NULL)
    {
        DBGPRINT(RT_DEBUG_ERROR, ("WscEncryptData::Allocate memory size(=1024) failed\n"));
        return;
    }
    ...
    {
        DBGPRINT(RT_DEBUG_TRACE, ("WscEncryptData: EncryptInit failed\n"));
    }
    ...
    *ctx_len = 0;
    while(data_len)
    {
        ...
        {
            DBGPRINT(RT_DEBUG_TRACE, ("WscEncryptData: EncryptUpdate failed\n")); 
        }
    ...
    {
        DBGPRINT(RT_DEBUG_TRACE, ("WscEncryptData: EncryptFinal failed\n"));
    }
    ...
    *ctx_len += outLen;
    ...
    if (outBuf)
        PlatformFreeMemory(outBuf, 1024);
}
    ...
    unsigned char *plainText,   int *ptx_len)
{
    ...
    if (outBuf == NULL)
    {
        DBGPRINT(RT_DEBUG_ERROR, ("WscDecryptData::Allocate memory size(=1024) failed\n"));
        return;
    }
    ...
    {
        DBGPRINT(RT_DEBUG_TRACE, ("WscEncryptData: DecryptInit failed\n"));
    }
    ...
    *ptx_len=0;
    while(data_len)
    {
        ...
        {
            DBGPRINT(RT_DEBUG_TRACE, ("WscDecryptData: DecryptUpdate failed\n"));
        }
    ...
    {
        DBGPRINT(RT_DEBUG_TRACE, ("WscDecryptData: DecryptFinal failed\n"));
    }
    ...
    unsigned char *key, unsigned int keyBits )
{
    ...
    DBGPRINT(RT_DEBUG_TRACE, ("WscDeriveKey: Deriving a key of %d bits\n", keyBits));           
    ...
    for(i = 0; i < iterations; i++)
    {
        ...
        memcpy(input,&temp,4);
        hmac_sha(kdk, kdk_len, input, 4+str_len+4, hmac,  32);
    ...
    {
        DBGPRINT(RT_DEBUG_TRACE, ("WscDeriveKey: Key derivation generated less bits than asked\n"));
        return 1; //failed
    }

#pragma prefast(suppress: __WARNING_USING_UNINIT_VAR, "output variable already be initialized above")    
...
// ************************************************************************
//  DH Key functionality
// ************************************************************************

/***********  THE DIFFIE-HELLMAN KEY EXCHANGE PROTOCOL   ***********
 *
 * Wifi-Simple-Config :
 * The 1536 bit MODP group used by Simple Config is taken from RFC 3526.
 * The prime(p) is: 2^1536 - 2^1472 - 1 + 2^64 * { [2^1406 pi] + 741804 }
...
static unsigned char DH_P_VALUE[MODP_1536_LEN] =
{
...
static unsigned char DH_G_VALUE[] = {0x00, 0x00, 0x00, 0x02};


/* GenerateDHPublicKey:
 * @ran_buf: pointer to the random byte input
 * @ran_len: the length of the random byte
 * @dhkey: pointer to the dhkey output (send to the peer)
 * @dhkey_len: the length of the dhkey in bytes
 *
 * dhkey= [g^(random value) mod p]
 */
void GenerateDHPublicKey(unsigned char *ran_buf, int ran_len, 
                        unsigned char *dhkey, int *dhkey_len)
{

    NN_DIGIT m[MAX_NN_DIGITS], e[MAX_NN_DIGITS], g[MAX_NN_DIGITS], dh[MAX_NN_DIGITS];
    unsigned long eDigits, mDigits;


    //convert octet string to integer
    NN_Decode(g, MAX_NN_DIGITS, DH_G_VALUE, sizeof(DH_G_VALUE)/sizeof(unsigned char));    // generator
    NN_Decode(m, MAX_NN_DIGITS, DH_P_VALUE, MODP_1536_LEN);     // modulus: prime
    NN_Decode(e, MAX_NN_DIGITS, ran_buf, ran_len);              // exponent

    // length of (n, e) in digits
    eDigits = NN_Digits(e, MAX_NN_DIGITS); 
    mDigits = NN_Digits(m, MAX_NN_DIGITS);


    //
    // computation, dhkey= [g^(random value) mod p].
    //
    NN_ModExp(dh, g, e, eDigits, m, mDigits);


    // convert integrt to octet string
    *dhkey_len = DH_KEY_LEN;
    NN_Encode(dhkey, *dhkey_len, dh, mDigits);

}

/* GenerateDHSecreteKey:
 * @ran_buf: pointer to the random byte input
 * @ran_len: the length of the random byte
 * @peer_dhkey: pointer to the peer dhkey input 
 * @peer_dhkey_len: the length of the peer dhkey in bytes
 * @secrete_dhkey: pointer to the local dhkey output (for KDK)
 * @secrete_dhkey_len: the length of the local dhkey in bytes
 *
 * secrete_dhkey= (peer_dhkey)^(random value) mod p
 */
                        ...
                        unsigned char *secrete_dhkey, int *secrete_dhkey_len)
{
    ...
    //convert octet string to integer   
    NN_Decode(g, MAX_NN_DIGITS, peer_dhkey, peer_dhkey_len);    // generator
    NN_Decode(m, MAX_NN_DIGITS, DH_P_VALUE, MODP_1536_LEN);     // modulus: prime
    NN_Decode(e, MAX_NN_DIGITS, ran_buf, ran_len);              // exponent
    ...
    //
    // computation, secrete_dhkey= (peer_dhkey)^(random value) mod p.
    //
    NN_ModExp(dh, g, e, eDigits, m, mDigits);


    // convert integrt to octet string
    *secrete_dhkey_len = DH_KEY_LEN;
    NN_Encode(secrete_dhkey, *secrete_dhkey_len, dh, mDigits);
}
...
void NN_ModExp (NN_DIGIT *a, NN_DIGIT *b, NN_DIGIT *c, unsigned int cDigits, NN_DIGIT *d, unsigned int dDigits)
{
     ...
     */
#pragma prefast(suppress: __WARNING_USING_UNINIT_VAR, "bPower will be initialized by NN_Assign(..)")
