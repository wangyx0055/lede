...
copy %SRC_ROOT%\Include\QATool_Support %SRC_ROOT%\Include\QAToolDef.h
...
IF "%DDK_TARGET_OS%"=="Win7" (
        IF "%WindowsSdkDir%"==""    (
        build -cZgw ) ELSE (
        msbuild /p:Configuration=%MSBUILD_CONFIGURATION% /p:Platform="%MSBUILD_PLATFORM%" /flp1:warningsonly;logfile=msbuild.wrn /flp2:errorsonly;logfile=msbuild.err MsBuild_Win_MtkUsb.sln
        )
        goto COPYFILE7
) ELSE (
    msbuild /p:Configuration=%MSBUILD_CONFIGURATION% /p:Platform="%MSBUILD_PLATFORM%" /flp1:warningsonly;logfile=msbuild.wrn /flp2:errorsonly;logfile=msbuild.err MsBuild_Win_MtkUsb.sln
...
if "%CPU_X%"=="x64" (
copy PLATFORM\INF\USB\mtkwlux_win7.inf %RELEASE_FOLDER%\%CPU_X%  
...
if "%CPU_X%"=="x64" (
copy PLATFORM\INF\USB\mtkwlux.inf %RELEASE_FOLDER%\%CPU_X%  
copy PLATFORM\INF\RaCoInstx.dll         %RELEASE_FOLDER%\%CPU_X%\RaCoInstx.dll
copy PLATFORM\Ndis6\USB\%CPU_X%\*.sys %RELEASE_FOLDER%\%CPU_X%  
copy PLATFORM\Ndis6\USB\%CPU_X%\*.pdb %Symbol%\%CPU_X%
