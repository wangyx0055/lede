...
/*
	TXINFO format of normal packet.
	...
	info_type[b31~b30]:
		...
		others: reseved
	d_port[b29~b27]:
		the packet will be forwarded through which FCE port
		000: WLAN port
		001: CPU Rx port
		010: CPU Tx port
		011: Host port (PCIe)
		100: Virtual CPU Rx port
		101: Virtual CPU Tx port
		110: discard
	QSEL[b26~b25]:
		packet buffer Q selection
			Dest. in PBF	Function	TxPriority
		00: Tx0Q			mgmt 	High
		01: Tx1Q			HCCA	Medium
		10: Tx2Q			EDCA	Lowest for Channel 1
		11: Tx3Q			EDCA	Lowest for Channel 2
	wiv[b24]:
		with IV
	rsv1[b23~b22]:
		Reserved
	cso[b21]:
		Checksum offload. Inform if this packet needs to do checksum.
	tso[b20]:
		TCP segmentation offload. inform if needs to do TCP segmentation.
	pkt_80211[b19]:
		0: this is a 802.3 packet
		1: this is a 802.11 packet
	rsv0[b18]:
		Reserved bit
	tx_burst[b17]:
		force DMA transmit frame from current selected endpoint
	next_vld[b16]:
		host driver info DMA current frame is not the last one in current TxQ.
	pkt_len[b15~b0]:
		specify the frame length in unit of byte, it includes:
			WI, 802.11 header, and payload, but TXINFO is not included.
*/
...
#ifdef RELEASE_EXCLUDE
/*
	TXINFO format of command packet.
...
/*
	...
	FRAG: to inform TKIP engine this is fragement, so that TKIP MIC is appended
			by driver at the last fragment; hardware TKIP engine only need to 
			insert IV/EIV and ICV
	MIMOps: the remote peer is in dynamic MIMO-PS mode
	CFACK: 
	TS:
	AMPDU:
	MpduDensity:
	txop: for txop mode
		...
		2: Backoff
		3: SIFIS
			if 0, 	txop for the MPDU frame will be handles by ASIC refer to the registers
			if 1/2/3, send the MPDU frame by following rule when previous frame exchange is successful
	...
	iTxBF:
	PHYMODE:
		0: CCK, 1: OFDM, 2: HTMIX, 3: HT-GF, 4: VHT 
*/
#endif /* RELEASE_EXCLUDE */
#ifdef RT_BIG_ENDIAN
...
/* 
	...
	WIFI INFO for RX translation
	MT7650 series:
		RXWI | WIFI_INFO | 802.3 Mac Header | IP Payload|
			Note: a).802.3 Mac header is 4-byte aligned
				  b).IP payload is 2-byte aligned
		
	MT7601 series:
		RXWI | WIFI_INFO | 2-byte-PAD | 802.3 Mac Header | IP Payload|
			Note: a).802.3 Mac header is 2-byte aligned
				  b).IP payload is 4-byte aligned
*/
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
/*
	RX FCE INFO format for packet to Host CPU
		info_type[b31~b30]:
			00: packet
			01: command
			10/11: reserved
		s_port[b29~b27]: The frame is routing from s_port
			000: WLAN port
			001: CPU RX port
			010: CPU TX port
			011: Host port(PCI-E)
			...
			101: Virtual CPU TX port
			110: discard
		qsel[b26~b25]: The frame is routing to D-Port Queue 0~3

		pcie_intr[b24]: embedded CPU parse L4 pkt and generate 
				PCI-E intrrupt per packet base to do D0-offload
		mac_len[b23~b21]:
		l3l4_done[b20]:
		pkt_80211[b19]: set 1 indicate its a 802.11 frame, else set as 0 for 802.3 frame
		ip_err[b18]:
		tcp_err[b17]:
		udp_err[b16]:
		rsv[b15~b14]: used by on-chip CPU
		pkt_len[b13~b0]: Total length of this packet including
				RXINFO when Rx, i.e., (Length of MAC + 4)
*/
#endif /* RELEASE_EXCLUDE */
#ifdef RT_BIG_ENDIAN
...
/*
	RXINFO format:
	hdr_trans[b31]: IP checksum error
	vlan_tag[b30]: TCP checksum error
	ichk_bps[b29]: IP checksum by-pass(hw doesn't do checksum)
	tchk_bps[b28]: TCP checksum by-pass
	rsv0[b27~b22]: Reserved bits
	pn_len[b21~b19]: IV/EIV/PN padding length in unit of 4bytes
	wapi_kid[b18]: WAPI key ID
	bssid_idx3[b17]: BSS index bit3, use together with BSS index bit2:bit0 in RXWI
	dec[b16]: Decrypted frame
	ampdu[b15]: AMPDU aggregated frame
	l2pad[b14]: 2-byte zero and padding after MAC header(+HTC)
	rssi[b13]: RSSI/SNR/PHY rate are vaild
	htc[b12]: 4-byte HTC are padding after MAC header
	amsdu[b11]: AMSDU aggregated frame
	mic_err[b10]: TKIP MIC error
	icv_err[b09]: ICV/AES MIC error
	crc_err[b08]: CRC error
	mybss[b07]: My BSSID frame
	bc[b06]: Broadcast frame
	mc[b05]: Multicast frame
	u2me[b04]: Unicast to me frame
	frag[b03]: Fragmented data frame. TKIP MIC will be skipped on fragmented data frame
	null[b02]: Null data frame
	data[b01]: Data frame
	BA[b00]: BA session, 
			0: not under agreement; 
			1: under BA agreement(need packet reordering)
...
#ifdef MT7601
/*
	RXWI wireless information format for mt7601, in PBF. invisible in driver.
*/
#ifdef RT_BIG_ENDIAN
typedef	struct GNU_PACKED _RXWI_XMAC{
	/* Word 0 */
	UINT32		tid:4;
	UINT32		MPDUtotalByteCnt:12;
	UINT32		UDF:3;
