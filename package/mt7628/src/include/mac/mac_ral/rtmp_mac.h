...
#if 0
MT7650 - PWR_PIN_CFG[2:0]: obsolete, no function
#endif
...
/* Bluetooth active counter MAC register */
...
#define	RTMP_MAC_SHR_MSEL_LOCK(_pAd, _shr_msel, _irqFlag)					\
	do{ \
		if (_pAd->chipCap.FlgIsSupSpecBcnBuf == TRUE)								\
		{ \
			UINT32  _val, _bit = (1 << 19), __status = 0;	\
																				\
			RTMP_SEM_EVENT_WAIT(&_pAd->ShrMemSemaphore, __status);			\
			if (__status != 0)														\
			{																	\
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("SHR_MSEL: failed to get semaphore\n"));	\
			}																	\
																				\
			_pAd->ShrMSel = _shr_msel;											\
			RTMP_IO_READ32(_pAd, PBF_SYS_CTRL, &_val);					\
