...
#if 0 /* Don't send cmd to f/w due to we won't recover the registers that cleared by f/w */
...
VOID MTUsbMlmeRadioOff(RTMP_ADAPTER *pAd)
{
...
VOID MTUsbMlmeRadioOn(RTMP_ADAPTER *pAd)
{
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s()\n", __func__));
...
{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s()-->:HIF_TYPE=%d\n", __FUNCTION__, pAd->chipCap.hif_type));
...
#ifndef RTMP_MAC_USB
		mac_val = 0x3e013;
		HW_IO_WRITE32(pAd, 0x2d004, mac_val);
#endif

		// PSE/MAC Clock setting, Register work-around for FPGA, now it's take cared by ROM code!
		//HW_IO_READ32(pAd, 0x1100, &mac_val);
		//mac_val = 0x26110080;
		//HW_IO_WRITE32(pAd, 0x1100, mac_val);
...
INT mt_hif_sys_init(RTMP_ADAPTER *pAd)
{
	...
	if (IS_USB_INF(pAd))
	{
		...
		HIF_IO_WRITE32(pAd, UDMA_WLCFG_0, UsbCfg.word);
	}
#endif /* RTMP_MAC_USB */
