...
skipUpdate:
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): HtCfg0=0x%x, HtCfg1=0x%x, LgCfg0=0x%x, LgCfg1=0x%x \n",
				__FUNCTION__, HtCfg0.word, HtCfg1.word, LgCfg0.word, LgCfg1.word));
...
//+++Add by shiang for debug
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():RtsRate=%d\n",
		...
		switch(OperationMode)
		{
			...
			case 8:
				...
				if (pAd->rts_option == 1) {
				        ...
				        ProtCfg.word = 0x01744004;      /*duplicaet legacy 24M. BW set 1.*/
				        ProtCfg4.word = 0x03f44084;
				        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Enhance RTS/CTS ------\n"));
				}
#endif /* ENHANCE_ULTP */
...
#ifdef RELEASE_EXCLUDE
	/* AP get the Avg. RSSI to select antenna every 5 secs to fix the corner case.
	 * If some STA's RSSI is too bad, the traffic could be lost and AP only can get
	 * few RSSI sample. AP collect STA's RSSI for a long time and to average the  RSSI,
	 * can make the bad RSSI STA be take care. 
...
#endif /* CONFIG_STA_SUPPORT */
	{
			...
			OPSTATUS_TEST_FLAG(pAd, fOP_AP_STATUS_MEDIA_STATE_CONNECTED)) 
		{
			...
			if (pAd->StaCfg.NumOfAvgRssiSample < 5)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MlmePeriodicExec: no traffic/beacon, reset RSSI\n"));
			}
...
#ifdef RELEASE_EXCLUDE
			/* As AP or STA init, driver will config one antenna as Main antenna, but this one maybe not the great choose.
			 * EvaluateStableCnt is used to force antenna switch to get Avg. RSSI from two antenna and choose
			 * the better one as initial state.
			 */
#endif /* RELEASE_EXCLUDE */
		...
		else
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MlmePeriodicExec: no connect, switch to another one to try connection\n"));
...
VOID RtAsicResetBBPAgent(RTMP_ADAPTER *pAd)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Reset BBP Agent busy bit.!! \n"));
...
INT RtAsicSetMacAddrExt(RTMP_ADAPTER *pAd, BOOLEAN enable)
{
	UINT32 mac_val;
...
#ifdef RT3593
#ifdef CONFIG_STA_SUPPORT
	// TODO: shiang, check TxopThCfg.field.RDG_IN_THRES/RDG_OUT_THRES!!
	if (IS_RT3593(pAd) && (bEnable == FALSE)) {
		/*
			reset Tx/Rx RDG threshold, or the throughput can not 
			be increased due to rate switch function in station mode.
		*/
		TXOP_THRESHOLD_CFG_STRUC TxopThCfg = {{0}};

		RTMP_IO_READ32(pAd, TXOP_THRES_CFG, &TxopThCfg.word);
		TxopThCfg.field.RDG_IN_THRES = 0;
		TxopThCfg.field.RDG_OUT_THRES = 0;
...
INT RtAsicSetPreTbtt(RTMP_ADAPTER *pAd, BOOLEAN enable)
{
	UINT32 timeout = 0;
...
#ifdef MT7601
		if (IS_MT7601(pAd))
			beaconLen = pTxWI->TXWI_X.MPDUtotalByteCnt;
		else
#endif /* MT7601 */
...
#ifdef MT7601
			if (IS_MT7601(pAd))
				beaconLen = localTxWI.TXWI_X.MPDUtotalByteCnt;
			else
#endif /* MT7601 */
...
#ifdef RELEASE_EXCLUDE
		/*
			Note: If you disable Short Slot Time function in 2.4GHz,
			the CWmin of BK must be set to 8.
			Or uplink (BK), uplink (BE), dnlink (BK) the case will not PASS
			whatever PCI or USB.
		*/
#endif /* RELEASE_EXCLUDE */
...
#define RT3593_MINIMUM_AMPDU_RESET(pAd, MinimumAMPDUSize)							\
	if (IS_RT3593(pAd))													\
	{																		\
		/*	MinimumAMPDUSize = (0x000A0fff | (MinimumAMPDUSize << 12)); */	\
		MinimumAMPDUSize = (0x00001000 | (MinimumAMPDUSize << 12));								\
		if (pAd->ApCfg.MAX_PSDU_LEN != MinimumAMPDUSize)							\
		{																	\
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Write 0x%x to MAC register "			\
			...
			pAd->ApCfg.MAX_PSDU_LEN = MinimumAMPDUSize;								\
...
#ifdef RT3593
	if (IS_RT3593(pAd)) {
		pAd->ApCfg.MAX_PSDU_LEN = MAX_AGGREGATION_SIZE | 0x00001000;
	}
#endif /* RT3593 */
...
#ifdef CONFIG_AP_SUPPORT
VOID RtAsicGetTxTsc(RTMP_ADAPTER *pAd, UCHAR apidx, UCHAR *pTxTsc)
{
	USHORT Wcid;
	USHORT offset;
	UCHAR IvEiv[8];
	...
	IN PCIPHER_KEY		pCipherKey)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): BssIndex=%d, KeyIdx=%d\n",
				__FUNCTION__, BssIndex,KeyIdx));
/*============================================================================================*/

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s(): %s key #%d\n",
				__FUNCTION__, CipherName[CipherAlg], BssIndex*4 + KeyIdx));
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, (" 	Key = %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
		pKey[0],pKey[1],pKey[2],pKey[3],pKey[4],pKey[5],pKey[6],pKey[7],pKey[8],pKey[9],pKey[10],pKey[11],pKey[12],pKey[13],pKey[14],pKey[15]));
	if (pRxMic)
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, (" 	Rx MIC Key = %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
			pRxMic[0],pRxMic[1],pRxMic[2],pRxMic[3],pRxMic[4],pRxMic[5],pRxMic[6],pRxMic[7]));
	}
	if (pTxMic)
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, (" 	Tx MIC Key = %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
...
/*============================================================================================*/
...

	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Read: SHARED_KEY_MODE_BASE at this Bss[%d] KeyIdx[%d]= 0x%x \n", BssIndex,KeyIdx, csr1.word));
		if ((BssIndex%2) == 0)
		{
			if (KeyIdx == 0)
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Write: SHARED_KEY_MODE_BASE at this Bss[%d] = 0x%x \n", BssIndex, csr1.word));
	...
	IN ULONG        uEIV)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: wcid(%d) 0x%08lx, 0x%08lx \n", 
...
VOID RtAsicMcsLutUpdate(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
	...
	if(RTMP_TEST_MORE_FLAG(pAd, fASIC_CAP_MCS_LUT) && (pEntry->wcid < 128))
	{
...
#endif /* RTMP_MAC */
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): HIF Type Error !!!\n", __FUNCTION__));
			return;
		}
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s():MCS_LUT update, write to MAC=0x%08x, Value=0x%04x, WCID=%d\n",
					__FUNCTION__, wcid_offset, pEntry->HTPhyMode.word, pEntry->wcid));

		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO, ("\tWcid=%d, APMlmeSetTxRate - CurrTxRateIdx=%d, MCS=%d, STBC=%d, ShortGI=%d, Mode=%d, BW=%d \n"
			                                     "\                            ETxBf=%d, ITxBf=%d\n\n", 
			pEntry->wcid,
	...
	IN	UINT8			KeyTabFlag)
{
	...
	if (Wcid > 254)
	{		
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s:Invalid wcid(%d)\n", __FUNCTION__, Wcid));
		return;
	}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:WCID #%d, KeyIdx #%d, WCIDAttri=0x%x, Alg=%s\n",
	...
	IN PCIPHER_KEY		pCipherKey)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("	Key = %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
		pKey[0],pKey[1],pKey[2],pKey[3],pKey[4],pKey[5],pKey[6],pKey[7],pKey[8],pKey[9],pKey[10],pKey[11],pKey[12],pKey[13],pKey[14],pKey[15]));
	if (pRxMic)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("	Rx MIC Key = %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
			pRxMic[0],pRxMic[1],pRxMic[2],pRxMic[3],pRxMic[4],pRxMic[5],pRxMic[6],pRxMic[7]));
	}
	if (pTxMic)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("	Tx MIC Key = %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
			pTxMic[0],pTxMic[1],pTxMic[2],pTxMic[3],pTxMic[4],pTxMic[5],pTxMic[6],pTxMic[7]));
	}
}
				...
				for (index = 0; index < NUM_OF_2850_CHNL; index++)
				{
					if (Channel == RFRegTable[index].Channel)
					{
						...
						if (1)
						{
							...
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():#%d(RF=%d, ) , R2=0x%08x,  R3 = 0x%08x \n",
								__FUNCTION__, Channel, pAd->RfIcType, R2, R3));
						}
						else
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():#%d(RF=%d, ) , R2=0x%08x \n",
...
#endif /* defined(RT28xx) || defined(RT2880) || defined(RT2883) */
			default:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():#%d : Unkonwn RFIC=%d\n",
...
#ifdef MT76x0
	if (IS_MT76x0(pAd)) {
		// TODO: MT76x0 series has its VCO calibration function! 
		return;
	}
#endif /* MT76x0 */
	...
	for (i = 0; i < seg; i += 2)
	{
		for (reg_addr = aggReg[i] ; reg_addr <= aggReg[i+1] ; reg_addr += 4)
		{
			...
			if (cnt < (cnt_len -1)) {
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():Get AggSize at Reg(0x%x) with val(0x%08x) [AGG_%d=>%ld, AGG_%d=>%ld]\n",
						__FUNCTION__, reg_addr, reg_val.word, cnt, aggCnt[cnt], cnt+1, aggCnt[cnt+1]));
				cnt += 2;
			} else {
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():Get AggSize at Reg(0x%x) failed, no enough buffer(cnt_len=%d, cnt=%d)\n",
	...
	IN BOOLEAN bQosNull)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TxWI:\n"));
	...
	for (offset = 0; offset < TXWISize; offset += 4)  
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("offset: %02d %08x\n", offset, Value));
		RTMP_IO_WRITE32(pAd, HW_NULL2_BASE + offset, Value);
	}

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("802.11 header:\n"));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Write GroupCipher Mode: %d\n", pAd->StaCfg.GroupCipher));
...
INT RtAsicWaitMacTxRxIdle(RTMP_ADAPTER *pAd)
{
	...
	do
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Check if MAC_STATUS_CFG is busy(=%x)\n", val));
		RtmpusecDelay(1000);
	} while (Index++ < 100);
...
INT RtAsicSetMacWD(RTMP_ADAPTER *pAd)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("Warning, MAC isn't ready \n"));
	}
...
INT RtAsicTOPInit(RTMP_ADAPTER *pAd)
{
...
#ifdef RT3290
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("MACVersion=0x%x\n", pAd->MACVersion));
	if (IS_RT3290(pAd))
	{
		UINT32 MacRegValue;
...
#ifdef RELEASE_EXCLUDE
		/*
			Too many times for reading of efuse(enter/exit L1) will make device hang up
	...
	{
		RTMP_IO_READ32(pAd, AUX_CTRL, &mac_val);
		mac_val |= 0x402;
		RTMP_IO_WRITE32(pAd, AUX_CTRL, mac_val);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("AUX_CTRL = 0x%x\n", mac_val));
	}
#endif /* defined(RT3090) || defined(RT3592) || defined(RT3390) || defined(RT3593)  || defined(RT5390) || defined(RT5392) || defined(RT5592) */
	...
	if (opmode == OPMODE_STA)
	{
		...
		*/
		...
		if (IS_RT3883(pAd) || IS_RT3593(pAd))
			dac = pAd->Antenna.field.TxPath - 1;
		else
...
INT RtAsicSetBW(RTMP_ADAPTER *pAd, INT bw)
{
	...
	IN UCHAR index) 
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():%x:%x:%x:%x:%x:%x\n",
