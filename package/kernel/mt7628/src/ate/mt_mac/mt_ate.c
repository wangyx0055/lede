...
static INT32 MT_ATESetupFrame(RTMP_ADAPTER *pAd, UINT32 TxIdx)
{
	...
	if (pAd->NullContext.InUse == FALSE)
	{
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ATECtrl->Addr1:%02x:%02x:%02x:%02x:%02x:%02x\n",
						PRINT_MAC(ATECtrl->Addr1)));

		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ATECtrl->Addr2:%02x:%02x:%02x:%02x:%02x:%02x\n",
						PRINT_MAC(ATECtrl->Addr2)));
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ATECtrl->Addr3:%02x:%02x:%02x:%02x:%02x:%02x\n",
...
static INT32 MT_ATEStartTx(RTMP_ADAPTER *pAd)
{
	...
	//	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("bulk out count = %d\n", atomic_read(&pAd->BulkOutRemained)));

		if (atomic_read(&pAd->BulkOutRemained) == 0)
		{
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Send packet continuously\n"));
		...
		{
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Send packets depend on counter\n"));
...
static INT32 sdio_setup_frame(RTMP_ADAPTER *pAd, UINT32 q_idx){
	...
	#if 0 /* Debug */
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("FC:%04x\n", pAd->NullFrame.FC));
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Duration:%04x\n", pAd->NullFrame.Duration));
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ATECtrl->Addr1:%02x:%02x:%02x:%02x:%02x:%02x\n", PRINT_MAC(ATECtrl->Addr1)));
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ATECtrl->Addr2:%02x:%02x:%02x:%02x:%02x:%02x\n", PRINT_MAC(ATECtrl->Addr2)));
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ATECtrl->Addr3:%02x:%02x:%02x:%02x:%02x:%02x\n", PRINT_MAC(ATECtrl->Addr3)));
	#endif /* Debug */
...
static INT32 sdio_test_frame_tx(RTMP_ADAPTER *pAd)
{
	...
	#if 0
	RTMP_SDIO_READ32(pAd, WHIER, &reg);
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Before ,WHEIR:0x%08x\n",reg));
	reg |= TX_DONE_INT_EN;
	RTMP_SDIO_WRITE32(pAd, WHIER, reg);
	RTMP_SDIO_READ32(pAd, WHIER, &reg);
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("After ,WHEIR:0x%08x\n",reg));
	#endif
