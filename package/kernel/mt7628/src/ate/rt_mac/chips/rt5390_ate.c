	...
	if (pATEInfo->bIQCompEnable == TRUE)
	{
		...
		if (Channel <= 14)
		{
			for (BBPIndex = 0; BBPIndex <= 7; BBPIndex++)
			{
				...
				if (E2PValue == 0xFFFF)
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::EEPROM 0x%X is not calibrated !\n", __FUNCTION__, 
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::IQ compensation is done\n", __FUNCTION__));
		}
		else
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::5 GHz band not supported !\n", __FUNCTION__));
	}
	else
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::Not support IQ compensation since not RT5390 or not enabled !\n", __FUNCTION__));
}
	...
	IN UCHAR 				Channel)
{
	...
	if (pATEInfo->bIQCompEnable == TRUE)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::Not support IQ compensation since not RT5392 or not enabled !\n", __FUNCTION__));
}
    	...
    	IN PRTMP_ADAPTER 		pAd)
{
	...
	if (IS_RT5390(pAd))
	{
		if (pAd->ate.TxWI.TXWI_O.PHYMODE == MODE_CCK)
		{
			/* RF for ATE CCK */
	...
	else if (IS_RT5392(pAd))
	{
		...
		{
			if (pAd->ate.TxWI.TXWI_O.BW == BBP5392Reg_BW[i].BW)
	...
	{
		if (pAd->ate.TxWI.TXWI_O.BW == RF539xReg_BW[i].BW)
	...
	*/
	...
	if (Channel <= 14)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("SwitchChannel#%d(RF=%d, Pwr0=%u, Pwr1=%u, %dT) to , R1=0x%08x, R2=0x%08x, R3=0x%08x, R4=0x%08x\n",
								  Channel, 
								  pAd->RfIcType, 
	...
	else
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("SwitchChannel#%d(RF=%d, %dT) to , R1=0x%08x, R2=0x%08x, R3=0x%08x, R4=0x%08x\n",
								  Channel, 
								  pAd->RfIcType, 
								  pAd->Antenna.field.TxPath,
								  pAd->LatchRfRegs.R1, 
	...
	IN char 					index)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s::Only TxPower0 and TxPower1 are adjustable\n", __FUNCTION__));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("TxPower%d is out of range !!!\n", index));
	}
...
#endif /* RTMP_RF_RW_SUPPORT */
	{
		if (pAd->ate.Channel <= 14)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::(TxPower=%d, R=%ld, BBP_R94=%d)\n", __FUNCTION__, TxPower, R, Bbp94));
		}
#ifdef A_BAND_SUPPORT
		else /* 5.5 GHz */
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::(TxPower=%d, R=%lu)\n", __FUNCTION__, TxPower, R));
		}
#endif /* A_BAND_SUPPORT */
	...
	IN PRTMP_ADAPTER		pAd)
{
	...
	{	
		if (pATEInfo->TxWI.TXWI_O.BW == BW_20) /* BW 20 */
		{
			R66 = LNAGain*2 + 0x1C;
		}
		else /* BW 40 */
...
UCHAR RTATEGetTssiByChannel(PRTMP_ADAPTER pAd, UCHAR Channel)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("Write BBP R47 = 0x%x\n", BBPR47.byte));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Channel %d, Calibrated Tx.Power0= 0x%x\n", Channel, pAd->ate.TxPower0));
	...
	if (i >= 500)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("TSSI status not ready!!! (i=%d)\n", i));

	/* read BBP R49[6:0] and write to EEPROM 0x6E */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("Read  BBP_R49\n")); 
	RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R49, &BbpData);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("BBP R49 = 0x%x\n", BbpData)); 
	BbpData &= 0x7f;
...
CHAR GetPowerDeltaFromTssiRatio(CHAR TssiOfChannel, CHAR TssiBase)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("TssiOfChannel = %d, TssiBase = %d, TssiRatio = %ld\n", TssiOfChannel,  TssiBase, TssiRatio));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("MinTssiDeltaIndex = %d, MinTssiDelta = %ld, PowerDelta = %d\n", MinTssiDeltaIndex,  MinTssiDelta, PowerDelta));
	
	return (PowerDelta);
}

INT RT5390_ATETssiCalibration(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{    	
	UINT 		i = 0;
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Not support TSSI calibration since not 5390 chip or EEPROM not set!!!\n"));
		return FALSE;
	}
	...
	BBPR47.field.TssiUpdateReq = 1;
	BBPR47.field.TssiReportSel = 0;							
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Write BBP R47 = 0x%x\n", BBPR47.byte));
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R47, BBPR47.byte);		
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("(Calibrated) Tx.Power0= 0x%x\n", pAd->ate.TxPower0));
	...
	if (i >= 500)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TSSI status not ready!!! (i=%d)\n", i));
		return FALSE;
	}	
	...
	EEPData &= 0xff00;
	EEPData |= BbpData;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Write  E2P 0x6e: 0x%x\n", EEPData)); 		
...
INT RT5390_ATETssiCalibrationExtend(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{  
	UCHAR inputData;
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("Not support TSSI calibration since not 5390 chip or EEPROM not set!!!\n"));
		return FALSE;
	}	
	else
	{				
		...
		EEPData &= 0xff00;
		EEPData |= TssiBase;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("Write  E2P 0x6E: 0x%x\n", EEPData)); 
		RT28xx_EEPROM_WRITE16(pAd, EEPROM_TSSI_OVER_OFDM_54, EEPData);
		...
		*/
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("Channel %d, PowerDeltaPerChannel= 0x%x\n", CurrentChannel, PowerDeltaPerChannel[CurrentChannel]));
		...
		for (CurrentChannel = 3; CurrentChannel <= 14; CurrentChannel += 4)
		{
			...
			/* MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("CurrentChannel=%d, TssiDeltaPerChannel[CurrentChannel+2] = 0x%x, EEPData=0x%x\n", CurrentChannel, TssiDeltaPerChannel[CurrentChannel+2], EEPData)); */
			...
			/* MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("offset=0x%x, EEPData = 0x%x\n", (EEPROM_TSSI_DELTA_CH3_CH4 +((CurrentChannel-3)/2)),EEPData));	*/
		}
...
INT RT5392_ATEReadExternalTSSI(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Not support TSSI calibration since not 5392 chip or EEPROM not set!!!\n"));
		return FALSE;
	}
	else
	{		
		...
		RT28xx_EEPROM_READ16(pAd, EEPROM_G_TX_PWR_OFFSET, EEPData);
		pAd->ate.TxPower0 = (UCHAR)(EEPData & 0xff);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("(Calibrated) TxPower0= 0x%x\n", pAd->ate.TxPower0));
		
		RT28xx_EEPROM_READ16(pAd, EEPROM_G_TX2_PWR_OFFSET, EEPData);
		pAd->ate.TxPower1 = (UCHAR)(EEPData & 0xff);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("(Calibrated) TxPower1= 0x%x\n", pAd->ate.TxPower1));
	...
	IN PRTMP_ADAPTER 		pAd) 
{
	...
	if (pAd->ate.TxWI.TXWI_O.BW == BW_40)
	{
		if (pAd->ate.Channel > 14)
		{
	...
	{ 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[temp. compensation] BBP_R49 = %02x, current temp = %d\n", BbpR49.byte, CurrentTemp));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[temp. compensation] RefTemp = %d\n", RefTemp));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[temp. compensation] index = %d\n", pAd->TxPowerCtrl.LookupTableIndex));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[temp. compensation] f(%d)= %d\n", pAd->TxPowerCtrl.LookupTableIndex - 1, LookupTable[LookupTableIndex - 1]));
		...
		if (CurrentTemp > RefTemp + LookupTable[LookupTableIndex + 1] + ((LookupTable[LookupTableIndex + 1] - LookupTable[LookupTableIndex]) >> 2) &&
			LookupTableIndex < 32)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[temp. compensation] ++\n"));
			LookupTableIndex++;
			pAd->TxPowerCtrl.LookupTableIndex++;
		}
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[temp. compensation] idxTxPowerTable=%d, idxTxPowerTable2=%d, TuningTableUpperBound=%d\n",
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[temp. compensation] (tx0)RF_TX_ALC = %x, MAC_PowerDelta = %d, TuningTableIndex = %d\n",
			TxPowerTuningTableEntry0->RF_TX_ALC, TxPowerTuningTableEntry0->MAC_PowerDelta, TuningTableIndex0));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[temp. compensation] (tx1)RF_TX_ALC = %x, MAC_PowerDelta = %d, TuningTableIndex = %d\n",
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[temp. compensation] Update RF_R49[0:5] to 0x%x\n", TxPowerTuningTableEntry0->RF_TX_ALC));
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[temp. compensation] Update RF_R50[0:5] to 0x%x\n", TxPowerTuningTableEntry1->RF_TX_ALC));
...
#if 0 /* The case of power percentage should not be considered in ATE mode */
	/* 
	...
	*/
	...
	for (i = 0; i < maxTxPwrCnt; i++)
	{
		if (TxPwr[i] != 0xffffffff)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::After compensation, Offset = 0x%04X, RegisterValue = 0x%08lX\n",
...
==========================================================================
*/
INT	RT539x_Set_ATE_TX_BW_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	INT 		powerIndex;
	...
	if (BBPCurrentBW == 0)
	{
		pAd->ate.TxWI.TXWI_O.BW = BW_20;
	}
	else
	{
		pAd->ate.TxWI.TXWI_O.BW = BW_40;
 	}

	if ((pAd->ate.TxWI.TXWI_O.PHYMODE == MODE_CCK) && (pAd->ate.TxWI.TXWI_O.BW == BW_40))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s::(Warning) CCK only supports 20MHZ!!\nBandwidth switch back to 20\n", __FUNCTION__));
		pAd->ate.TxWI.TXWI_O.BW = BW_20;
	}

	if (pAd->ate.TxWI.TXWI_O.BW == BW_20)
	{
		if (pAd->ate.Channel <= 14)
		{
			/* BW 20 at BG band */
...
#if 0 /* Set BBP_R66 via ATE_CHIP_RX_VGA_GAIN_INIT(pAd) */
		/* Set BBP R66=0x3C */
...
#ifdef RELEASE_EXCLUDE
	    /*
			If Channel=14, Bandwidth=20M and Mode=CCK, 
			Set BBP R4 bit5=1 (to set Japan filter coefficients).
			This code segment will work only when ATETXMODE and ATECHANNEL
			are set to MODE_CCK and 14 respectively before ATETXBW is set to 0.
	    */
#endif /* RELEASE_EXCLUDE */
...
#if 0 /* Set BBP_R66 via ATE_CHIP_RX_VGA_GAIN_INIT(pAd) */
		/* Set BBP R66=0x3C */
		ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R66, 0x3C); /* Rorscha ? */
#endif /* 0 */
		ATE_CHIP_RX_VGA_GAIN_INIT(pAd);
	...
	/* Set BBP R69=0x12 */
	ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R69, 0x12);
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::Current BW = %d\n",
				__FUNCTION__, pAd->ate.TxWI.TXWI_O.BW));
...
==========================================================================
*/
INT	RT539x_Set_ATE_TX_FREQ_OFFSET_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{	
	...
	if (RFFreqOffset >= 96)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s::Out of range(0 ~ 95)\n", __FUNCTION__));
		return FALSE;
	}
...
#endif /* 0 */		
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::RFFreqOffset = %d\n", __FUNCTION__, RefFreqOffset));
...
#ifdef RELEASE_EXCLUDE
	/* 
		RT5392 has a temperature sense resistor to do temperature compensation.
	    	In order to keep the consistency, TSSI calibration of RT5392 is hooked to the pointer of ExtendedTssiCalibration.
	  	Thus, the command to calibrate RT5392 is the same as the others, by using "iwpriv ra0 set ATETSSICBAEX=1".
	*/
#endif /* RELEASE_EXCLUDE */
	.RxVGAInit = RT539x_ATERxVGAInit,
