...
/*	RTMP_RF_REGS *RFRegTable = NULL; */
	...
	for (index = 0; index < NUM_OF_3053_CHNL; index++)
	{
		...
		if (Channel == FreqItems3053[index].Channel)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Channel = %d, BBP_R109 = 0x%X\n",
				__FUNCTION__,
				Channel,
				BbpR109.byte));
...
*/
			...
			/* 2012/09/20: fix VCO unlock issue */
...
/*		ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R86, 0); */	
...
#ifdef RELEASE_EXCLUDE
			/* 2012/09/20: fix SGI compatibility issue */
#endif /* RELEASE_EXCLUDE */
	...
	if (IS_RT3593(pAd))
	{
		...
		if (IS_USB_INF(pAd))
		{
			...
			*/
			Value = ((Value & ~0x00009090) | 0x00000090);
		}
#endif /* RTMP_MAC_USB */
	...
	if (Channel > 14)
	{
		R66 = 0x20 + (pAd->hw_cfg.lan_gain * 5) / 3;
	}
	else
	{
...
INT	RT3593_Set_ATE_TX_BW_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	INT powerIndex;
	UCHAR value = 0;
	UCHAR BBPCurrentBW;
...
INT	RT3593_Set_ATE_TX_FREQ_OFFSET_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
...
static VOID RT3593ATEAsicAdjustTxPower(
        IN PRTMP_ADAPTER pAd)
{
    INT                   i, j;
    CHAR               DeltaPwr = 0;
	...
	if (TRUE)
	{
		/* TX power compensation for temperature variation based on TSSI. try it per second */
		if (TRUE /* pAd->Mlme.OneSecPeriodicRound % 4 == 0 */)
		{
			if (pAd->ate.Channel <= 14)
			{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("** DeltaPwr =%d, DeltaPowerByBbpR1= %d **\n",
			DeltaPwr, DeltaPowerByBbpR1));
	...
	for (i=0; i < CfgOfTxPwrCtrlOverMAC.NumOfEntries; i++)
	{
		if (CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[i].RegisterValue != 0xffffffff)
		{
			for (j=0; j<8; j++)
			{
				...
				else if ((Value + TotalDeltaPower) > 0xC) /* ASIC can only support 0xC as maximum value */
				{
					Value = 0xC;
				}
				else
...
struct _ATE_CHIP_STRUCT RALINK3593 =
{
	...
	.AdjustTxPower = RT3593ATEAsicAdjustTxPower,
