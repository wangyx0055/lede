			...
			{ 
				...
				switch (TssiInfo1 & 0x3)
				{
					...
					case MODE_CCK: /* CCK */
						...
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("CCK: MCS = %d\n", pkt_MCS));
						if (MCS != pkt_MCS)
							continue;
						break;
					case MODE_OFDM: /* OFDM */
						...
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("OFDM: MCS = %d\n", pkt_MCS));
						if (MCS != pkt_MCS)
							continue;
						break;
					case MODE_HTMIX: /* HT */
...
{    
	...
	{                              
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TSSI CALIBRATION: Channel[1] TSSI=%x\n", TSSI_CH1)); 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TSSI CALIBRATION: Channel[7] TSSI=%x\n", TSSI_CH7)); 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TSSI CALIBRATION: Channel[13] TSSI=%x\n", TSSI_CH13)); 
		...
		for (i = 1; i <= 14; i++)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TSSI CALIBRATION: Channel[%d] TSSI_x=%d\n", i, TSSI_x_10000[i])); 
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TSSI CALIBRATION: Channel[%d] TSSI_power_delta=%d\n", i, TSSI_power_delta[i])); 
		}
    ...
    IN PRTMP_ADAPTER pAd)
{
	...
	for (index = 0; index < NUM_OF_3020_CHNL; index++)
	{
		...
		if (Channel == RtmpFreqItems3020[index].Channel)
		{
			...
			step = (*((UINT32 *)(RALINK_SYSCTL_BASE + 0x10)));
	...
	*/
	...
	{ 
		/* Xtal=40M */
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RT5350:SwitchChannel#%d(RFIC=%d, Pwr0=%d, Pwr1=%d, %dT), N=0x%02X, K=0x%02X, R=0x%02X\n",
		Channel, 
		pAd->RfIcType, 
		TxPwer,
		TxPwer2,
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RT5350Xtal20M:SwitchChannel#%d(RFIC=%d, Pwr0=%d, Pwr1=%d, %dT), N=0x%02X, K=0x%02X, R=0x%02X\n",
		Channel, 
		pAd->RfIcType, 
		TxPwer,
		TxPwer2,
	...
	IN char index)
{
...
#ifdef RELEASE_EXCLUDE
		/* 
			When QA is used for Tx, pATEInfo->TxPower0 and real tx power
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : (TxPower%d=%d, RFValue=%x)\n", __FUNCTION__, index, TxPower, RFValue));
...
INT	RT5350_Set_ATE_TX_FREQ_OFFSET_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
...
#ifdef RELEASE_EXCLUDE
	/* 802.3 and 802.11 share one crystal */
	/* to prevent LAN port link down when adjusting RF_R17 */
#endif /* RELEASE_EXCLUDE */
	pAd->chipCap.RfReg17WtMethod = RF_REG_WT_METHOD_NONE /* 0 */;

	if ((RFValue & 0x80) == 0)
	{
		ATE_RF_IO_WRITE8_BY_REG_ID(pAd, RF_R17, (UCHAR)(RFValue + 0x80));
		RtmpOsMsDelay(1);
		RTMP_SYS_IO_WRITE32(0xb0000048, 0x22010002);
		RTMP_SYS_IO_WRITE32(0xb0000048, 0x22010006);
		RTMP_SYS_IO_WRITE32(0xb0000048, 0x22010002);
		RTMP_SYS_IO_WRITE32(0xb0000048, 0x22010000);
	}

	RFValue = RFValue & 0x7F; /* bit7 = 0 */

	if (RFValue2 > RFValue)
	{
		for (offset = 1; offset <= (RFValue2 - RFValue); offset++)
		{
			ATE_RF_IO_WRITE8_BY_REG_ID(pAd, RF_R17, (UCHAR)(RFValue + offset + 0x80));
			RtmpOsMsDelay(1);
		}
	}	
	else
	...
	pAd->chipCap.RfReg17WtMethod = RF_REG_WT_METHOD_STEP_ON /* 1 */;
#endif /* 1 */
...
/*		if (pATEInfo->OneSecPeriodicRound % 4 == 0) */
		{
			...
			if (bKeepRF == FALSE)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RF_R49 = %u\n", RFValue));			  
			}

			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Pwr0 = 0x%02x\n", (pAd->TxPower[pATEInfo->Channel-1].Power)));			  
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Current index of tuning table = %d {RF_TX_ALC = 0x%02x, MAC_PowerDelta = %d}\n",
...
/*					*pTxAgcCompensate = -((UCHAR)R3); */
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("-- Tx Power, BBP R1=%x, TssiRef=%x, TxAgcStep=%x, step = -%d\n",
