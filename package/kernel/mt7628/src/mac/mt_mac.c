...
VOID NICUpdateRawCounters(RTMP_ADAPTER *pAd)
{
	...
	MAC_IO_READ32(pAd, MIB_MB0SDR2, &mac_val);
	TxRetryCount = mac_val & 0xffff;

	MAC_IO_READ32(pAd, WTBL_BTCRn, &mac_val);
	TxSuccessCount = mac_val & 0xffff;

	if ( TxRetryCount == 0 )
		pAd->RalinkCounters.OneSecTxNoRetryOkCount += TxSuccessCount;
	else
		pAd->RalinkCounters.OneSecTxRetryOkCount += TxSuccessCount;
	pAd->RalinkCounters.OneSecTxFailCount += TxFailCount;

#ifdef STATS_COUNT_SUPPORT
	pAd->WlanCounters.TransmittedFragmentCount.u.LowPart += TxSuccessCount;
	...
	{
		pAd->WlanCounters.RetryCount.u.LowPart += TxFailCount;
	}
	...
	IN HTTRANSMIT_SETTING *pTransmit)
{
	...
	txd_0->ut_sum = 0;
	txd_0->ip_sum = 0;
	if (txd_0->ip_sum || txd_0->ut_sum)
		txd_0->eth_type_offset = info->eth_type_offset; // TODO:
...
VOID write_tmac_info_Data(RTMP_ADAPTER *pAd, UCHAR *buf, TX_BLK *pTxBlk)
{
	...
	if (txd_0->ip_sum || txd_0->ut_sum) {
		...
		txd_0->eth_type_offset = TBD; // TODO:
	}
#endif /* CONFIG_CSO_SUPPORT */
...
//YF_TODO
	...
	if (txd_long == FALSE)
	{
		...
		if (pTransmit) {
			...
			TRACE_IPARP_RECORD_TX_MCS_SETTING(
					pAd, RTMP_GET_PACKET_TRACEIPARP(pTxBlk->pPacket), pTransmit);
...
#ifdef MT7636
	// TODO: shiang, is that necessary here?
	if (IS_MT76x6(pAd))
	{
		HW_IO_WRITE32(pAd, MCU_PCIE_REMAP_1, pAd->chipCap.WtblPseAddr);
#ifdef RTMP_PCI_SUPPORT
...
#ifdef MT7636
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\twm%d\n", wtbl_1_d0->field.wm));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\ttx_ps:%d\n", wtbl_1_d2->field.tx_ps));
#else
