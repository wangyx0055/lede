...
static VOID NICInitRT3883RFRegisters(IN PRTMP_ADAPTER pAd)
{
	...
	/*
	  RT3883 F version must change RF 32 and 33 to new value
		Gary T, change RF_R32/33 for Fix flatness issue/NC. 20110825
	*/
	...
	/*RF_R02: Resistor calibration, RF_R02 = RF_R30 */
	...
	/*MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("SS \n")); */
	RtmpusecDelay(1000);
	RfReg &= 0x7F;
	RT30xxWriteRFRegister(pAd, RF_R02, (UCHAR)RfReg);     
	...
	IN struct _RTMP_ADAPTER *pAd)
{
	...
	bbp_val = 0x26 + pAd->hw_cfg.lan_gain;
#ifdef CONFIG_ATE
	if (ATE_ON(pAd))
	{
...
static VOID RT3883_ChipBBPAdjust(RTMP_ADAPTER *pAd)
{
	UCHAR r66_val, bbp_val;
	UINT8 rf_bw, ext_ch;
	...
	/*  TX/Rx : control channel at lower */
	AsicSetCtrlCh(pAd, ext_ch);
	bbp_set_ctrlch(pAd, ext_ch);

	bbp_set_agc(pAd, r66_val, RX_CHAIN_ALL);
	...
	if (pAd->CommonCfg.Channel > 14)
		...
		bbp_val = 0x0A;
	RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R70, bbp_val);
}
...
static VOID RT3883_NetDevNickNameInit(
	IN struct _RTMP_ADAPTER *pAd)
{
	snprintf((RTMP_STRING *) pAd->nickname, sizeof(pAd->nickname), "RT3883STA");
}
	...
	IN	ULONG			freqOffset)
{
	...
	//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 2, ("offset = %lx\n", freqOffset));
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	for (i = 0; i < MAX_TXPOWER_ARRAY_SIZE; i++) {
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("20MHz BW, 2.4G band, %d = %lx \n", i, pAd->Tx20MPwrCfgGBand[i]));
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("40MHz BW, 2.4G band, %d = %lx \n", i, pAd->Tx40MPwrCfgGBand[i]));
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("20MHz BW, 5G band, %d = %lx \n", i, pAd->Tx20MPwrCfgABand[i]));
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("40MHz BW, 5G band, %d = %lx \n", i, pAd->Tx40MPwrCfgABand[i]));
	}
...
/*		RT28xx_EEPROM_READ16(pAd, EEPROM_A_TX2_PWR_OFFSET + (choffset - 14) + i * 2, Power2.word); */
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("E2PROM: TxPower[%03d], Channel = %d, Power = %d, Power2 = %d, Power3 = %d\n", i, pAd->TxPower[i].Channel, pAd->TxPower[i].Power, pAd->TxPower[i].Power2, pAd->TxPower[i].Power3 ));
	}
#endif /* RELEASE_EXCLUDE */
}
...
#if 0
	/*config CLKCFG0: set GPIO8(gpio#31) to GPIO mode */
	value = le32_to_cpu(*(UINT32 *)(0xb000002c));
	value &= ~(1 << 12);
	*((volatile uint32_t *)(0xb000002c)) = cpu_to_le32(value);

	/*config gpio#31 direction to output */
	value = le32_to_cpu(*(UINT32 *)(0xb000064c));
	value |= (1 << 7);
	*((volatile uint32_t *)(0xb000064c)) = cpu_to_le32(value);

	if (Channel > 14)
	{
		/*5G band: clear gpio#31 to 0 */
		*((volatile uint32_t *)(0xb0000658)) = cpu_to_le32(0x80);
	}
	else
	{
		/*2.4G band: set gpio#31 to 1 */
		*((volatile uint32_t *)(0xb0000654)) = cpu_to_le32(0x80);
	}
#endif

		/*set GPIO2(gpio#25) to GPIO mode */
		value = le32_to_cpu(*(UINT32 *)(0xb0000014));
		value &= ~(1 << 2);
...
#if 0
	/* init EEPROM offset */
	pAd->EepromOffset.RssiBg = EEPROM_RSSI_BG_OFFSET_33;
	pAd->EepromOffset.RssiA = EEPROM_RSSI_A_OFFSET_33;
	pAd->EepromOffset.Nic3 = EEPROM_NIC3_OFFSET_33;
	pAd->EepromOffset.CountryRegion = EEPROM_COUNTRY_REGION_33;
	pAd->EepromOffset.DefineMaxTxPwr = EEPROM_DEFINE_MAX_TXPWR_33;
	pAd->EepromOffset.Freq = EEPROM_FREQ_OFFSET_33;
	pAd->EepromOffset.LedAGConf = EEPROM_LEDAG_CONF_OFFSET_33;
	pAd->EepromOffset.LedACTConf = EEPROM_LEDACT_CONF_OFFSET_33;
	pAd->EepromOffset.LedPolarity = EEPROM_LED_POLARITY_OFFSET_33;
	pAd->EepromOffset.LNA = EEPROM_LNA_OFFSET_33;
	pAd->EepromOffset.LNA2 = EEPROM_LNA_OFFSET2_33;
	pAd->EepromOffset.G_TX_PWR = EEPROM_G_TX_PWR_OFFSET_33;
	pAd->EepromOffset.G_TX2_PWR = EEPROM_G_TX2_PWR_OFFSET_33;
	pAd->EepromOffset.G_TX3_PWR = EEPROM_G_TX3_PWR_OFFSET_33;
	pAd->EepromOffset.G_TSSI_BOUND1 = EEPROM_G_TSSI_BOUND1_33;
	pAd->EepromOffset.G_TSSI_BOUND2 = EEPROM_G_TSSI_BOUND2_33;
	pAd->EepromOffset.G_TSSI_BOUND3 = EEPROM_G_TSSI_BOUND3_33;
	pAd->EepromOffset.G_TSSI_BOUND4 = EEPROM_G_TSSI_BOUND4_33;
	pAd->EepromOffset.G_TSSI_BOUND5 = EEPROM_G_TSSI_BOUND5_33;
	pAd->EepromOffset.A_TX_PWR = EEPROM_A_TX_PWR_OFFSET_33;
	pAd->EepromOffset.A_TX2_PWR = EEPROM_A_TX2_PWR_OFFSET_33;
	pAd->EepromOffset.A_TX3_PWR = EEPROM_A_TX3_PWR_OFFSET_33;
	pAd->EepromOffset.A_TSSI_BOUND1 = EEPROM_A_TSSI_BOUND1_33;
	pAd->EepromOffset.A_TSSI_BOUND2 = EEPROM_A_TSSI_BOUND2_33;
	pAd->EepromOffset.A_TSSI_BOUND3 = EEPROM_A_TSSI_BOUND3_33;
	pAd->EepromOffset.A_TSSI_BOUND4 = EEPROM_A_TSSI_BOUND4_33;
	pAd->EepromOffset.A_TSSI_BOUND5 = EEPROM_A_TSSI_BOUND5_33;
	pAd->EepromOffset.TxPowerByRate20MHz24G = EEPROM_TXPOWER_BYRATE_20MHZ_2_4G_33;
	pAd->EepromOffset.TxPowerByRate40MHz24G = EEPROM_TXPOWER_BYRATE_40MHZ_2_4G_33;
	pAd->EepromOffset.TxPowerByRate20MHz5G = EEPROM_TXPOWER_BYRATE_20MHZ_5G_33;
	pAd->EepromOffset.TxPowerByRate40MHz5G = EEPROM_TXPOWER_BYRATE_40MHZ_5G_33;
	pAd->EepromOffset.BbpBase = EEPROM_BBP_BASE_OFFSET_33;
#endif
}
	...
	*/
	...
	if (index == MAX_NUM_OF_CHANNELS)
	{
		...
		for (index = 0; index < NUM_OF_3883_CHNL; index++)
		{
...
#ifdef RELEASE_EXCLUDE
			/* Gary T,  change RF_32/33 for flatness issue in HT20 and HT40, 20110825 */
#endif /* RELEASE_EXCLUDE */
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RT3883: SwitchChannel#%d(RF=%d, Pwr0=%d, Pwr1=%d, %dT), N=0x%02X, K=0x%02X, R=0x%02X\n",
	...
	)
	{
		...
		*/		
		for (macIdx = 0; macIdx < MAX_LEN_OF_MAC_TABLE; macIdx++)
		{
			...
			/* MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("MCS = %d\n", pEntry->HTPhyMode.field.MCS)); */
			break;
		}
		...
		if ((bNoRTS == FALSE) && (pAd->BATable.numDoneOriginator))
		{
			...
			*pOperationMode = 8;
			...
			/* MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("RTS/CTS protection ON\n")); */
		}		
		else
		{
			RT3883_CWC_AsicSet(pAd, 1);
			/* MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("RTS/CTS protection OFF\n")); */
		}
	}
}
	...
	IN PRTMP_ADAPTER		pAd)
{
	...
	*/
	...
	pChipCap->FlgITxBfBinWrite = FALSE;
#endif /* TXBF_SUPPORT */
...
#ifdef RTMP_FLASH_SUPPORT
	pChipCap->EEPROM_DEFAULT_BIN = RT3883_EeBuffer;
#endif /* RTMP_FLASH_SUPPORT */
...
#ifdef RELEASE_EXCLUDE
/*
	5G BW40:  R61: 0x13 --> -76dBm per MHz (spec -70dBm) --> Signal Generator: ~ -38dBm  => margin ~ 6 dBm
	5G BW20:  R61: 0x23 --> -76dBm per MHz (spec -70dBm) --> Signal Generator: ~ -38dBm  => margin ~ 6 dBm
	2G BW40:  R61: 0x03 --> -76dBm per MHz (spec -70dBm) --> Signal Generator: ~ -45dBm  => margin ~ 6 dBm
	2G BW20:  R61: 0x03 --> -73dBm per MHz (spec -70dBm) --> Signal Generator: ~ -45dBm  => margin ~ 3 dBm
	2G 11B :  R61: 0x03 --> -80dBm per MHz (spec -70dBm) --> Signal Generator: ~ -52dBm  => margin ~ 10dBm
*/
#endif /* RELEASE_EXCLUDE */
INT RT3883_set_ed_cca(RTMP_ADAPTER *pAd, BOOLEAN enable)
{
	UINT32 mac_val;
	UCHAR bbp_val;
	...
	if (enable) {
		...
		// bit 0~7 for high threshold
		if (pAd->CommonCfg.Channel > 14) {
			bbp_val = pAd->CommonCfg.BBPCurrentBW == BW_40 ? \
						0x13: 0x23;
		} else {
			bbp_val = 0x03;
		}
