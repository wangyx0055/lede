	...
	*/
	...
	switch (pAd->RfIcType)
	{
		...
		case RFIC_2750:
				...
				if (Channel == RFRegTable[index].Channel)
				{
					...
					if (Channel > 14)
					{
						...
						if ((TxPwer >= -7) && (TxPwer < 0))
						{
							...
							R3 |= (TxPwer << 10);
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): TxPwer=%d \n", __FUNCTION__, TxPwer));
						}
						else
						{
...
#if 0
							/*
								It is impossible that TxPwer will be larger than 15 here.
								This code is useless and complained by coverity.
							*/
							TxPwer2 = (TxPwer2 > 0xF) ? (0xF) : (TxPwer2);
#endif
							
							R4 |= (TxPwer2 << 7);
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): TxPwer2=%d \n", __FUNCTION__, TxPwer2));
						}
						else
						{
							TxPwer2 = (TxPwer2 > 0xF) ? (0xF) : (TxPwer2);
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("SwitchChannel#%d(RF=%d, Pwr0=%lu, Pwr1=%lu, %dT) to , R1=0x%08x, R2=0x%08x, R3=0x%08x, R4=0x%08x\n",
										Channel, 
										pAd->RfIcType, 
										(R3 & 0x00003e00) >> 9,
										(R4 & 0x000007c0) >> 6,
