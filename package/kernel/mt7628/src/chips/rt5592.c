...
/*
	History for RT5592 Linux Porting:
	1. Change RF settings for RT5592 from RT5572 driver.
	2. Do not use MCU to write RF R17 or RF R17 will be 0. (maybe no the command?)
	3. Must set SDP0 field in each receive buffer descriptor because 5592
	    has RX DMA scattering function or you will not have any receive
	    interrupt. If you have ever set SDP0 to 0, DMA will deadlock and
	    you need to power off/on your card; Or it can not work any more.
*/
#endif /* RELEASE_EXCLUSE */
...
UCHAR NUM_RF5592REG_5G = (sizeof(RF5592Reg_5G) / sizeof(REG_PAIR));

/* RF for G band per channel */
const REG_PAIR_CHANNEL RF5592Reg_Channel_2G[] =
{
	{RF_R23, 1, 10, 0x07},
	{RF_R23, 11, 14, 0x06},
	{RF_R59, 1, 10, 0x07},
...
const REG_PAIR_CHANNEL RF5592Reg_Channel_5G[] =
{
	{RF_R12, 36, 64, 0x2E},
	{RF_R12, 100, 165, 0x0E},
	{RF_R13, 36, 64, 0x22},
...
#ifdef RTMP_MAC_USB
UCHAR NUM_RF5592REG_CHANNEL_2G = (sizeof(RF5592Reg_Channel_2G) / sizeof(REG_PAIR_CHANNEL));
#endif /* RTMP_MAC_USB */
#ifdef RTMP_MAC_PCI
...
#ifdef RTMP_MAC_USB
static UCHAR RT5592_EeBuffer[EEPROM_SIZE] = {
0x72, 0x55, 0x02, 0x01, 0x00, 0x0c, 0x43, 0x30, 0x92, 0x00, 0x92, 0x30, 0x14, 0x18, 0x01, 0x80,
0x00, 0x00, 0x92, 0x30, 0x14, 0x18, 0x00, 0x00, 0x01, 0x00, 0x6a, 0xff, 0x13, 0x02, 0xff, 0xff,
0xff, 0xff, 0xc1, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
0x8e, 0x75, 0x01, 0x43, 0x22, 0x08, 0x27, 0x00, 0xff, 0xff, 0x16, 0x01, 0xff, 0xff, 0xd9, 0xfa,
0xcc, 0x88, 0xff, 0xff, 0x0a, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff,
	...
	IN UCHAR			Ant)
{
	...
	if (Ant == 0) /* 0: Main antenna */
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("AsicSetRxAnt, switch to main antenna\n"));
	}
	else /* 1: Aux. antenna */
	{
		RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R152, &BbpValue);
		...
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R152, BbpValue);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("AsicSetRxAnt, switch to aux. antenna\n"));
	}
}
...
static VOID RT5592FilterCalibration(IN PRTMP_ADAPTER pAd)
{
	...
	do
	{
		...
		do
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Wait RF calibration done BBP_R0 value = 0x%02x\n", BBPValue));
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Retry = %d, CalRF57_PassBand = 0x%02x\n", ReTry, CalRF57_PassBand));
		...
		while (TRUE)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("loopcnt = %d, CalRF57_StopBand = 0x%x,\
			...
			if (loopcnt++ > 100)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - can not find a valid value, loopcnt = %d\
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s CaliBW20RfR24 = 0x%x, CaliBW40RfR24 = 0x%x\n", 
...
static VOID NICInitRT5592RFRegisters(IN PRTMP_ADAPTER pAd)
{
	...
	/* RF for G band per channel */
	RTUSBMultiWrite_nBytes(pAd, 0x7158, (PUCHAR)RF5592Reg_Channel_2G, sizeof( RF5592Reg_Channel_2G), sizeof( RF5592Reg_Channel_2G));

	/* RF for CCK */
	RTUSBMultiWrite_nBytes(pAd,  0x7232, (PUCHAR)RF5592Reg_CCK, sizeof( RF5592Reg_CCK), sizeof( RF5592Reg_CCK));

	/* RF for G band OFDM */
	RTUSBMultiWrite_nBytes(pAd,  0x723C, (PUCHAR)RF5592Reg_OFDM_2G, sizeof( RF5592Reg_OFDM_2G), sizeof( RF5592Reg_OFDM_2G));
	
	/* RF for A band */
	RTUSBMultiWrite_nBytes(pAd,  0x713A, (PUCHAR)RF5592Reg_5G, sizeof( RF5592Reg_5G), sizeof( RF5592Reg_5G));

	/* RF for A band per channel */
	for (i = 0; i < 4; i++)
	{
		PUCHAR Pos = (PUCHAR)RF5592Reg_Channel_5G;
	
		if (i != 3)
			RTUSBMultiWrite_nBytes(pAd, 0x7168 + (64 * i), Pos + (64 * i), 64, 64);
		else
			RTUSBMultiWrite_nBytes(pAd, 0x7168 + (64 * i), Pos + (64 * i), 4, 4);
	}

	/* RF for A/G band BW */
	RTUSBMultiWrite_nBytes(pAd, 0x722C, (PUCHAR)RF5592Reg_BW_2G_5G, sizeof(RF5592Reg_BW_2G_5G), sizeof(RF5592Reg_BW_2G_5G));

	/* RF for A/G band OFDM */
	RTUSBMultiWrite_nBytes(pAd, 0x7238, (PUCHAR)RF5592Reg_OFDM_2G_5G, sizeof(RF5592Reg_OFDM_2G_5G), sizeof(RF5592Reg_OFDM_2G_5G));

	/* Load channel RF register to shared memory */
	/* BBP setting */
	RTUSBMultiWrite_nBytes(pAd, 0x723E, (PUCHAR)BBP5592Reg_2G, sizeof(BBP5592Reg_2G), sizeof(BBP5592Reg_2G));
	
	/* BBP for G band GLRT */
	RTUSBMultiWrite_nBytes(pAd,  0x724E, (PUCHAR)BBP5592Reg_GLRT_2G, sizeof(BBP5592Reg_GLRT_2G), sizeof(BBP5592Reg_GLRT_2G));

	/* BBP for A band */
	RTUSBMultiWrite_nBytes(pAd, 0x7246, (PUCHAR)BBP5592Reg_5G, sizeof(BBP5592Reg_5G), sizeof(BBP5592Reg_5G));
...
#ifdef RELEASE_EXCLUDE
/*
 * HW resolve bbp hang issue after C version. 
 * Enable HW recovery mechanism by set bit7 of BBP_R254.
 */
...
/*
	...
	==========================================================================
 */
static UCHAR RT5592_ChipAGCAdjust(
	IN PRTMP_ADAPTER		pAd,
	IN CHAR					Rssi,
	IN UCHAR				OrigR66Value)
{
	...
	if (Rssi > RSSI_FOR_MID_LOW_SENSIBILITY)
	{
		if (pAd->LatchRfRegs.Channel <= 14)
			R66 = 0x1C + (pAd->hw_cfg.lan_gain << 1) + 0x20;
		else
			R66 = 0x24 + (pAd->hw_cfg.lan_gain << 1) + 0x20;
	...
	{
		if (pAd->LatchRfRegs.Channel <= 14)
			R66 = 0x1C + (pAd->hw_cfg.lan_gain << 1);
		else
			R66 = 0x24 + (pAd->hw_cfg.lan_gain << 1);
...
static VOID RT5592_ChipBBPAdjust(RTMP_ADAPTER *pAd)
{
	UINT8 rf_bw, ext_ch;
...
#ifdef CONFIG_SWITCH_CHANNEL_OFFLOAD
static UINT32 RT5592_ChannelParamsAlloc(PRTMP_ADAPTER pAd)
{

	RTMP_CHIP_CAP *pChipCap = &pAd->chipCap;

	os_alloc_mem(NULL, (UCHAR **) &pChipCap->ChannelParam, 32);
	
	if (pChipCap->ChannelParam == NULL) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			 ("%s: pChipCap->ChannelParam Allocate memory fail!!!\n",
			  __FUNCTION__));
		return -1;
	}
...
static VOID RT5592_ChannelParamsInit(RTMP_ADAPTER *pAd, UCHAR Channel)
{
	...
	*/
	...
	if (i == MAX_NUM_OF_CHANNELS)
	{
		...
		*Pos = TxPwer2;
		Pos++;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("AsicSwitchChannel: Can't find the Channel#%d \n", Channel));
	}	
	...
	*Pos = pAd->CommonCfg.BBPCurrentBW;
	...
	for (i = 0; i < RT5592_Frequency_Plan[pAd->chipCap.XtalType].totalFreqItem; i++, pFrequencyItem++)
	{
		if (Channel == pFrequencyItem->Channel)
		{
			...
			/* N */
			UINT16 tmpN = cpu2le16(pFrequencyItem->N);
			NdisMoveMemory(Pos, &tmpN, 2);
			Pos += 2;
			
			/* K */
			*Pos = pFrequencyItem->K;
			Pos++;

			/* Mod */
			*Pos = pFrequencyItem->mod;
			Pos++;

			/* R */
			*Pos = pFrequencyItem->R;
			Pos++;
	...
	/* Tssi gain for a band */
	*Pos = pAd->TxPowerCtrl.TssiGain[IEEE80211_BAND_5G];
	Pos++;
				
	/* Tssi gain for g band */
	*Pos = pAd->TxPowerCtrl.TssiGain[IEEE80211_BAND_2G];
	Pos++;
				
	/* Phy mode */
	*Pos = pAd->CommonCfg.PhyMode;
	Pos++;

	/* TxPath */
	*Pos = pAd->Antenna.field.TxPath;
	Pos++;

	/* RxPath */
	*Pos = pAd->Antenna.field.RxPath;
	Pos++;
				
	/* RfFreqOffset */
	tmpRfFreqOffset = cpu2le32((UINT32)(pAd->RfFreqOffset));
	NdisMoveMemory(Pos, &tmpRfFreqOffset, 4);
	Pos += 4;
	...
	/* LNA gain */
{
	...
	if (Channel <= 14)
		lan_gain = pAd->BLNAGain;
	else if (Channel <= 64)
		lan_gain = pAd->ALNAGain0;
	else if (Channel <= 128)
		lan_gain = pAd->ALNAGain1;
	else
		lan_gain = pAd->ALNAGain2;
	*Pos = lan_gain;
	Pos++;
}
	...
	IN BOOLEAN					bScan)
{
	...
	if (IS_USB_INF(pAd))
	{
		...
		if (ret != 0)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():get reg_atomic failed!\n", __FUNCTION__));
			return;
		}
		bInLock = TRUE;
	}
	...
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("RT5592_ChipSwitchChannel: Retry count exhausted\n"));
	}
...
/*	UCHAR PreRFValue; */
	...
	*/
...
#ifdef RELEASE_EXCLUDE
			/*
				We found that if RF_R06 (VCO) is not program again here, 
				Rx will be all CRC err under STA mode (AP mode is OK)
			*/
#endif
			RTMP_WriteRF(pAd, RF_R06, 0xE0, 0xFB);
	...
	if (Channel <= 14)
	{
		...
		RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R98, &bbp_val);
		if (Channel == 14)
			bbp_val |= 0x80; /* set b'[7]=1 to fix Japan ch=14 bandwidth issue */
		else
			bbp_val &= ~(0x80); /* set b'[7]=0 */
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R98, bbp_val)
...
VOID RT5592_AsicExtraPowerOverMAC(RTMP_ADAPTER *pAd)
{
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (loop == 101)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("BBP re-calibaration fail! \n"));
	else
	{
#ifdef IQ_CAL_SUPPORT
		/* IQ Calibration */
...
VOID rtmp_cso_enable(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Enable TSO Support\n"));
};


VOID rtmp_cso_disable(RTMP_ADAPTER *pAd)
{
	INTERNAL_1_STRUCT Internal_1;
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Disable TSO Support\n"));
};
...
static const RTMP_CHIP_CAP RT5592_ChipCap = {
...
#endif /* RTMP_TEMPERATURE_COMPENSATION */
#ifdef RTMP_FLASH_SUPPORT
	.EEPROM_DEFAULT_BIN = RT5592_EeBuffer,
#endif /* RTMP_FLASH_SUPPORT */
...
#endif /* DFS_SUPPORT */
#ifdef RELEASE_EXCLUDE
	/* RT5592 Version A ~ C bugs: we can not use 64B; Or RV packets will lost */
#endif /* RELEASE_EXCLUDE */
	.WPDMABurstSIZE = 3,
...
#ifdef RTMP_MAC_PCI
#ifdef CONFIG_STA_SUPPORT
	/* only for RT5592, not RT5572 */
	.HW_PCIE_PS_SUPPORT = TRUE,
	.HW_PCIE_PS_L3_ENABLE = TRUE,
#endif /* CONFIG_STA_SUPPORT */
#endif /* RTMP_MAC_PCI */
	...
	.FreqCalInitMode = FREQ_CAL_INIT_MODE2,
	.FreqCalMode = FREQ_CAL_MODE1,
	.RxWIFrqOffset = RXWI_FRQ_OFFSET_FIELD1,
#endif /* CONFIG_STA_SUPPORT */
#endif /* RTMP_FREQ_CALIBRATION_SUPPORT */
...
#ifdef CONFIG_STA_SUPPORT
	.ChipAGCAdjust = RT5592_ChipAGCAdjust,
#endif /* CONFIG_STA_SUPPORT */
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	pChipCap = &pAd->chipCap;

	rtmp_phy_probe(pAd);
#ifdef RTMP_MAC_PCI
	if (IS_PCI_INF(pAd)) {
		pAd->ChipCap.tx_hw_hdr_len = pAd->ChipCap.TXWISize;
		pAd->ChipCap.rx_hw_hdr_len = pAd->ChipCap.RXWISize;
	}
#endif /* RTMP_MAC_PCI */

#ifdef RTMP_MAC_USB
	if (IS_USB_INF(pAd)) {
		pAd->ChipCap.tx_hw_hdr_len = TXINFO_SIZE + pAd->ChipCap.TXWISize;
		pAd->ChipCap.rx_hw_hdr_len = RXDMA_FIELD_SIZE + pAd->ChipCap.RXWISize;
	}
#endif /* RTMP_MAC_USB */
	...
	if ((MacValue & 0x30) == 0x30) /* bit4&5 = 1 */
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("This is RT5592 EP version!\n"));
	}
#endif /* RT5592EP_SUPPORT */
	...
	pChipCap->WMM0ACBulkOutAddr[0] = 0x1;
	pChipCap->WMM0ACBulkOutAddr[1] = 0x2;
	pChipCap->WMM0ACBulkOutAddr[2] = 0x3;
	pChipCap->WMM0ACBulkOutAddr[3] = 0x4;
	//pChipCap->WMM1ACBulkOutAddr = 0x9;
	pChipCap->DataBulkInAddr = 0x81;
