...
#endif
	if (!IS_MT76x0(pAd)) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: Incorrect NIC\n", __FUNCTION__));
		return;
	}
		...
		((pAd->chipCap.PAType == EXT_PA_2G_ONLY) && (RfBand & RF_A_BAND))) 
	{
		...
		for (i = 0; i < MT76x0_RF_INT_PA_RegTb_Size; i++)
		{
			if (MT76x0_RF_INT_PA_RegTb[i].BwBand & RfBand) {
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s: INT_PA_RegTb - B%d.R%02d = 0x%02x\n", 
							__FUNCTION__, 
							MT76x0_RF_INT_PA_RegTb[i].Bank,
							MT76x0_RF_INT_PA_RegTb[i].Register,
							MT76x0_RF_INT_PA_RegTb[i].Value));
		...
		*/
		...
		for (i = 0; i < MT76x0_RF_EXT_PA_RegTb_Size; i++)
		{
			if (MT76x0_RF_EXT_PA_RegTb[i].BwBand & RfBand) {
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s: EXT_PA_RegTb - B%d.R%02d = 0x%02x\n", 
							__FUNCTION__, 
							MT76x0_RF_EXT_PA_RegTb[i].Bank,
							MT76x0_RF_EXT_PA_RegTb[i].Register,
							MT76x0_RF_EXT_PA_RegTb[i].Value));
...
static void mt76x0_nic_init_rf_registers(PRTMP_ADAPTER pAd)
{
	...
	*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: B0.R22 = 0x%02x\n", __FUNCTION__, RFValue));
...
static void mt76x0_nic_init_mac_registers(PRTMP_ADAPTER pAd)
{
	...
	*/
	...
	if (((trsw_mode & ~(0xFFCF)) >> 4) == 0x3) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: TRSW = 0x3\n", __FUNCTION__));
	}
	...
	BOOLEAN	 bScan)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): MAC_STATUS_CFG = 0x%08x, bbp_ch_idx = %d\n", __FUNCTION__, RegValue, bbp_ch_idx));
...
#ifdef RELEASE_EXCLUDE
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Stored_BBP_InitGain=%x !!!\n", pAd->CommonCfg.lna_vga_ctl.agc_vga_init_0));
#endif /* RELEASE_EXCLUDE */
...
INT MT76x0_ReadChannelPwr(RTMP_ADAPTER *pAd)
{
...

	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("E2PROM: TxPower[%03d], Channel=%d, Power[Tx:%d]\n",
	...
	UCHAR Level)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s Tx success = %ld\n", 
		__FUNCTION__, (ULONG)pAd->WlanCounters.TransmittedFragmentCount.u.LowPart));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s Rx success = %ld\n", 
	...
	*/
	...
	if (MTxCycle >= 2000)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Check TxQ page count max\n"));
		RTMP_IO_READ32(pAd, 0x0a30, &MacReg);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("0x0a30 = 0x%08x\n", MacReg));

		RTMP_IO_READ32(pAd, 0x0a34, &MacReg);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("0x0a34 = 0x%08x\n", MacReg));

		RTMP_IO_READ32(pAd, 0x438, &MacReg);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("0x438 = 0x%08x\n", MacReg));
		bResetWLAN = TRUE;
	}
	...
	*/
	...
	if (MTxCycle >= 2000)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Check MAC Tx idle max(0x%08x)\n", MacReg));
		bResetWLAN = TRUE;
	}
	...
	*/
	...
	if (MTxCycle >= 2000)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Check RxQ page count max\n"));
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("0x0430 = 0x%08x\n", MacReg));
		bResetWLAN = TRUE;
	}
	...
	*/
	...
	if (MTxCycle >= 2000)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Check MAC Rx idle max(0x%08x)\n", MacReg));
		bResetWLAN = TRUE;
	}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("0x13D4 = 0x%08X, 0x13D8 = 0x%08X, 0x13D4 = 0x%08X\n", 
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: dec_gband_bw40_delta = %d, bw40_gband_delta = %d\n", 
		__FUNCTION__, dec_gband_bw40_delta, bw40_gband_delta));	
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: dec_aband_bw40_delta = %d, bw40_aband_delta = %d\n", 
		__FUNCTION__, dec_aband_bw40_delta, bw40_aband_delta));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: dec_aband_bw80_delta = %d, bw80_aband_delta = %d\n", 
	...
	*/
	...
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx20MPwrCfgGBand[0](0x1314) = 0x%08X\n", __FUNCTION__, data));
	data = (t4 << 24) | (t3 << 16) | (t2 << 8) | t1; 
	pAd->Tx40MPwrCfgGBand[0] = data;
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx40MPwrCfgGBand[0](0x1314) = 0x%08X\n", __FUNCTION__, data));
	...
	*/
	...
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx20MPwrCfgGBand[1](0x1318) = 0x%08X\n", __FUNCTION__, data));
	data = (t4 << 24) | (t3 << 16) | (t2 << 8) | t1; 
	pAd->Tx40MPwrCfgGBand[1] = data; 
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx40MPwrCfgGBand[1](0x1318) = 0x%08X\n", __FUNCTION__, data));
	...
	*/
	...
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx20MPwrCfgGBand[2](0x131C) = 0x%08X\n", __FUNCTION__, data));
	data = (t4 << 24) | (t3 << 16) | (t2 << 8) | t1; 
	pAd->Tx40MPwrCfgGBand[2] = data; 
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx40MPwrCfgGBand[2](0x131C) = 0x%08X\n", __FUNCTION__, data));
	...
	*/
	...
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx20MPwrCfgGBand[3](0x1320) = 0x%08X\n", __FUNCTION__, data));
	data = (t4 << 24) | (t3 << 16) | (t2 << 8) | t1; 
	pAd->Tx40MPwrCfgGBand[3] = data; 
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx40MPwrCfgGBand[3](0x1320) = 0x%08X\n", __FUNCTION__, data));
	...
	*/
	...
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx20MPwrCfgGBand[4](0x1324) = 0x%08X\n", __FUNCTION__, data));
	data = (t2 << 8) | t1;
	pAd->Tx40MPwrCfgGBand[4] = data; 			
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx40MPwrCfgGBand[4](0x1324) = 0x%08X\n", __FUNCTION__, data));
	...
	*/
	...
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx20MPwrCfgABand[0](0x1314) = 0x%08X\n", __FUNCTION__, data));
	data = (t4 << 24) | (t3 << 16); 
	pAd->Tx40MPwrCfgABand[0] = data;
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx40MPwrCfgABand[0](0x1314) = 0x%08X\n", __FUNCTION__, data));	
	...
	*/
	...
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx20MPwrCfgABand[1](0x1318) = 0x%08X\n", __FUNCTION__, data));
	data = (t4 << 24) | (t3 << 16) | (t2 << 8) | t1; 
	pAd->Tx40MPwrCfgABand[1] = data;			
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx40MPwrCfgABand[1](0x1318) = 0x%08X\n", __FUNCTION__, data));
	...
	*/
	...
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx20MPwrCfgABand[2](0x131C) = 0x%08X\n", __FUNCTION__, data));
	data = (t2 << 8) | t1; 
	pAd->Tx40MPwrCfgABand[2] = data;
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx40MPwrCfgABand[2](0x131C) = 0x%08X\n", __FUNCTION__, data));
	...
	*/
	...
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx20MPwrCfgABand[3](0x1320) = 0x%08X\n", __FUNCTION__, data));
	data = (t4 << 24) | (t3 << 16); 
	pAd->Tx40MPwrCfgABand[3] = data;
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx40MPwrCfgABand[3](0x1320) = 0x%08X\n", __FUNCTION__, data));
	...
	*/
	...
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx20MPwrCfgABand[4](0x1324) = 0x%08X\n", __FUNCTION__, data));
	data = (t2 << 8) | t1;
	pAd->Tx40MPwrCfgABand[4] = data;	
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx40MPwrCfgABand[4](0x1324) = 0x%08X\n", __FUNCTION__, data));
	...
	*/
	...
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: Tx80MPwrCfgABand[0](0x13D8) = 0x%08X\n", __FUNCTION__, data));
...
void mt76x0_antenna_sel_ctl(RTMP_ADAPTER *ad)
{
	...
	if (IS_USB_INF(ad)) {
		...
		if (ret != 0) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("WlanEnLock get failed(ret=%d)\n", ret));
			return;
		}
	}
#endif /* RTMP_MAC_USB */
	...
	*/
	...
	if (e2p_val & 0x8000) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - Dual antenna mode\n", __FUNCTION__));
	} else {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - Single antenna mode\n", __FUNCTION__));
	}
...
void mt76x0_vco_calibration(RTMP_ADAPTER *ad, UINT8 channel)
{
	...
	*/
	...
	if (vco_mode == 0x30)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s - Calibration Mode: Open loop, closed loop, and amplitude @ ch%d\n", 
		...
		*/
		...
		if ((channel == 1) || (channel == 7) || (channel == 9) || (channel >= 13))
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s - wrong input channel\n", __FUNCTION__));
			return;
		}
	...
	IN BOOLEAN bSave)
{
	...
	*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s - Channel = %u, param = 0x%x, bSave = %d, cal_mode = %u\n",
	...
	if (cal_mode == FULL_CALIBRATION) {
		...
		*/			
		if (IS_MT76x0E(pAd)) {
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s - DPD_CALIBRATION = 0x%x\n", __FUNCTION__, dpd_val));
		}
	}
}
	...
	IN BOOLEAN bSaveCal)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s - Channel = %d, bPowerOn = %d, bFullCal = %d, bSaveCal = %d\n",
	...
	if (IS_USB_INF(pAd)) {
		...
		if (ret != 0) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("cal_atomic get failed(ret=%d)\n", ret));
			return;
		}
	}
#endif /* RTMP_MAC_USB */
	...
	OUT PCHAR pTssiReport)
{
	...
	{
    	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\x1b[31m%s: FW is accessing RF Regs...\x1b[m\n", __FUNCTION__));
		return FALSE;
	}
	...
	*/
	...
	if (wait >= 2000) {
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Wait 0x2088[4]=0 but time's up, give up this time!\n"));
#endif /* RELEASE_EXCLUDE */
		reg_val &= ~(0x10);
...
void mt76x0_temp_sensor(RTMP_ADAPTER *ad)
{
	...
	*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s - Dout=%d (0x%x), temp_offset=%d (0x%x), current_temp=%d (0x%x)\n",
...
void mt76x0_temp_trigger_cal(RTMP_ADAPTER *ad)
{
	...
	if ((temp_diff > 20) || (temp_diff < -20)) {						
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s - do VCO re-calibration (last_vco_temp=%d, current_temp=%d)\n", 
	...
	if ((temp_diff > 30) || (temp_diff < -30)) {						
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s - do full calibration again (last_full_cal_temp=%d, current_temp=%d)\n", 
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: vendor_id = 0x%04x, device_id = 0x%04x\n", __FUNCTION__, vendor_id, device_id));
	
	RT28xx_EEPROM_READ16(pAd, 0x26, ee_val);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: 0x26 = 0x%x\n", __FUNCTION__, ee_val));
	RT28xx_EEPROM_READ16(pAd, 0x28, ee_val);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: 0x28 = 0x%x\n", __FUNCTION__, ee_val));
	...
	if (pos != 0)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Read 1 (Ralink PCIe Link Control Register) offset 0x%x = 0x%x\n", 
				__FUNCTION__, pAd->RLnkCtrlOffset, Configuration));

		RTMP_IO_READ32(pAd, 0x64, &reg_val);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Read 0x64 = 0x%x\n", __FUNCTION__, reg_val));
		reg_val |= 0x00000020;
		RTMP_IO_WRITE32(pAd, 0x64, reg_val);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: write 0x64 = 0x%x\n", __FUNCTION__, reg_val));

		RTMP_IO_WRITE32(pAd, 0x1F080, pAd->RLnkCtrlConfiguration); /* Please DON'T read it. */
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: write 0x1F080 = 0x%x\n", __FUNCTION__, pAd->RLnkCtrlConfiguration));
		
		Configuration &= 0x103;  /* 0x1: L0s, 0x2: L1, 0x3: L0s+L1 */
		pci_write_config_word(pObj->pci_dev, pAd->RLnkCtrlOffset, Configuration);
		pci_read_config_word(pObj->pci_dev, pAd->RLnkCtrlOffset, &Configuration);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Read 2 (Ralink PCIe Link Control Register) offset 0x%x = 0x%x\n", 
				__FUNCTION__, pAd->RLnkCtrlOffset, Configuration));
	}
	else
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: Cannot locate PCIe Device's Link Control Register Offset!\n", __FUNCTION__));
	return;
}
	...
	IN PRTMP_ADAPTER pAd)
{
...
#endif /* MT76x0_TSSI_CAL_COMPENSATION */
	{
		...
		if ((pAd->DefaultTargetPwr == 0x00) || (pAd->DefaultTargetPwr == 0xFF)) {
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: EEPROM target power Error! Use Default Target Power = 0x%x\n", 
				__FUNCTION__, pAd->DefaultTargetPwr));
		} else {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s: DefaultTargetPwr = %d\n", 
				__FUNCTION__, pAd->DefaultTargetPwr));
		}
		...
		*/
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("SkuBasePwr = 0x%x,  DefaultTargetPwr = 0x%x, ChannelPwrAdj 0x13b4: 0x%x\n", 
	...
	IN  RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("0x%x: 0x%x\n", TX_PWR_CFG_0, reg_val));
...
#ifdef MT76x0_TSSI_CAL_COMPENSATION
VOID mt76x0_tssi_init_target_pwr(RTMP_ADAPTER *pAd)
{
	if (pAd->chipCap.bInternalTxALC)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: target_power = 0x%x, delta_power = 0x%x\n", __FUNCTION__, target_power, delta_power));
		...
		*/
		...
		Value |= (0x2F2F << 16);
		RTMP_IO_WRITE32(pAd, TX_ALC_CFG_0, Value);
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE,("0x13b0: 0x%08x (%d)\n",
	...
	IN  SHORT *pTSSI_Linear)
{
	...
	*/
	...
	if (MTxCycle >= 200)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: We cannot wait too long, give up!\n", __FUNCTION__));
		bbp_val &= ~(0x10);
		RTMP_BBP_IO_WRITE32(pAd, CORE_R34, bbp_val);
		return FALSE;
	}
	...
	*pTSSI_Linear = (CHAR)(bbp_val&0xFF);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: CORE_R35 = 0x%X, TSSI_Linear = (CHAR)(BBP_Value&0xFF) = 0x%X\n", __FUNCTION__, bbp_val, *pTSSI_Linear));

	if (Channel > 14)
	{
		*pTSSI_Linear = *pTSSI_Linear + 128;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: TSSI_Linear = TSSI_Linear + 128 = 0x%X\n", __FUNCTION__, *pTSSI_Linear));
	}
	...
	*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: TSSI_Linear = 0x%X\n", __FUNCTION__, *pTSSI_Linear));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: INFO_1 = 0x%X\n", __FUNCTION__, pAd->chipCap.tssi_info_1));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: INFO_2 = 0x%X\n", __FUNCTION__, pAd->chipCap.tssi_info_2));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: INFO_3 = 0x%X\n", __FUNCTION__, pAd->chipCap.tssi_info_3));
	return TRUE;
}
	...
	*pTSSI_Tx_Mode = (pAd->chipCap.tssi_info_1 & 0x7);
	...
	if (*pTSSI_Tx_Mode == 0)
	{
		...
		*/
		...
		if (Tx_Rate > 3)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s ==> CCK Mode :: Unknown Tx_Rate = %d, return here.\n", __FUNCTION__, Tx_Rate));
			return FALSE;
		}
		...
		*pTargetPA_mode = (CHAR) pAd->chipCap.rate_pwr_table.CCK[Tx_Rate].RF_PA_Mode;
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> CCK Mode :: TargetPower = %d\n", *pTargetPower));
	}
	else if (*pTSSI_Tx_Mode == 1)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s ==> OFDM Mode :: Unknown Tx_Rate = 0x%x, return here.\n", __FUNCTION__, Tx_Rate));
			return FALSE;
		}
		...
		*pTargetPA_mode = pAd->chipCap.rate_pwr_table.OFDM[index].RF_PA_Mode;
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> OFDM Mode :: TargetPower0 = %d (MCS%d)\n", *pTargetPower, index));
	}
	else if (*pTSSI_Tx_Mode == 4)
	{
		...
		if (Tx_Rate > 9)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s ==> VHT Mode :: Unknown Tx_Rate = %d, return here.\n", __FUNCTION__, Tx_Rate));
			return FALSE;
		}

		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> VHT Mode :: CurrentPower0 = %d, pAd->chipCap.tssi_table.VHT[%d].MCS_Power = %d\n", 
		...
		*pTargetPA_mode = (CHAR) pAd->chipCap.rate_pwr_table.VHT[Tx_Rate].RF_PA_Mode;
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> VHT Mode :: TargetPower0 = %d (MCS%d)\n", *pTargetPower, Tx_Rate));
	}
    else
    {
		...
		else
		{
			if (Tx_Rate > 9)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s ==> HT Mode :: Unknown Tx_Rate = %d, return here.\n", __FUNCTION__, Tx_Rate));
				return FALSE;
			}
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> HT Mode :: TargetPower0 = %d (MCS%d)\n", *pTargetPower, Tx_Rate));
		return TRUE;
	}
	return TRUE;
}
	...
	IN  INT *tssi_delta0)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> 1) tssi_offset = %d (0x%x)\n", tssi_offset, tssi_offset));

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("\n"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> EstimateDeltaPower\n"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> TSSI_Tx_Mode = %d\n", TSSI_Tx_Mode));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> TargetPower = %d\n", TargetPower));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> TSSI_Linear = %d\n", TSSI_Linear));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> 2) tssi_offset0 = %d\n", tssi_offset));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> TargetPA_mode = %d\n", TargetPA_mode));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> TXBE_OFFSET+R4 = 0x%X\n", BBP_Value));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> tssi_target = %d\n", (tssi_target) >> 13));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> dB x slope = %d (0x%x), tssi_offset = %d(0x%x)\n", tssi_meas, tssi_meas, tssi_offset, tssi_offset));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> (tssi_offset-50) = (%d)(0x%x)\n", (tssi_offset-50), (tssi_offset-50)));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> (tssi_offset-50)<< 10 = (%d)(0x%x)\n", ((tssi_offset-50) << 10), ((tssi_offset-50) << 10)));
	if( pAd->hw_cfg.cent_ch > 14 )
		tssi_meas += ((tssi_offset-50) << 10); // 5G: offset s4.3
	else
		tssi_meas += (tssi_offset << 9); // 2G: offset s3.4
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> measure db = %d (0x%x) %d\n", tssi_meas, tssi_meas, (tssi_meas) >> 13));
	...
	IN  RTMP_ADAPTER *pAd)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s ==> Get target power failed, return here.\n", __FUNCTION__));
		return;
	}

	RTMP_IO_READ32(pAd, TX_ALC_CFG_1, &reg_val);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("(0x13B4) Before compensation 0x%08X\n", reg_val));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s ==> reg_val = 0x%08X\n", __FUNCTION__, reg_val));
	RTMP_IO_WRITE32(pAd, TX_ALC_CFG_1, reg_val);
	RTMP_IO_READ32(pAd, TX_ALC_CFG_1, &reg_val);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("(0x13B4) After compensation 0x%08X\n", reg_val));
}
#endif /* MT76x0_TSSI_CAL_COMPENSATION */
	...
	OUT PCHAR comp_level)
{
	...
	if ((temp_minus_bdy == NULL) || (temp_plus_bdy == NULL)) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): temp table boundary is NULL\n", __FUNCTION__)); 
		return FALSE;
	}
	...
	if (enable_tx_alc) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): enable_tx_alc = %s\n", 
	...
	if (mt76x0_get_tssi_report(pAd, bResetTssiInfo, &pAd->CurrTemperature) == TRUE) {
					...
					tx_alc_comp) == TRUE) 
		{
			...
			*/     
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): failed to get the compensation level\n", __FUNCTION__)); 
		}
	}
}
	...
	IN CHAR temp_reference)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s: upper_bound = 0x%02x (%d), lower_bound = 0x%02x (%d)\n",
		__FUNCTION__, upper_bound, upper_bound, lower_bound, lower_bound));
	
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("*** %s: %s_temp_bdy_table[-7 .. +7] = %d %d %d %d %d %d %d * %d * %d %d %d %d %d %d %d, temp_reference=%d\n",
		__FUNCTION__,
		(band == A_BAND) ? "5G" : "2.4G",
	...
	IN PCHAR temp_plus_bdy)
{
	...
	if (mp_temp < temp_minus_bdy[1]) {
		...
		*/
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("mp_temperature=0x%02x, step = -%d\n", mp_temp, idx-1));                    
	...
	IN const UINT32 table_size)
{
	...
	if (start_idx < table_size) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): ERROR - incorrect start index (%d)\n",
			__FUNCTION__, start_idx));
		return FALSE;
	}

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): load %s %s table from eeprom 0x%x to 0x%x (start_idx = %d)\n",
		__FUNCTION__, 
		(band == A_BAND) ? "5G" : "2.4G",
	...
	if (table_idx > table_size) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): WARNING - eeprom table may not be loaded completely\n", __FUNCTION__));
		return FALSE;
	} else {	
		for (table_idx = 0; table_idx < table_size; table_idx++)
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("\tboundary_table[%d] = %3d (0x%02X)\n", 
...
void mt76x0_read_tx_alc_info_from_eeprom(PRTMP_ADAPTER pAd)
{
	...
	if (IS_MT76x0(pAd)) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: EEPROM_MT76x0_TEMPERATURE_OFFSET (0xD1) = 0x%x\n", 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: TemperatureOffset = 0x%x\n", 
	...
	if (pAd->chipCap.bInternalTxALC)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: tssi_2G_target_power = %d, tssi_5G_target_power = %d\n", 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: tssi_slope_2G = 0x%x, tssi_offset_2G = 0x%x\n", 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: tssi_slope_5G_Group_1 = 0x%x, tssi_offset_5G_Group_1 = 0x%x\n", 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: tssi_slope_5G_Group_2 = 0x%x, tssi_offset_5G_Group_2 = 0x%x\n", 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: tssi_slope_5G_Group_3 = 0x%x, tssi_offset_5G_Group_3 = 0x%x\n", 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: tssi_slope_5G_Group_4 = 0x%x, tssi_offset_5G_Group_4 = 0x%x\n", 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: tssi_slope_5G_Group_5 = 0x%x, tssi_offset_5G_Group_5 = 0x%x\n", 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: tssi_slope_5G_Group_6 = 0x%x, tssi_offset_5G_Group_6 = 0x%x\n", 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: tssi_slope_5G_Group_7 = 0x%x, tssi_offset_5G_Group_7 = 0x%x\n", 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: tssi_slope_5G_Group_8 = 0x%x, tssi_offset_5G_Group_8 = 0x%x\n", 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: tssi_5G_channel_boundary_1 = %d, tssi_5G_channel_boundary_2 = %d\n", 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: tssi_5G_channel_boundary_3 = %d, tssi_5G_channel_boundary_4 = %d\n", 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: tssi_5G_channel_boundary_5 = %d, tssi_5G_channel_boundary_6 = %d\n", 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: tssi_5G_channel_boundary_7 = %d\n", 
	...
	if (pAd->bAutoTxAgcG | pAd->bAutoTxAgcA) {	
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): channel boundary index = %u, temp reference offset = %d\n",
...
#endif /* RTMP_TEMPERATURE_COMPENSATION */
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("No TSSI or Temperature Tx ALC not enabled\n"));
}
...
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s: after compensation - MAC offset = 0x%04X, CR value = 0x%08lX\n",
...
static void mt76x0_show_pwr_info(RTMP_ADAPTER *ad)
{
	...
	if ( ch_init_pwr & 0x20 )
		ch_init_pwr -= 64;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s - 0x%08X = 0x%08X, ch_init_pwr = %d\n",
	...
	if ( ch_delta_pwr & 0x20 )
		ch_delta_pwr -= 64;	
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s - 0x%08X = 0x%08X, ch_delta_pwr = %d\n",
							__FUNCTION__, TX_ALC_CFG_1, data, ch_delta_pwr));

	/* TX_PWR_CFG_0, MAC 0x1314 */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\t - Tx20MPwrCfgGBand[0] = 0x%08lX\n", ad->Tx20MPwrCfgGBand[0]));
	/* TX_PWR_CFG_1, MAC 0x1318 */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\t - Tx40MPwrCfgGBand[0] = 0x%08lX\n", ad->Tx40MPwrCfgGBand[0]));
	/* TX_PWR_CFG_2, MAC 0x131C */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\t - Tx40MPwrCfgGBand[1] = 0x%08lX\n", ad->Tx40MPwrCfgGBand[1]));
	/* TX_PWR_CFG_3, MAC 0x1320 */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\t - Tx40MPwrCfgGBand[2] = 0x%08lX\n", ad->Tx40MPwrCfgGBand[2]));
	/* TX_PWR_CFG_2, MAC 0x1324 */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\t - Tx40MPwrCfgGBand[3] = 0x%08lX\n", ad->Tx40MPwrCfgGBand[3]));
	/* TX_PWR_CFG_0, MAC 0x1314 */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\t - Tx20MPwrCfgABand[0] = 0x%08lX\n", ad->Tx20MPwrCfgABand[0]));
	/* TX_PWR_CFG_1, MAC 0x1318 */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\t - Tx40MPwrCfgABand[0] = 0x%08lX\n", ad->Tx40MPwrCfgABand[0]));
	/* TX_PWR_CFG_2, MAC 0x131C */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\t - Tx40MPwrCfgABand[1] = 0x%08lX\n", ad->Tx40MPwrCfgABand[1]));
	/* TX_PWR_CFG_3, MAC 0x1320 */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\t - Tx40MPwrCfgABand[2] = 0x%08lX\n", ad->Tx40MPwrCfgABand[2]));
	/* TX_PWR_CFG_2, MAC 0x1324 */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\t - Tx40MPwrCfgABand[3] = 0x%08lX\n", ad->Tx40MPwrCfgABand[3]));
#ifdef DOT11_VHT_AC
	/* TX_PWR_CFG_8, MAC 0x13D8 */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\t - Tx80MPwrCfgABand[0] = 0x%lX\n", ad->Tx80MPwrCfgABand[0]));
#endif /* DOT11_VHT_AC */
	
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s - Per-Rate Power:\n", __FUNCTION__));
	for (index = 0; tx_pwr_cfg_index[index] != 0; index++)
	{
		RTMP_IO_READ32(ad, tx_pwr_cfg_index[index], &data);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\t - 0x%x: 0x%08x\n", tx_pwr_cfg_index[index], data));
	}
...
INT Set_AntennaSelect_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	*/
	...
	CHIP_CALIBRATION(pAd, RXDCOC_CALIBRATION, 1);
#endif
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_AntennaSelect_Proc:: AntennaSelect = %d (0x%08x=0x%08x)\n", val, AGC1_R0, reg_val));
