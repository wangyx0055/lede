	...
	IN UCHAR InitTxRateIdx)
{
	...
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("AGS: ---> %s\n", __FUNCTION__));
	...
	do 
	{
		...
		{
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA,
			...
			{
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: ##############\n", __FUNCTION__));
			...
			)
		{
			...
			if ((pEntry->AGSCtrl.MCSGroup == 0) && 
			     ((pEntry->TxQuality[pCurrTxRate->upMcs2] > pEntry->TxQuality[pCurrTxRate->upMcs1]) &&
					(pCurrTxRate->upMcs1 != pCurrTxRate->ItemNo)))
			{
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("%s: ###########\n", __FUNCTION__));
		...
		if (UpRateIdx == pEntry->CurrTxRateIndex)
		{
			...
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("ags> Current rate is the best one!\n"));
			break;
		}
	...
	if (pAGSStatisticsInfo->AccuTxTotalCnt <= 15)
	{
		...
		)
		{
			...
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("%s: AGS: 2*2, RssiOffsetForAgs=%d\n", __FUNCTION__, RssiOffset));
...
#endif /* DOT11_VHT_AC */
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("%s: AGS: 1*1, RssiOffset=%d\n", __FUNCTION__, RssiOffset));
	...
	if (pEntry->fLastSecAccordingRSSI == TRUE)
	{
		...
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("%s: AGS: The MCS selection base on RSSI\n", __FUNCTION__));

		return;
	}

	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("%s: AGS: TrainUp:%d, TrainDown:%d\n", __FUNCTION__, TrainUp, TrainDown));
	...
	do
	{
		...
		else if (pEntry->AGSCtrl.MCSGroup > 0) /* even if TxErrorRatio > TrainUp */
		{
			...
			if (UpRateIdx != 0)
			{
				...
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("ags> not bad and not good\n"));
			}
		}
		...
		if (bTrainUpDown)
		{
			     ...
			     (pEntry->TxQuality[CurrRateIdx] >= AGS_TX_QUALITY_WORST_BOUND))
			{
				...
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("ags> rate down!\n"));
			}
					...
					(pEntry->TxQuality[UpRateIdx] <= 0)) /* Upgrade Tx rate */
			{
				...
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("ags> rate up!\n"));
			}
		}
	} while (FALSE);

	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("%s: AGS: pEntry->CurrTxRateIndex = %d, CurrRateIdx = %d, pEntry->LastSecTxRateChangeAction = %d\n", 
		__FUNCTION__, 
	...
	if ((pEntry->CurrTxRateIndex != CurrRateIdx) && 
	     (pEntry->LastSecTxRateChangeAction == RATE_UP))
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("%s: AGS: ++TX rate from %d to %d\n", 
	...
	else /* Tx rate remains unchanged. */
	{
		...
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("ags> no rate up/down!\n"));
	}
	...
	if (INFRA_ON(pAd))
	{
									...
									fCLIENT_STATUS_RDG_CAPABLE)) /* RDG capable */
		{
			     ...
			     ((pAd->RalinkCounters.OneSecReceivedByteCount + pAd->RalinkCounters.OneSecTransmittedByteCount) >= (50 * 1024)))
			{
				...
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("AGS: %s: RDG_IN_THRES = 0xFF\n", __FUNCTION__));
			}
			else
			{
				RTMP_IO_READ32(pAd, TX_LINK_CFG, &TxLinkCfg.word);
				...
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("AGS: %s: RDG_IN_THRES = 0x00\n", __FUNCTION__));
			}
		}
	}
	...
	IN UCHAR InitTxRateIdx)
{
	...
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("QuickAGS: ---> %s\n", __FUNCTION__));
	...
	do
	{
		...
		if ((pEntry->LastSecTxRateChangeAction == 1) && (CurrRateIdx != DownRateIdx))
		{
			...
			if (pAGSStatisticsInfo->TxErrorRatio > TrainDown)  /* Poor quality */
			{
				...
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("%s: QuickAGS: (UP) bad Tx ok count (Current PER:%d, NewMcs's TrainDown:%d)\n",
					__FUNCTION__, 
					pAGSStatisticsInfo->TxErrorRatio, TrainDown));
			}
			else /* Good quality */
			{
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("%s: QuickAGS: (UP) keep rate-up (Current PER:%d, NewMcs's TrainDown:%d)\n", 
					__FUNCTION__, 
					pAGSStatisticsInfo->TxErrorRatio,
					TrainDown));
...
// Don't quick check within train down case
			...
			if (pAGSStatisticsInfo->TxErrorRatio > TrainDown)  // Poor quality
			{
				...
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("%s: QuickAGS: (DOWN) bad tx ok count (Current PER:%d, NewMcs's TrainDown:%d)\n", 
			...
			else
			{
				...
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("%s: QuickAGS: (Down) keep rate-down (Current PER:%d, NewMcs's TrainDown:%d)\n", 
					__FUNCTION__, pAGSStatisticsInfo->TxErrorRatio, TrainDown));
			}
#endif			
		}
	...
	if (pEntry->LastSecTxRateChangeAction == 1) 
	{
		...
		switch (pEntry->AGSCtrl.MCSGroup)
		{
			...
			default: 
			{
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("%s: QuickAGS: Incorrect MCS group, pEntry->AGSCtrl.MCSGroup = %d\n", 
					__FUNCTION__, pEntry->AGSCtrl.MCSGroup));
			}
			break;
		}
		...
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("%s: QuickAGS: next MCS group,  pEntry->AGSCtrl.MCSGroup = %d\n", 
			__FUNCTION__, pEntry->AGSCtrl.MCSGroup));
	...
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("%s: QuickAGS: ++TX rate from %d to %d\n", 
			__FUNCTION__, CurrRateIdx, pEntry->CurrTxRateIndex));	
	...
	{
		bTxRateChanged = FALSE;
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("%s: QuickAGS: rate is not changed\n", 
			__FUNCTION__));
	}
	...
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("QuickAGS: <--- %s\n", __FUNCTION__));
}
	...
	IN INT idx)
{
	...
	if (AGSStatisticsInfo.AccuTxTotalCnt <= 15)
	{
			...
			while (idx < pTable[0])
			{
				...
				if (pCurrTxRate->Mode == MODE_VHT)
				{
					if (pCurrTxRate->Nss == NSS_1)
						...
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():Invalid Nss(%d)\n", __FUNCTION__, pCurrTxRate->Nss));
						mcs_idx_offset = 0;
					}
		...
		)
		{
			...
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s: AGS: 2*2, RssiOffsetForAgs=%d\n", __FUNCTION__, RssiOffset));
	...
	do
	{
		...
		if (bTrainUpDown)
		{
			     ...
			     (pEntry->TxQuality[CurrRateIdx] >= AGS_TX_QUALITY_WORST_BOUND))
			{
				pEntry->CurrTxRateIndex = DownRateIdx;
				pEntry->LastSecTxRateChangeAction = RATE_DOWN;
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("ags> rate down!\n"));
			}
			else if ((CurrRateIdx != UpRateIdx) && 
					(pEntry->TxQuality[UpRateIdx] <= 0)) /* Upgrade Tx rate */
			{
				pEntry->CurrTxRateIndex = UpRateIdx;
				pEntry->LastSecTxRateChangeAction = RATE_UP;
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("ags> rate up!\n"));
			}
		}
	} while (FALSE);
...
// Don't quick check within train down case
	...
	if ((bTxRateChanged == TRUE) && (pNextTxRate != NULL))
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO | DBG_FUNC_RA, ("ags> confirm current rate MCS = %d!\n", pEntry->CurrTxRateIndex));
