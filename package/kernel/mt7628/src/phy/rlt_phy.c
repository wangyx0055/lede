...
static INT rlt_bbp_is_ready(struct _RTMP_ADAPTER *pAd)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BBP version = %x\n", val));
	}
...
static INT rlt_bbp_init(RTMP_ADAPTER *pAd)
{
	...
	if (pAd->chipCap.pBBPRegTable)
	{
		...
		for (idx = 0; idx < pAd->chipCap.bbpRegTbSize; idx++)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("BBP[%x]=0x%x\n", 
...
static INT rlt_bbp_tx_comp_init(RTMP_ADAPTER *pAd, INT adc_insel, INT tssi_mode)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[temp. compensation] Set RF_R27 to 0x%x\n", rf_val));
#endif
	return 0;
}
...
//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): rxpath=%d, Set AGC1_R0=0x%x, agc_r0=0x%x\n", __FUNCTION__, rxpath, agc, agc_r0));
//		RTMP_BBP_IO_READ32(pAd, AGC1_R0, &agc);
//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): rxpath=%d, After write, Get AGC1_R0=0x%x,\n", __FUNCTION__, rxpath, agc));
...
//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): ext_ch=%d, Set AGC1_R0=0x%x, agc_r0=0x%x\n", __FUNCTION__, ext_ch, agc, agc_r0));
//		RTMP_BBP_IO_READ32(pAd, AGC1_R0, &agc);
//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): ext_ch=%d, After write, Get AGC1_R0=0x%x,\n", __FUNCTION__, ext_ch, agc));
...
static INT rlt_bbp_set_bw(struct _RTMP_ADAPTER *pAd, UINT8 bw)
{
...
#ifdef MT76x0
	// TODO: shiang, check for andes set bw code
	if (IS_MT7610U(pAd)) {
	...
	if (core != core_r1) {
#if 0
		RTMP_BBP_IO_READ32(pAd, CORE_R4, &core_r1);
		core_r1 |= 0x1;
		RTMP_BBP_IO_WRITE32(pAd, CORE_R4, core_r1);
		RtmpusecDelay(10);
#endif
#ifdef MT76x0
#if 0
		/* After check with shiang, RT85592 doesn't need to do delay. */
#endif
		if (IS_MT76x0(pAd))
		{
			if (bw == BW_80)
	...
	if ((pAd->CommonCfg.lna_vga_ctl.bDyncVgaEnable) && 
		...
		if (IS_MT76x2(pAd)) {
			if (dynamic_channel_model_adjust(pAd) == TRUE) {
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s:: no need to do dynamic vga\n", __FUNCTION__));			
				return;
			}
