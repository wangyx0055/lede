...
#if defined(RT65xx) || defined(MT7603) || defined(MT7628) || defined(MT7636)
	if (IS_RT65XX(pAd) || IS_MT7603(pAd) || IS_MT7628(pAd) || IS_MT76x6(pAd)) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): This chip(0x%x) should not call this function!\n",
...
static INT rtmp_bbp_init(RTMP_ADAPTER *pAd)
{
	...
	for (Index = 0; Index < NUM_BBP_REG_PARMS; Index++)
	{
		...
		if (IS_MT7601(pAd)) {
			if (	BBPRegTable[Index].Register == BBP_R65)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Stored_BBP_R65=%x @%s \n",
	...
	if (pAd->chipCap.pBBPRegTable)
	{
		...
		for (Index = 0; Index < pAd->chipCap.bbpRegTbSize; Index++)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BBP_R%d=0x%x\n", 
					reg_list[Index].Register, 
					reg_list[Index].Value));
		}
	}
