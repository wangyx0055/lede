    ...
    IN UCHAR InitTxRateIdx)
{
    ...
    DBGPRINT(RT_DEBUG_INFO, ("%s: QuickAGS: AccuTxTotalCnt = %d, TxSuccess = %d, TxRetransmit = %d, TxFailCount = %d, TxErrorRatio = %d\n",
        __FUNCTION__, 
    ...
    do 
    {
        ...
        if (UpRateIdx == pEntry->CurrTxRateIndex)
        {
            ...
            DBGPRINT_RAW(RT_DEBUG_INFO, ("ags> Current rate is the best one!\n"));
            break;
        }
    ...
    if (pAGSStatisticsInfo->AccuTxTotalCnt <= 15)
    {
        ...
        )
        {
            ...
            if (pTable == Ags2x2VhtRateTable)
                RssiOffset = AgsRssiOffsetTable[1][pPort->BBPCurrentBW];
#endif /* DOT11_VHT_AC */

            DBGPRINT_RAW(RT_DEBUG_INFO, ("%s: AGS: 2*2, RssiOffsetForAgs=%d\n", __FUNCTION__, RssiOffset));
        ...
        else
        {
            ...
            if (pTable == Ags1x1VhtRateTable)
                RssiOffset = AgsRssiOffsetTable[0][pPort->BBPCurrentBW];
#endif /* DOT11_VHT_AC */
            DBGPRINT_RAW(RT_DEBUG_INFO, ("%s: AGS: 1*1, RssiOffset=%d\n", __FUNCTION__, RssiOffset));
    ...
    if (pEntry->fLastSecAccordingRSSI == TRUE)
    {
        ...
        DBGPRINT_RAW(RT_DEBUG_INFO, ("%s: AGS: The MCS selection base on RSSI\n", __FUNCTION__));

        return;
    }

    DBGPRINT_RAW(RT_DEBUG_INFO, ("%s: AGS: TrainUp:%d, TrainDown:%d\n", __FUNCTION__, TrainUp, TrainDown));
    ...
    do
    {
        ...
        else if (pEntry->AGSCtrl.MCSGroup > 0) /* even if TxErrorRatio > TrainUp */
        {
            ...
            if (UpRateIdx != 0)
            {
                ...
                DBGPRINT_RAW(RT_DEBUG_INFO, ("ags> not bad and not good\n"));
            }
        }
    ...
    DBGPRINT_RAW(RT_DEBUG_INFO, ("%s: AGS: pEntry->CurrTxRateIndex = %d, CurrRateIdx = %d, pEntry->LastSecTxRateChangeAction = %d\n", 
        __FUNCTION__, 
    ...
    if ((pEntry->CurrTxRateIndex != CurrRateIdx) && 
         (pEntry->LastSecTxRateChangeAction == RATE_UP))
    {
        DBGPRINT_RAW(RT_DEBUG_INFO, ("%s: AGS: ++TX rate from %d to %d\n", 
    ...
    else /* Tx rate remains unchanged. */
    {
        ...
        DBGPRINT_RAW(RT_DEBUG_INFO, ("ags> no rate up/down!\n"));
    }
    ...
    {
#if (OS_WIN==1)  
        PMAC_TABLE_ENTRY BssidMacEntry = MlmeSyncMacTabMatchedRoleType(pPort, ROLE_WLANIDX_BSSID);
        if ((pPort->CommonCfg.bRdg == TRUE) &&
            CLIENT_STATUS_TEST_FLAG(BssidMacEntry,
...
#endif
        {
                 ...
                 ((pAd->Counter.MTKCounters.OneSecReceivedByteCount + pAd->Counter.MTKCounters.OneSecTransmittedByteCount) >= (50 * 1024)))
            {
                ...
                DBGPRINT_RAW(RT_DEBUG_INFO, ("AGS: %s: RDG_IN_THRES = 0xFF\n", __FUNCTION__));
            }
            else
            {
                RTMP_IO_READ32(pAd, TX_LINK_CFG, &TxLinkCfg.word);
                ...
                DBGPRINT_RAW(RT_DEBUG_INFO, ("AGS: %s: RDG_IN_THRES = 0x00\n", __FUNCTION__));
            }
        }
    }
    ...
    do
    {
        ...
        if ((pEntry->LastSecTxRateChangeAction == 1) && (CurrRateIdx != DownRateIdx))
        {
            ...
            if (pAGSStatisticsInfo->TxErrorRatio > TrainDown)  /* Poor quality */
            {
                ...
                DBGPRINT_RAW(RT_DEBUG_INFO, ("%s: QuickAGS: (UP) bad Tx ok count (Current PER:%d, NewMcs's TrainDown:%d)\n",
                    __FUNCTION__, 
                    pAGSStatisticsInfo->TxErrorRatio, TrainDown));
            }
            else /* Good quality */
            {
                DBGPRINT_RAW(RT_DEBUG_INFO, ("%s: QuickAGS: (UP) keep rate-up (Current PER:%d, NewMcs's TrainDown:%d)\n", 
                    __FUNCTION__, 
                    pAGSStatisticsInfo->TxErrorRatio,
                    TrainDown));
...
// Don't quick check within train down case
            ...
            if (pAGSStatisticsInfo->TxErrorRatio > TrainDown)  // Poor quality
            {
                ...
                DBGPRINT_RAW(RT_DEBUG_INFO, ("%s: QuickAGS: (DOWN) bad tx ok count (Current PER:%d, NewMcs's TrainDown:%d)\n", 
                    __FUNCTION__, pAGSStatisticsInfo->TxErrorRatio, TrainDown));
            }
            else
            {
                if (pTable == AGS3x3HTRateTable)
                ...
                DBGPRINT_RAW(RT_DEBUG_INFO, ("%s: QuickAGS: (Down) keep rate-down (Current PER:%d, NewMcs's TrainDown:%d)\n", 
                    __FUNCTION__, pAGSStatisticsInfo->TxErrorRatio, TrainDown));
            }
#endif          
        }
    ...
    if (pEntry->LastSecTxRateChangeAction == 1) 
    {
        ...
        switch (pEntry->AGSCtrl.MCSGroup)
        {
            ...
            default: 
            {
                DBGPRINT_RAW(RT_DEBUG_INFO, ("%s: QuickAGS: Incorrect MCS group, pEntry->AGSCtrl.MCSGroup = %d\n", 
                    __FUNCTION__, pEntry->AGSCtrl.MCSGroup));
            }
            break;
        }
        ...
        DBGPRINT_RAW(RT_DEBUG_INFO, ("%s: QuickAGS: next MCS group,  pEntry->AGSCtrl.MCSGroup = %d\n", 
            __FUNCTION__, pEntry->AGSCtrl.MCSGroup));
    ...
    {
        DBGPRINT_RAW(RT_DEBUG_INFO, ("%s: QuickAGS: ++TX rate from %d to %d\n", 
            __FUNCTION__, CurrRateIdx, pEntry->CurrTxRateIndex));   
    ...
    {
        bTxRateChanged = FALSE;
        DBGPRINT_RAW(RT_DEBUG_INFO, ("%s: QuickAGS: rate is not changed\n", 
            __FUNCTION__));
    }
    ...
    DBGPRINT_RAW(RT_DEBUG_INFO, ("QuickAGS: <--- %s\n", __FUNCTION__));
}
    ...
    IN INT idx)
{
...
#if (OS_WIN==1)    
    PMP_PORT pPort = pAd->PortList[FXXK_PORT_0];
    MAC_TABLE_ENTRY *pEntry = &pPort->MacTab.Content[idx];
#else
    ...
    DBGPRINT(RT_DEBUG_INFO, ("%s: QuickAGS: AccuTxTotalCnt = %ld, TxSuccess = %ld, TxRetransmit = %ld, TxFailCount = %ld, TxErrorRatio = %ld\n",
        __FUNCTION__, 
    ...
    DBGPRINT(RT_DEBUG_INFO, ("%s: AGS: Rssi = %d, TxSuccess = %lu, TxRetransmit = %lu, TxFailCount = %lu, TxErrorRatio = %lu\n", 
        __FUNCTION__, 
    ...
    do
    {
        ...
        if (bTrainUpDown)
        {
                 ...
                 (pEntry->TxQuality[CurrRateIdx] >= AGS_TX_QUALITY_WORST_BOUND))
            {
                pEntry->CurrTxRateIndex = DownRateIdx;
                pEntry->LastSecTxRateChangeAction = RATE_DOWN;
                DBGPRINT_RAW(RT_DEBUG_INFO, ("ags> rate down!\n"));
            }
            else if ((CurrRateIdx != UpRateIdx) && 
                    (pEntry->TxQuality[UpRateIdx] <= 0)) /* Upgrade Tx rate */
            {
                pEntry->CurrTxRateIndex = UpRateIdx;
                pEntry->LastSecTxRateChangeAction = RATE_UP;
                DBGPRINT_RAW(RT_DEBUG_INFO, ("ags> rate up!\n"));
            }
        }
    } while (FALSE);
    ...
    /* Tx rate fast train up/down */
    if ((bTxRateChanged == TRUE) &&
        (!pAd->SoftAP.ApCfg.ApQuickResponeForRateUpTimerRunning))
    {
    ...
    IN INT idx)
{
    ...
    DBGPRINT_RAW(RT_DEBUG_TRACE, ("QuickAGS: ---> %s\n", __FUNCTION__));
    pAd->SoftAP.ApCfg.ApQuickResponeForRateUpTimerRunning = FALSE;
#if (OS_WIN==1)     
    pEntry = MlmeSyncMacTabMatchedWlanIdx(pPort, idx);  /* point to information of the individual station */
...
// Don't quick check within train down case
            ...
            if (pAGSStatisticsInfo->TxErrorRatio > TrainDown)  // Poor quality
            {
                ...
                DBGPRINT_RAW(RT_DEBUG_INFO,
                    ("%s: QuickAGS: (DOWN) bad tx ok count (Current PER:%d, NewMcs's TrainDown:%d)\n", 
                    __FUNCTION__, pAGSStatisticsInfo->TxErrorRatio, TrainDown));
            }
            else
            {
    ...
    if ((bTxRateChanged == TRUE) && (pNextTxRate != NULL))
    {
        DBGPRINT_RAW(RT_DEBUG_INFO, ("ags> confirm current rate MCS = %d!\n", pEntry->CurrTxRateIndex));
