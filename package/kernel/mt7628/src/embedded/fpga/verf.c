...
static INT asic_pbf_reset(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): Do PBF reset\n",__FUNCTION__));
...
static INT asic_cap_mode_set(RTMP_ADAPTER *pAd, BOOLEAN cap)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): cap=%d, mac_val=0x%x\n",
	...
	RTMP_IO_READ32(pAd, RLT_PBF_CFG, &mac_val);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): Config CaptureMode for RLT_PBF_CFG=0x%x\n",
...
VOID cap_dump(RTMP_ADAPTER *pAd, CHAR *bank1, CHAR *bank2, INT len)
{
	...
	for (i = 0; i < len; i++)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s", str_buf));
...
static INT asic_cap_buf_get(RTMP_ADAPTER *pAd, INT bank, UCHAR *buf, UINT32 cap_offset)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): Read Bank %d Info, startAddr=0x%x, endAddr=0x%x, len=%d\n",
...
INT cap_status_chk_and_get(RTMP_ADAPTER *pAd)
{
	...
	if (pAd->fpga_ctl.do_cap && (pAd->fpga_ctl.cap_done == FALSE)) {
		...
		if ((mac_val & 0x10000) == 0x0) {
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s():Capture done(DSCOPE_STS=0x%x, cap_offset=0x%x):\n",
	...
	if (pAd->fpga_ctl.cap_done && (read_cap == TRUE))
	{
		...
		if (!mem)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(%d):malloc failed\n",
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s():Capture Info:\n", __FUNCTION__));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tCaptureMode=%s\n", 
					(pAd->fpga_ctl.cap_type == CAP_MODE_ADC6 ? "ADC6" : 
					(pAd->fpga_ctl.cap_type == CAP_MODE_ADC8 ? "ADC8" : "FEQ"))));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tTriggerMode=%s\n",
					pAd->fpga_ctl.cap_trigger == CAP_TRIGGER_AUTO ? "Auto" : "Manual"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tTriggerOffset=%d\n", pAd->fpga_ctl.trigger_offset));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tStartAddr=0x%x\n", s_addr));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tEndAddr=0x%x\n", e_addr));
...
INT asic_cap_start(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): Config CaptureMode for CAP Registers\n", __FUNCTION__));
	RTMP_BBP_IO_READ32(pAd, DSC_R0, &bbp_val);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tDSC_RO=0x%x\n", bbp_val));
	RTMP_IO_READ32(pAd, DSCOPE_CTL, &mac_val);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tDSCOPE_CTL=0x%x\n", mac_val));
	RTMP_IO_READ32(pAd, CAP_CTL, &mac_val);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tCAP_CTL=0x%x\n", mac_val));
	RTMP_IO_READ32(pAd, DSCOPE_STS, &mac_val);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tDSCOPE_STS=0x%x\n", mac_val));
...
VOID cap_mode_init(RTMP_ADAPTER *pAd)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s():cap buffer allocation failed!\n", __FUNCTION__));
		return;
	}
...
VOID cap_mode_deinit(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("CaptureMode=%d\n", pAd->fpga_ctl.cap_support));
