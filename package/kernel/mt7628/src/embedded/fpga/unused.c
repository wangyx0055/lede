...
/*	PULONG		pTxD;*/
	...
	switch (QueIdx)
	{
		case QID_AC_BE:
			...
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("All QID_AC_BE DESCRIPTOR  \n "  ));
			for (i=0;i<TX_RING_SIZE;i++)
			{
				ptemp= (PULONG)pAd->TxRing[QID_AC_BE].Cell[i].AllocVa;
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("[%02d]  %08lx: %08lx: %08lx: %08lx\n " , i, *ptemp,*(ptemp+1),*(ptemp+2),*(ptemp+3)));
			}
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("  \n "  ));
			break;
		case QID_AC_BK:
			...
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("All QID_AC_BK DESCRIPTOR  \n "  ));
			for (i=0;i<TX_RING_SIZE;i++)
			{
		...
		case QID_AC_VI:
			...
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("All QID_AC_VI DESCRIPTOR \n "  ));
			for (i=0;i<TX_RING_SIZE;i++)
			{
		...
		case QID_AC_VO:
			...
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("All QID_AC_VO DESCRIPTOR \n "  ));
			for (i=0;i<TX_RING_SIZE;i++)
			{
		...
		case QID_MGMT:
			...
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, (" All QID_MGMT  DESCRIPTOR \n "  ));
    ...
    if (QueIdx < NUM_OF_TX_RING) {
    	...
    	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("TxRing%d, TX_DTX_IDX=%d, TX_CTX_IDX=%d\n", QueIdx, Ac0HwIdx, Ac0SwIdx));
    	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE,(" 	TxSwFreeIdx[%d]", AC0freeIdx));
    	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE,("	pending-NDIS=%ld\n", pAd->RalinkCounters.PendingNdisPacketCount));
...
/*	RXD_STRUC *pRxD;*/
	...
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("All RX DSP  \n "  ));
	for (i=0;i<RX_RING_SIZE;i++)
	{
		ptemp = (UINT32 *)pAd->RxRing[0].Cell[i].AllocVa;
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("[%02d]  %08x: %08x: %08x: %08x\n " , i, *ptemp,*(ptemp+1),*(ptemp+2),*(ptemp+3)));
	}
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("RxRing[0], RX_DRX_IDX=%d, RX_CRX_IDX=%d \n", Ac0HwIdx, Ac0SwIdx));
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE,(" 	RxSwReadIdx [%d]=", AC0freeIdx));
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE,("	pending-NDIS=%ld\n", pAd->RalinkCounters.PendingNdisPacketCount));
}
