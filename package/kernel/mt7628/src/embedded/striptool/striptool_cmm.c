...
{
	FILE *ConfigFile;
    char Command[255], Line[2000], ReleaseEmbeddedPath[255];
    int index;
	...
	strncpy(ReleaseEmbeddedPath, pReleasePath, 255);
	strcat(ReleaseEmbeddedPath, "embedded");
...
#ifdef ECOS
    sprintf(Command, "mv %s/embedded/os/ecos/Makefile %s/embedded/os/ecos/Makefile.6", pBuildDirectory, pBuildDirectory);
    system(Command);
#endif /* ECOS */
...
#ifdef ECOS
    sprintf(Command, "mv %s/os/ecos/Makefile.6 %s/os/ecos/Makefile", ReleaseEmbeddedPath, ReleaseEmbeddedPath);
    system(Command);
#endif /* ECOS */
    ...
    sprintf(Command, "mv %s/Makefile %s/Makefile.old", ReleaseEmbeddedPath, ReleaseEmbeddedPath);
    system(Command);
    sprintf(ReadFileName, "%s/Makefile.old", ReleaseEmbeddedPath);
    sprintf(WriteFileName, "%s/Makefile", ReleaseEmbeddedPath);
    ...
    sprintf(Command, "rm -Rf %s/Makefile.old", ReleaseEmbeddedPath);
    system(Command);    

    /* Handle Special Tags - os/linuc/config.mk */
    sprintf(Command, "mv %s/%s %s/%s.old", ReleaseEmbeddedPath, CONFIG_MK, ReleaseEmbeddedPath, CONFIG_MK);
    system(Command);
    sprintf(ReadFileName, "%s/%s.old", ReleaseEmbeddedPath, CONFIG_MK);
    sprintf(WriteFileName, "%s/%s", ReleaseEmbeddedPath, CONFIG_MK);
	...
	sprintf(Command, "rm -Rf %s/%s.old", ReleaseEmbeddedPath, CONFIG_MK);
    system(Command);
#endif
}
