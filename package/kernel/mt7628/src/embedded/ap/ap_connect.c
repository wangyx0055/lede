...
VOID APMakeBssBeacon(RTMP_ADAPTER *pAd, INT apidx)
{
	...
	if (pMbss->bcn_buf.BeaconPkt == NULL) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():Invalid BeaconPkt for MBSS[%d]\n",
					__FUNCTION__, apidx));
		return;
	}
...
//Carter, 20140306 for MT7603, merge MakeAllBeacon into UpdateAllBeacon
        ...
        if (pMbss->bcn_buf.bcn_state != BCN_TX_UNINIT) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s()=>BSS%d:BcnPkt not init(%d)!\n",
...
VOID APUpdateBeaconFrame(RTMP_ADAPTER *pAd, INT apidx)
{
	...
	if (pAd->fpga_ctl.no_bcn) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s():Bcn Tx is blocked!\n", __FUNCTION__));
		return;
	}
#endif /* MT_MAC */
	...
	if (pAd->chipCap.hif_type == HIF_MT) {
        ...
        if (FreeNum <= 0) {
            MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s()=>BSS%d:BcnRing FreeNum is not enough!\n",
						__FUNCTION__, apidx));
            return;
        }
...
VOID APUpdateAllBeaconFrame(RTMP_ADAPTER *pAd)
{
		...
		(FlgQloadIsAlarmIssued == TRUE)))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("DTIM Period reached, BSS20WidthReq=%d, Intolerant40=%d!\n",
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("\tNow RecomWidth=%d, ExtChanOffset=%d, prevBW=%d, prevExtOffset=%d\n",
