...
/*#endif // WIFI_TEST */
	...
	trace_iparp_check_table(&pAd->iparp_trace_tx, TRACE_IPARP_TX_MASK);
	trace_iparp_check_table(&pAd->iparp_trace_rx, TRACE_IPARP_RX_MASK);
#endif
}
...
#ifdef RELEASE_EXCLUDE
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO,("SUBTYPE_ASSOC_REQ\n"));
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("AUTH seq=%d Octet=%02x %02x %02x %02x %02x %02x %02x %02x\n",
					Seq,
		...
		case SUBTYPE_ACTION_NO_ACK:
			*Machine = ACTION_STATE_MACHINE;
			...
			if ((pFrame->Octet[0] & 0x7F) == MT2_ACT_VENDOR) /*  Sometimes Sta will return with category bytes with MSB = 1, if they receive catogory out of their support */
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Vendor Action frame OUI= 0x%x\n", *(PULONG)&pFrame->Octet[1]));
				/* Now support WFA P2P */
				if (RTMPEqualMemory(&pFrame->Octet[1], P2POUIBYTE, 4) && (P2P_INF_ON(pAd)))
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Vendor Action frame P2P OUI= 0x%x\n", *(PULONG)&pFrame->Octet[1]));
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("APAsicRxAntEvalTimeout : RealRxPath=%d, Rssi=%d, %d, %d \n",
