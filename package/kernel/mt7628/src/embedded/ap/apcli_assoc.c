	...
	IN PVOID SystemSpecific3)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Repeater Cli ASSOC - enqueue APCLI_MT2_ASSOC_TIMEOUT\n"));
		...
		*/
...
#endif /* DOT11_VHT_AC */
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : Force AP-client as Non-HT mode\n", __FUNCTION__));
		}
		...
		if (pAd->bApCliCertTest == TRUE)
		{
			    ...
			    && (pAd->CommonCfg.Channel <= 14)) 
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: BssCoexistMgmtSupport = 1\n", __FUNCTION__));
			}
			MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen,
					1, &ExtCapIe,
  					1, &extInfoLen,
		...
		if(MAC_ADDR_EQUAL(Addr2, pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.Bssid))
		{
			...
			if(Status == MLME_SUCCESS) 
			{
...
#endif /* MAC_REPEATER_SUPPORT */
				{
					...
					{
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("There is vht le at Assoc Rsp ifIndex=%d vht_cap_len=%d\n", ifIndex,ie_list->vht_cap_len));
    ...
    IN UINT ifIndex)
{
	...
	if ((pAd->ApCfg.ApCliTab[ifIndex].ReqVarIELen + 17) <= IW_CUSTOM_MAX)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("pAd->ApCfg.ApCliTab[%d].ReqVarIELen + 17 > MAX_CUSTOM_LEN\n",ifIndex));
