...
/*	{"WnmMaxIdlePeriod",					Set_WnmMaxIdlePeriod_Proc}, */
	...
	{"trace_iparp_dbg_tx",				trace_iparp_set_dbg_tx},
	{"trace_iparp_dbg_rx",				trace_iparp_set_dbg_rx},
	{"trace_iparp_tx_cri_chk_pnt",		trace_iparp_set_tx_critical_chk_pnt},
	{"trace_iparp_rx_cri_chk_pnt",		trace_iparp_set_rx_critical_chk_pnt},
...
*/
		...
		case OID_802_11_P2P_LISTEN_CHANNEL:
			...
			else
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:: listen_ch = %d.\n", __FUNCTION__, listen_ch));

				{
					...
					*/
					...
					{
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Listen Channel out of range, using default.\n"));
						pAd->P2pCfg.ListenChannel = 1;
					}

					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:: Listen Channel = %d.\n", __FUNCTION__, pAd->P2pCfg.ListenChannel));
				}
			}
			break;
			...
			else
			{
				...
				if (pAd->P2pCfg.WscMode == WSC_PIN_MODE)
				{
					if (method == 1)
					{
						...
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("    *       PIN Code = %08u                     *\n", pAd->ApCfg.ApCliTab[0].WscControl.WscEnrolleePinCode));
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("    *************************************************\n"));
		...
		case OID_802_11_MESH_CHANNEL:
			...
			}
			{
				...
				else
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BW_%d, control_channel(%d), CentralChannel(%d) \n",
	...
	IN INT cmd)
{
	...
	case OID_802_11_AUTHENTICATION_MODE:
...
#ifdef RELEASE_EXCLUDE
			        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Query::RT_OID_WE_VERSION_COMPILED (=%d)\n", we_version_compiled));
#endif /* RELEASE_EXCLUDE */
		        	break;
		...
		case OID_802_11_P2P_MODE:
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11_P2P_MODE (Len=%d, Rule=%s)\n", sizeof(char),pAd->P2pCfg.Rule));
			break;
		...
		case OID_802_11_P2P_DEVICE_NAME:
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11_P2P_DEVICE_NAME (Len=%d, DeviceName=%s)\n", pAd->P2pCfg.DeviceNameLen,pAd->P2pCfg.DeviceName));
			break;
		...
		case OID_802_11_P2P_MAC_ADDR:
			...
			/*MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11_P2P_MAC_ADDR (Len=%d, Rule=%s)\n", sizeof(char),pAd->P2pCfg.GroupOpChannel)); */
			break;
		...
		case OID_802_11_P2P_CTRL_STATUS:
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11_P2P_MODE (Len=%d, DeviceName=%s)\n", pAd->P2pCfg.DeviceNameLen,pAd->P2pCfg.DeviceName));
			break;
		...
		case OID_802_11_P2P_SCAN_LIST:
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11_P2P_SCAN_LIST (Len=%d, Rule=%s)\n", sizeof(char),pAd->P2pCfg.GroupOpChannel));
		...
		case OID_802_11_GET_STA_HSINFO:
			{
				...
				UCHAR buf[sizeof(struct _sta_hs_info)];
				PMAC_TABLE_ENTRY	pEntry = NULL;
				
				copy_from_user(StaMacAddr, wrq->u.data.pointer, MAC_ADDR_LEN);
				pEntry = MacTableLookup(pAd, StaMacAddr);
				if (pEntry)
				{
					NdisMoveMemory(pEntry->hs_info.addr, StaMacAddr, MAC_ADDR_LEN);					
					wrq->u.data.length = sizeof(struct _sta_hs_info);
					Status = copy_to_user(wrq->u.data.pointer, &pEntry->hs_info, sizeof(struct _sta_hs_info));
				}
				else
				{
...
#if defined (RT2883) || defined (RT3883)
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("FixedRate: %d\n", pAd->CommonCfg.FixedRate));
#endif /* defined (RT2883) || defined (RT3883) */
...
#endif /* STREAM_MODE_SUPPORT */
#ifdef TXBF_SUPPORT
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ITxBfEn: %d\n", pAd->CommonCfg.RegTransmitSetting.field.ITxBfEn));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ITxBfTimeout: %ld\n", pAd->CommonCfg.ITxBfTimeout));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ETxBfTimeout: %ld\n", pAd->CommonCfg.ETxBfTimeout));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ETxBfEnCond: %ld\n", pAd->CommonCfg.ETxBfEnCond));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ETxBfNoncompress: %d\n", pAd->CommonCfg.ETxBfNoncompress));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("ETxBfIncapable: %d\n", pAd->CommonCfg.ETxBfIncapable));
#endif /* TXBF_SUPPORT */
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("==>RTMPIoctlBBP32()\n"));
#endif /* RELEASE_EXCLUDE */
	...
	if (argLen > 0)
	{
		...
		while ((this_char = strsep((char **)&ptr, ",")) != NULL)
		{
			...
			{ /* Write */
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("bbpID=%04x, value=0x%lx\n", bbpId, bbpValue));
					if ((bbpId <= 0x2fff) && (bbpId >= 0x2000))
					{
						/* according to Andy, Gary, David require. */
	...
	IN	RTMP_STRING *arg)
{
	...
	if ((token != NULL) && (strlen(token)>1))
	{
		...
		*token = '\0';
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, (" Repeate Client Full !!!\n"));
			return FALSE;
		}
...
INT Set_CwCTest_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if ((x >= 0) && (x <= 2))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("CwC %s\n", Cwc_mode_str[x]));
	}
	else
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("current FlgCWC = %d\n", pAd->FlgCWC));
	}
...
*/
    ...
    while (1) {
        printk("mac = 0x%x\n", mac_val);
        if (mac_val & TX_DMA_BUSY) {
            RtmpOsMsDelay(1);
            i++;
            if (i > 1000) {
                printk("disable PDMA TX too long\n");
                return FALSE;
            }
            HIF_IO_READ32(pAd, MT_WPDMA_GLO_CFG, &mac_val);
        }
	...
	IN	RTMP_STRING *arg)
{
	...
	ActHeaderInit(pAd, &Hdr, macAddr, pAd->ApCfg.MBSSID[BSS0].wdev.bssid, macAddr);
	...
	IN	ULONG					Data)
{
	...
	switch(cmd)
	{
...
#if 0 //Do we still have to do this?
#ifdef MBSS_SUPPORT
		...
		case CMD_RTPRIV_IOCTL_SHOW_CONNSTATUS:
			...
			wrq->u.data.length = sizeof(UINT);
			if (copy_to_user(wrq->u.data.pointer, (void *)&bConnect, wrq->u.data.length))
			{
				Status = -EFAULT;
			}
			break;
...
INT Set_MacWatchDog_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("EnableMacWD = %d \n", pAd->bEnableMacWD));
...
INT Set_KickOutHwNull_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("KickOutHwNull = %d \n", bEnableWD));
	...
	IN	RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: set falseCCA threshold %lu for microwave oven application!!\n", __FUNCTION__, th));
	...
	IN	RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: set low falseCCA threshold %lu for microwave oven application!!\n", __FUNCTION__, th));
