...
static UCHAR AddNINTENDO_ENTRY(PRTMP_ADAPTER pAd, PRT_NINTENDO_SSID ptmp, UCHAR * pDS_Addr)
{
	...
	for(loopi = 0; (loopi < NINTO_INFO.DS_TABLE.number) && (loopi < NINTENDO_MAX_ENTRY); loopi++)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("same MAC %02x%02x%02x%02x%02x%02x\n",
						PRINT_MAC(pDS_Addr)));
			break;
		}
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("add  MAC %02x%02x%02x%02x%02x%02x\n",
					PRINT_MAC(pDS_Addr)));
...
extern UCHAR DelNINTENDO_ENTRY(PRTMP_ADAPTER pAd, UCHAR * pDS_Addr)
{
	...
	for(loopi = 0; loopi < NINTO_INFO.DS_TABLE.number && loopi < NINTENDO_MAX_ENTRY; loopi++)
	{
		if(!memcmp(pDS_Addr, NINTO_INFO.DS_TABLE.entry[loopi].DS_Addr, MAC_ADDR_LEN))
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("DelNINTENDO_ENTRY loopi %d\n",loopi));
	...
	IN	RTMP_IOCTL_INPUT_STRUCT	*wrq)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RTMPIoctlNintendoCapable No parameter\n"));
	}
	else if(2 == wrq->u.data.length)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RTMPIoctlNintendoCapable %02x %02x\n",temp[0],temp[1]));
		if(0x30 == temp[0])
		{
			NintendoDisable(pAd);
		}
		else if(0x31 == temp[0])
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("iwpriv ra0 nintendocapable 0/1\n"));
	}
	...
	IN	RTMP_IOCTL_INPUT_STRUCT	*wrq)
{
	...
	if(1 == wrq->u.data.length)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" wrq->u.data.length %02x Use iwprive?\n",wrq->u.data.length));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" NINTENDO_TABLE size not match %02x %02x\n",length,wrq->u.data.length));
		return;
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" NINTENDO_TABLE number not match %02x %02x\n",pset->number,NINTO_INFO.DS_TABLE.number));
		goto out;
	}
	for(loopi = 0; loopi < NINTO_INFO.DS_TABLE.number && loopi < NINTENDO_MAX_ENTRY; loopi++)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" NINTENDO_TABLE entry[%d].registe %d \n",loopi,pset->entry[loopi].registe));
	...
	PUCHAR pDS_Addr)
{
	...
	if (32 == DS_SsidLen)
	{
		...
		&& (pNINTENDOSSID_REQ->zero2 == 0))
		{
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("DS login on\n"));
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("DS login off\n"));
			}
			if (NINTENDO_PROBE_REQ_SIGNAL & pNINTENDOSSID_REQ->registe)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("DS SIGNAL\n"));
			}
		}
	}
	else
	{
		...
		for(loopi = 0; loopi < NINTO_INFO.DS_TABLE.number && loopi < NINTENDO_MAX_ENTRY; loopi++)
		{
			if(!memcmp(pDS_Addr, NINTO_INFO.DS_TABLE.entry[loopi].DS_Addr, MAC_ADDR_LEN))
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("CheckNINTENDO_TABLE DS_TABLE.entry[%d].registe %d\n",loopi,NINTO_INFO.DS_TABLE.entry[loopi].registe));
				if (0 != NINTO_INFO.DS_TABLE.entry[loopi].registe)
				{
					...
					for(loopi=0;loopi<NINTENDO_WEPKEY_LN;loopi++) 
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%02x", pAd->NINTENDO_WEP_KEY[loopi]));
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("\n"));
					ErrorAck = 0;
				}
			}
