	...
	IN ULONG FrameLen)
{
	...
	if (pkt) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): TimPkt2 is NULL!\n", __FUNCTION__));
	}
}
#endif
#endif /* MT_MAC */
...
VOID APMakeBssTimFrame2(RTMP_ADAPTER *pAd, INT apidx)
{
	...
	if (pMbss->tim_buf.TimPkt2 == NULL) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():Invalid TimPkt2 for MBSS[%d]\n",
					__func__, apidx));
		return;
	}
