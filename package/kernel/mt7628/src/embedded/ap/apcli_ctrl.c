...
#if 0 //Carter for Cameo request, don't move ra0 to new channel! just jump back to origin channel to wait user's operation
		ApCliLinkDown(pAd, ifIndex);
		for (i = 0; i < MAX_APCLI_NUM; i++) {
			if (i == ifIndex)
			...
			//copy CfgSsid/Bssid to other Apcli link
			NdisZeroMemory(
				pAd->ApCfg.ApCliTab[i].CfgSsid,
				MAX_LEN_OF_SSID);
			pAd->ApCfg.ApCliTab[i].CfgSsidLen = 0;

			NdisZeroMemory(
				pAd->ApCfg.ApCliTab[i].CfgApCliBssid,
				ETH_LENGTH_OF_ADDRESS);
			NdisCopyMemory(
				pAd->ApCfg.ApCliTab[i].CfgSsid,
				pApCliEntry->CfgSsid,
				pApCliEntry->CfgSsidLen);
			pAd->ApCfg.ApCliTab[i].CfgSsidLen = pApCliEntry->CfgSsidLen;
			NdisCopyMemory(
				pAd->ApCfg.ApCliTab[i].CfgApCliBssid,
				pApCliEntry->CfgApCliBssid,
				ETH_LENGTH_OF_ADDRESS);

			//Copy authmode/encrypType to other Apcli link
			pAd->ApCfg.ApCliTab[i].AuthMode = pApCliEntry->AuthMode;
			pAd->ApCfg.ApCliTab[i].WepStatus = pApCliEntry->WepStatus;
			pAd->ApCfg.ApCliTab[i].PairCipher = pApCliEntry->WepStatus;
			pAd->ApCfg.ApCliTab[i].GroupCipher = pApCliEntry->WepStatus;
			pAd->ApCfg.ApCliTab[i].bMixCipher = FALSE;
			pAd->ApCfg.ApCliTab[i].DefaultKeyId = pApCliEntry->DefaultKeyId;
			RTMPMakeRSNIE(pAd, pAd->ApCfg.ApCliTab[i].AuthMode, pAd->ApCfg.ApCliTab[i].WepStatus, (i + MIN_NET_DEVICE_FOR_APCLI));

			NdisCopyMemory(pAd->ApCfg.ApCliTab[i].PSK, pApCliEntry->PSK, pApCliEntry->PSKLen);
			pAd->ApCfg.ApCliTab[i].PSKLen = pApCliEntry->PSKLen;
			RT_CfgSetWPAPSKKey(pAd,
				pAd->ApCfg.ApCliTab[i].PSK,
				pAd->ApCfg.ApCliTab[i].PSKLen,
				(PUCHAR)pAd->ApCfg.ApCliTab[i].CfgSsid,
				pAd->ApCfg.ApCliTab[i].CfgSsidLen,
				pAd->ApCfg.ApCliTab[i].PMK);

			pAd->ApCfg.ApCliTab[i].Enable = TRUE;
		}
		MlmeEnqueue(pAd, APCLI_CTRL_STATE_MACHINE, APCLI_CTRL_DISCONNECT_REQ, 0, NULL, ifIndex);
#endif
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():ProbeResp success. SSID=%s, Bssid=%02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("(%s) Apcli Auth Rsp Failure.\n", __FUNCTION__));
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("(%s) Peer DeAssoc Req.\n", __FUNCTION__));
