...
{	
	...
	*/
	...
	if (bGoBack && (channel != pAd->LatchRfRegs.Channel))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("P2PDiscCanlAction - Restore to channel %d\n",channel));
	}
...
static VOID P2PDiscScanCanlAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:: The Device Discovery time has expired, stop!!\n", __FUNCTION__));
#ifdef WIDI_SUPPORT
	*pCurrState = P2P_DISC_IDLE;
#else /* WIDI_SUPPORT */
...
static VOID P2PDiscListenAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:: Listen interval - %d\n", __FUNCTION__, pP2PCtrl->P2pCounter.ListenInterval));
