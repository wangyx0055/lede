...
static INT32
AndesMTSdioSubmitCmdMsgToSDIOWorker(
        IN struct _RTMP_ADAPTER *prAd,
        IN struct cmd_msg *prMsg,
        IN BOOLEAN fgIsRetry
        );

INT32
AndesMTSendCmdMsgToSdio(
        IN PRTMP_ADAPTER prAd
)
{
	INT32 Ret = 0;
	struct MCU_CTRL *ctl = &prAd->MCUCtrl;
	PNDIS_PACKET NetPkt = NULL;
	struct cmd_msg *msg = NULL;

	if ((msg = AndesDequeueCmdMsg(ctl, &ctl->txq_sdio)) != NULL) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("dequeue txq_sdio for tx sdio cmd\n"));

		if (RTMP_TEST_FLAG(prAd, fRTMP_ADAPTER_NIC_NOT_EXIST)
				|| RTMP_TEST_FLAG(prAd, fRTMP_ADAPTER_SUSPEND)) {

			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s RTMP_TEST_FLAG no pass\n",__FUNCTION__));
			if (!msg->need_wait)
				AndesFreeCmdMsg(msg);
			goto bailout;
		}
		...
		if (!((prAd->MCUCtrl.Stage == FW_DOWNLOAD_SCATTER) || (prAd->MCUCtrl.Stage == ROM_PATCH_DOWNLOAD_SCATTER))) {
			Ret = MTSDIOTxAcquireResource(prAd, MTSDIOTxGetCmdResourceType(prAd),
				MTSDIOTxGetPageCount(GET_OS_PKT_LEN(NetPkt), TRUE));
			if (Ret) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("(%s): CMD no resource\n", __FUNCTION__));
                        #if 0
		...
		/* send to SDIO host */
		Ret = MTSDIOCmdTx(prAd, GET_OS_PKT_DATAPTR(NetPkt),
...
bailout:
		if (Ret)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("kick out cmd to sdio host fail\n"));
			return Ret;
		}
		else
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("kick out cmd to sdio host ok\n"));
		}
	}
...
{
	INT32 u4Ret = 0;
	struct MCU_CTRL *prCtl = &prAd->MCUCtrl;


	if (fgIsRetry) {
		AndesQueueHeadCmdMsg(&prCtl->txq_sdio, prMsg, prMsg->state);
	} else {
		AndesQueueTailCmdMsg(&prCtl->txq_sdio, prMsg, prMsg->state);
	}

	MTSDIOWorkerThreadEventNotify(prAd, SDIO_EVENT_CMD_BIT);

	if (!prMsg->need_wait) {
		if (!AndesWaitForSdioCompleteTimeout(prMsg, prMsg->tx_sdio_timeout)) {

#if 0 // the msg may already be dequeued by the worker thread
		AndesUnlinkCmdMsg(msg, &ctl->txq_sdio);
#endif
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("(%s) the msg is timeout (need_wait %d)\n", __FUNCTION__, prMsg->need_wait));
		}
...
)
{
	...
	if (!OS_TEST_BIT(MCU_INIT, &prCtl->flags)) {
		u4Ret = -1;
       }

#if 0
	if (prMsg->state != tx_retransmit) {
		PNDIS_PACKET prNetPkt = prMsg->net_pkt;
		/*
 		 * append to meet block unit and zero four bytes padding
 		 * when using sdio block mode
 		 */
		memset(OS_PKT_TAIL_BUF_EXTEND(prNetPkt, prCtl->SDIOPadSize), 0x00, SDIO_END_PADDING);
	}
#endif
    ...
    // Put here, in case of preemption occured and make sure msg is in either ackq or kickq
	u4Ret = AndesMTSdioSubmitCmdMsgToSDIOWorker(prAd, prMsg, FALSE);
	...
	if (Ret) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:submit cmd msg to SDIO host fail(%d)\n", __FUNCTION__, Ret));
	}
#endif
...
INT32 AndesMTUSBKickOutCmdMsg(PRTMP_ADAPTER ad, struct cmd_msg *msg)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::submit urb success(%d)\n", __FUNCTION__, ret));
	}
...
#ifdef RTMP_SDIO_SUPPORT
	if ((Ctl->Stage == FW_NO_INIT) || (Ctl->Stage == FW_DOWNLOAD) || (Ctl->Stage == ROM_PATCH_DOWNLOAD) || (Ctl->Stage == ROM_PATCH_DOWNLOAD_SCATTER) || (Ctl->Stage == FW_DOWNLOAD_SCATTER))
#else
...
#if 0
	CmdFwLog2Host(pAd, ANDES_LOG_TO_EVENT);
#endif
...
#if 0
	ret = CmdChPrivilege(pAd, CMD_CH_PRIV_ACTION_REQ, control_chl, central_chl,
...
#ifdef RTMP_SDIO_SUPPORT
		OS_LOAD_CODE_FROM_BIN(&cap->FWImageName, cap->fw_bin_file_name, obj->sdio_dev, &cap->fw_len);
#endif
	} else {
		cap->FWImageName = cap->fw_header_image;
	}
...
//#endif
	...
	UINT32  counter =0;
	RtmpOsMsDelay(1000);
        RTMP_SDIO_READ32(ad, WCIR, &value);

	while(!GET_W_FUNC_RDY(value)){
		counter++;
		if(counter >500){
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s ready bit time out\n",__FUNCTION__));
			ret = 1;
			goto done;
		}
		RTMP_SDIO_READ32(ad, WCIR, &value);
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: &pAd->MCUCtrl=%p\n",__FUNCTION__,Ctl));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): findish download fw ya! \n", __FUNCTION__));

	/* We reset TC quota here, because PSE re-init is done */
	MTSDIOTxResetResource(ad);
...
#ifdef RTMP_SDIO_SUPPORT
		OS_LOAD_CODE_FROM_BIN(&cap->rom_patch, cap->rom_patch_bin_file_name, obj->sdio_dev, &cap->rom_patch_len);
#endif
	} else {
...
static VOID ExtEventTmrCalResultHandler(RTMP_ADAPTER *pAd, UINT8 *Data, UINT32 Length)
{
    ...
    UINT32   u4FactorA = 0, u4FactorB = 0, u4FactorC = 0, u4FactorResult = 0;

    if (pAd->chipCap.TmrEnable > TMR_DISABLE) {
        u4FactorA = pTmrCalResult->u4TmrCPCnt;
        u4FactorB = pTmrCalResult->u4TmrResult;
        u4FactorC = pTmrCalResult->u4TmrTxFE_5;

        u4FactorResult = u4FactorA + u4FactorB + ((u4FactorC-4)*33/4) - 6400;//unit is 0.25 nsec
        pAd->chipCap.TmrCalResult = u4FactorResult;
    }

    MTWF_LOG(DBG_CAT_ALL,
                DBG_SUBCAT_ALL,
                DBG_LVL_OFF,
                ("%s::FW LOG, A (0x%x), B (0x%x), C (0x%x), TmrCalResult (0x%x)\n",
                __func__,
                u4FactorA,
                u4FactorB,
                u4FactorC,
...
VOID AndesMTSdioFwInit(RTMP_ADAPTER *pAd)
{
	...
	RTMP_SET_FLAG(pAd, fRTMP_ADAPTER_MCU_SEND_IN_BAND_CMD);
#if 1
	RTMP_SDIO_WRITE32(pAd, WHIER, (RX0_DONE_INT_EN |RX1_DONE_INT_EN | TX_DONE_INT_EN));
                    ...
                    u32 start_tsf)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("\x1b[31m @@@@@ %s:channel(%u,%u), bw(%u,%u), role(%u,%u), cycle_time(%u,%u), wait_time(%u), null_cnt(%u), start_tsf(0x%ld) \x1b[m\n",
...
#if 0
	printk("=====> Send MCC start: extch(%d, %d), tx pwr(%d, %d, %d, %d)\n",
	    extch_1st, extch_2nd,
        ch0_tx_0_pwr, ch0_tx_1_pwr,
        ch1_tx_0_pwr, ch1_tx_1_pwr);
    hex_dump("In-Band",
        GET_OS_PKT_DATAPTR(msg->net_pkt), GET_OS_PKT_LEN(msg->net_pkt));
#endif
	ret = AndesSendCmdMsg(ad, msg);
...
int CmdMccStop(struct _RTMP_ADAPTER *ad, u8 parking_idx, u8 auto_resume_mode, u16 auto_resume_interval, u32 auto_resume_tsf)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("\x1b[32m @@@@@ %s:parking_channel_idx(%u), auto_resume_mode(%u), auto_resume_tsf(0x%08x) \x1b[m\n",
