	...
	IN PRTMP_ADAPTER	pAd)
{
	...
	for (iter=0; iter < NULLFRAMESPACE; iter++ )
	{
		...
		{
	 	   MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Coex: Send protection frame %d\n",iter));
		...
		IN BOOLEAN			Disable)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: COEX AFH Start Ch = %d, AFH End Ch = %d, Channel = %d, CentralChannel = %d\n",
		__FUNCTION__,
	...
	BOOLEAN RemoveBSS)
{
	...
	for(j = 1; j < NULLFRAMESPACE; j++)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX: j = %d\n",j));
  			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX: Find BSSID Address = %02X:%02X:%02X:%02X:%02X:%02X\n",pAd->HASH_BSSID[j][0],pAd->HASH_BSSID[j][1],
			...
			if (RemoveBSS == TRUE)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX: Remove BSSID Address  = %02X:%02X:%02X:%02X:%02X:%02X\n",pAd->HASH_BSSID[j][0],pAd->HASH_BSSID[j][1],
				pAd->HASH_BSSID[j][2],pAd->HASH_BSSID[j][3],
				pAd->HASH_BSSID[j][4], pAd->HASH_BSSID[j][5]));
	...
	return HashIdx;
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX:Miss search  j = %d\n",j));
	}
	for(j = 1; j < NULLFRAMESPACE; j++)
	{
		if (pAd->HASH_BSSID[j][MAC_ADDR_LEN] == 0)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX: free space j = %d\n",j));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX: Insert MAC Address  = %02X:%02X:%02X:%02X:%02X:%02X\n",pAd->HASH_BSSID[j][0],pAd->HASH_BSSID[j][1],
			pAd->HASH_BSSID[j][2],pAd->HASH_BSSID[j][3],
			pAd->HASH_BSSID[j][4], pAd->HASH_BSSID[j][5]));
			return HashIdx;
		}
	...
	IN	PMAC_TABLE_ENTRY pEntry)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==>PrepareProtectionFrame\n"));
	...
	pTxWI->TXWI_N.MCS= 0;
	  ...
	  MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Protection FrameAddress =%x \n",FrameAddress));
	//
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Type =%x \n",Type));
...
)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX: Protection FrameBaseNumber=%d\n",n0));
...
///the following code block shoule not be enabled.
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX: LMthreshold = %d, MHthreshold = %d\n",LMthreshold,MHthreshold));
	if (pAd->coexRXManualAGCGain.bStopAGC)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX: Stop AGC "));
	}
	else
	{
 ...
 UCHAR CoexMode )
{
	...
	else if  (CoexMode == COEX_MODE_FDD || CoexMode == COEX_MODE_RESET)
	{
		...
		if (pAd->CommonCfg.BACapability.field.RxBAWinLimit != pAd->CommonCfg.REGBACapability.field.RxBAWinLimit )
		{
			...
			BAOriSessionTearDown(pAd, BSSID_WCID, 0, FALSE, FALSE);
			BARecSessionTearDown(pAd, BSSID_WCID, 0, FALSE);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX: FDD (Reset)mode: Set RxBASize to %d\n", pAd->CoexMode.RxBAWinLimit));
			//expect to build BA
			BAOriSessionSetUp(pAd, pEntry, 0, 0, 100, TRUE);
		}
	}
	...
	)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: CoexOperation = %d, CoexMode = %d\n, PktLength = %d\n",
		__FUNCTION__,
...
VOID MLMEHook(
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX: CoexWLANPrivilegeTime = %x\n",  pAd->CommonCfg.CoexWLANPrivilegeTime));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX: unit = %x\n",  unit));
		...
		)
{
	...
	//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("COEX: StaCfg.AvgRssi0 = %d\n",pAd->StaCfg.RssiSample.AvgRssi0));
	...
	if (IS_MT76XXBTCOMBO(pAd) && (BT_STATUS_TEST_FLAG(pAd,fBTSTATUS_BT_ACTIVE)))
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("!!COEX:Strange CoexMode = %d\n",pAd->CoexMode.CurrentMode));
		}
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("COEX: pAd->CoexMode.CoexTDDRSSITreshold = %d\n",pAd->CoexMode.CoexTDDRSSITreshold));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("COEX: pAd->CoexMode.CoexFDDRSSITreshold = %d\n",pAd->CoexMode.CoexFDDRSSITreshold));
	...
	)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX: BT ACTIVE !!!! \n"));
	}
	if  (BT_STATUS_TEST_FLAG(pAd,fBTSTATUS_BT_BWL))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX:  bandwidth-limited link such as A2DP\n"));
	}
	if  (BT_STATUS_TEST_FLAG(pAd,fBTSTATUS_BT_BE))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX: best effort to serve non-QoS-guaranteed link such as PAN, FTP\n"));
	}
	if  (BT_STATUS_TEST_FLAG(pAd,fBTSTATUS_BT_SYNC))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX: synchronous link(s) such as SCO, eSCO\n"));
	}
	if  (BT_STATUS_TEST_FLAG(pAd,fBTSTATUS_BT_SNIFF))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX: periodic transport(s) such as SNIFF, LE_CONN, LE_ADV\n"));
	}
	if  (BT_STATUS_TEST_FLAG(pAd,fBTSTATUS_BT_SCATTER))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX: scatter operation either PAGE or INQUIRY\n"));
	}

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX: BT_LINK_CNT = %d, BTE_LINK_CNT= %d, TotalCount = %d, BT_SLAVE_IND = %d\n",
...
#if 0
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MT7636 COEX: CoexWLANPrivilegeTime = %x\n",  pAd->CommonCfg.CoexWLANPrivilegeTime));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX: unit = %x\n",  unit));
#endif
	switch (WlanStatus)
	{
		case MT7636_WLAN_Device_ON :
			AndesCoexOP(pAd, STATUS_RADIO_ON);
			break;

		case MT7636_WLAN_Device_OFF :
			AndesCoexOP(pAd, STATUS_RADIO_OFF);
			break;

		case MT7636_WLAN_SCANREQEST_2G :
			AndesCoexOP(pAd, STATUS_SCAN_G_BAND);
			break;

		case MT7636_WLAN_SCANDONE_2G :
			AndesCoexOP(pAd, STATUS_SCAN_G_BAND_END);
			break;

		case MT7636_WLAN_SCANREQEST_5G :
			AndesCoexOP(pAd, STATUS_SCAN_A_BAND);
			break;

		case MT7636_WLAN_SCANDONE_5G :
			AndesCoexOP(pAd, STATUS_SCAN_A_BAND_END);
			break;

		case MT7636_WLAN_LINK_START:
			AndesCoexOP(pAd, STATUS_LINK_UP);
			break;

		case MT7636_WLAN_LINK_DONE:
			AndesCoexOP(pAd, STATUS_LINK_DOWN);
			break;
		default:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Not support the wifi status\n"));
			//SendAndesWLANStatus(pAd, WlanStatus, 0, BssHashID);
	}
