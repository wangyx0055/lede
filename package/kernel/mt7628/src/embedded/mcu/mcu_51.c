	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (IS_RT65XX(pAd)) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): Ignore for MCU status check for 6590 now!\n", __FUNCTION__));
		return Status;
	}
#endif /* RT65xx */
...
#ifdef MT7601
	if (IS_MT7601(pAd)) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): Ignore for MCU status check for MT7601 now!\n", __FUNCTION__));
		return Status;
	}
#endif /* MT7601 */
...
#define NICLF_DEFAULT_USE()	\
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s - Use default firmware!\n", __FUNCTION__));
	...
	if (flg_default_firm_use == FALSE)
	{
		do
		{
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s - Error opening file %s\n", __FUNCTION__, src));
				NICLF_DEFAULT_USE();
				break;
			}
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: error file length (=%d) in RT2860AP.BIN\n",
			...
			else
			{
				...
				{
					/* CRC fail */
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: CRC = 0x%02x 0x%02x "
				...
				else
				{
					...
					if ((pAd->FirmwareVersion) < \
											((FIRMWARE_MAJOR_VERSION << 8) +
									  	 	 FIRMWARE_MINOR_VERSION))
					{
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: firmware version too old!\n", __FUNCTION__));
						NICLF_DEFAULT_USE();
						break;
					}
				}
		...
		else
		{
			...
			if (retval)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("--> Error %d closing %s\n", -retval, src));
			}
		}
	}
	...
	if (isMCUNeedToLoadFIrmware(pAd) == NDIS_STATUS_SUCCESS)
	{
		...
		if (Equal == FALSE)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: WOW stops to go into 4K ram codes ...\n", __FUNCTION__));
	...
	/* New 8k byte firmware size for RT3071/RT3072*/
	/*MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Usb Chip\n"));*/
	...
	else
	{
		...
		if ((Version != 0x2860) && (Version != 0x2872) && (Version != 0x3070))
		{
			if (FIRMWAREIMAGE_LENGTH == FIRMWAREIMAGE_WOW_LENGTH) /* size 0x3000 */
			{
				if (pAd->WOW_Cfg.bWOWFirmware == TRUE)
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: Load WOW firmware!!\n", __FUNCTION__));
				}
				else
				{
					pFirmwareImage = (PUCHAR)&FirmwareImage[FIRMWAREIMAGEV2_LENGTH]; /* normal offset: 0x1000 */
					FileLength = FIRMWAREIMAGEV1_LENGTH; /* 0x1000 */
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: Load normal firmware!!\n", __FUNCTION__));
				}
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("KH: bin file should be 8KB.\n"));
			Status = NDIS_STATUS_FAILURE;
		}
	}
	...
	*/
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("NICLoadFirmware: We need to load firmware\n"));	
		RTMP_WRITE_FIRMWARE(pAd, pFirmwareImage, FileLength); /* FirmwareRun VndReq 0x1/0x8 --> initDone = 1 */
	}
	else {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("NICLoadFirmware: firmware loaded already\n"));
		RTUSB_VendorRequest(
                pAd,
                USBD_TRANSFER_DIRECTION_OUT,
                DEVICE_VENDOR_REQUEST_OUT,
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): MCU is not ready!\n\n\n", __FUNCTION__));
		Status = NDIS_STATUS_FAILURE;
	}
	...
	*/
    ...
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<=== %s (status=%d)\n", __FUNCTION__, Status));
...
BOOLEAN RtmpAsicCheckCommanOk(RTMP_ADAPTER *pAd, UCHAR Command)
{
	...
	if (i < 200)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("--> AsicCheckCommanOk CID = 0x%x, CmdStatus= 0x%x \n", CID, CmdStatus));
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("--> AsicCheckCommanFail1 CID = 0x%x, CmdStatus= 0x%x \n", CID, CmdStatus));
	}
	else
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("--> AsicCheckCommanFail2 Timeout Command = %d, CmdStatus= 0x%x \n", Command, CmdStatus));
	}
	...
	*/
	...
	if (IS_RT3290(pAd))
	{
		...
		else if (Command == SLEEP_MCU_CMD)
		{
			...
			if (i >= 1000)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, (" %s: [boundary]DMA Tx keeps busy.  %d\n", __FUNCTION__, i ));
			}			
		}
	}
#endif /* RT3290 */
		...
		&& (pAd->StaCfg.PSControl.field.EnableNewPS == TRUE))
	{
			...
			&& (Command != WAKE_MCU_CMD) && (Command != RFOFF_MCU_CMD))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" Ban Mcu Cmd %x in sleep mode\n",  Command));
...
/*			RTMPWriteCBConfigXP(pAd, offset, Configuration);*/
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Write 70f; offset = %x, Configuration = %x. \n", offset, Configuration));
		}
...
/*			RTMPWriteCBConfigXP(pAd, offset, Configuration);*/
			pci_write_config_dword(pObj->pci_dev, offset, &Configuration);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RadioOnExec restore 70f; offset = %x, Configuration = %x. \n", offset, Configuration));
		}
	}
#endif /* 0 */
#if defined(RT5390) || defined(RT5392) || defined(RT5592)
	...
	)
	{
		...
		if ((Command != 0x84) && (Command != WAKE_MCU_CMD))
		{
			do
			{
				...
				RtmpusecDelay(2);
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("AsicSendCommanToMcu::Mail box is busy\n"));
			} while(i++ < 100);
	...
	do
	{
		...
		RtmpusecDelay(2);
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("AsicSendCommandToMcu::Mail box is busy\n"));
#endif /* RELEASE_EXCLUDE */
	} while(i++ < 100);
	...
	if (Command != 0x80)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("SW interrupt MCU (cmd=0x%02x, token=0x%02x, arg1,arg0=0x%02x,0x%02x)\n",
					 ...
					 UINT8 TIMByteOffset, UINT8 TIMBytePattern)
{
	...
	if (PwrOP == RADIO_ON)
	{
		...
		do
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("PSM :WakeUp Cmd Failed, retry %d\n", RetryRound));
		...
		if (RetryRound > 10)
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("PSM :ASIC 0x31 WakeUp Cmd may Fail %d*******\n", RetryRound));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Unknown %s:PwrOP(%d)\n", __FUNCTION__, PwrOP));
	}
