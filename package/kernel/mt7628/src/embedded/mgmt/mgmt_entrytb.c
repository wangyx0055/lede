		...
		if (IS_ENTRY_NONE(&pAd->MacTab.Content[i]))
		{
			...
			do
			{
				...
				if (ent_type == ENTRY_MESH)
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("-->Insert Entry wcid=%d, authmode=%d \n", i, pEntry->AuthMode));
					if (pEntry->AuthMode < Ndis802_11AuthModeWPA)
						pEntry->PrivacyFilter = Ndis802_11PrivFilterAcceptAll;
					else
						pEntry->PrivacyFilter = Ndis802_11PrivFilter8021xWEP;
	...
	if (pEntry && !IS_ENTRY_NONE(pEntry))
	{
		...
		if (MAC_ADDR_EQUAL(pEntry->Addr, pAddr))
		{
			...
			if (P2P_GO_ON(pAd))
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("MacTableDeleteEntry1 - Total= %d. p2pEntry = %d.\n", pAd->MacTab.Size, p2pEntryCnt));
			}
#endif /* P2P_SUPPORT */
