...
/*
	Several test cases as follows:

	1. UAPSD Buffer STA
		(1) The peer maybe in ACTIVE mode or SLEEP mode.
		(2) We will send a TDLS Setup request frame to a peer.
		(3) Simulate to receive a TDLS Setup response frame from the peer.
		(4) We will send a TDLS Setup confirm frame to a peer.

		--- peer ps status detection start
		(5) Send a Traffic Indication data frame to the peer through AP even
			all AC are non-UAPSD mode for the peer.
		(6) We will receive a Traffic Response frame to get the power save
			status for the peer.
		(7) We will reply a QoS Null EOSP frame if any AC is delivery-enabled.
		--- peer ps status detection stop

		(8) Set a static ARP entry for the peer.
		(9) Send a ping request to the peer.
		(10) Keep wake up until EOSP is sent.
		(11) Send a Traffic Indication data frame to the peer through AP.
		(12) The peer will send a QoS Null frame to get the indication from AP.
		(13) Simulate to receive a Traffic Response frame from the peer with PM=1.
		(14) Send the ping request to the peer with EOSP = 1.
		(15) Recover self power mode until next packet coming.

	2. UAPSD Sleep STA
		(1) We will send a TDLS Setup request frame to a peer.
		(2) Simulate to receive a TDLS Setup response frame from the peer.
		(3) We will send a TDLS Setup confirm frame to a peer.
		(4) We will send a QoS null frame with PM=1 to the peer.
			RISK: Maybe the peer does not receive the packet.
		(5) Simulate to receive a Traffic Indication from the peer through AP.
		(6) We will send a Traffic response frame to the peer.
		(7) Simulate to receive a EOSP frame from the peer.
		(8) Sleep again.

	3. non-UAPSD Buffer STA
		(1) We will send a TDLS Setup request frame to a peer.
		(2) Simulate to receive a TDLS Setup response frame from the peer.
		(3) We will send a TDLS Setup confirm frame to a peer.
		(4) Simulate to receive a QoS null frame with PM=1 from the peer.
		(5) Set a static ARP entry for the peer.
		(6) Send a ping request to the peer.
		(7) Send the ping request to the peer through AP.
		(8) Recover self power mode until next packet coming.
*/
    ...
    IN	STATE_MACHINE				*pFSM)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("tdls uapsd> initialization ok!\n"));
	return TRUE;
}
	...
	IN	USHORT						PsmNew)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("tdls uapsd> our PSM mode change!\n"));
		...
		if ((pTDLS->Valid) && (pTDLS->Status == TDLS_MODE_CONNECTED))
		{
			...
			*/
			if ((TDLS_UAPSD_ARE_PEER_IN_ACTIVE(pMacEntry)) &&
				(pMacEntry->RssiSample.LastRssi[0] != 0))
#else
			if (TDLS_UAPSD_ARE_PEER_IN_ACTIVE(pMacEntry))
#endif
			{
				/*
					pMacEntry->RssiSample.LastRssi[0] is used to check if
					we have ever received any packet from the peer.
				*/
				/* send a null frame to the peer directly */
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("tdls uapsd> send a NULL frame!\n"));

				RtmpEnqueueNullFrame(pAd, pMacEntry->Addr,
									pAd->CommonCfg.TxRate, pMacEntry->Aid,
									pMacEntry->func_tb_idx, TRUE, FALSE, 0);
				continue;
			}
	...
	IN	CHAR						*pArgv)
{
	...
	if (pMacEntry == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("tdls_cmd> ERROR! No such peer!\n"));
		return;
	}
	...
	IN	RTMP_STRING *pArgvIn)
{
	...
	Command = simple_strtol((RTMP_STRING *)BufCmd, 0, 10);
	...
	switch(Command)
	{
		...
		default: /* error command type */
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("tdls_cmd> ERROR! No such command!\n"));
	...
	OUT UCHAR						*pHeader802_3)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("tdls uapsd> LinkId = %d\n", LinkId));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("tdls uapsd> link is not yet built "
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("tdls uapsd> traffic ind was sent before!\n"));
    ...
    IN	MLME_QUEUE_ELEM				*pElem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("tdls uapsd> ====> %s\n", __FUNCTION__));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("TDLS - TDLS_UAPSD_PeerTrafficIndAction() can not find the LinkId!\n"));
		return;
	}
...
/*	hex_dump("PeerAddr=", PeerAddr, 6); */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("tdls uapsd> PU Buffer Status = 0x%x\n",
	...
	IN	CHAR						**ppArgv)
{
	...
	*ppArgv += ID+1; /* skip _ */
	...
	return simple_strtol((RTMP_STRING *)Buf, 0, 10);
	...
	IN	CHAR						*pArgv)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("tdls uapsd> LinkId = %d\n", LinkId));
	...
	IN	CHAR						*pArgv)
{
	...
	if (PeerPsMode != 0)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("tdls_cmd> Change to ACTIVE!\n"));
	}
	else
	{
		pMacEntry->PsMode = PWR_SAVE;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("tdls_cmd> Change to POWER SAVE!\n"));
	}
	// TODO: shiang-usw, we need to rmove upper setting and migrate to tr_entry->PsMode
	pAd->MacTab.tr_entry[pMacEntry->wcid].PsMode = pMacEntry->PsMode;
