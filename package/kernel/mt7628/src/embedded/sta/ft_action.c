	...
	OUT USHORT *pStatus)
{
	...
	*pStatus = pFtRspFrame->StatusCode;
	...
	while ((Length > 0) && (Length >= (2 + pEid->Len))) {
		...
		case IE_RSN:
#if 0
			NdisMoveMemory(pVIE + (*LengthVIE), &pEid->Eid,
				       pEid->Len + 2);
			*LengthVIE += (pEid->Len + 2);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				 ("PeerFtRspSanity IE_RSN . \n"));
#endif
			break;
		case IE_FT_MDIE:
			if (pEid->Len == 3) {
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("FT_OTD_ACTION :FT_OTD_ReqAction() \n"));
	...
	IN PVOID SystemSpecific3)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("FT_OTD_ACTION - FtTimeoutAction\n"));
}
