...
{
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC - InvalidState (state=%ld, msg_type = %ld)\n", 
...
VOID IWSC_Stop(RTMP_ADAPTER *pAd, BOOLEAN bSendNotification)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<----- IWSC_Stop\n"));
	...
	if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_REMOVE_IN_PROGRESS) == FALSE)
	{
		AsicDisableSync(pAd);
#if 0
		WscBuildBeaconIE(pAd, pWpsCtrl->WscConfStatus, FALSE, DEV_PASS_ID_PIN, pWpsCtrl->WscConfigMethods, BSS0);
		WscBuildProbeRespIE(pAd, WSC_MSGTYPE_REGISTRAR, pWpsCtrl->WscConfStatus, FALSE, DEV_PASS_ID_PIN, pWpsCtrl->WscConfigMethods, BSS0);
#else
    ...
    IN  PVOID SystemSpecific3)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!! IWSC_T1TimerAction !!!\n"));
	...
	if (pAd)
	{
		...
		if (pWpsCtrl)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!! IWSC_T1TimerAction - pWpsCtrl is null !!!\n"));
	}
	else
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!! IWSC_T1TimerAction - pAd is null !!!\n"));
}
    ...
    IN  PVOID SystemSpecific3)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!! IWSC_T2TimerAction - Become Enrollee !!!\n"));
	...
	if (pAd)
	{
		...
		if (pWpsCtrl)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!! IWSC_T2TimerAction - pWpsCtrl is null !!!\n"));		
	}
	else
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!! IWSC_T2TimerAction - pAd is null !!!\n"));
}
    ...
    IN  PVOID SystemSpecific3)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!! IWSC_EntryTimerAction !!!\n"));
	if (pAd)
	{
		...
		if (pIWscInfo)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!! IWSC_EntryTimerAction - pIWscInfo is null !!!\n"));
	}
	else
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!! IWSC_EntryTimerAction - pAd is null !!!\n"));
}
    ...
    IN  PVOID SystemSpecific3)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!! IWSC_DevQueryAction !!!\n"));
	if (pAd)
	{
		pIWscInfo = &pAd->StaCfg.IWscInfo;		
		if (pIWscInfo)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!! IWSC_DevQueryAction - pIWscInfo is null !!!\n"));
	}
	else
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!! IWSC_DevQueryAction - pAd is null !!!\n"));
}
					...
					)
				{
					...
					*/
					...
					if (RandomTime > 60)
						...
						RandomTime = 20;
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RandomTime = %d\n", RandomTime));
...
VOID IWSC_MlmeStopAction(RTMP_ADAPTER *pAd, PMLME_QUEUE_ELEM pElem) 
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("SinglePIN registrar now, keep going!!\n"));
...
#if 0
	if (pAd->StaCfg.IWscInfo.bIWscDevQueryReqTimerRunning)
		RTMPSetTimer(&pAd->StaCfg.IWscInfo.IWscDevQueryTimer, 5000);
#endif
}
...
VOID IWSC_ScanDoneAction(RTMP_ADAPTER *pAd, PMLME_QUEUE_ELEM pElem) 
{
	...
	{
		if (pAd->Mlme.IWscMachine.CurrState != IWSC_IDLE)
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC_ScanDoneAction:: CurrState = %ld\n", pAd->Mlme.IWscMachine.CurrState));
		return;
	}

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("-----> IWSC_ScanDoneAction\n"));	
	...
	{
#if 0
		ULONG		ApIdx = BSS_NOT_FOUND;

		ApIdx = IWSC_SearchWpsApByPinMethod(pAd);
		...
		{
			NDIS_802_11_SSID *pPeerSsid;
			os_alloc_mem(NULL, (UCHAR **)&pPeerSsid, sizeof(NDIS_802_11_SSID));
			if (pPeerSsid)
			{
				USHORT	config_method = 0;
				BSS_ENTRY *pBss = &pAd->ScanTab.BssEntry[ApIdx];
				NdisZeroMemory(pPeerSsid, sizeof(NDIS_802_11_SSID));
				NdisMoveMemory(pPeerSsid->Ssid, pBss->Ssid, pBss->SsidLen);
				pPeerSsid->SsidLength = pBss->SsidLen;
				RTMPZeroMemory(&pWpsCtrl->WscSsid, sizeof(NDIS_802_11_SSID));
				RTMPMoveMemory(&pWpsCtrl->WscSsid, pPeerSsid, sizeof(NDIS_802_11_SSID));
				IWSC_GetConfigMethod(pAd, pBss->VarIEs, pBss->VarIELen, &config_method);
				if (config_method & WPS_CONFIG_METHODS_KEYPAD)
					pAd->StaCfg.IWscInfo.bLimitedUIPeer = FALSE;
				else	
					pAd->StaCfg.IWscInfo.bLimitedUIPeer = TRUE;
				MlmeEnqueue(pAd,
				...
				os_free_mem(NULL, pPeerSsid);
			}
			else
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC_ScanDoneAction:: allocate memory fail!!\n"));
		}
#endif
	}
...
VOID IWSC_BuildSelRegStartNotification(RTMP_ADAPTER *pAd, UCHAR *pOutBuf, USHORT *pIeLen)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("-----> IWSC_BuildSelRegStartNotification\n"));
...
//	UCHAR 			Data[512];
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("-----> IWSC_BuildSelRegFinishNotification\n"));
...
VOID IWSC_SendActionFrame(RTMP_ADAPTER *pAd, UCHAR FrameType)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("IWSC_SendStartNotification:: allocate memory failed \n"));
		return;
	}
...
{	
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("-----> IWSC_PeerAction\n"));

	/*
		Marvell STA always send same non-zero Token value, thus we don't need to check Token value here.
	*/
#if 0
	if (pIWscInfo->PeerDialogToken != Token)
#endif
	{
		/*
		...
		*/
		...
		for (;;)
		{
			...
			if (TotalLen <= 0)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("No IWSC IE!\n<----- IWSC_PeerAction\n"));
				return;
			}
			pData = pData + Len;
		}
		...
		if (FrameType == IWSC_SEL_REG_START_NOTITY)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Receive SelRegStartNotification from %02X:%02X:%02X:%02X:%02X:%02X!\n",
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Receive SelRegFinishNotification from %02X:%02X:%02X:%02X:%02X:%02X!\n",
						PeerMAC[0], PeerMAC[1], PeerMAC[2], PeerMAC[3], PeerMAC[4], PeerMAC[5]));
		...
		{			
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Receive Device Query Request from %02X:%02X:%02X:%02X:%02X:%02X!\n",
						PeerMAC[0], PeerMAC[1], PeerMAC[2], PeerMAC[3], PeerMAC[4], PeerMAC[5]));			
			if (RandomVal < 20)
				RandomVal = 20;
			if (RandomVal > 50)
			...
			if (pEntry == NULL)
			{
				...
				pEntry = MacTableInsertEntry(pAd, PeerMAC, &pAd->StaCfg.wdev, ENTRY_AP, OPMODE_STA, FALSE);
			}
			...
			if (pEntry)
			{
                    ...
                    && (pEntry->WPA_Authenticator.WpaState < AS_INITPSK))                      
                {
					...
					if (pEntry->bPeerHigherMAC == FALSE)
					{
						...
						*/
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - EnqueueStartForPSKTimer.\n", __FUNCTION__));
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Receive Device Query Response from %02X:%02X:%02X:%02X:%02X:%02X!\n",
						PeerMAC[0], PeerMAC[1], PeerMAC[2], PeerMAC[3], PeerMAC[4], PeerMAC[5]));
			...
			{
				// Another adhoc joining, add to our MAC table. 
				pEntry = MacTableInsertEntry(pAd, PeerMAC, &pAd->StaCfg.wdev, ENTRY_ADHOC, OPMODE_STA, FALSE);
			}
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Unknow Frame Type!\n"));
		}
	}
#if 0
	else
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ignore this Action Frame!(%d, %d)\n", pIWscInfo->PeerDialogToken, Token));
#endif
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<----- IWSC_PeerAction\n"));
}
...
VOID IWSC_PeerProbeRequest(RTMP_ADAPTER *pAd, PMLME_QUEUE_ELEM pElem)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IWSC_PeerProbeRequest:: Not Registrar now(WscConfMode = %d)\n"
							, pWscCtrl->WscConfMode));
		return;
	}

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<----- IWSC_PeerProbeRequest\n"));
	...
	if (pWscCtrl->WscMode == WSC_PIN_MODE)
	{
		if (PeerConfigMethod & WPS_CONFIG_METHODS_KEYPAD)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC_PeerProbeRequest:: WscPinCode = %08d\n", pWscCtrl->WscPinCode));
		}
		else 
		{
			...
			else
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC_PeerProbeRequest:: Please input Peer's PIN\n"));
			}
		}
		if (PeerConfigMethod & WPS_CONFIG_METHODS_KEYPAD)
		{
...
VOID IWSC_PeerProbeResponse(RTMP_ADAPTER *pAd, PMLME_QUEUE_ELEM pElem)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IWSC_PeerProbeResponse:: Not Enrollee now(WscConfMode = %d)\n"
							, pWscCtrl->WscConfMode));
		return;
	}

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<----- IWSC_PeerProbeResponse\n"));
	...
	if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Cancel Scan Timer\n"));
	}
	...
	if (pAd->StaCfg.IWscInfo.bLimitedUI)
	{
		if (PeerConfigMethod & WPS_CONFIG_METHODS_KEYPAD)
		{
			if (pIWscInfo->bSinglePIN == FALSE)
			{
				...
				pWscCtrl->WscEnrolleePinCodeLen = 8;
				pWscCtrl->WscPinCodeLen = 8;			
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC_PeerProbeResponse:: WscPinCode = %d\n", pWscCtrl->WscPinCode));
				pWscCtrl->WscStatus = STATUS_WSC_IBSS_NEW_RANDOM_PIN;
			}
			else
				pWscCtrl->WscStatus = STATUS_WSC_IBSS_FIXED_PIN;
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC_PeerProbeResponse:: Please input Peer's PIN\n"));
...
VOID IWSC_PeerPIN(RTMP_ADAPTER *pAd, PMLME_QUEUE_ELEM pElem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("-----> IWSC_PeerPIN\n"));
	if (ADHOC_ON(pAd))
	{
...
BOOLEAN	IWSC_PeerEapolStart(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
	...
	if (pWpsCtrl->WscConfMode != WSC_REGISTRAR)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC_PeerEapolStart:: Not Registrar now(WscConfMode = %d)\n", pWpsCtrl->WscConfMode));
		return FALSE;
	}

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<----- IWSC_PeerEapolStart\n"));
	...
	if (pWpsCtrl->WscMode == WSC_PBC_MODE)
	{
		...
		*/
		...
		if (pAd->CommonCfg.WscPBCOverlap == TRUE)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC_PeerEapolStart: PBC Session Overlap!\n"));
	...
	else if (pWpsCtrl->WscMode == WSC_SMPBC_MODE)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC_PeerEapolStart:: pEntry->bIWscSmpbcAccept == FALSE\n"));
			return FALSE;
		}

		if (pWpsCtrl->EapMsgRunning == TRUE)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IWSC_PeerEapolStart:: Busy now\n"));
			return FALSE;
		}
...
VOID IWSC_AddSmpbcEnrollee(RTMP_ADAPTER *pAd, UCHAR *pPeerAddr)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<----- IWSC_AddSmpbcEnrollee (WscConfMode = %d, WscStatus = %d)\n", 
		...
		!pIWscInfo->bIWscEntryTimerRunning)
	{
		...
		if (pEntry == NULL)
			pEntry = MacTableInsertEntry(pAd, pPeerAddr, &pAd->StaCfg.wdev, ENTRY_ADHOC, OPMODE_STA, FALSE);
		if (pEntry)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("SMPBC Enrollee - %02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	else if (pIWscInfo->bIWscEntryTimerRunning)
	{
		...
		{
			pEntry->bIWscSmpbcAccept = TRUE;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Accept this peer enrollee - %02x:%02x:%02x:%02x:%02x:%02x\n",
										PRINT_MAC(pEntry->Addr)));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("-----> IWSC_AddSmpbcEnrollee\n"));
	return;
}
...
VOID IWSC_BuildDevQueryFrame(RTMP_ADAPTER *pAd, UCHAR *pOutBuf, USHORT *pIeLen)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("-----> IWSC_BuildDevQueryFrame\n"));
...
BOOLEAN IWSC_IpContentForCredential(RTMP_ADAPTER *pAd)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("pIWscInfo->IpDevCount = %d\n", 
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("pIWscInfo->bAssignWscIPv4 = %d\n", 
	...
	if (pIWscInfo->RegDepth == 0)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("1) SelfIpv4Addr = 0x%08x, PeerIpv4Addr = 0x%08x\n", 
	...
	else
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IMPOSSIBLE!! pIWscInfo->SelfIpv4Addr = %d\n", pIWscInfo->SelfIpv4Addr));
			return FALSE;
		}
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("2) SelfIpv4Addr = 0x%08x, PeerIpv4Addr = 0x%08x\n", 
			pIWscInfo->SelfIpv4Addr,
			pIWscInfo->PeerIpv4Addr));
		return TRUE;
	}
