	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("DLS - MlmeDlsReqAction() \n"));
			...
			for (i = (MAX_NUM_OF_DLS_ENTRY - 1);
				...
				if (!pAd->StaCfg.DLSEntry[i].Valid) {
					...
					if ((HtCapabilityLen != 0) && WMODE_CAP_N(pAd->CommonCfg.PhyMode))
					{
						...
						set_sta_ht_cap(pAd, pEntry, &HtCapability);
	...
	IN RXINFO_STRUC *pRxInfo)
{
	...
	if (pHeader->FC.Order
	    && (!(CLIENT_STATUS_TEST_FLAG(&pAd->MacTab.Content[BSSID_WCID], fCLIENT_STATUS_AGGREGATION_CAPABLE))))
	{
		pProto += 4;
	}
	...
	if (RTMPEqualMemory(EAPOL, pProto, 2)
		...
		if ((Len >=
			...
			if ((pEap->KeyDesc.KeyData[0] == 0xDD)
			    && (pEap->KeyDesc.KeyData[2] == 0x00)
			    && (pEap->KeyDesc.KeyData[3] == 0x0F)
			    && (pEap->KeyDesc.KeyData[4] == 0xAC)
			    && (pEap->KeyDesc.KeyData[5] == 0x02)) {
				pAddr = pEap->KeyDesc.KeyData + 8;	/* Tpe(1), Len(1), OUI(3), DataType(1), Reserved(2) */
				pSTAKey = pEap->KeyDesc.KeyData + 14;	/* Tpe(1), Len(1), OUI(3), DataType(1), Reserved(2), STAKey_Mac_Addr(6) */

				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
					 ("DLS - Receive STAKey Message-1 from %02x:%02x:%02x:%02x:%02x:%02x Len=%d, KeyDataLen=%d\n",
	...
	IN PUCHAR pDA)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Send DLS TearDown Frame \n"));
	...
	IN UINT DlsEntryIdx)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("====> MacTableInsertDlsEntry\n"));
	/* if FULL, return */
...
BOOLEAN MacTableDeleteDlsEntry(
	...
	IN PUCHAR pAddr)
{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("====> MacTableDeleteDlsEntry\n"));
...
INT Set_DlsAddEntry_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
...
INT Set_DlsTearDownEntry_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
