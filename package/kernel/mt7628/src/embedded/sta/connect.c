...
/* Disable scanning when ATE is running. */
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("CNTL - SCAN starts\n"));
	...
	IN MLME_QUEUE_ELEM *Elem)
{
...
#endif /* DOT11R_FT_SUPPORT */
	{
	            ...
	            {
	                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("COEX: ROAMING_START, Remove old protection frame and AFH information\n"));
...
/*			RTMPWPARemoveAllKeys(pAd);*/
	...
	if (P2P_GO_ON(pAd))
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Channel change , old channel:%d new channel:%d !! \n"
						,pAd->P2PChannel,pAd->CommonCfg.Channel));
#ifdef DOT11_N_SUPPORT
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Old ExtCh:%d new ExtCh:%d !! \n"
...
//---Add by shiang
	...
	*/
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WPS is in Prgress; Not sending this LINK UP\n"));
	}
	else
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Auth Mode is WPA2PSK or WPAPSK; Not sending this LINK UP\n"));
		}
	}
#endif /* WIDI_SUPPORT */
...
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("CNTL - !!! Set to short preamble!!!\n"));
#endif /* RELEASE_EXCLUDE */
...
#if defined(MT7636)
	// TODO: Currently, ucBssIndex = 0, need to consider further
	/* Infrom FW to enable RA here */
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: CmdExtStaRecUpdate(%d)\n",
					  __FUNCTION__, 1));

	CmdExtStaRecUpdate(pAd, 0, BSSID_WCID, (1 << STA_REC_AUTO_RATE_PARAMETER));
#endif /* MT7636 */
...
//start MCC when Infra connect
	...
	if ( IS_MT7601(pAd) )
	{
		...
		/* DPD-Calibration */
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("TX DPD Calibration\n"));
...
/* Allow go to sleep after linkdown steps. */
	...
	*/
	if (pAd->StaCfg.WscControl.bWscTrigger)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WPS is in Prgress; Not sending this LINK DOWN\n"));
	}
	else
	{
		...
		if (!MAC_ADDR_EQUAL(pAd->CommonCfg.LastBssid, ZERO_MAC_ADDR))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("LastBssid is zero; Not sending this LINK DOWN\n"));
	}
#endif /* WIDI_SUPPORT */
	...
	IN UCHAR ExtraCh)
{
	...
	*pBwFallBack = 0;
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): Input BW=%d, rf_channel=%d, vht_bw=%d, Channel=%d, vht_cent_ch=%d!\n",
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				...
				pAd->CommonCfg.CentralChannel));
#ifdef DOT11_VHT_AC
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("VHT-CentralCh=%d\n", pAd->CommonCfg.vht_cent_ch));
#endif /* DOT11_VHT_AC */
