			...
			if (RTMP_TIME_AFTER(now_time, pPeerEntry->InitRefTime + (pAd->StaCfg.TdlsInfo.TdlsRssiMeasurementPeriod * ((1000 * OS_HZ)/1000))))
			{
				...
				if (pPeerEntry->RetryCount >= 2)
				{
					...
					if (INFRA_ON(pAd))
					{
						...
						{
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("TDLS - can not find or full the LinkId!\n"));
						}
						else if (idx >= 0)
						{
	...
	IN	PUCHAR			pAddr)
{
	...
	for (idIdx = 0; idIdx < MAX_NUM_OF_TDLS_ENTRY; idIdx++)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("TDLS_ValidIdLookup - Find Link ID with Peer Address %02x:%02x:%02x:%02x:%02x:%02x(%d) \n", 
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("!!! TDLS_ValidIdLookup -  not found !!!\n"));
...
VOID TDLS_AutoSetupByRcvFrame(RTMP_ADAPTER *pAd, HEADER_802_11 *pHeader)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TDLS ===> TDLS_AutoSetupByRcvFrame\n"));
	...
	if ((pTdlsBlackEntry == NULL) && (pTdlsDiscoveryEntry == NULL))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("AutoSetupByRcvFrame trigger discovery request !!!\n"));
		...
		if (LinkId < MAX_NUM_OF_TDLS_ENTRY)
		{
			...
			if (pTdlsPeer)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("TDLS peer entry already on table !!!\n"));
			}
		}
		else if (rv)
		{
	...
	IN PUCHAR pMacAddr)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TDLS ===> TDLS_TunneledProbeRequest\n"));
	...
	IN PUCHAR pMacAddr)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TDLS ===> TDLS_TunneledProbeResponse\n"));
...
#endif /* WFD_SUPPORT */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IF(ra%d) Set_TdlsCapableProc::(bTdlsCapable=%d)\n", 
...
INT	Set_TdlsSetup_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (INFRA_ON(pAd))
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("TDLS - Set_TdlsSetup_Proc() AP Prohibited TDLS !!!\n"));
			return FALSE;
		}
...
INT	Set_TdlsTearDown_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (INFRA_ON(pAd))
	{
		...
		if (idx == -1 || idx == MAX_NUM_OF_TDLS_ENTRY)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("TDLS - Set_TdlsTearDown_Proc() can not find or full the LinkId!\n"));
			return FALSE;
		}
...
INT Set_TdlsAcceptWeakSecurityProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IF(ra%d) Set_TdlsAcceptWeakSecurityProc::(bAcceptWeakSecurity=%d)\n", 
...
INT	Set_TdlsTPKLifeTime_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF(ra%d) Set_TdlsTPKLifeTimeProc::(TdlsKeyLifeTime=%d)\n", 
...
INT	Set_TdlsAutoLinkProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF(ra%d) Set_TdlsAutoLinkProc::(TdlsAutoLink=%d)\n", 
...
Set_TdlsRssiMeasurementPeriodProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF(ra%d) Set_TdlsRssiMeasurementPeriodProc::(RssiMeasurementPeriod = %d secs)\n", 
...
Set_TdlsAutoDiscoveryPeriodProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF(ra%d) Set_TdlsAutoDiscoveryPeriodProc::(AutoDiscoveryPeriod = %d secs)\n", 
...
Set_TdlsAutoSetupRssiThresholdProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF(ra%d) Set_TdlsAutoSetupRssiThresholdProc::(AutoSetupRssiThreshold = %d dbm)\n", 
...
Set_TdlsDisabledPeriodByTeardownProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF(ra%d) Set_TdlsDisabledPeriodByTeardownProc::(DisabledPeriodByTeardown = %d secs)\n", 
...
Set_TdlsAutoTeardownRssiThresholdProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF(ra%d) Set_TdlsAutoTeardownRssiThresholdProc::(AutoTeardownRssiThreshold = %d dbm)\n", 
	...
	if ((token != NULL) && (strlen(token)>1))
	{
		...
		*token = '\0';
		...
		if (INFRA_ON(pAd))
		{
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s(%d):  can not find from %02x:%02x:%02x:%02x:%02x:%02x on TDLS entry !!!\n",
...
INT	Set_TdlsChannelSwitchBW_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
...
#if 0
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("B. %d. @!\n", pAd->CommonCfg.Channel));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!!40MHz Lower !!!\n"));
	}
#endif
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF(ra%d) Set_TdlsChannelSwitchBW_Proc::(TdlsChannelSwitchBW=%d)\n", 
...
INT	Set_TdlsChannelSwitchDisable_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (INFRA_ON(pAd))
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF(ra%d) Set_TdlsChannelSwitchDisable_Proc\n", pObj->ioctl_if));

			return TRUE;
		}
#ifdef RTMP_MAC_USB
		RTMP_FREQ_CAL_ENABLE(pAd);
#endif /* RTMP_MAC_USB */
	}
	...
	IN HT_CAPABILITY_IE *pHtCapability)
{
...
#endif /* DOT11_N_SUPPORT */
	{
		...
		CLIENT_STATUS_SET_FLAG(pEntry, fCLIENT_STATUS_WMM_CAPABLE);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TDLS - Receive Peer Legacy STA \n"));
	}
	...
	switch (TDLSActionType)
	{
		...
		case TDLS_ACTION_CODE_CHANNEL_SWITCH_RESPONSE:
			*MsgType = MT2_PEER_TDLS_CH_SWITCH_RSP;
			break;
		default:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("TDLS_MsgTypeSubst : unsupported TDLS Action Type(%d); \n", TDLSActionType));
			return FALSE;		
	}	
    ...
    IN ULONG            DataByteCount)
{
    ...
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("TDLS_CheckTDLSframe ===> \n"));
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("TDLS <=== TDLS_CheckTDLSframe \n"));
#endif // RELEASE_EXCLUDE //
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("CNTL - TDLS_CntlOidTDLSRequestProc set %02x:%02x:%02x:%02x:%02x:%02x with Valid=%d, Status=%d\n",
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("CNTL - TDLS Capable disable !!!\n"));
		return;
	}

	if (!INFRA_ON(pAd))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("CNTL - STA do not connect to AP !!!\n"));
		return;
	}
	...
	if (Idx < MAX_P2P_GROUP_SIZE)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::find entry in p2p table\n", __FUNCTION__));
			...
			!MAC_ADDR_EQUAL(pTDLS->MacAddr, pInfo->wfd_alternate_mac_addr_ie))
		{
			...
			*/
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WFD alternate mac address is:%02x:%02x:%02x:%02x:%02x:%02x\n", PRINT_MAC(pTDLS->MacAddr)));
		}
	}
#endif /* WFD_SUPPORT */
			...
			for (i = 0; i < MAX_NUM_OF_TDLS_ENTRY; i++)
			{
				if (!pAd->StaCfg.TdlsInfo.TDLSEntry[i].Valid)
				{
					...
					MlmeEnqueue(pAd,
								...
								&MlmeTdlsReq,
								0);
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("CNTL - TDLS setup case\n"));
					break;
				}
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("CNTL - TDLS  do not find vaild entry !!!!\n"));
			}	
		}
		else
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("CNTL - TDLS not changed in Idx = -1 (Valid=%d)\n", pTDLS->Valid));
			...
			for (i = 0; i < MAX_NUM_OF_TDLS_ENTRY; i++)
			{
				if ((pAd->StaCfg.TdlsInfo.TDLSEntry[i].Valid) &&(pAd->StaCfg.TdlsInfo.TDLSEntry[i].Status < TDLS_MODE_CONNECTED))
				{
					...
					MlmeEnqueue(pAd,
								...
								&MlmeTdlsReq,
								0);
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("CNTL - TDLS restart case\n"));
					break;
				}
			}	
		}
		else
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("CNTL - TDLS not changed in Idx = MAX_NUM_OF_TDLS_ENTRY (Valid=%d)\n", pTDLS->Valid));
	}
	else	/* found one in entry */
	{
			...
			pAd->StaCfg.TdlsInfo.TDLSEntry[Idx].WfdEntryInfo.wfd_PC == WFD_PC_P2P)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("CNTL - Peer TDLS PC is P2P\n"));
		}
		else
#endif /* WFD_SUPPORT */
		if ((!pTDLS->Valid) && (pAd->StaCfg.TdlsInfo.TDLSEntry[Idx].Status >= TDLS_MODE_CONNECTED))
		{
			...
			MlmeEnqueue(pAd,
						...
						&MlmeTdlsReq,
						0);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("CNTL - start tear down procedure\n"));
		}
		else if ((pTDLS->Valid) && (pAd->StaCfg.TdlsInfo.TDLSEntry[Idx].Status >= TDLS_MODE_CONNECTED)) 
		{
			...
			MlmeEnqueue(pAd,
						...
						&MlmeTdlsReq,
						0);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("CNTL - TDLS retry setup procedure\n"));
		}
		else
		{
#ifdef WFD_SUPPORT
...
#endif /* WFD_SUPPORT */
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("CNTL - TDLS not changed in entry - %d - Valid=%d, Status=%d\n",
	...
	IN	PUCHAR			pAddr)
{
	...
	for (i = 0; i < MAX_NUM_OF_TDLS_ENTRY; i++)
	{
		...
		if (pTDLS->Valid && MAC_ADDR_EQUAL(pAddr, pTDLS->MacAddr))
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("TDLS_SearchLinkId - Find Link ID with Peer Address %02x:%02x:%02x:%02x:%02x:%02x(%d) \n", 
	...
	if (i == MAX_NUM_OF_TDLS_ENTRY)
	{
		if (empty == 0)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("TDLS_SearchLinkId -  not found and full\n"));
			return MAX_NUM_OF_TDLS_ENTRY;
		}
		else
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("TDLS_SearchLinkId -  not found\n"));
			return -1;
		}	
	}
	...
	OUT UCHAR	TIIe[])
{
	...
	*pTILen = 0;
	...
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (action header) \n", __FUNCTION__));
		return FALSE;	
	}
	...
	if (RemainLen < 1)
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (dialog token) \n", __FUNCTION__));
		return FALSE;
	}	
	*pToken = *Ptr;
	...
	if (RemainLen < 2) 
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (capability) \n", __FUNCTION__));
		return FALSE;
	}	
	...
	{	
		...
		{				
			...
			default:
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("PeerTdlsSetupReqSanity - unrecognized EID = %d\n", pEid->Eid));
				break;
		}
	...
	if (LinkIdLen == 0)
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet - (link identifier) \n", __FUNCTION__));
		return FALSE;
	}
	else
	{
		...
		{
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("%s() - It's not my BSSID\n", __FUNCTION__));
			return FALSE;
		}	
		else if (!MAC_ADDR_EQUAL(LinkIdResponderAddr, pAd->CurrentAddress))
		{
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("%s() - It's not my MAC address\n", __FUNCTION__));
			return FALSE;
		}
	}
	...
	OUT UCHAR	TIIe[])
{
	...
	*pTILen = 0;
	...
	if (RemainLen < 2)
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (status code) \n", __FUNCTION__));
		return FALSE;
	}	
	...
	*pToken = *Ptr;
	...
	{	
		switch(pEid->Eid)
		{
			...
			default:
				// Unknown IE, we have to pass it as variable IEs
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("PeerTdlsSetupRspSanity - unrecognized EID = %d\n", pEid->Eid));
				break;
		}
	...
	{	
		switch(pEid->Eid)
		{
			...
			case IE_ADD_HT2:
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("PeerTdlsSetupConfSanity - wrong IE_ADD_HT. \n"));
				}
				break;
			...
			default:
				// Unknown IE, we have to pass it as variable IEs
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("PeerTdlsSetupConfSanity - unrecognized EID = %d\n", pEid->Eid));
				break;
		}
	...
	OUT UCHAR	FTIe[])
{
	...
	*pFTLen = 0 ;
	...
	if (RemainLen < (LENGTH_802_11 + LENGTH_802_1_H + LENGTH_TDLS_PAYLOAD_H))
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (cation header) \n", __FUNCTION__));
		return FALSE;	
	}
	...
	if (RemainLen < 2) 
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (reason code) \n", __FUNCTION__));
		return FALSE;	
	}
	...
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_WARN, ("PeerTdlsTearDownSanity --> Invaild packet length - (link identifier) \n"));
		return FALSE;	
	}
	// It's not my BSSID
	if (!MAC_ADDR_EQUAL(Ptr+2, pAd->CommonCfg.Bssid))
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_WARN, ("PeerTdlsTearDownSanity --> It's not my BSSID\n"));
		return FALSE;
	}	
	...
	{	
		switch(pEid->Eid)
		{
			...
			default:
				// Unknown IE, we have to pass it as variable IEs
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("PeerTdlsTearDownSanity - unrecognized EID = %d\n", pEid->Eid));
				break;
		}
	...
	else
	{
		...
		if (!MAC_ADDR_EQUAL(pAd->CurrentAddress, LinkIdInitiatorAddr))
		{
			...
			{
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("%s() - It's not my Address\n", __FUNCTION__));
				return FALSE;
			}
			else
			{
	...
	OUT UCHAR	*pToken)
{
	...
	*pToken = *Ptr;
	...
	if ((RemainLen < 20) || (Ptr[0] != IE_TDLS_LINK_IDENTIFIER) || (Ptr[1] != 18))
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (link identifier) \n", __FUNCTION__));
		return FALSE;
	}
	if (!MAC_ADDR_EQUAL(Ptr+2, pAd->CommonCfg.Bssid))
	{
	...
	*pToken = *Ptr;
	...
	{	
		switch(pEid->Eid)
		{
			...
			default:
				/* Unknown IE, we have to pass it as variable IEs */
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("PeerTdlsDiscovRspSanity - unrecognized EID = %d\n", pEid->Eid));
				break;
		}
	...
	OUT USHORT *pChSwitchTimeOut)
{
	...
	*pNewExtChannelOffset = 0;
	...
	if (RemainLen < (LENGTH_802_11 + LENGTH_802_1_H + LENGTH_TDLS_PAYLOAD_H))
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (ation header) \n", __FUNCTION__));
		return FALSE;	
	}
	...
	if (RemainLen < 1)
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (target channel) \n", __FUNCTION__));
		return FALSE;
	}	
	...
	*pTargetChannel = *Ptr;
	...
	if (RemainLen < 1)
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("%s() - Invaild packet length - (regulatory class) \n", __FUNCTION__));
		return FALSE;
	}
	
	*pRegulatoryClass = *Ptr;
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("%s() - Regulatory class = %d \n", __FUNCTION__, *pRegulatoryClass));
	...
	{	
		switch(pEid->Eid)
		{
			case IE_SECONDARY_CH_OFFSET:
				...
				{
					rv = FALSE;
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - wrong IE_SECONDARY_CH_OFFSET. \n", __FUNCTION__));
				}
				break;
			...
			case IE_TDLS_LINK_IDENTIFIER:
				...
				{
					rv = FALSE;
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - wrong IE_TDLS_LINK_IDENTIFIER. \n", __FUNCTION__));
				}
				break;
			...
			case IE_TDLS_CHANNEL_SWITCH_TIMING:
				...
				{
					rv = FALSE;
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s() - wrong IE_TDLS_CHANNEL_SWITCH_TIMING. \n", __FUNCTION__));
				}
				break;

			default:
				/* Unknown IE, we have to pass it as variable IEs */
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s() - unrecognized EID = %d\n", __FUNCTION__, pEid->Eid));
				break;
		}
	...
	OUT UCHAR						*pSA)
{
	...
	*/
	...
	{
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("%s() - "
	...
	IN PUCHAR arg)
{
	...
	for (i=0; i<MAX_NUM_OF_TDLS_ENTRY; i++)
	{
		if ((pAd->StaCfg.TdlsInfo.TDLSEntry[i].Valid) && (pAd->StaCfg.TdlsInfo.TDLSEntry[i].Status == TDLS_MODE_CONNECTED))
		{
			...
			/*MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%-8d\n", pAd->StaCfg.DLSEntry[i].TimeOut)); */

			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\n"));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\n%-19s%-4s%-4s%-4s%-4s%-7s%-7s%-7s","MAC", "AID", "BSS", "PSM", "WMM", "RSSI0", "RSSI1", "RSSI2"));
#ifdef DOT11_N_SUPPORT			
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%-8s%-10s%-6s%-6s%-6s%-6s", "MIMOPS", "PhMd", "BW", "MCS", "SGI", "STBC"));
#endif /* DOT11_N_SUPPORT */
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\n%02X:%02X:%02X:%02X:%02X:%02X  ",
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%-4d", (int)CLIENT_STATUS_TEST_FLAG(pEntry, fCLIENT_STATUS_WMM_CAPABLE)));
...
#ifdef DOT11_N_SUPPORT
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%-8d", (int)pEntry->MmpsMode));
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%-6d", pEntry->HTPhyMode.field.STBC));
#endif /* DOT11_N_SUPPORT */
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%-10d, %d, %d%%\n", pEntry->DebugFIFOCount, pEntry->DebugTxCount, 
...
VOID TDLS_DisablePktChannel(RTMP_ADAPTER *pAd, UCHAR QSel)
{
	...
	if (pAd->chipCap.hif_type == HIF_RLT) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): FixME!!\n", __FUNCTION__));
		return;
	}
#endif /* RLT_MAC */
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("unknow Out-Q\n"));
		return;
	}

	if (MTxCycle >= 10)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Polling %s Out-Q max\n", (QSel == FIFO_EDCA ? "EDCA" : "HCCA")));
	...
	IN UCHAR TargetChannelBW)
{
	...
	{	
		...
		if (pAd->MACVersion == 0x28600100)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!!rt2860C !!! \n" ));
		}	

		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("A. %d. %ld @!\n", TargetChannel, (jiffies * 1000) / OS_HZ));
	}
	...
	{	
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("B. %d. %ld @!\n", TargetChannel, (jiffies * 1000) / OS_HZ));
    }
    else
#endif /* DOT11_N_SUPPORT */
    {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%d... %ld @!\n", TargetChannel, (jiffies * 1000) / OS_HZ));
	}
	...
	IN UCHAR FrameType)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,  ("%s() --> unknow Entry !!!\n", __FUNCTION__));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s (error:: can't allocate NDIS PACKET)\n", __FUNCTION__));
		return NDIS_STATUS_FAILURE;
	}

#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s - len=%d, AID=%d, DA=%02x:%02x:%02x:%02x:%02x:%02x\n", 
							__FUNCTION__, HdrLen+DataLen, pEntry->Aid, PRINT_MAC(pHeader802_3)));
#endif /* RELEASE_EXCLUDE */
	...
	{
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_LOUD, ("TxRing(%d) full, SwQueue Len = %lu, packet required frag =%d!\n",
						QueIdx, pAd->TxSwQueue[QueIdx].Number, RTMP_GET_PACKET_FRAGMENTS(pPacket)));
#ifdef RTMP_MAC_PCI
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("DeqPkt -> Not enough free TxD[%d] (TX_CTX_IDX=%u, TxSwFreeIdx=%u)!!!\n",
	...
	IN	BOOLEAN bWaitACK)
{
	...
	if (((pAd->StaCfg.wdev.AuthMode == Ndis802_11AuthModeWPA) || 
		...
		(pAd->StaCfg.wdev.PortSecured == WPA_802_1X_PORT_NOT_SECURED)) 
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("===> %s() - Port not Secured !!!\n", __FUNCTION__));
		return;
	}
...
#ifdef RTMP_MAC_USB
#if 0
	RtmpUSBNullFrameKickOut(pAd, HCCA_PIPE | MGMT_USE_SPECIFIC_FLAG, NullFrame, Length);
#endif
	if (pAd->NullContext[1].InUse == FALSE)
	{
		...
		RTMPWriteTxInfo(pAd, pTxInfo, (USHORT)(Length + TXWISize), TRUE, EpToQueue[MGMTPIPEIDX], FALSE,  FALSE);
		pTxInfo->QSEL = FIFO_HCCA;
		...
		write_tmac_info(pAd, (UCHAR *)pTxWI, &mac_info, &Transmit);
...
#endif /* RT_BIG_ENDIAN */
		RTMPMoveMemory(&pWirelessPkt[TXWISize + TXINFO_SIZE], NullFrame, Length);
#ifdef RT_BIG_ENDIAN
		RTMPFrameEndianChange(pAd, (PUCHAR)&pWirelessPkt[TXINFO_SIZE + TXWISize], DIR_WRITE, FALSE);
#endif /* RT_BIG_ENDIAN */
		pAd->NullContext[1].BulkOutSize =  TXINFO_SIZE + TXWISize + Length + 4;
	...
	else
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("not enough space in MgmtRing, MgmtRingFullCount=%ld!\n",
									pAd->RalinkCounters.MgmtRingFullCount));
	}
#endif /* RTMP_MAC_PCI */
