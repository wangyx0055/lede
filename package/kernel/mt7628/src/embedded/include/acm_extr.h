...
/*
	Suggested by Hamilton, Mark [Mark.Hamilton@Polycom.com]

	Consider this scenario:  A wireless phone in a voice call.

	This results in traffic typically like:

	1) A two-way voice stream.  Actually, this is often two one-way voice
		streams, but that doesn・t affect this example, so I・ll ignore it.
		Voice streams can vary, for purposes of discussion let・s say it is
		a 30ms interval, 288 byte (each way), AC_VO traffic stream between
		this device and the "other end" of the call.

	2) A call control stream.  This typically means one or two UDP or TCP
		frames every few seconds carrying call state/status information,
		keepalives, etc., between the phone and the call server (PBX, etc.),
		usually request/response but with requests initiated at either/both
		ends, either AC_VI or AC_BE (let・s say AC_VI for this example again,
		it doesn・t really matter).

	3) Other background traffic.
		DHCP, ARP, syslog, network management stuff, etc.  Uses AC_BE, and
		is quite intermittent.

	To maximize power savings, what I・d really like to do is:

	1) Wake up the radio every 30ms, as a voice packet is ready.

	2) Send the voice packet.

	3) Quickly receive any/all downlink traffic coming to the phone.
		This includes the expected voice packet, but also anything else
		that needs to be processed.

	4) Whenever there is something other than voice to send, send it.
		Note that this is likely to be the result of processing either some
		downlink packet received and then run through various levels of
		process, or the result of some local (user) action on the device or
		local timed event.  All of those can happen quite asynchronously to
		the voice packets.

	5) Maximize the time between #3 and the next #1, and spend that time with
		the radio in low power state, exceptionally waking only for #4 packets.

	I believe I have three choices for U-APSD settings (other than leave
	everything legacy, which we all agree is yucky):

	1) Set AC_VO to DE and TE ("DanTE"), leave the rest legacy.

	2) Set all four ACs to DanTE.

	3) Set AC_VO to DanTE and the other three to DE only.

	With option #1, I have to figure out that there is non-voice downlink
	traffic waiting for me, and I have to PS-Poll to get it.
	This means paying a lot of attention to Beacons (frequently), and extra
	traffic for the Polls.  Neither of those are good for battery life.

	With option #2, I get "what I・d really like to do" items 1 through 3,
	but whenever I send an uplink frame for #4, I have to wait for the
	delivery that I accidentally triggered.  Note, especially, that with
	#4 type traffic, there is virtually never going to be any downlink
	response waiting for me, so this delivery will virtually always be
	a QoS-Null.  That・s a waste of time and battery.

	Further, it is quite possible that there will a voice frame queued when
	my #4 event decides to send the uplink, so I might get that.  Of course,
	I・m expecting, and would like to handle, my downlink voice frames on the
	same timing as my uplink voice frames that is, in response to a #2 packet,
	not a #4 packet.  Can I handle this, sure, but it・s more implementation
	stuff to deal with.

	Option #3 is perfect.  I get exactly what I want, which maximizes my
	battery life and simplifies my implementation.

	So, I think there is a use for DE only ACs.  And, we only get those with
	TSPEC control over U-APSD (unfortunately I・d love to change that if it
	were only possible at this point!).
*/
/*
	TODO:

	1) Support for ADDTS Action frames before the establishment of an RSN
	SA is not recommended, due to the potential for Denial-of-Service
	attacks.

	2) When Protected Management Frames is supported, ADDTS frames should
	be protected.

	3) WFA security policy recommends that ADDTS Requests only be accepted
	after an RSN SA is established.  Thus, at least one Access Category must
	not require Admission Control, so that the 802.1X frames can be exchanged
	(on that AC) before a traffic stream is admitted, whenever RSN security
	is being used.
*/
...
#ifdef RELEASE_EXCLUDE
/*
	In WMM ACM TG discussion,
	We have at TEs observed some implementations which set b0 in TS Info.
*/
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
/*
	In IEEE802.11e, for the field, this field has no particular
	guaranteed resultant behaviour, i.e. the sender can set it to any
	value and the receiver can interpret it any way it wants.

	In WMM ACM plan, No change to spec, but ensure that all Minimum PHY
	Rates used in TSPECs in the test spec correspond to an operational
	rate of both the AP and the STA.

	The Minimum PHY Rate in a TSPEC in an ADDTS Request shall be in the AP・s
	operational rate set, if the TS is uplink or bidi.  AND The Minimum PHY
	Rate in a TSPEC in an ADDTS Request shall be in the STA・s operational
	rate set, if the TS is downlink or bidi
*/
#endif /* RELEASE_EXCLUDE */
...
/*
	In WMM ACM TG discussion,
	the AP may deliver all buffered MSDUs and MMPDUs to the STA during an
	SP triggered by the STA.
	If the STA wants only 2, 4 or 6 to be delivered, then it can pass
	1, 2 or 3 in the Max SP Length of the QoS Info field in the WMM
	Information IE in the Association Request
	(even though all the four U-APSD flags are 0).
*/
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
/*
	A STA FAILs the MT adherence tests if, over the 30-s sniffing
	period:

	Over the 30-s sniffing period the used time is more than 31.5 times
	the Medium Time (this corresponds to a 5% overrun: 3.3% for the effect
	described previously, i.e. wireshark sniffer effect and 1.7% for
	measurement error, retransmission dribble, and so on);

	OR if:
	Any one-second sniffing period has a used time more than 2.03 times
	the Medium Time (this corresponds to a factor of two for the effect
	described previously, and 1.7% for measurement error, retransmission
	dribble, and so on).
*/
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
/*
 	U-APSD changes on non-ACM AC recommendation

	Have a test where all the TSPEC body subfields are set to zero,
	other than:
	TS Info (set as normal, including TID which is subject to
			replacement rules as normal)
	Nominal MSDU size (set to 0x0001, i.e. 1 octet, not fixed)
	Mean Data Rate (set to 8, i.e. 8 bps) i.e. one packet per second
	Minimum PHY Rate (set as normal)
	SBA (set to 0x2001)
*/
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
/*
	WMM-AC is probably the first time Action frames are being used in
	(traditional) power-save mode.  We have at TEs observed problems
	where an AP failed to obey the PS rules, e.g. sent an ADDTS Response
	without setting the appropriate TIM bit and waiting for the STA to send
	a PS Poll.
*/
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
/*
	Set b6 of the "Reserved" octet in the WMM Parameter IE (see section 2.2.2
	of WMM spec) to indicate that the sender thereof is a WMM-AC AP.
*/
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
/*
	Reserved fields or bits must be set to 0 on tx and ignored on rx, and
	that reserved field values or bit combinations must not be used.
*/
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
/*
	The formula in A.3 assumes "Normal ack"; if the AP understood "No ack"
	then it could return a smaller Medium Time.
	Say that an AP may reject TSPECs with Ack Policy other than Normal ack
	using status code Refused.
*/
#endif /* RELEASE_EXCLUDE */
...
#define ACM_CC_FUNC_11N_AGG				/* 11N AMSDU/AMPDU aggregation support */
#endif /* DOT11_N_SUPPORT */

//#define WMM_ACM_PKT_NUM_DEBUG			/* Packet Number debug */
...
//#define ACM_CC_FUNC_RETRY_DISABLE		/* For STATION */

//#define ACM_CC_FUNC_PS_MGMT_FME		/* Send MGMT frame with PM=1 */
#ifdef RELEASE_EXCLUDE
/*
	But I am not prefer to use action frame with PM = 1 to get the repsone in
	UAPSD mode because (1) application needs to send a trigger frame to get the
	reponse; (2) if Max SP Length is NOT 0 (all packets), we maybe not get the

	reponse after the trigger frame, maybe the response	is still queued in VO

	queue except it is always put in the head of queue.
*/
#endif /* RELEASE_EXCLUDE */
...
#define WMM_ACM_DEBUG_TIME_MAX_NUM_REC	200




/* --------------------------- Type Redefinition --------------------------- */
#if 0 /* open the redefinition when your system does not support */
#ifndef UCHAR
...
#define ACMR_IS_AP_MODE(__pAd)			((__pAd)->OpMode == OPMODE_AP)
#define ACMR_IS_STA_MODE(__pAd)			((__pAd)->OpMode == OPMODE_STA)

/* get TXOP of the AC */
#define ACMR_TXOP_BSS_GET				ACM_InfoTxopBssGet
/* check if WMM is enabled */
#define ACMR_WMM_IS_ENABLED				ACM_InfoWmmStatusGet
/* get AP MAC */
#define ACMR_AP_ADDR_GET				ACM_InfoApAddrGet
/* get the device entry of MAC */
#define ACMR_STA_ENTRY_GET				ACMP_InfoStaEntryGet
/* get the port secure */
#define ACMR_IS_PORT_SECURE				ACM_InfoPortSecureGet
/* check if preamble is short */
#define ACMR_STA_IS_SPREAMBLE			ACM_InfoShortPreambleGet
/* get channel busy time */
#define ACMR_CHAN_BUSY_GET				ACM_InfoChanBusyGet
...
#define ACMR_CHAN_UTIL_GET(__pAd)		(__pAd)->phy_ctrl.QloadChanUtil

	/* get TXOP of the AC */
#define ACMR_AP_TXOP_BSS_GET(__pAd, __AcId)	\
		((__pAd)->ApCfg.BssEdcaParm.Txop[(__AcId)])

	/* check if WMM is enabled */
#define ACMR_AP_WMM_IS_ENABLED(__pAd)	(__pAd)->ApCfg.MBSSID[BSS0].wdev.bWmmCapable
...
#define ACMR_CHAN_GET(__pAd)						(__pAd)->CommonCfg.Channel
	...
	/* get current channel utilization */
#define ACMR_CHAN_UTILIZATION_GET(__pAd)			(__pAd)->phy_ctrl.QloadChanUtil
...
#define ACMR_SUP_RATE_MAX_GET(__pAd, __pCdb, __MaxRate)					\
		{																\
			RtmpDrvMaxRateGet(__pAd,									\
							__pCdb->MaxHTPhyMode.field.MODE,			\
							__pCdb->MaxHTPhyMode.field.ShortGI,			\
							__pCdb->MaxHTPhyMode.field.BW,				\
							__pCdb->MaxHTPhyMode.field.MCS,				\
							&__MaxRate);								\
		};
#else
...
#define ACMR_SUP_RATE_CHECK(__pAd, __Rate, __FlgIsSupRate)				\
		{																\
			UINT32 __RateIndex = ((__Rate << 1) / 1000000);				\
			UINT32 __IdRate, __RateLen;									\
			...
			{															\
				if (__RateIndex == (UINT32)(__pRateSup[__IdRate]&0x7f))	\
				{														\
			...
			{															\
				...
				for(__IdRate=0; __IdRate<__RateLen; __IdRate++)			\
				{														\
					if (__RateIndex == (UINT32)(__pRateExt[__IdRate]&0x7f))	\
					{													\
						__FlgIsSupRate = 1; break;						\
					}													\
				}														\
...
#define ACMR_IS_11N_ONLY_SUPPORT(__pAd, __pCdb)			\
		(WMODE_HT_ONLY((__pAd)->CommonCfg.PhyMode) || \
...
#define ACMR_SUP_RATE_CHECK_11N(__pAd, __pCdb, __Rate, __FlgIsSupRate)	\
		{																\
			...
			ACMR_DEBUG(ACMR_DEBUG_TRACE,								\
						("acm_msg> 20/40 %d, SGI %d, Rate %d\n",		\
						__FlgIs2040, __FlgIsSGI, __Rate));				\
...
#define ACMR_IS_11N_SUPPORT(__pAd)						\
		(WMODE_CAP_N(__pAd)->CommonCfg.PhyMode)
...
#define ACMR_STA_IS_11N_SUPPORT(__pCdb)					\
	...
	/* get BG protection flag in ERP element */
#define ACMR_ERP_IS_BG_PROTECTION_ENABLED(__pAd)		\
		(__pAd->ApCfg.ErpIeContent & 0x02)

#define ACMR_STATION_IS_NOT_IN_PS_MODE(__pCdb)			\
		(__pCdb->PsMode != PWR_SAVE)
	...
	/* get TXOP of the AC */
#define ACMR_STA_TXOP_BSS_GET(__pAd, __AcId)	ACMR_TXOP_AP_GET((__pAd), (__AcId))

	/* check if WMM is enabled */
#define ACMR_STA_WMM_IS_ENABLED(__pAd)		(__pAd)->MlmeAux.APEdcaParm.bValid
...
#define ACMR_STA_PS_MODE_ACTIVE(__pAd)										\
	{																		\
		...
		/*7636 psm*/													\
		if ((__pAd)->StaCfg.PwrMgmt.Psm == PWR_SAVE)								\
		{																	\
...
#define ACMR_STA_PS_MODE_RECOVER(__pAd)										\
	{																		\
		...
		{																	\
			...
			if (!((__pAd)->StaCfg.wdev.UapsdInfo.bAPSDCapable &&					\
				(__pAd)->CommonCfg.APEdcaParm.bAPSDCapable))				\
				RTMPSendNullFrame((__pAd), (__pAd)->CommonCfg.TxRate, FALSE, PWR_SAVE);\
			else RTMPSendNullFrame((__pAd), (__pAd)->CommonCfg.TxRate, TRUE, PWR_SAVE);\
...
#define ACMR_UAPSD_BACKUP(__pAd)												\
	...
	/* get station power save, active or power save */
#define ACMR_IS_IN_ACTIVE_MODE(__pAd)				\
		(__pAd->StaCfg.PwrMgmt.Psm == PWR_ACTIVE)

	/* get WMM & UAPSD Capability */
#define ACMR_WMM_CAPABLE_GET(__pAd)					\
		__pAd->CommonCfg.bWmmCapable

#define ACMR_APSD_CAPABLE_GET(__pAd)				\
		__pAd->StaCfg.wdev.UapsdInfo.bAPSDCapable

	/* Retry count change */
#define ACMR_RETRY_GET(__pAd, __RetryCountOldSettings)						\
	{																		\
		UINT32 __CSR=0;														\
		RTMP_IO_READ32((__pAd), TX_RTY_CFG, &__CSR);						\
		if ((__CSR & 0x0000FFFF) != 0x0000)									\
			(__RetryCountOldSettings) = (__CSR & 0x0000FFFF);				\
	}

/*
	Note: In USB, RTMP_IO_READ32/RTMP_IO_WRITE32 will suspend, do not
	call protect, ex: spin_lock.
*/
#ifdef ACM_CC_FUNC_RETRY_DISABLE
...
#else
#define ACMR_RETRY_DISABLE(__pAd)
#endif /* ACM_CC_FUNC_RETRY_DISABLE */

/*
	Note: In USB, RTMP_IO_READ32/RTMP_IO_WRITE32 will suspend, do not
	call protect, ex: spin_lock.
*/
#define ACMR_RETRY_ENABLE(__pAd, __RetryCountOldSettings)					\
	{																		\
		UINT32 __CSR=0;														\
		RTMP_IO_READ32((__pAd), TX_RTY_CFG, &__CSR);						\
		__CSR = __CSR | (__RetryCountOldSettings);							\
		RTMP_IO_WRITE32((__pAd), TX_RTY_CFG, __CSR);						\
	}
...
#define ACMR_DEBUG_TRACE			DBG_LVL_TRACE
#define ACMR_DEBUG_ERR				DBG_LVL_ERROR
#define ACMR_SEM_FAIL_PRINT()		\
	ACMR_DEBUG(ACMR_DEBUG_ERR, ("acm_err> Semaphore Lock/Unlock! %s\n", __FUNCTION__))
...
#define ACMR_LIST_ENTRY				RT_LIST_ENTRY

/* init the list */
#define ACMR_LIST_INIT				initList

/* remove one from the list */
#define ACMR_LIST_REMOVE_FRM_HEAD	removeHeadList

/* insert one to the list */
#define ACMR_LIST_INSERT_TO_TAIL	insertTailList

#define ACMR_LIST_ALLOC_AND_INSERT_TO_TAIL(__pAd, __pStreamList, __pStreamReq)\
{																			\
	ACM_STREAM *__pStream;													\
	__pStream = ACM_TC_Duplicate(__pAd, __pStreamReq);						\
	ACMR_LIST_INSERT_TO_TAIL(__pStreamList, ((ACMR_LIST_ENTRY *)__pStream));\
}

/* insert a ACL station */
#define ACMR_ACL_INSERT_TAIL(__pAd, __pEntry)							\
{																		\
...
#define ACMR_ACL_REMOVE_HEAD(__pAd, __pEntry)							\
{																		\
	ACMR_LIST *__pList = &((ACM_CTRL_BLOCK *)__pAd->pACM_Ctrl_BK)->ACL_List;\
	if ((__pList != NULL) && (__pEntry != NULL))						\
...
#define ACM_ACL_SIZE_GET(__pAd, __Size)								\
{																		\
	ACMR_LIST *__pList = &((ACM_CTRL_BLOCK *)__pAd->pACM_Ctrl_BK)->ACL_List;\
	if (__pList != NULL)												\
...
#define ACM_ACL_ENTRY_GET(__pAd, __pMac, __FlgIsFound, __pEntry)		\
{																		\
	ACMR_LIST *__pList = &((ACM_CTRL_BLOCK *)__pAd->pACM_Ctrl_BK)->ACL_List;\
	...
	{																	\
		...
		{																\
			...
			insertTailList((__pList), (RT_LIST_ENTRY *)(__pAclEntry));		\
			__FlgIsFound = TRUE;										\
			break;														\
		}																\
		insertTailList((__pList), (RT_LIST_ENTRY *)(__pAclEntry));			\
...
#define ACM_ACL_ENTRY_DEL(__pAd, __pMac)								\
{																		\
	ACMR_LIST *__pList = &((ACM_CTRL_BLOCK *)__pAd->pACM_Ctrl_BK)->ACL_List;\
	UINT32 __Size, __IdEntry;											\
	ACM_ACL_ENTRY *__pAclEntry;											\
...
#define ACMR_ENCRYPT_TKIP					Ndis802_11TKIPEnable
#define ACMR_ENCRYPT_AES					Ndis802_11AESEnable
...
#define ACMR_MGMT_FME_FREE					MlmeFreeMemory


/* ------------------ MACRO: Utility Related ------------------ */

	/*	atoi & atoh */
#define ACMR_ARG_ATOI(__pArgv)				simple_strtol((RTMP_STRING *) __pArgv, 0, 10)
#define ACMR_ARG_ATOH(__Buf, __Hex)			AtoH((RTMP_STRING *) __Buf, __Hex, 1)
...
#define ACMR_HT_IS_BA_BUILT(__pCdb, __UP)							\
	...
	/* if use AMPDU */
#define ACMR_11N_IS_AMPDU_USED(__pCdb, __pPacket)					\
	((__pCdb->TXBAbitmap & (1<<(RTMP_GET_PACKET_UP(__pPacket)))) != 0)

	/* if use AMSDU */
#define ACMR_11N_IS_AMSDU_USED(__pCdb, __pPacket)					\
	((!ACMR_11N_IS_AMPDU_USED(__pCdb, __pPacket)) &&				\
	(CLIENT_STATUS_TEST_FLAG(__pCdb, fCLIENT_STATUS_AMSDU_INUSED)))

#define ACMR_11N_IS_AMSDU_ENABLED(__pCdb)							\
	(CLIENT_STATUS_TEST_FLAG(__pCdb, fCLIENT_STATUS_AMSDU_INUSED))

#define ACMR_11N_AMSDU_MAX_SIZE(__pAd)								\
	(RX_BUFFER_AGGRESIZE)
...
#define ACM_ACK_POLICY_BLOCK			0x03 /* immediate BA */
	UCHAR  AckPolicy;
...
typedef struct _ACM_TSPEC {
	...
	/*
		...
		<20101013>
		In the case of aggregated MSDUs (A-MSDU), this field specifies the
		nominal size, in octets, of the A-MSDU.

		OR When A-MSDUs are used, this field instead similarly specifies the
		nominal size, in octets, of the A-MSDUs; the Fixed subfield has the
		same meaning as for MSDUs.

		Note that Nominal MSDU, in the case of A-MSDUs, is the size of the
		A-MSDU.
	*/
...
#define ACM_TSPEC_SUSPENSION_MIN			((UINT32)500000)
	...
	*/
	...
	/*
		...
		<20101013>
		The Mean Data Rate should be calculated as follows:

		Mean data Rate = (Nominal MSDU size x 8) / MSDU Aggregation) x pps 
		Where; "MSDU Aggregation" is the number of MSDUs in the Nominal MSDU

		OR
		The Mean Data Rate field is 4 octets long and contains an unsigned
		integer that specifies the average data rate, in units of bits per
		second, for transport of MSDUs/A-MSDUs belonging to this traffic
		within the bounds under this traffic specification. The Mean Data Rate
		does not include the MAC and PHY overheads incurred in transferring
		the MSDUs/A-MSDUs, except that for A-MSDUs it includes the A-MSDU
		subframe header and pad overheads.
	*/
	UINT32  MeanDataRate;
	...
	*/
	...
	/*
		Maximum Burst Size or Nominal MSDU Aggregation (11n)
		...
		<20101013> Nominal MPDU Aggregation:

		If the TS Info Ack Policy Field Values is set to 00, then the TSPEC
		contains the optional Maximum Burst Size; if the TS Info Ack Policy
		Field Values is set to 11, then the TSPEC contains a value for Nominal
		MPDU Aggregation.

		The Nominal MPDU Aggregation field is 4 octets long and contains an
		unsigned integer that indicates the average number of Nominal MSDUs
		belonging to the traffic under this traffic specification to be
		aggregated in an A-MPDU.  The Nominal MSDU Aggregation field may be
		non-zero only if the STA has an HT-association with the AP.

		A value of zero indicates that A-MPDU aggregation is not used.

		A value of 1 indicates that although only one nominal MSDU is
		aggregated, an implicit BA is expected.
	*/
	UINT32  MaxBurstSize;

	/*
		The Nominal MSDU Aggregation field is 4 octets long and contains an
		unsigned integer that indicates the average number of MSDUs belonging
		to the traffic under this traffic specification to be aggregated in
		an A-MSDU and/or an A-MPDU.

		The Nominal MSDU Aggregation field may be non-zero only if the STA has
		an HT-association with the AP.

		A value of zero indicates that neither A-MSDU nor A-MPDU aggregation
		is used.

		If the TS Info Ack Policy field is set to the value for HT-immediate
		block acknowledgement, then the Nominal MSDU Aggregation field shall
		be set to a non-zero value.

		Note1: if the ACK Policy bits in the TS Info field are set to normal
			acknowledgement, the value is the average number of MSDUs to be
			aggregated in an A-MSDU.
		Note2: if the ACK Policy bits in the TS Info field are set to
			HT-immediate block acknowledgement, the value is the average number
			of MSDUs to be aggregated in an A-MPDU (including the case where
			MSDUs are first aggregated into A-MSDUs and then further aggregated
			in A-MPDUs).
	*/
	/* bit0 ~ bit11 */
#define ACMR_11N_AGG_NUM_OF_MSDUS_GET(__Aggregation)						\
	(__Aggregation & 0xFFF)
...
#define ACM_STREAM_TYPE_WIFI			0x01	/* WME/WSM */
	...
	/*
		If the Burst Size Definition is set to 0, then the TSPEC contains the
		optional Maximum Burst Size; (Normal)

		if the Burst Size Definition is set to 1, then the TSPEC contains a
		value for Nominal MSDU Aggregation. (AMSDU)
		
		If the TS Info Ack Policy field is set to the value for HT-immediate
		block acknowledgement, then the Burst Size Definition shall be set to
		1. (AMPDU)
	*/
	UCHAR	BurstSizeDefn;	/* for 11n aggregation */
...
typedef struct _ACM_CTRL_INFO {
	...
	/* flags: UAPSD, TSPEC life time check */
	BOOLEAN FlgIsTspecUpasdEnable, FlgIsTspecTimeoutEnable;
...
========================================================================
*/
ACM_EXTERN VOID ACMP_FrameBwAnnSend(
	ACM_PARAM_IN	PRTMP_ADAPTER		pAd,
...
========================================================================
*/
ACM_EXTERN BOOLEAN ACMP_StatisticsGet(
	ACM_PARAM_IN	PRTMP_ADAPTER		pAd,
...
ACM_EXTERN INT ACM_Ioctl(
	ACM_PARAM_IN	PRTMP_ADAPTER 		pAd,
	ACM_PARAM_IN	RTMP_STRING *pArgv);
