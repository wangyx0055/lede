...
typedef pUSB_DEVICE_INFO *PUSB_DEV;
typedef pUSBHST_URB purbb_t;
...
#define AP_DRIVER_VERSION			"1.0.0.0"
#ifdef MULTIPLE_CARD_SUPPORT
...
#define AP_PROFILE_PATH_RBUS		"/tgtsvr/RT2860AP.dat"
#define RT2880_AP_DRIVER_VERSION	"1.1.0.0"
...
#define STA_DRIVER_VERSION			"1.0.0.0"
#ifdef MULTIPLE_CARD_SUPPORT
...
#define PROFILE_PATH			"/etc/Wireless/RT2860i.dat"
#define STA_PROFILE_PATH_RBUS	"/tgtsvr/RT2860STA.dat"
...
extern UCHAR VxDbgBuffer[256];

#define RTMP_ISR_LOG(LEVEL, X0, X1, X2, X3, X4, X5, X6) 					\
        if ((LEVEL) <= RTDebugLevel)	 								\
            logMsg((char *)X0, (int)X1, (int)X2, (int)X3, (int)X4, (int)X5, (int)X6);

extern void	sysAsciiShow (char *);
static void inline RtmpVxPrintWrapper(const char *format, ...)
{
	va_list	vaList;		/* traverses argument list */
	int		nChars;
	
	memset(VxDbgBuffer, 0, 256);
	va_start(vaList, format);
    	nChars = vsprintf(VxDbgBuffer, format, vaList);
    	va_end(vaList);
	if (nChars > 255)
	{
		printf("Error: nChars =%d!\n", nChars);
		nChars = 255;
	}
	VxDbgBuffer[nChars] = EOS;
	sysAsciiShow((char *)VxDbgBuffer);
}
...
do{                                   \
    ...
    {                               \
    	...
    	{					\
    		logMsg(("ERROR!!!\n"), 0,0,0,0,0,0);		\
		logMsg(("%s():call DBGPRINT_RAW() in line %d!\n"), (int)__FUNCTION__, __LINE__, 0, 0, 0, 0);\
    	}					\
	else					\
		printErr Fmt; 	\
    }                               \
...
{                                   			\
	...
	{											\
		logMsg(("ERROR!!!\n"), 0,0,0,0,0,0);		\
		logMsg(("%s():call DBGPRINT_ERR() in line %d!\n"), (int)__FUNCTION__, __LINE__, 0, 0, 0, 0);\
	} else {									\
    		printErr("ERROR!!! ");      	\
    		printErr Fmt;                 	 	\
	}											\
...
{									\
	if (INT_CONTEXT()){				\
		logMsg(("%s(): call DBGPRINT_S() in line %d!\n"), (int)__FUNCTION__, __LINE__, 0, 0, 0,0);\
	}else{							\
		printErr Fmt;					\
	}								\
}
...
#define DBGPRINT_S(Fmt)
#define DBGPRINT_ERR(Fmt)
#define RTMP_ISR_LOG(DBG_SW, X0, X1, X2, X3, X4, X5, X6)	
#endif
...
#define RTUSB_SUBMIT_URB(pUrb)		usbHstURBSubmit(pUrb)
#define RTUSB_URB_ALLOC_BUFFER(pUsb_Dev, BufSize, pDma_addr)				usb_buffer_alloc(pUsb_Dev, BufSize, GFP_ATOMIC, pDma_addr)
#define RTUSB_URB_FREE_BUFFER(pUsb_Dev, BufSize, pTransferBuf, Dma_addr)	usb_buffer_free(pTransferBuf)
#define RTUSB_FREE_URB(pUrb)		usb_free_urb(pUrb)
#define RTUSB_UNLINK_URB(pUrb)		usbHstURBCancel(pUrb)
...
typedef struct pt_regs void;
typedef INT32 URBCompleteStatus;

USBHST_STATUS RTUSBBulkOutDataPacketComplete(URBCompleteStatus Status, purbb_t purb, struct pt_regs *pt_regs);
USBHST_STATUS RTUSBBulkOutMLMEPacketComplete(URBCompleteStatus Status, purbb_t pUrb, struct pt_regs *pt_regs);
USBHST_STATUS RTUSBBulkOutNullFrameComplete(URBCompleteStatus Status, purbb_t pUrb, struct pt_regs *pt_regs);
USBHST_STATUS RTUSBBulkOutRTSFrameComplete(URBCompleteStatus Status, purbb_t pUrb, struct pt_regs *pt_regs);
USBHST_STATUS RTUSBBulkOutPsPollComplete(URBCompleteStatus Status, purbb_t pUrb, struct pt_regs *pt_regs);
USBHST_STATUS RTUSBBulkRxComplete(URBCompleteStatus Status, purbb_t pUrb, struct pt_regs *pt_regs);
USBHST_STATUS RTUSBBulkCmdRspEventComplete(URBCompleteStatus Status, purbb_t pUrb, struct pt_regs *pt_regs);

/* Fill Bulk URB Macro */
#define RTUSB_FILL_TX_BULK_URB	RTUSB_FILL_BULK_URB
#define RTUSB_FILL_HTTX_BULK_URB	RTUSB_FILL_BULK_URB
...
#define RTUSB_FILL_BULK_URB(pUrb,	\
				...
				Complete,	\
				pContext)				\
	if ((uEndpointAddress & 0x80) == 0){ /* For Bulk Out */	\
		USBHST_FILL_BULK_URB(pUrb, pUsb_Dev->hDevice, uEndpointAddress, pTransferBuf, BufSize,	\
							      0,	\
							      Complete, pContext, USBHST_FAILURE);	\
	}else{ /* For Bulk In */	\
		USBHST_FILL_BULK_URB(pUrb, pUsb_Dev->hDevice, uEndpointAddress, pTransferBuf, BufSize,	\
...
extern void dump_urb(purbb_t purb);
...
#define rtusb_urb_status   nStatus

#define RTMP_USB_URB_STATUS_GET(__pUrb)		((purbb_t)__pUrb)->nStatus
...
#define RTUSB_CONTROL_MSG(pUsb_Dev, uEndpointAddress, Request, RequestType, Value,Index, tmpBuf, TransferBufferLength, timeout, ret)	\
		...
		{	\
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("vendor request direction is failed\n"));	\
			ret = -1;	\
		}
