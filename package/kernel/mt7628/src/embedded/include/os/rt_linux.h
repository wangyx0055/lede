...
#define AP_PROFILE_PATH_RBUS		"/etc/wireless/mt7628/mt7628.dat"
#else
#warning OPENWRT undefine!!!!!!
...
#define RT_MOD_INC_USE_COUNT() \
	...
	{ \
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: cannot reserve module\n", __FUNCTION__)); \
		return -1; \
	}
...
{												\
	spin_unlock_irqrestore((spinlock_t *)(__lock), ((unsigned long)__irqflags));	\
}
...
do{	\
	NTSTATUS Status = 0; \
	Status = mtusb_read_reg((_A), (_R), _pV);	\
	if (Status != 0)	\
	{	\
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s, line(%d), CR Read Error\n", __FUNCTION__, __LINE__));	\
	}\
} while(0)
...
do{	\
	NTSTATUS Status = 0; \
	Status = mtusb_write_reg((_A), (_R), (_V));	\
	if (Status != 0)	\
	{	\
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s, line(%d), CR Write Error\n", __FUNCTION__, __LINE__));	\
	}\
} while(0)
