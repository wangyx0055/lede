...
typedef	struct GNU_PACKED _SUB_WSC_IE
{
	UCHAR	Type;
	UCHAR	Length;
	UCHAR	Data[1];	/* variable length data */
}	SUB_WSC_IE, *PSUB_WSC_IE;
...
#define WSC_HDR_BTN_MR_PRESS_FLG_GET(__pAd, __FlgIsPressed)			\
	{									\
		...
		__gpio_value &= 0xfffb; 						\
                ...
                __gpio_pol = HAL_REG32(RTMP_PIO_CTL_ADDR + RTMP_PIO2100_POL_OFFSET); \
                __gpio_value = HAL_REG32(RTMP_PIO_CTL_ADDR + RTMP_PIO2100_DATA_OFFSET); \
		__FlgIsPressed = ((~__gpio_value) & 0x4)?1:0;					\
	}
#endif
...
#define DEV_PASS_ID_NFC_HANDOVER		0x0007
...
#define STATUS_WSC_PIN_NO_AP					0x010d
...
#ifdef DPA_T
typedef	struct	_WSC_VENDOR_EXT
{
	INT	    Length;		/* Length of saved message */
	UCHAR	*pData;		/* Contents */
}	WSC_VENDOR_EXT, *PWSC_VENDOR_EXT;
#endif /* DPA_T */
...
#ifdef APCLI_SUPPORT
#define MAX_NUMBER_OF_WPS_PIN_AP 16
typedef struct _UUID_BSSID_CH_INFO_LIST{
UUID_BSSID_CH_INFO ApUuidBssid;
BOOLEAN enrolleMacAddrType;
UCHAR 	rssi;
struct _UUID_BSSID_CH_INFO_LIST * next;
struct _UUID_BSSID_CH_INFO_LIST * prev;

}UUID_BSSID_CH_INFO_LIST, *PUUID_BSSID_CH_INFO_LIST;
#endif
...
#ifdef APCLI_SUPPORT	
	UCHAR					WscPINBssCount;	
	BOOLEAN					TryNextWpsPINAp;
	UUID_BSSID_CH_INFO		ApUuidBssid[MAX_NUMBER_OF_WPS_PIN_AP];
...
#ifdef APCLI_SUPPORT
    BOOLEAN                 WscPINTimerRunning;
    RALINK_TIMER_STRUCT     WscPINTimer;
#endif
