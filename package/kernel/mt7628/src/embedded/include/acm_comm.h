...
#define ACM_TSPEC_SEM_UNLOCK(__pAd, __LabelSemErr)						\
	...
	/*	1. irq lock;
		2. irq unlock; */
#if 0
/* when WLAN module use IRQ, use the definition */
#define ACM_TSPEC_IRQ_LOCK_CHK_RTN(__pAd, __SplFlags, __LabelSemErr, __RtnValue)\
        do { ACMR_OS_SPIN_LOCK_IRQ(&((__pAd)->AcmTspecIrqLock), (__SplFlags));	\
			 if (ACMR_ADAPTER_DB == NULL) {									\
				ACMR_OS_SPIN_UNLOCK_IRQ(&((__pAd)->AcmTspecIrqLock), (__SplFlags));\
				return (__RtnValue); }										\
             if (0) goto __LabelSemErr; } while(0);
...
#define ACM_TSPEC_IRQ_UNLOCK(__pAd, __SplFlags, __LabelSemErr)				\
        do { ACMR_OS_SPIN_UNLOCK_IRQ(&((__pAd)->AcmTspecIrqLock), (__SplFlags)); } while(0);
#else
...
#define ACM_DELTS_SEND(__pAd, __pCdb, __pStream, __pLabErr)					\
	{																		\
		if (ACM_DeltsFrameSend(__pAd, __pCdb, __pStream) == FALSE)			\
			goto __pLabErr;													\
	}
...
#define ACM_TCLAS_PROCESSING_ONE		1
	UCHAR Processing;
} GNU_PACKED ACM_ELM_TCLAS_PROCESSING;
...
/* reset retry count */
STATIC VOID ACM_ASIC_RetryCountCtrl(
 	ACM_PARAM_IN	PRTMP_ADAPTER	pAd);
...
/* ----- Information get from other module function ----- */
