...
#define RSP_EVT_TYPE_COEX_RSP_EVENT		9
...
#ifdef RTMP_MAC_USB
INT32 AndesRltUsbLoadFw(struct _RTMP_ADAPTER *pAd);
INT32 AndesRltUsbEraseFw(struct _RTMP_ADAPTER *pAd);
INT32 AndesRltUsbLoadRomPatch(struct _RTMP_ADAPTER *pAd);
INT32 AndesRltUsbEraseRomPatch(struct _RTMP_ADAPTER *pAd);
VOID AndesRltUsbFwInit(struct _RTMP_ADAPTER *ad);
INT32 AndesRltUsbKickOutCmdMsg(struct _RTMP_ADAPTER *pAd, struct cmd_msg *msg);
#endif /* RTMP_MAC_USB */
...
INT32 AndesRltInitGain(struct _RTMP_ADAPTER *pAd, UINT8 channel, BOOLEAN force_mode, UINT32 gain_from_e2p);
INT32 AndesRltDynamicVga(struct _RTMP_ADAPTER *pAd, UINT8 channel, BOOLEAN mode, BOOLEAN ext, INT32 rssi, UINT32 false_cca);
INT32 AndesRltLedOp(struct _RTMP_ADAPTER *pAd, UINT32 led_idx, UINT32 link_status);
struct cmd_msg *andes_alloc_cmd_msg(struct _RTMP_ADAPTER *pAd, UINT32 length);
#ifdef MT7601
INT32 AndesBBPRandomWritePair(struct _RTMP_ADAPTER *pAd, RTMP_REG_PAIR *RegPair, UINT32 Num);
INT32 AndesBBPRandomWrite(struct _RTMP_ADAPTER *pAd, UINT32 Num, ...);
#endif /* MT7601 */
...
{															\
	...
	if ( _i == MAX_CALIBRATION_WAIT_TIME )					\
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("ANDES_WAIT_CALIBRATION_DONE timeout!\n"));	\
	else														\
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("ANDES_WAIT_CALIBRATION_DONE %d\n", _i));	\
}
#endif /* RTMP_PCI_SUPPORT */
