...
static void rti_wlan_dsr(cyg_vector_t vector, cyg_ucount32 count, cyg_addrword_t data)
{
	...
	if (data != 0)
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("rti_wlan_dsr: data is NULL\n"));
}
...
static bool rti_ecos_init(struct cyg_netdevtab_entry *tab)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> rti_ecos_init()\n"));
                          ...
                          int len)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("rt_ecos_control()\n"));	
	pAd = (PRTMP_ADAPTER) RtmpOsGetNetDevPriv(sc);	
	if (pAd == NULL)
	    return;
