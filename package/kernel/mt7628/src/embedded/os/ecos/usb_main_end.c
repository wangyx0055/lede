...
NDIS_STATUS USBDevConfigInit(pUSB_DEVICE_INFO USBDev, UINT8 uInterfaceNumber, RTMP_ADAPTER *pAd)
{
	...
	if(!pBuffer)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("USBDevConfigInit: Allocate USB device descriptor failed!!\n"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("USBDevConfigInit(): usbHstGetDescriptor Failed!!\n"));
	...
	if (!pBuffer)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("USBDevConfigInit: Allocate USB configuration descriptor failed!!\n"));
		return NDIS_STATUS_FAILURE;
	}
	...
	if(!pBuffer)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("USBDevConfigInit(): Allocate pBuffer for entire configuration descriptor failed!!\n"));
		return NDIS_STATUS_FAILURE;	
	}
    ...
    while (uInterfaceCount < pConfigDesc->bNumInterfaces)
    {
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Invalid configuration descriptor\n"));
		...
		while (uEndpointCount < pInterfaceDesc->bNumEndpoints)
		{
			...
			if( (uCurEndpointAddress & USB_ENDPOINT_DIR_MASK) == USB_ENDPOINT_IN){
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BulkIN Endpointaddress : %x\n", uCurEndpointAddress));
			...
			/* Get EDCA Tx information for EP1~EP4 OUT, HCCA Tx information for  EP5 Out, and reserved for EP6 OUT */
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BulkOUT Endpointaddress : %x\n", uCurEndpointAddress));
			}
    ...
    )
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("muxDevLoad() failed for ralink USB wireless device\n"));
		return USBHST_FAILURE;
	} else {
		if (muxDevStart(USBEndCookie) == ERROR)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("muxDevStart() failed for ralink USB wireless device\n"));
			return USBHST_FAILURE;
		}
	}
	...
	IN RTMP_VX_INIT_STRING *initToken)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===>RtmpVxUsbEndLoad()!\n"));
	...
	if (!handle)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RtmpVxUsbEndLoad(): Allocate memory for os handle failed!!\n"));
		goto ERROR0;
	}
	...
	if (!pUSBDev)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RtmpVxUsbEndLoad() : Allocate memory for USB dev info failed!!\n"));
		goto ERROR1;
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RtmpVxUsbEndLoad(): Allocate RtmpAdapter Block Failed!!\n"));
		goto ERROR2;
	}
...
/*USBDevInit============================================== */
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RtmpVxUsbEndLoad(): USBDevConfigInit Failed!!\n"));
		goto ERROR3;
	}
...
/*NetDevInit============================================== */
	...
	if (pENDDev == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" malloc failed for main physical net device!\n"));
		goto ERROR3;
	}
	...
	if (status == NDIS_STATUS_FAILURE)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Allocate NetPool failed!\n"));
		goto ERROR4;
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Can not init net device %s!\n", "ra0"));
		goto ERROR5;
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Can not init MIB of net device %s!\n", "ra0"));
		goto ERROR5;
	}
...
void RtmpVxUsbDriverRegister()
{
	...
	for (index = 0; index < sizeof(RT_USB_DEVICE_ID) / sizeof(USB_DEVICE_ID); index++ )
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RtmpVxUsbDriverRegister(): Failed\n"));
			break;
		}
	}
...
VOID RTUSBWatchDog(IN RTMP_ADAPTER *pAd)
{
	...
	if ((MACValue & 0xff) !=0 )
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TX QUEUE 0 Not EMPTY(Value=0x%0x). !!!!!!!!!!!!!!!\n", MACValue));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("TX QUEUE 1 Not EMPTY(Value=0x%0x). !!!!!!!!!!!!!!!\n", MACValue));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Maybe the Rx Bulk-In hanged! Cancel the pending Rx bulks request!\n"));
		if ((!RTMP_TEST_FLAG(pAd, (fRTMP_ADAPTER_RESET_IN_PROGRESS |
									fRTMP_ADAPTER_BULKIN_RESET |
									fRTMP_ADAPTER_HALT_IN_PROGRESS |
									fRTMP_ADAPTER_NIC_NOT_EXIST))))
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Call CMDTHREAD_RESET_BULK_IN to cancel the pending Rx Bulk!\n"));
	...
	for (idx = 0; idx < NUM_OF_TX_RING; idx++)
	{
		...
		if ((pAd->BulkOutPending[idx] == TRUE) && pAd->watchDogTxPendingCnt)
		{
				...
				)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Maybe the Tx Bulk-Out hanged! Cancel the pending Tx bulks request of idx(%d)!\n", idx));
				if (pUrb)
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Unlink the pending URB!\n"));
					/* unlink it now */
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Unkonw bulkOut URB maybe hanged!!!!!!!!!!!!\n"));
				}
			}
			else
			{
	...
	if((needDumpSeq == TRUE) /*&& (pAd->CommonCfg.bDisableReordering == 0)*/)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("NICUpdateRawCounters():The Queueing pkt in reordering buffer:\n"));
			NdisAcquireSpinLock(&pBAEntry->RxReRingLock);
			mpdu_blk = pBAEntry->list.next;
			while (mpdu_blk)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("\t%d:Seq-%d, bAMSDU-%d!\n", count, mpdu_blk->Sequence, mpdu_blk->bAMSDU));
