	...
	else if ((strcmp(net_dev->dev_name,"wds0") == 0) || (strcmp(net_dev->dev_name,"wdsi0") == 0))
	{
		...
		for(index = 0; index < MAX_WDS_ENTRY; index++)
		{
			if (pAd->WdsTab.WdsEntry[index].wdev.if_dev == net_dev)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): I/F(wds%d): cmd = 0x%08x\n", 
...
#ifdef RELEASE_EXCLUDE
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): I/F(apcli%d): cmd = 0x%08x\n",
							__FUNCTION__, pObj->ioctl_if, cmd));
#endif /* RELEASE_EXCLUDE */
				break;
			}
		...
		case RTPRIV_IOCTL_WSC_CALLBACK:
			{
				...
				pObj->WscMsgCallBack = wrq->u.data.pointer;
                                if (pObj->WscMsgCallBack)
                                        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Set pObj->WscMsgCallBack 0x%x\n", pObj->WscMsgCallBack));
                                else {
                                        retVal = EINVAL;
                                        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Set pObj->WscMsgCallBack is NULL\n"));
                                }
			}
			break;
