...
DC_URB *usb_alloc_urb()
{
	...
	if (!pURB)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : URB meory insufficient\n", __FUNCTION__));
		return NULL;
	}
...
void * usb_buffer_alloc(DC_USB_DEVICE *pUSBDev, unsigned int Size, ra_dma_addr_t **pDMAAddr)
{
	...
	if (!pVirtualAddress)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : usb pool resource is not available\n", __FUNCTION__));	
		return NULL;
	}
	...
	if (pDcRamBlkAligned)
	{
		...
		if (pData_dma == NULL)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : usb dma resource is not available\n", __FUNCTION__));
			RtmpDCRamBlockMemFree(pVirtualAddress);
			pVirtualAddress = NULL;
		}
		*pDMAAddr = pData_dma;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("dataDma=0x%x,pDMAAddr=0x%x\n", pData_dma, *pDMAAddr));
	}
	else
	{
		*pDMAAddr = NULL;
...
DC_USB_PIPE * GetUSBPipeFromEndpoint(UINT8 EndpointAddress)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : Can not find USBPipe for this EndpointAddress[%x]", __FUNCTION__, EndpointAddress));
	return NULL;
}
...
DC_USB_DEVICE *GetUSBDevFromUSBPipe(DC_USB_PIPE *pUSBPipe)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : Can not find USBDev for this USBPipe[%p]", __FUNCTION__, pUSBPipe));
	return NULL;
}
									...
									UINT16 wValue, UINT16 wIndex, void *pBuffer, UINT16 wLength, int timeout)
{
	...
	if (!pUSBDev)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : Invalide Device Information.\n", __FUNCTION__));
		goto ERROR0;
	}
	...
	if (!pSetup)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : Insufficient Resource for USB buffer\n", __FUNCTION__));
	...
	if (!pURB)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : Insufficient Resource for URB\n", __FUNCTION__));
		Result = NDIS_STATUS_FAILURE;
		goto ERROR1;
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : dc_uh_core_transfer failed\n", __FUNCTION__));
		Result = NDIS_STATUS_FAILURE;
		goto ERROR3;
	}
...
INT32 usb_submit_urb(DC_URB *pURB)
{
	...
	if (!pUSBDev)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : Can not find USBDev from URB[%p]", __FUNCTION__, pURB));
	...
	if (Result != DC_SUCCESS)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : usb_submit_urb failed!!\n", __FUNCTION__));
		Result = NDIS_STATUS_FAILURE;
	}
	else
	{
...
NDIS_STATUS usb_unlink_urb(DC_URB *pURB)
{
	...
	*/
	...
	if (Result != DC_SUCCESS)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : dc_uh_core_pipe_reset failed!!\n", __FUNCTION__));
		Result = NDIS_STATUS_FAILURE;
...
static void rtusb_dataout_complete(unsigned long data)
{
	...
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO, ("--->rtusb_dataout_complete\n"));
	...
	/*MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_LOUD, ("Done-B(%d):I=0x%lx, CWPos=%ld, NBPos=%ld, ENBPos=%ld, bCopy=%d!\n", BulkOutPipeId, in_interrupt(), pHTTXContext->CurWritePosition, */
	...
	else	/* STATUS_OTHER */
	{
		...
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("BulkOutDataPacket failed: ReasonCode=%d!\n", Status));
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("\t>>BulkOut Req=0x%lx, Complete=0x%lx, Other=0x%lx\n", pAd->BulkOutReq, pAd->BulkOutComplete, pAd->BulkOutCompleteOther));
		DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("\t>>BulkOut Header:%x %x %x %x %x %x %x %x\n", pBuf[0], pBuf[1], pBuf[2], pBuf[3], pBuf[4], pBuf[5], pBuf[6], pBuf[7]));
		/*DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, (">>BulkOutCompleteCancel=0x%x, BulkOutCompleteOther=0x%x\n", pAd->BulkOutCompleteCancel, pAd->BulkOutCompleteOther)); */
...
static void rtusb_null_frame_done_tasklet(unsigned long data)
{
...
#ifdef RELEASE_EXCLUDE
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO, ("--->rtusb_null_frame_complete_tasklet\n"));
#endif /* RELEASE_EXCLUDE */
	...
	else	/* STATUS_OTHER */
	{
		...
		{
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("Bulk Out Null Frame Failed, ReasonCode=%d!\n", Status));
			RTMP_SET_FLAG(pAd, fRTMP_ADAPTER_BULKOUT_RESET);
...
static void rtusb_pspoll_frame_done_tasklet(unsigned long data)
{
...
#ifdef RELEASE_EXCLUDE
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO, ("--->rt2870_pspoll_frame_complete_tasklet\n"));
#endif /* RELEASE_EXCLUDE */
	...
	else /* STATUS_OTHER */
	{
		if ((!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_RESET_IN_PROGRESS)) &&
			...
			(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BULKOUT_RESET)))
		{
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("Bulk Out PSPoll Failed\n"));
			RTMP_SET_FLAG(pAd, fRTMP_ADAPTER_BULKOUT_RESET);
			pAd->bulkResetPipeid = (MGMTPIPEIDX | BULKOUT_MGMT_RESET_FLAG);
			RTEnqueueInternalCmd(pAd, CMDTHREAD_RESET_BULK_OUT, NULL, 0);
		}
...
static void rx_done_tasklet(unsigned long data)
{
...
#ifdef RELEASE_EXCLUDE
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO, ("--->rx_done_tasklet\n"));
#endif /* RELEASE_EXCLUDE */

	/*DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("rxDone, st=%d\n", Status)); */
	...
	else	 /* STATUS_OTHER */
	{
									...
									fRTMP_ADAPTER_NIC_NOT_EXIST))))
		{
			...
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("Bulk In Failed. Status=%d, BIIdx=0x%x, BIRIdx=0x%x, actual_length= 0x%x\n", 
...
static void rtusb_mgmt_dma_done_tasklet(unsigned long data)
{
...
#ifdef RELEASE_EXCLUDE
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO, ("--->rt2870_mgmt_dma_done_tasklet\n"));
#endif /* RELEASE_EXCLUDE */
	...
	if (Status != USB_ST_NOERROR)
	{
		...
		if ((!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_RESET_IN_PROGRESS)) &&
			...
			(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BULKOUT_RESET)))
		{
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("Bulk Out MLME Failed, Status=%d!\n", Status));
...
static void rtusb_hcca_dma_done_tasklet(unsigned long data)
{
	...
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("--->hcca_dma_done_tasklet\n"));
...
#ifdef RELEASE_EXCLUDE
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO, ("--->ac3_dma_done_tasklet\n"));
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO, ("--->ac2_dma_done_tasklet\n"));
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_INFO, ("--->ac1_dma_done_tasklet\n"));
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("--->rtusb_ac0_dma_done_tasklet\n"));
#endif /* RELEASE_EXCLUDE */
...
static void rtusb_ate_ac0_dma_done_tasklet(unsigned long data)
{
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("--->ATE_RTUSBBulkOutDataPacketComplete\n"));
#endif /* RELEASE_EXCLUDE */
		...
		if ((ATE_ON(pAd)) && (pAd->ate.bQATxStart == TRUE))
		{
			if (pAd->ate.QID == BulkOutPipeId)
			{
				...
				*/
				...
				pAd->ate.TxDoneCount++;
#ifdef RELEASE_EXCLUDE
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("pAd->ate.TxDoneCount == %d\n", pAd->ate.TxDoneCount));
#endif /* RELEASE_EXCLUDE */
	...
	else
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("BulkOutDataPacket Failed STATUS_OTHER = 0x%x . \n", Status));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, (">>BulkOutReq=0x%lx, BulkOutComplete=0x%lx\n", pAd->BulkOutReq, pAd->BulkOutComplete));
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("pNullContext->pAd = 0x%lx\n", (ULONG)&pNullContext->pAd));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("pNullContext->pUrb = 0x%lx\n", (ULONG)&pNullContext->pUrb));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("pNullContext->TransferBuffer = 0x%lx\n", (ULONG)&pNullContext->TransferBuffer));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("pNullContext->BulkOutPipeId = %d\n", pNullContext->BulkOutPipeId));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("pNullContext->BulkOutSize = %ld\n", pNullContext->BulkOutSize));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("pNullContext->InUse = %d\n", (pNullContext->InUse==TRUE)));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("pNullContext->bWaitingBulkOut = %d\n", (pNullContext->bWaitingBulkOut==TRUE)));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("pNullContext->IRPPending = %d\n", (pNullContext->IRPPending==TRUE)));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("pNullContext->LastOne = %d\n", (pNullContext->LastOne==TRUE)));
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Bulk Out Remained = %d\n", atomic_read(&pAd->BulkOutRemained)));
#endif /* RELEASE_EXCLUDE */
	}
	...
	{
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Continue to BulkOut ! \n"));
#endif /* RELEASE_EXCLUDE */
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	*/
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: unable to start RtmpTimerQThread\n", RTMP_OS_NETDEV_GET_DEVNAME(pAd->net_dev)));
		return NDIS_STATUS_FAILURE;
	}
	...
	if (status == NDIS_STATUS_FAILURE) 
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: unable to start RTUSBCmdThread\n", RTMP_OS_NETDEV_GET_DEVNAME(pAd->net_dev)));
		return NDIS_STATUS_FAILURE;
	}
...
DC_RETURN_CODE RtmpDcUSBTaskFunc(IN ULONG Context)
{
	...
	while(1)
	{
		...
		{
			dc_log_printf("%s:(%d) %s waiting failed\n",__FUNCTION__,status,pTask->taskName);
			continue;
		}
...
DC_RETURN_CODE RtmpDcUSBTaskInit(void)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Create USBTask failed!\n"));
	}
