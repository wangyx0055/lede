...
static NDIS_STATUS USBDevConfigInit(DC_USB_DEVICE *pUSBDev, DC_USB_INTERFACE *pIntf, RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("NumEndpoints=%d\n", pAd->NumberOfPipes ));
	...
	for (Index = 0; Index < pAd->NumberOfPipes; Index++)
	{
            ...
            && (pUSBEndDescr->bEndpointAddress != 0x87)) 
		{
			...
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("BULK IN MaximumPacketSize = %d.\n", pAd->BulkInMaxPacketSize));
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("EP address = 0x%2x  .\n", pUSBEndDescr->bEndpointAddress));
		}
                    ...
                    && (pUSBEndDescr->bEndpointAddress != 0x07)) 
		{
			...
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("BULK OUT MaximumPacketSize = %d.\n", pAd->BulkOutMaxPacketSize));
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("EP address = 0x%2x.\n", pUSBEndDescr->bEndpointAddress));
 		}
		...
		if (!DC_USB20_IS_ENDPOINT_CONTROL(pUSBEndDescr))
		{
			...
			if (!pUSBPipe)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: Create new USB pipe specific to this endpoint fail.\n", __FUNCTION__));
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : Open USB Pipes for BULK IN/OUT transfer fail.\n", __FUNCTION__));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : Could not find both bulk-in and bulk-out endpoints.\n", __FUNCTION__));
		return NDIS_STATUS_FAILURE;
	}
...
{	
	...
	if (!pOSCookie)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : Allocate memory for os handle failed.\n", __FUNCTION__));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: Allocate memory for pAd failed.\n", __FUNCTION__));
		goto error1;
	}
	...
	if(Result != NDIS_STATUS_SUCCESS)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : Initialize WIFI driver USB BULK IN/OUT attributes failed.\n", __FUNCTION__));
		goto error2;
	}
...
DC_RETURN_CODE rtusb_probe(DC_USB_DEVICE *pUSBDev, DC_USB_INTERFACE *intf)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : USB Device Vendor ID = %x, Product ID = %x.", __FUNCTION__, dd->idVendor, dd->idProduct));
	...
	if (Result == TRUE)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : Ralink driver supports this attached USB device.", __FUNCTION__));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : Ralink driver do not support this attached USB device.", __FUNCTION__));
	}
...
DC_RETURN_CODE RTUSBDriverRegister()
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ralink USB Dongle Init."));
	...
	if (Result != DC_SUCCESS)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : dc_uh_class_driver_register failed.", __FUNCTION__));
	}
