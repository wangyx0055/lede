	...
	IN	unsigned long timeout)
{
	...
	if (status == TX_SUCCESS)
	{
		...
		if (status != TX_SUCCESS)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Add_Timer failed(%d)!\n", status));
		}
	}
	else 
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("timerQuery failed(%d)!\n", status));
	}
}
	...
	IN	LIST_HEADER				*pTimerList)
{
	...
	*/
	...
	if (status != TX_SUCCESS)
	{
		...
		if (status == TX_SUCCESS)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("timer already init-ed!\n"));
			return;
		}
		pRaTimer = (PRALINK_TIMER_STRUCT)data;
		pRaTimer->Valid = FALSE;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("init_Timer failed!\n"));
	}
}
...
RTMP_OS_FD RtmpOSFileOpen(char *pPath,  int flag, int mode)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): Function not implemented!\n", __FUNCTION__));
...
static DC_RETURN_CODE RtmpDCBufCallback(DC_RAMMGR_BUFFER_ENTRY *entry)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():entry->status=%d, bufferId=%d!\n", 
	...
	if((entry->status == DC_BUFFER_AVAILABLE) && ( bufferId < RTMP_DC_RAM_ID_MAX))
	{
		for (blkIdx =0 ; blkIdx < (sizeof(RtmpDCRamMgrPSBBuf) / sizeof(RTMP_DC_RAM_BLOCK_ELEMENT)); blkIdx++)
		{
			if (bufferId == RtmpDCRamMgrPSBBuf[blkIdx].Id)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(%s): base=%x, size=%x\n", 
				...
				if ((retCode == DC_SUCCESS) && (pDcRamBlock->DcRamBlkLock.mutex_status == TX_SUCCESS))
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: init DC RAM Block for (%s) success!\n", __FUNCTION__, pDcRamBlock->name));
				}
				break;
			}
		}
...
DC_RETURN_CODE RtmpTxBootStrapTask(void)
{
	...
	if (retCode != DC_SUCCESS)
		return retCode;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Create USBTask success\n"));
...
DC_RETURN_CODE RtmpTX_application_define(void *ignore)
{
	...
	*/
	...
	if (retCode != DC_SUCCESS)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("dc_am_global_setup failed(%d)!\n", retCode));
		return retCode;
	}
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): dc_am_global_setup() return success\n", __FUNCTION__));
	...
	*/
	...
	for (idx= 0; idx < (sizeof(RtmpDCRamMgrPSBBuf) / sizeof(RTMP_DC_RAM_BLOCK_ELEMENT)); idx++)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("allocate memory for %s failed!\n", pPSBMemEntry->name));
			goto request_failed;
		}
	}
	...
	if (status != DC_SUCCESS)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Create BootStrapTask failed!\n"));
		goto request_failed;
	}
	...
	if (status != DC_SUCCESS)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RTUSBDriverRegister failed(%d)!\n", status));
	}
	else
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): RTUSBDriverRegister() return success\n", __FUNCTION__));
	}
...
DC_RETURN_CODE wifi_init(IN DC_NVRAM_STORE_t *pNvram_store, IN DC_WIFI_INIT_CONFIG *operations)
{
	...
	if (ret != DC_SUCCESS)
    {
    	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("dc_nvram_object_register() ERR [%#x]", ret));
        return ret;
    }
	...
	if (ret != DC_SUCCESS)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RtmpTX_application_define ERR [%x]", ret));
		return ret;
	}
