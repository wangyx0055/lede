	...
	else if (*pRTAuthMode == Ndis802_11AuthModeWPAPSK)
	{
		if (*pRTWepStatus == Ndis802_11TKIPEnable)
			...
			*pSYAPEncr = DC_WIFI_AP_ENCR_WPAPSK_AES;
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Unknown ciphersuite(RTAuthMode = %x, pRTWepStatus = %x", 
...
static int SYWIFIAPScanTrigger(IN PRTMP_ADAPTER pAd)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("AP Scan In Progress"));
    ...
    IN  PVOID SystemSpecific3)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("SY:FoolProof = %d\n", i));
								  ...
								  OUT NDIS_802_11_WEP_STATUS *pRTWepStatus)
{
	...
	case DC_WIFI_IFACE_ENCR_WPA2PSK_AES:
		...
		*pRTWepStatus = Ndis802_11AESEnable;
		break;
	default:
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Unknown ciphersuite(SYEncr = %x, SYAuth = %x", 
										*pSYEncr, *pSYAuth));
	}
}
					...
					OUT NDIS_802_11_AUTHENTICATION_MODE *pRTAuthMode)
{
	...
	case WSC_AUTHTYPE_WPANONE:
		*pRTAuthMode = Ndis802_11AuthModeWPANone;
		break;
	default:
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Unknown WSCAuthType = %x", *pWSCAuthType));
		break;
	}
}
					...
					OUT NDIS_802_11_WEP_STATUS *pRTWepStatus)
{
	...
	case WSC_ENCRTYPE_AES:
		*pRTWepStatus = Ndis802_11AESEnable;
		break;
	default:
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Unknow WSCEncrType = %x", *pWSCEncrType));
		break;
	}
}
							...
							OUT UCHAR *RTCipherAlg)
{
	if (*pWSCKeyLength == 5 || *pWSCKeyLength == 10)
		...
		*RTCipherAlg = CIPHER_WEP128;
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Invalid Key Length (%d)\n", *pWSCKeyLength));
}
...
static inline DC_WIFI_IFACE_MODE SYBSSMode(IN UCHAR BssType)
{
	...
	case BSS_INFRA:
		...
		break;
	default:
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Unknow BSSMode = %x", BssType));
	}
...
SY_WPS_PIN_CONTINUE_START:
	...
	while (pSyControl->StopWpsAPSearch != TRUE && APIndex < pSyControl->NumAPsFound)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("SY:PinContinue(APIndex = %d, NumsAP = %d, SSID = %s)\n",
...
int SYControlInit(IN PRTMP_ADAPTER pAd)
{
	...
	if (!pSYAPScanResult)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Memory is not availabe for AP results!!"));
		goto ERROR_FAIL;
	}
	...
	if (ret != DC_SUCCESS)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("SYTask init fail"));
						   ...
						   IN WifiAccessPointsCallback *pWiFiAPScancallback)
{
	...
	if (!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Networks is down!!"));
		return DC_FAILURE;
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("!!! Link UP, Port Not Secured! ignore this set::OID_802_11_BSSID_LIST_SCAN\n"));
		return DC_FAILURE;
	}
						...
						IN void *pWPSCallback)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Unknown WPS Mode[%x]\n", WPSMode));
		return DC_FAILURE;
	}
