...
ULONG RTDebugLevel = DBG_LVL_WARN;
ULONG RTDebugFunc = 0;
	...
	OUT PNDIS_PACKET   *ppOutPacket)
{
	...
	*ppOutPacket = OSPKT_TO_RTPKT(pkt);
	...
	if (SrcBufLen < 14)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s() --> Ndis Packet buffer error !!!\n", __FUNCTION__));
		return (NDIS_STATUS_FAILURE);
	}

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("---> RTMPCloneNdisPacket: in scatter # =%d, packet len = %d\n",
	...
	if (TotalLen != (ULONG)PacketInfo.TotalPacketLength)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, RT_DEBUG_DMA,("RTMPCloneNdisPacket Chk1 (TotalLen=%d) TotalPacketLength = %d\n", TotalLen, PacketInfo.TotalPacketLength));
	...
	if ((TotalLen+14) != (ULONG)PacketInfo.TotalPacketLength)
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, RT_DEBUG_DMA,("RTMPCloneNdisPacket Chk2 (TotalLen=%d) TotalPacketLength = %d\n", TotalLen, PacketInfo.TotalPacketLength));
	...
	IN	CHAR			Rssi)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : The type(%0x02x) is not valid.\n", __FUNCTION__, type));			       		       		
		return;
}

	if (event >= event_table_len)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : The event(%0x02x) is not valid.\n", __FUNCTION__, event));			       		       		
		return;
	}	
	...
	if((pBuf = kmalloc(IW_CUSTOM_MAX_LEN, GFP_ATOMIC)) != NULL)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : Can't allocate memory for wireless event.\n", __FUNCTION__));			       		       				
#else
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : The Wireless Extension MUST be v15 or newer.\n", __FUNCTION__));	
	...
	IN	PCHAR			arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> %s (wrq->u.data.length = %d)\n\n", __FUNCTION__, wrq->u.data.length));

	if (wrq->u.data.length != EEPROM_SIZE)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: error length (=%d) from host\n",
			   __FUNCTION__, wrq->u.data.length));
		return FALSE;
	}
