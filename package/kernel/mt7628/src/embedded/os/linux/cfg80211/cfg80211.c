...
#endif /* LINUX_VERSION_CODE */
{
	...
	if (!pDev || !pDev->ieee80211_ptr)
	{	
		CFG80211DBG(RT_DEBUG_ERROR, ("%s> pDev || pDev->ieee80211_ptr NULL\n", __FUNCTION__));
		return 0;
	}
...
#endif /* LINUX_VERSION_CODE: 3.6.0 */
{
	...
	/* get channel number */	
	ChanId = ieee80211_frequency_to_channel(pChan->center_freq);	
	CFG80211DBG(RT_DEBUG_OFF, ("%s: CH = %d, Type = %d, duration = %d, cookie=%d\n", __FUNCTION__, 
...
#endif /* LINUX_VERSION_CODE: 3.6.0 */	
{
    ...
    /* get channel number */
    ChanId = ieee80211_frequency_to_channel(pChan->center_freq);
    CFG80211DBG(RT_DEBUG_TRACE, ("80211> Mgmt Channel = %d\n", ChanId));
	...
	IN struct device				*pDev)
{
	...
	*pPriv = (ULONG)pAd;
	...
	if (wiphy_register(pWdev->wiphy) < 0)
	{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("80211> Register wiphy device pWdev->wiphy->mgmt_stypes[2].rx=0x%x\n",pWdev->wiphy->mgmt_stypes[2].rx));
	return pWdev;
	...
	IN struct net_device		*pNetDev)
{
	...
	if (pCfg80211_CB->pCfg80211_Wdev == NULL)
	{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("80211> CFG80211_Register wdev(0x%x)\n",pCfg80211_CB->pCfg80211_Wdev));
