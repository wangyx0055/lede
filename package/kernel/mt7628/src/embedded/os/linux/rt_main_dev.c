...
int MainVirtualIF_open(struct net_device *net_dev)
{
	...
	{
		OS_WAIT(10);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Card not ready, NDIS_STATUS_SUCCESS!\n"));
	}
#else
...
int rt28xx_open(VOID *dev)
{
	...
	if (sizeof(ra_dma_addr_t) < sizeof(dma_addr_t))
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Fatal error for DMA address size!!!\n"));
...
struct net_device_stats *RT28xx_get_ether_stats(struct net_device *net_dev)
{
	...
	if (pAd)
	{
	    ...
	    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("RT28xx_get_ether_stats --->\n"));
