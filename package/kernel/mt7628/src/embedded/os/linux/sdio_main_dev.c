...
//	RTMP_STRING *print_name;
   ...
   ASSERT(id);
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("SDIO probe ! register %s\n", RTMP_DRV_NAME));
   ...
   sdio_release_host(func);
   if (ret) {
       MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():sdio_set_block_size failed!\n",__FUNCTION__));
       goto err;
   }   
...
#endif
   if (ret) {
       MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():rt_sdio_interrupt failed!\n",__FUNCTION__));
       goto err_out_free_netdev;
   }
      
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("SDIO probe finsih %s\n", RTMP_DRV_NAME));
...
/*	RTMP_DRIVER_CFG80211_INIT(pAd, pdev); */
	...
	*/
	...
	if (rv) {
      MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s():DevAttach failed!\n", __FUNCTION__));
		goto err_out_free_netdev;
}
...
#ifdef PRE_ASSIGN_MAC_ADDR
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("@%s MAC address: %02x:%02x:%02x:%02x:%02x:%02x\n",
   ...
   return ret;
	...
	/* --------------------------- ERROR HANDLE --------------------------- */	
   MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s erro happen\n",__FUNCTION__));
err_out_free_netdev:
...
static void rt_sdio_remove (struct sdio_func *func)
{
	...
	if (ret != NDIS_STATUS_SUCCESS) 
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("sdio_release_irq failure\n"));
	}
	sdio_release_host(func);
...
/*NetDev de-Init============================================== */
	...
	if (ret != NDIS_STATUS_SUCCESS) 
		{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("sdio_disable_func failure\n"));
	}
	sdio_release_host(func);
...
#ifndef WCX_SUPPORT
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s SDIO !!!! register\n", __FUNCTION__));
...
{
	int ret = 0;
	DBGPRINT(RT_DEBUG_ERROR, ("%s: register to sdio\n", __FUNCTION__));
	ret = sdio_register_driver(&rt_sdio_drv);
	return ret;
}
...
{
	int ret = 0;
	DBGPRINT(RT_DEBUG_ERROR, ("%s: unregister to sdio\n", __FUNCTION__));
	sdio_unregister_driver(&rt_sdio_drv);
