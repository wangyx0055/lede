...
#if defined(CONFIG_SUPPORT_OPENWRT)
#define FIRST_AP_PROFILE_PATH		"/etc/wireless/mt7628/mt7628.dat"
#else
#define FIRST_AP_PROFILE_PATH		"/etc/Wireless/RT2860/RT2860.dat"
#warning FIRST_AP_PROFILE_PATH:RT2860!!!!!!!!!!!!!!!!!!!!!!!!!!
#endif /* CONFIG_SUPPORT_OPENWRT */
...
INT get_dev_config_idx(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("chip_id1=0x%x, chip_id2=0x%x, pAd->MACVersion=0x%x\n", first_card, second_card, pAd->MACVersion));
...
NDIS_STATUS	RTMPReadParametersHook(RTMP_ADAPTER *pAd)
{
	...
	if (src && *src)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Open file \"%s\" failed!\n", src));
		}
		else 
		{
			...
			if (buffer) {
				...
				if (retval > 0)
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Read file \"%s\" failed(errCode=%d)!\n", src, retval));
			...
			{
				retval = NDIS_STATUS_FAILURE;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Close file \"%s\" failed(errCode=%d)!\n", src, retval));
			}
		}
	...
	for (i = 0; i < pAd->ApCfg.BssidNum; i++)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Reset ra%d hostapd support=FLASE", i));
	}
#endif /*HOSTAPD_SUPPORT */
...
#endif /* RT_CFG80211_P2P_SUPPORT */
	{
		...
		*/
...
#endif /* RTMP_MAC_PCI */
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("DTIM=%d/%d, tx mcast/bcast out...\n",pAd->ApCfg.DtimCount,pAd->ApCfg.DtimPeriod));
	...
	IN struct sk_buff 		*skb)
{
	...
	if (data == 0)
	{
		...
		if (data == 0)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s::Error\n", __FUNCTION__));
			return 1;
		}
	}
	...
	else if ((data[12] == 0x08) && (data[13] == 0x06)) /* ARP */
	{
		...
		{ 
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("ARP reply from %d.%d.%d.%d to %d.%d.%d.%d\n",
			...
			for (i=0; i<MAX_LEN_OF_MAC_TABLE; i++)
			{
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("ARP spoofing packet %d.%d.%d.%d %02x:%02x:%02x:%02x:%02x:%02x\n",
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("ARP request from %d.%d.%d.%d to %d.%d.%d.%d\n",
						data[28], data[29], data[30], data[31], data[38], data[39], data[40], data[41]));
			...
			for (i=0; i<MAX_LEN_OF_MAC_TABLE; i++)
			{
				if (memcmp(data+22, pAd->MacTab.Content[i].Addr, 6) == 0)
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("add arp %d.%d.%d.%d %02x:%02x:%02x:%02x:%02x:%02x\n",
								data[28], data[29], data[30], data[31],
								data[22], data[23], data[24], data[25], data[26], data[27]));    
					return 0;
				}
...
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Incming Mac=%02x:%02x:%02x:%02x:%02x:%02x, NewMAC=%02x:%02x:%02x:%02x:%02x:%02x!\n",
