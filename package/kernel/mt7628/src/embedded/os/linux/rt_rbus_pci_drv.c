...
// TODO: shiang-7615, for debug
		...
		DBGPRINT(RT_DEBUG_OFF, ("%s(): device_id=0x%x, HWVersion=0x%x, FWVersion=0x%x, pAd->ChipID=0x%x, HIF_SYS_REV=0x%x\n", 
...
VOID RTMPInitPCIeLinkCtrlValue(RTMP_ADAPTER *pAd)
{
    ...
    if (pos != 0)
    {
		...
		if (vendor_id == PCIBUS_INTEL_VENDOR)
			...
			bFindIntel = TRUE;
					RTMP_SET_PSFLAG(pAd, fRTMP_PS_TOGGLE_L1);
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PSM : Aspire 8530 AMD NB. Use L1 Toggle. \n"));
				}
#endif
