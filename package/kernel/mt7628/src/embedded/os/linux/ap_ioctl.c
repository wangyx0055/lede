...
volatile PRX_PKT_QUEUE p_queue = NULL;

int enqueue(PRX_PKT_QUEUE p_queue,UCHAR * data, USHORT datasize)
{
	if(p_queue != NULL)
	{
		//printk("datasize:%d\n",datasize);
		if(((p_queue->tail + 1) % RX_QUEUE_SIZE) != p_queue->head)
		{	
			NdisZeroMemory(p_queue->buf[p_queue->tail].data,RX_80211_PKT_SIZE);
			NdisMoveMemory(p_queue->buf[p_queue->tail].data,data,datasize);
			p_queue->buf[p_queue->tail].length = datasize;
			p_queue->tail = ((p_queue->tail + 1) % RX_QUEUE_SIZE);
		}
	}
	return NDIS_STATUS_SUCCESS;
}

int dequeue(PRX_PKT_QUEUE p_queue,UCHAR * data)
{
	...
	if(p_queue != NULL)
	{
		...
		{
			Status = copy_to_user(data,(void *)&(p_queue->buf[p_queue->head]),sizeof(RX_PKT));
			p_queue->head = ((p_queue->head + 1) % RX_QUEUE_SIZE);
		}
	}
	return Status;
}
...
INT rt28xx_ap_ioctl(struct net_device *net_dev, struct ifreq *rq, int cmd)
{
    ...
    if(cmd== SIOCGIWNAME){
	    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IOCTL::SIOCGIWNAME\n"));
...
/*        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s() I/F(ra%d)(flags=%d): cmd = 0x%08x\n", __FUNCTION__, pObj->ioctl_if, RT_DEV_PRIV_FLAGS_GET(net_dev), cmd)); */
    }
...
/*	    	        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): I/F(ra%d)(flags=%d): cmd = 0x%08x\n", __FUNCTION__, index, RT_DEV_PRIV_FLAGS_GET(net_dev), cmd)); */
	        ...
	        {
/*	        	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): can not find I/F\n", __FUNCTION__)); */
	            return -ENETDOWN;
	        }
	    }
	    else    /* ioctl command from I/F(ra0) */
	    {
			GET_PAD_FROM_NET_DEV(pAd, net_dev);	
    	    pObj->ioctl_if = MAIN_MBSSID;
/*	        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): can not find I/F and use default: cmd = 0x%08x\n", __FUNCTION__, cmd)); */
	    }
	...
	else if (RT_DEV_PRIV_FLAGS_GET(net_dev) == INT_WDS)
	{
		...
		for(index = 0; index < MAX_WDS_ENTRY; index++)
		{
			if (pAd->WdsTab.WdsEntry[index].dev == net_dev)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): I/F(wds%d)(flags=%x): cmd = 0x%08x\n", __FUNCTION__, pObj->ioctl_if, RT_DEV_PRIV_FLAGS_GET(net_dev), cmd));
	...
	else if (RT_DEV_PRIV_FLAGS_GET(net_dev) == INT_APCLI)
	{
		...
		for (index = 0; index < MAX_APCLI_NUM; index++)
		{
...
#ifdef RELEASE_EXCLUDE
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): I/F(apcli%d)(flags=%x): cmd = 0x%08x\n", __FUNCTION__, pObj->ioctl_if, RT_DEV_PRIV_FLAGS_GET(net_dev), cmd));
#endif /* RELEASE_EXCLUDE */
				break;
			}
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): can not find Apcli I/F\n", __FUNCTION__));
				return -ENETDOWN;
			}
		}
    ...
    {
/*    	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("IOCTL is not supported in WDS interface\n")); */
    	return -EOPNOTSUPP;
    }
#endif /* 0 */
	...
	switch(cmd)
	{
        ...
        case SIOCGIFHWADDR:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IOCTLIOCTLIOCTL::SIOCGIFHWADDR\n"));
		...
		case SIOCGIWESSID:  /*Get ESSID */
			{
              ...
              /*erq->length = pAd->ApCfg.MBSSID[pObj->ioctl_if].SsidLen; */
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IOCTL::SIOCGIWESSID (Len=%d, ssid=%s...)\n", erq->length, (char *)erq->pointer));
			}
			break;
...
/*				struct iw_range range; */
		...
		case SIOCSIWGENIE:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("ioctl SIOCSIWGENIE apidx=%d\n",apidx));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("ioctl SIOCSIWGENIE length=%d, pointer=%x\n", wrqin->u.data.length, wrqin->u.data.pointer));
...
/* end of modification */
		...
		case RTPRIV_IOCTL_SHOW_CONNSTATUS:
			Status = RTMP_AP_IoctlHandle(pAd, wrq, CMD_RTPRIV_IOCTL_SHOW_CONNSTATUS, 0, NULL, 0);
			break;

#ifdef CONFIG_ELIAN_SUPPORT
		case RTPRIV_IOCTL_MSC_CTRL:
			{
				char result[512] = {0};
				if(access_ok(VERIFY_READ,wrqin->u.data.pointer,wrqin->u.data.length) != TRUE)
				{
				...
				msc_cmd_handler(wrqin->u.data.pointer,wrqin->u.data.length,result,pAd);
				wrqin->u.data.length = strlen(result);
				Status = copy_to_user(wrqin->u.data.pointer,result,wrqin->u.data.length);
			}
			break;
	   case RTPRIV_IOCTL_GET_80211_DATA:
			{
				if(osal_strncmp(wrqin->u.data.pointer, "start", 5) == 0)
				{ 
					if(p_queue != NULL)
					{
						printk("already init queue!!!\n");
						break;
					}
					Status = os_alloc_mem(NULL,(UCHAR **)&p_queue,sizeof(RX_PKT_QUEUE));
					if(Status == NDIS_STATUS_FAILURE)
					{
						p_queue = NULL;
						printk("os_alloc_mem fail!!!\n");
						break;
					}
					memset(p_queue,0,sizeof(RX_PKT_QUEUE));
					printk("init queue success!!!\n");
				}
				else if(osal_strncmp(wrqin->u.data.pointer, "stop", 4) == 0)
				{
					if(p_queue != NULL)
					{
						os_free_mem(NULL,p_queue);
						p_queue = NULL;
						printk("deinit queue success!!!\n");
					}
					else
					{
						printk("already deinit queue!!!\n");
					}
				}
				else if(osal_strncmp(wrqin->u.data.pointer ,"result", 6) == 0)
				{
					if(p_queue == NULL)
					{
					...
					Status = dequeue(p_queue,(UCHAR *)wrqin->u.data.pointer);
				}
