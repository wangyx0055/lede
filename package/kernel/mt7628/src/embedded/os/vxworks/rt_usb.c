	...
	IN RTMP_ADAPTER *pAd)
{
	...
	*/
	...
	{
		DBGPRINT (DBG_LVL_ERROR, ("%s: unable to start RtmpTimerQThread\n", RTMP_OS_NETDEV_GET_DEVNAME(pAd->net_dev)));
	...
	if (status == NDIS_STATUS_FAILURE) 
	{
		DBGPRINT (DBG_LVL_ERROR, (": unable to start RTUSBCmdThread\n", RTMP_OS_NETDEV_GET_DEVNAME(pAd->net_dev)));
		return NDIS_STATUS_FAILURE;
	}
...
pUSBHST_URB usb_alloc_urb(int iso_packets, int mem_flags)
{
	...
	if (!pURB) 
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : Failed: URB meory insufficient.\n", __FUNCTION__));
		return NULL;
	}
...
void * usb_buffer_alloc(pUSB_DEVICE_INFO pUSBDEV, unsigned int  size, unsigned mem_flags, ra_dma_addr_t *DMAAddr)
{
	...
	if(!pVirtualAddress)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : usb pool source is not available.\n", __FUNCTION__));	
		return NULL;
	}
...
USBHST_STATUS usb_ctrl_msg_complete(pUSBHST_URB pURB) /* Ptr to USB Request Block */
{
    ...
    {
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : URB context is NULL.\n", __FUNCTION__));
									...
									UINT16 wValue, UINT16 wIndex, void *pBuffer, UINT16 wLength, int timeout)
{
    ...
    if (!pSetup) 
	{
    	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : Insufficient Resource for Setup packet.\n", __FUNCTION__));
	...
	if (!EventId) 
	{
	    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : Insufficent Resource for completion event.\n", __FUNCTION__));
		Result = USBHST_INSUFFICIENT_RESOURCE;
		goto ERROR3;
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : call usbHstURBSubmit not successfully.\n", __FUNCTION__));
		goto ERROR4;		
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : Timeout Occurred, RequestType = 0x%x, Request = 0x%x, wValue = 0x%x, wIndex = 0x%x, wLength = 0x%x.\n", 
...
USBHST_STATUS usb_unlink_urb(pUSBHST_URB pURB)
{
	...
	if (!pURB) 
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : pURB[%p] is invalid.\n", __FUNCTION__, pURB));
	...
	if (Result != USBHST_SUCCESS)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s : call usbHstURBCancel to unlink URB failed\n.", __FUNCTION__));
	}
