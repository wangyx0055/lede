...
int rt28xx_irq_release(IN PNET_DEV pNetDev)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("disConnect() the IRQ with status=%d!\n", status));
...
/*		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (">>>fOP_STATUS_DOZE<<<\n")); */
	...
	/*RTMP_ISR_LOG (DBG_LVL_TRACE, "%s():0x%x\n", __FUNCTION__, IntSource.word, 0, 0, 0, 0); */
	
/*	RTMP_IO_READ32(pAd, INT_SOURCE_CSR, &IsrAfterClear); */
/*	RTMP_IO_READ32(pAd, MCU_INT_SOURCE_CSR, &McuIsrAfterClear); */
/*	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("====> RTMPHandleInterrupt(ISR=%08x,Mcu ISR=%08x, After clear ISR=%08x, MCU ISR=%08x)\n", */
...
#ifdef DBG
	if ((RTDebugLevel == DBG_LVL_LOUD) && (((++print_int_count) % 100) == 0))
	{
		UINT32 reg = 0;
		int Count, free;
