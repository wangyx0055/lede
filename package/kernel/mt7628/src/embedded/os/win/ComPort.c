	...
	)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: RTPCIComWrite --> InsertByteCount=%d\n", InsertByteCount));
	...
	else
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: RTPCIComWrite --> ComBuffer is full!!!\n"));
	}
...
//	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: RTPCIComWrite --> AvailDataCount=%d\n", AvailDataCount);
	...
	)
{
	...
	// Remove from buffer and transmit it
	uartTX.word = RTMP_Dummy_IO_READ32(pDevExt, UARTTX);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: RTPCIComWrite --> uartTX.field.TxFrCnt=%d, wantTxnum=%d\n", uartTX.field.TxFrCnt,(MAX_COM_BUFFER_SIZE-pDevExt->comTxBuffer.TxFreeCount)));
	if(pDevExt->comTxBuffer.TxFreeCount != MAX_COM_BUFFER_SIZE)
	{
		for(i=0; i<uartTX.field.TxFrCnt; i++)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: RTPCIHandleUartTxThDoneInterrupt -->=%X\n", *(pDevExt->comTxBuffer.pComTxBuf + pDevExt->comTxBuffer.TxRemovePointer)));
			RTMP_Dummy_IO_WRITE32(pDevExt, 
							UARTTX, 
							*(pDevExt->comTxBuffer.pComTxBuf + pDevExt->comTxBuffer.TxRemovePointer));
	...
	)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: RTPCIHandleUartIdleThDoneInterrupt --> RxByteCnt=%d\n", uartRX.field.RxByteCnt));

	if((uartRX.field.RxDv == 1) && (uartRX.field.RxByteCnt <= pDevExt->comRxBuffer.RxFreeCount))
	{
		data = (UCHAR)uartRX.field.RXD;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("PciDumpr: RTPCIHandleUartIdleThDoneInterrupt --> RXD=%x\n", uartRX.field.RXD));
		RtlCopyMemory(
