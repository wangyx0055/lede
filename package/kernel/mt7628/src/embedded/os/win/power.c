...
//  PUNICODE_STRING    tagString;
    ...
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("USB: Wdm2Power==>\n"));
    //
...
//          ntStatus = STATUS_PENDING;
    ...
    case IRP_MN_WAIT_WAKE:
        ...
        if(!NT_SUCCESS(ntStatus)) {
            ...
            BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 1, ("Lower drivers failed the wait-wake Irp\n"));
        }
...
//        ntStatus = STATUS_PENDING;
    ...
    default:
        ...
        if(!NT_SUCCESS(ntStatus)) {
            ...
            BulkUsb_MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, 1, ("Lower drivers failed default power Irp\n"));
        }
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Power %I",Irp));
...
NTSTATUS PowerSetPower( IN RTMP_ADAPTER *dx, IN PIRP Irp)
{
	...
	if( PowerType==SystemPowerState)
	{
		...
		if( DesiredDevicePowerState<dx->PowerState)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("System state %d.  Increase device power to %d",
		...
		else if( DesiredDevicePowerState>dx->PowerState)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("System state %d.  Decrease device power to %d",
						PowerState.SystemState, DesiredDevicePowerState));
	...
	else if( PowerType==DevicePowerState)
	{
		...
		if( DesiredDevicePowerState<dx->PowerState)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Increase device power to %d", DesiredDevicePowerState));
			// Process on way up stack...
			PoStartNextPowerIrp(Irp);
			IoCopyCurrentIrpStackLocationToNext(Irp);
		...
		else if( DesiredDevicePowerState>dx->PowerState)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Decrease device power to %d", DesiredDevicePowerState));
			// Set power state
	...
	//	Unrecognised Set Power
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Power: unrecognised power type %d",PowerType));
#endif
...
NTSTATUS DefaultPowerHandler( IN RTMP_ADAPTER *dx, IN PIRP Irp)
{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("DefaultPowerHandler"));
...
NTSTATUS OnCompleteIncreaseSystemPower( IN PDEVICE_OBJECT fdo, IN PIRP Irp, IN PVOID context)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("OnCompleteIncreaseSystemPower %x",status));
	if( !NT_SUCCESS(status))
		return status;
...
NTSTATUS OnCompleteIncreaseDevicePower( IN PDEVICE_OBJECT fdo, IN PIRP Irp, IN PVOID context)
{
	...
	if (Irp->PendingReturned)
		IoMarkIrpPending(Irp);
	status = Irp->IoStatus.Status;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("OnCompleteIncreaseDevicePower %x",status));
	if( !NT_SUCCESS(status))
		return status;
...
NTSTATUS SendDeviceSetPower( IN RTMP_ADAPTER *dx, IN DEVICE_POWER_STATE NewDevicePowerState)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("SendDeviceSetPower to %d", NewDevicePowerState));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("SendDeviceSetPower: Device state not set properly by us.  Setting again"));
					...
					IN POWER_STATE PowerState, IN PVOID Context, IN PIO_STATUS_BLOCK IoStatus)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("OnCompleteDeviceSetPower"));
	psdsp = (PSDSP)Context;
...
void SetPowerState( IN RTMP_ADAPTER *dx, IN DEVICE_POWER_STATE NewDevicePowerState)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("SetPowerState %d", NewDevicePowerState));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("SetPowerState IRQL %d fail", KeGetCurrentIrql()));
	}
	else
