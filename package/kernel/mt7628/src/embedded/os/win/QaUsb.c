				    ...
				    IN ULONG Arg2)
{
	...
	//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("CallUSBDI()>>>\n"));
	...
	if(KeGetCurrentIrql() < DISPATCH_LEVEL)
	{
		...
		//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("KeGetCurrentIrql() < DISPATCH_LEVEL (no QueueWorkItem)\n"));
		...
		if (ntStatus == STATUS_PENDING)
		{
			...
			//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("CallUSBDI: waiting for URB completion\n"));
			...
			if (ntStatus == STATUS_TIMEOUT)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("OSWaitEvent STATUS_TIMEOUT\n"));
				return(STATUS_TIMEOUT);
			}
			//IoWaitStatus = NdisWaitEvent((PNDIS_EVENT)&IoEvent,NDIS_IO_WAIT);
			//if (!IoWaitStatus)
			//{
			//	DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR,("OSWaitEvent STATUS_TIMEOUT\n"));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("KeGetCurrentIrql() >= DISPATCH_LEVEL (QueueWorkItem)\n"));
...
VOID WorkItemProcess_MUsb(PVOID Context)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("WorkItemProcess_MUsb()>>>\n"));
