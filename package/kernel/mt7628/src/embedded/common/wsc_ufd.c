	...
	INOUT RTMP_STRING **pXmlData)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("======> WscPassXmlDeclare\n"));
	
	ptr = rtstrstr(*pXmlData, (RTMP_STRING *)XML_DECLARE_START);
	if (ptr == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WscPassXmlDeclare: missing XML Declare <?xml\n"));
		return FALSE;
	}
	...
	if (ptr == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("XML syntax error: missing XML Declare ?>\n"));
		return FALSE;
	}
	...
	OUT	NDIS_802_11_SSID	*pSsid)
{
	...
	if (ptr == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WscGetXmlSSID: missing <ssid\n"));
		return FALSE;
	}
	...
	OUT USHORT	*pAuthType)
{
	...
	*pAuthType = 0;
	...
	if (ptr == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WscGetXmlAuth: missing %s\n", XML_AUTH_START));
		return FALSE;
	}
	...
	OUT USHORT	*pEncrType)
{
	...
	*pEncrType = 0;
	...
	if (ptr == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WscGetXmlEncr: missing %s\n", XML_ENCR_START));
		return FALSE;
	}
	...
	OUT USHORT	*pKeyLen)
{
	...
	if (ptr == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WscGetXmlKey: missing %s\n", XML_KEY_START));
		return FALSE;
	}
	...
	OUT UCHAR	*pKeyIndex)
{
	...
	*pKeyIndex = 1;
	...
	if (ptr == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WscGetXmlKeyIndex: missing %s\n", XML_KEY_INDEX_START));
		return FALSE;
	}
...
#if 0
/*
	About Dual Radio APs
	1.	If a Wsetting.wfc file doesn't contain both the channel2Dot4 and channel5Dot0 elements,
		the the AP sould configure both radio identically - that is, the same SSID and Network Key.
	2.	If a Wsetting.wfc file contains the channel2Dot4 and channel5Dot0 elements, the the AP should
		configure the 5.0-GHz radio to the SSID value that the file specifies. The 2.4-GHz radio should
		be configured to the same SSID with a "-2" appended. If the original SSID is greater than 30 chars,
		then it should be truncated to 30 chars and a "-2" added to the end. All of the remaining
		configuration elements should remain the same.
		<<from "Windows Connect Now-UFD and Windows Vista">>
*/
#endif
	...
	IN RTMP_STRING *pUfdFileName)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("--> %s: pUfdFileName is NULL\n", __FUNCTION__));
		return FALSE;
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("--> %s: Error opening file %s\n", __FUNCTION__, pUfdFileName));
		return FALSE;
	}
	else 
	{
		...
		if (pXmlData == NULL)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("pXmlData mem alloc fail. (fileLen = %d)\n", fileLen));
			return FALSE;
		}
		...
		if (rv != fileLen)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RtmpOSFileRead fail, fileLen = %d\n", fileLen));
			RtmpOSFSInfoChange(&osFSInfo, FALSE);
			goto ReadErr;
		}
	}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WscReadProfileFromUfdFile\n"));
	if (WscPassXmlDeclare(&pXmlData))
	{
		if (WscGetXmlSSID(pXmlData, &pCredential->SSID))
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("SSID = %s(%d)\n", pCredential->SSID.Ssid, pCredential->SSID.SsidLength));
		}
		else
		{
			RV = FALSE;
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Credential.AuthType = 0x%04x\n", pCredential->AuthType));
		}
		else
		{
			RV = FALSE;
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Credential.EncrType = 0x%04x\n", pCredential->EncrType));
		}
		else
		{
			RV = FALSE;
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Credential.Key = %s (%d)\n", pCredential->Key, pCredential->KeyLength));
		}
		else
		{
			RV = FALSE;
		...
		*/
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("pCredential->KeyIndex = %d\n", pCredential->KeyIndex));
		}
		
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WscReadProfileFromUfdFile OK\n"));
	...
	IN  RTMP_STRING *pUfdFileName)
{
	...
	else 
	{
		...
		if ((offset = rtstrstr(pXmlTemplate, (RTMP_STRING *)XML_GUID_MARK)) != NULL)
		{
		...
		if ((offset = rtstrstr(pXmlTemplate, (RTMP_STRING *)XML_AP_GUID_MARK)) != NULL)
		{
			RtmpOSFileWrite(file_w, (RTMP_STRING *)pXmlTemplate, (int)(offset - pXmlTemplate));			
		...
		if ((offset = rtstrstr(pXmlTemplate, (RTMP_STRING *)XML_SSID_MARK)) != NULL)
		{
			RtmpOSFileWrite(file_w, (RTMP_STRING *)pXmlTemplate, (int)(offset - pXmlTemplate));			
		...
		if ((offset = rtstrstr(pXmlTemplate, (RTMP_STRING *)XML_AUTH_MARK)) != NULL)
		{
			RtmpOSFileWrite(file_w, (RTMP_STRING *)pXmlTemplate, (int)(offset - pXmlTemplate));			
			for (PWSC_UFD_AUTH_TYPE = WSC_UFD_AUTH_TYPE; PWSC_UFD_AUTH_TYPE->auth_str; PWSC_UFD_AUTH_TYPE++)
			{
			...
			if (bFound == FALSE)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("--> %s: Unknow Auth Type(=%x)\n", __FUNCTION__, pCredential->AuthType));
				bRtn = FALSE;
				goto out;
			}
		}
		if ((offset = rtstrstr(pXmlTemplate, (RTMP_STRING *)XML_ENCR_MARK)) != NULL)
		{
			bFound = FALSE;
			RtmpOSFileWrite(file_w, (RTMP_STRING *)pXmlTemplate, (int)(offset - pXmlTemplate));			
			for (PWSC_UFD_ENCR_TYPE = WSC_UFD_ENCR_TYPE; PWSC_UFD_ENCR_TYPE->encr_str; PWSC_UFD_ENCR_TYPE++)
			...
			if (bFound == FALSE)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("--> %s: Unknow Encr Type(=%x)\n", __FUNCTION__, pCredential->EncrType));
				bRtn = FALSE;
				goto out;
			}
		}
		if ((offset = rtstrstr(pXmlTemplate, (RTMP_STRING *)XML_KEY_MARK)) != NULL)
		{
