...
/*If GO start, we need to change to GO Channel*/
	...
	if (OpMode == OPMODE_AP)
	{
		...
		if (P2P_CLI_ON(pAd) && (ApScanRunning(pAd) == TRUE))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s::  Scan Done! reset APCLI CTRL State Machine!\n", __FUNCTION__));
	...
	if ((pAd->ScanCtrl.Channel == 0) || ScanPending) 
	{
		...
		if (count > 10)
		{
			...
			if (ScanType != SCAN_PASSIVE)
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Driver is Alive; ScanType %d\n", ScanType));
		}
#endif /* WIDI_SUPPORT */
...
#if 0
	DBGPRINT(RT_DEBUG_ERROR, ("%s, %u, scan_channel = %u, NumOfChannels = %u, LastScanChannel = %u, bScanning = %u\n",
...
#if 1
	DBGPRINT(RT_DEBUG_TRACE, ("%s, %u, scan_channel = %u, NumOfChannels = %u, LastScanChannel = %u, bScanning = %u\n",
			__FUNCTION__, __LINE__,
			scan_channel,
			PartialScanCtrl->NumOfChannels,
			PartialScanCtrl->LastScanChannel,
