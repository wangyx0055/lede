	...
	IN PRTMP_ADAPTER pAd)
{
...

	{
		...
		if (MTxCycle == 10000)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s(cnt=%d,time=0x%lx):stop MTx,macStatus=0x%x!\n", 
		...
		if (MRxCycle == 10000)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s(cnt=%d,time=0x%lx):stop MRx, macStatus=%d!\n",
...
INT Set_TestRxIQCalibration_Proc(
	...
	IN RTMP_STRING *arg)
{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TestRxIQCalibration !!!\n"));
	...
	IN RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("TestLOFTTxIQCalibration !!!\n"));
	...
	IN BOOLEAN bTransmit)
{
	...
	if (bTransmit)
	{
		...
		if (k_count == 200)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Wait Null Frame SendOut to MAX  !!!\n"));
		}
	}
	...
	IN UCHAR AntIdx)
{
	...
	for (k_count = 0; k_count < 64; k_count++)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("PM (%d), PM_inv (%d)\n", PM, PM_inv));
	...
	IN UCHAR AntIdx)
{
	...
	for (check_loop = 0; check_loop < 2; check_loop++)
	{
		if (AntIdx == 0)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("DPD Scaling for TX0 !!!\n"));
			if (check_loop == 0)
				RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R186, 0x02);
			else
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("DPD Scaling for TX1 !!!\n"));
			if (check_loop == 0)
				RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R186, 0x04);
			else
		...
		for (k_count = 0; k_count < 64; k_count++)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("(%d). AM_temp (%d), AM_Dealta (%d), AM_scaling (%d)\n", k_count, AM_temp, AM_Dealta, AM_scaling));
		}
	}
}
	...
	IN UCHAR AntIdx)
{
	...
	switch (AntIdx)
	{
		case 0:
			...
			*/
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, (" gain_atten_bb = %d, gain_atten_rf = %d !\n", gain_atten_bb, gain_atten_rf));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, (" txALC_init = %d, txALC_limit = %d !\n", txALC_init, txALC_limit));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, (" temp_comp = %d, tx_alc_txwi = %d !\n", temp_comp, tx_alc_txwi));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, (" txvga2 = %d, txvga3 = %d !\n", txvga2, txvga3));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, (" mac_gain_atten = %d !\n", mac_gain_atten));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, (" txALC_req = %d !\n", txALC_req));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, (" txALC_req_at = %d !\n", txALC_req_at));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, (" txALC_req_skip2 = %d !\n", txALC_req_skip2));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, (" txALC_req_skip3 = %d !\n", txALC_req_skip3));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, (" TX0: txALC_req_sat = %d !!!\n", txALC_req_sat));
		...
		case 1:
			...
			*/
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, (" txALC_req_skip2 = %d !\n", txALC_req_skip2));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, (" txALC_req_skip3 = %d !\n", txALC_req_skip3));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, (" TX1: txALC_req_sat = %d !!!\n", txALC_req_sat));
#endif /* RELEASE_EXCLUDE */
			// no dpd when power is less 15 dBm or greater 20 dBm
			bInternalTxALC = FALSE;
#ifdef RTMP_INTERNAL_TX_ALC
	...
	if (bNeedDoDPD == FALSE)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Don't need do DPD Calibration  !!!\n"));
	...
	for (check_loop = 0; check_loop < 2; check_loop++)
	{
		...
		for (index = 0; index < Max_Retry; index++)
		{
			while ((VGA_code_idx >= 0) && (VGA_code_idx <= 18))
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("RF Gain(%d), RF R3/4(%x)\n", RT6352_RFR3R4_TABLE[VGA_code_idx].Register, VGA_code));
				if (AntIdx == 0)
				{
				...
				RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R188, &byteValue);
				AM_63 = byteValue;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("AM_63 (%d)\n", AM_63));
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Appropriate VGA Gain(%d), R66(%x)\n", RT6352_VGA_TABLE[VGA_code_idx].Register, VGA_code));
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("AM_63 (%d)\n", AM_63));
					break;
				}
			}
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, (" VGA Code idx overflow(%d), AM_63(%d) !!!\n", VGA_code_idx, AM_63));
				DPD_Cal_success = FALSE;
				break;
			}
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("AM_SUM = %d, AM_10 = %d !!!\n", AM_SUM, AM_10));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("(AM_SUM - 5.5*AM_10) = (%d) @@\n", (AM_SUM - ((11 * AM_10) / 2))));

			if (AntIdx == 0)
			{
				if (((AM_SUM - ((11 * AM_10) / 2)) >= -3) && ((AM_SUM - ((11 * AM_10) / 2)) <= Pass_Thres))
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("DPD Calibration Pass for TX0 !!!\n"));
			...
			else
			{
				if (index == 4)
				{
					...
					{
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("DPD Calibration Pass for TX1 !!!\n"));
		...
		if (DPD_Cal_success)
		{
				...
				for (k_count = 0; k_count < 64; k_count++)
				{
					RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R187, ((4 * k_count) + 3));
					RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R188, &byteValue);
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): AM[%d] = %d !\n", __FUNCTION__, k_count, byteValue));
				}
		}
#endif /* RELEASE_EXCLUDE */
	}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, (" Do DPD Calibration !!!\n"));
	...
	if ((Ant0 == 2) || (Ant1 == 2))
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" Enable DPD Compensation !!!\n"));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, (" DPD Calibration Ant00 = %d, Ant01 = %d, Ant10 = %d,  Ant11 = %d!!!\n",
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, (" DPD Calibration Ant0 = %d, Ant1 = %d !!!\n", Ant0, Ant1));
	}
	...
	IN RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, (" TestDPDCalibration !!!\n"));
	...
	IN RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, (" TestDPDCalibrationTX0 !!!\n"));
	...
	if (Ant0 == 2)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, (" Enable DPD Compensation !!!\n"));
	...
	IN RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, (" TestDPDCalibrationTX1 !!!\n"));
	...
	IN PRTMP_ADAPTER pAd)
{
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("\t\tCheck CAL_R0(Cnt:%d, Value:0x%x)\n",
			cnt, bbp_val));
#endif /* RELEASE_EXCLUDE */
	...
	if (cal_val >= 0x40)
		cal_val -= 128;
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("\t\tCAL_R57:0x%x => cal_val=0x%02x\n",
	...
	IN BOOLEAN bTxCal)
{
	...
	do {
		RtmpusecDelay(500);
		RT635xReadRFRegister(pAd, RF_BANK5, RF_R01, &rf_val);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("ABB.R1 = %d, Cnt = %d \n", rf_val, cnt));
	...
	do {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s():Start BW Cal for %dMHz\n", 
...
do_cal:
...
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("\t\tR57(Init=%d,Cal=%d,Diff=%d,FilterTarget=%d,",
							cal_r32_init, cal_r32_val, cal_diff, filter_target));
		if (bTxCal)
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("TxAgcFc=%d)\n", tx_agc_fc));
		else
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("RxAgcFc=%d)\n", rx_agc_fc));
#endif /* RELEASE_EXCLUDE */
			...
			((cal_diff < filter_target) && (cmm_agc_fc == 0x3f)))
		{
			...
			if (bTxCal)
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("\t\tBW%dM FilterCal cannot find valid val(loop=%d), set RxAgcFc=0\n",
								(loop == 0 ? 20 : 40), loop));
#endif /* RELEASE_EXCLUDE */
			if (bTxCal)
				tx_agc_fc = 0;
...
#ifdef RELEASE_EXCLUDE
			if (bTxCal)
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("\t\ttx_agc_fc++ = %d, redo cal!\n", tx_agc_fc));
			else
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("\t\trx_agc_fc++ = %d, redo cal!\n", rx_agc_fc));
#endif /* RELEASE_EXCLUDE */
			goto do_cal;
		}
		else
		{
			...
			if (bTxCal)
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("\t\tget rx_agc_fc, cal done!\n"));
#endif /* RELEASE_EXCLUDE */
		}
	
#ifdef RELEASE_EXCLUDE
		if (bTxCal)
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\t=>BW %dMHz FilterCal Value=0x%x!\n",
							(loop == 0 ? 20 : 40), tx_agc_fc));
		else
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\t=>BW %dMHz FilterCal Value=0x%x!\n",
							(loop == 0 ? 20 : 40), rx_agc_fc));
#endif /* RELEASE_EXCLUDE */
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, (" RxDCOC_Calibration !!!\n"));
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, (" RxIQ Calibration !!!\n"));
	...
	{ /* Path control */
		...
		while( vga_idx < 11)
		{
			...
			if ((bbpval & 0xff)== 0x93)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Fatal Error: Calibration doesn't finish"));
				goto Restore_Value;
			}
			...
			/* Software AGC */
			bbpval1 = si - mi*mi;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RXIQ si=%d, sq=%d, riq=%d, bbpval %d, vga_idx %d\n",
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Sigma_i=%d, Sigma_q=%d, R_iq=%d\n", Sigma_i, Sigma_q, R_iq));

		if(((Sigma_i <= 1400 ) && (Sigma_i >= 1000))
#if 0
			&& ((Sigma_q <= 1400 ) && (Sigma_q >= 1000))
#else
			&& ((Sigma_i - Sigma_q) <= 112)			
			&& ((Sigma_i - Sigma_q) >= -112)
#endif
		...
		{
			R_iq = 10*(riq-(mi*mq));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RXIQ Sigma_i=%d, Sigma_q=%d, R_iq=%d\n", Sigma_i, Sigma_q, R_iq));

		/* G)   Calculate Gain/ Phase imbalance */
		G_rx = (1000 * Sigma_q) / Sigma_i;
		G_imb = ((-2) * 128 * (1000 - G_rx)) / (1000 + G_rx);
		Ph_rx = (R_iq * 2292 ) / (Sigma_i * Sigma_q);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RXIQ G_imb=%d, Ph_rx=%d\n", G_imb, Ph_rx));

			if((Ph_rx > 20 ) || (Ph_rx < -20) )
			{
				Ph_rx = 0;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RXIQ calibration FAIL\n"));
			}
		...
		else
		{
			G_imb = 0;
			Ph_rx = 0;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RXIQ Sigma_i=%d, Sigma_q=%d, R_iq=%d\n", Sigma_i, Sigma_q, R_iq));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RXIQ calibration FAIL\n"));
		}
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("RF Tx_0 self calibration start\n"));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("RF Tx self calibration end\n"));
