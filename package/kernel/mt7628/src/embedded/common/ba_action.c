	...
	OUT UCHAR			*pWinSize)
{
...
#ifdef RT3593
	RT3593_AP_MAX_BW_SIZE_GET(pAd, pEntryPeer, MaxSize);
#endif /* RT3593 */
#endif /* CONFIG_AP_SUPPORT */

#ifdef CONFIG_STA_SUPPORT
#ifdef RT3593
	RT3593_STA_MAX_BW_SIZE_GET(pAd, pEntryPeer, MaxSize);
#endif /* RT3593 */
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ba>WinSize=%d, MaxSize=%d, MaxPeerRxSize=%d\n", 
			*pWinSize, MaxSize, MaxPeerRxSize));

	MaxSize = min(MaxPeerRxSize, MaxSize);
	if ((*pWinSize) > MaxSize)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ba> reassign max win size from %d to %d\n", 
				*pWinSize, MaxSize));
...
BOOLEAN ba_reordering_resource_init(RTMP_ADAPTER *pAd, int num)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Allocate %d memory for BA reordering\n", (UINT32)(num*sizeof(struct reordering_mpdu))));
	...
	if (mem == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Can't Allocate Memory for BA Reordering\n"));
		return(FALSE);
	}
...
*/
		...
		{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("timeout[%d] (%lx-%lx = %d > %d): %x\n ", pBAEntry->list.qlen, Now32, (pBAEntry->LastIndSeqAtTimer), 
			   (int)((long) Now32 - (long)(pBAEntry->LastIndSeqAtTimer)), MAX_REORDERING_PACKET_TIMEOUT,
			   pBAEntry->LastIndSeq));
		ba_refresh_reordering_mpdus(pAd, pBAEntry);
			pBAEntry->LastIndSeqAtTimer = Now32;
	...
	IN BOOLEAN isForced)
{
	...
	if (TID >= NUM_OF_TID)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Wrong TID %d!\n", TID));
		return;
	}
...
/*		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("DeCline BA from Peer\n"));*/
/*		return;*/
	}
	...
	if (Idx == 0)
	{
		...
		if (pBAEntry == NULL)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s(): alloc BA session failed\n",
						__FUNCTION__));
			return;
		}
	}
...
/*
    if (pAd->CoexMode.TxBAWinLimit > 0)
        BAWinSize = min((UCHAR)pAd->CommonCfg.BACapability.field.RxBAWinLimit, pAd->CoexMode.TxBAWinLimit);
	*/
#endif
	...
	IN FRAME_ADDBA_RSP *pFrame)
{
	...
	if ((Idx !=0) && (pBAEntry->TID == TID) && (pBAEntry->ORI_BA_Status == Originator_WaitRes))
	{
	   ...
	   if(IS_MT76XXBTCOMBO(pAd) && (BT_STATUS_TEST_FLAG(pAd,fBTSTATUS_BT_ACTIVE)))
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():TXBAbitmap=%x, AMSDUCap=%d, BAWinSize=%d, TimeOut=%ld\n",
...
#ifdef MT76XX_BTCOEX_SUPPORT
    /* In coex mode, if RxBAWinLimit has been adjusted, need to consider its value */
    /*
    if (pAd->CoexMode.RxBAWinLimit > 0)
        BAWinSize = min(BAWinSize, pAd->CoexMode.RxBAWinLimit);
    */
#endif
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s(%ld): Idx = %d, BAWinSize(req %d) = %d\n",
	...
	if (pBAEntry != NULL)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MACEntry[%d]RXBAbitmap = 0x%x. BARecWcidArray=%d\n",
				pEntry->wcid, pEntry->RXBAbitmap, pEntry->BARecWcidArray[TID]));
	}
	else
	{
		Status = FALSE;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Can't Accept ADDBA for %02x:%02x:%02x:%02x:%02x:%02x TID = %d\n", 
...
BA_REC_ENTRY *BATableAllocRecEntry(RTMP_ADAPTER *pAd, USHORT *Idx)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("BA Recipeint Session (%ld) > %d\n", 
...
VOID BATableFreeOriEntry(RTMP_ADAPTER *pAd, ULONG Idx)
{
	...
	if (pBAEntry->ORI_BA_Status != Originator_NONE)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Wcid = %d, TID = %d\n", __FUNCTION__, pBAEntry->Wcid, pBAEntry->TID));

		if (pBAEntry->ORI_BA_Status == Originator_Done)
		{
			pAd->BATable.numDoneOriginator -= 1;
		 	pEntry->TXBAbitmap &= (~(1<<(pBAEntry->TID) ));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BATableFreeOriEntry numAsOriginator= %ld\n", pAd->BATable.numAsRecipient));
			/* Erase Bitmap flag.*/
		}
	...
	IN BOOLEAN bForceSend)
{
...
#else				
				MlmeEnqueue(pAd, ACTION_STATE_MACHINE, MT2_MLME_ORI_DELBA_CATE, sizeof(MLME_DELBA_REQ_STRUCT), (PVOID)&DelbaReq, 0);
				RTMP_MLME_HANDLER(pAd);
#endif
			}
			else
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(bForceSend):alloc memory failed!\n", __FUNCTION__));
			}
		}
	...
	pBAEntry = &pAd->BATable.BAOriEntry[Idx];
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("\t===>Idx = %d, Wcid=%d.TID=%d, ORI_BA_Status = %d \n", Idx, Wcid, TID, pBAEntry->ORI_BA_Status));
						 ...
						 IN      BOOLEAN         bPassive)
{
	...
	pBAEntry = &pAd->BATable.BARecEntry[Idx];
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("\t===>Idx = %ld, Wcid=%d.TID=%d, REC_BA_Status = %d \n", Idx, Wcid, TID, pBAEntry->REC_BA_Status));
    ...
    IN PVOID SystemSpecific3) 
{
	...
	if ((pAd->OpMode == OPMODE_STA) && IS_ENTRY_CLIENT(pEntry) && IS_P2P_ENTRY_NONE(pEntry))
		...
		RTMP_MLME_HANDLER(pAd);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("BA Ori Session Timeout(%d) : Send ADD BA again\n", pBAEntry->Token));
    ...
    IN PVOID SystemSpecific3) 
{
	...
	if ((pBAEntry->REC_BA_Status == Recipient_Accept))
	{
		...
		if (RTMP_TIME_AFTER((unsigned long)Now32, (unsigned long)(pBAEntry->LastIndSeqAtTimer + REC_BA_SESSION_IDLE_TIMEOUT)))
		{
			...
			/* flush all pending reordering mpdus */
			ba_refresh_reordering_mpdus(pAd, pBAEntry); 
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%ld: REC BA session Timeout\n", Now32));
		}
	}
}
...
VOID PeerAddBAReqAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s ==> (Wcid = %d)\n", __FUNCTION__, Elem->Wcid));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("BA - PeerAddBAReqAction----> \n"));
		...
		(pAd->bPermitRecBaDown == TRUE))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("no need Block Ack AT BT MODE!"));
#endif /* RELEASE_EXCLUDE */
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("ACTION - PeerBAAction() Status = %d\n", Status));
	}
	else 
#endif /* BT_COEXISTENCE_SUPPORT */
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("ACTION - PeerBAAction() allocate memory failed \n"));
		return;
	}
	...
	if (pMacEntry)
	{
		...
		{
			ActHeaderInit(pAd, &ADDframe.Hdr, pAddr, pAd->MeshTab.wdev.if_addr, pAddr);
		}
		else
#endif /* MESH_SUPPORT */
#ifdef CONFIG_AP_SUPPORT
	...
	*(USHORT *)(&ADDframe.BaParm) = cpu2le16(*(USHORT *)(&ADDframe.BaParm));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(%d): TID(%d), BufSize(%d) <== \n", __FUNCTION__, Elem->Wcid, ADDframe.BaParm.TID, 
...
VOID PeerAddBARspAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	if (PeerAddBARspActionSanity(pAd, Elem->Msg, Elem->MsgLen))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("\t\t StatusCode = %d\n", pFrame->StatusCode));
...

{
	...
	if (PeerDelBAActionSanity(pAd, Elem->Wcid, Elem->Msg, Elem->MsgLen))
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("BA - PeerDelBAAction----> ORIGINATOR\n"));
			BARecSessionTearDown(pAd, Elem->Wcid, pDelFrame->DelbaParm.TID, TRUE);
		}
		else
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("BA - PeerDelBAAction----> RECIPIENT, Reason = %d\n",  pDelFrame->ReasonCode));
						  ...
						  IN PFRAME_BA_REQ pMsg) 
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): BAR-Wcid(%ld), Tid (%d)\n", __FUNCTION__, Wcid, TID));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BAR(%ld) : Tid (%d) - %04x:%04x\n", Wcid, TID, pFrame->BAStartingSeq.field.StartSeq, pBAEntry->LastIndSeq ));

	if (SEQ_SMALLER(pBAEntry->LastIndSeq, pFrame->BAStartingSeq.field.StartSeq, MAXSEQ))
	{
		/*MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BAR Seq = %x, LastIndSeq = %x\n", pFrame->BAStartingSeq.field.StartSeq, pBAEntry->LastIndSeq));*/
...
VOID SendSMPSAction(RTMP_ADAPTER *pAd, UCHAR Wcid, UCHAR smps)
{
	...
	{
		MlmeFreeMemory(pAd, pOutBuffer);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("BA - Invalid WCID(%d)\n",  Wcid));
		return;
	}
	...
	{
		MlmeFreeMemory(pAd, pOutBuffer);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("BA - wdev is null\n"));
		return;
	}
	...
	{
		if (smps == MMPS_DISABLE)
			RTMP_ASIC_MMPS_DISABLE(pAd);
		else if ((smps == MMPS_DYNAMIC) || (smps == MMPS_STATIC))
...
VOID SendBeaconRequest(RTMP_ADAPTER *pAd, UCHAR Wcid)
{
	...
	MiniportMMRequest(pAd, QID_AC_BE, pOutBuffer, FrameLen);
	MlmeFreeMemory(pAd, pOutBuffer);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Radio - SendBeaconRequest\n"));
}
#endif /* CONFIG_AP_SUPPORT */
...
#ifdef CONFIG_STA_SUPPORT
#ifdef THREADX
#ifdef THREADX_RX_ONE_COPY
		DCU pDcu = NULL;
		int pktLen = GET_OS_PKT_LEN(pRxPkt);
		pDcu = os_alloc_packet_input(pktLen + LENGTH_802_3, DRIVER_ALLOC);
		if (pDcu)
		{
			RTMP_RTIP_NET_BUF *pNetBuf;
			pNetBuf = (RTMP_RTIP_NET_BUF *)pRxPkt;
			PUCHAR pDcuPtr;
			
			pDcuPtr = DCUTODATA(pDcu);
			memcpy(pDcuPtr, Header802_3, LENGTH_802_3);
			pDcuPtr += LENGTH_802_3;
			memcpy(pDcuPtr, (void *)GET_OS_PKT_DATAPTR(pRxPkt), pktLen);
#if TCP_PARTIAL_CHKSUM
			DCUTOPACKET(pDcu)->tcp_partial_chksum_present = 0;
#endif
			DCUTOPACKET(pDcu)->length = pktLen + LENGTH_802_3;
			pNetBuf ->pDcuBuf = pDcu;
			pNetBuf ->pDataHead = DCUTODATA(pDcu);
			pNetBuf->bDcuFlg = DRIVER_ALLOC;
			SET_OS_PKT_DATAPTR(pRxPkt, DCUTODATA(pDcu));
			SET_OS_PKT_LEN(pRxPkt, DCUTOPACKET(pDcu)->length);
			//dc_log_printf("ReorderMPDU->pi=0x%x, RtmpPI=0x%x", pNetBuf->pi, RtmpPI);
		}
		else
#endif // THREADX_RX_ONE_COPY //
#endif // THREADX //
#endif /* CONFIG_STA_SUPPORT */
		{
...
VOID Indicate_AMPDU_Packet(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk, UCHAR wdev_idx)
{
	...
	if (!RX_BLK_TEST_FLAG(pRxBlk, fRX_AMSDU) &&  (data_len > max_pkt_len))
	{
		...
		err_size++;
		if (err_size > 20) {
			 MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("AMPDU DataSize = %d\n", pRxBlk->DataSize));
	...
	{
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Duplicated rx pHeader->Seq: 0x%x, LastIndSeq:0x%x. drop\n",
					Sequence, pBAEntry->LastIndSeq));
#endif /* RELEASE_EXCLUDE */
		
		pBAEntry->nDropPacket++;
#ifdef TRACELOG_IPARP
	TRACE_IPARP_RX_CHK_PNT(pAd, RTMP_GET_PACKET_TRACEIPARP(pRxBlk->pRxPacket), P8, 0);
	...
	{
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Rcv old pHeader->Seq:0x%x, LastIndSeq:0x%x. drop\n" , Sequence, pBAEntry->LastIndSeq));
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%lu: Refresh. Seq=0x%x, #RxPkt=%d. LastIndSeq=0x%x.\n",
...
#ifdef MT_MAC
	if (pAd->chipCap.hif_type == HIF_MT) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): No need this for HIF_MT!\n", __FUNCTION__));
		return;
	}
#endif /* MT_MAC */
