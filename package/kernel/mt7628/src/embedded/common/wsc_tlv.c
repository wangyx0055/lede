	...
	IN  PWSC_CTRL           pWscControl)
{
		...
		switch (WscType)
		{
			...
			case WSC_ID_IPV4_SUBMASK:
				...
				if (pProfile->Profile[CurrentIdx].Ipv4SubMask!= IWSC_DEFAULT_IPV4_SUBMASK)
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WscProcessCredential --> different IPv4 subnet mask (=0x%08x)\n", 
			...
			case WSC_ID_IPV4_SUBMASK_LIST:
					...
					((WscLen%4) != 0))
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WscProcessCredential --> WscLen = %d, set AvaSubMaskListCount to be 0\n", WscLen));
				}
				else
				{
			...
			default:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WscProcessCredential --> Unknown IE 0x%04x\n", WscType));
				break;
		}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WscProcessCredential --> %d profile retrieved from credential\n", Cnt));
	return TRUE;
}
    ...
    unsigned char *key, unsigned int keyBits )
{
    ...
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("WscDeriveKey: Deriving a key of %d bits\n", keyBits));
    ...
    {
#ifdef RELEASE_EXCLUDE
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("WscDeriveKey: Key derivation generated less bits "
                              "than asked\n"));
#endif /* RELEASE_EXCLUDE */
	...
	UCHAR *pPSK1)
{
	...
	if (pTempPsk == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s - pTempPsk alloc failed.", __FUNCTION__));
		return;
	}
		...
		if (pTempPasswd)
		{
			...
			if (pWscCtrl->NfcPasswdCaculate == 2)
			{
				for (idx = 0; idx < (pWscCtrl->NfcPasswdLen/2); idx++)
					sprintf((RTMP_STRING *) pTempPasswd, "%s%02X", pTempPasswd, pWscCtrl->NfcPasswd[idx]);
	...
	OUT	VOID *pbuf)
{
	...
	if ((CurOpMode == STA_MODE) &&
		(pAdapter->StaCfg.EasyConfigInfo.bRaAutoWpsAp) &&
		(!pWscControl->bWscTrigger))
	{
		/*WPS Vendor Extension */
		NdisMoveMemory(VendorExt, RALINK_OUI, 3);
		NdisMoveMemory(VendorExt + 3, pAdapter->StaCfg.EasyConfigInfo.WpsEncryptPIN, 12);
		templen = AppendWSCTLV(WSC_ID_VENDOR_EXT, pData, (UINT8 *)VendorExt, 15);
		pData += templen;
		Len   += templen;
	}
#endif /* STA_EASY_CONFIG_SETUP */
...
#ifdef DPA_T
	/* Vendor Extension */
	if ((pWscControl->WpsVendorExt[0].Length > 0) &&
		(pWscControl->WpsVendorExt[0].Length <= 1024))
	{
		templen = AppendWSCTLV(WSC_ID_VENDOR_EXT,
				pData,	
				(UINT8 *)pWscControl->WpsVendorExt[0].pData,
				pWscControl->WpsVendorExt[0].Length);
		pData += templen;
		Len   += templen;
	}
#endif /* DPA_T */
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BuildMessageM1 - bUPnPMsgTimerRunning = %d, pWscControl->WscUseUPnP = %d, pWscControl->EapMsgRunning = %d\n",
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BuildMessageM1.\n"));
	return Len;
}
	...
	OUT	VOID *pbuf)
{
    ...
    pWscControl->WscRetryCount = 0;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BuildMessageM2.\n"));
	return Len;
}
	...
	OUT	VOID *pbuf)
{
    ...
    pWscControl->WscRetryCount = 0;
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BuildMessageM2D.\n"));
	return Len;
}
	...
	OUT	VOID *pbuf)
{
	...
	*/
    ...
    pWscControl->WscRetryCount = 0;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BuildMessageM3 : \n"));
	return Len;
}
...
LabelErr:
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BuildMessageM4 : \n"));
	return Len;
}
	...
	OUT	VOID *pbuf)
{
	...
	*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BuildMessageM5 : \n"));
	return Len;
}
	...
	OUT	VOID *pbuf)
{
	...
	*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BuildMessageM6 : \n"));
	return Len;
}
	...
	OUT	VOID *pbuf)
{
	...
	*/
    ...
    pWscControl->WscRetryCount = 0;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BuildMessageM7 : \n"));
	return Len;
}
...
/*	UCHAR				TB[256]; */
	...
	/*
		...
		If STA is WSC 1.0, re-assign authType and encyType.
	*/
#if 0 /* Some Win8 WSC 2.0 STA cannot handle mixed AuthType & EncrType @2014/01/15 */
	if (pWscControl->RegData.PeerInfo.Version2 == 0)
#endif
	{
		if (AuthType == (WSC_AUTHTYPE_WPAPSK | WSC_AUTHTYPE_WPA2PSK))
...
LabelErr:
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BuildMessageM8 : \n"));
	return Len;
}
	...
	OUT	VOID *pbuf)
{
    ...
    pWscControl->WscRetryCount = 0;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BuildMessageDONE : \n"));
	return Len;
}
	...
	OUT	VOID *pbuf)
{
    ...
    pWscControl->WscRetryCount = 0;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BuildMessageACK : \n"));
	return Len;
}
	...
	OUT	VOID *pbuf)
{
    ...
    pWscControl->WscRetryCount = 0;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("BuildMessageNACK : \n"));
	return Len;
}
...
//#endif /* CONFIG_AP_SUPPORT */
		...
		switch (WscType)
		{
			case WSC_ID_VERSION:
				if(pReg->SelfInfo.Version != *pData)
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Rx WPS Message Version mismatched %02x\n",*pData));
				FieldCheck[(WSC_TLV_BYTE2(WSC_ID_VERSION))] ^= (1 << WSC_TLV_BYTE1(WSC_ID_VERSION));
				break;
				
			case WSC_ID_MSG_TYPE:
				if(WSC_ID_MESSAGE_M1 != *pData)
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Rx WPS Message Type mismatched %02x\n",*pData));
				FieldCheck[(WSC_TLV_BYTE2(WSC_ID_MSG_TYPE))] ^= (1 << WSC_TLV_BYTE1(WSC_ID_MSG_TYPE));
				break;
			...
			case WSC_ID_DEVICE_PWD_ID:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("   WPS Registrar DPID %04x\n",pReg->SelfInfo.DevPwdId));
				if(WSC_DEVICEPWDID_DEFAULT == get_unaligned((PUSHORT) pData))/**(PUSHORT) pData) */
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Rx WPS           DPID PIN\n"));
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Rx WPS           DPID PBC\n"));
					pWscControl->RegData.SelfInfo.DevPwdId = cpu2be16(DEV_PASS_ID_PBC);
				}
				else
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Rx WPS           DPID unsupport\n"));
				}
				FieldCheck[(WSC_TLV_BYTE2(WSC_ID_DEVICE_PWD_ID))] ^= (1 << WSC_TLV_BYTE1(WSC_ID_DEVICE_PWD_ID));
				break;
			...
			case WSC_ID_VENDOR_EXT:
				...
				if (pWscControl->WscV2Info.bEnableWpsV2)
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM1 --> Version2 = %x\n", pReg->PeerInfo.Version2));
				}
#endif // WSC_V2_SUPPORT //
				break;

			default:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM1 --> Unknown IE 0x%04x\n", WscType));
				break;				
		}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM1 : \n"));
	return ret;
}
	...
	OUT	PWSC_REG_DATA 	pReg)
{
	...
	if (pWscControl->bTriggerByNFC && pWscControl->NfcModel == MODEL_HANDOVER)
	{
		...
		if(NULL == IV_DecrData)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Allocate IV_DecrData fail @ %s\n", __FUNCTION__));
			return ret;
		}
	}
#endif /* WSC_NFC_SUPPORT */
	...
	while (Length > 4)
	{
		...
		switch (WscType)
		{
			...
			case WSC_ID_ENROLLEE_NONCE:
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Rx M2 Compare enrollee nonce mismatched \n"));
				}
				FieldCheck[(WSC_TLV_BYTE2(WSC_ID_ENROLLEE_NONCE))] ^= (1 << WSC_TLV_BYTE1(WSC_ID_ENROLLEE_NONCE));
				break;
			...
			case WSC_ID_PUBLIC_KEY:
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("NfcModel(=%d) \n",  pWscControl->NfcModel));
					UCHAR HashData[SHA256_DIGEST_SIZE];
			...
			case WSC_ID_DEVICE_PWD_ID:
#if 0
				/*if(pReg->SelfInfo.DevPwdId != *(PUSHORT) pData)
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ProcessMessageM2 DPID R%d E%d\n",*(PUSHORT) pData ,pReg->EnrolleeInfo.DevPwdId));
			...
			case WSC_ID_VENDOR_EXT:
				...
				if (pWscControl->WscV2Info.bEnableWpsV2)
				{
					UCHAR tmp_data_len = 0;
					WscParseV2SubItem(WFA_EXT_ID_VERSION2, pData, WscLen, &pReg->PeerInfo.Version2, &tmp_data_len);
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM2 --> Version2 = %x\n", pReg->PeerInfo.Version2));
				}
#endif // WSC_V2_SUPPORT //
				break;
#ifdef WSC_NFC_SUPPORT
			case WSC_ID_ENCR_SETTINGS:
				...
				if (pWscControl->bTriggerByNFC && pWscControl->NfcModel == MODEL_HANDOVER)
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WSC_ID_ENCR_SETTINGS at %s\n", __FUNCTION__));
					if (WscLen <= 16)
						break;
					if (WscLen > 1024)
					{
						// ApEncrSetting is not enough
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ApEncrSettings array size is not enough, require %d\n", WscLen));
						break;
					}
...
#endif /* WSC_NFC_SUPPORT */
			default:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM2 --> Unknown IE 0x%04x\n", WscType));
				break;				
		}
	...
	if (pWscControl->bTriggerByNFC && bWscEncrSettings && pWscControl->NfcModel == MODEL_HANDOVER)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("M2 ApEncrSettings len = %d\n ", EncrLen));
...
/* out : */
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM2 : \n"));
	return ret;
}
	...
	OUT	PWSC_REG_DATA pReg)
{
	...
	while (Length > 4)
	{
		...
		switch (WscType)
		{
			...
			case WSC_ID_PUBLIC_KEY:
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM2D --> Receive WSC_ID_PUBLIC_KEY!! werid!\n"));
				break;
			...
			default:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM2D --> Unknown IE 0x%04x\n", WscType));
				break;
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM2D : \n"));
	return ret;
}
	...
	OUT	PWSC_REG_DATA pReg)
{
		...
		switch (WscType)
		{
			...
			case WSC_ID_REGISTRAR_NONCE:
				...
				if (RTMPCompareMemory(pReg->RegistrarNonce, pData, WscLen) != 0)
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Rx M3 Compare Registrar nonce mismatched \n"));
				FieldCheck[(WSC_TLV_BYTE2(WSC_ID_REGISTRAR_NONCE))] ^= (1 << WSC_TLV_BYTE1(WSC_ID_REGISTRAR_NONCE));
				break;
			...
			default:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM3 --> Unknown IE 0x%04x\n", WscType));
				break;				
		}
	...
	if( FieldCheck[0] || FieldCheck[1] || FieldCheck[2] || FieldCheck[3] || FieldCheck[4] || FieldCheck[5] || FieldCheck[6] )
		ret = WSC_ERROR_WANTING_FIELD;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM3 : \n"));
	return ret;
}
	...
	OUT	PWSC_REG_DATA pReg)
{
	...
	while (Length > 4)
	{
		...
		switch (WscType)
		{
			...
			case WSC_ID_ENROLLEE_NONCE:
				/* for verification with our enrollee nonce */
				if (RTMPCompareMemory(pReg->EnrolleeNonce, pData, WscLen) != 0)
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Rx M4 Compare enrollee nonce mismatched \n"));
				FieldCheck[(WSC_TLV_BYTE2(WSC_ID_ENROLLEE_NONCE))] ^= (1 << WSC_TLV_BYTE1(WSC_ID_ENROLLEE_NONCE));
				break;
			...
			case WSC_ID_ENCR_SETTINGS:
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("M4 ApEncrSettings len = %d\n ", EncrLen));
			...
			default:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM4 --> Unknown IE 0x%04x\n", WscType));
				break;
		}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM4 --> RHash1 not matched\n"));
		ret = WSC_ERROR_DEV_PWD_AUTH_FAIL;
		goto out;
	}
...
out :
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM4 : \n"));
	return ret;
}
	...
	OUT	PWSC_REG_DATA pReg)
{
	...
	while (Length > 4)
	{
		...
		switch (WscType)
		{
			...
			case WSC_ID_REGISTRAR_NONCE:
				/* for verification with our Registrar nonce */
				if (RTMPCompareMemory(pReg->RegistrarNonce, pData, WscLen) != 0)
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Rx M5 Compare Registrar nonce mismatched \n"));
				FieldCheck[(WSC_TLV_BYTE2(WSC_ID_REGISTRAR_NONCE))] ^= (1 << WSC_TLV_BYTE1(WSC_ID_REGISTRAR_NONCE));
				break;
			...
			case WSC_ID_ENCR_SETTINGS:
                ...
                EncrLen = sizeof(pReg->ApEncrSettings);
                AES_CBC_Decrypt(IV_DecrData + 16, (WscLen - 16),pReg->KeyWrapKey,sizeof(pReg->KeyWrapKey),IV_DecrData, 16, (UINT8 *) pReg->ApEncrSettings, (UINT *) &EncrLen);
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("M5 ApEncrSettings len = %d\n ", EncrLen));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM5 --> EHash1 not matched\n"));
...
out :
	if(IV_DecrData)
		os_free_mem(NULL, IV_DecrData);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM5 : \n"));
	return ret;
}
	...
	OUT	PWSC_REG_DATA pReg)
{
	...
	while (Length > 4)
	{
		...
		switch (WscType)
		{
			...
			case WSC_ID_ENROLLEE_NONCE:
				/* for verification with our enrollee nonce */
				if (RTMPCompareMemory(pReg->EnrolleeNonce, pData, WscLen) != 0)
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Rx M6 Compare enrollee nonce mismatched \n"));
				FieldCheck[(WSC_TLV_BYTE2(WSC_ID_ENROLLEE_NONCE))] ^= (1 << WSC_TLV_BYTE1(WSC_ID_ENROLLEE_NONCE));
				break;
			...
			case WSC_ID_ENCR_SETTINGS:
                ...
                EncrLen = sizeof(pReg->ApEncrSettings);
                AES_CBC_Decrypt(IV_DecrData + 16, (WscLen - 16),pReg->KeyWrapKey,sizeof(pReg->KeyWrapKey),IV_DecrData, 16, (UINT8 *) pReg->ApEncrSettings, (UINT *) &EncrLen); 
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("M6 ApEncrSettings len = %d\n ", EncrLen));
			...
			default:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM6 --> Unknown IE 0x%04x\n", WscType));
				break;
		}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM6 --> RHash2 not matched\n"));
        ret = WSC_ERROR_DEV_PWD_AUTH_FAIL;		
        goto out;
	}
...
out:
	if(IV_DecrData)
		os_free_mem(NULL, IV_DecrData);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM6 : \n"));
	return ret;
}
	...
	OUT	PWSC_REG_DATA pReg)
{
	...
	while (Length > 4)
	{
		...
		switch (WscType)
		{
...
#ifdef DPA_T
				if (pAdapter->StaCfg.bPriorityCtrl)
				{
					WscSortProfile(pAdapter, pWscControl);
				}
#endif /* DPA_T */
				break;
			...
			default:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM7 --> Unknown IE 0x%04x\n", WscType));
				break;
		}
	...
	if(IV_DecrData)
		os_free_mem(NULL, IV_DecrData);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM7 : \n"));
	return ret;
}
	...
	IN  PWSC_CTRL       pWscControl)
{
	...
	while (Length > 4)
	{
		...
		switch (WscType)
		{
			...
			case WSC_ID_ENROLLEE_NONCE:
				...
				if (RTMPCompareMemory(pReg->EnrolleeNonce, pData, WscLen) != 0)
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Rx M8 Compare enrollee nonce mismatched \n"));
				}
				break;
			...
			case WSC_ID_ENCR_SETTINGS:
                ...
                EncrLen = sizeof(pReg->ApEncrSettings);
                AES_CBC_Decrypt(IV_DecrData + 16, (WscLen - 16),pReg->KeyWrapKey,sizeof(pReg->KeyWrapKey),IV_DecrData, 16, (UINT8 *) pReg->ApEncrSettings, (UINT *) &EncrLen);                 
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("M8 ApEncrSettings len = %d\n ", EncrLen));
			...
			default:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM8 --> Unknown IE 0x%04x\n", WscType));
				break;
		}
	...
	if(IV_DecrData)
		os_free_mem(NULL, IV_DecrData);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ProcessMessageM8 : \n"));
	return ret;
}
