	...
	IN PVOID SystemSpecific3)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - enqueue MESH_CTRL_PLD to MESH_CTRL State-Machine.\n", __FUNCTION__));
	...
	IN PVOID SystemSpecific3)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - enqueue MESH_CTRL_MCS_TIMEOUT to MESH_CTRL State-Machine.\n", __FUNCTION__));
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	*pCurrState = MESH_CTRL_ACTIVATED;
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Get Join evt when Idle.\n", __FUNCTION__));
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Get Disconnect evt when Discovery.\n", __FUNCTION__));
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Get Scan-Finish evt when Discovery.\n", __FUNCTION__));
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Get Scan-Abort evt when Discovery.\n", __FUNCTION__));
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Get PLD evt when Activated.\n", __FUNCTION__));
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Get Disconnect evt when Activated.\n", __FUNCTION__));
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	*pCurrState = MESH_CTRL_UCG;
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Get UCG evt when Activated.\n", __FUNCTION__));
}
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Get Disconnect evt when UCG.\n", __FUNCTION__));
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Get Mcs evt when UCG.\n", __FUNCTION__));
	...
	IN PMESH_CONFIGURAION_IE pMeshConfig)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("pAd->MeshTab.pMeshNeighborTab equal NULL.\n"));
		return NULL;
	}
	...
	if (pNeighborEntry == NULL)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("pAd->MeshTab.pMeshNeighborTab FULL.\n"));
			return NULL;
		}
		...
		{
			pNeighborEntry = NULL;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: Neighbor tab full.\n", __FUNCTION__));
		}
	...
	IN PUCHAR pPeerMacAddr)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("pMeshNeighborTab empty.\n"));
		return;
	}
	...
	for (i = 0; i < MAX_NEIGHBOR_MP; i++)
	{
		...
		else
		{
			if (VALID_MESH_LINK_ID(pNeighborEntry->MeshLinkIdx))
			{
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Link%d:Neighbor ExtChOffset change from %d to %d , kill the link!\n"
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	else
	{
		...
		for (i = 0; i < pNeighborTab->NeighborNr; i++)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("(%d) CPI=%d Apl=%d channel=%d state=%d\n", i, pNeighborEntry->CPI,
				pNeighborEntry->MeshCapability.field.AcceptPeerLinks, pNeighborEntry->Channel, pNeighborEntry->State));

			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%d %d %d %d %d\n",
				...
				)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("(%d) pick LinkId=%ld\n", i, LinkIdx));
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (pAd->MeshTab.pMeshNeighborTab)
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s Fail to alloc memory for pAd->MeshTab.pMeshNeighborTab", __FUNCTION__));
	...
	while(i < MAC_ADDR_LEN)
	{
		if (pAd->MeshTab.wdev.if_addr[i] != pAd->MeshTab.MeshLink[idx].Entry.PeerMacAddr[i])
		{
			if (pAd->MeshTab.wdev.if_addr[i] > pAd->MeshTab.MeshLink[idx].Entry.PeerMacAddr[i])
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("The local MP is the Selector MP"));
			}
			else
			{
				pAd->MeshTab.MeshLink[idx].Entry.bValidLocalMpAsSelector = FALSE;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("The local MP is NOT the Selector MP"));	
			}
