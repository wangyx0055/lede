    ...
    OUT PUCHAR pAddr2) 
{
    ...
    {
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MlmeAddBAReqSanity fail - message lenght not correct.\n"));
        return FALSE;
    }	
	
    if ((pInfo->Wcid >= MAX_LEN_OF_MAC_TABLE))
    {
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MlmeAddBAReqSanity fail - The peer Mac is not associated yet.\n"));
        return FALSE;
    }	
    ...
    {
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MlmeAddBAReqSanity fail - Rx Reordering buffer too big or too small\n"));
        return FALSE;
    } 
	*/  
    ...
    {
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MlmeAddBAReqSanity fail - broadcast address not support BA\n"));
        return FALSE;
    }	
    ...
    IN ULONG MsgLen) 
{
    ...
    {
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("MlmeDelBAReqSanity fail - message lenght not correct.\n"));
        return FALSE;
    }	
	
    if ((pInfo->Wcid >= MAX_LEN_OF_MAC_TABLE))
    {
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("MlmeDelBAReqSanity fail - The peer Mac is not associated yet.\n"));
        return FALSE;
    }	

    if ((pInfo->TID & 0xf0))
    {
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("MlmeDelBAReqSanity fail - The peer TID is incorrect.\n"));
        return FALSE;
    }	

	if (NdisEqualMemory(pAd->MacTab.Content[pInfo->Wcid].Addr, pInfo->Addr, MAC_ADDR_LEN) == 0)
    {    	
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("MlmeDelBAReqSanity fail - the peer addr dosen't exist.\n"));		
        return FALSE;
    }	
	...
	OUT PUCHAR pAddr2)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("PeerAddBAReqActionSanity: ADDBA Request frame length size = %ld incorrect\n", MsgLen));
		return FALSE;
	}
	...
	*(USHORT *)(&pAddFrame->BaParm) = cpu2le16(*(USHORT *)(&pAddFrame->BaParm));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("PeerAddBAReqActionSanity: ADDBA Request Ba Policy[%d] not support\n", pAddFrame->BaParm.BAPolicy));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("ADDBA Request. tid=%x, Bufsize=%x, AMSDUSupported=%x \n", pAddFrame->BaParm.TID, pAddFrame->BaParm.BufSize, pAddFrame->BaParm.AMSDUSupported));
		return FALSE;
	}
    ...
    IN ULONG MsgLen)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s(): ADDBA Resp frame length incorrect(len=%ld)\n", __FUNCTION__, MsgLen));
		return FALSE;
	}
	/* we support immediate BA.*/
#ifdef UNALIGNMENT_SUPPORT
	...
	*(USHORT *)(&pAddFrame->BaParm) = cpu2le16(*(USHORT *)(&pAddFrame->BaParm));
	...
	if (pAddFrame->BaParm.BAPolicy != IMMED_BA)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s(): ADDBA Resp Ba Policy[%d] not support\n", __FUNCTION__, pAddFrame->BaParm.BAPolicy));
		return FALSE;
	}
...
/*	hex_dump("Beacon", Msg, MsgLen);*/
    ...
    while ((Length + 2 + pEid->Len) <= MsgLen)    
    {
        ...
        {
            MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s() - Variable IEs out of resource [len(=%d) > MAX_VIE_LEN(=%d)]\n",
        ...
        switch(pEid->Eid)
        {
            case IE_SSID:
                ...
                {
                    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s() - wrong IE_SSID (len=%d)\n", __FUNCTION__, pEid->Len));
                    goto SanityCheck;
                }
                break;
            ...
            case IE_SUPP_RATES:
                ...
                {
                    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s() - wrong IE_SUPP_RATES (len=%d)\n",__FUNCTION__, pEid->Len));
                    goto SanityCheck;
                }
                break;
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s() - wrong IE_HT_CAP. pEid->Len = %d\n", __FUNCTION__, pEid->Len));
			}
		...
		break;
            case IE_FH_PARM:
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(IE_FH_PARM) \n", __FUNCTION__));
                break;
            ...
            case IE_DS_PARM:
                    ...
                    *pChannel = *pEid->Octet;
					...
					IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
					{
						if (ChannelSanity(pAd, *pChannel) == 0)
						{
							...
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s() - wrong IE_DS_PARM (ch=%d)\n",__FUNCTION__,*pChannel));
                ...
                {
                    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s() - wrong IE_DS_PARM (len=%d)\n",__FUNCTION__,pEid->Len));
                    goto SanityCheck;
                }
                break;
            ...
            case IE_CF_PARM:
                ...
                {
                    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s() - wrong IE_CF_PARM\n", __FUNCTION__));
					if (pPeerWscIe)
            ...
            case IE_IBSS_PARM:
                ...
                {
                    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s() - wrong IE_IBSS_PARM\n", __FUNCTION__));
					if (pPeerWscIe)
						os_free_mem(NULL, pPeerWscIe);
                    return FALSE;
                }
            ...
            case IE_TIM:
                ...
                {
#ifdef P2P_SUPPORT
                    if (P2P_CLI_ON(pAd) && NdisEqualMemory(pBssid, pAd->ApCfg.ApCliTab[0].MlmeAux.Bssid, MAC_ADDR_LEN))
                    {
                        GetTimBit((PCHAR)pEid, pAd->ApCfg.ApCliTab[0].MlmeAux.Aid, &TimLen, pBcastFlag, pDtimCount, pDtimPeriod, pMessageToMe);
                    }
#endif /* P2P_SUPPORT */
...
#ifdef WIDI_SUPPORT
#ifdef CONFIG_STA_SUPPORT
                	if(pAd->StaCfg.bWIDI && SubType == SUBTYPE_BEACON)
					{
...
#ifdef RELEASE_EXCLUDE
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s() - Receive IE_WPA\n", __FUNCTION__));
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("IE_RSN length = %d\n", pEid->Len));
#endif /* RELEASE_EXCLUDE */
                break;
#ifdef WAPI_SUPPORT
...
#ifdef RELEASE_EXCLUDE
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("IE_WAPI length = %d\n", pEid->Len));
#endif /* RELEASE_EXCLUDE */
                break;
#endif /* WAPI_SUPPORT */
...
/*                  pQosCapability->bMoreDataAck    = pEid->Octet[0] & 0x08;*/
...
#ifdef RELEASE_EXCLUDE
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s() - IE_EXT_CAPABILITY(%d)!\n", __FUNCTION__, pEid->Len));
#endif /* RELEASE_EXCLUDE */
					break;
				}
#endif
...
#ifdef RELEASE_EXCLUDE
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("PeerBeaconAndProbeRspSanity - IE_EXT_CAPABILITY(%d)!\n", pEid->Len));
#endif // RELEASE_EXCLUDE //
				}
				break;
            default:
#ifdef RELEASE_EXCLUDE
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s() - unrecognized EID = %d\n", __FUNCTION__, pEid->Eid));
#endif /* RELEASE_EXCLUDE */ 
                break;
        }
	...
	/*
		...
		Only consider 5G case  because that if the AP does not carry both IE_DS_PARM and IE_ADD_HT in 2.4G,
		we will receive beacon from several adjecent channels and will set a wrong Channel info in BBS table more easily.
	*/
#endif /* RELEASE_EXCLUDE */
	LatchRfChannel = MsgChannel;
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
		{
			if ((Sanity & 0x4) && (CtrlChannel != 0) && (CtrlChannel != *pChannel))
			{
				AddHtInfo->ControlChan = *pChannel;
			}

			if(RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS) &&
				(Sanity & 0x4) && (CtrlChannel != 0) &&(CtrlChannel != pAd->LatchRfRegs.Channel) )
			{
				if (pPeerWscIe)
					os_free_mem(NULL, pPeerWscIe);
				return FALSE;
		...
		if (pPeerWscIe && (PeerWscIeLen > 0) && (PeerWscIeLen <= 512) && ( bWscCheck == TRUE))
		{
			...
			*LengthVIE += (PeerWscIeLen + 6);
				...
				(bFoundIWscIe == TRUE))
			{
				...
				bFoundIWscIe = FALSE;
#if 0
				hex_dump("PeerBeaconAndProbeRspSanity - IWSC IE", pPeerWscIe, PeerWscIeLen);
#endif
				WscGetDataFromPeerByTag(pAd, pPeerWscIe, PeerWscIeLen, WSC_ID_SEL_REGISTRAR, &bSelReg, NULL);				
				if (bSelReg)
				{
...
SanityCheck:
	...
	{
#ifdef WIDI_SUPPORT
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s Sanity Failed\n", __FUNCTION__));
#endif /* WIDI_SUPPORT */
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_LOUD, ("%s() - missing field, Sanity=0x%02x\n", __FUNCTION__, Sanity));
		return FALSE;
	}
	else
	{
	...
	*LengthVIE = 0; /* Set the length of VIE to init value 0*/
        ...
        switch(pEid->Eid)
	{
		...
		case IE_TIM:
			...
			{
#ifdef P2P_SUPPORT
				if (P2P_CLI_ON(pAd) && NdisEqualMemory(&ie_list->Bssid[0], pAd->ApCfg.ApCliTab[0].MlmeAux.Bssid, MAC_ADDR_LEN))
				{
...
#endif /* P2P_SUPPORT */
				if ((INFRA_ON(pAd) && NdisEqualMemory(&ie_list->Bssid[0], pAd->CommonCfg.Bssid, MAC_ADDR_LEN)) || bGetDtim)
				{
			...
			)
			{
				...
				*/
				...
				else
				{
					bWscCheck = FALSE;
					if (pEid->Len <= 4)
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Error!!! Incorrect WPS IE!\n", __FUNCTION__));
					if (pPeerWscIe == NULL)
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: Error!!! pPeerWscIe is null!\n", __FUNCTION__));
				}
		...
		case IE_EDCA_PARAMETER:
			if (pEid->Len == 18)
			{
				...
				int i;
				ie_list->EdcaParm->bValid          = TRUE;
				ie_list->EdcaParm->bQAck           = pEid->Octet[0] & 0x10;
				ie_list->EdcaParm->bQueueRequest   = pEid->Octet[0] & 0x20;
				ie_list->EdcaParm->bTxopRequest    = pEid->Octet[0] & 0x40;
				/* ie_list->EdcaParm->bMoreDataAck    = pEid->Octet[0] & 0x80;*/
				ie_list->EdcaParm->EdcaUpdateCount = pEid->Octet[0] & 0x0f;
				ptr = &pEid->Octet[2];
				for (i=0; i<4; i++)
				{
					UCHAR aci = (*ptr & 0x60) >> 5; /* b5~6 is AC INDEX*/
		...
		case IE_OPERATING_MODE_NOTIFY:
			if (pEid->Len == sizeof(OPERATING_MODE)) {
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s() - IE_OPERATING_MODE_NOTIFY(=%d)\n", __FUNCTION__, pEid->Eid));
			}
#endif /* DOT11_VHT_AC */
	...
	{	
		switch(pEid->Eid)
		{
			case IE_SUPP_REG_CLASS:
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PeerBeaconAndProbeRspSanity - wrong IE_SUPP_REG_CLASS (len=%d)\n",pEid->Len));
				}
				break;
			case IE_OVERLAPBSS_SCAN_PARM:
				...
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PeerBeaconAndProbeRspSanity - wrong IE_OVERLAPBSS_SCAN_PARM (len=%d)\n",pEid->Len));
				}
				break;

			case IE_EXT_CHANNEL_SWITCH_ANNOUNCEMENT:
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PeerBeaconAndProbeRspSanity - IE_EXT_CHANNEL_SWITCH_ANNOUNCEMENT\n"));
				break;
	...
	OUT UCHAR *pScanType) 
{
	...
	*pScanType = Info->ScanType;
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MlmeScanReqSanity fail - wrong BssType or ScanType\n"));
		return FALSE;
	}
}
#endif
    ...
    CHAR *pChlgText) 
{
    ...
    if (*pAlg == AUTH_MODE_OPEN)
    {
        ...
        {
            MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PeerAuthSanity fail - wrong Seg#\n"));
            return FALSE;
        }
    } 
    ...
    {
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PeerAuthSanity fail - wrong algorithm\n"));
        return FALSE;
    }
}
    ...
    OUT USHORT *pAlg) 
{
    ...
    *pAlg = pInfo->Alg;
    ...
    {
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MlmeAuthReqSanity fail - wrong algorithm\n"));
        return FALSE;
    }
}
		...
		switch(eid_ptr->Eid)
		{
			case IE_SUPP_RATES:
                if ((eid_ptr->Len <= MAX_LEN_OF_SUPPORTED_RATES) && (eid_ptr->Len > 0))
                {
                    ...
                    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PeerDlsReqSanity - IE_SUPP_RATES., Len=%d. Rates[0]=%x\n",eid_ptr->Len, Rates[0]));
                    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Rates[1]=%x %x %x %x %x %x %x\n", Rates[1], Rates[2], Rates[3], Rates[4], Rates[5], Rates[6], Rates[7]));
                ...
                else
                {
                    *pRatesLen = 8;
					...
					Rates[6] = 0x48;
					Rates[7] = 0x6c;
                    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PeerDlsReqSanity - wrong IE_SUPP_RATES., Len=%d\n",eid_ptr->Len));
                }
				break;
			...
			case IE_HT_CAP:
				if (eid_ptr->Len >= sizeof(HT_CAPABILITY_IE))
				{
					...
					*pHtCapabilityLen = sizeof(HT_CAPABILITY_IE);
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PeerDlsReqSanity - IE_HT_CAP\n"));
				}
				else
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PeerDlsReqSanity - wrong IE_HT_CAP.eid_ptr->Len = %d\n", eid_ptr->Len));
				}
				break;
	...
	while (((UCHAR*)eid_ptr + eid_ptr->Len + 1) < ((UCHAR*)Fr + MsgLen))
	{
		switch(eid_ptr->Eid)
		{
			case IE_SUPP_RATES:
                ...
                {
                    NdisMoveMemory(Rates, eid_ptr->Octet, eid_ptr->Len);
                    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PeerDlsRspSanity - IE_SUPP_RATES., Len=%d. Rates[0]=%x\n",eid_ptr->Len, Rates[0]));
                    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Rates[1]=%x %x %x %x %x %x %x\n", Rates[1], Rates[2], Rates[3], Rates[4], Rates[5], Rates[6], Rates[7]));
                    *pRatesLen = eid_ptr->Len;
                }
                else
                {
                    *pRatesLen = 8;
					...
					Rates[6] = 0x48;
					Rates[7] = 0x6c;
                    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PeerDlsRspSanity - wrong IE_SUPP_RATES., Len=%d\n",eid_ptr->Len));
                }
				break;
			...
			case IE_HT_CAP:
				if (eid_ptr->Len >= sizeof(HT_CAPABILITY_IE))
				{
					...
					*pHtCapabilityLen = sizeof(HT_CAPABILITY_IE);
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PeerDlsRspSanity - IE_HT_CAP\n"));
				}
				else
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PeerDlsRspSanity - wrong IE_HT_CAP.eid_ptr->Len = %d\n", eid_ptr->Len));
				}
				break;
	...
	OUT PEER_PROBE_REQ_PARAM *ProbeReqParam)
{
    ...
    {
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): sanity fail - wrong SSID IE\n", __FUNCTION__));
        return FALSE;
    } 
        ...
        switch(eid)
        {
	        case IE_VENDOR_SPECIFIC:
					...
					)
                {
					...
					*/
					if (pAd->ApCfg.MBSSID[MAIN_MBSSID].bHideSsid)
						AutoProvisionAssignSSID(pAd, eid_data, eid_len, pAddr2, MAIN_MBSSID, SsidLen, Ssid);
					...
					if (pEntry == NULL)
						pEntry = MacTableInsertEntry(pAd, pAddr2, wdev, ENTRY_AP, OPMODE_STA, FALSE);

					if (pEntry)
					{
						pEntry->bRaAutoWpsCapable = FALSE;
						if (WpsMICIntegrity(pAd, eid_data, (INT)eid_len, pEntry->Addr, pEntry->func_tb_idx))
						{
							BOOLEAN Cancel;							
							if (!OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED))
							{
								RTMPCancelTimer(&pAd->StaCfg.EasyConfigInfo.AdhocConnTimer, &Cancel);
								RTMPSetTimer(&pAd->StaCfg.EasyConfigInfo.AdhocConnTimer, ADHOC_CONNECTION_TIMEOUT);
							}
							pEntry->bRaAutoWpsCapable = TRUE;							
							pAd->StaCfg.WscControl.WscState = WSC_STATE_LINK_UP;
							NdisZeroMemory(pAd->StaCfg.WscControl.WscPeerMAC, MAC_ADDR_LEN);
							NdisMoveMemory(pAd->StaCfg.WscControl.WscPeerMAC, pAddr2, MAC_ADDR_LEN);
						}
						else
							MacTableDeleteEntry(pAd, pEntry->wcid, pEntry->Addr);
					}
					else
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("MacTable Insert Fail!\n"));
#endif /* CONFIG_STA_SUPPORT */
