    ...
    IN ULONG            DataByteCount)
{
    ...
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("RTMPCheckWAIframe ===> \n"));
...
{	
	...
	if (RTMPGetKeyParameter("WapiPreAuthifname", tmpbuf, 32, buffer, TRUE))
	{
		if (strlen(tmpbuf) > 0)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WapiPreAuthifname=%s, len=%d\n", 
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
	{
		...
		{											                 										
		    ...
		    {                                    
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WapiPsk=(%s), len=%d\n", tmpbuf, strlen(tmpbuf)));						
		    }
			else
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("The length of WAPI PSKPassPhrase is invalid(len=%d). \n", strlen(tmpbuf)));
			}			      						 
		}
	}
#endif /* CONFIG_STA_SUPPORT */													
	...
	{		
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
		{
			...
			if(simple_strtol(tmpbuf, 0, 10) == 0)  
			{
				...
				{
					err = 1;
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("The WAPI-PSK key length MUST be even in Hex mode\n"));						
				}
			}
			/* ASCII */
			else 
			...
			if (err)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WapiPskType=%s\n", (pAd->StaCfg.WapiPskType == HEX_MODE) ? "HEX" : "ASCII"));
		}
#endif /* CONFIG_STA_SUPPORT */
	...
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RTMPIoctlQueryWapiConf==>\n"));
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("RTMPWapiUskRekeyPeriodicExec ==>> \n"));
#endif /* RELEASE_EXCLUDE */        
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("RTMPWapiMskRekeyPeriodicExec ==>> \n"));
#endif /* RELEASE_EXCLUDE */        
	...
	IN PMAC_TABLE_ENTRY	pEntry)
{
	if (pEntry)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" RTMPInitWapiRekeyTimerAction : WAPI USK rekey timer (wcid-%d) \n", pEntry->wcid));
	...
	if (pEntry)
	{
			...
			(pAd->CommonCfg.wapi_usk_rekey_threshold > 0))
		{
			...
			if (pEntry->WapiUskRekeyTimerRunning == FALSE)
			{
				...
				pEntry->wapi_usk_rekey_cnt = 0;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" RTMPStartWapiRekeyTimerAction : WAPI USK rekey timer is started (%d) \n", pAd->CommonCfg.wapi_usk_rekey_threshold));
			}							
		}
	}
	else
	{
			...
			(pAd->CommonCfg.wapi_msk_rekey_threshold > 0))
		{
			...
			if (pAd->CommonCfg.WapiMskRekeyTimerRunning == FALSE)
			{
				...
				pAd->CommonCfg.wapi_msk_rekey_cnt = 0;
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" RTMPStartWapiRekeyTimerAction : WAPI MSK rekey timer is started \n"));				
			}
		}
	}
		...
		IN	UCHAR				flag)
{
	...
	{				
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Trigger WAPI for this sta(%02x:%02x:%02x:%02x:%02x:%02x)\n", PRINT_MAC(pAddr)));
	...
	OUT	UCHAR			*tx_tsc)
{
	...
	if (IS_HW_WAPI_SUPPORT(pAd))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s : WCID(%d) ", __FUNCTION__, Wcid));			
