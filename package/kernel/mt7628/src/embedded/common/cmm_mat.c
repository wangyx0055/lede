...
VOID dumpPkt(PUCHAR pHeader, int len)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("--StartDump\n"));
	for(i=0;i<len; i++)
	{
		if ( (i%16==0) && (i!=0))
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%02x ", tmp[i]& 0xff));
	}
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\n--EndDump\n"));
	...
	IN UCHAR    OpMode)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("%s(): protoType=0x%04x\n", __FUNCTION__, protoType));
	...
	for (i=0; i < MAX_MAT_SUPPORT_PROTO_NUM; i++)
	{
		if (protoType == MATProtoTb[i].protocol)
		{
			...
			IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
			{
				...
				if (tempIdx >= 64)
				{
					CliIdx = ((tempIdx - 64) % MAX_EXT_MAC_ADDR_SIZE);
					tempIdx = ((tempIdx - 64) / MAX_EXT_MAC_ADDR_SIZE);
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	for (i = 0; i < MAX_MAT_SUPPORT_PROTO_NUM; i++)
	{
			...
			if (pHandle->init != NULL)
		{
			...
			if (status == FALSE) 
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("MATEngine Init Protocol (0x%x) failed, Stop the MAT Funciton initialization failed!\n", MATProtoTb[i].protocol));
				goto init_failed;
			}
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MATEngine Init Protocol (0x%04x) success!\n", MATProtoTb[i].protocol));
		}
	}
