			...
			{							
				...
				if (bSameAP == FALSE)
				{
					...
					for(idx=0; idx<16; idx++)
						DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_ERROR, ("%02x  ", ApUuidBssid[index].Uuid[idx]));
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\n"));
...
if (CurOpMode == AP_MODE)
	...
	/* allocate memory */
	os_alloc_mem(NULL, (UCHAR **)&ApUuidBssidList, sizeof(UUID_BSSID_CH_INFO_LIST));
	...
	for (i = 0; i < pAd->ScanTab.BssNr; i++) 
	{
		...
		RootApChannel = pAd->ScanTab.BssEntry[i].Channel;
...
VOID WscWriteConfToDatFile(RTMP_ADAPTER *pAd, UCHAR CurOpMode)
{
	...
	#define CFG_WLN_SSID1 			0x43C0200
	#define CFG_WLN_AuthMode 		0x4960200
	#define CFG_WLN_EncrypType 		0x4490200
	#define CFG_WLN_WscConfMode 	0x4860100
	#define CFG_WLN_WscConfStatus	0x4870100
	#define CFG_WLN_DefaultKeyID	0x43A0200
	#define CFG_WLN_WPAPSK1 		0x49A0200
	#define CFG_WLN_WPAPSK2 		0x4B80200
	#define CFG_WLN_WPAPSK3 		0x4B90200
	#define CFG_WLN_WPAPSK4 		0x4BA0200
	#define CFG_WLN_WscSSID		0x48D0200
	#define CFG_WLN_WscNewKey		0x4920200
	#define CFG_WLN_WscAuthType	0x4900100
	#define CFG_WLN_WscEncrypType	0x4910100
	...
	if (CurOpMode == AP_MODE)
	{
		...
		snprintf((RTMP_STRING *) WepKeyName, sizeof(WepKeyName), "Key%dStr%d=", pAd->ApCfg.MBSSID[apidx].DefaultKeyId+1, apidx+1);
		snprintf((RTMP_STRING *) WepKeyFormatName, sizeof(WepKeyFormatName), "Key%dType=", pAd->ApCfg.MBSSID[apidx].DefaultKeyId+1);
	}
#endif /* CONFIG_AP_SUPPORT */
	...
	else 
	{
		...
		os_alloc_mem(pAd, (UCHAR **)&cfgData, fileLen));
		if (cfgData == NULL)
		{
			RtmpOSFileClose(file_r);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("mem alloc fail. (fileLen = %ld)\n", fileLen));
			goto out;
		}
	...
	RTMP_STRING *profileTemp = NULL;
	RTMP_STRING *profileTemp2 = NULL;
	RTMP_STRING *profileTemp3 = NULL;
	
	// TODO: shiang-7603, memory leak here!!
	os_alloc_mem(pAd, (UCHAR **)&profileTemp, 255));
	os_alloc_mem(pAd, (UCHAR **)&profileTemp2, 255));
	os_alloc_mem(pAd, (UCHAR **)&profileTemp3, 255));
	if (!profileTemp)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("profileTemp mem alloc fail. (255)\n"));
		goto WriteErr;
	}
	else
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("profileTemp mem alloc succeed. (255)\n"));

#ifdef CONFIG_AP_SUPPORT
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CFG_WLN_SSID1=%s\n", pAd->ApCfg.MBSSID[0].Ssid));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CFG_WLN_WscSSID=%s\n", pAd->ApCfg.MBSSID[0].Ssid));
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		for (index = 0; index < pAd->ApCfg.BssidNum; index++)
		{
			if (pAd->ApCfg.MBSSID[index].SsidLen)
			{
				...
				profileTemp2 = RTMPGetRalinkAuthModeStr(pAd->ApCfg.MBSSID[index].AuthMode);
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("pAd->ApCfg.MBSSID[%d].AuthMode=%s\n", index, profileTemp2));
				if (index == 0)
				{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CFG_WLN_AuthMode=%s\n", profileTemp));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CFG_WLN_WscAuthType=%d\n", WscAuthTypeTemp));
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		for (index = 0; index < pAd->ApCfg.BssidNum; index++)
		{
			profileTemp2 = RTMPGetRalinkEncryModeStr(pAd->ApCfg.MBSSID[index].WepStatus);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("pAd->ApCfg.MBSSID[%d].WepStatus=%s\n", index, profileTemp2));
			if (index == 0)
			{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CFG_WLN_EncrypType=%s\n", profileTemp));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CFG_WLN_WscEncrypType=%d\n", WscEncrypTypeTemp));
	...
	profileTemp3[64] = '\0';
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CFG_WLN_WPAPSK1=%s\n", &pWscControl->WpaPsk[0]));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CFG_WLN_WscNewKey=%s\n", &pWscControl->WpaPsk[0]));
	...
	pWscControl = &pAd->ApCfg.MBSSID[0].WscControl;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CFG_WLN_WscConfMode=%d\n", pWscControl->WscConfMode));
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CFG_WLN_WscConfStatus=%d\n", WscConfStatusTemp));
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		for (index = 0; index < pAd->ApCfg.BssidNum; index++)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("pAd->ApCfg.MBSSID[%d].DefaultKeyId=%d\n", index, pAd->ApCfg.MBSSID[index].DefaultKeyId+1));
			if (index == 0)
				snprintf(profileTemp, 255, "%d", pAd->ApCfg.MBSSID[index].DefaultKeyId+1);
			else
				snprintf(profileTemp, 255, "%s;%d", profileTemp, pAd->ApCfg.MBSSID[index].DefaultKeyId+1);
		}
	}
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CFG_WLN_DefaultKeyID=%s\n", profileTemp));
