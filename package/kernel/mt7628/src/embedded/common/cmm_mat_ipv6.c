...
/*
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): Good IPv6 unicast addr=%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n",
	...
	IN int index)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s():IPv6MacTable not init yet, so cannot do dump!\n", __FUNCTION__));
		return FALSE;
	}
	...
	for(; startIdx<= endIdx; startIdx++)
	{
		...
		while(pHead)
		{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("IPv6Mac[%d]:\n", startIdx));
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t:IPv6=%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x,Mac=%02x:%02x:%02x:%02x:%02x:%02x, lastTime=0x%lx, next=%p\n", 
	...
	IN PCHAR			pIPv6Addr)
{
	...
	while(pEntry)
	{
		...
		if (NdisEqualMemory(pIPv6Addr, pEntry->ipv6Addr, IPV6_ADDR_LEN))
    	{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Got Mac(%02x:%02x:%02x:%02x:%02x:%02x) of mapped IPv6(%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x)\n",
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IPv6MacTableUpdate():Insertion failed!\n"));
...
/*	spin_lock_irqsave(&IPMacTabLock, irqFlag); */
	...
	while(pEntry)
	{
        ...
        {
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): dstMac=%02x:%02x:%02x:%02x:%02x:%02x for mapped dstIPv6(%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x)\n", 
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): Get the dstIP=%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n", 
	...
	IN UINT32			offset)
{
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("A ICMPv6 packet! offset=%d! MsgType=0x%x! ICMPMsgLEn=%d!\n", 
	...
	switch (pICMPv6Hdr->type)
	{
...
#ifdef RELEASE_EXCLUDE
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("SRC_LL_ADDR, modify=TRUE, offset=%d!\n", offset));
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("TTGT_LL_ADDR, modify=TRUE, offset=%d!\n", offset));
#endif /* RELEASE_EXCLUDE */
		...
		default:
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Un-supported ICMPv6 msg type(0x%x)! Ignore it\n", pICMPv6Hdr->type));
			break;
	}
	...
	IN PUCHAR			pDevMacAdr)
{
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): Transmit a IPv6 Pkt=>SrcIP=%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x!\n", 
	...
	offset = IPV6_HDR_LEN;
	/*MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("NextProtocol=0x%x! payloadLen=%d! offset=%d!\n", nextProtocol, payloadLen, offset)); */
		  ...
		  nextProtocol != IPV6_NEXT_HEADER_NONE)
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("IPv6ExtHdrHandle failed!\n"));
			break;
		}
	}
	...
	IN MAT_STRUCT *pMatCfg)
{
	...
	else
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IPMacTable_init(): Allocate memory for IPv6MacTable failed!\n"));
			return FALSE;
		}
	}
	...
	if (!pEntry)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IPMacTable_init(): Allocate memory for IPMacTable broadcast entry failed!\n"));
		return FALSE;
		}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): IPv6MacTable already inited!\n", __FUNCTION__));
	}
...
typedef struct GNU_PACKED _rt_dhcpv6_option_hdr_
{
	USHORT 	opcode;
	USHORT 	oplen;
...
typedef struct GNU_PACKED _rt_dhcpv6_hdr_
{
	UCHAR msgType;
	UCHAR trans_id[3];
	UCHAR options[1];	/* hook point for options. */
	...
	IN UINT32			offset)
{
	...
	PUCHAR udpHdr;
	PNDIS_PACKET newSkb = NULL;
	UINT16 srcPort, dstPort, srcPort_ori, dstPort_ori;
	PUCHAR bootpHdr;
	UINT16 bootpFlag;
	UINT16 srcPort, dstPort;

	udpHdr = (PUCHAR)(pLayerHdr + offset);
	srcPort_ori = get_unaligned((PUINT16)(udpHdr));
	srcPort = OS_NTOHS(srcPort_ori);
	dstPort_ori = get_unaligned((PUINT16)(udpHdr+2));
	dstPort = OS_NTOHS(dstPort_ori);
		...
		(srcPort==547 && dstPort==546))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("DHCPv6 MsgType=0x%x!\n", pDHCPv6Hdr->msgType));
	...
	IN ULONG BufLen)
{
	...
	if ((!pIPv6MacTable) || (!pIPv6MacTable->valid))
	{
        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():IPv6MacTable not init yet!\n", __FUNCTION__));
		return;
	}
