	...
	if (token_q->token_inited == TRUE)
	{
		...
		if (token_list) {
			if ((token >=0) && (token < PKT_TX_TOKEN_ID_CNT)) {
				...
				if (pkt_buf == NULL) {
					DBGPRINT(RT_DEBUG_OFF, ("%s(): buggy here? token ID(%d) without pkt!\n",
								__FUNCTION__, token));
				}
				entry->pkt_buf = NULL;
				...
				DBGPRINT(RT_DEBUG_OFF, ("%s(): Invalid token ID(%d)\n", __FUNCTION__, token));
			}
		}
...
#if 0
		if (pkt_buf) {
			RELEASE_NDIS_PACKET(pAd, pkt_buf, NDIS_STATUS_FAILURE);
		}
#endif
	}

#ifdef CUT_THROGH_DBG
	DBGPRINT(RT_DEBUG_TRACE, ("%s(): DeQ for token[%d] %s! Before Head/Tail=%d/%d, After=%d/%d\n",
...
#ifdef CUT_THROGH_DBG
	DBGPRINT(RT_DEBUG_TRACE, ("%s(): EnQ for pkt[%p] %s! token=%d, Before Head/Tail=%d/%d, After=%d/%d\n",
...
INT cut_through_init(RTMP_ADAPTER *pAd)
{
	...
	if (token_q->token_inited == FALSE) {
		...
		DBGPRINT(RT_DEBUG_OFF, ("%s(): TokenList inited done!id_head/tail=%d/%d\n",
					__FUNCTION__, token_list->id_head, token_list->id_tail));
	}
	else
	{
		DBGPRINT(RT_DEBUG_ERROR, ("%s(): TokenList already inited!shall not happened!\n", __FUNCTION__));
		if (!token_q->list) {
			DBGPRINT(RT_DEBUG_ERROR, ("%s(): TokenList is NULL!\n", __FUNCTION__));
			return FALSE;
		}
		DBGPRINT(RT_DEBUG_OFF, ("\tlist.id_head=%d, list.id_tail=%d\n",
...
USHORT write_first_buf(RTMP_ADAPTER *pAd, TX_BLK *txblk, UCHAR *dma_buf)
{
	...
	rlt_tso_info_write(pAd,
					(TSO_INFO *)(&txblk->HeaderBuf[tso_pos]),
					txblk,
					first_buf_len);
#endif /* CONFIG_CSO_SUPPORT */
...
kick_out:
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_LOUD, ("RTMPFreeTXDUponTxDmaDone %d.\n", FREE));
...
#if 0
		if (pTxD->DMADONE != 1) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RTMPFreeMgmtDmaDoneError====> pTxD->DMADONE != 1.\n"));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("pMgmtRing->TxSwFreeIdx= %08lx, pMgmtRing->TxDmaIdx = %08lx\n",
...
#ifdef RLT_MAC
#ifdef MT7601
static inline INT mt7601_rx_info_2_blk(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk, PNDIS_PACKET pRxPacket)
{
	...
	rxwi_x = (struct _RXWI_XMAC *)(GET_OS_PKT_DATAPTR(pRxPacket) + RAL_RXINFO_SIZE);
	pRxBlk->pRxWI = (RXWI_STRUC *)(rxwi_x);
#ifdef RT_BIG_ENDIAN
	...
	pRxBlk->rx_signal.raw_rssi[0] = rxwi_x->SNR2; /* RSSI */
	pRxBlk->rx_signal.raw_rssi[1] = rxwi_x->SNR1; /* Antenna ID */
	pRxBlk->rx_signal.raw_rssi[2] = rxwi_x->bw; /* BW */
	pRxBlk->rx_signal.raw_snr[0] = rxwi_x->SNR0;
	pRxBlk->rx_signal.raw_snr[1] = 0;
	pRxBlk->rx_signal.raw_snr[2] = 0;
	pRxBlk->rx_signal.freq_offset = rxwi_x->FOFFSET;
...
#ifdef MT76x2
	if (IS_MT76x2(pAd)) {
		pRxBlk->rx_signal.raw_snr[0] = rxwi_n->bbp_rxinfo[2];
		pRxBlk->rx_signal.raw_snr[1] = rxwi_n->bbp_rxinfo[3];
	}
	else
#endif /* MT76x2 */
	{
...
static inline INT rtmp_rx_info_2_blk(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk, PNDIS_PACKET pRxPacket)
{
	...
	pRxBlk->rx_signal.bf_snr[0] = rxwi_o->BF_SNR0;
	pRxBlk->rx_signal.bf_snr[1] = rxwi_o->BF_SNR1;
	pRxBlk->rx_signal.bf_snr[2] = rxwi_o->BF_SNR2;
	...
	UCHAR RxRingNo)
{
...
#ifdef MT7601
			if (IS_MT7601(pAd))
				mt7601_rx_info_2_blk(pAd, pRxBlk, pRxPacket);
			else
#endif /* MT7601 */
	...
	*pbReschedule = bReschedule;
	...
	if (RxRingNo !=0 )
	{
		printk("<---%s(): RingNo=%d, RxSwReadIdx=%d, pRxPacket=0x%p\n", 
             		 __FUNCTION__, RxRingNo, pRxRing->RxSwReadIdx, pRxPacket);
...
/* management frame doesn't need encryption. so use RESERVED_WCID no matter u are sending to specific wcid or not */
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): Send VhtNDPA to peer(wcid=%d, pMacEntry=%p) with dataRate(PhyMode:%s, BW:%sHz, %dSS, MCS%d)\n",
...
#if 0
                        UINT8 *token;
                token = GET_OS_PKT_DATAPTR(pRxPacket)+25+32;

                if (*token == 0x76){
                    printk("RxRing1DoneInterruptHandle ok\n");
                }
                else{
                    ...
                    rxd_0 = (union rmac_rxd_0 *)(GET_OS_PKT_DATAPTR(pRxPacket));

                    rx_pkt_type = RMAC_RX_PKT_TYPE(rxd_0->word);

                    if ((rx_pkt_type == 0) || (rx_pkt_type == 7))
                    {  RELEASE_NDIS_PACKET(pAd, pRxPacket, NDIS_STATUS_SUCCESS);}
                    else { /* TODO: Still need Leonardo maintain */
                        //printk("leonardo500 RxRing1DoneInterruptHandle error rx_pkt_type: %x\n",rx_pkt_type);
                    }
                }
...
//---Add by Carter for MT7603
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s():change state as idle\n", __FUNCTION__));
		}
		else
		{
...
#if 0
//def CONFIG_AP_SUPPORT//Carter commented for update beacon by TxS, instead of dma_done
	if (pAd->ApCfg.MBSSID[0].bcn_buf.bcn_state != BCN_TX_IDLE) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("<--%s():bcn_state=%d\n",
					__FUNCTION__, pAd->ApCfg.MBSSID[0].bcn_buf.bcn_state));
	}
#endif /* CONFIG_AP_SUPPORT */
