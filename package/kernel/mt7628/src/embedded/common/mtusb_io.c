...
NTSTATUS mtusb_read_reg(RTMP_ADAPTER *pAd, UINT32 addr, UINT32 *value)
{
	NTSTATUS Status = 0;
	UINT32 localVal;
	UINT32 global_addr = mt_physical_addr_map(pAd, addr);

#if defined(MT7603) ||defined (MT7636)
	if ((global_addr >= 0x60000000) && (global_addr <= 0x604001ff) && OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_DOZE))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:: H/W is in fOP_STATUS_DOZE and access WF_MAC CR, addr(0x%8x), global_addr(0x%8x). Error\n", __FUNCTION__, addr, global_addr));
		return;
	}
#endif /*7603/7636*/	
	...
	*value = le2cpu32(localVal);
	...
	{
		*value = 0xffffffff;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s::addr(0x%8x), global_addr(0x%8x), read this address error\n", __FUNCTION__, addr, global_addr));
	}
...
NTSTATUS RTUSBWriteMACRegister(
	...
	IN BOOLEAN bWriteHigh)
{
	return mtusb_multiwrite(pAd, Offset, &Value, 4, 4);
}
