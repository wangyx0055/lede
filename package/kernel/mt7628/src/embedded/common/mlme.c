...
{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("SCANNING, suspend MSDU transmission ...\n"));
	...
	IN UCHAR OldUP)
{
	...
	if (NState == NDIS_STATUS_SUCCESS) 
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("send NULL Frame @%d Mbps to AID#%d...\n", RateIdToMbps[TxRate], AID & 0x3f));
...
VOID RTMPOffloadPm(RTMP_ADAPTER *pAd, UINT8 ucWlanIdx, UINT8 ucPmNumber, UINT8 ucPmState)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s--->::Current setting, ucWlanIdx(%d), ucPmNumber(%d), ucPmState(%d)\n", __FUNCTION__, ucWlanIdx, ucPmNumber, ucPmState));

	if (!OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_DOZE) && (ucPmState == ENTER_PM_STATE))
	{
		/* H/W enter PM4 offlaod here */
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::***** H/W Enter PM4 *****\n", __FUNCTION__));
	...
	else if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_DOZE) && (ucPmState == EXIT_PM_STATE))
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::***** H/W Exit PM4 *****\n", __FUNCTION__));
...
VOID MlmeHandler(RTMP_ADAPTER *pAd) 
{
	...
	while (!MlmeQueueEmpty(&pAd->Mlme.Queue)) 
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("System halted, removed or MlmeRest, exit MlmeTask!(QNum = %ld)\n",
						pAd->Mlme.Queue.Num));
			break;
		}
		...
		if (MlmeDequeue(&pAd->Mlme.Queue, &Elem)) 
		{
			...
			{
				DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("!!! reset MLME state machine !!!\n"));
...
#endif /* RTMP_MAC_USB | RTMP_SDIO_SUPPORT */
			...
			switch (Elem->Machine) 
			{
				...
				case APCLI_AUTH_STATE_MACHINE:
					...
					if(isValidApCliIf(apcliIfIndex))
					{
						...
						if (apcliIfIndex >= 64)
						{
							CliIdx = ((apcliIfIndex - 64) % MAX_EXT_MAC_ADDR_SIZE);
							apcliIfIndex = ((apcliIfIndex - 64) / MAX_EXT_MAC_ADDR_SIZE);
...
#endif
				default:
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): Illegal SM %ld\n",
...
NDIS_STATUS MlmeInit(RTMP_ADAPTER *pAd)
{
...

	{
		...
		if (Status == NDIS_STATUS_FAILURE) {
			DBGPRINT (DBG_LVL_ERROR,  ("%s: unable to start MlmeThread\n", RTMP_OS_NETDEV_GET_DEVNAME(pAd->net_dev)));
		}
	}
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<-- MLME Initialize\n"));
...
/*#endif  RT3090 */
			...
			if (IS_RT3290(pAd))
			{
				...
				)
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,(" RT3290: Read GPIO as Hardware controlled radio state\n"));
				}
			}
			else
#endif /* RT3290 */
...
#endif /* CONFIG_STA_SUPPORT */
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Ant-realrssi0(%d), Lastrssi0(%d), EvaluateStableCnt=%d\n", realavgrssi, pAd->RxAnt.Pair1LastAvgRssi, pAd->RxAnt.EvaluateStableCnt));
		}
		else
#endif /* ANT_DIVERSITY_SUPPORT */
		{
	...
	*/
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			*/
			...
			if (pAd->macwd > 1)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("MAC specific condition \n"));
...
#endif /* RTMP_MAC_USB */
				{
					...
					*/
					...
					if (((MacReg & 0x20000000) && (MacReg & 0x80)) || ((MacReg & 0x20000000) && (MacReg & 0x20)))
					{
						...
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN,("Warning, MAC specific condition occurs \n"));
					}
				}
			}
		}
	...
	if (pAd->CommonCfg.bEnTemperatureTrack == TRUE)
	{
		...
		{
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Track Power Compensation !!!\n"));
#endif /* RELEASE_EXCLUDE */
...
#endif /* RTMP_TEMPERATURE_COMPENSATION */
		{
			...
			if ((bbpval & 0x10) == 0)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Current Temperature from BBP_R49=0x%x\n", pAd->CurrTemperature));
				RT6352_TemperatureCalibration(pAd);
				pAd->CommonCfg.bEnTemperatureTrack = FALSE;
	...
	{
#if 0
		DBGPRINT(RT_DEBUG_TRACE, ("%s, %u, NumOfChannels = %u, LastScanChannel = %u, bScanning = %u, BreakTime = %u\n",
				__FUNCTION__, __LINE__,
				pAd->ScanCtrl.PartialScan.NumOfChannels,
				pAd->ScanCtrl.PartialScan.LastScanChannel,
				pAd->ScanCtrl.PartialScan.bScanning,
				pAd->ScanCtrl.PartialScan.BreakTime));
#endif		
...
VOID STAMlmePeriodicExec(RTMP_ADAPTER *pAd)
{
	...
	*/
	...
	)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PSM - Issue Sleep command)\n"));
	}
		...
		&& (NdisEqualMemory(pAd->CurrentAddress, pAd->EthConvert.EthCloneMac, MAC_ADDR_LEN) == FALSE))
	{
		...
		else
		{
			...
			DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE,("Write EthCloneMac to ASIC: =%02x:%02x:%02x:%02x:%02x:%02x\n",
			PRINT_MAC(pAd->CurrentAddress)));

			if(pAd->EthConvert.SSIDStrLen != 0)
			{
				/*DBGPRINT_RAW(DBG_CAT_ALL, DBG_LVL_TRACE, ("copy MlmeAux.Ssid to AutoReconnect!\n"));*/
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("MMCHK - CommonCfg.Ssid[%d]=%c%c%c%c... MlmeAux.Ssid[%d]=%c%c%c%c...\n",
...
#endif /*RTMP_PCI_SUPPORT*/	
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("COEX: StaCfg.AvgRssi[0] = %d\n",pAd->StaCfg.RssiSample.AvgRssi[0]));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("COEX:CoexMode = %d\n",pAd->CoexMode.CurrentMode));
	             if ((IS_MT7650(pAd) || IS_MT7630(pAd)) && (wlanFunInfo.field.COEX_MODE & 0x4))
		{
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("COEX:Strange CoexMode = %d\n",pAd->CoexMode.CurrentMode));
			}
		}
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("COEX: pAd->CoexMode.CoexTDDRSSITreshold = %d\n",pAd->CoexMode.CoexTDDRSSITreshold));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("COEX: pAd->CoexMode.CoexFDDRSSITreshold = %d\n",pAd->CoexMode.CoexFDDRSSITreshold));
	...
	*/
		...
		((pAd->Mlme.OneSecPeriodicRound % 5) == 4))	
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("!!! BBP_R150 = %d, BT_EN = %d\n",pAd->BbpWriteLatch[BBP_R150], pAd->BtFunCtrl.field.BT_EN));
		...
		if ((((pAd->WlanFunInfo.field.COEX_MODE&0x1E) == 0) || (pAd->BtFunCtrl.field.BT_EN == 0)) && (pAd->BbpWriteLatch[BBP_R150] != 0xc0))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!! EN BBP_R150 = %d\n",pAd->BbpWriteLatch[BBP_R150]));
		...
		else if(((pAd->WlanFunInfo.field.COEX_MODE&0x1E) != 0) && (pAd->BbpWriteLatch[BBP_R150] != 0x00))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!! DISABLE BBP_R150 = %d\n",pAd->BbpWriteLatch[BBP_R150]));
	...
	if (INFRA_ON(pAd))
	{
		...
		*/
...
#endif /* MT7628_FPGA */
		{
			/*7636 psm*/
#if defined(MT7636)
			if (wdev->Psm == PWR_ACTIVE)
#endif
			{
                RTMPSetAGCInitValue(pAd, BW_20);
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MMCHK - No BEACON. restore R66 to the low bound(%d) \n", (0x2E + pAd->hw_cfg.lan_gain)));
			}
		}
		...
		if (CQI_IS_DEAD(pAd->Mlme.ChannelQuality))
		{
			/*7636 psm*/
#if defined(MT7636)
			wdev->bBeaconLost = FALSE;
#endif			

			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MMCHK - No BEACON. Dead CQI. Auto Recovery attempt #%ld\n",
		...
		{
			pAd->RalinkCounters.BadCQIAutoRecoveryCount ++;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MMCHK - Bad CQI. Auto Recovery attempt #%ld\n",
						pAd->RalinkCounters.BadCQIAutoRecoveryCount));
			MlmeAutoReconnectLastSSID(pAd);
		}
		...
		if (pAd->StaCfg.bAutoRoaming)
		{
			...
			if (!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS) &&
				...
				(MaxRssi <= dBmToRoam))
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Rssi=%d, dBmToRoam=%d\n", MaxRssi, (CHAR)dBmToRoam));
				...
				if (rv == FALSE)
				{
					...
					{
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MMCHK - Roaming, No eligable entry, try new scan!\n"));
						pAd->StaCfg.LastScanTime = pAd->Mlme.Now32;
						MlmeAutoScan(pAd);
					}
				}
	...
	else if (ADHOC_ON(pAd))
	{
			...
			&& */OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MMCHK - excessive BEACON lost, last STA in this IBSS, MediaState=Disconnected\n")); 
			LinkDown(pAd, FALSE);
	...
	{
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("STAMlmePeriodicExec, no association so far\n"));
#endif /* RELEASE_EXCLUDE */		
#ifdef MESH_SUPPORT
		if (MESH_ON(pAd))
...
#ifdef RELEASE_EXCLUDE
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("pAd->StaCfg.bAutoReconnect is TRUE\n"));
#endif /* RELEASE_EXCLUDE */	
			}
		}
	}
...
VOID MlmeCheckForRoaming(RTMP_ADAPTER *pAd, ULONG Now32)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> MlmeCheckForRoaming\n"));
	...
	if (pRoamTab->BssNr > 0)
	{
		...
		if (pAd->Mlme.CntlMachine.CurrState == CNTL_IDLE)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MMCHK - Roaming attempt #%ld\n", pAd->RalinkCounters.PoorCQIRoamingCount));
...
BOOLEAN MlmeCheckForFastRoaming(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> MlmeCheckForFastRoaming\n"));
	/* put all roaming candidates into RoamTab, and sort in RSSI order*/
	BssTableInit(pRoamTab);
	for (i = 0; i < pAd->ScanTab.BssNr; i++)
	{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<== MlmeCheckForFastRoaming (BssNr=%d)\n", pRoamTab->BssNr));
	if (pRoamTab->BssNr > 0)
	{
		/* check CntlMachine.CurrState to avoid collision with NDIS SetOID request*/
		if (pAd->Mlme.CntlMachine.CurrState == CNTL_IDLE)
...
VOID MlmeSetPsmBit(RTMP_ADAPTER *pAd, USHORT psm)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MlmeSetPsmBit = %d\n", psm));
}
#endif /* CONFIG_STA_SUPPORT */
	...
	IN ULONG Now32)
{
	...
	*/	
	...
	/*7636 psm*/
#if defined(MT7636)	
	if (INFRA_ON(pAd) && (pMacEntry->wcid == BSSID_WCID) && (wdev->bBeaconLost) && (wdev->Psm == PWR_SAVE))
	{
...
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("MMCHK - CQI= %ld (Tx Fail=%u/Retry=%u/Total=%lu, Rx Fail=%u/Total=%lu, RSSI=%d dbm)\n", 
			ChannelQuality,
			OneSecTxFailCount,
...
/*===========================================================================*/
	do
	{
		...
		{			
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s(): invalid idx(%d)\n", __FUNCTION__, idx));
				return;
			}
		}
#endif /* APCLI_SUPPORT */
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
		{
				...
				(MaxDesire > RATE_11))
			{
				MaxDesire = RATE_11;
#ifdef RELEASE_EXCLUDE
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("%s():. 2 MaxDesire = %d. OpMode=%d\n", __FUNCTION__, MaxDesire, pAd->OpMode));
#endif /* RELEASE_EXCLUDE */
			}
			break;
		}
...
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("Exptected ACK rate[%d] = %d Mbps\n", RateIdToMbps[i], RateIdToMbps[CurrBasicRate]));
#endif /* RELEASE_EXCLUDE */
	}

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s():[MaxSupport = %d] = MaxDesire %d Mbps\n",
...
#ifdef RELEASE_EXCLUDE
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" %s(): (Rssi=%d, init TX rate = %d Mbps)\n",
...
//	ULONG BasicMCS;
		...
		)
	{
		...
		if (pDesireHtPhy->MCSSet[4] != 0)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("%s():<=== Use Fixed MCS = %d\n",__FUNCTION__, pMinHtPhy->field.MCS));
		}
...
#ifdef RELEASE_EXCLUDE
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("%s():<=== Use Fixed MCS = %d\n", __FUNCTION__, i));
#endif /* RELEASE_EXCLUDE */
				break;
			}
			if (i==0)
	...
	if (ie_list->HtCapabilityLen> 0)
	{
		...
		if (ie_list->vht_op_len) {
				...
				(pBss->CentralChannel != ie_list->AddHtInfo.ControlChan))
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():VHT cent_ch=%d, vht_op_info->center_freq_1=%d, Bss->CentCh=%d, change from CentralChannel to cent_ch!\n",
...
#define	WAC_DEVICE_INFORMATION_ID	0x03
						...
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("%s: Samsung OUI Type = 0x%02X\n", __FUNCTION__, oui_type));
						...
						{	
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: WAC Device Information: %d, %d, %d, %d, %d\n", 
	...
	if (HtCapabilityLen == 0)
	{
		...
		*/
		for (index = 0; index<MAX_TRIGGER_EVENT; index++)
		{
			...
			{
#ifdef RELEASE_EXCLUDE
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("ReturnDueToAlreadyInserted(EventIdx=%d,Ch=%d,Reg=%d)\n", 
...
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("Insert EventA Entry(EvtIdx=%d,Ch=%d,Reg=%d,Mac=%02x:%02x:%02x:%02x:%02x:%02x\n", 
	...
	IN UCHAR SsidLen) 
{
	...
	for (i = 0; i < pAd->ScanTab.BssNr; i++) 
	{
			...
			(SSID_EQUAL(Ssid, SsidLen, pInBss->Ssid, pInBss->SsidLen) || bIsHiddenApIncluded))
		{
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("StaCfg.IEEE80211dClientMode == Rt802_11_D_Strict, but this AP doesn't have country IE.\n"));
				continue;
			}
#endif /* EXT_BUILD_CHANNEL_LIST */
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("STA is in N-only Mode, this AP don't have Ht capability in Beacon.\n"));
				continue;
			}
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("STA is in GN-only Mode, this AP is in B mode.\n"));
				continue;
			}
#endif /* DOT11_N_SUPPORT */
	...
	IN ULONG Priv)
{
	...
	if (MlmeQueueFull(Queue, 1)) 
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s():full, msg dropped and may corrupt MLME\n", __FUNCTION__));
#endif /* RELEASE_EXCLUDE */		
		return FALSE;
	}
	...
	*/
	...
	NdisReleaseSpinLock(&(Queue->Lock));
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("MlmeEnqueue, num=%ld\n",Queue->Num));
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
		/* Can't add debug print here. It'll make Mlme Stat machine freeze.*/
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): full and dropped\n", __FUNCTION__));
#endif /* RELEASE_EXCLUDE */
		RTMP_MLME_HANDLER(pAd);
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s():num=%ld\n", __FUNCTION__, Queue->Num));
#endif /* RELEASE_EXCLUDE */
	Queue->Entry[Tail].Occupied = TRUE;
	Queue->Entry[Tail].Machine = Machine;
	Queue->Entry[Tail].MsgType = MsgType;
...
#ifdef RELEASE_EXCLUDE
        /* Can't add debug print here. It'll make Mlme Stat machine freeze.*/
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("MlmeEnqueueForWsc: full, msg dropped and may corrupt MLME\n"));
#endif /* RELEASE_EXCLUDE */
...
#ifdef RELEASE_EXCLUDE
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("MlmeEnqueueForWsc, num=%ld\n",Queue->Num));
#endif /* RELEASE_EXCLUDE */
    Queue->Entry[Tail].Occupied = TRUE;
    Queue->Entry[Tail].Machine = Machine;
    Queue->Entry[Tail].MsgType = MsgType;
...
BOOLEAN MlmeDequeue(MLME_QUEUE *Queue, MLME_QUEUE_ELEM **Elem) 
{
	...
	*Elem = &(Queue->Entry[Queue->Head]);    
	...
	NdisReleaseSpinLock(&(Queue->Lock));
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("MlmeDequeue, num=%ld\n",Queue->Num));
#endif /* RELEASE_EXCLUDE */
	return TRUE;
}
...
VOID MlmeRestartStateMachine(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MlmeRestartStateMachine \n"));
...
BOOLEAN RTMPCheckChannel(RTMP_ADAPTER *pAd, UCHAR CentralCh, UCHAR ch)
{
	...
	for (k = 0;k < pAd->ChannelListNum;k++)
	{
		...
		{
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("UpperChannel=%d  is in Channel List[%d]\n", UpperChannel, k ));
#endif /* RELEASE_EXCLUDE */
		...
		{
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("LowerChannel=%d  is in Channel List[%d]\n", LowerChannel, k ));
#endif /* RELEASE_EXCLUDE */
			NoEffectChannelinList ++;
		}
	}

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Total Channel in Channel List = [%d]\n", NoEffectChannelinList));
...
VOID RTMPUpdateMlmeRate(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():=>MlmeTransmit=0x%x\n",
				__FUNCTION__, pAd->CommonCfg.MlmeTransmit.word));
}
#endif /* CONFIG_STA_SUPPORT */
...
VOID AsicEvaluateRxAnt(RTMP_ADAPTER *pAd)
{
		 ...
		 && (pAd->CommonCfg.RxAntDiversityCfg == ANT_SW_DIVERSITY_ENABLE))
	{
		...
		/* the other is failed antenna remove:two physical antenna can rx and tx*/
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("AntDiv - before evaluate Pair1-Ant (%d,%d)\n",
...
/*			if (pAd->CommonCfg.bRxAntDiversity == ANT_DIVERSITY_DISABLE)*/
		...
		{
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("AsicEvaluateRxAnt : RealRxPath=%d \n", pAd->Mlme.RealRxPath));
#endif /* RELEASE_EXCLUDE */
...
/*				pAd->RxAnt.EvaluateStableCnt = 0;*/
			...
			IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("AsicRxAntEvalAction::After Eval(fix in #%d), <%d, %d>, RcvPktNumWhenEvaluate=%ld\n",
...
#ifdef RELEASE_EXCLUDE
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("AsicRxAntEvalTimeout : RealRxPath=%d, AvgRssi=%d, %d, %d \n",
...
/*
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("Sending trigger frame and enter service period when support APSD\n"));
...
VOID AsicCheckForHwRecovery(RTMP_ADAPTER *pAd)
{
	if (!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_NIC_NOT_EXIST))
	{
		...
		*/
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("AsicCheckForHwRecovery!!\n"));
