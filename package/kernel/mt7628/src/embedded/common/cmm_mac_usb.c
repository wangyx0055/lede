	...
	IN RTMP_STRING *pBufName)
{
	...
	*ppUrb = RTUSB_ALLOC_URB(0);
	if (*ppUrb == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("<-- ERROR in Alloc urb struct for %s !\n", pBufName));
		return NDIS_STATUS_RESOURCES;
	}

	*ppXBuffer = RTUSB_URB_ALLOC_BUFFER(pObj->pUsb_Dev, bufLen, pDmaAddr);
	if (*ppXBuffer == NULL) {
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("<-- ERROR in Alloc Bulk buffer for %s!\n", pBufName));
		return NDIS_STATUS_RESOURCES;
	}
...
VOID RTMPFreeTxRxRingMemory(PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("---> RTMPFreeTxRxRingMemory\n"));
...
NDIS_STATUS	NICInitRecv(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("--> NICInitRecv\n"));
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("--> NICInitTransmit\n"));
	...
	do
	{
		...
		for (i = 0; i < MGMT_RING_SIZE; i++)
		{
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("<-- ERROR in Alloc TX MLMEContext[%d] urb!! \n", i));
		...
		for (i = 0; i < BCN_RING_SIZE; i++)
		{
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("<-- ERROR in Alloc TX pBcnContext[%d] urb!! \n", i));
				Status = NDIS_STATUS_RESOURCES;
				goto err;
			}
			pBcnContext->pAd = pAd;
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<-- NICInitTransmit(Status=%d)\n", Status));
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<-- NICInitRecv(Status=%d)\n", Status));
	return Status;
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	do
	{
		...
		for (i = 0; i < BCN_RING_SIZE; i++)
		{
			...
			if (pBcnContext->pUrb == NULL)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("<-- ERROR in Alloc TX BCNContext[%d] urb!! \n", i));
				Status = NDIS_STATUS_RESOURCES;
				goto err;
			}
			pBcnContext->pAd = pAd;
...
VOID RT28XXDMAEnable(RTMP_ADAPTER *pAd)
{
	...
	if (pAd->chipCap.hif_type == HIF_RTMP) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():EnableWPDMA\n", __FUNCTION__));
	...
	if (IS_MT76x0(pAd) || IS_MT76x2(pAd)) {
		...
		if ((pAd->BulkInMaxPacketSize >= 512) && (pAd->usb_ctl.usb_aggregation))
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("disable usb rx aggregagion\n"));
	...
	IN ULONG			UpdatePos)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,("%s():pBeaconFrame/Sync(%p/%p) is NULL\n",
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("--->RTUSBBssBeaconStop()!\n"));
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("--->RTUSBBssBeaconStart()!\n"));
#endif /* RELEASE_EXCLUDE */
	pBeaconSync = pAd->CommonCfg.pBeaconSync;
	if (pBeaconSync && pBeaconSync->EnableBeacon)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RTUSBBssBeaconStart:BeaconFactor=%d, BeaconRemain=%d!\n",
	...
	IN PRTMP_ADAPTER pAd)
{
    ...
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("UsbMlmeRadioOn()\n"));
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,("UsbMlmeRadioOFF()\n"));
...
VOID RT28xxUsbAsicRadioOn(RTMP_ADAPTER *pAd)
{
	...
	RtmpusecDelay(50);
	AsicSetWPDMA(pAd, PDMA_TX_RX, TRUE);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" %s():Enable WPDMA\n", __FUNCTION__));
...
#ifdef RELEASE_EXCLUDE
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("AsicCheckCommand %s:Command=0x%x, cnt=%d, CID=0x%x, CmdStatus=0x%x \n",
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Send WOW enable cmd (%d/%d/%d)\n", pAd->WOW_Cfg.nDelay, pAd->WOW_Cfg.nSelectedGPIO, pAd->WOW_Cfg.nHoldTime));
	RTMP_IO_READ32(pAd, GPIO_HOLDTIME_OFFSET, &Value);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Hold time: 0x7020 ==> %x\n", Value));
}
	...
	IN PRTMP_ADAPTER pAd)
{
    ...
    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("MCU back to normal mode (%d/%d)\n", pAd->WOW_Cfg.nDelay, pAd->WOW_Cfg.nSelectedGPIO));
}
#endif /* RTMP_MAC */
#endif /* WOW_SUPPORT */
#endif /* RTMP_MAC_USB */
