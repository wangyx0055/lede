...
static UINT32 AGSP_TRAIN_SEQ_END = 0;
	...
	IN RTMP_SA_TRAINING_PARAM *pSAStaEntry)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF | DBG_FUNC_SA, 
	...
	if (pMacEntry == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF | DBG_FUNC_SA, 
				...
				__FUNCTION__, PRINT_MAC(pSAStaEntry->macAddr)));
		return FALSE;
	}
...
#error "Need to fix ConvertToRssi() before compile this feature"
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE | DBG_FUNC_SA, 
	...
	IN MAC_TABLE_ENTRY *pEntry)
{
	...
	if ((pEntry->mcsInUse >= 0) && (pEntry->mcsInUse <= 32))
	{
		...
		//		__FUNCTION__, pEntry->mcsInUse, pEntry->mcsUsage[pEntry->mcsInUse], 
	...
	printk("\n");
#if 0	
	printk("\tPreSNR[%d]:%2d(%d-%d), PreSNR[%d]:%2d(%d-%d), PreSNR[%d]:%2d(%d-%d)\n", 
				0, pEntry->avgPreSNR[0], pEntry->sumPreSNR[0], pEntry->cntPreSNR[0],
				1, pEntry->avgPreSNR[1], pEntry->sumPreSNR[1], pEntry->cntPreSNR[1],
				2, pEntry->avgPreSNR[2], pEntry->sumPreSNR[2], pEntry->cntPreSNR[2]);
#endif
	printk("\n");
}
...
VOID sa_dump_agsp(RTMP_SA_AGSP_MAP *pAgspList)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Start Dump AGSP List:\n"));
	cnt = 0;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tHeaderPin  GPIO RegOffset  GPIOBit\n"));
	while (memcmp(pAgspEntry, &AGSP_ENTRY_END, sizeof(RTMP_SA_AGSP_MAP))!= 0)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\t%d\t0x%x\t%d\n", 
...
VOID sa_dump_train_seq(UINT32 *pTrainSeq)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Start Dump SA TrainingSequence:\n"));
	while(*pAntPair != AGSP_TRAIN_SEQ_END)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t0x%x(%d, %d)\n", *pAntPair, 
	...
	IN UCHAR txNss)
{
	...
	if (pMacEntry)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("SmartAntTrainLogDump(Ant:0x%x-%d,TimeInterval:0x%lx-0x%lx)\n", 
				pTrainLog->antPattern, pTrainLog->patternOffset,
				pTrainLog->srtTime, pTrainLog->endTime));
		
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tBW:MCS=%d:%d, txMcs=%d\n",
				pMacEntry->HTPhyMode.field.BW, pMacEntry->HTPhyMode.field.MCS,
				pTrainLog->txMcs));

		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\ttxCnt=%d,noRtyCnt=%d,reTry=%d,rtyFail=%d,PER=%d!\n",
				pTrainLog->txCnt,pTrainLog->txNoRtyCnt, pTrainLog->txRtyCnt, pTrainLog->txRtyFailCnt, pTrainLog->PER));
		if (pTrainLog->rxCnt)
		{
			int i;
#error "Need to fix ConvertToRssi() before compile this feature"
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tRxCnt=%d\n", pTrainLog->rxCnt));
			for(i = 0 ; i < txNss; i++)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\tAnt%d:Rssi=%d(%d), SNR=%d(%d), preSNR=%d(%d)\n",
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tNo valid RxCnt!\n"));
		}

#ifdef SA_DBG
		if (pAd->smartAntDbgOn)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tRssi Distribution:\n"));
			for (i = 0 ; i < txNss; i++)
			{
				...
				for ( j =0 ; j < 33; j++)
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%6d", pTrainLog->rssiDist[i][j]));
				}
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\n"));
			}
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tSNR Distribution:\n"));
			for (i = 0 ; i < txNss; i++)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\tSNR-%d:\n\t\t\t", i));
				for ( j =0 ; j < 33; j++)
				{
					if (j == 16)
					{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%6d", pTrainLog->SNRDist[i][j]));
				}
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\n"));
			}
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tBF_SNR Distribution:\n"));
			for (i = 0 ; i < txNss; i++)
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\tBF_SNR-%d:\n\t\t\t", i));
				for ( j =0 ; j < 33; j++)
				{
					if (j == 16)
					{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%6d", pTrainLog->preSNRDist[i][j]));
				}
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\n"));
			}
		}
...
#if 0	
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): Try find antPattern(0x%x-0x%x) in TrainSeq(0x%x, cnt=%d)\n",
			__FUNCTION__, antPattern, revPattern, *pTrainSeq, pSAParam->trainSeqCnt));
#endif

	offset = 0;
	do{
#if 0
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s():check pTrainSeq(0x%x) at offset(%d)\n", 
				__FUNCTION__, *(pTrainSeq + offset), offset));
#endif
	...
	IN UINT32 newAntCfg)
{
	...
	if ((regVal & 0x400) != 0x400)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO | DBG_FUNC_SA,("Change as GPIO Mode(0x%x)\n", regVal));
		RTMP_SYS_IO_READ32(SA_REG_GPIO_MODE, &regVal);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO | DBG_FUNC_SA,("After Change, now GPIO_MODE value is 0x%x\n", regVal));
	}
	...
	if ((regVal & SA_REG_GPIO_DIR_OUTPUT_MASK) != SA_REG_GPIO_DIR_OUTPUT_MASK)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO | DBG_FUNC_SA,("The GPIO output direction value is 0x%x, change to 0x%x!\n", 
	...
	RTMP_SYS_IO_READ32(regAddr, &regVal);
	//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("OldGPIOVal=0x%x, oldAntCfg=0x%x, newAntCfg=0x%x!\n",regVal, oldAntCfg, newAntCfg));
#if 0
	...
	RTMP_SYS_IO_WRITE32(regAddr + SA_REG_GPIO_OFFSET_SET, newAntCfg);

	// clear the old antenna setting
	
	if (regVal)
	{
		regVal &= (~newAntCfg);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Clean the oldAnt(addr=0x%x, val=0x%x)\n", regAddr + SA_REG_GPIO_OFFSET_RESET, regVal));
		RTMP_SYS_IO_WRITE32(regAddr + SA_REG_GPIO_OFFSET_RESET, regVal);
	}
#else
	regVal &= (~SA_REG_GPIO_DIR_OUTPUT_MASK);
	newAntCfg &= SA_REG_GPIO_DIR_OUTPUT_MASK;
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO | DBG_FUNC_SA, 
				("Write the AntPattern(addr=0x%x, val=0x%x)\n", regAddr, regVal));
#endif

	RTMP_SYS_IO_READ32(regAddr, &regVal);
	//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("After Cfg, GPIOVal=0x%x!\n",regVal));
	...
	IN UINT32 selectRule)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, 
				...
				__FUNCTION__, pTrainEntry->curAntPattern, 
...
check:
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\tCheck Ant(0x%x-%d), AntTrainLog->antWeight(%d)\n",
	...
	if (weight && (pTrainEntry->curAntPattern != 0))
	{
		if (pCurLog->antWeight > weight)
		{
			...
			{
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\t\tGPIO of CandAnt(0x%x-%d) in TSeq(0x%x) and TLog(0x%x) are mismatch\n",
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\t\tAnt(0x%x-%d) weight(%d) > required(%d), ignore\n",
						pTrainEntry->curAntPattern, pTrainEntry->patternOffset,
		...
		else
		{
			...
			switch (selectRule)
			{
				case ANT_SELECT_IGNORE_BASE:
					...
					{
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("\t\tByRule(%d), Ant(0x%x-%d) == BaseAnt(0x%x-%d), ignore\n",
	...
	IN UINT32 newAnt)
{
	...
	//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("newCfg=0x%x, oldCfg=0x%x\n", newCfg, oldCfg));
	...
	IN UCHAR weight)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO | DBG_FUNC_SA, 
				...
				__FUNCTION__, weight, antPattern));
	...
	if ((RTDebugFunc & DBG_FUNC_SA) == DBG_FUNC_SA)
	{
		for (cnt = 0; cnt < pSAParam->trainSeqCnt; cnt++)
		{
			pCurLog = (RTMP_SA_TRAIN_LOG_ELEMENT *)(pTrainEntry->pTrainInfo + cnt);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tantPattern:0x%x, weight:%d\n", pCurLog->antPattern, pCurLog->antWeight));
		}
	}
	...
	IN UCHAR weight)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():pTrainInfo is NULL\n", __FUNCTION__));
		return FALSE;
	}
	...
	IN UCHAR *pMacAddr)
{
	...
	*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s():sizeof(RTMP_SA_TRAINING_PARAM)=%d, offset=%d, cleanLen=%d!\n", 
			__FUNCTION__, sizeof(RTMP_SA_TRAINING_PARAM), offset, cleanLen));
...
#if 0
	if (pTrainLog)
		pTrainEntry->pTrainInfo = pTrainLog;
#endif
	...
	IN BOOLEAN bForced)
{
	...
	if (pMacAddr)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE | DBG_FUNC_SA, 
					...
					__FUNCTION__, PRINT_MAC(pMacAddr), bForced));
	}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): delete %s\n", 
					__FUNCTION__, (bDeleted == TRUE ? "success" : "failed")));
	...
	IN SMART_ANTENNA_STRUCT *pSAParam)
{
	...
	if (pSAParam == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():pSAParam is NULL\n", __FUNCTION__));
		return FALSE;
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("After Selection, Cannot find target TrainEntry\n"));
		return FALSE;
	}
}
	...
	IN SMART_ANTENNA_STRUCT *pSAParam)
{
	...
	{
		os_free_mem(pAd, pSAParam->pTrainMem);
	...
	{
		os_free_mem(pAd, pSAParam->pTrainSeq);
	}
	...
	IN UINT32 *pTrainSeqLst)
{
	...
	if (cnt == 0)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Error:TrainSeqCnt is zero!\n"));
		return FALSE;
	}

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():TrainCnt=%d\n", __FUNCTION__, cnt));
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():malloc failed\n", __FUNCTION__));
		return FALSE;
	}
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("AllocTrainMemDone(addr=0x%lx,entrySize=%d,totalSize=%d)\n", 
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tTrainMemoryLayout:\n"));
	for (cnt = 0; cnt < SA_ENTRY_MAX_NUM; cnt++)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\ttrainInfo[%d]=%p\n", cnt, 
	...
	IN RTMP_SA_TRAINING_PARAM *pTrainEntry)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF | DBG_FUNC_SA,("Method 2:\n"));
	pCanLog2 = NULL;
	...
	//MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Now calculate the PER of each antenna!\n"));
	majorPin = -1;
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF | DBG_FUNC_SA,("\tStage 1:\n"));
	for (cnt = 0; cnt < pSAParam->trainSeqCnt; cnt++)
	{
		if (txCnt[cnt])
	...
	IN RTMP_SA_TRAINING_PARAM *pTrainEntry)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO | DBG_FUNC_SA, ("\tPhase 2:\n")); 
	pCanLog = NULL;
	for (cnt = 0; cnt < pSAParam->trainSeqCnt; cnt++)
	{
		pCurLog = (RTMP_SA_TRAIN_LOG_ELEMENT *)(pTrainEntry->pTrainInfo + cnt);
	...
	IN RTMP_SA_TRAINING_PARAM *pTrainEntry)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():phase 1, get totalTxNoRtyCnt(trainWeight=%d)\n", 
				__FUNCTION__, pTrainEntry->trainWeight));
	...
	*/
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():phase 3, find candidate by PER(< max[PerUBnd=%d, McsTrainUpBnd=%d]) and Weight(<%d)\n", 
							__FUNCTION__, PerUBnd, mcsPerBnd, ANT_WEIGHT_CAND_LOW));
	for (cnt = 0; cnt < pSAParam->trainSeqCnt; cnt++)
	{
		pCurLog = (RTMP_SA_TRAIN_LOG_ELEMENT *)(pTrainEntry->pTrainInfo + cnt);
		curSNR = 0;
			...
			((pCurLog->PER <= PerUBnd) || (pCurLog->PER <= mcsPerBnd)))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tCheck Ant(0x%x-%d)=>AWeight=%3d,CWeight=%3d,PER=%4d,SNR=%2d,txCnt=%d(S:%d,R:%d,F:%d)\n", 
					pCurLog->antPattern, pCurLog->patternOffset, 
	...
	IN UCHAR antWeight)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Dump antWeight after adjust antenna weight!\n"));
	for (cnt = 0; cnt < pSAParam->trainSeqCnt; cnt++)
	{
		pTrainLog = (RTMP_SA_TRAIN_LOG_ELEMENT *)(pTrainEntry->pTrainInfo + cnt );
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tantPattern(0x%x-%d), antWeight=%d, candWeight=%d\n", 
					pTrainLog->antPattern, pTrainLog->patternOffset, 
	...
	IN RTMP_SA_TRAINING_PARAM *pTrainEntry)
{
	...
	if (pMacEntry)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():dump RSSI result\n", __FUNCTION__));
		for (i = 0; i < pSAParam->txNss; i++)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tRSSI[%d]: prev=%d, cur=%d\n", 
			...
			if (pMacEntry->baseAvgRSSI[i] != 0)
			{
					...
					((pMacEntry->baseAvgRSSI[i] - pMacEntry->curAvgRSSI[i]) > pSAParam->rssiVar))
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF | DBG_FUNC_SA, 
								...
								__FUNCTION__, i, pMacEntry->baseAvgRSSI[i], 
	...
	IN RTMP_SA_TRAINING_PARAM *pTrainEntry)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():Check consistence of training environment(BaseInfo:0x%x-%d,CurLog:0x%x-%d)\n", 
			__FUNCTION__, pBaseInfo->antPattern, pBaseInfo->patternOffset,
				pCurLog->antPattern, pCurLog->patternOffset));
	for (i = 0; i < pSAParam->txNss; i++)
	{
#error "Need to fix ConvertToRssi() before compile this feature"
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\tBase-RSSI[%d](cnt:%d,avgRSSI:%d), Current-RSSI[%d](cnt:%d,avgRSSI[%d])\n",
	...
	if (!is_ant_equal(pBaseInfo->antPattern,pCurLog->antPattern))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():ERROR!BaseInfo(0x%x-%d) not match CurLog(0x%x-%d)\n", 
				__FUNCTION__, pBaseInfo->antPattern, pBaseInfo->patternOffset,
				pCurLog->antPattern, pCurLog->patternOffset));
		bIsConsisant = FALSE;
	}
		...
		if (pBaseInfo->avgRSSI[i] != 0)
		{
				...
				((pBaseInfo->avgRSSI[i] - pCurLog->avgRSSI[i]) > pSAParam->rssiVar))
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF | DBG_FUNC_SA, 
							...
							__FUNCTION__, i, pBaseInfo->avgRSSI[i], 
...
//	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE | DBG_FUNC_SA, ("\tUpdate TrainInfo of Ant(0x%x-%d)=>\n", 
//				pTrainEntry->curAntPattern, pTrainEntry->patternOffset));
	...
	switch (pSAParam->candMethod)
	{
		...
		default:
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, 
						...
						__FUNCTION__, pSAParam->candMethod));
			break;
	}
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	if (!RTMP_SA_WORK_ON(pAd))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF | DBG_FUNC_SA,
					...
					__FUNCTION__, pAd->smartAntEnable, (ULONG)pAd->pSAParam));
	...
	if ((pMacEntry == NULL) || (pTrainEntry->pTrainInfo == NULL))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF | DBG_FUNC_SA,
					...
					__FUNCTION__, pMacEntry, pTrainEntry->pTrainInfo));
		RTMP_IRQ_UNLOCK(&pAd->smartAntLock, irqFlag);
		return FALSE;
	}
	...
	if (IS_ENTRY_CLIENT(pMacEntry))
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():ERR!!pCurTrainInfo(0x%p) and PatternOffset(0x%p-%d) not equal!\n",
						__FUNCTION__, pTrainEntry->pCurTrainInfo, tmpLogPtr, pTrainEntry->patternOffset));
			pTrainEntry->pCurTrainInfo = tmpLogPtr;
		}

		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():curAnt=0x%x-%d, TStage=%d,TWeight=%d!\n", 
					__FUNCTION__, pTrainEntry->curAntPattern, pTrainEntry->patternOffset,
		...
		if (pTrainEntry->bLastRnd == 1)
		{
			...
			*/
			...
			if (bConsistantEnv == FALSE)
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF | DBG_FUNC_SA, ("EndOfTraining:\n"));
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF | DBG_FUNC_SA, 
...
#error "Need to fix ConvertToRssi() before compile this feature"
				...
				else if (pTrainEntry->trainStage == SA_CONFIRM_STAGE)
				{
					...
					else
					{
						...
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE | DBG_FUNC_SA, ("\t\tChange from Confirm to Monitor stage:\n"));
						if (pCanLog)
						{	int i;
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	if (pSAParam->bStaChange || pSAParam->bRssiChange)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE | DBG_FUNC_SA, 
					...
					__FUNCTION__, pSAParam->bStaChange, 
		...
		NdisGetSystemUpTime(&nowTime);
		pTrainEntry->time_to_start = nowTime + pSAParam->trainDelay * OS_HZ;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():set time_to_start=0x%lx\n", 
					__FUNCTION__, pTrainEntry->time_to_start));
	...
	if (IS_ENTRY_CLIENT(pMacEntry))
	{
		...
		if (pSAParam->bStaChange)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s(): The SmartAnt StaChanged with antPattern(0x%x-%d), pCurLog(0x%p, 0x%x-%d)\n", 
					__FUNCTION__, pTrainEntry->curAntPattern, pTrainEntry->patternOffset,
					pTrainEntry->pCurTrainInfo, pTrainEntry->pCurTrainInfo->antPattern, 
					pTrainEntry->pCurTrainInfo->patternOffset));
		...
		if (pTrainEntry->trainStage != SA_INVALID_STAGE)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE | DBG_FUNC_SA, 
						...
						__FUNCTION__, nowTime, pMacEntry->wcid, saStage[pTrainEntry->trainStage], 
						pTrainEntry->curAntPattern, pTrainEntry->patternOffset,
			...
			)
		{
			...
			if (doTrain == TRUE)
			{
				...
				{
					BOOLEAN bGoFastTune;
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tIn confirm stage, check need to do fast tunning\n"));
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tbGoFastTune=%d!trainWeight=%d!\n", bGoFastTune, pTrainEntry->trainWeight));
				}
				if (pTrainEntry->trainStage == SA_INIT_STAGE)
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\ttrainStage is INIT stage, reset all trainWeight!\n"));
					// update trianing info for antenna patterns which has txCnt larger than average TxCnt
	...
	if (bNeedTune)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Call SA SwitchTimer!\n\n"));
		RTMPSetTimer(&pSAParam->saSwitchTimer, pSAParam->chkPeriod);
	}
	...
	IN UCHAR txNss)
{
	...
	idx3 = (pRxWI->RXWI_O.SNR2 >> 3);
	if (idx3 < 32)
		pTrainLog->SNRDist[2][idx3]++;
	else
		pTrainLog->SNRDist[2][32]++;
	
	// record the BF_SNR distribution
	if (pRxWI->RXWI_O.BF_SNR0 < 32)
		pTrainLog->preSNRDist[0][pRxWI->RXWI_O.BF_SNR0]++;
	else
		pTrainLog->preSNRDist[0][32]++;
	
	if (pRxWI->RXWI_O.BF_SNR1 < 32)
		pTrainLog->preSNRDist[1][pRxWI->RXWI_O.BF_SNR1]++;
	else
		pTrainLog->preSNRDist[1][32]++;

	if (pRxWI->RXWI_O.BF_SNR2 < 32)
		pTrainLog->preSNRDist[2][pRxWI->RXWI_O.BF_SNR2]++;
	else
		pTrainLog->preSNRDist[2][32]++;
...
#ifdef DOT11N_SS3_SUPPORT
	if (pRxWI->RXWI_O.SNR2)
	{
		pTrainLog->sumSNR[2] += (pRxWI->RXWI_O.SNR0 & 0xff);
		pTrainLog->cntSNR[2]++;
	}

	// PreSNR
	if (pRxWI->RXWI_O.BF_SNR0)
	{
		pTrainLog->sumPreSNR[0] += (pRxWI->RXWI_O.BF_SNR0 & 0xff);
		pTrainLog->cntPreSNR[0]++;
	}
	if (pRxWI->RXWI_O.BF_SNR1)
	{
		pTrainLog->sumPreSNR[1] += (pRxWI->RXWI_O.BF_SNR1 & 0xff);
		pTrainLog->cntPreSNR[1]++;
	}
	if (pRxWI->RXWI_O.BF_SNR2)
	{
		pTrainLog->sumPreSNR[2] += (pRxWI->RXWI_O.BF_SNR2 & 0xff);
		pTrainLog->cntPreSNR[2]++;
	}
#endif // DOT11N_SS3_SUPPORT //
	pTrainLog->rxCnt++;
...
INT sa_pkt_radio_info_update(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk, MAC_TABLE_ENTRY *pEntry)
{
	...
	else
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Err!Receive A frame with RSSI0=%d!TotalZeroCnt=%d!\n",
	...
	else
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Err!Receive A frame with RSSI1=%d!TotalZeroCnt=%d!\n",
					pRxBlk->rx_signal.raw_rssi[1], pEntry->rssi_zero_cnt[1]));
	}

#ifdef DOT11N_SS3_SUPPORT		
	if (pRxBlk->rx_signal.raw_rssi[2])
	{
		pEntry->curRSSI[2] += (pRxBlk->rx_signal.raw_rssi[2] & 0xff);
		pEntry->cntRSSI[2]++;
	}
#endif /* DOT11N_SS3_SUPPORT */
	...
	if (pAd->chipCap.hif_type == HIF_RTMP) {
		if (pRxBlk->rx_signal.raw_snr[2])
		{
			pEntry->sumSNR[2] += (pRxBlk->rx_signal.raw_snr[2] & 0xff);
			pEntry->cntSNR[2]++;
		}

		/* PreSNR */
		if (pRxBlk->pRxWI.RXWI_O.BF_SNR0)
		{
			pEntry->sumPreSNR[0] += (pRxBlk->pRxWI.RXWI_O.BF_SNR0 & 0xff);
			pEntry->cntPreSNR[0]++;
		}
		if (pRxBlk->pRxWI.RXWI_O.BF_SNR1)
		{
			pEntry->sumPreSNR[1] += (pRxBlk->pRxWI.RXWI_O.BF_SNR1 & 0xff);
			pEntry->cntPreSNR[1]++;
		}
		if (pRxBlk->pRxWI.RXWI_O.BF_SNR2)
		{
			pEntry->sumPreSNR[2] += (pRxBlk->pRxWI.RXWI_O.BF_SNR2 & 0xff);
			pEntry->cntPreSNR[2]++;
		}
	}
#endif /* DOT11N_SS3_SUPPORT */
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	for (macIdx = 1; macIdx < MAX_LEN_OF_MAC_TABLE; macIdx++)
	{
		if ((pEntry = &pAd->MacTab.Content[macIdx]) == NULL)
			continue;
...
int RtmpSAStart(RTMP_ADAPTER *pAd)
{
	...
	*/
	if (!RTMP_SA_WORK_ON(pAd))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():pAd->smartAntEnable=%d, pSAParam=0x%x\n", 
				__FUNCTION__, pAd->smartAntEnable, (UINT32)pSAParam));
		pAd->smartAntEnable = FALSE;
		RTMP_IRQ_UNLOCK(&pAd->smartAntLock, irqFlags);
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():AGSP(0x%x)/TrainSeq(0x%x) not configured, use default!\n",
				__FUNCTION__, (UINT32)pSAParam->agsp, (UINT32)pSAParam->pTrainSeq));

		sa_train_db_exit(pAd, pSAParam);
		if (pSAParam->agsp && (pSAParam->agsp != DefaultAGSP))
		{
			os_free_mem(pAd, pSAParam->agsp);
			pSAParam->agsp = NULL;
		}
	...
	if (pSAParam->pTrainMem == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Allocate memory for TrainLog failed!\n"));
		pAd->smartAntEnable = FALSE;
	...
	*/
	...
	{	
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():Entry[%d]: pTrainLog=0x%lx,Addr=%02x:%02x:%02x:%02x:%02x:%02x\n", 
				__FUNCTION__, i, (ULONG)pSAStaEntry->pTrainInfo, 
	...
	for (i = 0; i < SA_ENTRY_MAX_NUM; i++)
	{
		...
		{
			int j;
			// find first valid sta as the target
			for( j = 1; j < MAX_LEN_OF_MAC_TABLE; j++)
			{
		...
		if (is_valid_train_entry(pAd, pSAParam, pSAStaEntry))
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():Ready to do SA training for %02x:%02x:%02x:%02x:%02x:%02x!\n", 
					__FUNCTION__, PRINT_MAC(pSAStaEntry->macAddr)));
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s(): The SmartAnt start with antPattern(0x%x-%d), pCurLog(0x%p, 0x%x-%d)\n", 
					__FUNCTION__, pSAStaEntry->curAntPattern, pSAStaEntry->patternOffset,
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():set time_to_start=0x%lx\n", __FUNCTION__, pSAStaEntry->time_to_start));

			retVal = TRUE;
		}
		else
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():TrainEntry(%02x:%02x:%02x:%02x:%02x:%02x) is not valid\n", 
						__FUNCTION__, PRINT_MAC(pSAStaEntry->macAddr)));
			//retVal = FALSE;
			//pAd->smartAntEnable = FALSE;
		}
...
int RtmpSAInit(RTMP_ADAPTER *pAd)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Allocate SAParam memory(0x%p) with size(0x%x)\n", 
	...
	if (pAd->pSAParam)
	{
		...
		pSAParam->txNss = pAd->CommonCfg.TxStream; // SA_DEFAULT_TX_NSS
		pSAParam->saMsc = SA_DEFAULT_MSC;
		pSAParam->saMcsBound = (pAd->CommonCfg.TxStream * 8) - 1;	// SA_DEFAULT_MCS_BOUND
		...
		if (status == FALSE)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():DBInit failed!\n", __FUNCTION__));
		}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR | DBG_FUNC_SA, 
...
{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("iwpriv usage for SmartAntenna Related Sub Commands:\n"));

		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tiwpriv ra0 set sa\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\tshow this help message out\n"));
		
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tiwpriv ra0 set sa=0~1\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\t0: disable the SA mechanism\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\t1: enable the SA mechanism\n"));
		
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tiwpirv ra0 set sa_msc=0~255\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\tset the condition of stable MCS count for trigger the SA training algorithm\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\t\t0: run SA algorithm no matter MCS is in stable stage or not\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\t\t1<= x <=254: run SA algorithm when MCS stay in stable stage for x times\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\t\t255: reserved and should not use now\n"));

		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tiwpriv ra0 set sa_sta=xx:xx:xx:xx:xx:xx\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\tSet the target station used to support Smart Antenna\n"));
		
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tiwpriv ra0 set sa_agsp\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\t\n"));
		
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tiwpriv ra0 set sa_maxAntennaTry\n"));

		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tiwpriv ra0 set sa_dbg\n"));

		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tiwpriv ra0 set sa_tp\n"));

		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tiwpriv ra0 set sa_ant=T0-T1\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\tSet the static tx antenna as Antenna HeaderPin T0 and T1\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\t\t 1<= LowestPinBoundaryInAGSP <= T0, T1 <= HighestPinBoundayInAGSP <=255\n"));
...
INT	Set_DbgLogOn_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	RTDebugFunc = (Value & 0xff);

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE | DBG_FUNC_SA, 
				("%s():Set RTDebugFunc=0x%lx!\n", 
...
{
	pAd->pSAParam->maxAntTry = (UCHAR)simple_strtol(arg, 0, 10);
...
{
	pAd->pSAParam->chkPeriod = (int)simple_strtol(arg, 0, 10);
	return TRUE;
}
...
INT Show_SA_CfgInfo_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Dump the SmartAntenna Configuration Info:\n"));


	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tSAEnable=%s\n", pAd->smartAntEnable == TRUE ? "TRUE" : "FALSE"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tSADbg=%s\n", pAd->smartAntDbgOn == TRUE ? "ON" : "OFF"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\tpSAParam:\n"));
	if (pSAParam == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\tThe SMART_ANTENNA_STRUCT is not initialized!\n\n"));
		return 0;
	}

	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\tsaMode=%s\n",SA_MODE[pAd->pSAParam->saMode]));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\tTxNss=%d\n", pSAParam->txNss));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\tsaMcsStableCnt=%d\n", pSAParam->saMsc));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\tsaMcsBound=%d\n", pSAParam->saMcsBound));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\tmaxAntTry=%d\n", pSAParam->maxAntTry));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\tTrainCond=%d\n", pSAParam->trainCond));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\tcandMethod=%d\n", pSAParam->candMethod));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\ttrainDelay=%d\n", pSAParam->trainDelay));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\tTrainingPeriod=%d\n", pSAParam->chkPeriod));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\trssiVar=%d\n", pSAParam->rssiVar));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\tbSkipConfStage=%d\n", pSAParam->bSkipConfStage));	
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\tbStaChange=%d\n", pSAParam->bStaChange));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\tagspCnt=%d\n", pSAParam->agspCnt));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\ttrainSeqCnt=%d\n", pSAParam->trainSeqCnt));
	
	for (i = 0; i < SA_ENTRY_MAX_NUM; i++)
	{
		pStaEntry = &pSAParam->trainEntry[i];
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\tTrainEntry:\n"));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\t\tbStatic=%d\n", pStaEntry->bStatic));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\t\tRealTimeCntOfMcsInStableStage=%d\n", pStaEntry->mcsStableCnt));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\t\t\ttrainStage=%d\n", pStaEntry->trainStage));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\t\tbTraining=%s\n", pStaEntry->bTraining ? "TRUE" : "FALSE"));
		if (pStaEntry->curAntPattern == 0)
			sa_get_curAntPattern(pSAParam, pStaEntry);
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\t\tcurAntPattern=0x%x-%d\n", 
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\t\tMapped MacTableEntry=0x%x\n", (UINT32)pStaEntry->pMacEntry));
		if (pStaEntry->pMacEntry)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\t\t\tWCID=%d\n", pStaEntry->pMacEntry->wcid));
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\t\t\tBW:MCS=%d:%d\n", pStaEntry->pMacEntry->HTPhyMode.field.BW, 
											pStaEntry->pMacEntry->HTPhyMode.field.MCS));
		}
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\t\tAGSP:\n"));
		pAgspEntry = pSAParam->agsp;
		if (pAgspEntry == NULL)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\t\t\tThe AGSP is not initialized, dump default one!\n"));
			pAgspEntry = &DefaultAGSP[0];
		}
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\t\t\tHeaderPin  GpioRegOffset  GpioBit\n"));
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\t\tTrainingSequence(cnt=%d):\n", pSAParam->trainSeqCnt));
		pTrainingId = pSAParam->pTrainSeq;
		if (pTrainingId == NULL)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\t\t\tThe TrainingSequence is not initialized, dump default one!\n"));
			pTrainingId = &DefaultTrainSeq[0];
		}
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\t\t\t"));
		while(*pTrainingId != 0)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,(" 0x%x", *pTrainingId));
			pTrainingId++;
		}
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\n"));
	}
...
int Set_SA_TrainSeq_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *args)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF | DBG_FUNC_SA,("Input string is:%s\n", args));
	...
	else
	{
		...
		if (strlen(ptr))
		{
			entryCnt+=1; // add the last one
		}

		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF | DBG_FUNC_SA,("entryCnt=%d\n", entryCnt));
		if (entryCnt < 2)
			return FALSE;
...
{	
	...
	delayTime = simple_strtol(args, NULL, 10);
	pSAParam = pAd->pSAParam;
...
int Set_SA_AntCand_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *args)
{
	...
	candMethod = simple_strtol(args, NULL, 10);
	...
	if (pSAParam)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():Set AntennaCandidateSelectMethod=%d!\n",__FUNCTION__, pSAParam->candMethod));
		return TRUE;
	}
	else
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():SmartAntenna feature is not enabled yet\n", __FUNCTION__));
		return FALSE;
	}
}
...
int Set_SA_TrainCond_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *args)
{
	...
	trainCond = simple_strtol(args, NULL, 10);
...
INT Set_SA_RssiVariance_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *args)
{
	...
	rssiVar = simple_strtol(args, NULL, 10);
	pSAParam = pAd->pSAParam;
	if (pSAParam == NULL)
		return FALSE;
...
INT Set_SA_RssiThreshold_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *args)
{
	...
	rssiThold = simple_strtol(args, NULL, 10);
	pSAParam = pAd->pSAParam;
	if (pSAParam == NULL)
		return FALSE;
...
INT Set_SA_SkipConfirmStage_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *args)
{
	...
	bSkipConfStage = simple_strtol(args, NULL, 10);
	pSAParam = pAd->pSAParam;
	if (pSAParam == NULL)
		return FALSE;
...
int Set_SA_AGSP_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *args)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Input string is:\n"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("\t\t%s\n", args));
	...
	else
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("entryCnt=%d\n", entryCnt));
		if (entryCnt < 2)
			return FALSE;
		...
		*/
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Allocate Memory for AGSP failed\n"));
			return FALSE;
		}
		NdisZeroMemory(pAgspList, i);
		
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Allocate memory for AGSP success(addr=0x%lx, size=%d)\n", 
					(ULONG)pAgspList, i));
			...
			if (strlen(token))
			{
				...
				do
				{
					...
					if (strlen(subToken))
					{
						...
						{
							case 0:
								pAgspEntry->hdrPin = (UINT8)simple_strtol(subToken, 0, 10);
								break;
							case 1:
								A2Hex(pAgspEntry->regOffset, subToken);
								break;
							case 2:
								pAgspEntry->gpioBit = (UINT32)simple_strtol(subToken, 0, 10);
								break;
							default:
								break;
						}
				...
				if (i >= (entryCnt - 1))
				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Parsing whole AGSP list down, handle cnt=%d!\n", i));
					break;
				}
			}
		}while(pPos);
...
INT	Set_SA_StaticAntPair_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (pSAParam == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Smart Antenna is not initialized!\n"));
		return FALSE;
	}
	...
	do{
		...
		if (strlen(token) > 0)
		{
			pinVal[cnt] = (UCHAR)simple_strtol(token, 0, 10);
			cnt++;
		}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Error: AntPairSetting(%d) not equal txNss(%d)!!!\n", cnt, pSAParam->txNss));
		return FALSE;
	}
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():Error!! AGSP(0x%lx) or TrainSeq(0x%lx) is not set yet!",
				__FUNCTION__, (ULONG)pSAParam->agsp, (ULONG)pSAParam->pTrainSeq));
		return FALSE;
	}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("prevAntPattern=0x%x, newAntPattern=0x%x\n", 
			pTrainEntry->curAntPattern, antPattern));
	...
	if (pTrainEntry->curAntPattern)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Static AntPattern=0x%x, AGSP Offset=%d!\n", pTrainEntry->curAntPattern, pTrainEntry->patternOffset));
		return TRUE;
	}
	else
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Static Set AntPattern(0x%x) failed!\n", pTrainEntry->curAntPattern));
		return FALSE;
	}
}
...
INT	set_SA_txNss_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Error: Request txNss(%ld) out of range(1~%d) of hardware limitation\n", 
	...
	else
	{
		...
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Error: Smart Antenna not properly initialized!\n"));
		}
	}
...
INT	Set_SA_Mode_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (pSAParam)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():Disable SA before change the saMode!\n", __FUNCTION__));
		RtmpSAStop(pAd);
		pSAParam->saMode = Value;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():saMode=%s!\n",__FUNCTION__, SA_MODE[pSAParam->saMode]));
		return TRUE;
	}
	else
	{
...
INT Set_SA_Station_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():The SmartAntenna funciton is not enabled\n", __FUNCTION__));
		return FALSE;
	}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s():MacAddr=%02x:%02x:%02x:%02x:%02x:%02x\n", 
...
INT Set_SA_McsBound_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (pSAParam)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s(): saMCSBound=%d\n", __FUNCTION__, pSAParam->saMcsBound));
		return TRUE;
	}
	else
	{
...
INT	Set_McsStableCnt_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (pSAParam)
	{
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s(): saMsc=%d\n", __FUNCTION__, pSAParam->saMsc));
		return TRUE;
	}
	else
	{
...
INT	Set_SmartAnt_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Change the SmartAntenna feature as %s\n", 
				(pAd->smartAntEnable == TRUE ? "TRUE" : "FALSE")));
	...
	IN UCHAR antCnt)
{
	...
	for (i = 0; i < antCnt; i++)
	{
		if (pMacEntry->cntRSSI[i])
		{
			pMacEntry->curAvgRSSI[i] = ConvertToRssi(pAd, pMacEntry->curRSSI[i] / pMacEntry->cntRSSI[i], i);
		}
		else
		{
			pMacEntry->curAvgRSSI[i] = 0;
		}
		
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE | DBG_FUNC_SA, ("\tcurRSSI[%d]=%d,cntRSSI[%d]=%d, avgRssi[%d]=%d,ZeroRssiCnt=%d!\n", 
				i, pMacEntry->curRSSI[i], i, pMacEntry->cntRSSI[i], i, pMacEntry->curAvgRSSI[i],
				pMacEntry->rssi_zero_cnt[i]));
		
		pMacEntry->cntRSSI[i] = 0;
		pMacEntry->curRSSI[i] = 0;
		pMacEntry->rssi_zero_cnt[i] = 0;
	}
...
PUCHAR UCHAR rtmp_sa_get_nextMcs(UCHAR *pRateTb, UCHAR mcs)
{
	int tbIdx, tbSize, entryStep;
	RTMP_RA_LEGACY_TB *pCurrTxRate;
	...
	for (tbIdx = 0; tbIdx < tbSize; tbIdx++)
	{
		pCurrTxRate = (RTMP_RA_LEGACY_TB *) &pRateTb[(tbIdx+1)*5];
		if (pCurrTxRate->CurrMCS == mcs)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE | DBG_FUNC_SA, ("The Next MCS(%d) rate table colum is %d\n",
						mcs, pCurrTxRate->TrainUp));
			return pCurrTxRate->TrainUp;
		}
	}
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	if (RTMP_SA_WORK_ON(pAd))
	{
		SMART_ANTENNA_STRUCT *pSAParam;
		RTMP_SA_TRAINING_PARAM *pTrainEntry;
		RTMP_SA_TRAIN_LOG_ELEMENT tmpLog;
		TX_STA_CNT0_STRUC staTxCnt0;
		TX_STA_CNT1_STRUC staTxCnt1;
		INT macIdx;
		...
		pSAParam = pAd->pSAParam;
		pTrainEntry = &pSAParam->trainEntry[0];
		if ((pTrainEntry->bTraining == FALSE) && (pTrainEntry->trainStage == SA_INVALID_STAGE))
		{
			MAC_TABLE_ENTRY *pEntry;
			UINT32 ant;

			ant = sa_get_ant_by_gpio(pSAParam);
			printk("Antenna 0x%x\t\tSwTxCnt HwTxCnt HwRtyCnt SwRxCnt  HwPER(1/K)  SNR  RSSI  MCS\n", ant);
			printk("----------------------------------------------------------------------------\n");
			for (macIdx = 1; macIdx < MAX_LEN_OF_MAC_TABLE; macIdx++)
			{
				pEntry = &pAd->MacTab.Content[macIdx];
			...
			if ((pAd->chipCap.hif_type == HIF_RTMP) || (pAd->chipCap.hif_type == HIF_RLT)) {
				/* first read out the status counter from hardware */
				...
				/* dump the result out */
				sa_dump_stat_result(pAd, pEntry);
			...
			NicGetTxRawCounters(pAd, &staTxCnt0, &staTxCnt1);
			NdisZeroMemory(&tmpLog, sizeof(RTMP_SA_TRAIN_LOG_ELEMENT));
			sa_read_clean_train_result(pTrainEntry, &tmpLog);

			if(tmpLog.txCnt)
				tmpLog.PER = (tmpLog.txRtyCnt * 1000) / tmpLog.txCnt;
			else
				tmpLog.PER = 0xffffffff;

			printk("S-Ant Stats Result(LumpSum)\n");
			printk("\tLSTxCnt:%d, LSNoRtyCnt:%d, LSRtyCnt:%d, LSRtyFailCnt=%d, LSPER:%d(/1000)\n", 	
					tmpLog.txCnt, tmpLog.txNoRtyCnt, tmpLog.txRtyCnt, tmpLog.txRtyFailCnt, tmpLog.PER);
			printk("\n");
		}
	}
...
int Bits2Pin(UINT32 val, UCHAR *antPair)
{
	/* 
		maximum we can parsing 4 pins from the input, 
		e.g., for a input value, maximum can have 4 bits set as 1, others all shall set as zero
	*/
	int i, pinHdrCnt, pinHdrVal;
	//unsigned char pinId;

	
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Input val=0x%x!\n", val));

	i = 1;
	pinHdrCnt = pinHdrVal = 0;
	while(val)
	{
		if (val & 0x1)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Get a PinBit, val=0x%x, i=%d\n", val, i));
			pinHdrVal |= (i << (pinHdrCnt * 8));
			pinHdrCnt++;
		}
		
		if (pinHdrCnt == 3)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("Alreay get 4 bits from the input, ignore the rest val(0x%x)\n", val));
			break;
		}
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("For the Result: pinHdrVal=%d, pinHdrCnt=%d, restVal=0x%x!\n", pinHdrVal, pinHdrCnt, val));
